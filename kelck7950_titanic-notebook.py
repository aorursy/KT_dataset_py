...
# We are going to remove some columns, because they are irrelevant for analysis and prediction 
...
sns.countplot(x='Survived', hue="Embarked", data=titanic_data, order=[1,0], ax=axis2)
...
embark_dummies_titanic  = pd.get_dummies(titanic_data['Embarked'])
...
titanic_data = titanic_data.join(embark_dummies_titanic)
...
titanic_data['Fare'].plot(kind='hist', figsize=(15,3),bins=100, xlim=(0,50))
...
titanic_data['Age'].dropna().astype(int)
...
titanic_data["Age"][np.isnan(titanic_data["Age"])] = rand_1
...
titanic_data['Person'] = titanic_data[['Age','Sex']].apply(get_person,axis=1)
...
person_dummies_titanic  = pd.get_dummies(titanic_data['Person'])
...
titanic_data = titanic_data.join(person_dummies_titanic)
...
sns.factorplot('Person',data=titanic_data,kind='count',ax=axis1)
...
person_perc = titanic_data[["Person", "Survived"]].groupby(['Person'],as_index=False).mean()

sns.barplot(x='Person', y='Survived', data=person_perc, ax=axis2, order=['male','female','child'])



titanic_data.drop(['Person'],axis=1,inplace=True)
...
coeff_df = DataFrame(titanic_data.columns.delete(0))
