...
def model(input_shape):
    ...
    X = Conv2D(32, (7, 7), strides = (1, 1),padding = 'Same', activation ='relu',name = 'conv0')(X_input) 
    ...
    X = Flatten()(X)
    X = DropConnect(Dense(257, activation="relu"), prob=0.5)(X)
    #X = Dense(257, activation='relu', name='fc1')(X)
    X = BatchNormalization()(X)
   
    #Dense -> BN -> DropOut
    X = DropConnect(Dense(126, activation="relu"), prob=0.5)(X)
...
#history = model.fit_generator(datagen.flow(train_X, train_Y, batch_size=32), validation_data=(test_X, test_Y), steps_per_epoch=len(train_X)//32, epochs=10)
