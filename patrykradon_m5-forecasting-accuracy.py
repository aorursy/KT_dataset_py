...
days_to_forecast = ['F'+str(x) for x in range(1, 29)]
...
try:
    ...
    stationarity_differences = pd.read_csv('/kaggle/input/m5-precomputed/stationarity_differences.csv').iloc[:,0]
...
def eval_series_data_gen(in_points = 120, out_points=28, day_zero=1913, max_row=30490, day_step=1):    
...
model.load_weights('/kaggle/input/checkpoint6/croc_model6.ckpt')
