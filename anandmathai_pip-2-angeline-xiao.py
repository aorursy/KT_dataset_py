...
plt.figure(4)
plt.plot(time,position_y,'o', label='numerical position')
...
v = 20.0               # enter the correct initial velocity.
phi = 0.0              # enter the correct phase
...
v = 5.0               # enter the correct initial velocity.
phi = 0.0              # enter the correct phase
...
plt.legend()
plt.show()
x_error = np.abs(position_x - exact_position_x)
y_error = np.abs(position_y - exact_position_y)

plt.figure(17)
plt.plot(time,x_error,linewidth=1,label='x error',color='magenta')
plt.plot(time,y_error,linewidth=1,label='y error', color='purple')
plt.title(f"Absolute X and Y error")
plt.xlabel(f"time [s]") 
plt.ylabel(f"error [m]") 
plt.legend()
plt.show()

t1k_error = (position_x**2 + position_y**2)**(1/2) - (exact_position_y**2 + exact_position_x**2)**(1/2)


plt.figure(18)
plt.plot(time,t1k_error,linewidth=1,label='time step 1k error')
