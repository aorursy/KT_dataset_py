...
# pool2 = nn.MaxPool2d(kernel_size=3,stride=1)

# conv3 = make_layers(256,384,kernel_size=3,stride=1,padding=1) 

# pool3 = nn.MaxPool2d(kernel_size=3,stride=1)

# conv4 = make_layers(384,3755,kernel_size=3,stride=1,padding=1) 
# X = torch.rand(20, 1, 48, 48)
...
# ap = nn.AvgPool2d(kernel_size=5,stride=1)
...
class NinNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv = nn.Sequential(  
            ...
            make_layers(384,3755,kernel_size=3,stride=1,padding=1),
...
# images400 = images[labels<100]

# labels400 = labels[labels<100]
...
for i in tqdm(range(701, 761)):

    images = np.load(dir_path + "images" + "%03d"%(i) + ".npy", allow_pickle=True)

    labels = np.load(dir_path + "labels" + "%03d"%(i) + ".npy", allow_pickle=True)

    images_tst = np.append(images_tst, images)

    labels_tst = np.append(labels_tst, labels)

images_trn = []

labels_trn = []

numbers = [i for i in range(1, 661)]

random.shuffle(numbers)

for i in tqdm(range(1, 241)):

    images = np.load(dir_path + "images" + "%03d"%(numbers[i]) + ".npy", allow_pickle=True)

    labels = np.load(dir_path + "labels" + "%03d"%(numbers[i]) + ".npy", allow_pickle=True)

    images_trn = np.append(images, images_trn)

    labels_trn = np.append(labels, labels_trn)
class MyDataset(torch.utils.data.Dataset):

    def __init__(self, images, labels, std_size=48):
        ...
        self.transforms = tv.transforms.RandomChoice([    
    ...
    def __getitem__(self, index):
        ...
        img_scale = h/w * np.random.uniform(0.5, 2.5)

        if img_scale > 1:

            new_size = int(self.std_size/img_scale), self.std_size

        else:

            new_size = self.std_size, int(self.std_size * img_scale)

        img = self.transforms(img).resize(new_size)

        new_img = Image.new('L', (self.std_size, self.std_size))

        new_img.paste(img, ((self.std_size-new_size[0])//2, (self.std_size-new_size[1])//2))

        return self.to_tensor(new_img), self.labels[index]
...
def resize(img, size):
    ...
    return Image.fromarray(np.uint8(img),'L').resize(size)
import gc

import sys

images_trn = [Image.fromarray(img) for img in tqdm(images_trn)]

gc.collect()

images_tst = [Image.fromarray(img) for img in tqdm(images_tst)]
...
def cross_entropy_loss(inputs, targets):
    ...
    return (-torch.log(F.softmax(inputs, dim=1)+1e-8)*targets).sum()/inputs.shape[0]

def acc_fn(predicts, labels):

    return (predicts.argmax(1)==labels).float().mean()
...
train_dataset = MyDataset(images_trn, labels_trn)

train_iter = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

test_dataset = MyDataset(images_tst, labels_tst)
...
log_fmt = "Epoch %d Train Loss %f Train Accuracy %f Test Loss %f Test Accuracy %f with Time %f"
...
checkpoint = torch.load("../input/20000925/NiNPro74.pt")
...
for epoch in tqdm(range(1, num_epoches+1)):  
    ...
    if epoch % 1 == 0:
        ...
        images_trn = [Image.fromarray(img) for img in images_trn]
    ...
    for batch_images, batch_labels in train_iter:
...
#       onehot编码，正确的标签分配概率1-prob_err，错误的标签平分概率prob_err

#       加上prob_err的动机是考虑到人工标注可能存在的错误 

        Y = torch.full(out.shape, prob_err/(num_char-1), device=device).scatter(1, batch_labels.view(-1,1), 1-prob_err)
...
#         loss = loss_fn(out, batch_labels)
        ...
        acc = acc_fn(out, batch_labels)

        

        loss_sum += loss.item() * batch_labels.shape[0]

        acc_sum += acc.item() * batch_labels.shape[0]
    ...
    if epoch%1==0:
        ...
        loss_trn.append(loss_sum/n)

        acc_trn.append(acc_sum/n)
        ...
        with torch.no_grad():
            ...
            for batch_images, batch_labels in test_iter:
...
#                 loss = loss_fn(out, batch_labels)
            ...
            loss_tst.append(loss_sum/n)

            acc_tst.append(acc_sum/n)

        end_epoch = timeit.default_timer()

    time_used = end_epoch - start_epoch

    msg = log_fmt%(epoch + finished, loss_trn[-1], acc_trn[-1], loss_tst[-1], acc_tst[-1], time_used)

    print(msg)

    state = {'net':net.state_dict(), 'optimizer':optimizer.state_dict(), 'epoch':epoch + finished}

    torch.save(state, "./NiNPro" + str((epoch + finished + 1)//2*2) + ".pt")

    if time_used + end_epoch > 9*3600:
