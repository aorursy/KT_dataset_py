import pandas as pd
import seaborn as sns
from learntools.advanced_pandas.indexing_selecting_assigning import *

reviews = pd.read_csv("../input/wine-reviews/winemag-data-130k-v2.csv", index_col=0)
pd.set_option("display.max_rows", 5)
reviews.head()
check_q1(pd.DataFrame())
reviews["description"]  #or reviews.description
reviews.description[0]
reviews.iloc[0,:]
reviews.description.head(10)   #reviews.iloc[0:10, 1]
reviews.iloc[[1,2,3,5,8],:]  #reviews.iloc[[1, 2, 3, 5, 8]]
reviews.loc[[0,1,10,100],["country","province","region_1","region_2"]]
reviews.loc[:100,["country","variety"]]
reviews[reviews.country == "Italy"]
reviews[reviews.region_2.notnull()]
reviews.loc[:,"points"]   #reviews.points
reviews.loc[:1000,"points"]
reviews.iloc[-1000:, 3] #reviews.points.tail(1000)
reviews.loc[reviews.country == "Italy","points"] #reviews[reviews.country == "Italy"].points
reviews.loc[(reviews.country.isin(["Italy","France"])) & (reviews.points >=  90),"country"] 
#reviews[reviews.country.isin(["Italy", "France"]) & (reviews.points >= 90)].country