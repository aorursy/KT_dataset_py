...
data.columns
data.head()
# safe_loans =  1 => safe, 优质贷款
# safe_loans = -1 => risky, 不良贷款
#TODO
...
# safe_loan因为很多，所以抽样时按照 bad_one 占 safe_ones 的比例
safe_loans = safe_ones.sample(frac=percentage, random_state=33) #TODO safe_loan因为很多，所以抽样
...
class MyDecisionTree(BaseEstimator):
    ...
    def fit(self, X, Y, data_weights = None):
        ...
        target = Y.columns[0]
        self.root_node = self.create_tree(data_set, features, target, 
                                         current_depth=0, max_depth=self.max_depth, min_error=self.min_error) # TODO
...
#             return self.creat_leaf(target_values)  # TODO
...
print("model_3 training accuracy :", model_3.score(trainX, trainY)) # best on training set
print("model_1 testing accuracy :", model_1.score(testX, testY))  #underfitting
print("model_2 testing accuracy :", model_2.score(testX, testY))
print("model_3 testing accuracy :", model_3.score(testX, testY))  # overfitting
