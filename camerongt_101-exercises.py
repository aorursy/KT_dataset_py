...
def is_positive_even(number):
    ...
    if number >0 and not bool(number % 2):
...
def starts_and_ends_with_vowel(word):
        ...
        if word[-1] and word[0] in ['a','e','i','o','u']:
...
def mode(sequence):
    ...
    newDict = dict(Counter(sequence))

    answer=0

    

    for items in newDict.items():

        if items[1] == max(newDict.values()):
...
def get_highest_number(yourList):
...
def get_smallest_number(yourList):
...
def only_positive_numbers(lists):
...
def only_negative_numbers(lists):
...
def get_paper_title(paper):
    ...
    newList= list(paper.items())

    newList.sort(key=lambda var:var[0])
...
# Write a function called highest_priced_book that takes in the above defined list of dictionaries "books" and returns the dictionary containing the title, price, and author 

# of the book with the highest priced book.
...
def lowest_price_book(book):
    ...
    for item in book:
        ...
        newList.append(get_price(item))

    low = max(newList)

    for item in book:

        if get_price(item) < low:
...
def get_average_item_price(cart):
    ...
    return count / number_of_item_types(cart)
...
def get_average_spent_per_item(cart):
    ...
    return price / total_number_of_items(cart)
