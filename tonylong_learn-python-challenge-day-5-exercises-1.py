...
def estimate_average_slot_payout(n_runs):
    ...
    0.5
    """
    return sum([play_slot_machine() -1 for time in range(n_runs)])/n_runs
...
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    for _ in range(n_simulations):
        ...
        while balance >= 1 and play_times < n_spins:
            play_times +=  1
            balance = balance - 1 + play_slot_machine()
            
        simu_results.append(play_times >= n_spins)
        
    return sum(simu_results) / n_simulations
