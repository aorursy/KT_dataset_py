...
class CIFAR10Model(ImageClassificationBase):
    def __init__(self):
        super().__init__()
        #hidden layer
        self.linear1 = nn.Linear(input_size, 256)  #256 is the hidden size
...
history
history += fit(25, .02, model, train_loader, val_loader)
history += fit(30, 0.01, model, train_loader, val_loader)
history += fit(30, 1e-3, model, train_loader, val_loader)
history += fit(50, 1e-5, model, train_loader, val_loader)
plot_losses(history)
plot_accuracies(history)
evaluate(model, test_loader)
arch = "Four layers (256, 128, 32, 10)"
lrs = [.02, .01, 1e-3, 1e-5]
epochs = [25, 30, 30,50]
