...
class RNNModel(nn.Module):
    ...
    def forward(self,x):
        ...
        out,hn= self.rnn(x,h0) # hn shape layer_dim, batch_size, hidden_dim out, shape batch_size, seq_dim, hidden_dim
...
images= Variable(next(iter(train_loader))[0].float())

output = model(images)

summary(model, input_size=(input_dim, input_dim))
