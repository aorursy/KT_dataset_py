...
X_novotes = exclude(X, 'votes')
model2 = LinearModel(scale = True)
model2.fit(X_novotes, Y)
model2.coefficients(plot = True, top = 10)
model2.analyse(plot = True)
X_clean = exclude(X, ['votes','runtime_(minutes)','genre_(Drama)'])
model3 = LinearModel(scale = True)
model3.fit(X_clean, Y)
model3.analyse(plot = True)
# Your code goes here.
# hcat
# remove columns
# scale
