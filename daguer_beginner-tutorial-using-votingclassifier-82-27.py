...
#fill basic missing value for 'Embark' feature and convert it in to dummy variable
...
combined_train_test['Age'].groupby(by=combined_train_test['Pclass']).mean().sort_values(ascending=False)
...
combined_train_test['Sex'].groupby(by=combined_train_test['Sex']).count().sort_values(ascending= False)
combined_train_test.groupby(['Survived','Sex'])['Survived'].count()
#convert featuer variable sex into dumy variable
...
combined_train_test['Title'].groupby(by=combined_train_test['Title']).count().sort_values(ascending=False)
#create a dictionary to map the titles
...
combined_train_test['Name_Length'].groupby(by=combined_train_test['Name_Length']).count().sort_values(ascending=False)[:5]
#create name length category
...
combined_train_test.head()
combined_train_test['Name_Length_Category'].groupby(by=combined_train_test['Name_Length_Category']).count().sort_values(ascending=False)
#对不连续的数字或者文本进行编号
...
combined_train_test[['Name_Length_Category','Survived']].corr()
first_name_dummies_df=pd.get_dummies(combined_train_test['Name_Length_Category'],prefix=combined_train_test[['Name_Length_Category']].columns[0])
...
combined_train_test['First_Name'].groupby(by=combined_train_test['First_Name']).count().sort_values(ascending=False)[:5]
first_name_dummies_df=pd.get_dummies(combined_train_test['First_Name'],prefix=combined_train_test[['First_Name']].columns[0])
...
last_name_dummies_df=pd.get_dummies(combined_train_test['Last_Name'],prefix=combined_train_test[['Last_Name']].columns[0])
...
last_name_dummies_df=pd.get_dummies(combined_train_test['Original_Name'],prefix=combined_train_test[['Original_Name']].columns[0])
...
if(combined_train_test['Fare'].isnull().sum()!=0):
    ...
    combined_train_test.info()
#DIVIDE FATE for those sharing the same ticket
...
Pclass_1_mean_fare
#create a new plcass_fare_category variable from pclass and fare features
...
combined_train_test['Pclass'].head()
combined_train_test['Pclass'].replace([1,2,3],[Pclass_1_mean_fare,Pclass_2_mean_fare,Pclass_3_mean_fare],inplace=True)
#parch and sibsp
...
print(combined_train_test.groupby(['Survived','Family_Size'])['Survived'].count())
...
combined_train_test['Age'].groupby(by=combined_train_test['Title']).mean().sort_values(ascending=False)
#creater age_null columns to indicate nan values

combined_train_test['Age_Null']=combined_train_test['Age'].apply(lambda x:1 if(pd.notnull(x)) else 0)
combined_train_test['Age_Null'][:5]
...
def fill_missing_age(missing_age_train, missing_age_test):
    ...
    if __name__ == '__main__':
        ...
        gbm_reg_param_grid = {'n_estimators': list(range(400,700,50)), 'max_depth': [3,4,5], 'learning_rate':[0.01,0.02,0.03], 'max_features': [4]}
...
combined_train_test.loc[(combined_train_test.Age.isnull()),'Age']=fill_missing_age(missing_age_train,
                                                                                   ...
                                                                                   missing_age_test)
sum(n < 0 for n in combined_train_test.Age.values.flatten())
...
combined_train_test.Age.values
combined_train_test.Age.values.flatten()
...
age_cat_dummies_df=pd.get_dummies(combined_train_test['Age_Category'],prefix=combined_train_test[['Age_Category']].columns[0])

age_cat_dummies_df.head()
combined_train_test=pd.concat([combined_train_test,age_cat_dummies_df],axis=1)
#drop_col_not_req(combined_train_test,age_cat_dummies_df.columns.values)
combined_train_test['Ticket']
...
#drop columns that are not require
...
def get_top_n_features(titanic_train_data_X, titanic_train_data_y, top_n_features):
    ...
    rf_param_grid = {'n_estimators' : [570], 'min_samples_split':[2], 'max_depth':[25]}#30,20
    ...
    ada_param_grid = {'n_estimators' : [500,550,600], 'learning_rate': [0.6]}#500,600,0.5,0.6
    ...
    et_param_grid = {'n_estimators' : [450], 'min_samples_split':[5], 'max_depth':[30]}#
...
features_to_pick = 450

titanic_train_data_X=train_data_X

titanic_train_data_y=train_data_y

titanic_test_data_X=test_data_X
...
rf_est = ensemble.RandomForestClassifier(n_estimators = 570, criterion = 'gini', max_features = 'sqrt', max_depth = 6, min_samples_split = 2, min_samples_leaf = 2, n_jobs = 50, random_state = 42, verbose = 1)

gbm_est = ensemble.GradientBoostingClassifier(n_estimators = 600, learning_rate = 0.6, loss = 'exponential', min_samples_split = 3, min_samples_leaf = 2, max_features ='sqrt', max_depth = 10,  random_state = 42, verbose = 1)

#gbm_est = ensemble.GradientBoostingClassifier(n_estimators = 900, learning_rate = 0.0008, loss = 'exponential', min_samples_split = 3, min_samples_leaf = 2, max_features ='sqrt', max_depth = 10,  random_state = 42, verbose = 1)

et_est = ensemble.ExtraTreesClassifier(n_estimators = 450, max_features = 'sqrt', max_depth = 30,  n_jobs = 25, criterion = 'entropy', random_state = 42, verbose = 1)
...
#submission.to_csv("20170831submission.csv", index = False)
...
def run_kfold(clf):
    ...
    for train_index, test_index in kf:
        ...
        X_train, X_test = titanic_train_data_X.values[train_index], titanic_train_data_X.values[test_index]

        y_train, y_test = titanic_train_data_y.values[train_index], titanic_train_data_y.values[test_index]
...
rf_est = ensemble.RandomForestClassifier(n_estimators = 470, criterion = 'gini', max_features = 'sqrt', max_depth = 16, min_samples_split = 2, min_samples_leaf = 2, n_jobs = 50, random_state = 42, verbose = 1)

run_kfold(voting_est)