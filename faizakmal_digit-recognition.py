...
images = labeled_images.iloc[0:5000,1:] #[baris, kolom]
labels = labeled_images.iloc[0:5000,:1]
train_images, test_images,train_labels, test_labels = train_test_split(images, labels, test_size=0.2, random_state=0)
# now we gonna load the second image, reshape it as matrix than display it
i = 1
#print(train_labels.iloc)
img=train_images.iloc[i].values
img=img.reshape((28,28)) #ubah data tersebut dalam bentuk matriks dengan img.reshape(28,28) yang berisi 28 array dan 28 elemen
plt.imshow(img,cmap='gray') #ptl.imshow untuk menampilkan gambar grafik & cmap adalah warna dalam grafik yang terdaftar dalam matplotlib
...
a = np.unique(train_labels) #angka digit dari 0 - 9
print(a)
indeks = [65, 123, 3809, 98, 755, 41, 44, 190, 82, 21]
for i in indeks:
    img=train_images.iloc[i].values
    img=img.reshape((28,28))
    plt.figure()
    plt.imshow(img,cmap='gray')
    plt.title("Kelas ke - " + str(train_labels.iloc[i,0])) #untuk membuat judul data
#desk = train_images.iloc[i].describe()
#print(desk)
