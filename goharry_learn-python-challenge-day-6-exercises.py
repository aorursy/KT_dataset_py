...
def is_valid_zip(zip_code):
    ...
    """
    return str(zip_code).isdigit() and len(str(zip_code)) == 5
...
def word_search(doc_list, keyword):
    ...
    """
    tmp = []
    new_list = [x.lower() for x in doc_list]
    for doc in new_list:
        word_list = [words.strip(',.?') for words in doc.split()]
        if keyword.lower() in word_list:
            tmp.append(new_list.index(doc))
    return tmp
    """
    indices = []
