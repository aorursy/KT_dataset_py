...
fig.add_legend()
fig = sns.FacetGrid(df[df.sepal_length>5.5], hue="species", aspect=3) # aspect=3 permet d'allonger le graphique
fig.map(sns.kdeplot, "sepal_width", shade=True)
fig.add_legend()
fig = sns.FacetGrid(df[(df.sepal_length>5.5) & (df.sepal_width>3)], hue="species", aspect=3) # aspect=3 permet d'allonger le graphique
fig.map(sns.kdeplot, "petal_length", shade=True)
fig.add_legend()
fig = sns.FacetGrid(df[(df.sepal_length>5.5) & (df.sepal_width>3) & (df.petal_length>3)], hue="species", aspect=3) # aspect=3 permet d'allonger le graphique
...
dtc1 = tree.DecisionTreeClassifier(max_depth = 4, min_samples_leaf = 10)
dtc1.fit(X_train,y_train)
plt.figure(figsize=(30,30))
tree.plot_tree(dtc1, feature_names=X_train.columns, class_names=['setosa','versicolor','virginica'], fontsize=14, filled=True)
y_dtc1 = dtc1.predict(X_test)
print(accuracy_score(y_test, y_dtc1))
cm = confusion_matrix(y_test, y_dtc1)
print(cm)
pd.crosstab(y_test, y_dtc1, rownames=['Reel'], colnames=['Prediction'], margins=True)
from sklearn import ensemble
rf = ensemble.RandomForestClassifier()
rf.fit(X_train, y_train)
y_rf = rf.predict(X_test)
