...
poverty_level.poverty_rate.replace(['-'],0.0,inplace = True)#in order to change '-' to '0'
poverty_level.poverty_rate = poverty_level.poverty_rate.astype(float)#we should change datatype to float
area_list = list(poverty_level['Geographic Area'].unique())#list of geographic area
area_poverty_ratio = []

for i in area_list:
    a = poverty_level[poverty_level['Geographic Area']==i] #a : geographical area in the i index of the data frame
...
for i in area_list:
    a = percent_over_25_completed_highSchool[percent_over_25_completed_highSchool['Geographic Area'] == i]
    area_highschool_rate = a.percent_completed_hs.mean()
...
kill.race.dropna(inplace = True)#droping nan values
