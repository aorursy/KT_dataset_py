...
test.info()
test.isnull().sum()
msno.matrix(df=train.iloc[:, :], figsize=(6, 4), color=(0.3, 0.5, 0.2))
def bar_chart(feature):
    ...
    df.plot(kind = 'bar', stacked='True',color = ('r', 'orange','yellow','green','blue','navy','violet'),figsize=(10,5))
...
bins = [0, 6, 12, 17, 24, 37, 60, np.inf]
...
submission.to_csv('Team4 test.csv', index=False)
submission = pd.read_csv('Team4 test.csv')
