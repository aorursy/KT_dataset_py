...
result = query_covid("Range of incubation periods for the disease in humans (and how this varies across age and health status) and how long individuals are contagious, even after recovery.")
display(HTML(result.to_html()))
%%capture

result = query_covid("Prevalence of asymptomatic shedding and transmission (e.g., particularly children)")
display(HTML(result.to_html()))
%%capture

result = query_covid("Seasonality of transmission.")
display(HTML(result.to_html()))
%%capture

result = query_covid("Physical science of the coronavirus (e.g., charge distribution, adhesion to hydrophilic/phobic surfaces, environmental survival to inform decontamination efforts for affected areas and provide information about viral shedding).")
display(HTML(result.to_html()))
%%capture

result = query_covid("Persistence and stability on a multitude of substrates and sources (e.g., nasal discharge, sputum, urine, fecal matter, blood).")
display(HTML(result.to_html()))
%%capture

result = query_covid("Persistence of virus on surfaces of different materials (e,g., copper, stainless steel, plastic).")
display(HTML(result.to_html()))
%%capture

result = query_covid("Natural history of the virus and shedding of it from an infected person")
display(HTML(result.to_html()))
%%capture

result = query_covid("Implementation of diagnostics and products to improve clinical processes")
display(HTML(result.to_html()))
%%capture

result = query_covid("Disease models, including animal models for infection, disease and transmission")
display(HTML(result.to_html()))
%%capture

result = query_covid("Tools and studies to monitor phenotypic change and potential adaptation of the virus")
display(HTML(result.to_html()))
%%capture

result = query_covid("Immune response and immunity")
display(HTML(result.to_html()))
%%capture

result = query_covid("Effectiveness of movement control strategies to prevent secondary transmission in health care and community settings")
display(HTML(result.to_html()))
display(HTML(result.to_html()))
%%capture

result = query_covid("Role of the environment in transmission")
