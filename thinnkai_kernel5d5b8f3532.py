...
plt.legend(loc="upper right",title="Surived~Sibsp")
import matplotlib.pyplot as plt
...
plt.legend(loc="upper right",title="Surived~Parch")
import matplotlib.pyplot as plt
...
plt.legend(loc="upper right",title="Surived~Embarked")
...
train['Alone']=np.where((train["SitSp"]+train["Parch"])>0,0,1)

train.drop(['SitSp','Parch'], axis =1,inplace=True) 
train[['Pclass','Embarked','Sex']]
