...
def yenimetot(y_test,y_score):
    ...
    return roc_auc[3],fpr[3],tpr[3],thr[3]
rock_data=pd.read_csv("../input/emg-4/0.csv",header=None) #Rock gesture signals

scis_data=pd.read_csv("../input/emg-4/1.csv",header=None)  #Siccors gesture signals

paper_data=pd.read_csv("../input/emg-4/2.csv",header=None)  #Paper gesture signals

ok_data=pd.read_csv("../input/emg-4/3.csv",header=None)  #Ok gesture signals
...
plt.title('Classify gestures by Dataset ile Sınıflandırma',fontsize = 20,color='blue')
