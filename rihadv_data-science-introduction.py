...
# Scatter Plot usage 
# x = open, y = close
data.plot(kind='scatter', x='Volume_(BTC)', y='Volume_(Currency)',alpha = 0.5,color = 'yellow')
...
# We have to change column name of "Volume_(BTC)", because we need to remove parentheses to save our code from syntax error
data = data.rename(columns={'Volume_(BTC)': 'Volume_BTC'})  
data.Volume_BTC.plot(kind = 'hist',bins = 20,figsize = (8,8))
plt.show()
...
x = data['Weighted_Price']>19650     # Display prices greater than $19650
data[x]
