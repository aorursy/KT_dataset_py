...
CATEGORIES
#First We create the data for the training
X=[]
Y=[]
...
for train, test in kfold.split(X_train):
    ...
    result = model.fit(x=X_train[train],y=Y_train[train],batch_size=64,epochs=60,verbose=1,shuffle=False,initial_epoch=0, validation_split=0.2)
    result.history.keys()
    poltResuts(result)
    
#result = model.fit(x=X_train,y=Y_train,batch_size=64,epochs=60,verbose=1,shuffle=False,initial_epoch=0, validation_split=0.2)
#result.history.keys()
