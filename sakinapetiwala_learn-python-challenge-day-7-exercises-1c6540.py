...
def conditional_roulette_probs(history):
    ...
    for key in temp.keys():
        ...
        for j in range(len(history)-1):
            if history[j] == key:
                a.append(history[j+1])
        for element in a:
            temp[key][element] = a.count(element)/len(a)

    return temp

def my_agent(wheel):
    """ Make a wheel that has an Average Gain Per Simulation as a positive number
        https://www.kaggle.com/learn-forum/59121
    """
