...
class BaggedTreeClassifier(BaseEstimator):
    ...
    def score(self, X, Y):
        ...
        return np.mean(Y == P)
baggedtc = BaggedTreeClassifier(50)
cross_val_score(baggedtc, X, Y, cv=8).mean()
fakerf = FakeRandomForest(50)
...
print("linear regression performance: %f" % (cross_val_score(lr, trainX, trainY, cv=8).mean()))
print("random forest regressor performance: %f" % (cross_val_score(rfregressor, trainX, trainY, cv=8).mean()))
