...
#df_train2.info()
for pc in sorted(df_train['Pclass'].unique()):

    nulos = df_train[df_train['Pclass']==pc]['Cabin'].isnull().sum()

    conhecidos = len(df_train[df_train['Pclass']==pc]['Cabin']) - nulos

    print('Classe {}: {} nulos, {} conhecidos'.format(pc, nulos, conhecidos))
## Cria um novo dataframe (df_train2) sem a coluna Cabin
...
# plt.title('Idade')
for pc in sorted(df_train2['Pclass'].unique()):

    for sex in df_train2['Sex'].unique():

        loc = (df_train2['Pclass'] == pc) & (df_train2['Sex'] == sex)

        media = df_train2[loc]['Age'].mean()

        nulos = df_train2[loc]['Age'].isnull().sum()

        conhecidos = len(df_train2[loc]['Age']) - nulos

        print('Classe {}, gênero {} --> {:.1f}'.format(pc, sex, media))

        print('Idade conhecida: {}, desconhecida: {}'.format(conhecidos, nulos))

        df_train2.loc[loc & df_train2[loc]['Age'].isnull(), 'Age'] = media  # preenchimento
df_train2.info()
# consertando a idade que eu sei que está errada

df_train2[df_train2['Age'] == 133]
df_train2.loc[df_train2['Age'] == 133, 'Age'] = 33
df_train2['Age'].hist()
...
# Corrigindo para o valor correto

df_train.loc[df_train['Fare'] == df_train['Fare'].max(), 'Fare'] = 512.3292
df_train[df_train['Fare'] == df_train['Fare'].min()]
# Vou ignorar o sinal negativo

df_train.loc[df_train['Fare'] < 0, 'Fare'] = -1 * df_train.loc[df_train['Fare'] < 0, 'Fare']
df_train[df_train['Fare']>0].sort_values('Fare')[:5]
...
## recarregando os nossos dados já tratados com feature hashing

df_train = pd.read_csv('train_no_nulls_no_outliers_feat_hash.csv')

df_train.head(2)
novas_colunas_polinomiais = poly.fit_transform(df_train[['Fare', 'SibSp', 'Parch']])

novas_colunas_polinomiais = pd.DataFrame(novas_colunas_polinomiais, 

                                         columns=['p'+str(i) for i in range(novas_colunas_polinomiais.shape[1])]) 



# removendo a primeira coluna que sempre vem 1 como valor e as colunas p1, p2 e p3 que são as nossas colunas originais.

novas_colunas_polinomiais.drop(['p0', 'p1', 'p2', 'p3'], axis=1, inplace=True)



novas_colunas_polinomiais.head()
df_train2 = pd.concat([df_train,novas_colunas_polinomiais], axis=1)

df_train2.to_csv('train_no_nulls_no_outliers_feat_hash_poli.csv', index=False)
## recarregando nossos dados

df_train = pd.read_csv('train_no_nulls_no_outliers_feat_hash_poli.csv')
