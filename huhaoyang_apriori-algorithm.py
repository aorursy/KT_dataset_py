...
class Apriori:
        ...
        for i in range(len_Lksub1):
            for j in range(1, len_Lksub1):
                ...
                if l1[0:k-2] == l2[0:k-2]:
                    ...
                    temp = list_Lksub1[i] | list_Lksub1[j]
                    
                    flag = True
                    for item in temp:
                        temp_2 = temp - frozenset([item])
                        if temp_2 not in Lksub1:
                            flag = False
                            break
                    if flag:
                        Ck.add(temp)
            ...
            for freq_set in L[i]:
                for sub_set in sub_set_list:
                    if sub_set.issubset(freq_set):
                        ...
                        conf = self.calconf(sub_set,freq_set)
    ...
    def calconf(self,sub_set,freq_set):
        item_count_1 = 0
        item_count_2 = 0
        for item in self.transactions:
            if sub_set.issubset(item):
                item_count_1 += 1
        for item in self.transactions:
            if freq_set.issubset(item):
                item_count_2 += 1
        resultconf = item_count_2/float(item_count_1)
        return resultconf
