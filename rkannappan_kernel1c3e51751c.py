...
Y_onehot=tf.one_hot(indices=Y,depth=number_classes)
...
conv2_w=tf.get_variable("conv2_w",[3,3,64,128],initializer=tf.random_normal_initializer(stddev=1e-2))

conv2_b=tf.get_variable("conv2_b",[128],initializer=tf.random_normal_initializer(stddev=1e-2))
...
conv3_w=tf.get_variable("conv3_w",[5,5,128,256],initializer=tf.random_normal_initializer(stddev=1e-2))

conv3_b=tf.get_variable("conv3_b",[256],initializer=tf.random_normal_initializer(stddev=1e-2))
...
flatten = tf.reshape(relu3,[-1,8*8*256])
...
output = tf.layers.dense(inputs=fc2,units=number_classes,activation=None,use_bias=True)
...
for epoch in range(0,EPOCHS):
    ...
    for step in tqdm(range(int(len(x_train)/BATCH_SIZE)),desc=('Epoch'+str(epoch))):
        ...
        #taking in the next batch for training
        ...
        #only use first 1000 images from testing ds to make it simpler
