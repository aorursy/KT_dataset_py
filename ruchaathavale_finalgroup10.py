...
sam = pd.read_csv('/kaggle/input/liverpool-ion-switching/sample_submission.csv')
...
score = accuracy_score(sam.open_channels, preds)
print(score)

#df_test['open_channels']=np.round(np.clip(preds,0,10).astype(int))
#df_test[['time'.'open_chennels']].to_csv("submit.csv",index=False)
