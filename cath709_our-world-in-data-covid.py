...
#sp_capa_quot_fr_url = "https://www.data.gouv.fr/fr/datasets/r/dd0de5d9-b5a5-4503-930a-7b08dc0adc7c"
s = requests.get(sp_capa_quot_fr_url).content
capa_fr_ds = pd.read_csv(io.StringIO(s.decode('utf-8')), delimiter=';')
capa_fr_ds
capa_fr_ds.rename(columns={'P':'p'}, inplace=True)
capa_fr_ds.rename(columns={'T':'t'}, inplace=True)
...
incidence_fr_ds = pd.read_csv(io.StringIO(s.decode('utf-8')), delimiter=';')
incidence_fr_ds.rename(columns={'P':'p'}, inplace=True)
incidence_fr_ds.rename(columns={'T':'t'}, inplace=True)
...
newdf = owid_ds.loc[(owid_ds['location'] == "Ireland") & (owid_ds.date > "2020-03-28")].copy()
newdf.loc[:, ['date', 'new_tests', 'new_tests_smoothed', 'new_tests_smoothed_per_thousand', 'total_tests']].tail(18)

def setitests(row):
    if ((row.date >= '2020-06-10') & (row.date <= '2020-06-16')):
        row["new_tests_smoothed"] = 2679
        if ((row.date == '2020-06-16')):
            row.total_tests = 386572
    return row
newdf = newdf.apply(setitests, axis='columns')
newdf.loc[:, ['date', 'new_tests', 'new_tests_smoothed', 'new_tests_smoothed_per_thousand', 'total_tests']].tail(18)
...
def pop_base(row):
    ...
    row.case_trend = row.case_trend / (row.population / million) 
    
    return row

def setirelandtests(row):
    if ((row.date >= '2020-06-24') & (row.date <= '2020-06-30')):
        row["new_tests_smoothed"] = (24607 / 7)
        if ((row.date == '2020-06-30')):
            row.total_tests = 429698
    return row
...
def baselineSubplotCountry(fig, countryname, row, col):   
    ...
    if ((countryname == 'France') & (customFrance == True)):
        ...
        newdf['new_tests_per_thousand'] = newdf['new_tests'] / newdf['population'] * 1000
    ...
    if ((countryname == 'Ireland') & (customIreland == True)):
        newdf = newdf.apply(setirelandtests, axis='columns')
        newdf['new_tests_per_thousand'] = newdf['new_tests'] / newdf['population'] * 1000
        newdf['new_tests_smoothed_per_thousand'] = newdf['new_tests_smoothed'] / newdf['population'] * 1000
    ...
    newdf['case_trend'] = newdf['rolling_cases'].diff(2*rollingWindow)
    ...
    def mlabel(row):
        row.Label ="Total Cases: {:,} <br> Date: {}".format(int(row.total_cases), row.date)
        return row
    ...
    if (Ro_est == True):        
        fig.add_trace(go.Bar(x=xaxis, y=newdf['case_trend'], name=('case_trend ' + countryname),  hovertext=newdf["Label"],
                            ),
                      row, col, secondary_y=False)        
...
owid_ds.loc[(owid_ds['location'] == 'Spain') & (owid_ds.date > "2020-02-28") & (owid_ds.total_deaths.isnull())]
countrysToCheck = ['Ireland', 'France',  'United Kingdom', 'United States', 'Romania', 'Italy', 'Germany', 'Spain', 'Portugal', 'Australia']
counter = 0
fig = go.Figure()
...
fig = make_subplots(
    ...
    specs=[[{"secondary_y": True}, {"secondary_y": True}],
           ...
           [{"secondary_y": True}, {"secondary_y": True}],
           [{"secondary_y": True}, {"secondary_y": True}]],
    subplot_titles=("Ireland", "France", "UK", 'US', "Romania","Italy","Germany", "Spain"))
...
fig.update_layout(coloraxis=dict(colorscale='Bluered_r'), 
                  showlegend=False,
                  title_text="Number of New Tests per million people to date: " + chart_date)
...
fig.update_layout(coloraxis=dict(colorscale='Bluered_r'), 
                  showlegend=False,
                  title_text="Number of New Deaths per million people to date: " + max_date, height=700,)

fig.update_layout(plot_bgcolor='rgb(250, 242, 242)')
fig.show()
countrysToCheck = ['Ireland', 'France']
counter = 0
fig = go.Figure()
...
fig.update_layout(coloraxis=dict(colorscale='Bluered_r'), 
                  showlegend=False,
                  title_text="Number of New Cases per million people to date: " + max_date, height=700,)

fig.update_layout(plot_bgcolor='rgb(250, 242, 242)')
fig.update_yaxes(range=[0, 200])

fig.show()
countrysToCheck = ['Ireland', 'France',  'United Kingdom', 'United States', 'Netherlands', 'Italy', 'Germany', 'Spain', 'Portugal', 'Australia', 'Romania']
counter = 0
fig = go.Figure()
...
col = 1
for currCountry in countrysToCheck:
    currdfc = baselineSubplotCountry(fig, currCountry, 1, 1)
    #max_date = np.where(currdfc["short_date"] == currdfc["short_date"].max())[0]
    #currdfc = currdfc.iloc[max_date]
...
fig.update_layout(coloraxis=dict(colorscale='Bluered_r'), 
                  ...
                  height=700,
                  annotations=[dict(x="2020-07-03",y=25, xref="x", yref="y",text="UK revision",
...
fig.update_layout(plot_bgcolor='rgb(250, 242, 242)', height=800)
fig.show()
fcs.columns
max_date = np.where(fcs["date"] == fcs["date"].max())[0]
latestdfc = fcs.iloc[max_date]
latestdfc.loc[:, ['date', 'location', 'total_cases', 'population', 'stringency_index', 'new_tests_smoothed', 'new_cases', 'rolling_cases', 'rolling_deaths']]
countrysToCheck = ['Ireland', 'France',  'United Kingdom', 'United States', 'Romania', 'Italy', 'Germany', 'Spain', 'Portugal', 'Australia']
counter = 0
fig = go.Figure()
...
col = 1
for currCountry in countrysToCheck:
    df = fcs.loc[(fcs.location == currCountry) & (fcs.date > "2020-05-25")]
    #max_date = np.where(currdfc["short_date"] == currdfc["short_date"].max())[0]
    #currdfc = currdfc.iloc[max_date]
    
    fig.add_trace(go.Bar(x=df.date, y=df['case_trend'], name=('case_trend ' + currCountry),  hovertext=df["Label"],
                            ), row, col, secondary_y=False)        
...
fig.update_layout(coloraxis=dict(colorscale='Bluered_r'), 
                  showlegend=False,
                  title_text="Cases trend per million people", height=800)
...
fig.update_layout(
    ...
    yaxis={'title':'New Tests per milion people ',
        'type':'log'},
    title_text="COVID-19: Daily tests vs. Daily new confirmed cases per million - Colour: Deaths per million",
    height=700,
    showlegend=False,
)
...
# Ireland: Occupied Critical Care Beds: 263 (Including 21 COVID 19+ patients) 2000-18-june-2020
# Available Critical Care Beds: 94*
irelandICUCapacity = (263) / (263 + 94)
irelandICUCapacity # https://www.covidexitstrategy.org/definitions-and-criteria