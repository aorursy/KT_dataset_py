...
query = ("""
         SELECT Commits.repo_name AS repo, 
             COUNT(Commits.commit) AS commit_cnt
         FROM `bigquery-public-data.github_repos.sample_commits` as Commits
         INNER JOIN `bigquery-public-data.github_repos.sample_files` as Files             
         ...
         LIMIT 10
         """)
github.query_to_pandas_safe(query, max_gb_scanned=15)