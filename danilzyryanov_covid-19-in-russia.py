...
full_table = pd.read_csv('/kaggle/input/covid19-russia-regions-cases/covid19-russia-cases.csv')

region_info = pd.read_csv('/kaggle/input/covid19-russia-regions-cases/regions-info.csv')

full_table.sample(6)
region_info.rename(columns = {"Region": "Region/City"}, inplace = True)
...
full_grouped = full_table.groupby(['Date', 'Region/City'])['Confirmed', 'Deaths', 'Recovered', 'Active'].sum().reset_index()



# new cases ======================================================

temp = full_grouped.groupby(['Region/City', 'Date', ])['Confirmed', 'Deaths', 'Recovered']

temp = temp.sum().diff().reset_index()



mask = temp['Region/City'] != temp['Region/City'].shift(1)
...
temp.columns = ['Region/City', 'Date', 'Day-Confirmed', 'Day-Deaths', 'Day-Recovered']
...
full_grouped = pd.merge(full_grouped, temp, on=['Region/City', 'Date'])
...
day_wise['No. of regions'] = full_grouped[full_grouped['Confirmed']!=0].groupby('Date')['Region/City'].unique().apply(len).values
...
country_wise = country_wise.groupby('Region/City')['Confirmed', 'Deaths', 'Recovered', 'Active', 'Day-Confirmed'].sum().reset_index()
...
# pop.columns = ['Region/City', 'Population']



# merged data

# country_wise = pd.merge(country_wise, pop, on='Region/City', how='left')
...
# country_wise[country_wise['Population'].isna()]['Region/City'].tolist()
