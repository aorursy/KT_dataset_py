...
for k, (train_index, test_index) in enumerate(kf.split(df2)):
    ...
    model.compile(optimizer=Adam(lr=6e-6), loss=dice_coef_loss, metrics=["binary_accuracy", iou, dice_coef])
    ...
    results = model.evaluate(test_gen, steps=len(df3) / BATCH_SIZE)
