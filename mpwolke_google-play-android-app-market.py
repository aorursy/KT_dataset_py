...
df = pd.read_csv('../input/google-apps/The Android App Market on Google Play/datasets/user_reviews.csv', encoding='ISO-8859-2')

df.head()
df.isnull().sum()
# filling missing values with NA

df[['Translated_Review', 'Sentiment', 'Sentiment_Polarity', 'Sentiment_Subjectivity']] = df[['Translated_Review', 'Sentiment', 'Sentiment_Polarity', 'Sentiment_Subjectivity']].fillna('NA')
...
def clean_text(text):
    ...
    return text
df["Translated_Review"]=df["Translated_Review"].apply(str).apply(lambda x:clean_text(x))
...
Image(url = 'https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcR7-8eXwRntTU8vv80Fw8881fhPlrEnHc65ElVBk3glpLmhwQOc&usqp=CAU',width=400,height=400)