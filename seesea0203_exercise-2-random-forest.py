...
data.head()
data[0] = data.apply(lambda x:0 if x[0]=='e' else 1, axis=1)# TODO
# 每一列如果有null，用"missing"代替
cols = np.arange(1,23)
for col in cols:
    #TODO
    if np.any(data[col].isnull()):
        data.loc[data[col].isnull(), col] = 'missing' # Because the column_name and column_index are consistent here, we can either use data.loc[,] or data.iloc[,]
labelEncoders = {}#TODO
...
def transform(df):
    ...
    return X
# 准备数据和标签
X = transform(data.loc[:,1:])#TODO
Y = data[0].as_matrix()
logistic_model = LogisticRegression()#TODO
...
forest = RandomForestClassifier(n_estimators=10)#TODO
...
class FakeRandomForest(BaseEstimator):
    ...
    def fit(self, X, Y, n_features=None):
        ...
        for m in range(self.M):
            ...
            X_current, Y_current = X[idx], Y[idx]
...
baggedtc = BaggedTreeClassifier(10)
cross_val_score(baggedtc, X, Y, cv=8).mean()
fakerf = FakeRandomForest(10)
...
def transform_numerical(df):
    ...
    for col, scaler in scalers.items():
        # TODO
        result[:, i] = scaler.transform(df[col].values.astype(np.float64).reshape(1,-1)) # 在numpy中，一个列表虽然是横着表示的，但它是列向量。
        # (n, )为 1×n； ( ,n)为 n×1.
        i += 1
    return result    
...
rfregressor = RandomForestRegressor(n_estimators=20)# TODO
...
print("linear regression performance: %f" % (cross_val_score(lr,trainX, trainY).mean())) #TODO
print("random forest regressor performance: %f" % (cross_val_score(rfregressor, trainX, trainY).mean())) #TODO))
lr.fit(trainX, trainY)
print("linear regression test score: %f" % (lr.score(testX, testY)))# TODO))
rfregressor.fit(trainX, trainY)
