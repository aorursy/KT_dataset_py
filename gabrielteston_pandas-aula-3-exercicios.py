...
sum(fires_in_brazil[(fires_in_brazil["state"] == "Alagoas") & (fires_in_brazil["month"] == "Janeiro")]["number"])/len(fires_in_brazil[(fires_in_brazil["state"] == "Alagoas") & (fires_in_brazil["month"] == "Janeiro")]["number"])
len(fires_in_brazil["state"].unique())
fires_brazil_by_state = fires_in_brazil.groupby(by = ['state']).sum().reset_index()

fires_brazil_by_state
fires_in_brazil["state"].map(str.upper)
pd.DataFrame([[1,2],[3,4]]).apply(sum, axis="columns")
fires_brazil_by_state
fires_brazil_by_state["number"] =fires_brazil_by_state.apply(lambda row: row["number"]/sum(fires_brazil_by_state["number"]), axis=1)
fires_brazil_by_state
