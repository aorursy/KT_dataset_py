...
def clean_text(x, method = 'basic'):
    ...
    return x
files()
data = read('Tweets.csv')
head(data,7)
describe(data)
tallies = tally(data['airline_sentiment'], percent = True)
plot(data = tallies, x = 'airline_sentiment', y = 'tally', style = 'barplot')
tally(data['negativereason'])
time = hours(data['tweet_created'])
head(time, 10)
by_time = tabulate(time, data['airline_sentiment'], method = 'count_percent')
head(by_time, 4)
plot(data = by_time, x = 'tweet_created', y = 'Percent%', column = 'airline_sentiment', style = 'scatterplot')
by_airline = tabulate(data['airline'], data['airline_sentiment'], method = 'count_percent')
plot(data = by_airline, x = 'airline_sentiment', y = 'Percent%', column = 'airline')
text = data['text']
head(text, 10)
splitted = split(text, ' ')
...
data = exclude(data, 'airline_sentiment')
model = LinearModel(logistic = True, early_stopping = 20)
model.fit(tallied, y)
help(clean_text)
cleaned_text_basic = clean_text(text, method = 'basic')
head(cleaned_text_basic, 4)
tallied = tally(cleaned_text_basic, multiple = True, min_count = 10)
head(tallied, 5)
model = LinearModel(logistic = True, fast = True)
model.fit(tallied, y)
model = LinearModel(logistic = True, fast = True, scale = False)
model.fit(tallied, y)
tallied = tally(cleaned_text_basic, multiple = True, min_count = 10, method = 'tfidf')
head(tallied, 5)
model = LinearModel(logistic = True, fast = True, scale = False)
model.fit(tallied, y)
model.analyse(plot = True)
model.plot(model.predict(tallied), y)
quote = text[40]
print(quote)
print(y[40])
model.predict(row(tallied, 40))
sentiment = 'positive'
...
def basic_chatbot(line):
    ...
    if sentiment == 'positive':
        ...
        print('Oh. Sorry to hear that.')
basic_chatbot(10)
### CHANGE BELOW number (not 40 and 60 and 8000):
basic_chatbot(40)
basic_chatbot(60)
basic_chatbot(8000)