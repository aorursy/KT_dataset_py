...
def exactly_one_topping(ketchup, mustard, onion):
    ...
    b=int(mustard)
    c=int(onion)
    return (a and not (b or c)) or (not (a or b) and c) or (b and not (a or c))
    pass
...
def should_hit(player_total, dealer_total, player_aces):
    ...
    state, or False if the player should stay. player_aces is the number of aces the player has.
    """
    if player_total<dealer_total and not(player_aces==1):
        return True
    if player_total<=10:
        return True
    elif player_total==20 and player_aces==1:
        return False
    elif player_total>=20 and player_total<=21:
        return False
    else:
        return False
