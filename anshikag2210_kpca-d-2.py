...
#find covariance matrix covar for polynomial kernel where d=2
...
for i in range(0,1000):
    ...
    for j in range(0,1000):
        ...
        covar[i][j]=(x[i]*x[j]+y[i]*y[j]+1)*(x[i]*x[j]+y[i]*y[j]+1)
...
for i in range(0,1000):
    ...
    for j in range(0,1000):
        ...
        temp_res1+=(first_eval[j]*(x[i]*x[j]+y[i]*y[j]+1)*(x[i]*x[j]+y[i]*y[j]+1))

        temp_res2+=(second_eval[j]*(x[i]*x[j]+y[i]*y[j]+1)*(x[i]*x[j]+y[i]*y[j]+1))
