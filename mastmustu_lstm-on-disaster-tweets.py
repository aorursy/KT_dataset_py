...
with open(os.path.join('../input/glove6b50dtxt/glove.6B.%sd.txt' % EMBEDDING_DIM)) as f:
...
sentences = train["text_cleaned"].fillna("DUMMY_VALUE").values

possible_labels = ["fake", "real"]
...
test_sentences = test["text_cleaned"].fillna("DUMMY_VALUE").values
...
pred= r.model.predict(test_data)
# prepare text samples and their labels
...
submit_df['target'] = pred >= 0.5 

submit_df['target'] = submit_df['target'].astype(int)



submit_df.to_csv('LSTM_4.csv', index=False)
submit_df