...
%matplotlib inline
#creating data with the help of numpy
...
df.iloc[136,2] = np.NaN
sns.heatmap(df.isnull(), yticklabels=False, cmap='plasma');
import plotly.express as px
fig = px.bar(y=df1.speed,x=df1.index,title='Speed of animals',color_discrete_sequence=['#333333'])

fig.show()
df1.sort_values(by='lifespan',inplace=True)

fig = px.bar(y=df1.lifespan,x=df1.index,title='Life Span of animals',color_discrete_sequence=['#D63412'])

fig.show()
fig = px.bar(df1,y=['speed', 'lifespan'],x=df1.index,title='Speed of animals',color_discrete_sequence=['#333333','#D63412'])
