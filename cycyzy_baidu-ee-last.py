...
data2_path='../input/baiducompetition-final-model/'

test_data = load_test_data(data_path+'test1.json')

test2_data = load_test_data(data2_path+'test2.json')
...
#                 print(data)
            ...
            qa_start=[0]*len(token_ids)

            qa_end=[0]*len(token_ids)
...
#                 type_set.append(enent_type)
                ...
                for role,argument_and_index in arguments.items():

                        for argument_index,argument in argument_and_index:

                            start=None

                            argument_token_ids=self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(argument))

                            for index in range(len(mapping)):

                                if mapping[index][0]+split_off==argument_index:
                            ...
                            if start:
                                ...
                                end=min(start+len(argument_token_ids),len(token_ids))
...
#             cls_type
    ...
    def __getitem__(self,index):
        ...
        qa_start=torch.tensor(np.array(feature.qa_start).astype(np.float32))

        qa_end=torch.tensor(np.array(feature.qa_end).astype(np.float32))
...
train_loader,train_features=get_loader(train_data[:10],train_mode=True)
...
from tqdm import tqdm_notebook
# def get_token_ids_cls_feat(cls_list):
...
#     for cls_text in cls_list:

#         cls_token=tokenizer.tokenize(cls_text)[:cls_id_len-2]

#         cls_id=tokenizer.convert_tokens_to_ids(['[CLS]']+cls_token+['[SEP]'])

#         if len(cls_id)<cls_id_len:

#             cls_id+=[0]*(cls_id_len- len(cls_id))

#         token_ids_feat.append(cls_id)

#     token_ids_feat=np.array(token_ids_feat)

#     print(token_ids_feat.shape)

#     token_ids_feat=torch.tensor(token_ids_feat)

#     print(token_ids_feat.size())

#     return token_ids_feat

# cls_token_ids=get_token_ids_cls_feat(list(class_type_map_65.keys()))
# ##########4-24改进
...
#     def __init__(self,model_dim=512,num_heads=8,drop_out=0.0):

#         super(multiHeadAttention, self).__init__()



#         self.dim_per_head= model_dim//num_heads   #每一个attention的维度,num_heads是有多少头
...
#         self.dot_product_attention=ScaledDotProductAttention(drop_out)

#         self.linear_final=nn.Linear(model_dim,model_dim)



#         self.drou_out=nn.Dropout(drop_out)
...
#         dim_per_head=self.dim_per_head #每个头的维度

#         num_heads=self.num_heads #有多少个头

#         batch_size=key.size(0) #B是多少



#         #线性变换 [batch_size,seq,dim_per_head*num_heads]
...
#             #[num_heads*batch_size,seq_q ,seq_k]
...
class zy_Model(nn.Module):
    ...
    def __init__(self, n_classes=3,cls_token_ids=None):
...
#         self.qa_hidden_fc= nn.Sequential(nn.Linear(dim, 2))
        ...
        self.hidden_fc_crf2= nn.Sequential(nn.Linear(dim, 5),nn.ReLU(True))

        self.hidden_fc= nn.Sequential(nn.Linear(dim+5, num_labels),nn.ReLU(True))
        ...
        self.crf2=CRF(5,batch_first=True) #3

#         self.cls_token_ids=cls_token_ids

#         self.try_multi_attention=multiHeadAttention(dim,num_heads=16,drop_out=0.1)

#         self.feed_forward=PositionalWiseFeedForward(dim,dim*2)
...
#     def get_cls_features(self,token_ids):

#         attention_mask=(token_ids > 0)

#         last_seq,pooled_output,hidden_state=self.bert_model(input_ids=token_ids,attention_mask=attention_mask)
...
#         cls_bert_feat=self.get_cls_features(self.cls_token_ids)

#         cls_bert_feat=cls_bert_feat.repeat(ids.size()[0],1,1)

#         ##multi-head-attention 没有attention_mask

#         multi_feat,_=self.try_multi_attention(last_seq_feat,cls_bert_feat,cls_bert_feat)

#         multi_feat=self.feed_forward(multi_feat)

# #         print(multi_feat.size())
...
#         pooled_output=self.mask_mean(last_seq,attention_mask*1)
...
#         p_end=p_end.squeeze().sigmoid()

#         p_start=p_start.squeeze().sigmoid()
...
#         qa_feat=self.mask_mean(outs.detach(),attention_mask*1)
...
#         print(emissions.size())
        ...
        if not is_test:
            ...
            cls_loss=cls_loss/cls_label_65.size()[1]

            for i in range(cls_label_6.size()[1]):

                cls_6_loss+=criten(out_6[:,i]**2,cls_label_6[:,i])

            cls_6_loss=cls_6_loss/cls_label_6.size()[1]

#             for i in range(qa_start.size()[1]):

#                 qa_loss+=criten(p_start[:,i]**2,qa_start[:,i])

#                 qa_loss+=criten(p_end[:,i]**2,qa_end[:,i])

            qa_loss=qa_loss/(qa_start.size()[1]*2)
            ...
            return loss,cls_loss+cls_6_loss+c_number_loss+c_type_loss+loss_crf
...
#             print(de)
    ...
    for index in range(len(decodes)):
        ...
        for i,label in enumerate(decode[off:len(mapping)+off]):
            ...
            if label > 0 and label<=len(map_65_class_type)*2:
        ...
        results[item_id]=list(set([i[0] for i in word_results[item_id]]))
    ...
    if not is_test:
        ...
        cls_res=metric_fn_word(results,label_results,False)

        print('cls:',cls_res) #分类的结果

        print('crf2:',metric_fn_word(results_crf2,label_results_crf2)) #crf2的结果

        print('with crf2:',res_with2) #crf1 with crf2

        print('without crf2:',res) #crf1 without crf2

        return res

    else:

        return results,word_results
...
# losses=train_model(model,train_loader,val_loader,val_features,accumulation_steps=2,early_stop_epochs=2,epochs=20,model_save_path='recall_last.pkl')
# for l in losses:

#     print(l)
model=zy_Model(num_labels).cuda(DEVICE)

model_save_path='../input/shijianchouquqa/recall_last.pkl'
...
# with open('last_recall_val.pkl','wb') as f:

#     pickle.dump(val_word_result,f)
test_loader,test_features=get_loader(test2_data,batch_size=8,test_mode=True,train_val=False)
len(test_features)
test_features[0]
test_result,test_words_result=validation_fn(model,test_loader,test_features,is_test=True)
...
with open('last_recall_test2.pkl','wb') as f:
