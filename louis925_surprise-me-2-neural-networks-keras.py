...
model1 = ensemble.GradientBoostingRegressor(learning_rate=0.2, random_state=3,
                    n_estimators=180, subsample=0.77, max_depth=10)
...
model3 = XGBRegressor(learning_rate=0.15, random_state=3, n_estimators=300, subsample=0.78, 
...
model4.fit(nn_train[0], nn_train[1], epochs=65, verbose=0, batch_size=512, shuffle=True)
...
# del train; del data;
sub1['preds1'] = np.expm1(sub1['preds1']).clip(lower=0.)
sub1['preds3'] = np.expm1(sub1['preds3']).clip(lower=0.)
sub1['preds4'] = np.expm1(sub1['preds4']).clip(lower=0.)
sub1[['id','preds1']].to_csv('preds1.csv', index=False)
sub1[['id','preds3']].to_csv('preds3.csv', index=False)
sub1[['id','preds4']].to_csv('preds4.csv', index=False)
sub_true = pd.read_csv('../input/val6wk/sample_submission.csv')
assert (sub_true['id'] == sub1['id']).all()
def rmsle(y, y0):
...
rmsle1 = rmsle(sub1['preds1'], sub_true['visitors'])
rmsle3 = rmsle(sub1['preds3'], sub_true['visitors'])
rmsle4 = rmsle(sub1['preds4'], sub_true['visitors'])
print('Test: 1 %.6f 3 %.6f 4 %.6f' % (rmsle1, rmsle3, rmsle4))
# from hklee
...
sub_merge.head()
rmsle_merge = rmsle(sub_merge['visitors'].values, sub_true['visitors'].values)
print('Test Merge RMSLE: %.6f' % rmsle_merge)
