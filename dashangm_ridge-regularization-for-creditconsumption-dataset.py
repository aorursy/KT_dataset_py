...
testdt['cc_cons']= ridge.predict(testdt)
testdt['cc_cons'] = np.exp(testdt['cc_cons'])-1
testdt['cc_cons'].shape

testdt.shape
submissions_ridge = pd.concat([test_id_col, testdt['cc_cons']], axis=1) 
submissions_ridge.to_csv('submission_LinearReg_Ridge.csv', index=False) 
submissions_ridge