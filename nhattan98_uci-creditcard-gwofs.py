...
print(listdir('../input/ucicreditcard'))
def decode(datum):
    return np.argmax(datum)

df=pd.read_excel('../input/ucicreditcard/default of credit card clients.xlsx')
print(df.head())
df.boxplot()
...
df_scaled.boxplot()
# df = df_scaled
X = df_scaled.loc[:, df.columns != 'Outcome']
y = df_scaled.loc[:, 'Outcome']
...
#            print(Positions[i])
        ...
        print('Iteration',l,'--',Alpha_score,'Num feature:',np.sum(Alpha_pos))
...
plt.title('Training and Validation accurarcy')
plt.legend()
fig.savefig('Train&Val_acc.svg', format='svg', dpi=1200)
...
plt.legend()
plt.show()
fig2.savefig('Train&Val_loss.svg', format='svg', dpi=1200)
