...
countries_without_ppm.country
countries_without_ppm.to_csv("countries_without_ppm.csv")
query2 = """SELECT DISTINCT pollutant
            FROM `bigquery-public-data.openaq.global_air_quality`
            WHERE value = 0"""
pollutants_zero = open_aq.query_to_pandas_safe(query2)
pollutants_zero.pollutant
pollutants_zero.to_csv("pollutants_zero.csv")