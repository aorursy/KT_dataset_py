...
tel=pd.read_csv(r"../input/WA_Fn-UseC_-Telco-Customer-Churn.csv")
...
tel['SeniorCitizen']=tel['SeniorCitizen'].replace({1:"Yes",0:'No'})
def lab(tel):
    ...
    if tel['tenure']<=12:
        ...
        return 'Tenure_gt_60'
tel['tenure_group']=tel.apply(lambda tel:lab(tel),axis=1)
churn=tel[tel['Churn']=='Yes']

not_churn=tel[tel['Churn']=='No']
Id_col=['customerID']
...
lab=tel['Churn'].value_counts().keys().tolist()
val=tel['Churn'].value_counts().values.tolist()
...
py.iplot(fig2)
avg_tgc = tel.groupby(["tenure_group","Churn"])[["MonthlyCharges",
...
py.iplot(fig2)
tel_df=tel.copy()
tel=tel.drop('tenure_group',axis=1)
...
for i in bin_cols :
    ...
    tel[i] = le.fit_transform(tel[i])
tel=pd.get_dummies(data=tel, columns=multi_cols)
std = StandardScaler()
...
tel=tel.merge(scaled,left_index=True,right_index=True, how='left')
summary = (df_tel_og[[i for i in df_tel_og.columns if i not in Id_col]].
...
X = tel[[i for i in tel.columns if i not in Id_col + target_col]]

Y = tel[target_col + Id_col]
...
train,test=train_test_split(tel,test_size=.25, random_state=111)
