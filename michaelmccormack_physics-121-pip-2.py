...
period_osc = (2.0 * np.pi)/omega            # calculate the period of the oscillation (from the input variables)
...
steps_rev = 50000     # enter the number of time steps per revolution 
...
def der(yin, omega_f):                                              # no explicit time dependence

    return np.array([yin[2],yin[3],omega_f*yin[3],-omega_f*yin[2]])
...
def euler(yin,omega_f,step):

    #complete the function definition here

    return yin + step*der(yin,omega_f)
...
for ii in range(1,time_steps):
    ...
    position_x[ii] = yin[0]                        # save the new position components

    position_y[ii] = yin[1]

    

    #derivation of numerical results from exact ones: radius and
...
plt.title(f"Difference in radius of Analytical and Exact solutions")
