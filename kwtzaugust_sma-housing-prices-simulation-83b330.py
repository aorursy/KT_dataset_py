...
def utility_general_vectorised(house):
    ...
    LOC_COEF = 500 # UPDATE(weets, 191126)
...
def utility_function_vectorised(person, house):
    ...
    IDIO_COEF = 0.2 # UPDATE(weets, 191126)
...
def generate_person():
    ...
    person = {
        ...
        'preferred_location_x': 10*np.random.uniform(), # UPDATE(weets, 191125)

        'preferred_location_y': 10*np.random.uniform() # UPDATE(weets, 191125)
...
for x in range(10):
    ...
    for y in range(10):
        ...
        houses[(x,y)] = {
            ...
            'amenities': np.random.uniform(), # UPDATE(weets, 191125)
...
# experiment with empty houses

houses = houses.drop([(a,5) for a in range(5)])
...
def gen_asks():
    ...
    PROBA_SELL = 0.8 # arbitrary threshold; TODO: turn into adjustable param
...
def update_market_price():
    ...
    # build linear regression model for market_price
        ...
        houses['market_price'] = houses['amenities'].apply(lambda am: lm.predict(np.array(am).reshape(1,-1)).item())
