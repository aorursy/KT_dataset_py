...
# Your code goes here :)
# 1. How many stories are there of each type (in the "type" column) in the full table?
...
query2 = """SELECT deleted, type, COUNT(id)
            FROM `bigquery-public-data.hacker_news.full`
            GROUP BY type, deleted
            HAVING deleted is TRUE AND type = 'comment'
        """
...
query3 = """SELECT type, deleted, COUNTIF(id > 0 AND type = 'comment')
            FROM `bigquery-public-data.hacker_news.full`
            GROUP BY type, deleted
