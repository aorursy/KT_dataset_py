...
def diamond(height):
    ...
    l, r = '/', '\\'
    # Find the "radius" of the diamond (used in lots of calculations)
    rad = height // 2
    for row in range(height):
        # First time we pass the halfway mark, swap the left and right characters
        if row == rad:
            l, r = r, l
        if row < rad:
...
def conditional_roulette_probs(history):
    ...
    # We have counts, but still need to turn them into probabilities
    probs = {}
    for prev, nexts in counts.items():
        # Total spins that landed on prev (not counting the very last spin)
