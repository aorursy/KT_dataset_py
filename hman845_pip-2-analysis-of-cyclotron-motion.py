...
time_steps = 2000                      # calculate the number of time steps (from the input variables)
...
def euler(yin,omega,step):
    ...
    for i in range(len(yout)):
        ...
        estimate = yin[i] + (step * derivative[i])
...
time_steps = 6000                      # calculate the number of time steps (from the input variables)
...
plt.figure("analytical vs numerical solution")
...
plt.show()
difference = exact_position_x - position_x



plt.figure("quantify error")

plt.plot(time, difference)

plt.title(f"Difference between analytical solution vs numerical solution of x(t)")

plt.xlabel(f"time [s]") 

plt.ylabel(f"Difference in x position [m]") 
