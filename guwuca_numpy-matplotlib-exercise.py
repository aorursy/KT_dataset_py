...
cos_y = np.cos(x)
# TODO
plt.plot(x,sin_y, color='red', linewidth=3.0, label='Sin', linestyle='-.',marker='+')
plt.plot(x, cos_y, color='green', linewidth=1.5, label='Cos', linestyle='--')
plt.legend(loc=(0,1))
x = np.linspace(-np.pi, np.pi, 256)
sin_y = np.sin(x)
cos_y = np.cos(x)
...
plt.text(0,0.5, 'subplot(2,2,1)', ha='left',va='center',size=20, alpha=0.7)

plt.subplot(2,2,2)
plt.xticks([]), plt.yticks([])
plt.text(0.5,0.5, 'subplot(2,2,2)',ha='center',va='center',size=20,alpha=.3)
...
# TODO hist plot
y=np.random.normal(2,3.5, 1000)
plt.xlim([-14,16])
plt.xticks(np.linspace(-14,16,16))
plt.hist(y,30)
plt.show()
# TODO scatter plot
...
y=np.log10(x)
e=abs(np.random.normal(0,0.1, len(y)))
