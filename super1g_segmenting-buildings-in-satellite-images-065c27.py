...
import numpy as np
map_base_dir = '../input/synthetic-word-ocr/'
...
map_img_tn_dir = '../input/tndata/'
path = [os.path.join('../input/tundata/tntntnt/tntntnt', filename) for filename in os.listdir('../input/tundata/tntntnt/tntntnt')]
images = []
pic= []
for i in path:
    img = np.array(imread(i))
    images.append(img)
    
pic.append(img)
img1=(np.stack(images, 0)/255.0).astype(np.float32)
img5=(np.stack(pic, 0)/255.0).astype(np.float32)
print('x', img1.shape, img1.dtype, img1.min(), img1.max())
print('x', img5.shape, img5.dtype, img5.min(), img5.max())


predd = seg_model.predict(img5)
from PIL import Image
fig, (ax1, ax2) = plt.subplots(1, 2, figsize = (24, 24))
montage_rgb = lambda x: np.stack([montage2d(x[:, :, :, i]) for i in range(x.shape[3])], -1)
ax1.imshow(montage_rgb(img5))
ax2.imshow(montage2d(predd[:, :, :, 0]), cmap = 'bone_r')
p = montage2d(predd[:, :, :, 0])
print('x', p.shape, p.dtype)
ig = Image.fromarray(p, 'RGB')
ig
ax2.set_title('Prediction')
img2 = img1[0:4]
predd2 = seg_model.predict(img2)
fig, (ax1, ax2) = plt.subplots(1, 2, figsize = (48, 48))
montage_rgb = lambda x: np.stack([montage2d(x[:, :, :, i]) for i in range(x.shape[3])], -1)
ax1.imshow(montage_rgb(img2))
ax2.imshow(montage2d(predd2[:, :, :, 0]), cmap = 'bone_r')
ax2.set_title('Prediction')
img3 = img1[4:8]
predd3 = seg_model.predict(img3)
fig, (ax1, ax2) = plt.subplots(1, 2, figsize = (48, 48))
montage_rgb = lambda x: np.stack([montage2d(x[:, :, :, i]) for i in range(x.shape[3])], -1)
ax1.imshow(montage_rgb(img3))
ax2.imshow(montage2d(predd3[:, :, :, 0]), cmap = 'bone_r')
ax2.set_title('Prediction')
from matplotlib.pyplot import plot
img = Image.fromarray(montage2d(predd3[:, :, :, 0]))
print(img)
img.save("tmp.tiff")
print('XXX')
img2 = Image.open('tmp.tiff')
img2.show()
ig = Image.fromarray(p, 'RGB')
ig
