...
data[['bad_loans', 'safe_loans']].head(10)
...
print(trainX.shape, trainY.shape, type(testX), type(testY))
...
def best_split_entropy(data, features, target):
    ...
    for feature in features:
        ...
        p_left = len(left_split)/num_data_points
        p_right = 1 - p_left
        entropy_split =p_left * left_entropy + p_right * right_entropy #TODO
    ...
    def predict_single_data(self, tree, x, annotate = False):   
        ...
        if tree.is_leaf:
            ...
            if split_feature_value == 0:
                ...
                return self.predict_single_data(tree.left, x, annotate = annotate)# TODO
            else:
                #如果数据在该特征上的值为0，交给右子树来预测
                return self.predict_single_data(tree.right, x, annotate = annotate) # TODO    
...
m.score(testX, testY)
m.count_leaves()
model_1 = MyDecisionTree(max_depth = 3, min_error = 0) # TODO
model_2 = MyDecisionTree(max_depth = 7, min_error = 0)# TODO
model_3 = MyDecisionTree(max_depth =15, min_error = 0)# TODO
