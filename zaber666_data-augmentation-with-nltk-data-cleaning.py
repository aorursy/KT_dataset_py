...
def new_row(row, n_samples=1): 
    ...
    return new_rows
changed_rows = [new_row(row, n_samples=2) for _, row in train_df.iterrows()]

len(changed_rows)
changed_df = pd.concat(changed_rows, axis=0)
augmented_df = pd.concat([train_df, changed_df], ignore_index=True)