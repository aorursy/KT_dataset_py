...
data['class'] = data.apply(lambda row: -1 if row[0] == 'e' else 1, axis=1) # TODO
data.head()
def dummies(data, columns=['pclass','name_title','embarked', 'sex']):
    ...
    return data
target = 'class' # TODO
cols = data.columns.drop(target) # TODO
...
def best_split_weighted(data, features, target, data_weights):
    ...
    for feature in features:
        ...
        # 计算当前划分之后的分类犯错率
        error = (left_misses + right_misses) * 1.0 / sum(data_weights) # TODO
...
class WeightedDecisionTree(BaseEstimator):
    ...
    def fit(self, X, Y, data_weights = None):
        ...
        self.root_node = create_weighted_tree(data_set, data_weights, features, 
                               target, current_depth = 0, max_depth = self.max_depth, min_error=self.min_error) # TODO
...
class MyAdaboost(BaseEstimator):
    ...
    def fit(self, X, Y):
        ...
        for m in range(self.M):
            ...
            # 计算加权错误
            weighted_error = alpha.dot(prediction != Y[self.target]) # TODO
            
            # 计算当前模型的权重
            model_weight = 0.5 * (np.log(1 - weighted_error) - np.log(weighted_error)) # TODO
            
            # 更新数据的权重
            alpha = alpha * np.exp(-model_weight * Y[self.target] * prediction) # TODO
            # 数据权重normalize
...
class MySemiAdaboost(BaseEstimator):
    ...
    def score(self, testX, testY):
        result = self.predict(testX)
        return accuracy_score(testY[self.target], result) 
m2 = MySemiAdaboost(20)
m2.fit(trainX, trainY)
m2.score(testX, testY)
