...
df.head()
# To check all the messages are string or not
all(df['message'].apply(lambda x : type(x)==str))
...
bag_of_words_transformer = CountVectorizer(analyzer=all_words).fit(df["message"])
