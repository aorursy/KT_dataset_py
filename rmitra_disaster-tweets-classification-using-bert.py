...
# Remove leading and trailing whitespaces from the tweets before any data processing
...
def bert_encode(tweets, tokenizer, max_len=280): 
    # max_len : maximum length of tweets
    all_tokens = []
    all_masks = []
    all_segments = []
    
    for tweet in tweets:
        tweet = BertTokenizer.tokenize(tweet)
        
        tweet = tweet[:max_len-2]
        input_sequence = ['[CLS]'] + tweet + ['[SEP]']
        pad_len = max_len - len(input_sequence)
        
        tokens = BertTokenizer.convert_tokens_to_ids(input_sequence)
        tokens += [0] * pad_len
...
def build_model(bert_layer, max_len=280):
...
BertTokenizer = FullTokenizer(vocab_file, do_lower_case)
train_input = bert_encode(df_train['text_cleaned'].str.lower().values, BertTokenizer)
test_input = bert_encode(df_test['text_cleaned'].str.lower().values, BertTokenizer)
train_labels = df_train['target'].values
model = build_model(bert_layer)
model.summary()
%%time

train_history = model.fit(train_input, train_labels, validation_split=0.2, epochs=5, batch_size=16)
%%time

test_pred = model.predict(test_input)
submission = pd.read_csv('../input/nlp-getting-started/sample_submission.csv')
submission['target'] = test_pred.round().astype(int)
print('Submission Set Shape = {}'.format(submission.shape))
