...
#best_params, best_score, optimal_model = hyperparameter_tune(base_model, parameters, 50, 3, train_X, train_y)
...
for filename in os.listdir(dirname)[0:]: #withou lupus dataset
    ...
    for train_index, test_index in skf.split(X,y):
        ...
        best_params,result,optimal_model = hyperparameter_tune(clf, model_params, 50, 3, X_train, y_train)
        ...
        if len(y_test.unique()) == 2:
            ...
            accuracy, TPR, FPR, precision, roc_auc, PR_curve = binaryStat(clf, X_test, y_test, y_pred)
        else:
            accuracy, TPR, FPR, precision, roc_auc, PR_curve = multiClassStat(clf, X_test, y_test, y_pred)
        df_redults = df_redults.append({'dataset_name': filename , 'algorithm_name':'NU-SVC',
                                        'cross_validation': fold, 'hyper_params':best_params,
        ...
        print( best_params, accuracy, TPR, FPR, precision, roc_auc, PR_curve,
