...
row1=pd.DataFrame(df.dtypes).T.rename(index={0:'column type'})
row1=row1.append(pd.DataFrame(df.isnull().sum()).T.rename(index={0:'null values'}))
row1=row1.append(pd.DataFrame(df.isnull().sum()/df.shape[0]*100).T.
