...
end_time = start_time+N_loops*period_osc             # calculate the end time of the propagation (from the input variables)
...
#By changing the number of loops to 8, the accuracy of the program becomes significantly more accurate due to the averaging that can now occur to reduce any anomalies. 

#As shown visually through the graphs, there is a significant difference in the amount of data that can be analyzed. 

#Due to the increase in number of loops there is an increase in the number of peaks and troughs for each wave graph and more certainty to the circles in the circular graphs.

#The wider numerical values around the exact position in the comparison graph between the solutions displays the difference in accuracy of the analytical and numerical solutions.

#The error of the numerical solution increases with time.

#With the extra data, it can be said that the results are shown to be more consistent and thus, more reliable.
...
#In order to increase the accuracy of the numerical solution and decrease the error size, I have increased the number of time steps per revolution.

#To make the difference between the previous results more obvious, I have increased it to 7000 as opposed to the previous number of 1000.

#In this way, it is apparent by looking at the "comparison for analytical and numerical solution" graph that the numerical solution's errors have been significantly reduced due to this change in the initial parameters.
...
#vary the number of loops 1-8 with code.

#take the error values from the graphs.

#subtract error values from the exact position.

#plot against time using the array below and substituting said numbers for the error values.
...
plt.xlabel('$numerical error values$ ')
...
N_loops = 1         # enter number of loops the particle should complete 
