...
def twiddle(radius, tol=0.2): 
    ...
    p = [0.0, 0.0, 0.0]
    dp = [1.0, 1.0, 1.0]
    robot = make_robot(radius)
    x_trajectory, y_trajectory, best_err = run(p, radius)
    while (sum(dp) > tol):
        for i in range(len(p)):
            p[i] += dp[i]
            robot = make_robot(radius)
            x_trajectory, y_trajectory, err = run(p, radius)
            if (err < best_err):
                best_err = err
                dp[i] *= 1.1
            else:
