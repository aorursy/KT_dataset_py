...
full_set.shape
full_set.head()
train_X, test_X, train_y, test_y = train_test_split(full_set.iloc[:, :-1], full_set.iloc[:, -1], stratify=full_set.iloc[:, -1], random_state=7777)
...
scores = cross_validate(vote, train_X, train_y, cv=StratifiedKFold(n_splits=10, random_state=7777), scoring=scoring, return_train_score=True)
