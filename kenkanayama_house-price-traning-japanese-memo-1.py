...
for eta,max_depth in list(itertools.product(eta_list, max_depth_list)):
    ...
    dtrain = xgb.DMatrix(X_train[features], y_train) # トレーニングデータのXGBoost用のデータを生成
    dvalid = xgb.DMatrix(X_valid[features], y_valid) # 検証用データのXGBoost用のデータを生成
    ...
    print("Validating...")
    score = gbm.best_score # アーリーストッピングが発生した場合に使える「gbm.best_score」
