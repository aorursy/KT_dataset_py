...
class my_gbdt(object):
    ...
    def __init__(self):
        ...
        self.min_samples_split=8#每棵子树的节点的最小数目

        self.max_depth=4#每棵子树的最大层数

        self.num_of_trees=50#最大弱学习器（树）个数
    ...
    def LossFunc(self,y,t):
        ...
        loss=np.exp(loss) / np.expand_dims(np.sum(np.exp(loss), axis=1), axis=1)#由于是分类任务，所以采用了softmax

#         return((loss*loss)/loss.shape[1]/2)#手写一个平方损失函数，适用于回归
    ...
    def fit(self, X, y):
        ...
        for i in range(1,self.num_of_trees):
            ...
            gradient = self.LossFunc(y, y_pred)

            self.trees[i].fit(X, gradient)#对残差进行下一棵树的训练

            y_pred -= np.multiply(self.lr, self.trees[i].predict(X))
    ...
    def predict(self, X):
        ...
        for i in range(1, self.num_of_trees):
            ...
            pred_proba -= np.multiply(self.lr, self.trees[i].predict(X))

        pred_proba = np.exp(pred_proba) / np.expand_dims(np.sum(np.exp(pred_proba), axis=1), axis=1)

        pred_proba = np.argmax(pred_proba, axis=1)#分类任务，对于-1到1间的值进行处理，得到最后的预测结果
