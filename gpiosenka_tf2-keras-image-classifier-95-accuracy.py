...
import random as rd
def get_data_sets(source_dir, output_dir,mode,subject,image_size,t_split, v_split, rand_seed ):
    ...
    if mode !='all':
        ...
        d_path=os.path.join(source_dir, "train")
    ...
    if mode != 'all':    # read data from  trai, test and valid directories
        ...
        print_data(tr_data[1],t_data[1], v_data[1], class_list)
...
def read_files(dir_path, class_list, image_size,mode):
    ...
    for d in dir_list:   # these should be the class directories
        ...
        if os.path.isdir(d_path):  #only process directories ignore any files in dir
            ...
            partition=d_path.split('/')

            length=len(partition)

            desc= partition[length-2]+ '-' + partition[length-1] 

            desc='{0:30s}'.format(desc)
...
def display_pred(source_dir,output_dir,pred,t_files,t_labels,class_list,subject, model_size):
    ...
    if errors<=25:
        ...
        msg='{0}{1:^30s}{0:3s}{2:^20s}{0:3s}{3:20s}{0:3s}{4}'

        print(msg.format(' ', 'File Name', 'True Class', 'Predicted Class', 'Probability'))

        for i in range(0,errors):

            msg='{0}{1:^30s}{0:3s}{2:^20s}{0:3s}{3:20s}{0:5s}{4:^6.2f}'
...
def save_model(output_dir,subject, accuracy,r_model, image_size):
    ...
    print (' Saving final model to {0}',format(model_save_path))
...
def TF2_classify(source_dir,output_dir,mode,subject, t_split=10, v_split=5, epochs=20,batch_size=80,
    ...
    #data_sets=process_data(source_dir,output_dir, subject,image_size,t_split, v_split,rand_seed)
