...
def menu_is_boring(meals):
    ...
    """
    lenght = len(meals)
    return sum([ i > 0 and i < lenght and meals[i-1] == meals[i]  for i in range(len(meals))])
...
def estimate_average_slot_payout(n_runs):
    ...
    """
    tange = range(n_runs)
    return sum([play_slot_machine() -1 for i in tange])/n_runs 
        ...
        for spin in range(n_spins):
            if(balance >= 1):
               balance += play_slot_machine() - 1               
                ...
                success = False
        list_results.append(success)
#     print(list_results)
#     print(sum(list_results))
    return sum(list_results)/n_simulations
