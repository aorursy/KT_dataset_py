...
def train_model(model, criterion, optimizer, scheduler, no_epochs=25):
    ...
    return model
def radam(parameters, lr=lr, betas=(0.9, 0.999), eps=1e-8, weight_decay=0):
...
flags['no_epochs'] = no_epochs
#train_model(model, criterion, optimizer, scheduler, no_epochs=no_epochs)
def _mp_fn(rank, flags):
    global acc_list
    torch.set_default_tensor_type('torch.FloatTensor')
    a = train_model(model=model, criterion=criterion, optimizer=optimizer, scheduler=scheduler, no_epochs=no_epochs)
