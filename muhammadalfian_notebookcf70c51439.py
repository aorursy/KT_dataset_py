...
train=pd.read_csv('../input/stmm-challenge/train.csv')
train
train.describe()
test_data=pd.read_csv('../input/stmm-challenge/test.csv')
test_data
train.isnull().sum().sum()
# checking the different values contained in the diagnosis column

diagnosis = train['diagnosis'].value_counts()
...
sns.heatmap(train.corr(), cmap = 'pink', annot = True, linewidths = 0.5, fmt = '.1f')
...
train['diagnosis'] = train['diagnosis'].map(diagnosis_map)
train
#Feature Selection
train = train[['radius_worst','texture_worst','perimeter_worst','area_worst','smoothness_worst','compactness_worst','concavity_worst','concave points_worst','symmetry_worst','fractal_dimension_worst','diagnosis']]
...
#Mengkategorikan Umur (karena continuous, sulit dibuat decision tree)
#train_data['Age'] = pd.cut(train_data['Age'],3,labels = ['Muda','Dewasa','Tua'])
#test_data['Age'] = pd.cut(test_data['Age'],3,labels = ['Muda','Dewasa','Tua'])
#create model
from sklearn.ensemble import RandomForestClassifier
...
output.to_csv('my_submission_GG.csv', index=False)
print("Your submission was successfully saved!")
