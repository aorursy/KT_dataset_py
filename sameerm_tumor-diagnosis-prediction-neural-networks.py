...
#To read Breast Cancer Wisconsin (Diagnostic)data
DATA=pd.read_csv("../input/data.csv")
#Data Structure
DATA.head()
#To summaize the data
DATA.describe()
# To check the feature names as a list
...
#Assigning
d = {'M':1,'B':0}
#To replace M/B with 0 or 1 for the neural net classification
DATA = DATA.replace(d)
#To remove column 32 - unknown purpose
DATA = DATA.drop(['Unnamed: 32'],axis=1) 
#To remove the id - not needed for the neural net
DATA = DATA.drop(['id'],axis=1) 
#make a temporary dataset with only our feature vectors
DATA_temp = DATA.drop(['diagnosis'],axis=1)
#create our Numpy array of feature vectors to be used in our neural net
X = np.array(DATA_temp).T 
#create our Numpy array of diagnosis to be used in our neural net
...
#Normalizing our dataset by subtracting the mean and dividing by the max
X_normalized = (X-X_mean)/(X_max) 
#Seaborn Visualization 
Z = DATA. diagnosis
 # M = 212, B = 357
ax = sns.countplot(Z,label="Count")      
B, M = Z.value_counts()
...
#Note: we will use a learning rate of 0.1 for now
...
#To predict Malignant Tumor
dummy,m1 = X_train.shape
dummy,m2 = X_cv.shape

train_predict = predict(X_train,Y_train,1000,1.5,X_train,Y_train)
CV_predict = predict(X_cv,Y_cv,1000,1.5,X_train,Y_train)
count_true_pos = 0
count_train_pos = 0
