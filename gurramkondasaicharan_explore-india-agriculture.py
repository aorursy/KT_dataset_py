...
# k.head()['Crop             '].values
data[2].describe()
data[2].groupby('State').sum()
cols = data[2].columns
data[2].groupby('Crop')[cols[:-1]].sum().plot(kind='bar', figsize=(12,6));
data[2].groupby('State')[cols[:-1]].sum().plot(kind='bar', figsize=(12,6));
plt.title('State-wise '+cols[-1], color='red', fontsize=20)

data[2].groupby('State')[cols[-1]].sum().plot(kind='pie', figsize=(12,12));
plt.title('Crop-wise '+cols[-1], color='red', fontsize=20)

data[2].groupby('Crop')[cols[-1]].sum().plot(kind='pie', figsize=(12,12));
data[4].head()
