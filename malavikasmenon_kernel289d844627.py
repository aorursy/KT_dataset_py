...
%matplotlib inline
data_filepath="../input/MonthlyTemperature.csv"

tempData=pd.read_csv(data_filepath,index_col="Date",parse_dates=True)
tempData.shape
tempData.dropna(axis=0, how='all', thresh=None, subset=None, inplace=True)

tempData.shape
print(tempData.head(13))

print(tempData.tail(13))

plt.figure(figsize=(20,15))

sns.lineplot(x=tempData.index,y=tempData["Temp"])
tempData['month'] = tempData.index.day

tempData['year'] = tempData.index.year

pivot = pd.pivot_table(tempData, values='Temp', index='month', columns='year', aggfunc='mean')

print(tempData.describe())

print(tempData.tail(13))
year_avg = pd.pivot_table(tempData, values='Temp', index='year', aggfunc='mean')

year_avg['10 Years MA'] = year_avg['Temp'].rolling(10).mean()

year_avg[['Temp','10 Years MA']].plot(figsize=(20,6))

plt.title('Yearly Average Temperatures in Kochi')

plt.xlabel('Years')

plt.ylabel('Temperature')

plt.xticks([x for x in range(1901,2015,3)])

plt.show()
train = tempData[:-60].copy()

val = tempData[-60:-12].copy()

test = tempData[-12:].copy()
...
def check_stationarity(y, lags_plots=48, figsize=(22,8)):
    ...
    ax1.set_title('Kochi Temperature Variation')
