...
df_pgen1.head()
df_psense1.head()
# importing basic libraries for data visualation
...
#UNIVARIATE TIME SERIES FORECASTING USING FBPROPHET AND ARIMA
#Import Libraries
import pandas as pd
import numpy as np
...
# plotting DC_POWER vs DATE to have a general idea on how the data(dc_power) varies.
#The best part about time series data and decomposition is that you can break down the data into the following:
#Time Series Decomposition. 
from pylab import rcParams
import statsmodels.api as sm
...
# slicing the rows with first source key
df_temp

single_yield = df_pgen1.groupby('DATE').agg(TOTAL_YIELD = ('DAILY_YIELD', max),
                                         DATE = ('DATE',max)
                                        )
single_yield.head()
...
plt.title('daily yield prediction for the inverter with source key "1BY6WEcLGh8j5v7 "')
#predict DC_POWER 
