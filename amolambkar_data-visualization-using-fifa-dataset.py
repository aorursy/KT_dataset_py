...
plt.show()
sns.lineplot(x='Potential',y='Special',data=data)
data.plot.scatter(x='Preferred Foot',y='Potential')
   

data.plot.scatter(x='Special',y='Potential')
...
plt.show()
fig = sns.FacetGrid(data,hue='Preferred Foot',aspect=5)

fig.map(sns.kdeplot,'Wage',shade=True)
...
sns.factorplot(x='Skill Moves',data = data , kind = 'count', hue = 'Preferred Foot')
plt.figure(figsize = (15, 6))
