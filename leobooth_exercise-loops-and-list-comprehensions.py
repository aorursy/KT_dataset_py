...
def menu_is_boring(meals):
    ...
    """
    for day, meal in enumerate(meals):
        if day >= 1 and (meal == meals[day-1]):
            return True
...
def estimate_average_slot_payout(n_runs):
    ...
    for run in range(n_runs):
        payouts += play_slot_machine()
    
    return payouts/n_runs - 1
...
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    for _ in range(n_simulations):
        remaining_balance = start_balance
        spins = 0
        while remaining_balance >= 1 and spins < n_spins:
            remaining_balance -= 1
            remaining_balance += play_slot_machine()
            spins += 1
        if spins == n_spins:
            successes += 1
