...
volcanos_last_eruption = volcanos['Last Known Eruption']
#Remove unknown1: first replace unknown with nan
volcanos_last_eruption = volcanos_last_eruption.replace('Unknown', np.nan)
#Remove unknown2: drop the na
volcanos_last_eruption = volcanos_last_eruption.dropna()
volcanos_last_eruption
#Split the year and merge the BCE/CE data into it
volcanos_year = []

for last_eruption in volcanos_last_eruption:
    if last_eruption.endswith('BCE'):
        volcanos_year.append(0-int(last_eruption.split(' ')[0]))
    else:
        volcanos_year.append(int(last_eruption.split(' ')[0]))
#plot
