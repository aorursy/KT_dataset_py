...
import urllib
# Abre uma conexao com o site e lê o conteúdo (Aqui você terá o código inteiro da página que está lendo)

site = urllib.request.urlopen('https://www.comprasgovernamentais.gov.br/').read()
#Este site não deixa acessar sem que você responda algumas perguntas como localização...Dá um erro de Forbbiden 403

conexao2 = urllib.request.urlopen('https://desciclopedia.org/wiki/Categoria:Melhores_artigos')
#Na headers você vai colocando os dados da sua maquina e localizacao para que o site permita que vc acesse....
...
import urllib
#site='https://pt.wikipedia.org/wiki/Caneta'

#conexao=urllib.request.Request(site)

#conexao()

conexao2 = urllib.request.urlopen('https://pt.wikipedia.org/wiki/Caneta')
import requests
...
dfs[4]
# Concatena as tabelas (A ultima tabela não é inclusa)
...
import urllib
site='https://atendimento.tecnospeed.com.br/hc/pt-br/articles/360021494734-Tabela-de-C%C3%B3digo-de-UF-do-IBGE.html'
...
import pandas as pd
siape = pd.read_json('http://www.portaltransparencia.gov.br/api-de-dados/orgaos-siape?pagina=1')
...
# Traz os dados de Orgao SIAPE da página 1 até a página 6 # vai preencher a lista vazia. Coloca o 7 no range pq o intervalo final é aberto... 
...
# Enquanto não está fazio, traz a próxima URL
