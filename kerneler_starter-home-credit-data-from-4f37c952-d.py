...
# over30_test_sel_feats_from_fma.csv has 48744 rows in reality, but we are only loading/previewing the first 100 rows
df1 = pd.read_csv('../input/over30_test_sel_feats_from_fma.csv', delimiter=',', nrows = nRowsRead)
df1.dataframeName = 'over30_test_sel_feats_from_fma.csv'
nRow, nCol = df1.shape
print(f'There are {nRow} rows and {nCol} columns')
df1.head(5)
plotHistogram(df1, 10, 5)
plotCorrelationMatrix(df1, 84)
plotScatterMatrix(df1, 20, 10)
plotPCA(df1, 2) # 2D PCA
plotPCA(df1, 3) # 3D PCA
nRowsRead = 100 # specify 'None' if want to read whole file
# over30_train_sel_feats_from_fma.csv has 307511 rows in reality, but we are only loading/previewing the first 100 rows
df2 = pd.read_csv('../input/over30_train_sel_feats_from_fma.csv', delimiter=',', nrows = nRowsRead)
df2.dataframeName = 'over30_train_sel_feats_from_fma.csv'
nRow, nCol = df2.shape
print(f'There are {nRow} rows and {nCol} columns')
df2.head(5)
plotHistogram(df2, 10, 5)
plotCorrelationMatrix(df2, 84)
...
df3 = pd.read_csv('../input/over50_test_sel_feats_from_fma.csv', delimiter=',', nrows = nRowsRead)
df3.dataframeName = 'over50_test_sel_feats_from_fma.csv'
nRow, nCol = df3.shape
print(f'There are {nRow} rows and {nCol} columns')
df3.head(5)
plotHistogram(df3, 10, 5)
plotCorrelationMatrix(df3, 32)
