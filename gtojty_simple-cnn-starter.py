...
def decode_target(target, text_labels=False, threshold=0.5):
    ...
    for i, x in enumerate(target):
        if (x >= threshold):
            if text_labels: result.append(labels[i] + "(" + str(i) + ")")
...
predict_single(test_dataset[38][0])
...
sub_fname = 'Gtojty_submission.csv'
submission_df.to_csv(sub_fname, index=False)
!pip install jovian --upgrade
import jovian
jovian.commit(project='zerogans-protein-competition_new')