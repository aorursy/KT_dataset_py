...
msno.heatmap(test)
msno.bar(test, color = 'b', figsize = (10,8))
...
plt.gca().set_xlabel('Number Of classes')

# Prepare Data

df = train.groupby('target').size()
...
plt.title("Pie Chart of different types of Class")
...
train.head()
text = train[["text_cleaned","target"]]
data_lm = (TextList.from_df(text)
...
data_clas = (TextList.from_df(train, cols=["text_cleaned"], vocab=data_lm.vocab)
