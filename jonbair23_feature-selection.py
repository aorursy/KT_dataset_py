...
Xs = Xs.dropna(axis = 1)
#0 -> 5, 13,16,17,23,28->33,37,39,40,49,50,60,62,63,64,65,66,67,61 
...
reg.predict(xtestvals)
def rmse(p,targets): 
    return np.sqrt((p - targets)**2).mean() 
p = reg.predict(xtestvals)
...
rfe = RFE(estimator=reg, n_features_to_select=4, step=1,).fit(trainx,trainy)
p = rfe.predict(xtestvals)
error = rmse(p,ytestvals)
error
predictions = rfe.predict(test_vals)
output=pd.DataFrame({'Id':np.arange(1,2127),'OverallScore':predictions})
output.to_csv('out_featureselectioin.csv', index = False)
