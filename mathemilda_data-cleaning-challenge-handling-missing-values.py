...
### what values are missing and how much.
only_missing=[]
for i in range(0,43):
    if sf_missing_values_count[i]:
        only_missing.append(i)

print(len(only_missing))
sf_missing_values_count[only_missing]
...
# To replace with 0s we are to know what is a column type. 
sf_permits.iloc[:, only_missing].dtypes
subset_sf_data = sf_permits.loc[:, 'Unit'].head()
subset_sf_data
# At this point seems likely that if a unit number is missing then it is 0.
subset_sf_data.fillna(0)
subset_sf_data = sf_permits.loc[:, 'Issued Date' :  'Completed Date' ].head(10)
subset_sf_data
