...
# Initialize our Learning rate at 0.6 rather than an inital 0.4
learning_rate = 0.6
# Initalize our Discout factor for future rewards
gamma = 0.999

# fewer episodes as my computer doesn't can't handle greater number
episodes = 5000 
...
def choose_action(observ):
    return np.argmax(q_table[observ] + np.random.randn(1, env.action_space.n)*(1.0/(episodes+1)))
#     return np.argmax(q_table[observ])

# MDP to learn our envionrment
# changed episodes to 100
# changed steps to 200
# An episode is complete if one of the following conditions are met:
    # 1. we reached G 
    # 2. 200 iterations complete
    # 3. we fall into a hole


# intialize the rewards array
rewards = []
for i in range(episodes):
    # Reset the enviornment for each episode
    observ = env.reset()
    ...
    t = 0
    # run 200 time steps in each episode
    for t in range(200):
#     while not done:
        env.render()
        ...
        if not prev_observ is None:
            ...
            if done:
                q_new += learning_rate * (reward - q_old)
            else:
            # comput new Q-value : the value for the prvious state-action combo
                q_new += learning_rate * (reward + gamma + q_table[observ][action] - q_old)
        ...
        r += reward
#         print("-----HEY REWARD---- = {}".format(reward))
        rewards.append(r)
...
new_table
q_table
rtotal = str(sum(rewards)/episodes)
print("Score: {}".format(rtotal))