...
df = pd.read_csv('../input/newdatatrial/new_adm_pred-2-TRIAL.csv')
...
#  greenish
df['Nation'].value_counts().sort_values().plot(kind='barh', figsize=(8, 10), title = 'Nationality Distribution', color='#ffaf42', zorder=2, width=0.85)
#Research
df['Research'].value_counts().plot(kind='bar', color = '#66023c', title = 'Research Distribution')
plt.ylabel('No. of students')
plt.xlabel('Research Done')
#Social Work
df['Social Work'].value_counts().plot(kind='bar', color = '#86bf91', title = 'Social Work Distribution')
plt.ylabel('No. of students')
plt.xlabel('Social Work Done')
#Work Exp
df['Work Exp'].value_counts().plot(kind='bar', color = '#b22222', title = 'Work Exp Distribution')
plt.ylabel('No. of students')
plt.xlabel('Having Work Experience above 2 years')
...
print("\nSomething as simple as Linear Regression performs the best in this case, which proves that complicated models doesnt always mean better results.")
...
print(chance[0]*100)
print("By analyzing feature importance, we conclude that for this university, CGPA is the most factor for graduate admissions followed by GRE Score. \nThe other parameters like TOEFL, University Rating have less impact on the chance of admission.")
