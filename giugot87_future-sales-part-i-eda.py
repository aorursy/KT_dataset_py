...
df["date"] = pd.to_datetime(df["date"], format = "%d.%m.%Y")
x = df[["date", "item_cnt_day"]].set_index("date").resample("M").sum()#.plot()
x.head()
#df["date"] = pd.to_datetime(df["date"], format = "%d.%m.%Y")
#x = df[["date", "item_cnt_day"]].set_index("date").resample("M").sum().plot() # serie storiche resample in pandas-->scelgo il periodo che
x = df[["date", "item_cnt_day"]].set_index("date").resample("M").sum()#.plot()
df['Year'] = df['date'].dt.year
df['Month'] = df['date'].dt.month
# df['is_quarter_start'] = df['date'].dt.
df.head()
...
df_for_question_1.head()
df_resample_questions_1 = df_for_question_1.resample("D")['item_cnt_day'].sum().to_frame()
df_resample_questions_1['MA_sales'] = df_resample_questions_1['item_cnt_day'].rolling(window = 7).mean()
df_resample_questions_1.head(10)
df_resample_questions_1['MA_7_var'] = df_resample_questions_1['MA_sales'].diff()/df_resample_questions_1['MA_sales'].shift(1)
df_resample_questions_1.head(10)
df_resample_questions_1.dropna(inplace=True, axis='rows')
fig = plt.figure(figsize=(20,10))
ax = fig.add_subplot(111)
ax.plot(df_resample_questions_1['MA_sales'], label ='Moving Average')

ax2 = ax.twinx()
ax2.plot(df_resample_questions_1['MA_7_var'], label ='Moving Average 7 days', color='red')
...
df_for_question_2.head()
df_for_question_2.set_index('date', inplace=True)
df_for_question_2[df_for_question_2['city_id']==0]
name_city = df_for_question_2[df_for_question_2['city_id']==0]['city'][0]
name_city
df_for_question_2_city0 = df_for_question_2[df_for_question_2['city_id']==0]
df_resample_questions_2 = df_for_question_2_city0.resample("W")['item_cnt_day'].sum().to_frame()
df_resample_questions_2.head()
# df_timeindex = df.set_index("date").resample("M")["item_cnt_day"].sum().to_frame()
df_timeindex = df_resample_questions_2
...
result.observed.plot(ax = axes[0], legend = False)
axes[0].set_ylabel('Observed')
axes[0].set_title("Decomposition of a series for the city "+ name_city)
...
df_for_question_3[["item_cnt_day", "item_category_name"]]
df_for_question_3.groupby("item_category_name")["item_cnt_day"].sum().to_frame().shape
df_for_question_3.groupby("item_category_name")["item_cnt_day"].sum().to_frame().sort_values("item_cnt_day", ascending = False)
df_for_question_3["item_cnt_day"].sum()
...
# sort the values in the smaller df inplace
short_df = short_df.sort_values("item_cnt_day", ascending = False)
...
labels = ['{} \n Sales :{}k {}% of total'.format(item_category_name, sales/1000, round(pct, 2)*100) for item_category_name, sales, pct in zip(short_df.index, my_values, my_pct) if round(pct, 2)*100>1]
labels
# create a color palette, mapped to the previous values
cmap = matplotlib.cm.Blues
...
# Remove our axes, set a title and display the plot
plt.title("Sales items category and their % over total sales", fontsize = 23, fontweight = "bold")
plt.axis('off')
plt.tight_layout()
df.pivot_table(index = ["date"], columns = ['item_id'], values = "item_cnt_day", aggfunc = sum)
