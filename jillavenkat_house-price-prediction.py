...
#Family Sales probably should be excluded from analysis because they might not reflect real world sale price

Sale_Condition = train_data.groupby('SaleCondition')
Sale_Condition['SalePrice'].describe()
familysale = train_data[train_data['SaleCondition']== 'Family']
plt.scatter(familysale['GrLivArea'], familysale['SalePrice'], c = "blue", marker = "s")
plt.title("Looking for outliers")
plt.xlabel("GrLivArea")
plt.ylabel("SalePrice")
plt.show()
#Distribution is not linear like other categories so I am going to exclude them out

train_data = train_data[train_data['SaleCondition']!= 'Family']
...
Skewed_Feature_Check = ['LotFrontage', 'LotArea','MasVnrArea', 'BsmtUnfSF', 'TotalBsmtSF', 
...
# intiate and fit the final model

Lasso_model_final= Lasso(alpha=.003)
...
Final_labels = np.expm1(Lasso_model_final.predict(test_data))


## Saving prediction file to CSV - basic model with parameter tuning

pd.DataFrame({'Id': ID_test, 'SalePrice': Final_labels}).to_csv('Predictions.csv', index =False)   
...
rfecv = RFECV(estimator=Lasso_model, step=1, cv=KFold(10),
...
# Fit and transform the data based on the reduced number of features
...
Index
#build the new dataframes using those features
...
# retesting the models again using the same process but with reduced feature data set



print("Lasso Train")
print((np.sqrt(-cross_val_score(rfecv, X_train_new, y_train, scoring="neg_mean_squared_error", cv = 10)).mean()))
print()
print("Lasso Test")
print((np.sqrt(-cross_val_score(rfecv, X_test_new, y_test, scoring="neg_mean_squared_error", cv = 10)).mean()))
print()


Ridge_model_final = Ridge(alpha=5)

print("Ridge Train")
print((np.sqrt(-cross_val_score(Ridge_model_final, X_train_new, y_train, scoring="neg_mean_squared_error", cv = 10)).mean()))
print()
print("Ridge Test")
print((np.sqrt(-cross_val_score(Ridge_model_final, X_test, y_test, scoring="neg_mean_squared_error", cv = 10)).mean()))

#Check the residuals and predictions again

rfecv.fit(X_train_new, y_train)

y_train_pred =rfecv.predict(X_train_new)
y_test_pred = rfecv.predict(X_test_new)
...
rfecv.fit(Train_data_new, y)
Ridge_model_final.fit(Train_data_new, y)
Final_labels_new = np.expm1(rfecv.predict(Test_data_new))
Final_labels_Ridge = np.expm1(Ridge_model_final.predict(Test_data_new))
## Saving prediction file to CSV - basic model with parameter tuning

pd.DataFrame({'Id': ID_test, 'SalePrice': Final_labels_new}).to_csv('Lassorfecv.csv', index =False) 
