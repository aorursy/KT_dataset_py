# This Python 3 environment comes with many helpful analytics libraries installed

# It is defined by the kaggle/python Docker image: https://github.com/kaggle/docker-python

# For example, here's several helpful packages to load



import numpy as np # linear algebra

import pandas as pd # data processing, CSV file I/O (e.g. pd.read_csv)



# Input data files are available in the read-only "../input/" directory

# For example, running this (by clicking run or pressing Shift+Enter) will list all files under the input directory



import os

for dirname, _, filenames in os.walk('/kaggle/input'):

    for filename in filenames:

        print(os.path.join(dirname, filename))



# You can write up to 5GB to the current directory (/kaggle/working/) that gets preserved as output when you create a version using "Save & Run All" 

# You can also write temporary files to /kaggle/temp/, but they won't be saved outside of the current session
import tensorflow as tf

from tensorflow import keras

from tensorflow.keras import layers



from sklearn.preprocessing import LabelEncoder

from sklearn.model_selection import train_test_split

import matplotlib.pyplot as plt

from keras.layers import Dense, Dropout
train_data= pd.read_csv('../input/nicht-mnist/train.csv',

                       header=None,index_col=0)

test_data= pd.read_csv('../input/nicht-mnist/test.csv',

                      header=None,index_col=0)
train_data
test_data
y= train_data.pop(1)

x=train_data
x=x/255.0

test_data=test_data/255.0
x= x.values.reshape(-1,28,28,1)

test_data=test_data.values.reshape(-1,28,28,1)
y=y.values
label_encoder=LabelEncoder()

y=pd.DataFrame(label_encoder.fit_transform(y))
x_train, x_val, y_train, y_val=train_test_split(x,y,

                                                test_size=0.2,

                                               random_state=1)
myModel= keras.models.Sequential([

    layers.Conv2D(filters=32, kernel_size=3, padding='same',activation='relu', input_shape=(28,28,1)),

    layers.MaxPool2D(pool_size=2, strides=2),

    layers.Flatten(),

    layers.Dense(128,activation='relu'),

    layers.Dense(10,activation='softmax')

])
myModel.compile(optimizer="Adam",

               loss= keras.losses.SparseCategoricalCrossentropy(from_logits=True),

               metrics=['accuracy'])
history= myModel.fit(x_train,

                    y_train,

                    validation_data=(x_val,y_val),

                    epochs=20)
test_loss, test_acc=myModel.evaluate(x_val,y_val, verbose=1)

print('Test Accuracy: ', test_acc)
pro_model=keras.Sequential([myModel,

                           layers.Softmax()])
predictions = pro_model.predict(test_data)
results = np.argmax(predictions,axis = 1)

results = pd.Series(results,name="target")

submission = pd.concat([pd.Series(range(0,9364),name = "Id"),results],axis = 1)

submission['target'] = label_encoder.inverse_transform(submission['target'])

submission.head()

submission.to_csv('sub_nicht_cnn.csv', index=False)