...
class Feature(object):
    ...
    def __init__(self,item_id,or_text,mapping,mapping_off,token_ids,labels,entype_trigger_indexs,cls_label_65,cls_label_6,cls_label_number,cls_type,slip_off,crf_trigger,qa_start,qa_end,sig_ids,ans_label):
    ...
    def __repr__(self):
        ...
        s += "ans_label: %s" % (str(self.ans_label))
...
#         print('*'*50)
        ...
        for data in tqdm(split_data_list):
            ...
            sig_ids=[0]*len(token_ids)

#             for trigg in all_trigger:

#                 if text.find(trigg)!=-1:

#                     trigg_id=self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(trigg))

#                     starts=search_list(trigg_id, token_ids)

#                     for start in starts:

#                         sig_ids[start:start+len(trigg_id)]=[1]*len(trigg_id)
...
#                 type_set.append(enent_type)
                ...
                for a in ans:
                    ...
                    a_token_ids=self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(an[1]))
                    ...
                    for index in range(len(mapping)):
                        ...
                        if mapping[index][0]+split_off==an_index:

                            if not text[mapping[index][0]:].startswith(an[1]):

                                print(text)

                                print(text[mapping[index][0]:])
                            ...
                            start_index=index+1 #因为有cls

                            break

                    if start_index:

                        ans_label[start_index]=1

                        for i in range(1,len(a_token_ids)):

                            ans_label[start_index+i]=2
...
#             cls_type
            ...
            feature=Feature(item_id=item_id,
                           ...
                           qa_end=qa_end,sig_ids=sig_ids,ans_label=ans_label
    ...
    def __getitem__(self,index):
        ...
        seg_ids=torch.tensor(feature.sig_ids)
        ...
        ans_labels=torch.tensor(np.array(feature.ans_label).astype(np.float32)).long()

        return token_ids,seg_ids,labels,cls_label_65,cls_label_6,cls_label_number,cls_label_type,crf_trigger,qa_start,qa_end,ans_labels
    ...
    def collate_fn(self, batch):
        ...
        qa_end=torch.stack([x[9] for x in batch])

        ans_labels=torch.stack([x[10] for x in batch])
...
def debug_loader(train_data):
    ...
    for token_ids, seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger,qa_start,qa_end,ans_labels in train_loader:
...
m
val_loader,val_features=get_loader(valid_data,train_mode=True)
...
class zy_Model(nn.Module):
    ...
    def __init__(self, n_classes=3):
        ...
        self.qa_hidden_fc= nn.Sequential(nn.Linear(dim, 2)) #qa_feat

        self.hidden_fc_2= nn.Sequential(nn.Linear(dim+2, 3),nn.ReLU(True)) #3分类

#         self.hidden_fc= nn.Sequential(nn.Linear(dim+3+2, num_labels),nn.ReLU(True))

#         self.hidden_fc_ans= nn.Sequential(nn.Linear(dim, 3),nn.ReLU(True))
...
#         self.zy_cls_6_hidden= nn.Sequential(nn.Linear(dim+num_labels, len(class_type_map)),nn.ReLU(True),nn.Linear(len(class_type_map), len(class_type_map)))
        ...
        self.zy_cls_number_hidden= nn.Sequential(nn.Linear(dim+3, 3),nn.ReLU(True),nn.Linear(3, 3))
...
#         self.crf2_ans=CRF(3,batch_first=True)

#         self.weights=nn.ModuleList([nn.Linear(dim,1) for _ in range(24)])
    ...
    def get_weight_seq(self,x):

        layer_logits = []

        for i,logits in enumerate(self.weights):

            layer_logits.append(logits(x[i]))

        layer_logits=torch.cat(layer_logits,2)  #[2, 160, 24]

        layer_dist=F.softmax(layer_logits,dim=2)

        seq_out = torch.cat([torch.unsqueeze(xi,2) for xi in x],2) #[2, 160, 24, 1024]
    ...
    def forward(self,ids,seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger,qa_start,qa_end,ans_label,is_test=False):

        attention_mask = (ids > 0)

        last_seq,_,hidden_state=self.bert_model(input_ids=ids,token_type_ids=seg_ids,attention_mask=attention_mask)

        

#         last_seq=self.get_weight_seq(hidden_state[1:])
...
#         emissions_ans=self.hidden_fc_ans(last_seq)

        

        emissions2=self.hidden_fc_2(torch.cat((last_seq,outs.detach()),2)) #3分类

#         emissions=self.hidden_fc(torch.cat((last_seq,emissions2.detach(),outs.detach()),2)) #多分类
        ...
        crf_feat=self.mask_mean(emissions2.detach(),attention_mask*1)

        #65

#         cls_hid=torch.cat([type_feat,crf_feat],1)

#         out_65=self.zy_cls_65_hidden(cls_hid).sigmoid()

#         #6

#         cls_hid_6=torch.cat([type_feat,crf_feat],1)
...
#         number_feat=self.mask_mean(last_seq,attention_mask*1)
        ...
        cls_hid_number=torch.cat([type_feat,crf_feat],1)
...
#             emissions_ans_loss=-self.crf2_ans(emissions_ans, ans_label, mask=attention_mask,reduction='mean')

            c_qaloss=(c_number(p_start,qa_start)+c_number(p_end,qa_end))/2

            return loss_crf,c_qaloss+c_number_loss
...
def debug_label():
    ...
    for token_ids, seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger,qa_start,qa_end,ans_labels in loader:

        print(token_ids.size())

        print(cls_number.size())

        loss,cls_loss = model(token_ids, seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger,qa_start,qa_end,ans_labels,is_test=False)

        print(loss)

        print(cls_loss)

        y= model(token_ids, seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger,qa_start,qa_end,ans_labels,is_test=True)
...
#     torch.cuda.manual_seed(2020)
    ...
    for i,(ids,seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger,qa_start,qa_end,ans_labels) in enumerate(bar):

        decode2= model(ids.cuda(DEVICE),seg_ids.cuda(DEVICE),labels.cuda(DEVICE),cls_label_65.cuda(DEVICE),cls_label_6.cuda(DEVICE),cls_number.cuda(DEVICE),cls_type.cuda(DEVICE),crf_trigger.cuda(DEVICE),qa_start.cuda(DEVICE),qa_end.cuda(DEVICE),ans_labels.cuda(DEVICE),is_test=True)
...
#     label_word_results=collections.defaultdict(list) #预测的结果
    ...
    for index in range(len(decodes2)):
...
#     res=metric_fns(word_results,label_word_results,results_crf2,label_results_crf2,True)

    if not is_test:

        res=metric_fn_word(results_crf2,label_results_crf2,log=False)
...
#         print('with crf2:',res_with2)

        return res

    else:

        return results_crf2,label_results_crf2
...
def train_model(model,train_loader,val_loader,val_features,v2=None,v2_f=None,accumulation_steps=2,early_stop_epochs=2,epochs=4,model_save_path='pytorch_zy_model_true.pkl'):  
...
#     crf_p=[n for n, p in param_optimizer if str(n).find('crf1')!=-1] #trigger more

    crf_p2=[n for n, p in param_optimizer if str(n).find('crf2')!=-1] #trigger3
...
#     print(crf2_w)
    ...
    optimizer_grouped_parameters = [
            ...
            {'params': [p for n, p in param_optimizer if not any(nd in n for nd in no_decay) and n not in crf_p2], 'weight_decay': 0.8},

            {'params': [p for n, p in param_optimizer if any(nd in n for nd in no_decay) and n not in crf_p2], 'weight_decay': 0.0},
    ...
    for epoch in range(1,epochs+1):
        ...
        for i,(ids,seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger,qa_start,qa_end,ans_labels) in enumerate(bar):
            ...
            loss,cls_loss_65 = model(ids.cuda(DEVICE),seg_ids.cuda(DEVICE),labels.cuda(DEVICE),cls_label_65.cuda(DEVICE),cls_label_6.cuda(DEVICE),cls_number.cuda(DEVICE),cls_type.cuda(DEVICE),crf_trigger.cuda(DEVICE),qa_start.cuda(DEVICE),qa_end.cuda(DEVICE),ans_labels.cuda(DEVICE),is_test=False)
        ...
        if v2:
            ...
            print('v2:',validation_fn(model,v2,v2_f))
...
# tra_loader,tra_features=get_loader(train_data,batch_size=16,train_mode=True,train_val=True)
# model=zy_Model(num_labels).cuda(DEVICE)

# losses=train_model(model,tra_loader,val_loader,val_features,accumulation_steps=2,early_stop_epochs=2,epochs=20,model_save_path='cv_recall.pkl')
# for l in losses:
...
# model_save_path='cv_recall.pkl'

# model.load_state_dict(torch.load(model_save_path))
# validation_fn(model,val_loader,val_features,is_test=False)
# val_off_recall,val_off_real=validation_fn(model,val_loader,val_features,is_test=True)
# test_off_recall,test_off_real=validation_fn(model,test_loader,test_features,is_test=True)
# import pickle

# with open('ans_recall_large_val.pkl','wb') as f:

#     pickle.dump(val_off_recall,f)

# with open('ans_recall_large_test.pkl','wb') as f:

#     pickle.dump(test_off_recall,f)
...
for i,(train_index , test_index) in enumerate(kf.split(train_data)):
    ...
    losses=train_model(model,tra_loader,valid_loader,valid_features,val_loader,val_features,accumulation_steps=2,early_stop_epochs=2,epochs=5,model_save_path='cv_recall{}.pkl'.format(i+1))
...
#     model_save_path='../input/shijianchouquqa/cv_recall{}.pkl'.format(i+1)
    ...
    off_recall,off_real=validation_fn(model,valid_loader,valid_features,is_test=True)

    with open('off_recall_{}.pkl'.format(i+1),'wb') as f:

        pickle.dump(off_recall,f)

    #val 预测

    val_off_recall,val_off_real=validation_fn(model,val_loader,val_features,is_test=True)

    with open('val_recall_{}.pkl'.format(i+1),'wb') as f:
    ...
    test_off_recall,_=validation_fn(model,test_loader,test_features,is_test=True)

    with open('test_recall_{}.pkl'.format(i+1),'wb') as f:
...
# #     losses=train_model(model,tra_loader,valid_loader,valid_features,val_loader,val_features,accumulation_steps=2,early_stop_epochs=2,epochs=20,model_save_path='cv_recall{}.pkl'.format(i+1))
