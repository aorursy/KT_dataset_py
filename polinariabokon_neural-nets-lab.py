...
print('F1-score:', f1_score(y_valid, y_pred,average='macro')) # log_reg: ~0.2
scaler = StandardScaler()
mlp = MLPClassifier(solver='lbfgs')
...
model_grid = GridSearchCV(model, param_grid, cv=5, scoring='f1_macro',
                          n_jobs=-1) 
model_grid.fit(X_train, y_train)
print(model_grid.best_params_)
print(model_grid.best_score_)
pd.DataFrame(model_grid.cv_results_).T
results_df = pd.DataFrame(model_grid.cv_results_)
plt.plot(results_df['param_mlp__alpha'], results_df['mean_test_score'])

# Подписываем оси и график
plt.xlabel('mlp__alpha')
