...
def menu_is_boring(meals):
    ...
    """
    for i, meal in enumerate(meals[1:]):#i is offset by 1
        if meal == meals[i]:
            return True
    return False
...
def estimate_average_slot_payout(n_runs):
    ...
    0.5
    """
    return stat.mean([play_slot_machine()-1 for i in range(n_runs)])
...
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    #print(len(successes), successes.count(True), n_simulations)
    #print(start_balance, n_spins, n_simulations, successes)
    return stat.mean(successes)    
