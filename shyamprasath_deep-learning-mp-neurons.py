...
train = pd.read_csv("/kaggle/input/padhai-mp-neuron-like-unlike-classification/train.csv") 

test = pd.read_csv("/kaggle/input/padhai-mp-neuron-like-unlike-classification/test.csv")
...
x_binarised_test = test_new[test_new.columns.difference(['PhoneId'])].apply(pd.cut, bins = 2, labels = [0,1])
...
for b in range(x_binarised_train.shape[1]+1):
    ...
    print(b, accuracy/x_binarised_train.shape[0])
...
test = pd.DataFrame(mp.predict(x_binarised_test).astype('int'))
# submission = pd.concat([test_new['PhoneId'], test], axis = 1)

# submission.columns = ['PhoneId', 'Class']
