...
# 这里要写你的地址
net = load_net('../input/mycheckpoint10/last-checkpoint.bin')
class BaseWheatTTA:
    """ author: @shonenkov """
    image_size = 512
...
# 执行 TTA
def make_tta_predictions(images, score_threshold=0.4):
    with torch.no_grad():
    ...
    return predictions
# 执行模型的融合
def run_wbf(predictions, image_index, image_size=512, iou_thr=0.5, skip_box_thr=0.7, weights=None):
