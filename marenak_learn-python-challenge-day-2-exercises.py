...
def to_smash(total_candies , friends = 3):
    ...
    the given number of candies evenly between a specified number of friends (if none is provided, then default is 3).
...
#error in usage of abs()
x = -10
y = 5
 # Which of the two variables above has the smallest absolute value?
smallest_abs = min(abs(x), abs(y))
#return not in function. it will return none?
def f(x):
    y = abs(x)
    return y
