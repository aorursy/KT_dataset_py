...
# internal layer 1->internal layer 2 with nVal nodes
iLayer2 = Dense(nVal, activation='relu')(iLayer)
# prev layer -> output (1 node)
output = Dense(1, activation='sigmoid')(iLayer2) 
...
# Train on all the data for submission
x_data = np.concatenate([signalData[logCol+linCol][:].values,
                         backgroundData[logCol+linCol][:].values])
y_data = np.concatenate([(signalData["mc_flavour"][:]==5).values.astype(np.int),
...
y_eval = np.concatenate([(signalData["mc_flavour"][nTrainSig:]==5).values.astype(np.int),
                         (backgroundData["mc_flavour"][nTrainBkg:]==5).values.astype(np.int)])
x_data_rescaled = x_data / np.max(np.abs(x_data),axis=0)
x_eval_rescaled = x_eval / np.max(np.abs(x_data),axis=0)

# dtrain = xgb.DMatrix(x_data_rescaled, label = y_data)
# dtest = xgb.DMatrix(x_eval_rescaled, label=y_eval)
...
max_depths = [2,3,4,5]
etas = [0.2,0.4,0.7,1]
num_rounds = [50,100,200,400]

#xgb.cv(param, dtrain, num_round, nfold=5,
#       metrics={'error'}, seed=0,
#       callbacks=[xgb.callback.print_evaluation(show_stdv=True)])
param = {'max_depth': 5, 'eta': 0.12, 'objective': 'binary:logistic','gamma':0.4,'alpha':0.4,'min_child_weight':0.4}
bst = xgb.train(param, dtrain, 165)
...
#     for j in range(4):
#         for k in range(4):
#             param = {'max_depth': max_depths[i], 'eta': etas[j], 'objective': 'binary:logistic'}
#             bst = xgb.train(param, dtrain, num_rounds[k])
#             ypred = bst.predict(dtest)
#             print((sum(np.round(ypred,0) * y_eval) + sum((1-np.round(ypred,0)) * (1-y_eval))) / len(y_eval))
#             results[i,j,k] = (sum(np.round(ypred,0) * y_eval) + sum((1-np.round(ypred,0)) * (1-y_eval))) / len(y_eval)
print(results[3,0,1])
print(max_depths[3],etas[0],num_rounds[1])
print(0.79186944)
print((sum(np.round(ypred,0) * y_eval) + sum((1-np.round(ypred,0)) * (1-y_eval))) / len(y_eval))
print(sum(np.round(ypred,0)).astype(np.int))
...
pred_values = np.round(bst.predict(x_testM),0).astype(np.int)
print(sum(pred_values))
testData = pd.read_csv(path+"competitionData.csv")
# apply log10 to columns that need it
for l in logCol:
    testData[l] = np.log10(testData[l])
x_test = testData[logCol+linCol].values
x_test_rescaled = x_test / np.max(np.abs(x_test),axis=0)


testData["Prediction1"] = pred_values

# solution to submit
display(testData[["Id","Prediction1"]]) # display 5 rows
# write to a csv file for submission
testData.to_csv("submit11.csv.gz",index=False,columns=["Id","Prediction1"],compression="gzip") # Output a compressed csv file for submission: see /kaggle/working to the right