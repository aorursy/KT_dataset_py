...
q_6.solution()
data.head()
# adding days of travel
#datetime = pd.to_datetime(data['pickup_datetime'], format="%Y/%m/%d")
data['date'] = pd.to_datetime(data['pickup_datetime'].apply(lambda x: x[:10]), format="%Y/%m/%d")
...
# Create a PermutationImportance object on second_model and fit it to new_val_X and new_val_y
perm3 = PermutationImportance(third_model).fit(new_val_X, new_val_y)
