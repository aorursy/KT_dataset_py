...
df = pd.read_json("/kaggle/input/emotional-data/emotion_id_tweets.json", lines = True)

df.head()
df.info()
# check number of labels for each comment
df["annotation"].apply(lambda x : len(x["labels"])).value_counts()
## Remove empty data
lables_mask = df["annotation"].apply(lambda x : True if len(x["labels"])==1 else False)
df = df[lables_mask]

# Extract lables from list
df['CATEGORY'] = df['annotation'].apply(lambda x: x['labels'][0])
df = df[(df['CATEGORY']=='trust')|(df["CATEGORY"]=='joy')|(df["CATEGORY"]=='anticipation')]
#set(df['content'].apply(lambda x: split_count(x)))
set(df['content'].apply(lambda x: ''.join(re.findall(r'[^\x00-\x7F]+', x))))
...
#read emojis to text mapping data from .pkl file
import pickle



with open('UNICODE_EMO.pickle', 'wb') as handle:
    pickle.dump(EMO_UNICODE, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
emojis = ['☹','☺☺','⚫','✅','✊', '❤️', '❤', '⬆️']
for i in emojis:
    cvt = convert_emojis(i)
    
    print(i+':'+cvt)
##english text to indonesian text dictionary
englis_to_indo = {"frowning_face" : 'wajah mengerut',
"smiling_face": 'wajah tersenyum',
"black_circle": "lingkaran hitam",
"white_heavy_check_mark" : "tanda centang putih berat",
"raised_fist": "mengangkat fist",
"red_heart️": "merah hati️",
"up_arrow️": " hati merah"}
...
# Function for removing non ASCII charecters like 'Ϫ','ó','ü','©','▲','→' and emoticons
...
def convert_to_indo(text):
    for k,v in englis_to_indo.items():
        text =re.sub(k,v,text)
    return text 
def preprocess(df,dask_=False ,convert_emoji=True,convert_to_ind=True, accented_chars=True,mentions_and_urls=True,email=True,punctuation=True,
               excess_fullstops=True,ghost_char=True,excess_whitespace=True):
    ...
    if convert_emoji: 
        print('convert emojis to english text charecters now!')
        if dask_:
            _series = (dd
                               .from_pandas(_series, npartitions=4)
                               .apply( lambda x: convert_emojis(x),meta=('content', 'object'))
                               .compute(scheduler='processes')
                                )
        else:
            _series = _series.apply(lambda x: convert_emojis(x))
    
    if convert_to_ind: 
        print('convert english emojis text to indonesian text!')
        if dask_:
            _series = (dd
                               .from_pandas(_series, npartitions=4)
                               .apply( lambda x: convert_to_indo(x),meta=('content', 'object'))
                               .compute(scheduler='processes')
                                )
        else:
            _series = _series.apply(lambda x: convert_to_indo(x))
                          
    if accented_chars: 
        print('Removing accented charecters now!')
        if dask_:
            _series = (dd
                               .from_pandas(_series, npartitions=4)
                               .apply( lambda x: remove_accented_chars(x),meta=('content', 'object'))
                               .compute(scheduler='processes')
                                )
        else:
            _series = _series.apply(lambda x: remove_accented_chars(x))
            
    if mentions_and_urls: 
        print('Removing mentions and urls now!')
        if dask_:
            _series = (dd
                               .from_pandas(_series, npartitions=4)
                               .apply( lambda x: remove_mentions_and_urls(x),meta=('content', 'object'))
                               .compute(scheduler='processes')
                                )
        else:
            _series = _series.apply(lambda x: remove_mentions_and_urls(x))
    ...
    if email: 
        ...
        if dask_:
            _series = (dd
                               .from_pandas(_series, npartitions=4)
                               .apply( lambda x: remove_email(x),meta=('content', 'object'))
                               .compute(scheduler='processes')
                                )
        else:
    ...
    if punctuation: 
        print('Removing punctuations now!')
        if dask_:
            _series = (dd
                               .from_pandas(_series, npartitions=4)
                               .apply( lambda x: remove_punctuation(x),meta=('content', 'object'))
                               .compute(scheduler='processes')
                                )
        else:
    ...
    if excess_fullstops: 
        print('Removing multiple spaces now!')
        if dask_:
            _series = (dd
                               .from_pandas(_series, npartitions=4)
                               .apply( lambda x: remove_excess_fullstops(x),meta=('content', 'object'))
                               .compute(scheduler='processes')
                                )
        else:
            _series = _series.apply(lambda x: remove_punctuation(x))  
    ...
    if ghost_char: 
        print('Removing ghost charecters now!')
        if dask_:
            _series = (dd
                               .from_pandas(_series, npartitions=4)
                               .apply( lambda x: remove_ghost_char(x),meta=('content', 'object'))
                               .compute(scheduler='processes')
                                )
        else:
    ...
    if excess_whitespace: 
        print('Removing white spaces now!')
        if dask_:
            _series = (dd
                               .from_pandas(_series, npartitions=4)
                               .apply( lambda x: remove_excess_whitespace(x),meta=('content', 'object'))
                               .compute(scheduler='processes')
                                )
        else:
            _series = _series.apply(lambda x: remove_excess_whitespace(x))     
    ...
    return _series         
df['content'] = preprocess(df,dask_=True,convert_emoji=True )
...
def accuracy_per_class(preds, labels):
    label_dict_inverse = {v: k for k, v in encode_dict.items()}
...
model.to(device)
## laod best model dictionary saved in previous steps in our case it is ~54%(f1 score) in 2nd epochs where validation loss and train loss are 1.08,1.21 respectively
