...
d = {'LotArea': [train_ds.LotArea.mean(), test_ds.LotArea.mean()], 'name': ['Train','Test']}
...
edited_train.head()
edited_train = pd.get_dummies(edited_train, columns=['OverallQual', 'SaleCondition', 'OverallCond'])
...
test = pd.get_dummies(test, columns=['OverallQual', 'SaleCondition', 'OverallCond'])
...
sub.to_csv('submissionfinal3.csv', index=False)