...
df.rename(columns={'Track.Name' : 'track_name' , 'Artist.Name' : 'artist_name' , 'Beats.Per.Minute' : 'beats_per_minute' , 'Energy' : 'Energy' , 'Danceability' : 'Danceability' , 
                   'Loudness..dB..' : 'Loudness(dB)','Valence.':'Valence','Length.':'Length','Acousticness..' : 'Acousticness','Speechiness.':'Speechiness',
...
df.groupby('artist_name')['track_name'].agg(len).sort_values(ascending = False)
...
plt.axis('equal')
plt.show()
x=df.loc[:,['Energy','Length','Danceability','beats_per_minute', 'Acousticness']]
...
scores=cross_val_score(LinSVC,X_train,y_train,scoring='accuracy',cv=3).mean()*1
print(scores)
