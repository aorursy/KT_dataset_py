...
data= pd.read_csv("../input/checking/main.csv")
data.head()
dat=data
dat.columns
df = data
...
df['TC '].replace({'sport':1,'game':2,'ECommerce':3,'holiday':4},inplace=True)
...
cluster_labels = cluster.fit_predict(dff) 
cluster.labels_
accuracy_score(df.iloc[:,4], cluster.labels_)
