...
total_missin=missing_value_count.sum()

(total_missin/total_cell)*100
# look at the # of missing points in the first ten columns
missing_values_count[0:10]
...
non_missing_column = sf_permits.dropna(axis=1)
print("original df columns in number %d" % sf_permits.shape[1])
print("non_missing df columns in number %d" % non_missing_column.shape[1])
