...
def eliminate_outliers(df):
    ...
    for column in df.columns:
        ...
        if(df[column].dtype != "object"):
            ...
            df[column] = [i if (i >= low and i <= high) else df[column].mean() for i in df[column]]
...
features = ['Rooms', 'Bathroom', 'Car', 'Landsize', 'BuildingArea', 'YearBuilt', 'Propertycount', 'Distance', 'Suburb', 'CouncilArea', 'Regionname']
...
eliminate_outliers(X)

classify_categorical(X)
...
eliminate_outliers(X_test)

classify_categorical(X_test)
