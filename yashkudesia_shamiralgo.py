#!/usr/bin/env python
# coding: utf-8

# In[61]:


import random 
import struct


# ### As, the N and K are given, (N=4 and k=2) .So our polynomial will be y=a+bx , where a will be our secret and b will be randomly generate cofficient from the GF(256)
# 
# 
# 

# In[95]:


#global variables
field_size=256


# ### Generating the Shares

# In[109]:


def evalPolynomial(x,b,s): 
    return (x*b+s)

def generateB(): 
    b = random.randrange(0, field_size)
    return b

def generateShares(n,k,secret):
    # as per question, n=4,k=2
    b = generateB()
    shares = [] 
    for i in range(1,n+1): 
        x = random.randrange(1, field_size) 
        shares.append([x, evalPolynomial(x,b,secret)]) 
    return shares


# ### Reconstruction of Polynomial 

# the efficient method of reconstruction is to use byte values of log and exponential to eveluate the addition/multiplication/division

# In[111]:


log_val=bytes([ ])
exp_val=bytes([
      
])
def bytes_to_int(tb):
    result = struct.unpack('>H', tb)
    return result

def addGf256(a,b):
    return (byte) (a ^ b)

def subGf256(a,b):
    return addGF256(a,b)

def multGF256(a,b):
    if a==0 or b==0:
        return 0
    return exp_val[(log_val[(a)]) + (log_val[(b)])]

def divGF256(a,b):
    return multGF256(a, exp_val[255 - (log_val[(b)])])


# In[133]:


def recostruct(shares):
    print("Reconstruction ongoing.....")
    for j in range(len(shares)):
        print("Itertaing Shares...")
        xj, yj = shares[j][0],shares[j][1]
        prod = int(1)
        
        for i in range(len(shares)): 
            xi = shares[i][0] 
            if i != j:
                temp = int(int(xi)/(xi-xj))
                print("temp = ",temp)
                prod = multGF256(prod,temp)
            print("Performing Calculation..")
                  
        print("Checkpoint 1")
        print(prod,xj,yj)
        prod = multGF256(prod,yj)
        print("Checkpoint 2")
        print(sums,prod)
        sums = addGF256(sums,int(prod))
          
    return int(round(int(sums),0))


# In[ ]: