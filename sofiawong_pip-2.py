...
period_osc =  2*(np.pi)*10/v                     # calculate the period of the oscillation (from the input variables)

end_time =  period_osc*N_loops                      # calculate the end time of the c (from the input variables)

time_steps = steps_rev*(end_time-start_time)/(period_osc)                  # calculate the number of time steps (from the input variables)

# --- defininition of time array for propagation 

step = (end_time-start_time)/(time_steps-1)
time = np.linspace(int(start_time),int(end_time),int(time_steps))
...
position_x = np.zeros(int(time_steps))
position_y = np.zeros(int(time_steps))
velocity_x = np.zeros(int(time_steps))
velocity_y = np.zeros(int(time_steps))
...
for ii in range(1,int(time_steps)):
