...
dataset = dataset[~dataset.subtask_b.isna()]
...
for i in range(5):
    ...
    train_df = train_df.append(train_df[train_df.subtask_b==0])

    

train_dataset = get_dataset(train_df , max_len).shuffle(10000).batch(batch_size)
...
print(classification_report(y_true, y_pred, labels=[0,1], target_names=['UNT', 'TIN']))
...
for i in range(3):
    ...
    dataset = dataset.append(dataset[dataset.subtask_b==0])
...
test_set.to_csv("subtask_b.csv", index=False)