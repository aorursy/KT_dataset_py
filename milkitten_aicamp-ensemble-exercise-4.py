...
trainX, trainY = train_data.iloc[:,1:], pd.DataFrame(train_data.iloc[:,0])# TODO

testX, testY = test_data.iloc[:,1:], pd.DataFrame(test_data.iloc[:,0])# TODO
type(trainY)
class TreeNode:
...
def node_weighted_mistakes(targets_in_node, data_weights):
    ...
    return (weight_positive, -1) if weight_positive < weight_negative else (weight_negative, 1)
#test
...
def best_split_weighted(data, features, target, data_weights):
    ...
    for feature in features:
        ...
        left_misses, left_class = node_weighted_mistakes(left_split[target], left_data_weights)# TODO            



        # 计算右边分支里犯了多少错 (加权结果！！)

        right_misses, right_class = node_weighted_mistakes(right_split[target], right_data_weights)# TODO

            

        # 计算当前划分之后的分类犯错率

        error = (left_misses + right_misses)/sum(data_weights)# TODO
...
def create_weighted_tree(data, data_weights, features, target, current_depth = 0, max_depth = 10, min_error=1e-15):
    ...
    split_feature = best_split_weighted(data, features, target, data_weights)# TODO  # 根据加权错误来划分
...
class MyAdaboost(BaseEstimator):
    ...
    def fit(self, X, Y):
        ...
        for m in range(self.M):
            ...
            weighted_error = alpha[prediction != Y[target]].sum()# TODO
            ...
            alpha = alpha*np.exp(-model_weight*Y[target]*prediction)# TODO
