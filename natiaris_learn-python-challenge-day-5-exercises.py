...
def estimate_average_slot_payout(n_runs):
    ...
    """
    return sum(play_slot_machine() for _ in range(n_runs)) / n_runs - 1
estimate_average_slot_payout(1_000_000_000)
q4.solution()
def simulate_slots(balance, n_spins):
...
def slots_survival_probability(balance, n_spins, n_simulations):
    ...
    .25
    """
    return sum(simulate_slots(balance, n_spins) for _ in range(n_simulations)) / n_simulations
