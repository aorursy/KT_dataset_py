...
def displayMetrics(modelName, model):
    ...
    printConfusionMatrix(modelName, y_trueval, y_predval)
...
# print(y_true)
print('Validation Accuracy: ', accuracy_score(y_true, y_pred ))
printConfusionMatrix('Pneumonia detection', y_true, y_pred)
...
# fileName = 'model_deepunet1.h5'#'model_deepunetfinal.h5'
# h5Path = os.path.join(root_dir, 'input', 'h5files',fileName)
# if not loadweightsDirectlyToModel:
...
loadweightsDirectlyToModel
fileName = 'model_deepunetfinal.h5'
h5Path = os.path.join(root_dir, 'input', 'h5files',fileName)
if loadweightsDirectlyToModel:
    ...
    deepunetmodel.save_weights(fileName)
if not loadweightsDirectlyToModel:
    print('List of metrics available: ', deepunetmodel.metrics_names)
if not loadweightsDirectlyToModel:
    plt.figure(figsize=(12,4))
    plt.subplot(131)
...
cnnmodel = create_network(input_size=128, channels=32, n_blocks=2, depth=4)
...
saveOrLoadWeights(fileName)
plotGraphs(cnnhistory)
displayImagesWithBoundingBox(cnnmodel)
displayMetrics('CNN Model',cnnmodel)
...
fileName = "resnetmodel.h5"
saveOrLoadWeights(fileName)
plotGraphs(resnethistory)
displayImagesWithBoundingBox(resnetmodel)
displayMetrics('ResNet Model',resnetmodel)