...
train_id, train_label, train_feat = train_data[:, 0], train_data[:, 1], train_data[:, 2:] #id 序号 #label 存活 #feat 信息

test_id, test_feat = test_data[:, 0], test_data[:, 1:]



train_feat[:, [1, 6]] = None #1和6 name和ticket
...
param = {
    ...
    'gamma': 0.1,        # 最小损失分裂。越大越保守，一般0.1、0.2

    'max_depth': 4,      # 树的最大深度。越大模型越复杂，越容易过拟合

    'subsample': 0.8,    # 子样本数目。是否只使用部分的样本进行训练，这可以避免过拟合化。
...
#[best] train-error:0.12219 eval-error:0.11173
...
#[best] valid_0's binary_error: 0.106145
...
class my_gbdt(object):
    ...
    def __init__(self):
        ...
        self.n_estimators = 27                   # 树的数量

        self.learning_rate = 0.15                # 学习率

        

        self.min_samples_split = 2               # 每颗子树节点分割时候有的最小的样本数

        self.max_depth = 5                       # 每颗子树的最大层次
    ...
    def fit(self, 
        ...
        pred_proba = self.trees[0].predict(X)

        for i in range(self.n_estimators):

            gradient = np.exp(pred_proba) / np.expand_dims(np.sum(np.exp(pred_proba), axis=1), axis=1) - y

            self.trees[i].fit(X, gradient) 

            pred_proba -= self.learning_rate * self.trees[i].predict(X)
