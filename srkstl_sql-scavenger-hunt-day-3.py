...
query = """SELECT COUNT(consecutive_number) AS Qty, 
...
plt.plot(accidents_by_day.Qty)
plt.title("Number of Accidents by Rank of Day \n (Most to least dangerous)")
print(accidents_by_day)
hour_query = """
    SELECT EXTRACT(hour from timestamp_of_crash), COUNT(consecutive_number)
    FROM `bigquery-public-data.nhtsa_traffic_fatalities.accident_2015`
...
hitAndRun_query = """
    ...
    ORDER BY 2 DESC
"""
accidents_hitAndRun = accidents.query_to_pandas_safe(hitAndRun_query)
# library for plotting
import matplotlib.pyplot as plt

# make a plot to show that our data is, actually, sorted:
plt.plot(accidents_hitAndRun.f0_)
plt.title("Number of Hit and Run Accidents by State \n (Most to least dangerous)")

print (accidents_hitAndRun)