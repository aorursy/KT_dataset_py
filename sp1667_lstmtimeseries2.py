...
df = pd.read_csv("../input/time-series/rnn.csv").reset_index()
...
def load_data(stock, seq_len, days_out):
...
dout = 10 # How many days ahead in the future are you predicting



x_train, y_train, x_valid, y_valid, x_test, y_test,f_batch = load_data(df, seq_len, days_out = dout)
...
model.add(GRU(32,input_shape=(seq_len-dout,1)))
...
for i,j in zip(x_test,y_test):
    ...
    predictions.append(Scaler.inverse_transform(model.predict(i.reshape(-1,50,1)))[0][-1])

    true.append(Scaler.inverse_transform([j])[0][-1])
plt.figure(figsize=(40, 5));
...
for i in range(len(x_test)):
    ...
    prediction_ = (model.predict(seq.reshape(-1,50,1)))

    prediction__ = prediction_[0][-1]
    ...
    seq = np.vstack([seq[10:],prediction_.reshape(-1,1)])
