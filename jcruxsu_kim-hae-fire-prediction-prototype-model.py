...
train = pd.read_csv("../input/se-valid-train-set/se_train_na1.csv")

valid =  pd.read_csv("../input/se-valid-train-set/se_valid_na1.csv")
...
valid_v1 = valid
train =train.drop(columns=['bldng_us_clssfctn','year','emd_nm','Class'])

train =train.drop(columns=['id','dt_of_fr'])

test =test.drop(columns=['id','dt_of_fr','bldng_us_clssfctn','emd_nm','fr_yn','year'])

valid_v1 = valid_v1.drop(columns=['id','dt_of_fr'])

valid_v1 = valid_v1.drop(columns=['bldng_us_clssfctn','emd_nm'])
train_v1 =train_v1.drop(columns=['bldng_us_clssfctn','emd_nm','year','Class'])
...
valid_v1['fr_yn'] = valid_v1['fr_yn'].map(binary_y)
...
valid_v1['bldng_us'] = valid_v1['bldng_us'].map(bldng_us_cols_dic)
...
plt.title("wnd")
train[train['fr_yn']==1]['wnd']

train_v1['wnd'].loc[train_v1['wnd']!='none'] = pd.to_numeric(train_v1['wnd'].loc[train_v1['wnd']!='none'])/180*np.pi

valid_v1['wnd'].loc[valid_v1['wnd']!='none'] = pd.to_numeric(valid_v1['wnd'].loc[valid_v1['wnd']!='none'])/180*np.pi

train_v1['wnd'] = train_v1['wnd'].replace({'none':-1})

valid_v1['wnd'] = valid_v1['wnd'].replace({'none':-1})

test['wnd'].loc[test['wnd']!='none'] = pd.to_numeric(test['wnd_drctn'].loc[test['wnd']!='none'])/180*np.pi

test['wnd'] = test['wnd'].replace({'none':-1})
train_v1['wnd'].head()
#train_v1 = train_v1.drop(columns=['wnd'])

#test = test.drop(columns=['wnd'])

#valid_v1= valid_v1.drop(columns=['wnd'])
...
sns.kdeplot(train_v1['bldng_cnt_in_50m'])
...
sns.kdeplot(train['no_tbc_zn_dstnc'])
...
valid_v1['sft_emrgnc_bll_dstnc'] = regularization_mean_std_d(valid_v1['sft_emrgnc_bll_dstnc']+smallE)
...
valid_v1['hmdt'] = regularization_mean_std_d(valid_v1['hmdt'])

test['hmdt'] = regularization_mean_std_d(test['hmdt'])
sns.kdeplot((valid_v1['fr_wthr_fclt_dstnc']))
...
cctv_in_100m_reg_log =regularization_log_d(np.asarray(train['cctv_in_100m']))
train_v1['cctv_in_100m'] = regularization_log_d(train_v1['cctv_in_100m']+smallE)

valid_v1['cctv_in_100m'] = regularization_log_d(valid_v1['cctv_in_100m']+smallE)

test['cctv_in_100m'] = regularization_log_d(test['cctv_in_100m']+smallE)
sns.kdeplot(np.asarray(train['ahsm_dstnc']))
ahsm_dstnc_mean_std_d = regularization_mean_std_d(train['ahsm_dstnc'])
sns.kdeplot(ahsm_dstnc_mean_std_d)

train_v1['ahsm_dstnc'] = regularization_mean_std_d(train_v1['ahsm_dstnc']+smallE)

valid_v1['ahsm_dstnc'] = regularization_mean_std_d(valid_v1['ahsm_dstnc']+smallE)
...
valid_v1['mlt_us_yn'] = valid_v1['mlt_us_yn'].map(binary_y)
...
#valid_v1.drop(columns=['mlt_us_yn'])

#test.drop(columns=['mlt_us_yn'])
f,ax = plt.subplots(1,1,figsize=(18,8))

sns.countplot('mlt_us_yn',hue='fr_yn',data=train)
...
sns.kdeplot((train['lnd_ar']+smallE))
...
valid_v1['ttl_ar'] =  regularization_log_d(valid_v1['ttl_ar'] +smallE)

test['ttl_ar'] = regularization_log_d(test['ttl_ar'] +smallE)
sns.kdeplot(train_v1['ttl_ar'])
sns.kdeplot(train['bldng_ar'])
...
#valid_v1['bldng_ar'] = regularization_log_d(valid_v1['bldng_ar']+ smallE)

#test['bldng_ar'] = regularization_log_d(test['bldng_ar']+ smallE)
sns.kdeplot((train_v1['floor']))
...
valid_v1 =  valid_v1.drop(columns =['floor'])

test=  test.drop(columns =['floor'])
sns.kdeplot((train['tbc_rtl_str_dstnc']))
train_v1['tbc_rtl_str_dstnc'] = regularization_mean_std_d(train_v1['tbc_rtl_str_dstnc']+smallE)

valid_v1['tbc_rtl_str_dstnc'] = regularization_mean_std_d(valid_v1['tbc_rtl_str_dstnc']+smallE)

test['tbc_rtl_str_dstnc'] = regularization_mean_std_d(test['tbc_rtl_str_dstnc']+smallE)
train_v1['dt_of_athrztn']= pd.to_numeric(train_v1['dt_of_athrztn'])

valid['dt_of_athrztn']= pd.to_numeric(valid['dt_of_athrztn'])

sns.kdeplot((train_v1['dt_of_athrztn']))
train_v1['dt_of_athrztn'] = regularization_mean_std_d(train['dt_of_athrztn'])

valid_v1['dt_of_athrztn'] = regularization_mean_std_d(valid_v1['dt_of_athrztn'])

test['dt_of_athrztn'] = regularization_mean_std_d(test['dt_of_athrztn'])
bldng_archtctr_one_hot_df = pd.get_dummies(train_v1['bldng_archtctr'])

valid_bldng_archtctr_one_hot_df  = pd.get_dummies(valid_v1['bldng_archtctr'])
...
valid_v1['bldng_archtctr'] = valid_v1['bldng_archtctr'].map(bldng_archtctr_dic)
...
valid_v1['jmk'] = valid_v1['jmk'].map(jmk_dic)
...
valid_v1['rgnl_ar_nm'] = valid_v1['rgnl_ar_nm'].map(rgnl_ar_nm_dic)
...
valid_v1['rgnl_ar_nm2'] = valid_v1['rgnl_ar_nm2'].map(rgnl_ar_nm2_dic)
...
valid_v1['lnd_us_sttn_nm'] = valid_v1['lnd_us_sttn_nm'].map(lnd_us_sttn_nm_dic)

test['lnd_us_sttn_nm'] = test['lnd_us_sttn_nm'].map(lnd_us_sttn_nm_dic)
sns.kdeplot((train_v1['hm_cnt']))
...
valid_v1['rd_sd_nm'] = valid_v1['rd_sd_nm'].map(rd_sd_nm_dic)
...
train_v1['fr_sttn_dstnc'] =regularization_mean_std_d(train_v1['fr_sttn_dstnc']+smallE)

valid_v1['fr_sttn_dstnc'] =regularization_mean_std_d(valid_v1['fr_sttn_dstnc']+smallE)

test['fr_sttn_dstnc'] =regularization_mean_std_d(test['fr_sttn_dstnc']+smallE)
ele = 'ele_engry_us_'
...
for one in ele_engry_us_cols:
    ...
    valid_v1[one]=regularization_log_d( valid_v1[one]+smallE)
...
valid_gas_engry_us_df = valid_v1[gas_engry_us_cols]

valid_ele_engry_us_df = valid_v1[ele_engry_us_cols]

#valid_v1 = valid_v1.drop(columns=drop_ele_gas)

#test = test.drop(columns=drop_ele_gas)
from keras import *
...
valid_y =valid_v1['fr_yn']

valid_v1= valid_v1.drop(columns=['fr_yn'])
...
#ada.fit(train_v1.values, train_y_v1.values)
...
def predict_test(test):
    ...
    return test_res
ensem_res = predict_test(train_v1)
...
param_grid = [
    ...
    {'n_estimators': [3,8,20],'max_features' : [2,4,6,8]},
