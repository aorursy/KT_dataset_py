...
data_train['Age'].describe().to_frame()
...
plt.show()
tab = pd.crosstab(data_train['Embarked'], data_train['Pclass'])
...
data_train.groupby('family_size').size().reset_index(name='counts')
...
data_test['fare_group'] = data_test['Fare'].map(fare_group_fun) # the range apply to both dataset
