...
sample.head()
train.tail(10)
train_y = train[(train["date"] == "2020-08-07")|(train["date"] == "2020-07-31")].groupby("name")["end"].diff().dropna()

test_y = train[(train["date"] == "2020-08-14")|(train["date"] == "2020-08-07")].groupby("name")["end"].diff().dropna()
train_X = train[(train["date"] < "2020-07-31")&(train["date"] > "2020-06-15")].iloc[:,1:]

train_X.shape
test_X = train[(train["date"] < "2020-08-07")&(train["date"] > "2020-06-22")].iloc[:,1:]

test_X.shape
sub_X = train[(train["date"] < "2020-08-14")&(train["date"] > "2020-06-27")].iloc[:,1:]
