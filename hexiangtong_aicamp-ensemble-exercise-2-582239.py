# necessary imports
%matplotlib inline
import os
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt


from sklearn.ensemble import AdaBoostClassifier
from sklearn.tree import DecisionTreeClassifier
from sklearn.linear_model import LogisticRegression
from sklearn.model_selection import cross_val_score

np.random.seed(19)

data_folder = "../input"
#data_folder = "data"
# data = pd.read_csv(os.path.join(data_folder, "breastCancer.csv"))
data = pd.read_csv('./breastCancer.csv')
data.head()
data['diagnosis'].value_counts()
data.drop('id',axis=1,inplace=True)
data.drop('Unnamed: 32',axis=1,inplace=True)
# data['diagnosis'] = data['diagnosis'].apply(lambda x: 1 if x == 'M' else -1)#TODO 如果是零数据权重无法更新
data['diagnosis'] = data['diagnosis'].apply(lambda x : +1 if x=='M' else -1)
data.describe()
data.info()
import seaborn as sns
sns.countplot(data['diagnosis'])
features = data.columns[1:7]
target = 'diagnosis'
features
i = 0
for feature in features:

    bins = 25     
    #25个小区间
    # 将特征的直方图画出来
    # TODO
#     plt.hist(data[feature][data[target] == -1], bins=bins, color = 'lightblue', label = 'Good', alpha = 1)
#     plt.hist(data[feature][data[target] == 1], bins=bins, color = 'k', label = 'Bad', alpha = 0.5)
    plt.hist(data[feature][data['diagnosis'] == -1], bins=bins, color='lightblue', label= 'B-healthy', alpha=1)
    plt.hist(data[feature][data['diagnosis'] == 1], bins=bins, color='k', label='M-bad', alpha=0.5)
    plt.xlabel(feature)
    plt.ylabel('Amount of count')
    
    plt.legend()
    
    plt.show()
from sklearn.model_selection import train_test_split
train_data, test_data = train_test_split(data, test_size=0.3)
trainX, trainY = train_data[data.columns[1:]], train_data[target]# TODO
testX, testY = test_data[data.columns[1:]], test_data[target]
logistic_model = LogisticRegression()
print("Logistic Regression performance: %f" % (cross_val_score(logistic_model, trainX, trainY, cv=8).mean()))
tree_model = DecisionTreeClassifier()
print("Decision Tree performance: %f" % (cross_val_score(tree_model, trainX, trainY, cv=8).mean()))
ada_model = AdaBoostClassifier(n_estimators=200)#TODO
print("Decision Tree performance: %f" % (cross_val_score(ada_model, trainX, trainY, cv=8).mean()))
logistic_model = LogisticRegression()
logistic_model.fit(trainX, trainY)
print("Logistic Regression test performance: %f" % logistic_model.score(testX, testY))
tree_model = DecisionTreeClassifier()
tree_model.fit(trainX, trainY)
print("Decision Tree test performance: %f" % tree_model.score(testX, testY))
ada_model = AdaBoostClassifier(n_estimators=200)
ada_model.fit(trainX, trainY)
print("Adaboost test performance: %f" % ada_model.score(testX, testY))
from sklearn.base import BaseEstimator
class Adaboost(BaseEstimator):
    
    def __init__(self, M):
        self.M = M# TODO
        
    def fit(self, X, Y):
        self.models = []# TODO
        self.model_weights = []# TODO
        
        N, _ = X.shape
        alpha = np.ones(N)/N# TODO
        
        for m in range(self.M):
            tree = DecisionTreeClassifier(max_depth=2)# TODO
            tree.fit(X,Y,sample_weight=alpha)# TODO
            prediction = tree.predict(X)# TODO
            
            # 计算加权错误
            weighted_error = alpha.dot(prediction != Y)# TODO
            
            # 计算当前模型的权重
            model_weight = 0.5 * (np.log(1 - weighted_error) - np.log(weighted_error))# TODO
            
            # 更新数据的权重
            alpha = alpha * np.exp(-model_weight * Y * prediction)# TODO
            
            # 数据权重normalize
            alpha = alpha / alpha.sum()# TODO
            
            self.models.append(tree)
            self.model_weights.append(model_weight)
            
    def predict(self, X):
        N, _ = X.shape
        result = np.zeros(N)
        for wt, tree in zip(self.model_weights, self.models): # 每一个模型tree对应的权重是wt
            result += wt * tree.predict(X)# TODO
        
        return np.sign(result) # 返回是1或者-1的结果
    
    def score(self, X, Y):
        prediction = self.predict(X)# TODO
        return np.mean(prediction == Y)
adamodel = Adaboost(200)# TODO
print("Adaboost model performance: %f" % (cross_val_score(adamodel, trainX.as_matrix().astype(np.float64), trainY.as_matrix().astype(np.float64), cv=8).mean()))
adamodel.fit(trainX.as_matrix().astype(np.float64), trainY.as_matrix().astype(np.float64))
print("Adaboost model test performance: %f" % adamodel.score(testX.as_matrix().astype(np.float64), testY.as_matrix().astype(np.float64)))
