...
#     print(type(y_pred))
    ...
    iou = tf.math.subtract(tf.constant(1.0,dtype=tf.float32),tf.math.abs(tf.math.divide(area_inter,tf.math.subtract(tf.math.add(area_true,area_pred),area_inter))))
...
model.load_weights("../input/weight-15-pass/weight_15_epoch.h5")
model.fit(dataset_train,epochs=1,shuffle=False,steps_per_epoch=1810,validation_steps=100,validation_data=dataset_validation)
...
model.save_weights("weight_15_epoch.h5",overwrite=True)