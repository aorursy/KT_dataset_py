...
starbucks.head()
[print(starbucks.iloc[i]) for i in range(len(starbucks)) if (starbucks.iloc[i].isnull().any())]
...
CA_joined = pd.DataFrame.join(pd.DataFrame.join(CA_pop, CA_high_earners, on="GEOID"),CA_median_age, on="GEOID")
...
CA_stats = CA_counties.merge(CA_joined, on="GEOID")
...
starbucks_gdf.head()
starbucks_roasteries = gpd.sjoin(sel_counties, starbucks_gdf)

starbucks_roasteries.shape
# Fill in your answer
...
for idx, row in starbucks_roasteries.iterrows():
