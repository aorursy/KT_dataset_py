...
tqdm.pandas()
tiff_root = Path('..') / 'input' / '3d-dinosaur-teeth' / 'tiff'
...
all_tiff_df['region'] = all_tiff_df['full_id'].map(lambda x: x.parts[1])

all_tiff_df['view'] = all_tiff_df['full_id'].map(lambda x: x.parts[2] if len(x.parts)>2 else '')

all_tiff_df['slice'] = all_tiff_df['path'].map(lambda x: int(x.stem))
...
cur_slice_df.head(3)
fossil_data = np.stack(cur_slice_df['path'].progress_map(read_compressed_tiff), 0)
...
im_args = dict(cmap = 'bone', vmin = 1500, vmax = 3500)
...
try_all_threshold(fossil_data[slice_idx])
%matplotlib inline
