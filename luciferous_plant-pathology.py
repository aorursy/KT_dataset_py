...
pred=model2.predict(np_image_list[9:10])
print(label_transformer.inverse_transform(pred))
label_encoder = open(r'/kaggle/working/label_transform.pkl', 'rb')
label_transformer = pickle.load(label_encoder)
imgpath='../input/test-image/test.jpg'
def convert_image_to_array(image_dir):
    try:
        image = cv2.imread(image_dir)
        if image is not None :
...
PREDICTED_CLASSES2 = model2.predict_classes(np_image_list)
