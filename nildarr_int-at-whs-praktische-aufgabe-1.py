...
def gib_folgezustaende(zustand):
    ...
    m,k,b = zustand
    direction = -1 if b else 1
    
    #Missionare und Kannibalen dürfen per se zu je 0<missionare|kannibalen<=boatSize in einem Boot einsteigen
    listeM = [direction * i for i in range(boatSize+1)] #direction schonmal mitberücksicht
    listeK = listeM[:]
    
    #Für ein gültiges Boot muss gelten: 0<missionare+kannibalen<=boatSize.
    listeBoat = []
    for m in listeM:
        for k in listeK:
            if 0 < abs(m + k) <= boatSize:
                listeBoat.append([m,k,b])
    
    #Nicht alle möglichen Bootsbesetzungen sind für einen Spielzustand gültig, weil die Anzahl der Missionare und Kannibalen
    #immer im Intervall [0:max_value] liegen müssen. Arithmetische Separierung. 
    fZustaende = [[zustand[0] + boat[0], zustand[1] + boat[1], 0 if b else 1] for boat in listeBoat]
    fZustaende = [z for z in fZustaende if 0 <= z[0] <= max_value and 0 <= z[1] <= max_value]
