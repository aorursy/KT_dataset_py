...
def cross_validation_visualization_many_in_one(*error_plots, title="cross validation", target_dir=TARGET_DIR, plotfunction=plt.plot):
    ...
    for marker, ep in zip(marker_list, error_plots): # zip() stops once the end of either of its arguments is reached.
        ...
        plotfunction(ep.xvalues, ep.err_train, marker=marker, linestyle = '--', label='Training '+ ep.model_name)

        plotfunction(ep.xvalues, ep.err_test, marker=marker, label='Validation '+ ep.model_name)
...
#model fitting
    ...
    if (x is not None) and (y is not None):
        ...
        history = model.fit(x, y, epochs=number_epochs, batch_size=DEFAULT_BATCH_SIZE) # , validation_split=0.2
...
class Architecture_NN:
    ...
    modes = ["const"] # ["const", "ratio" , "pow", "exp"]
...
def grid_search_layer_size(NN_depth, initial_layer_sizes, mode, epochs, dropout=0.25, learning_rate=None, maxnorm=None, decay_rate=None):
    ...
    for initial_layer in initial_layer_sizes:
        ...
        metric_test = None # history.history['val_mean_squared_error'][-1]
...
#         plt.semilogy(np.sqrt(history.history['val_mean_squared_error']))
    ...
    return n_parameters, train_metrics, test_metrics, model
def grid_search_mode_and_size(NN_depth, initial_layer_sizes, epochs=30, dropout=0.25, learning_rate=None, maxnorm=None, decay_rate=None):
    ...
    for mode in Architecture_NN.modes:
        ...
        n_parameters, train_metrics, test_metrics, model = grid_search_layer_size(NN_depth, initial_layer_sizes, mode, epochs, dropout=dropout, learning_rate=learning_rate, maxnorm=maxnorm, decay_rate=decay_rate)
...
initial_layer_sizes = [100]

train_metric_by_depth, test_metric_by_depth = [], []

for depth in NN_depths:

    metric_train, metric_test, model = grid_search_mode_and_size(depth, initial_layer_sizes, epochs=epochs)
...
#                 NN_depths, "Number of layers".format(learning_rate),

#                 np.sqrt(train_metric_by_depth), np.sqrt(test_metric_by_depth)

#         ),

#         title="RMSE_of_the_dense_NN", plotfunction=plt.plot
