...
train_df = pd.read_csv("/kaggle/input/fastai/train.csv",parse_dates=['Date'],index_col="Id")
test_df = pd.read_csv("/kaggle/input/fastai/test.csv",parse_dates=['Date'],index_col="ForecastId")
...
y_train_cc = np.log1p(y_train_cc)
y_train_ft = np.log1p(y_train_ft)
def common_preprocess(df):
    add_datepart(df,'Date')
    train_cats(df)
    df,_,_ = proc_df(df,max_n_cat=173) 
    return df
    
def encode(df):
    total_cases = df.ConfirmedCases.values.sum()
    cases_by_country = df.groupby('Country_Region')['ConfirmedCases'].sum()
    df.Country_Region = df.Country_Region.map(cases_by_country)
    df1 = df[['Province_State','ConfirmedCases']]
    df1 = df1.groupby('Province_State',as_index=False).sum()
    dic = dict(zip(df1.Province_State,df1.ConfirmedCases))
    df['Province_State']= df['Province_State'].map(dic)
    df.Province_State =  df['Province_State']/df.Country_Region
    df.Country_Region = df.Country_Region/total_cases
    
    
full_df = common_preprocess(full_df)

full_df.head()

full_df.head()
full_df.drop('Year',axis=1,inplace=True)
x_train,x_test = split_val(full_df,train_df.shape[0])
len(x_train)
...
df_raw.head()
n_trn = int(0.8*len(df_raw))
n_trn
x_train,x_valid = split_val(df_raw,n_trn)
y_train1,y_valid1 = split_val(y_train_cc,n_trn)
y_train2,y_valid2 = split_val(y_train_ft,n_trn)
len(x_valid)
len(x_train)
len(df_raw)
...
y_train_ft = np.log1p(y_train_ft)
x_train.head()
n3 = RandomForestRegressor(n_jobs=-1)
n4 = RandomForestRegressor(n_jobs=-1)


n3.fit(df_raw,y_train_cc)
n4.fit(df_raw,y_train_ft)


n3_score = n3.score(df_raw,y_train_cc)
n4_score = n4.score(df_raw,y_train_ft)

pred1 = n3.predict(x_test)
pred2 = n4.predict(x_test)
x_test.shape
submission = pd.DataFrame({'ForecastId': test_id,'ConfirmedCases':pred1,'Fatalities':pred2})
