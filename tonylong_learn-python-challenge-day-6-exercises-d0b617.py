...
def word_search(doc_list, keyword):
    ...
    for i in range(len(doc_list)):
        word_list = doc_list[i].lower().split()
        word_list_striped = [w.strip('.?!#$%^&*,') for w in word_list]
        if(keyword.lower() in word_list_striped):
...
def conditional_roulette_probs(history):
    ...
    for i in range(len(history) -1) :
        ...
        if(predict.get(cur_num) == None):
            predict[cur_num] = {}
        if(predict[cur_num].get(next_num) == None):
            predict[cur_num][next_num] = 1
        else:
            predict[cur_num][next_num] += 1

    for k,v in predict.items():
        cnt = sum(predict[k].values())
        for sub_key in v.keys():
            predict[k][sub_key] = predict[k][sub_key] / cnt
