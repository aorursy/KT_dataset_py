...
np.random.seed(0)
kickstarters_2017
# generate 1000 data points randomly drawn from an exponential distribution (指数分布)
...
# get the index of all positive pledged (Box-Cox only takes postive values)
index_of_positive_pledged = kickstarters_2017.pledged > 0

# get only positive pledged (using their indexes)
positive_pledged = kickstarters_2017.usd_pledged_real.loc[index_of_positive_pledged]
...
sns.distplot(normalized_pledges, ax=ax[1])
ax[1].set_title("normalized_pledged data")