...
first_goals_ids=[]
def is_first_goal(row):
    if row['is_goal'] > 0:
        if row['id_odsp'] not in first_goals_ids:
            first_goals_ids.append(row['id_odsp'])
            row['first_goal'] = 1
        else:
            row['first_goal'] = 0
    else:
        row['first_goal'] = 0
    return row
events = events.apply(is_first_goal, axis=1)
events.info()
goals2 = events[events['first_goal'] == 1] # events where a goal resulted

# When do the goals occur?
plt.hist(goals2.time, 100)
plt.xlabel("Time")
plt.ylabel("Number of Goals at Time")
plt.title("When First Goals Occur")
plt.show()
