...
# seaborn kütühanesindeki plot tiplerini buradan görebiliriz
...
train_df[["Parch","Pclass"]].groupby(["Parch"], as_index = False).mean()
...
g = sns.FacetGrid(train_df, col = "Survived", row="Parch", size = 3.5)
...
plt.show()
g = sns.FacetGrid(train_df, col = "Survived", row = "Pclass", size = 3.5)
...
list(train_df["Age"][train_df["Age"].isnull()].index)
train_df.head(10)
name=train_df["Name"]
...
train_df.head()
train_df.head()
train_df["Fsize"] = train_df["SibSp"] + train_df["Parch"] + 1 # Sibling Number + Parent-Children Number
...
plt.show()
train_df = pd.get_dummies(train_df, columns= ["family_size"]) # family_size 2 feature'a bölünecek.

train_df.head()
train_df["Embarked"].head()
sns.countplot(x=train_df["Embarked"], data=train_df)
...
# strip komutu ile baştaki ya da sondaki ekstra boşluk giderilir
