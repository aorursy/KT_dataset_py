...
rootPath = "/kaggle/input/tcsemotion/emotion/"

trainPath = "/kaggle/input/tcsemotion/emotion/train/"

os.listdir(rootPath)
images = os.listdir(trainPath)

labels = pd.read_csv(rootPath+'emotion_dataset.csv')
labels.head()
labels['emotion'].unique()
labels['emotion'].hist(bins=4)
data = labels.values
...
X_train, y_train = prepareData(labels, trainPath, 4)
...
def new_model(input_shape):
    ...
    emotion = Dense(4, activation='softmax')(x)
...
plt.show()
model.save('tcsemotion.h5')
model_yaml = model.to_yaml()

with open("model_emotion.yaml", "w") as yaml_file:
