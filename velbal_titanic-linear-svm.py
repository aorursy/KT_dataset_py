...
# We adopted the hyper parameter, p_c=64.0 in SVM with rbf kernel
# See the link: https://www.kaggle.com/velbal/titanic-svm-with-grid-search
p_c=64.0
print(p_c)
# Model construction with the optimized parameter: c
model=svm.SVC(kernel='linear',C=p_c) 
model.fit(x_train,y_train)
# Predicting training data with the model we have constructed 
yp_train=model.predict(x_train)
accuracy=metrics.accuracy_score(y_train,yp_train)
confusin=metrics.confusion_matrix(y_train,yp_train)
print(accuracy)
print(confusin)
# Predicting test data with the model we have constructed 
