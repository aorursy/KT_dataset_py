...
def model_olustur2(isim,model,x,y):
    ...
    print(isim," Algoritması başarım sonucu: ",np.mean(cross_val_score(model,x,y,cv=5)))
...
plt.title('Credit Card fraud Detection Dataset ile Sınıflandırma',fontsize = 20,color='blue')
...
def graph_roc_curve_multiple(log_fpr, log_tpr, knear_fpr, knear_tpr, svc_fpr, svc_tpr, tree_fpr, tree_tpr,rfc_fpr,rfc_tpr,nb_fpr,nb_tpr,gbc_fpr,gbc_tpr,abc_fpr,abc_tpr,ysa_fpr,ysa_tpr):
    ...
    plt.plot(log_fpr, log_tpr, label='Logistic Regression Classifier Score: {:.4f}'.format(roc_auc_score(y, cross_val_predict(lrc,x,y,cv=5))))

    plt.plot(knear_fpr, knear_tpr, label='KNears Neighbors Classifier Score: {:.4f}'.format(roc_auc_score(y, cross_val_predict(knn,x,y,cv=5))))

    plt.plot(svc_fpr, svc_tpr, label='Support Vector Classifier Score: {:.4f}'.format(roc_auc_score(y, cross_val_predict(svc,x,y,cv=5))))

    plt.plot(tree_fpr, tree_tpr, label='Decision Tree Classifier Score: {:.4f}'.format(roc_auc_score(y, cross_val_predict(dtc,x,y,cv=5))))

    plt.plot(rfc_fpr, rfc_tpr, label='Random Forest Classifier Score: {:.4f}'.format(roc_auc_score(y, cross_val_predict(rfc,x,y,cv=5))))

    plt.plot(nb_fpr, nb_tpr, label='Naive Bayes Classifier Score: {:.4f}'.format(roc_auc_score(y, cross_val_predict(nb,x,y,cv=5))))

    plt.plot(gbc_fpr, gbc_tpr, label='Gradient Boosting Classifier Score: {:.4f}'.format(roc_auc_score(y, cross_val_predict(gfc,x,y,cv=5))))

    plt.plot(abc_fpr, abc_tpr, label='AdaBoosting Classifier Score: {:.4f}'.format(roc_auc_score(y, cross_val_predict(abc,x,y,cv=5))))

    plt.plot(ysa_fpr, ysa_tpr, label='Neural Network Score: {:.4f}'.format(roc_auc_score(y, cross_val_predict(ysa,x,y,cv=5))))
