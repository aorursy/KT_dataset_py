...
img_shape = img.shape
img_shape
img, label = dataset[0]#play with it
...
for i in range(len(dataset)):
    _,label = dataset[i]
    if classes[label] not in cl.keys():
        cl[classes[label]] = 1
    else:
        cl[classes[label]]+=1
cl
...
for images, _ in train_loader:
    ...
    plt.axis('off')#on it
    plt.imshow(make_grid(images, nrow=16).permute((1, 2, 0)))#play with nrow & Figsize
    break
...
class CIFAR10Model(ImageClassificationBase):
    ...
    def forward(self, xb):
        ...
        out = F.relu(out)
        our = self.linear3(out)
        return out
model = to_device(CIFAR10Model(input_size,out_size=output_size,hidden_size=155), device)
history = [evaluate(model, val_loader)]
history
history += fit(10,1e-1, model, train_loader, val_loader)
history
history += fit(20, 1e-2, model, train_loader, val_loader)
history += fit(36, 1e-4, model, train_loader, val_loader)
history += fit(12, 1e-3, model, train_loader, val_loader)
