...
raw_df = pd.read_csv("../input/celsius-to-fahrenheit/CelsiustoFahrenheit.csv")
df = raw_df.copy()
df.shape
df.head()
df.tail()
...
model.summary()
model.compile(optimizer=Adam(0.1), loss='mse', metrics = ['mae', 'mse'])
...
pred.shape
test_df = pd.DataFrame(pred, columns = ['pred'])
actual = 9/5 * X + 32
actual.shape
