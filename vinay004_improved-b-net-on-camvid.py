...
def load_image_test():
  ...
  test_batch = sorted(os.listdir(test_data))
  test_label_batch = sorted(os.listdir(test_label))
...
def upsample(filters , size, Batch_Norm = False, apply_dropout = False):
    result = Sequential()
    ...
    result.add(LeakyReLU(alpha=0.1))
    return result
...
model = BNETv2(output_channels = 3)
...
checkpoint2 = ModelCheckpoint('bnetv2_categorical.hdf5',
                                 monitor='val_loss',
                                 verbose=1,
                                 save_best_only=True,
                                 mode='auto')
...
def create_mask(pred_mask):
  ...
  return pred_mask[0]
pred_mask = model.predict(np.expand_dims(X_train[4], axis=0))
display([X_train[4], y_train[4], create_mask(pred_mask)])
...
checkpoint2 = ModelCheckpoint('bnetv2_tversky.hdf5',
                                 monitor='val_loss',
                                 verbose=1,
                                 save_best_only=True,
                                 mode='auto')
with tf.device('/device:GPU:0'):
    history_tversky = model.fit(np.array(X_train), np.array(y_train),
                        batch_size=batch_size,
                        epochs=epochs,
                        callbacks = [checkpoint2],
                        validation_data=(np.array(X_val), np.array(y_val)))
model.load_weights('bnetv2_tversky.hdf5')
X_test,y_test = load_image_test()
print('\n# Evaluate on test data')
results = model.evaluate(np.array(X_test),np.array(y_test),batch_size=128)
print('test loss, test acc:', results)
...
ax = plt.subplot(211)
ax.set_facecolor('#F8F8F8')
ax.plot(history_tversky.history['loss'])
ax.plot(history_tversky.history['val_loss'])
ax.set_title('model')
ax.set_ylabel('BNET')
#ax.set_ylim(0.28,1.05)
ax.set_xlabel('epoch')
...
ax2 = plt.subplot(212)
ax2.set_facecolor('#F8F8F8')
ax2.plot(history_tversky.history['accuracy'])
ax2.plot(history_tversky.history['val_accuracy'])
ax2.set_title('model ')
ax2.set_ylabel('BNET')
#ax.set_ylim(0.28,1.05)
ax2.set_xlabel('epoch')
ax2.legend(['train', 'valid.'])
pred_mask = model.predict(np.expand_dims(X_test[4], axis=0))
display([X_test[4], y_test[4], create_mask(pred_mask)])
