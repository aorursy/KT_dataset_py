import pandas as pd

import numpy as np

import seaborn as sns

import matplotlib.pyplot as plt

%matplotlib inline





google_data = pd.read_csv('/kaggle/input/google-play-store-apps/googleplaystore.csv') 
type(google_data)
google_data.head(10)
google_data.tail(10)      
google_data.shape


google_data.describe()       # Summary Statistics
google_data.boxplot()
google_data.hist()
google_data.info()
google_data.isnull()
# Count the number of missing values in each column

google_data.isnull().sum()
google_data[google_data.Rating > 5]
google_data.drop([10472],inplace=True)
google_data[10470:10475]
google_data.boxplot()
google_data.hist()
threshold = len(google_data)* 0.1 #10% of (my rows = 10840)

threshold


google_data.dropna(thresh=threshold, axis=1, inplace=True) 
print(google_data.isnull().sum())
google_data.shape
#Define a function impute_median

def impute_median(series):

    return series.fillna(series.median())
google_data.Rating = google_data['Rating'].transform(impute_median) 
#count the number of null values in each column

google_data.isnull().sum()
# modes of categorical values

print(google_data['Type'].mode())

print(google_data['Current Ver'].mode())

print(google_data['Android Ver'].mode())
# Fill the missing categorical values with mode

google_data['Type'].fillna(str(google_data['Type'].mode().values[0]), inplace=True)

google_data['Current Ver'].fillna(str(google_data['Current Ver'].mode().values[0]), inplace=True)

google_data['Android Ver'].fillna(str(google_data['Android Ver'].mode().values[0]), inplace=True)
#count the number of null values in each column

google_data.isnull().sum()


### Let's convert Price, Reviews and Ratings into Numerical Values

google_data['Price'] = google_data['Price'].apply(lambda x: str(x).replace('$', '') if '$' in str(x) else str(x))

google_data['Price'] = google_data['Price'].apply(lambda x: float(x))

google_data['Reviews'] = pd.to_numeric(google_data['Reviews'], errors='coerce')
google_data['Installs'] = google_data['Installs'].apply(lambda x: str(x).replace('+', '') if '+' in str(x) else str(x))

google_data['Installs'] = google_data['Installs'].apply(lambda x: str(x).replace(',', '') if ',' in str(x) else str(x))

google_data['Installs'] = google_data['Installs'].apply(lambda x: float(x))
google_data.head(10) 
google_data.tail(10)
google_data.describe()  #Summary Stats After Cleaning
grp = google_data.groupby('Category')

x = grp['Rating'].agg(np.mean)

y = grp['Price'].agg(np.sum)

z = grp['Reviews'].agg(np.mean)

print(x)

print(y)

print(z)
plt.figure(figsize=(12,5))

plt.plot(x, 'ro')

plt.xticks(rotation=90)

plt.show()

plt.figure(figsize=(16,5))

plt.plot(x,'ro', color='r')

plt.xticks(rotation=90)

plt.title('Category wise Rating')

plt.xlabel('Categories-->')

plt.ylabel('Rating-->')

plt.show()
plt.figure(figsize=(16,5))

plt.plot(y,'r--', color='b')

plt.xticks(rotation=90)

plt.title('Category wise Pricing')

plt.xlabel('Categories-->')

plt.ylabel('Prices-->')

plt.show()
plt.figure(figsize=(16,5))

plt.plot(z,'g^', color='g')

plt.xticks(rotation=90)

plt.title('Category wise Reviews')

plt.xlabel('Categories-->')

plt.ylabel('Reviews-->')

plt.show()
f,ax = plt.subplots(figsize = (5,5))

sns.heatmap(google_data.corr(), annot = True, fmt = '.2f', ax = ax)

plt.show()

google_data['Category'] = google_data.index
google_data['Reviews_int'] = google_data.Reviews.astype(int)

f,ax = plt.subplots(figsize = (5,5))

sns.heatmap(google_data.corr(), annot = True, fmt = '.2f', ax = ax)

plt.show()
#Number of free and paid Apps available

plt.figure(figsize=(10,10))

plt.subplot(1,2,1)

sns.countplot(x='Type',data=google_data)

plt.title("Number of Apps Available: Free v/s Paid")



#Most installed apps based on Category

plt.subplot(1,2,2)

sns.barplot(x='Type',y='Installs',data=google_data,ci=None)

plt.title("Number of Apps installed: Free v/s Paid")

plt.tight_layout()
#Ratings of Apps and the number of installed

plt.figure(figsize=(15,15))

sns.barplot(y='Installs',x='Rating',data=google_data,ci=None)

plt.xticks(rotation=45)

plt.title("Number of Apps and ratings ")
#Checking if there are any missing values

sns.heatmap(google_data.isna())
google_data.sample()
google_data.isna().sum()
google_data.sample(5)
rating= google_data['Rating'].dropna()

size= google_data['Size'].dropna()

installs= google_data['Installs'][google_data.Installs!=0].dropna()

reviews= google_data['Reviews'][google_data.Reviews!=0].dropna()

types = google_data['Type'].dropna()

price = google_data['Price']



data = pd.concat([rating, size, np.log(installs), np.log10(reviews), types, price], axis=1)



plt.figure(figsize=(15,15))

sns.pairplot(data, hue='Type', palette="husl", markers= ['s','d'])

plt.show()
plt.figure(figsize=(13,13))

sns.jointplot(x=data['Reviews'], y=data['Installs'], kind='reg', color='blue')

plt.show()
len(google_data.Category.value_counts().sort_values(ascending=False).index)