...
class SigmoidNeuronMy:
  ...
  def optimumValue(self , X , Y, loss_fn="mse"):
        ...
        lr = np.linspace(0, 0.05, 6)

        epochs = np.linspace(0, 100, 11)

        #lr_mesh , epochs_mesh = np.meshgrid(lr,epochs)

        #loss = np.zeros(lr_mesh.shape)
        ...
        for j in tqdm_notebook(range(len(lr)), total=len(lr), unit="lr"):
            ...
            for i in tqdm_notebook(range(len(epochs)), total=len(epochs), unit="epoch"):
                ...
                for k in range (4):
                    ...
                    self.fit(X1_train, y1_train, epochs=epochs[i].astype("int")+1, learning_rate=lr[j], loss_fn="ce", display_loss=False)
        ...
        #ax.contour3D(lr_mesh,epochs_mesh,loss,500,cmap='viridis')
