...
test_img = train_files[:1000]

test_mask = mask_files[:1000]
...
df3 = pandas.DataFrame(data={"filename": test_img, 'mask' : test_mask})
...
for k, (train_index, test_index) in enumerate(kf.split(df2)):
    ...
    model.compile(optimizer=Adam(lr=1e-6), loss={'seg':dice_coef_loss, 'class':'binary_crossentropy'}, \
...
arr = abs(selectors - np.mean(selectors))

model_selector = np.argmin(arr)



model = load_model(str(model_selector+1) + '_unet_brain_mri_seg.hdf5', custom_objects={'dice_coef_loss': dice_coef_loss, 'iou': iou, 'dice_coef': dice_coef})

    

test_gen = train_generator(df3, BATCH_SIZE,

                            dict(),

                            target_size=(im_height, im_width))

best_result = model.evaluate(test_gen, steps=len(df3) / BATCH_SIZE)

best_result = dict(zip(model.metrics_names,best_result))
