...
# look at the # of missing points in the first ten columns (Shows 'TimeSecs')
missing_values_count[0:10]
# look at the # of missing points in the sorted series (Shows 'PenalizedTeam')
missing_values_count.sort_values(ascending=False)[10:20]
missing_values_count_sf[['Street Number Suffix', 'Zipcode']]
# 'Street Number Suffix' is an optional value, so it could be missing because it don't exist.
# 'Zipcode' should exist every time, so it is missing because it wasn't recorded.
sf_permits.sample(1)
...
print("Left data after dropna(): %d" % sf_permits.dropna().shape[0])
