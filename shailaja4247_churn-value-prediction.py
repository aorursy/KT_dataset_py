...
fig = go.Figure(data=plot_data, layout=plot_layout)
pyoff.iplot(fig)
#Elbow mthod to find out optimum number of clusters in K-Means
sse={}
...
#Trying to run K-Means is giving an error, convert string to float, hence checking for null values first.
...
#Convert rows with blank value for Total Charges as NAN columns
df_data.loc[pd.to_numeric(df_data['TotalCharges'], errors='coerce').isnull(),'TotalCharges'] = np.nan
df_data = df_data.dropna()
df_data['TotalCharges'] = pd.to_numeric(df_data['TotalCharges'], errors='coerce')
#Elbow method to determine number of clusters for K-Means
sse={}
