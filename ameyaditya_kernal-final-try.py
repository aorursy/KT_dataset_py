...
# and for max pooling        MaxPBlock(no of layers , model)
    model.add(Conv2D(32,(3,3),input_shape=(104,104,3),activation='relu',padding='same'))
    model.add(MaxPooling2D(pool_size=(2,2), strides=(2,2),padding='same'))
...
def process_images(path):
    ...
    img = imresize(img,(104,104))
    #print(img.shape)
    #img = img[:,:,0]
    img = img.reshape(104,104,3)
    arr = np.zeros(shape=(1,104,104,3))
    arr[0] = img
    return arr
path = fetcher.train_path + '/' + fetcher.training_images_paths[1760]
...
imgp = imgp.reshape((104,104,3)) # input size here
...
predictions = model.predict_generator(TestBatchGenerator(fetcher),
                       val_samples = len(fetcher.test_images_paths),
                        max_q_size = 32)
# Code to see accuracy on training images
# noofpred = 0
