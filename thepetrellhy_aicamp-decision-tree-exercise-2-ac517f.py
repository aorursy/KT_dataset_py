...
%matplotlib inline
import os
#os.environ['PATH'] += os.pathsep + 'D:/Lib/Anaconda3_python36/pkgs/graphviz-2.38.0-4/Library/bin/graphviz'
import pandas as pd
import numpy as np
...
graph
# TODO 观察前20个特征的重要性
pd.concat([pd.DataFrame(trainX.iloc[:, 1:].columns, columns=['variable']), pd.DataFrame(model.feature_importances_, columns=['importance'])], axis=1).sort_values(by='importance', ascending=False)[:20]
