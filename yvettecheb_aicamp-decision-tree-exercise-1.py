...
Y = np.array([0] * (N//4) + [1] * (N//4) + [1] * (N//4) + [1] * (N//4))
...
DO NOT edit code below
"""
plt.scatter(X[:,0], X[:,1], s=100, c=Y, alpha=0.5) #c is color
plt.show()
# 将模型分别在X, Y数据上fit
#TODO
model.fit(X,Y)
...
print("score for Logistic Regression:", model_LG.score(X, Y))
plt.scatter(X[:,0], X[:,1], s=100, c=Y, alpha=0.5)
plot_decision_boundary(X,model_LG)
plt.show()
