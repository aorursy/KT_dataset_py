...
def estimate_average_slot_payout(n_runs):
    ...
    for i in range(n_runs):
        earn = earn + play_slot_machine()
    return (earn - n_runs*1)/n_runs
...
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    for i in range(n_simulations):
        money_whole = start_balance
        times = 0
        while money_whole >= 1 and times < n_spins:
            money_whole = money_whole + play_slot_machine() - 1
            times = times + 1
        if times == n_spins:
            n_succ = n_succ + 1
    return n_succ/n_simulations
