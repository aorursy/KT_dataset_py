...
def estimate_average_slot_payout(n_runs):
    ...
    """
    # (price, num_trials)
    # weights = [(1.5,5),(5,25),(25,100)]
    price_money = 0
    for run in range(n_runs):
        price_money = price_money + play_slot_machine();
    return (price_money - n_runs)/n_runs;
...
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    for sim in range(n_simulations):
        remaining_bal = start_balance;
        runs = 0
        while remaining_bal >= 1 and runs < n_spins:
            remaining_bal = remaining_bal -1;
            price_money = play_slot_machine();
            remaining_bal = remaining_bal + price_money;
            runs = runs + 1;
        completed_runs = completed_runs + (1 if runs == n_spins else 0);
    return completed_runs/n_simulations;
q5.check()
#q5.solution()