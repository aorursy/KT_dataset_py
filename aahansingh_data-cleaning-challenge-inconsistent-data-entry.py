...
# Bonus
# Instead of doing the replacement by hand we can automate so that all the columns get fixed at the same time.
for city in suicide_attacks['City'].unique():
    replace_matches_in_column(df=suicide_attacks, column='City', string_to_match=city)
cities = suicide_attacks['City'].unique()
cities.sort()
cities
missing_percentage = suicide_attacks.isna().sum() / suicide_attacks.shape[0]
print('Percentage of missing values:\n',missing_percentage)
suicide_attacks = suicide_attacks.fillna('NA')
for hos in suicide_attacks['Hospital Names'].unique():
    replace_matches_in_column(df=suicide_attacks, column='Hospital Names', string_to_match=hos,min_ratio=80)
hospitals = suicide_attacks['Hospital Names'].unique()
hospitals.sort()
hospitals