...
#create dictionary of candidate for multiple pie charts -- not completed
for candidate in range(1,len(sentarray)):
    if sentarray[candidate][0] != '': #if row doesn't have an empty candidate name value 
        print(sentarray[candidate]) 
        
print(tot)
print(sentiment_by_candidate[:5])
print(sentarray[:5])
print(sum(Trumpsent[1:]))
...
plt.axis('equal')
plt.title('Sentiment for Donald Trump - Will this be the end of America as we know it?...')
plt.show()
#fig = plt.figure() ##The following lines do not render an image; therefore, have commented out...
#fig = plt.figure()
