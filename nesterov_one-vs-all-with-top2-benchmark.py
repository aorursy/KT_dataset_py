...
for _, row in tqdm_notebook(df_target_train.iterrows(), total=df_target_train.shape[0]):
...
for g_id, positive_samples in tqdm.notebook.tqdm(genre2tracks.items()):
...
def get_test(k=1.0):
    g_prediction = {}
    for g_id, d in tqdm.notebook.tqdm(models.items()):
