...
full_df = pd.concat([titanic_df,test_df])
...
test_df.info()
#Processing missing values:Age,Cabin,Embarked,Fare,

#Embarked:replace missing values with mode

full_df.Embarked[full_df.Embarked.isnull()] = full_df.Embarked.dropna().mode().values
#Embarked
...
titanic_df['Sex'].loc[titanic_df['Sex'] == 'female'] = 1

titanic_df['Sex'].loc[titanic_df['Sex'] == 'male'] = 0
...
sns.barplot(x='Sex', y='Survived', data=sex_perc, ax=axis2, order=[1,0])
# Pclass
