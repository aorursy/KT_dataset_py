...
for i in tuned_parameters:
    ...
    lr=LogisticRegression(C=i, penalty='l1', class_weight= {0:1, 1:5})
...
for i in tuned_parameters:
    ...
    lr=LogisticRegression(C=i, penalty='l2', class_weight= {0:1, 1:5})
...
plt.show()
# Training the model using best C    
...
for i in range(len(all_features)):
    ...
    d.update({all_features[i]:weight_vect[i]})
...
x.field_names = ["Vectorizer", "Regularization", "Hyperameter", "AUC"]

x.add_row(["BOW","L1",C1,auc1])

x.add_row(["BOW","L2",C2,auc2])

x.add_row(["TFIDF","L1",C3,auc3])

x.add_row(["TFIDF","L2",C4,auc4])

x.add_row(["AwgW2V","L1",C5,auc5])

x.add_row(["AwgW2V","L2",C6,auc6])

x.add_row(["TFIDF W2V","L1",C7,auc7])

x.add_row(["TFIDF W2V","L2",C8,auc8])
