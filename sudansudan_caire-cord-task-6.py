...
run_example(query)
query = "What do we know about the effectiveness of non-pharmaceutical interventions for coovid-19? "
run_example(query)
query = "What is known about equity and barriers to compliance for non-pharmaceutical interventions for covid-19?"
run_example(query)
query = "How can we scale up covid-19 NPIs in a more coordinated way to give us time to enhance our health care delivery system capacity to respond to an increase in covid-19 cases?"
run_example(query)
query = "How can we scale up covid-19 NPIs by establishing funding to give us time to enhance our health care delivery system capacity to respond to an increase in covid-19 cases?"
run_example(query)
query = "How can we scale up covid-19 NPIs using infrastructure and authorities to support real time, authoritative (qualified participants) collaboration with all states to gain consensus on consistent guidance to give us time to enhance our health care delivery system capacity to respond to an increase in covid-19  cases?"
run_example(query)
query = "How can we scale up covid-19 NPIs by mobilizing resources to geographic areas where critical shortfalls are identified to give us time to enhance our health care delivery system capacity to respond to an increase in covid-19 cases?"
run_example(query)
query = "How can we do rapid design and execution of experiments to examine and compare NPIs currently being implemented for covid-19? "
run_example(query)
query = "How DHS Centers for Excellence could be leveraged to conduct the NPIs experiments for covid-19?"
run_example(query)
query = "What do we know about the efficacy of school closures, travel bans, bans on mass gatherings of various sizes, and other social distancing approaches for covid-19?"
run_example(query)
query = "What are the methods to control the spread in communities, barriers to compliance and how these vary among different populations for covid-19?"
run_example(query)
query = "What are the models of potential interventions to predict costs and benefits that take account of such factors as race, income, disability, age, geographic location, immigration status, housing status, employment status, and health insurance status for covid-19?"
run_example(query)
query = "Which Policies changes are necessary to enable the compliance of individuals with limited resources and the underserved with NPIs for covid-19?"
run_example(query)
query = "Why people fail to comply with public health advice for covid-19, even if they want to do so (e.g., social or financial costs may be too high)?"
run_example(query)
query = "What is the economic impact of covid-19 or any pandemic? "
run_example(query)
query = "What are policy and programmatic alternatives that lessen/mitigate risks to critical government services, food distribution and supplies regardless of ability to pay for covid-19? "
run_example(query)
query = "What are policy and programmatic alternatives that lessen/mitigate risks to access to health diagnoses, treatment, and needed care, regardless of ability to pay for covid-19?"
run_example(query)