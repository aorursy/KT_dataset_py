...
js = """<script>
 ...
 var divTag1 = document.getElementsByClassName("input")[4]
   ...
   var divTag2 = document.getElementsByClassName("input")[5]

   divTag2.style.display = 'none';</script>
...
js = """<script>
 ...
 var divTag1 = document.getElementsByClassName("input")[10]
   ...
   var divTag2 = document.getElementsByClassName("input")[11]

   divTag2.style.display = 'none';

   

   var divTag2 = document.getElementsByClassName("input")[12]

   divTag2.style.display = 'none';

   

   var divTag2 = document.getElementsByClassName("input")[13]
...
display(HTML(js))
tp = bdrates[['country_name','year','crude_birth_rate']].head()
pop_age =pd.read_csv('../input/midyear_population_age_sex.csv')

pop_age.head()
codes = [countries[country] if country != 'I prefer not to say' else None for country in pop_age['country_name']]
...
for row in range(0,pop_age.shape[0]):

    #print("{}".format(row), end = "\r")
    ...
    for k in range(0,10):
        ...
        less10 += pop_age.iloc[row]['population_age_'+str(k)]

    for k in range(60,100):

        more60 += pop_age.iloc[row]['population_age_'+str(k)]

    cless10.append(less10)

    cmore60.append(more60)

pop_age['less10']=cless10

pop_age['more60']=more60



min_pop_age = pop_age[['code','less10']]

less10_group = min_pop_age.groupby('code').count()



min_pop_age = pop_age[['code','more60']]

more60_group = min_pop_age.groupby('code').count()

gdata = [ dict(

        type = 'choropleth',

        locations = less10_group.index,

        z = less10_group['less10'],

        text = less10_group.index,
...
layout = dict(
    ...
    title = 'Pop - Younger than 10 years ',
...
gdata = [ dict(
        ...
        locations = more60_group.index,

        z = more60_group['more60'],

        text = more60_group.index,
