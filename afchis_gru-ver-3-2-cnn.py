...
for j in range(28):
    ...
    for k in range(28):
        ...
        for i in range(j):
            ...
            data_y1 = torch.cat((data_y1, one), 0)

        for i in range(28-j):

            data_y1 = torch.cat((data_y1, zero), 0)
...
data_y1.reshape(1, 22737)



data_y2 = torch.zeros(1)
...
for j in range(28):
    ...
    for k in range(28):
        ...
        for i in range(j):
            ...
            data_y2 = torch.cat((data_y2, zero), 0)

        for i in range(28-j):

            data_y2 = torch.cat((data_y2, one), 0)
...
data = torch.cat((data_y1, data_y2), 0).view(2, 22737)
...
class MyDataset(Dataset):
    ...
    def __init__(self):
        ...
        self.x_data = data[:, 1:].view(2, 29, 28 * 28)

        self.y_data = data[:, [0]].reshape(2).long()
...
hidden_zeros =  torch.zeros(1, 4, 28, 28).to(device)
...
for epoch in range(150):
    ...
    for i, data in enumerate(train_loader):
        ...
        for time in range(sequence_size - 1):

            #print('input: ', input[0, j])

            if time == 0:

                #print(input[0, j].shape)

                hidden, output = model(input[0, time], hidden_zeros)

            else:

                hidden, output = model(input[0, time], hidden)

            #print(j, output, label)
