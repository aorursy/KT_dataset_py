...
fig.update_layout(title_text='Remained Confirmed Cases per country of each day', title_x=0.5)
fig.show()
df = pd.read_csv('../input/nepal-earthquake/query.csv',header=0)

df.index= pd.to_datetime(df['time'])
df['time']=df.index.strftime('%Y-%m-%d %H:00:00')
fig=px.scatter_geo(df,lat='latitude',lon='longitude',color='mag',animation_frame='time',color_continuous_scale='Oranges',range_color=(5.,7.))
fig.show()
