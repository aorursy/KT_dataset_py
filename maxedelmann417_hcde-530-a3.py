...
all_names = {} #I wanted to create a dictionary that included all names in the dataset since the question is asking for the longest name regardless of gender
...
for line in open(NAMES_LIST, 'r').readlines():
    ...
    name, gender, count = line.strip().split(",") #as in the first example, this separated the variables



    if gender == "F" or "M": #This put all names into the new dictionary with its count

        all_names[name.capitalize()] = count 
...
longest_name = max(all_names, key=len) #this found the item with the highest value, the value being the length of the string
...
x = input("Enter capitalized prefix: ") #This created an input called x that would be the prefix that was being searched 



for name in all_names.keys(): #this created a for loop that would look for x in the all_names dictionary. At first I was confused as to how to make sure the items being returned were prefixes as opposed to being in another part of the word for example "ma" could be in both "Madison" or "Emma"

    if x in name[0:]: #my solution was to capitalize the names which would require the user to assign an order to the letters being searched. For example "Ma" would return "Madison" but not "Emma" due to the capitalization

        print(name + ": There were " + str(all_names[name]) + " babies named " + name)
NAMES_LIST = "/kaggle/input/a3data/yob2010.txt"



boys = {} #For this question the genders were being compared so I needed two separate dictionaries 
...
for line in open(NAMES_LIST, 'r').readlines():
    ...
    if gender == "F":  
        ...
        girls[name.capitalize()] = count 

    elif gender == "M":

        boys[name.capitalize()] = count
...
x = input("Please enter a name: ") #I set the input as x



for name in boys.keys(): #I made two for loops with nested conditionals. If x was equal to the input, it would return a statement for both boys and girls. 

    if name == x:

            print("There are " + str(boys[name]) + " boys named " + name)

            break

else: #If there were no names that were equal to the input, it would return a statement declaring that there were no matches.

    print("There are no boys named " + x)
...
for name in girls.keys():
    ...
    if name == x:
            ...
            print("There are " + str(girls[name]) + " girls named " + name)

            break

else:

    print("There are no girls named " + x)
