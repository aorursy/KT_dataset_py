    ...
    while clusterChanged:
        ...
        for i in range(numSamples):
            ...
            for j in range(k):
                ...
                Dist = euclDistance(centroids[j,:],dataSet[i,:])
        ...
        for cent in range(k):
            ...
            pointsInCluster = dataSet[np.nonzero(clusterAssment[:,0].A == cent)[0]]

            centroids[cent,:] = np.mean(pointsInCluster,axis = 0)

        if Iter <= 5 :

            print("show the iteraions = %d of my_k_means" % Iter)
