...
def displayExample(i):

    # Re-shape image into 28x28 pixels

    disp_image = X_all_data[i].reshape(28,28)

    # Display image using matplotlib

    fig.add_subplot(28, 28, i)

    plt.imshow(disp_image)

    print("Label: ",y_all_data[i])
...
for i in [1,2,3]:
    ...
    displayExample(i)
# Function to stop training once desired accuracy is achieved

desiredAccuracy = 0.999
...
class myCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if(logs.get('acc') >= desiredAccuracy):

            print(' DESIRED ACCURACY (' + str(desiredAccuracy) + ') REACHED')
