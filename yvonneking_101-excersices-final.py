...
# Write a function definition named circeumferenc that takes in a number representing a circle's radius and returns the circumference.
...
def has_vowels(word):
    ...
    return any (char in word for char in vowels)
...
def get_values_in_common(lst1, lst2):
    ...
    return set(lst1) - (set(lst1) - set(lst2))
...
def total_of_book_prices(words):
...
def get_average_book_price(words):
...
def highest_price_book(words):

    max_price = max(word["price"] for word in words)

    book = {}

    for word in words:

        if word["price"] == max_price:
...
def lowest_priced_book(words):

    min_price = min(word["price"] for word in words)

    book = {}

    for word in words:

        if word["price"] == min_price:
...
def total_number_of_items(lst1):
...
def get_average_item_price(lst1):
...
def get_average_spent_per_item(lst1):
...
def most_spent_on_item(words):

    items = words["items"]

    most_spent = max(item["price"] * item["quantity"] for item in items)
