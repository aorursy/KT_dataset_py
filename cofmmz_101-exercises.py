...
def count_vowels(string):
    counter = 0 # initialize the count
    for letter in string:
...
def remove_vowels(string):
    counter = ""
    for letter in string:
        if letter.lower() and is_vowel(letter) == False:  
            counter += letter
    return counter
...
def starts_and_ends_with_vowel(string):
        if string[-1] and string[0] in ['a','e','i','o','u']:
             return True
        else:
            return False
...
def mean(seq_nums):
    return sum(seq_nums) / len(seq_nums)
...
def median(seq_nums):
    return statistics.median(seq_nums)
...
def mode(frequency):
    return statistics.mode(frequency)
...
def product_of_all(seq_nums):
     return numpy.prod(seq_nums)
...
def get_highest_number(seq_nums):
    return max(seq_nums)
...
def get_smallest_number(seq_nums):
    return min(seq_nums)
...
def only_odd_numbers(seq_nums):
       return [i for i in seq_nums 
                       if i % 2 == 1]
...
def only_even_numbers(seq_nums):
       return [i for i in seq_nums 
                           if i % 2 == 0]
...
def only_positive_numbers(seq_nums):
       return [i for i in seq_nums 
                           if i > 0]
...
def only_negative_numbers(seq_nums):
       return [i for i in seq_nums 
                            if i < 0]
...
def count_evens(i):
    return len(only_even_numbers(i))
...
def only_positive_evens(seq_num_list):
    list0 = []
    for num in seq_num_list:
...
def only_positive_odds(seq_num_list):
    list0 = []
    for num in seq_num_list:
        if num > 0 and num %2 != 0:
            list0.append(num)
...
def only_negative_evens(seq_num_list):
    list0 = []
    for num in seq_num_list:
        if num < 0 and num %2 == 0:
            list0.append(num)
...
def only_negative_odds(seq_num_list):
    list0 = []
    for num in seq_num_list:
        if num < 0 and num %2 != 0:
            list0.append(num)
...
def get_unique_values(list_unique):
...
def get_values_in_common(list0, list1):
    return set(list0).intersection(set(list1))
...
def get_values_not_in_common(list0, list1):
    return set(list0).symmetric_difference(set(list1))
...
def get_number_of_books(list_of_dicts):
    return len(books)
...
def total_of_book_prices(list_dicts):
    return sum([books["price"] for books in list_dicts])
...
def get_average_book_price(list_dicts):
    list_dicts = 0
    for item in books:
        list_dicts += item.get("price")
    return (list_dicts / len(books))
...
def lowest_price_book(library):
     return min(library, key = lambda j: j['price'])
...
def most_spent_on_item(cart):
    value = cart['items']
    return max(value, key = lambda j: j['price'] * j['quantity'])
