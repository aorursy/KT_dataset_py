...
import pandas as pd
import spacy
import en_core_web_sm # Assigns context-specific token vectors, POS tags, dependency parse and named entities.
nlp = en_core_web_sm.load()
#nlp = spacy.load('en_core_web_md')
print('Sanity test:')
doc = nlp(u'I am a good programmer!') #breaks into tokens
print(type(doc))
print([t for t in doc])
#train_set = pd.read_csv('../input/first-quora-dataset/q_quora.csv')
train_set = pd.read_csv('../input/quoranewds/q_quroranew.csv')
...
#     text = re.sub('[0-9]+', pad_number, text)
    ...
    #     else:#         replace the string with a special word, i.e. quoted_item
    # Testcase:
    ...
    text = re.sub(' s ', " ", text)# reduce extra spaces into single spaces
    text = re.sub('[\s]+', " ", text)
    text = text.strip()
...
print([spacy_obj_dict[1]])
print(type(spacy_obj_dict[1]))
print(spacy_obj_dict[2])
print(spacy_obj_dict[3])
print(spacy_obj_dict[10])
# Doc is basically a sequence of tokens
...
doc2 = nlp('Apple has launched a product!')
doc3 = nlp('U.K. has launched !')


print(doc1[1].text, '->', doc1[1].ent_type_) 
print(doc2[0].text, '->', doc2[0].ent_type_)
print(doc3[0].text, '->', doc3[0].ent_type_)

# if the second type count is extremely low, consider it as noise and the word has no second type
threshold_of_second_type = 3
#check
vote_dict = {}
#for token in spacy_obj_dict[1]:
for token in nlp("America is the country with Apple office in it"):
            
        if token.lower_ not in vote_dict:
            vote_dict[token.lower_] = {}
           # print(type(vote_dict[token.lower_]))
          #  print(vote_dict[token.lower_])
        ...
        vote_dict[token.lower_][token.ent_type_] += 1
for key in vote_dict:
  ...
  #  print(vote_dict[key].keys())
    ...
    print(bi_list[1])
    sorted_idx = np.argsort(bi_list[1]) # list of indices in sorted order
    print(sorted_idx)
    
    if sorted_idx.shape[0]>1: # dimension of row
        best_idx = sorted_idx[-1]
        second_idx = sorted_idx[-2]
        word_ent_type_dict[key] = bi_list[0][best_idx]
        print(word_ent_type_dict[key])
        if bi_list[1][second_idx]>threshold_of_second_type:
            word_ent_type_second_dict[key] = bi_list[0][second_idx]
            print(word_ent_type_second_dict[key])
    else:
        best_idx = sorted_idx[-1]
        word_ent_type_dict[key] = bi_list[0][best_idx]
        print(word_ent_type_dict[key])
...
for k in range(1,10):
    if k in spacy_obj_dict:
        s = process_question_with_spacy(spacy_obj_dict[k], show_fail=True, idx=k)
        print (s)
