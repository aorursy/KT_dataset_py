...
def wants_plain_hotdog(ketchup, mustard, onion):
    ...
    """
    return bool(not ketchup and not mustard and not onion)
...
def exactly_one_topping(ketchup, mustard, onion):
    ...
    on their hot dog.
    """
    return bool(int(ketchup) + int(mustard) + int(onion) ==1)
...
def should_hit(player_total, dealer_total, player_aces):
    ...
    state, or False if the player should stay. player_aces is the number of aces the player has.
    """
    return (True if player_total < dealer_total else False) and (True if player_aces <1 else False)
