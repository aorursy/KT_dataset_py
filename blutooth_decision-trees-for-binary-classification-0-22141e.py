...
sizeMeasurements.head()
# Question 1? Looking at the data, which features seem to show the most variation? Which do you think will be good for predicting cancer?  Why is it important
# to look at the data? 
...
# Question 2? What do you think a good train and test size is for data? Could this vary depending on the size of the data? 
# in this box we 
sns.set_style("whitegrid")
plotOne = sns.FacetGrid(sizeMeasurements, hue="target",aspect=2.5)
plotOne.map(sns.kdeplot,'mean smoothness',shade=True) 
plotOne.set(xlim=(0, sizeMeasurements['mean smoothness'].max()))
plotOne.add_legend()
plotOne.set_axis_labels('mean smoothness', 'Proportion')
plotOne.fig.suptitle('Smoothness vs Diagnosis (Blue = Malignant; Orange = Benign)')
plt.show()
...
# change the densityplot
plotOne.map(sns.kdeplot,'mean smoothness',shade=True)

# change - set the limits for the new quantity  
plotOne.set(xlim=(0, sizeMeasurements['mean smoothness'].max()))
...
# which features make for a good discriminator between the two? 

# What about correlation between features? Why is this important to consider? 
print("\nFeature Correlation:\n")
g = sns.heatmap(X_train.corr(),cmap="BrBG",annot=False)
# Include only certain features and build a classifier? 
...
X3=X
y3=y
# Use PCA to transform and de-correlate features 


variance_pct = 5 # num components 
...
# question 5: How many features should go into the model? Why should we decorrelate features? 
# change the number of components on the PCA by a
...
# what is our model actually doing? - how do you think we train a tree? 
...
# Question 6: what metric would you use to do a deicision trees splits based on what we have already covered. 
# summarise which features are most important 
...
# Why do we have a separate training and test?  Change the 