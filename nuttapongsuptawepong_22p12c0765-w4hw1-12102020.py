...
# ขอบคุณ code จาก https://www.kaggle.com/layyer/thai-mnist-lenet5
...
df_train_rule.head ( )
# เนื่องจากมีคน Share ไว้ในกลุ่ม Line จึงเอามาทดสอบว่า สมการนั้น ถูกต้องหรือไม่

# f1 = NaN => predict = f2 + f3
...
# f1 = 2 => predict = ( f2 + f3 ) * absolute ( f2 - f3 )
...
# f1 = 3 => predict = absolute ( ( ( f3 * ( f3 + 1 ) ) - ( f2 * ( f2 - 1 ) ) ) / 2 )
...
# f1 = 8 => predict = ( ( ( ( f2 ^ 2 ) + 1 ) * f2 ) + ( f3 * ( f3 + 1 ) ) ) % 99
...
df_train_rule_temp [ df_train_rule_temp [ "predict" ] != df_train_rule_temp [ "predict_calculated" ] ]
# ข้อมูลที่รูปไม่ตรงกับ category



# หลังจากสร้าง model เสร็จแล้ว ได้ลองนำ model มา predict ข้อมูล train

# จากนั้นไล่ดูรูปภาพที่ predict ได้ไม่ตรงกับข้อมูลจริง

# ซึ่งพบว่ามีหลายรูปภาพที่ข้อมูลไม่ตรงกับ category
...
df_submit_final
df_submit_final.to_csv ( "22p12c0765-Final2.csv", index = False )