...
class FashionTraining():
    ...
    def __init__(self, TrainingDataPath, valDataPath, numClassses, lr = 0.001, 
        ...
        self.ModelCkPt = ModelCheckpoint('fashionModel_v4.h5', monitor='val_loss', 
        ...
        self.epochs = epochs
        self.optimizer = Adam(self.LR,decay=1e-5)
        self.numCategories = numClassses
        self.model = None
        self.hist = None
        with open('FashionCategories.pkl','wb') as f:
            pickle.dump(self.train_generator.class_indices,f)
#-------------------------------------------------------------------            
    def preprocessing(self,images):
        ret = []
        #print('Len of images for preprocessing are : ', len(images))
 ...
 #------------------------------------------------------------------- 
    #defining the Generator for creating batches for Training
    def getTrainingData(self):
        tbatch = []
        while 1:
            x,y = list(self.train_generator.next())
            #Calling the preprocessing function for identifying the ROI
            tbatch = self.preprocessing(x)
            #Normalising the Images
            tbatch = np.array(tbatch)/255.0
            
            yield tbatch, y
            tbatch = []
        
 #------------------------------------------------------------------- 
    #defining the Generator for creating batches for Validation Images
    def getValidationData(self):
        vbatch = []
        while 1:
            x,y = list(self.validation_generator.next())
            #Calling the preprocessing function for identifying the ROI
            vbatch = self.preprocessing(x)
            #Normalising the Images
            vbatch = np.array(vbatch)/255.0
 ...
 #-------------------------------------------------------------------           
    def getModel(self, numClases):
        ...
        cnn4 = Sequential(name='1st')
        cnn4.add(Conv2D(32, kernel_size=(3, 3), activation='relu', input_shape=(100,100,3),name='1stConv'))
        cnn4.add(BatchNormalization(name='1stBatchNorm'))

        cnn4.add(Conv2D(32, kernel_size=(3, 3), activation='relu',name='2ndConv'))
        cnn4.add(BatchNormalization(name='2ndBatchNorm'))
        cnn4.add(MaxPooling2D(pool_size=(2, 2),name='1stpool'))
        cnn4.add(Dropout(0.25,name='drop1'))

        cnn4.add(Conv2D(64, kernel_size=(3, 3), activation='relu',name='2ndconv'))
        cnn4.add(BatchNormalization(name='3rdBatchNorm'))
        cnn4.add(Dropout(0.25,name='drop2'))

        cnn4.add(Conv2D(128, kernel_size=(3, 3), activation='relu',name='4Conv'))
        cnn4.add(BatchNormalization(name='4BatchNOrm'))
        cnn4.add(MaxPooling2D(pool_size=(2, 2),name='maxPool2'))
        cnn4.add(Dropout(0.25,name='drop4'))

        cnn4.add(Flatten(name='flat'))

        cnn4.add(Dense(1024, activation='relu',name='dense1'))
        cnn4.add(BatchNormalization(name ='5_BatchNorm'))
        cnn4.add(Dropout(0.25,name='drop5'))

        cnn4.add(Dense(1024, activation='relu',name='2Dense'))
        cnn4.add(BatchNormalization(name='6BatchNorm'))
        cnn4.add(Dropout(0.25,name='drop6'))

        cnn4.add(Dense(numClases, activation='softmax',name='LastLayer'))
        
        self.model = cnn4
        
        #self.model = Sequential()
        """input1 = Input(shape=(100,100,3),name='1stInput' )
        
        #1st convolution Layer
        conv1 = Conv2D(32, (3,3),activation = 'relu', kernel_initializer='he_uniform',
        ...
        maxpool0 = MaxPooling2D(pool_size=(2, 2),strides=(1,1),padding='same',name = 'pool0')(batchNorm1)
        drop1 = Dropout(0.25,name='dropout1')(maxpool0)
        #2nd convolution Layer
        conv2 = Conv2D(64, (3,3),activation = 'relu',kernel_initializer='he_uniform',
                                    kernel_regularizer=regularizers.l2(self.l2Lambda),
                                    data_format='channels_last',name='convlayer2')(drop1)
        batchNorm2 = BatchNormalization(name='batchNorm2')(conv2)
        maxpool1 = MaxPooling2D(pool_size=(2, 2),strides=(1,1),padding='same',name = 'pool1')(batchNorm2)
        drop2 = Dropout(0.25,name='dropout2')(maxpool1)
        
        #3rd convolution Layer
        conv3 = Conv2D(128, (3,3), strides=(2, 2),activation = 'relu',kernel_initializer='he_uniform',
                                    kernel_regularizer=regularizers.l2(self.l2Lambda),
                                    data_format='channels_last',name='convlayer3')(drop2)
        batchNorm3 = BatchNormalization(name='batchNorm3')(conv3)
        maxpool2 = MaxPooling2D(pool_size=(2, 2),strides=(1,1),padding='same',name = 'pool2')(batchNorm3)
        drop3 = Dropout(0.25,name='dropout3')(maxpool2)
        
        #4th convolution Layer
        '''conv4 = Conv2D(128, (3,3), strides=(2, 2), activation = 'relu',kernel_initializer='he_uniform',
                                    kernel_regularizer=regularizers.l2(self.l2Lambda),
                                    data_format='channels_last',name='convlayer4')(drop3)
        batchNorm4 = BatchNormalization(name='batchNorm4')(conv4)
        maxpool3 = MaxPooling2D(pool_size=(2, 2),strides=(1,1),padding='same',name = 'pool3')(batchNorm4)
        drop4 = Dropout(0.25,name='dropout4')(maxpool3)'''
        
        #flatten Layer
        flat1 = Flatten(name='flat1')(drop3)
        #fully connected Layers
        dense1 = Dense(784,activation='relu',kernel_initializer='glorot_uniform',
                                    kernel_regularizer=regularizers.l2(self.l2Lambda), name='FUll_1')(flat1) 
        batchNorm5 = BatchNormalization(name='batchNorm5')(dense1)
        drop5 = Dropout(0.25,name='dropout5')(batchNorm5)
        
        dense2 = Dense(784,activation='relu',kernel_initializer='glorot_uniform',
                                    kernel_regularizer=regularizers.l2(self.l2Lambda), name='FUll_2')(drop5) 
        batchNorm6 = BatchNormalization(name='batchNorm6')(dense2)
        drop7 = Dropout(0.25,name='dropout7')(batchNorm6)
        
        output = Dense(numClases, activation='softmax', name='FUll_3')(drop7) 
...
#-------------------------------------------------------------------
    def training(self):
        ...
        # =============================================================================
        self.hist = self.model.fit_generator(self.getTrainingData(),
                            steps_per_epoch =stepsPerEpochs,
                            epochs=self.epochs,
                            validation_data = self.getValidationData(),
        ...
        save_model(self.model,'finalModel_v4.h5')
        self.model.set_weights(self.model.get_weights())
        self.model.save(filepath='finalModel_weights_v4.h5')        
