...
N = 500    #  数据点的个数
D = 2   #  数据的维度 
X = np.random.randn(N,D)    #  创建N * D 的高斯随机数据 normal distribution

delta = 1.5

# 赋值数据和标签
X[:N//2] += np.array([delta, delta]) #前一半的数据往右上移动
X[N//2:] += np.array([-delta, -delta]) #后一半的数据往左下移动
...
plot_decision_boundary(X, model) #overfitting太严重了
plt.show()
...
#TODO 
#调用plot_decision_boundary 函数画出决策边界
plot_decision_boundary(X, model_depth_3) #边界比上一个宽松/简单平滑一些，也合理些
plt.show()
...
# 赋值给X和标签Y   500/4=125
X[:125] += np.array([delta, delta]) #往右上走1.75
X[125:250] += np.array([delta, -delta]) #往右下走1.75
X[250:375] += np.array([-delta, delta]) #往左上走1.75
X[375:] += np.array([-delta, -delta]) #往左下走1.75
