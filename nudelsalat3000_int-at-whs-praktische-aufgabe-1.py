        ...
        if z not in history+zustand:
            ...
            if res1: 
                ...
                if all_solutions:
                    ...
                    print("Solution found: ",res1,"- Length of Solution: ", len(res2),"- Solution: \n",res2)
...
## Zur Not dürfen Sie mir auch ein PDF des ausgeführten Notebooks senden- oder eine PDF 

### mit Programm und Ausführung zu einem Python-Skript (lokal oder bei repl.it ausgeführt)



### Für den Bonuspunkt erwarte ich, dass Sie mir ihre Lösung am kommenden Dienstag im Pool 0.15 

### vorführen können (über den Pool-Rechner im Web oder auf ihrer eigenen, mitgebrachten Maschine).
...
def gib_folgezustaende(zustand):
    ...
    for kboat in range(0,boatsize+1):

        for mboat in range(0,boatsize+1):

            if 0<(kboat+mboat)<=boatsize:

                if mboat>=kboat or (mboat==0 and kboat>0):

                    fzustaende.append([m+mboat*incr,k+kboat*incr,b_new])
...
shortest_path = [False,None]



def suche(zustand,history,all_solutions=False,shortest_solution=False,level=0,debug=1):
    ...
    for z in fzustaende:
        ...
        if z not in history+zustand:
            ...
            res1,res2 = suche(z,history+[zustand],all_solutions,shortest_solution,level+1,debug)
            ...
            if res1: 
                ...
                if shortest_solution:
                    ...
                    if shortest_path[1] is None:
                        ...
                        shortest_path=[True,res2]

                    if len(res2)<len(shortest_path[1]):

                        shortest_path=[True,res2]

                    if len(res2)==len(shortest_path[1]):

                        shortest_path.append(res2)
...
suche(startzustand,[],debug=2) # One solution
suche([4,4,1],[],shortest_solution=True,debug=0)

for item in shortest_path[1:-1]:

    print("Shortest solution: ",shortest_path[0],"- Length of Solution: ", len(item),"- Solution: \n",item)