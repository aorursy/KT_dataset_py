...
print(os.listdir("../input/sign-language-digits-characters-and-unknown"))
import numpy as np
...
#data = pd.read_csv('../input/sign-language-mnist-digits-and-characters/sign_mnist_train_digit_chars.csv', index_col=0)

data = pd.read_csv('../input/sign-language-digits-characters-and-unknown/sign_mnist_train_complete.csv', index_col=0)
...
label = [decode(i) for i in z.keys()]



labels = [chr(i) for i in z.keys()]
...
plt.show()
data_test = pd.read_csv('../input/sign-language-digits-characters-and-unknown/sign_mnist_test_complete.csv', index_col=0)
...
def get_prediction(img):
    ...
    pred = decode(pred.index(max(pred)))

    return (img, pred)
image, pred = get_prediction(x_test[-7].reshape(1, 784))
...
plt.show()
image, pred = get_prediction(x_test[-600].reshape(1, 784))
...
plt.show()
image, pred = get_prediction(x_test[100].reshape(1, 784))
