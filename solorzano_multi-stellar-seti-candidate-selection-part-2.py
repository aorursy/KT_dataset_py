...
anomaly_selection_fractions = np.logspace(np.log10(0.007), np.log10(0.07), 30)
skewness_selection_fractions = np.logspace(np.log10(0.001), np.log10(0.01), 30)
...
def plot_pos_frame(pos_frame, star_color, sun_color = 'blue', bstar_color = 'black'):    
    ...
    py.iplot(fig)
plot_pos_frame(candidates_with_extras, 'green')
candidates.to_csv('clustered-dim-candidates.csv', index=False)