...
test_base_path = '../input/asn10e-final-submission-coml-facial-recognition'
...
output_path = '../input/detection-weights'
...
C.model_path = weight_output_path
...
num_test_imgs = 4 # set this to what you want 
...
os.mkdir('output_bounding_boxes')
...
for idx, img_name in enumerate(imgs_path):
    ...
    for key in bboxes:
        ...
        for jk in range(new_boxes.shape[0]):
            ...
            # puts all bounding boxes into output_bounding_boxes folder

            img2 = cv2.imread(filepath)

            y = img2[real_y1:real_y2, real_x1:real_x2]

            cv2.imwrite('output_bounding_boxes/box' + str(bounding_box_num) + '.jpg', y)
...
class LfwDataGenerator(keras.utils.Sequence):
    ...
    def get_image_path(self, name, id):
        ...
        path_name = os.path.join("data/coml-face-dataset/train", name)

        return os.path.join(path_name, name) + "_" + f"{id}".zfill(1) + ".jpg"
...
model.load_weights('../input/recogweights/pretrained_model.hdf5')
...
def align_image(img):
        ...
        alignment = AlignDlib('../input/recog-model/landmarks.dat')
...
for path in os.listdir('../working/output_bounding_boxes'):

    if os.path.isfile(os.path.join('../working/output_bounding_boxes', path)):
...
for path in os.listdir('../input/comlfaces/COMLFaces'):

    if os.path.isfile(os.path.join('../input/comlfaces/COMLFaces', path)):

        boxes_num2 += 1
metadata = load_metadata('../working/output_bounding_boxes', boxes_num + 1)

metadata2 = load_metadata('../input/comlfaces/COMLFaces', boxes_num2 + 1)
embeddings = np.empty((metadata.shape[0], 128))



# use model to predict embeddings from the dataset
...
def show_pair(embeddings, embeddings2, id1, id2):

    if distance(embeddings2[id1], embeddings[id2]) < 0.3:

        print(distance(embeddings2[id1], embeddings[id2]))

        print(metadata2[id1].image_path())

        print(metadata[id2].image_path())
        ...
        #plt.imshow(align_image(load_image(metadata2[id1].image_path())))
...
for i in range(0, len(metadata2)):

    min = 5

    for y in range(0, len(metadata)):

        if distance(embeddings2[i], embeddings[y]) < min:

            min = distance(embeddings2[i], embeddings[y])
    ...
    # print(metadata2[i].image_path())

    # print(metadata[idx].image_path())

    # plt.suptitle(os.path.split(metadata2[i].image_path())[-1])

    plt.figure(figsize = (4, 2))

    # plt.subplot(1, 2, 1)

    # plt.imshow(align_image(load_image(metadata2[i].image_path())))

    plt.subplot(1, 2, 2)

    plt.imshow(align_image(load_image(metadata[idx].image_path())));

    filename = os.path.basename(metadata2[i].image_path())

    plt.xlabel(os.path.splitext(filename)[0])
annotations_df = pd.DataFrame(columns=['filename', 'person_id', 'id', 'xmin', 'xmax', 'ymin', 'ymax'])
...
for i in range(0, 47):
    ...
    if id == 9 or id == 12 or id == 26 or id == 28 or id == 38 or id == 45:

        annotations_df = annotations_df.append({'filename': '1.jpg', 
        ...
        annotations_df = annotations_df.append({'filename': '0.jpg', 
...
annotations_fn = 'output.csv'

annotations_df.to_csv(annotations_fn, index=False, index_label=False)