...
example_data_weights = np.array([1., 2., .5, 1., 1.]) # 0.5+1+1 = 2.5 < 3
...
def best_split_weighted(data, features, target, data_weights):
    ...
    for feature in features:
        ...
        # 计算当前划分之后的分类犯错率
        error = (left_misses + right_misses) * 1.0 / sum(data_weights)  # 1.0 成float

        # 更新应选特征和错误率，注意错误越低说明该特征越好, 就替换为最佳特征
        if error < best_error:
            best_error = error
...
def create_leaf(target_values, data_weights):
    ...
    return leaf 
# create decision tree，with data_weights，termination 1
...
class MyAdaboost(BaseEstimator):
    ...
    def fit(self, X, Y):
        ...
        for m in range(self.M):
            ...
            # 计算加权错误
            weighted_error = alpha.dot(prediction != Y[self.target] * prediction) 
...
m.score(testX, testY)
0.15*10 + (0.15-1)*20
15.5/(-0.15*(1-0.15)*30)-1.734

1/(np.exp(-np.log(0.15/0.85)-4.052)+1)
