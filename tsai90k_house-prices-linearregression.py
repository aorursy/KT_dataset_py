...
def plot_hist_with_target3(plt, df, feature, target, histogram_bins = 10):
    ...
    # plt.errorbar([0.0], [0.0], yerr = [0.0], fmt = 'ok', lw = 3, ecolor = 'sienna', mfc = 'sienna', label = target)
...
def log_no_err(l):
    def _log_no_err(n):
        if n <= 0:
            ...
            return np.log(n)
    return np.array([_log_no_err(n) for n in l])
...
def rmsle(y, h): 
    ...
    hl = log_no_err(ha)
    yl = log_no_err(ya)
    return np.sqrt(np.square(hl - yl).mean())
...
def train_linear_regressor_model(
    ...
    for period in range (0, periods):
        ...
        training_root_mean_squared_log_error = rmsle(targets_transform_fn(training_targets), 
                                                     targets_transform_fn(training_predictions))
        ...
        if do_validation == True:
            ...
            validation_root_mean_squared_log_error = rmsle(targets_transform_fn(validation_targets), 
                                                           targets_transform_fn(validation_predictions))

        # Occasionally print the current loss.
        msg = "  period %02d:" % (period)
        ...
        if training_root_mean_squared_log_error is not None:
