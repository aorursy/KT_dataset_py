...
gbdt_train_data = np.array(to_train_data(gbdt_train_data))

gbdt_valid_data = np.array(to_train_data(gbdt_valid_data))

gbdt_train_label = np.array(to_train_label(gbdt_train_label))

gbdt_train_label = gbdt_train_label[:, 1] == 1

gbdt_valid_label = np.array(gbdt_valid_label)
...
class my_gbdt(object):
    ...
    def __init__(self):
        ...
        self.tree = DecisionTreeRegressor(min_samples_split=2, max_depth=3)
    ...
    def get_init_val(self, y):
        ...
        return np.log(y_sum / (n - y_sum))

    

    def update_scores(self, tree, X, y, residual):

        # Doesn't work

        leaves = np.argwhere(tree.tree_.children_left == -1).squeeze()

        terminal = tree.apply(X)

        for leaf in leaves:

            idx = terminal == leaf

            score = residual[idx].sum() / (y[idx] * (1 - y[idx])).sum()

            tree.tree_.value[leaf, 0, 0] = score
    ...
    def fit(self, X, y, n_trees, lr, min_samples_split, max_depth):

        # TO DO

        self.trees = [DecisionTreeRegressor(min_samples_split=min_samples_split, max_depth=max_depth) for _ in range(n_trees)]

        self.lr = lr

        self.init_val = self.get_init_val(y)

        y_hat = np.ones_like(y) * self.init_val

        residual = y - self.sigmoid(y_hat)

        for tree in self.trees:

            tree.fit(X, residual)

            # self.update_scores(tree, X, y_hat, residual)
    ...
    def predict(self, X):
        ...
        pred_proba = self.init_val

        for tree in self.trees:

            pred_proba += tree.predict(X) * self.lr

        pred_proba = self.sigmoid(pred_proba)

        return (pred_proba > 0.5).astype(int)
my_model = my_gbdt()

my_model.fit(gbdt_train_data, gbdt_train_label, 35, 0.5, 10, 6)
