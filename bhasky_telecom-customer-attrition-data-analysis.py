...
telecom = pd.read_csv(r"../input/WA_Fn-UseC_-Telco-Customer-Churn.csv")
#first few rows
telecom.head()
...
def tenure_lab(telecom) :
    ...
    if telecom["tenure"] <= 12 :
        ...
        return "Tenure_gt_60"
telecom["tenure_group"] = telecom.apply(lambda telecom:tenure_lab(telecom),
                                      axis = 1)
...
#labels
lab = telecom["Churn"].value_counts().keys().tolist()
#values
val = telecom["Churn"].value_counts().values.tolist()
...
fig  = go.Figure(data=data,layout=layout)
py.iplot(fig)
telecom[['MonthlyCharges', 'TotalCharges','tenure',"tenure_group"]]

#scatter plot monthly charges & total charges by tenure group

def plot_tenure_scatter(tenure_group,color) :
    tracer = go.Scatter(x = telecom[telecom["tenure_group"] == tenure_group]["MonthlyCharges"],
                        y = telecom[telecom["tenure_group"] == tenure_group]["TotalCharges"],
...
def plot_churncharges_scatter(churn,color) :
    tracer = go.Scatter(x = telecom[telecom["Churn"] == churn]["MonthlyCharges"],
                        y = telecom[telecom["Churn"] == churn]["TotalCharges"],
                        mode = "markers",marker = dict(line = dict(color = "black",
                                                                   width = .2),
                                                       size = 4 , color = color,
                                                       symbol = "diamond-dot",
...
py.iplot(fig1)
py.iplot(fig2)
avg_tgc = telecom.groupby(["tenure_group","Churn"])[["MonthlyCharges",
...
tel_df = telecom.copy()
#Drop tenure column
telecom = telecom.drop(columns = "tenure_group",axis = 1)
...
telecom = pd.get_dummies(data = telecom,columns = multi_cols )
...
summary = (df_telecom_og[[i for i in df_telecom_og.columns if i not in Id_col]].
...
X = telecom[[i for i in telecom.columns if i not in Id_col + target_col]]
Y = telecom[target_col + Id_col]
...
bi_cs = telecom.nunique()[telecom.nunique() == 2].keys()
dat_rad = telecom[bi_cs]
...
cols    = [i for i in telecom.columns if i not in Id_col + target_col]
train_X = train[cols]
...
smote_X = telecom[cols]
smote_Y = telecom[target_col]
...
idc_rfe = pd.DataFrame({"rfe_support" :rfe.support_,
                       "columns" : [i for i in telecom.columns if i not in Id_col + target_col],
...
df_x = df_telecom_og[cols]
df_y = df_telecom_og[target_col]
...
def plot_tree_randomforest(columns,nf_estimators,
    ...
    dataframe = df_telecom_og[columns + target_col].copy()
