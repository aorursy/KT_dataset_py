...
object_columns_df = df.select_dtypes(include=['object']) #カテゴリカル変数のみを取り出し
numerical_columns_df =df.select_dtypes(exclude=['object']) #量的変数のみを取り出し
object_columns_df.head(3)
numerical_columns_df.head(3)
#変数ごとにnullを数えます
null_counts = object_columns_df.isnull().sum()
print("Number of null values in each column:\n{}".format(null_counts))

#nullが多い変数は'None'で埋めます
columns_None = ['BsmtQual','BsmtCond','BsmtExposure','BsmtFinType1','BsmtFinType2','GarageType','GarageFinish','GarageQual','FireplaceQu','GarageCond']
object_columns_df[columns_None]= object_columns_df[columns_None].fillna('None')
#nullが少ない列は列ごとの最頻値で埋めます
columns_with_lowNA = ['MSZoning','Utilities','Exterior1st','Exterior2nd','MasVnrType','Electrical','KitchenQual','Functional','SaleType']
object_columns_df[columns_with_lowNA] = object_columns_df[columns_with_lowNA].fillna(object_columns_df.mode().iloc[0])
object_columns_df.head(30)
#量的変数の各列ごとにnullの数を出力
