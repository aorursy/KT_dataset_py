...
df_all["GroupPorchArea"] = df_all["3SsnPorch"] + df_all["EnclosedPorch"]

df_all["GroupBsmtFinSF"] = df_all["BsmtFinSF1"] + df_all["BsmtFinSF2"]

df_all["GroupLowQualBsmUnf"] = df_all["LowQualFinSF"] + df_all["BsmtUnfSF"]

df_all["GroupSF"] = df_all["TotalBsmtSF"] + df_all["1stFlrSF"] + df_all["2ndFlrSF"]

df_all["GroupYear"] = df_all["YearRemodAdd"] + df_all["YearBuilt"]

df_all.drop(df_all[np.logical_and(df_all['GroupSF'] > 6000, df_all['SalePrice'] < 800000)].index)

df_all.drop(df_all[np.logical_and(df_all['GroupBsmtFinSF'] > 1000, df_all['SalePrice'] > 600000)].index)

df_all.drop(df_all[np.logical_and(df_all['GarageArea'] > 700, df_all['SalePrice'] > 600000)].index)

df_all.drop(df_all[np.logical_and(df_all['LotArea'] > 100000, df_all['SalePrice'] > 200000)].index)

df_all.drop(df_all[np.logical_and(df_all['LotFrontage'] > 300, df_all['SalePrice'] > 200000)].index)

df_all.drop(df_all[np.logical_and(df_all['MasVnrArea'] > 1200, df_all['SalePrice'] > 200000)].index)

df_all.drop(df_all[np.logical_and(df_all['MiscVal'] > 6000, df_all['SalePrice'] > 100000)].index)

df_all.drop(df_all[np.logical_and(df_all['OpenPorchSF'] > 400, df_all['SalePrice'] > 100000)].index)
...
drop_cols = ['Id', 'Dataset', "3SsnPorch", "EnclosedPorch", "BsmtFinSF1", "BsmtFinSF2", "LowQualFinSF", 

"BsmtUnfSF", "TotalBsmtSF", "1stFlrSF", "2ndFlrSF", "YearRemodAdd", "YearBuilt",
