...
filename = "../input/rockyeni2key.csv"
...
def start_test(training, test, drop):
    ...
    classifiers = [
        ...
        # LinearSVC(class_weight="balanced", max_iter=100000, penalty='l2', random_state=42),
        ...
        #XGBClassifier(class_weight="balanced", learning_rate=0.01, random_state=42, n_jobs=-1),
    ...
    param_grid = [
        ...
        # {'C': [0.0041], 'tol': [1e-11]}, # Linear

        # {"kernel": ['poly'], 'C': [math.pow(10,i) for i in range(-3,1)], 'degree': [3]}, # Poly

        # {'C': [1.2], 'tol': [0.1]}, # Logistic

        # {}, # Naive

        #{"max_depth": [3], "min_child_weight": [2], 'subsample': [0.9], 'colsample_bytree': [0.65], 'reg_alpha': [1.1]},

        # {"max_depth": [3], "max_features": [10], "min_samples_leaf": [0.1],

        # "min_samples_split": [1.0], "criterion": ["entropy"]},

        {'n_estimators': [200], 'max_depth': [7], "max_features": [4],
    ...
    [accuracy, f1] = k_fold_cv(classifiers[0], X_train, y_train, param_grid[0])
    ...
    for clf_name, clf in zip(names, classifiers):
        ...
        [accuracy, f1] = k_fold_cv(clf, X_train, y_train, param_grid[i])
        ...
        # best_params = hyperopt(X_train, y_train, X_val, y_val, clf, param_grid[i])
...
def hyperopt(X_train, y_train, X_val, y_val, clf, param_grid):

    opt = GridSearch(model=clf, param_grid=param_grid)
...
# for curr in itertools.combinations(drop, 6):
...
# 2. Test: CF, Resolution, Melody Leap

results_hypothesis = start_test(training.copy(), test.copy(), ["mean_pitch","dissonance","consonance"])
