...
train['Screen Size'].mean()
def data_binarize(data):

    data['os_name']= data['os_name'].map(lambda x: 1 if x in ['Android', 'iOS', 'Blackberry', 'Nokia'] else 0)

    data['Num_cores']= data['Num_cores'].map(lambda x: 1 if x in ['Octa', 'Quad', 'Deca', 'Hexa', 'Tru-Octa'] else 0)

    data['Weight'] = data['Weight'].map(lambda x: 0 if x > 180 else 1)
    ...
    data['Capacity']= data['Capacity'].map(lambda x: 1 if x>=3000 else 0)
...
bs=columns_new.get_indexer(['Browser'])[0]
...
t_bs=columns_new.get_indexer(['Browser'])[0]
...
def data_special(data,bd,bs,wc):
    ...
    for i in range(m):
        ...
        elif data[i][bd]=='Samsung' or 'Nokia':

            if data[i][bs]=='WAP' or data[i][bs]=='WAP 2.0' or data[i][bs]=='yes':
    ...
    return data
X=data_special(X,bd,bs,wc)

test_new=data_special(test_new,t_bd,t_bs,t_wc)
X[1,:]
test_new[1,:]
def data_special1(data,bd,bs,wc):

    m=data.shape[0]

    brand_high=['Xiaomi', 'Realme', 'Samsung', 'Honor', 'Vivo', 'OPPO',

        'Xiaomi Poco', 'OnePlus', 'Nokia', 'Asus', 'Huawei', 'Lenovo',

        'Apple', 'Reliance', 'Moto', 'Gionee', 'Infinix', 'Google', 'Blackberry', 'Oppo', 

        'Sony', 'Coolpad', 'Panasonic', 'Motorola','LeEco',1]
    ...
    return data
X=data_special1(X,bd,bs,wc)

test_new=data_special1(test_new,t_bd,t_bs,t_wc)
...
Y_test_pred
test_pred = mp_neuron.predict(test_new)
...
Y_test_pred
test_pred1 = mp_neuron.predict(test_new)
