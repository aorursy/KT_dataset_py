...
def build_model():
    ...
    ids_ = input_ids[:, :max_len]

    att_ = attention_mask[:, :max_len]

    tok_ = token_type[:, :max_len]
    ...
    x1 = tf.keras.layers.Conv1D(768,2)(x1)
    ...
    x2 = tf.keras.layers.Conv1D(768,2)(x2)
    ...
    x1_padded = tf.pad(x1, [[0, 0], [0, max_seq_length - max_len]], constant_values=0.)

    x2_padded = tf.pad(x2, [[0, 0], [0, max_seq_length - max_len]], constant_values=0.)
...
oof_start = np.zeros((ids.shape[0],max_seq_length-1))

oof_end = np.zeros((ids.shape[0],max_seq_length-1))

preds_start = np.zeros((test_ids.shape[0],max_seq_length-1))

preds_end = np.zeros((test_ids.shape[0],max_seq_length-1))
...
#         save_weights_only=True, mode='auto', save_freq='epoch')
    ...
    weight_fn = '../input/tfrobertapaddingmodel/%s-roberta-%i.h5'%(VER,fold)
