...
shelter_outcomes['datetime'].plot.line()
shelter_outcomes['datetime'].value_counts().sort_values().plot.line()
shelter_outcomes['datetime'].value_counts().resample('Y').sum().plot.line()
shelter_outcomes['datetime'].value_counts().resample('M').sum().plot.line()
shelter_outcomes['datetime'].value_counts().resample('W').sum().plot.line()
shelter_outcomes['datetime'].value_counts().resample('D').sum().plot.line()
crypto.head()
crypto.volume.plot.line()
lag_plot(crypto.volume)
lag_plot(crypto.volume.sample(50))
lag_plot(crypto.volume.sample(200))
