...
# train-error:0.10253	eval-error:0.13967
...
class my_gbdt(object):
    ...
    def fit(self, X, y):
        ...
        new_y = np.array(y.copy())

        for i in range(1, self.len):

            tree_old = self.trees[i-1]

            tree_new = self.trees[i]

            pred_prob = tree_old.predict(X)

            new_y = new_y - pred_prob

            tree_new.fit(X, new_y)
    ...
    def predict(self, X):
        ...
        pred_probs = [self.trees[i].predict(X) for i in range(self.len)]

        pred_probs = np.sum(np.array(pred_probs), 0)
