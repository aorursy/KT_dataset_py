...
!kaggle competitions download -c 18011854kbopredicton
...
training_epochs=4000 # epoches 2배로 늘리기

batch_size=100

Scaler = preprocessing.StandardScaler()
train_data=pd.read_csv('kbo_train.csv',header=None,skiprows=1,usecols=range(0,9))

test_data=pd.read_csv('kbo_test.csv',header=None,skiprows=1,usecols=range(0,8))
...
# 레이어 수 증가 64->64, 총 7개의 레이어 이용

linear1 = torch.nn.Linear(8, 64,bias=True)
...
linear7 = torch.nn.Linear(64, 1,bias=True)
...
correct=prediction.cpu().detach().numpy().reshape(-1,1)
...
!kaggle competitions submit -c 18011854kbopredicton -f submit.csv -m "submit"