...
x = tf.Variable(np.array([1,2])) # TODO
y = tf.Variable(3, dtype = 'int64') # TODO
f = tf.pow(x, y) # TODO
...
with tf.Session() as sess:
    ...
    print(x.eval())
    print(y.eval())
    result = sess.run(f, feed_dict = {x:[3,4], y:4}) # TODO
print(result)
...
class LinearRegressionTF(BaseEstimator):
    ...
    def fit(self, trainX,trainY):
        ...
        # 参数的定义
        self.W = tf.Variable(np.random.randn()) # TODO
        self.b = tf.Variable(np.random.randn()) # TODO
        
        # 线性模型
        self.pred = self.X * self.W + self.b # TODO
        
        # mean squre error
        cost = tf.reduce_sum((self.pred - self.Y) ** 2)/(2*N) # TODO
        ...
        for epoch in range(self.training_epochs):
            ...
            if (epoch+1) % display_step == 0:
                c = self.sess.run(cost, feed_dict = {self.X: x, self.Y: y}) # TODO
                print("Epoch:", '%04d' % (epoch+1), "cost=", "{:.9f}".format(c), \
            "W=", self.sess.run(self.W), "b=", self.sess.run(self.b))    # do we need to run these?
        ...
        training_cost = self.sess.run(cost, feed_dict = {self.X: x, self.Y: y} ) # TODO
...
class LogisticRegressionTF(BaseEstimator):
    ...
    def fit(self, trainX,trainY):
        ...
        _, c = trainY.shape # TODO assuming trainY is one hot encoded.
        # 图的输入
        ...
        # 参数的定义
        self.W = tf.Variable(np.random.randn(D, c)) # TODO
        self.b = tf.Variable(np.random.randn(c)) # TODO
...
#             print('epoch: ', epoch)
            for (x, y) in zip(trainX, trainY):
                ...
                self.tempX = x
                self.tempY = y
#                 self.sess.run(optimizer, feed_dict = {self.X: x.reshape(-1,D), self.Y: y.reshape(-1, c)})  #this line does not work after first run of "展示训练结果"
...
trainY[0]
logisticTF = LogisticRegressionTF(learning_rate, 1000, display_step = 100, annotate=False) #TODO
...
xtr = tf.placeholder('float', shape = (None, 784)) # TODO
xte = tf.placeholder('float', shape = (784)) # TODO
distance = tf.sqrt(tf.reduce_sum(tf.square(tf.subtract(xtr, xte)), axis = 1)) # TODO
...
class NaiveBayesTF(BaseEstimator):
    ...
    def fit(self, trainX, trainY):
        ...
        input_x = tf.placeholder(tf.float64, points_by_class.shape) # TODO
    ...
    def predict(self, testX):
        ...
        cond_probs = tf.reduce_sum(self.dist.log_prob(np.reshape(np.tile(testX, [1, num_classes]), [-1, num_classes, num_features])), axis = 2) # TODO
