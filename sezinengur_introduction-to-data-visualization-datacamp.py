...
data_women = pd.read_csv("../input/percent-bachelors-degrees-women-usa-csv/percent-bachelors-degrees-women-usa.csv")
data_women.head()
data_women.keys()
year = np.array(data_women["Year"])
year
physical_sciences = np.array(data_women["Physical Sciences"])
physical_sciences
computer_science = np.array(data_women["Computer Science"])
computer_science
...
health = np.array(data_women["Health Professions"])
health
education = np.array(data_women["Education"])
education
...
data_auto = pd.read_csv("../input/auto-mpg/auto.csv")
data_auto.head()
mpg = np.array(data_auto["mpg"])
hp = np.array(data_auto["hp"])
...
img = plt.imread("../input/astronaut/480px-Astronaut-EVA.jpg")
...
image = plt.imread('../input/unequalized/640px-Unequalized_Hawkes_Bay_NZ.jpg')
...
# Parameter "kind" to specify how to visualize the joint variation of two continuous random variables
...
# covariance matrix between the continuous variables in the auto-mpg dataset.
cov_matrix = data_auto.drop([ 'cyl','yr','size'], axis=1).corr(method='spearman')
cov_matrix
...
# Display the heatmap
plt.show()
data_stocks = pd.read_csv('../input/stocks/stocks.csv')
data_stocks.head()
# Convert data_stocks['Date'] from string to datetime
data_stocks['Date'] = pd.to_datetime(data_stocks['Date'])
# Set data_stocks['Date'] as the index and delete the column
data_stocks.index = data_stocks['Date']
del data_stocks['Date']
# Check if the index changed
type(data_stocks.index) # Yeap!
# Create pandas series for Apple stocks with date index
aapl = data_stocks.AAPL
aapl.head()
type(aapl)
# Create pandas series for IBM, CSCO and MSFT stocks with date index
ibm = data_stocks.IBM
csco = data_stocks.CSCO
msft = data_stocks.MSFT
data_stocks.head()
...
# Windowed averages of the series aapl Computed over windows of width 30 days
mean_30 = aapl.rolling(window=30).mean()
mean_30.head()
mean_30[-5:]
# Windowed averages of the series aapl Computed over windows of width 75, 125, and 250 days
...
# Windowed standard deviations of the series aapl computed over windows of width 30, 75, 125, and 250 days 
...
image
plt.imshow(image)
# We have already loaded the image into an array
image
...
# Source = http://imgsrc.hubblesite.org/hu/db/images/hs-2004-32-b-small_web.jpg
image_nebula = plt.imread('../input/nebula/hs-2004-32-b-small_web.jpg')
