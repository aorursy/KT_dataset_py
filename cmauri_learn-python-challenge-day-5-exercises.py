...
def menu_is_boring(meals):
    ...
    """
    for idx in range(len(meals)-1):
        if meals[idx] == meals[idx+1]:
            return True
    return False
...
def estimate_average_slot_yield(n_runs):
    ...
    """
    return sum([play_slot_machine()-1 for x in range(n_runs)])/n_runs
estimate_average_slot_yield(100000)
q4.solution()
