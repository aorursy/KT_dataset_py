...
lista = torch.zeros(10)
lista2 = []
for img, label in dataset:
    lista[label] += 1
    # lista2 for another way - Just practicing
    lista2.append(label)
lista
# Another way
import pandas as pd
labels_count = pd.Series(lista2).groupby(lista2).count()
labels_count
...
class CIFAR10Model(ImageClassificationBase):
    ...
    def forward(self, xb):
        ...
        return out
model = to_device(CIFAR10Model(input_size, hidden_size1=1024, hidden_size2=512,hidden_size3=32, out_size=output_size), device)
model
history = [evaluate(model, val_loader)]
history
history += fit(15, 0.1, model, train_loader, val_loader)
history += fit(15, 0.01, model, train_loader, val_loader)
history += fit(5, 0.04, model, train_loader, val_loader)
history += fit(10, 0.001, model, train_loader, val_loader)
plot_losses(history)
plot_accuracies(history)
evaluate(model, test_loader)
# 5 layers
arch = (3072, 1024, 512, 64, 10)
lrs = [0.1, 0.01, 0.04, 0.001]
epochs = [15, 15, 5, 10]
test_acc = 0.5622
test_loss = 1.2670
