...
data['sex'].value_counts(normalize = 1)
# 性别与幸存的关系
...
# 调用measure_performance观察模型在testX, testY上的表现
...
pd.DataFrame({'variable': trainX.columns, 'importance': model.feature_importances_}).sort_values(by = ['importance'], ascending = False)[:20]