...
def cambiarLabelEscuelasCercanas(x):

    

    if (x == 0):

        return 'sin escuela cerca'

    if (x == 1):

        return 'con escuela cerca'
df['escuelascercanas'] = df['escuelascercanas'].map(lambda x : cambiarLabelEscuelasCercanas(x))
...
df['escuelas cercanas'] = df['escuelascercanas']
#tipo de prop (filtrado por centro escuela cercana) en violinplot con el precio
...
g_13 = sns.violinplot(data = df, x = 'categorias_de_tipo_de_propiedad', y = 'precio', hue = 'escuelas cercanas',
...
plt.title('Distribucion de precios en funcion de si tiene escuela cercana por categoria de propiedad')
...
#tipo de prop (filtrado por centro escuela cercana) en boxplot con el precio
...
g_13 = sns.boxplot(data = df, x = 'categorias_de_tipo_de_propiedad', y = 'precio', hue = 'escuelas cercanas',
