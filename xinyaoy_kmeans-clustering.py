...
def my_k_means(dataSet, k):
    ...
    while(epoch<epoches):
        ...
        for i in range(len(dataSet)):
            ...
            min_dis = euclDistance(dataSet[i],centroids[0])

            min_idx = 0

            for j in range(1,k):

                if euclDistance(dataSet[i],centroids[j])<min_dis:

                    min_idx = j

                    min_dis = euclDistance(dataSet[i],centroids[j])

            clusterAssment[i][0] = min_idx
        ...
        for i in range(k):
            ...
            centroids[i][0] = x_mean/num

            centroids[i][1] = y_mean/num
...
#                     Loss +=  euclDistance(dataSet[j],centroids[i])
