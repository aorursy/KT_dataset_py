...
train_data.City=train_data.AveMonthSpend*12
test_data.City=test_data.AveMonthSpend*12

train_data.PostalCode=train_data.TotalChildren.subtract(train_data.NumberChildrenAtHome, fill_value=0)
test_data.PostalCode=test_data.TotalChildren.subtract(test_data.NumberChildrenAtHome, fill_value=0)

train_data.PostalCode=train_data.PostalCode**(1)
test_data.PostalCode=test_data.PostalCode**(1)
...
features = ['TotalChildren','YearlyIncome',
            'HomeOwnerFlag','NumberChildrenAtHome','Gender','MaritalStatus','BirthDate','CountryRegionName','Occupation','PostalCode','Suffix','Education'
            ]
...
xgb = XGBClassifier(n_estimators=400, learning_rate=0.049, random_state=1,min_child_weight=6 )
...
acc_xgb = (preds == val_y).sum().astype(float) / len(preds)*1000
