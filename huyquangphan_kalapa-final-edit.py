...
train = pd.read_csv('../input/kalapa-credit-score/train.csv')

test = pd.read_csv('../input/kalapa-credit-score/test.csv')
...
# count_enc = LabelEncoder().fit_transform(t[c for c in cat_features])

count_enc = t[cat_features].apply(LabelEncoder().fit_transform)



# count_enc = ce.BinaryEncoder().fit_transform(t[cat_features])
...
for i, (train_idx, val_idx) in enumerate(cv.split(TRAIN, LABEL)):
    ...
    gbm = CatBoostClassifier(eval_metric='AUC', 
                             ...
                             random_seed=2020).fit(X_train, y_train, 
