...
articles = pd.read_sql_query("select title from articles where tags is not null and title is not null order by published desc", db)
...
articles = pd.read_sql_query("select source from articles where tags is not null order by published desc", db)
...
articles = pd.read_sql_query("select case when (Publication = '' OR Publication IS NULL) THEN '[None]' ELSE Publication END AS Publication from articles where tags is not null order by published desc", db)
...
articles = pd.read_sql_query("select strftime('%Y-%m', published) as Published from articles where tags is not null and published >= '2020-01-01' order by published desc", db)
...
articles = pd.read_sql_query('select count(*) as count, case when design=1 then "systematic review" when design in (2, 3) then "control trial" ' + 
                             ...
                             'where tags is not null and design > 0 group by design order by published desc', db)
