...
data.head()
data[0] = data['class'].apply(lambda x: 0 if x == 'e' else 1)# TODO
data.shape
# 每一列如果有null，用"missing"代替
cols = np.arange(1,23)
for col in cols:
    #TODO
    if data.iloc[:,col].isnull().values.any():
#         data.iloc[:,col].fillna('missing')
        data.iloc[:,col].fillna('missing')
...
for col, encoder in labelEncoders.items():
    print(encoder.transform(data.iloc[:,col]) + 3)
# 用于测试数据的变换
def transform(df):
    #TODO
    N,_ = df.shape
...
forest = RandomForestClassifier(100)#TODO
...
forest = FakeRandomForest(100)#TODO
print("Random Forest performance: %f" % (cross_val_score(tree_model, X, Y, cv=8).mean()))
forest.fit(X,Y)
forest.predict(X)
