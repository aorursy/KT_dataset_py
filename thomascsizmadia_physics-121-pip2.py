...
plt.legend
plt.show()
# This script calculates the square of the length of the difference between numerical solution and the analytical solution at a time t


x_diff = exact_position_x - position_x

y_diff = exact_position_y - position_y

error = np.sqrt(x_diff ** 2 + y_diff ** 2)


plt.figure('error')
plt.plot(time,error,label='\epsilon')


plt.title(f"Error in numerical solution with respect to time")
plt.xlabel(f"time [s]") 
plt.ylabel(f"error $\epsilon$") 
plt.legend
plt.show()
