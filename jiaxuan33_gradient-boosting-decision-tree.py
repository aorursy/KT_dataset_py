...
print('gbdt valid data: ', gbdt_valid_data[0])
...
class my_gbdt(object):
    ...
    def tree(min_sample_split,max_depth):
        return DecisionTreeRegressor(min_sample_split,max_depth)
    def fit(self, X, y):
        # TO DO
        min_sample_split=2
        max_depth=2
        init_val= self.tree(max_depth)
        n_rows=len(y)
        subsample=None
        prediction=np.full(y.shape,self.tree)
        self.residuals=y-prediction
    ...
    def predict(self, X):
        ...
        pred_proba = residual+self.predict(X)
