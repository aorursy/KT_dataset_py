...
print(valor2)
import numpy as np
argumento = complex(2.0*np.pi,1.0)
expressao = (np.sin(np.pi**2)-np.cos(argumento))**3
expressao
lista_bolada = [1,2,3]
...
x1 = np.arange(-2.0, 2.001, 0.001)
x2 = np.arange(1.0, 2.001, 0.001)
x3 = np.arange(-0.999, 0.999, 0.001)
...
# plotando as curvas
plt.plot(x1, y1, label ='$y = asinh(x)$')
plt.plot(x2, y2, label ='$y = acosh(x)$')
plt.plot(x3, y3, label ='$y = atanh(x)$')

# título do gráfico
plt.title('Gráficos Trigonométricos')
...
for s in str:
    if s == 'a':
        break
    print(s,end='')
cont_i = np.arange(1,21,1)
cont_j = np.arange(1,10,1)
soma = [0]
...
while soma < 1000000:
    soma = soma + contador_i
    contador_i = contador_i + 1
    
print('Numero de elementos:', contador_i)
