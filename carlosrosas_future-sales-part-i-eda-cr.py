...
## Nota - removing one of the sum() will return a breakdown of the null values per column 
...
df_q1= create_df()
#check the first vales of df_q1

df_q1.head()
#calculate the daily total sales

df_q1["date"] = pd.to_datetime(df_q1["date"], format ="%d.%m.%Y") #convert the column date to datetime values

df_q1 = df_q1[["date","item_cnt_day"]].set_index("date").resample("D").sum()

df_q1
#calculate the moving average 7 days 

df_q1["ma7d"] = df_q1["item_cnt_day"].rolling(window = 7).mean()

df_q1

#calculate the variation in moving average

df_q1_var = df_q1

df_q1_var["variation"] = df_q1_var["ma7d"].diff()/df_q1_var["ma7d"].shift(1)

df_q1_var.head(20)

#plot the sales, variation & moving average 



#separate x & y

y_variation = df_q1['variation']
...
plot1 = ax.plot(y_ma7d, label = "7 days moving average of sales", color ="black", alpha = 0.5)



# create a secondary axis and plot the 

ax_bis = ax.twinx()

plot2 =ax_bis.plot(y_variation, label = "moving average sales variation", color = "yellow", alpha=0.5)
...
ax.set_title("Total daily sales and 7-days variation");
...
df_q2.head()
#selection of the city with the city_id ==29 ("Якутск")

df_q2 = df_q2[df_q2["city_id"]==29]
df_q2["date"] = pd.to_datetime(df["date"], format = "%d.%m.%Y")

df_q2_timeindex= df_q2.set_index("date").resample("W")["item_cnt_day"].sum().to_frame()

df_q2_timeindex.head()
# decompose the series using stats module
...
result_q2 = seasonal_decompose(df_q2_timeindex["item_cnt_day"])
...
result_q2.observed.plot(ax = axes[0], legend = False)
...
result_q2.trend.plot(ax = axes[1], legend = False)
...
result_q2.seasonal.plot(ax = axes[2], legend = False)
...
result_q2.resid.plot(ax = axes[3], legend = False)
...
xtick_location = df_q2_timeindex.index.tolist()
...
df_q3.head()
#groupby by item_category_name and sum all the values

short_df2 = df_q3.groupby("item_category_name")["item_cnt_day"].sum().to_frame()

 



#sort the values in the short_df2 in place 

short_df2.sort_values("item_cnt_day", ascending=False, inplace=True) 



#get the x & y values

my_values = short_df2["item_cnt_day"]

my_pct = short_df2["item_cnt_day"]/short_df2["item_cnt_day"].sum()



# create custom labels for each item_category with their total sales and overall contribution 

# we add an if condition to display only the labels of those item_categories with sales equal to 1% or over

labels = ['{} - Sales :{}k \n {}% of total'.format(item_category, sales/1000, round(pct, 2)*100) if (pct>=0.01) else "" for item_category, sales, pct in zip(short_df2.index, my_values, my_pct)]
...
# we want to normalize our values, otherwise an item_category will have the darkest collor and all the others will pale
...
plt.title("Sales by item_category and their % over total sales", fontsize = 25, fontweight = "bold")
...
# Top 3 product captures 49% of the total sales: DVD, PC & CD. Other relevant products are PS3 and Blu-Ray.
