...
XPD_train =  pd.DataFrame(data=X_train[:,:])

X_binarised_train  = XPD_train.apply(pd.cut, bins=2, labels=[1,0])





Y_binarised_train = pd.DataFrame(data=Y_train[:])



mp_neuron = MPNeuron()

mp_neuron.fit(X_binarised_train.values, Y_binarised_train.values,epochs = 100, display_plot=True)





XPD_test =  pd.DataFrame(data=X_test[:,:])

X_binarised_test  = XPD_test.apply(pd.cut, bins=2, labels=[1,0])

Y_pred_binarised_test = mp_neuron.predict(X_binarised_test.values)
