...
plt.show()
train_generator1 = aug.flow_from_directory(
        '../input/database/project_db',
        target_size=(128, 128),
...
history1=model.fit_generator(
        generator=train_generator1,validation_data=test_generator,steps_per_epoch=5443 // 32,epochs=100,validation_steps=1000 // 8)
predict2=model.predict_generator(test_generator,1000)
y_pred1 = np.argmax(predict2, axis=1)
targets=test_generator.class_indices
from sklearn.metrics import classification_report, confusion_matrix,accuracy_score
cf1=confusion_matrix(test_generator.classes, y_pred1)
print(cf1)
print(targets)
print("accuracy : ",accuracy_score(test_generator.classes, y_pred1)*100)
import seaborn as sns
sns.heatmap(cf1, annot=True)
import sklearn
print("Kappa coefficient : ",sklearn.metrics.cohen_kappa_score(test_generator.classes, y_pred1))
import matplotlib.pyplot as plt
