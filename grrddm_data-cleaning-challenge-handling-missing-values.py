...
f"{total_missing / total_cells:0.2%}"
# your turn! Find out what percent of the sf_permit dataset is missing
missing_vals_col = sf_permits.isnull().sum()
print(missing_vals_col.sort_values(ascending=False)[:10])
total_cells = np.product(sf_permits.shape)
missing_cells = missing_vals_col.sum()

print(f"{missing_cells / total_cells: 0.2%}")
# look at the # of missing points in the first ten columns
missing_values_count[0:10]
...
sf_permits.shape
(f"By removing every column with missing values we lose {12 / 43 - 1: 0.2%} of the columns")
