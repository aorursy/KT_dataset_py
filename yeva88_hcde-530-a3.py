...
for line in open(NAMES_LIST, 'r').readlines():
    ...
    total_babies = str(len(girls) + len(boys)) # Adding up boys and girls counts to calculate total babies

    # Then, converting the count to a string



print("There were " + total_babies + " babies in the database.")
...
raw_input = input("Please type a name: ") # Take a name input 

name_to_use = raw_input.lower() # Convert the input to a lower-case string
...
for name in boys.keys():
    ...
    if name == name_to_use: 
        ...
        print("There are " + str(boys[name]) + " boys named " + name_to_use)

        for name in girls.keys():

            if name == name_to_use:

                print("There are " + str(girls[name]) + " girls named " + name_to_use)