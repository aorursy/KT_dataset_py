...
q='communicating with high-risk populations'

res = search(q, 'high_risk')

res.head(20)[['title','url']].style.format({'url': make_clickable})
q='information sharing and inter-sectoral collaboration'

res = search(q, 'info_sharing')

res.head(20)[['title','url']].style.format({'url': make_clickable})
q='data standards and nomenclature'

res = search(q, 'data_standards')

res.head(20)[['title','url']].style.format({'url': make_clickable})
q='governmental public health'

res = search(q, 'govt_public_health')

res.head(20)[['title','url']].style.format({'url': make_clickable})
q='clarify community measures'
