...
steps=[("r",RandomForestClassifier(n_estimators=100,random_state=42,max_depth=12,max_features="log2"))]
modelR=Pipeline(steps=steps)
modelR.fit(x_train,y_train)
modelR.score(x_test,y_test)
yr_pred=modelR.predict(x_test)
...
submission=res.to_excel("RFC.xlsx",index=False)