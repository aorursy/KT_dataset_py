...
query = """ SELECT merkle_root, COUNT(transaction_id) as merkle_trans
                FROM `bigquery-public-data.bitcoin_blockchain.transactions`
            GROUP BY merkle_root
            ORDER BY merkle_trans DESC
        """
num_merkle = bitcoin_blockchain.query_to_pandas_safe(query, max_gb_scanned=37)
print(num_merkle.head())