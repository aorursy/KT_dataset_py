...
data = pd.read_csv('/kaggle/input/us-accidents/US_Accidents_June20.csv', encoding = "ISO-8859-1")
data['Date'] = data[u'Start_Time'].apply(lambda x: str(x)) + '-' + data[u'End_Time'].apply(lambda x: str(x)) #создает колонку день-месяц(где-то сохраняется случай)
