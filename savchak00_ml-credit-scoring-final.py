...
sns.countplot(x='ProductId', hue='IsDefaulted', data=train, ax=axes[0]);

sns.countplot(x='IsFinalPayBack', hue='IsDefaulted', data=train, ax=axes[1]);
...
#train['DefaultedProductId'] = train['ProductId'].map(train.groupby('ProductId').sum()['IsDefaulted'])

#test['DefaultedProductId'] = test['ProductId'].map(train.groupby('ProductId').sum()['IsDefaulted'])
train
...
X_train
xgb1 = XGBClassifier(max_depth=4,colsample_bytree=0.6,min_child_weight=10,learning_rate=0.25,n_estimators=100,objective = "binary:logistic",random_state=26)
...
xgb_cv = GridSearchCV(xgb1,param_grid=parameters,cv=5,n_jobs=-1)
