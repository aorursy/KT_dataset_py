...
df.label = df.label.apply (lambda i : labels[i] ) # Remplace le numéro de produit par son type
df.head()
y = df['label']
...
X1 = np.array(X)
print(X1[0])
image = X1[50].reshape(28,28) #reconstruction de la ligne 50 en une matrice 28x28
print(image)
plt.imshow(image)
