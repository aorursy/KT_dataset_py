...
def get_sq_res_label(data_frame):
    return data_frame['error_' + LABEL_COLUMN] ** 2
...
transform_expected_sq_res = get_cv_model_transform(work_data, get_sq_res_label, extract_error_vars, get_error_trainer(), 
                n_splits=3, response_column='expected_sq_res_' + LABEL_COLUMN)
work_data = transform_expected_sq_res(work_data)
print_evaluation(work_data, get_sq_res_label, 'expected_sq_res_' + LABEL_COLUMN)
...
transform_list = [transform_init, 
                  ...
                  transform_blend,
                  transform_error, transform_expected_sq_res, transform_anomaly]
...
normal_star_pool = data[(data_anomalies < anomaly_std) & (data_anomalies > -anomaly_std)]
normal_control_group = normal_star_pool.sample(len(candidates))
...
def save_data(data_frame, file_name):
    data_frame[SAVED_COLUMNS].to_csv(file_name, index=False)
