...
def train_audio(y_train, y_val, y_test, batch_size, epochs, learning_rate):  
    ...
    model_audio.add(Dense(1,activation='sigmoid'))
...
def combined(duration,y_pred_audio,y_pred_text,y_test):
    ...
    for alpha in alpha_range:
        ...
        y_pred_labels=get_class(y_pred_combined)

        print("MAtthew coeff="+str(matthews_corrcoef(y_test, y_pred_labels)))

        

        save_path ="CNN_clf_LateFusion_duration_{dur}_alpha_{alph}.pkl".format(dur=duration,alph=alpha)

        pickle.dump(y_pred_combined, open(save_path, 'wb'))

        

        test_f1 = f1_score(y_test,y_pred_labels)

        print("F1 for alpha={alpha_val}:".format(alpha_val=alpha)+" "+str(test_f1))
