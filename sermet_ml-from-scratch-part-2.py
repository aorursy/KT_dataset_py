...
model = ensemble_rad_lr
model.fit(train_X1,train_Y1)
prediction=model.predict(test_X1)
print(metrics.accuracy_score(prediction,test_Y1))
...
labels=[0, 1]
cm = confusion_matrix(test_Y1,prediction, labels)
#pd.crosstab(confusion_matrix, rownames=['True'], colnames=['Predicted'], margins=True)
...
fpr, tpr, thresholds = roc_curve(test_Y1, prediction)
plt.plot([0,1],[0,1],'k--')
plt.plot(fpr,tpr, label='ens')
plt.xlabel('fpr')
plt.ylabel('tpr')
plt.title('All 3 classifiers combined ROC curve')
plt.show()
#Area under ROC curve
from sklearn.metrics import roc_auc_score
roc_auc_score(test_Y1,prediction)