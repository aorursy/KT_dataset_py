...
def barPlot(variable):
    ...
    print("varValue is ",varValue)

    print("varValue.index is",varValue.index)

    print("varValue.index.values",varValue.index.values)
...
def histPlot(variable):
    ...
    plt.hist(traindf[variable],bins=100)
...
traindfLen1=len(traindf)

traindf=pd.concat([traindf,testdf],axis=0).reset_index(drop=True)
traindf.head()
...
np.mean(traindf[traindf["Pclass"]==3]["Fare"])
