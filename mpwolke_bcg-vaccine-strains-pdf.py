...
pdf = os.path.join(outputFolder,'TB.pdf[2]')

with Img(filename=pdf, resolution=300) as img:

    img.compression_quality = 99

    img.convert("RGBA").save(filename='/kaggle/working/TB.jpg') # intro page to preview later
# Parse a PDF file and convert it to CSV using PyTesseract

import pytesseract

pdfimage = Image.open('/kaggle/working/TB.jpg')
...
wordBarGraphFunction(df.T,0,"Most Common Words on Page 2 of TB")
...
parsed = parser.from_file('/kaggle/working/TB.jpg') 
...
pdf = os.path.join(outputFolder,'TB.pdf[1]')

with Img(filename=pdf, resolution=300) as img:

    img.compression_quality = 99

    img.convert("RGBA").save(filename='/kaggle/working/TB.jpg')

pdfimage1 = Image.open('/kaggle/working/TB.jpg')
# PDF to JPG for p4
...
pdfimage2 = Image.open('/kaggle/working/TB.jpg')



# PDF to JPG for p3

pdf = os.path.join(outputFolder,'TB.pdf[3]')
...
pdfimage3 = Image.open('/kaggle/working/TB.jpg')
...
# Parse a PDF file and convert it to CSV using Tika-Python (p2-4)

tika.initVM()

parsed = parser.from_file('/kaggle/working/TB.jpg')

parsed2 = parser.from_file('/kaggle/working/TB.jpg')
...
dfcombined = pd.concat([df, df2, df3]) # combine pages 2-4
#Explore page 2 -  BCG vaccine strains. Here I don't know how many pages each Cheat Sheet. There are 4 pages 
...
# I will do this for Pages 1 to 4 but
...
pdfimage1 = Image.open('/kaggle/working/TB.jpg')
#Explore page 1 - Mueller Report. Here I don't know how many pages each Cheat Sheet. There are 30 pages 
...
dfcombined.head() # preview csv of 1-4
