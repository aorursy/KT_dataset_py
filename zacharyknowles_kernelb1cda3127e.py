...
# show the trend of cat and dog group by their outcome type by year
...
#read the file
file = files()
data_origin = read(file[0]);
head(data_origin, 10)

#exclude the column which don't have an impact on the prediction
data= exclude(data_origin,['animalid','name','datetime','outcomesubtype'])
data
#transfer all age data to the unit 'days' ,eg: 3days
data = Age_process(data)
data
ageuponoutcome = tally(data['ageuponoutcome'],multiple = True,min_count = 10)
ageuponoutcome
#use bag of words on the color of animals, eg:white/black transfer to [white, black]
data['color'] = remove(data['color'], ' ')
splits = split(data['color'], '/')
head(splits, 10)
#use tally on color 
color = tally(splits,multiple = True,min_count = 10)
#head(color, 10)
color
#plot all the statistics of all colors
sum_color = col_operation(color, 'sum')
plot(x = sum_color, style = 'barplot')
data['breed'] = remove(data['breed'], ' ')
splits = split(data['breed'], '/')
breed = tally(splits,multiple = True,min_count = 10)
head(breed, 10)
#use tally on sexuponoutcome
a = tally(data['sexuponoutcome'],multiple = True,method = 'count')
head(a,10)
##use tally on animaltype
b = tally(data['animaltype'],multiple = True,minimum = 3,method = 'count')
head(b,10)
#combine all training data
NewData = hcat(color, breed,ageuponoutcome,a,b)
head(NewData, 10)
...
#analyse the effect of each attribute
result = model.coefficients()
head(result, 10)
result
#clean the data for linear training model
data2= exclude(data_origin,['animalid','name','datetime','outcomesubtype','breed'])
data2 = replace(data2, [['Dog',1], ['Cat',2]])
data2 = Age_process(data2)
data2 = processcolor(data2)
data2 = sex_process(data2)
...
Y
#traning set
X = exclude(data2, 'outcometype' )
X
#train model
model = LinearModel(logistic = True, layers = 1)
model.fit(X, Y)

#analyse the statistics of attribute
