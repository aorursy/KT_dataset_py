...
classcounter = dict.fromkeys(classes, 0)

for x in dataset:

    img, label = x

    classcounter[classes[label]] += 1
...
class CIFAR10Model(ImageClassificationBase):
    ...
    def __init__(self, input_size, output_size):
        ...
        self.in_layer = nn.Linear(input_size, 8192)

        

        self.hidden1 = nn.Linear(8192, 2048)
...
history
history += fit(20, 0.6, model, train_loader, val_loader)
history += fit(10, 0.4, model, train_loader, val_loader)
history += fit(5, .2, model, train_loader, val_loader)
history += fit(10, 0.1, model, train_loader, val_loader)
plot_losses(history)
plot_accuracies(history)
evaluate(model, test_loader)
arch = "6 layers (8192, 2048, 1024, 256, 64, 10)"
lrs = [[0.5, 0.4, 0.3, 0.1, 0.01]]
epochs = [20, 10, 5, 10]
test_loss, test_acc = evaluate(model, test_loader)

# test_acc = evaluate(model, test_loader)[1]

# test_loss = evaluate(model, test_loader)[0]
