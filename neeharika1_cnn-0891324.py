...
(A_train, B_train),(A_test, B_test) = cifar10.load_sdata()
...
A = tf.placeholder(tf.float32, [None, 32, 32, 3]) # [N,H,W,C]

B = tf.placeholder(tf.int64, [None,1])



A_extend = tf.reshape(A, [-1, 32,32,3])

B_onehot = tf.one_hot(indices = B, depth = Tot_Classes)
...
conv1 = tf.nn.conv2d(A_extend, conv1_w, strides = [1,1,1,1], padding= 'SAME')+conv1_b
...
fl1 = tf.layers.dense(inputs= flatten, units = 512, activation = tf.nn.relu, use_bias =True)



"""first fully connect layer"""

fl2 = tf.layers.dense(inputs= fl1, units = 512, activation = tf.nn.relu, use_bias =True)



"""output layer"""

out_put = tf.layers.dense(inputs=fl2, units = Tot_Classes, activation = None, use_bias= True)



"""loss function"""

lossf= tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=B_onehot, logits=out_put))



"""accuracy function"""

accur = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(out_put, axis = 1), B[:,0]), dtype = tf.float32))



"""optimizer"""

opti = tf.train.AdamOptimizer(0.001).minimize(lossf)
...
for epoch in range(0, EPOCHS):
    ...
    for step in tqdm(range(int(len(A_train)/BATCH_SIZE)), desc=('Epoch '+str(epoch))):

        """get next batch of training data"""

        A_batch = A_train[step*BATCH_SIZE:step*BATCH_SIZE+BATCH_SIZE]

        B_batch = B_train[step*BATCH_SIZE:step*BATCH_SIZE+BATCH_SIZE]

        """train"""

        lossf_value, _ = sessi.run([lossf, opti], feed_dict={A: A_batch, B: B_batch})

        

        """her, to make the code simple, I only use the first 1000 images from testing dataset to test the network."""

        lossf_value, accuracy_value = sessi.run([lossf, accur], feed_dict= {A:A_test[:1000], B: B_test[:1000]})

        print('Epoch lossf: ', lossf_value,'  accuracy:  ', accuracy_value)
import numpy as np
...
conv1_w_extract = sessi.run(conv1_w)
...
conv1_fmaps = sessi.run(relu1, feed_dict = {A: [A_train[0]]})
...
w2v_df = pd.DataFrame(vectors, columns = ['A1', 'A2'])

w2v_df['word'] = words

w2v_df = w2v_df[['word', 'A1', 'A2']]
...
for word, x1, x2 in zip(w2v_df['word'], w2v_df['A1'], w2v_df['A2']):
