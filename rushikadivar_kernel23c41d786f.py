...
data_Name = {classes[label]:0 for label in range(num_classes)}
for data in dataset:
    img, label = data
    data_Name[classes[label]]+= 1
data_Name
...
history
history += fit(7, 0.04, model, train_loader, val_loader)
history += fit(7, 0.003, model, train_loader, val_loader)
history += fit(7, 0.0002, model, train_loader, val_loader)
history += fit(7, 0.00001, model, train_loader, val_loader)
plot_losses(history)
plot_accuracies(history)
final_Result = evaluate(model, test_loader)
print(final_Result)
arch = "3 layers (1024,256,10)"
lrs = [0.04, 0.003, 0.0002, 0.00001]
epochs = [7]
