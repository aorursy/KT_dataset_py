...
class CovingtonParser(nn.Module):
    ...
    def oracle(self, deptree):

        sequence = []

        S1, S2, Buff, graph = self.init_config(len(deptree))

        while(len(Buff)>0 or (len(S2)>0 and S2[0]!=0)):

            if(len(S1)>0 and deptree.get_arc(Buff[0], S1[-1])!=None and S1[-1]!=0):

                sequence.append((S1,S2,Buff,graph,self.LEFT_ARC))

                S1, S2, Buff, graph = self.left_arc(deptree.get_arc(Buff[0], S1[-1])[1], S1, S2, Buff, graph)

            elif(len(S1)>0 and deptree.get_arc(S1[-1], Buff[0])!=None):

                sequence.append((S1,S2,Buff,graph,self.RIGHT_ARC))

                S1, S2, Buff, graph = self.right_arc(deptree.get_arc(S1[-1], Buff[0])[1], S1, S2, Buff, graph)

            elif(len(S1)==0):

                sequence.append((S1,S2,Buff,graph,self.SHIFT))

                S1, S2, Buff, graph = self.shift(S1, S2, Buff, graph)

            else:

                sequence.append((S1,S2,Buff,graph,self.NO_ARC))

                S1, S2, Buff, graph = self.no_arc(S1, S2, Buff, graph)
...
covington = CovingtonParser()

istream = open('/kaggle/input/covington-parsing/fr_sequoia-ud-train.conllu')

deptree = DepGraph.read_tree(istream)

sequence = covington.oracle(deptree)
