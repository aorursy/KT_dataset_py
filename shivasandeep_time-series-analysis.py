...
stock_prices_samples = "../input/stock_prices_sample.csv"
def mean_absolute_percentage_error(y_true, y_pred):

    return np.mean(np.abs((y_true - y_pred) / y_true)) * 100
data = stock_prices_sample
data.head()
data.info()
data = data[data.TICKER != 'GEF']
data = pd.read_csv(stock_prices_samples,index_col=['DATE'], parse_dates=['DATE'])
...
#Smooth by the previous 30 days (by month) WIndow size 30
