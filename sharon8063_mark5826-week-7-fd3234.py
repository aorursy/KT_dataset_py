...
n = 10000
p = 20
X, y = make_classification(random_state = 0, n_samples = int(n/2), n_features = p, n_classes = 2)
model = Inference(logistic = True)
model
model.fit(X, y)
model.coefficients()
X, y = make_classification(random_state = 0, n_samples = int(n/2), n_features = p, n_classes = 3, n_informative = int(p/3))
model = Inference(logistic = True)
model.fit(X, y)
model.coefficients()
data = read('techcrunch_posts.csv')
describe(data)
head(data, 5)
text = data['content']
cleaned = clean_text(text)
head(cleaned, 5)
cleaned_tallied = tally(cleaned, multiple = True, min_count = 1000, method = 'count')
X = cleaned_tallied.values
columns = cleaned_tallied.columns
from scipy import sparse
from sklearn.metrics.pairwise import pairwise_distances
C = sparse.csr_matrix(cleaned_tallied.values)
topics = 10
size = C.shape[1]

means2 = C.power(2).mean(0)
mean = np.square(C.mean(0))
variances = np.array(means2 - mean).flatten()

V = np.vstack((np.arange(size), variances)).T
V = np.flipud(V[np.argsort(V[:,1])])

top = V[:,0][:topics*10].astype(np.uint)
top_values = C[:,top].todense()

cosine_d = pairwise_distances(top_values.T, metric = 'cosine', n_jobs = 1)
largest_variance = np.flipud(cosine_d.mean(axis = 1).argsort())[:topics]
base_topics = top[largest_variance]

#----------------------------------------------------------------------
other_words = list(set(range(size)) - set(base_topics))

#----------------------------------------------------------------------
Y = C[:,base_topics].T.todense()
X = C[:,other_words].T

distances = pairwise_distances(X = X, Y = Y, metric = 'cosine', n_jobs = 1)

probabilities = 1-distances
probabilities = np.divide(probabilities, probabilities.sum(1).reshape(-1,1), out = probabilities)
C.shape
print(columns[top])
print(columns[base_topics], base_topics)
base_prob = pd.DataFrame(np.eye(topics))
base_prob.index = base_topics
base_prob.columns = base_topics

prob = pd.DataFrame(probabilities)
prob.index = other_words
prob.columns = base_topics

probabilities = pd.concat([prob, base_prob], 0)
probabilities.columns = columns[base_topics]
probabilities.index = probabilities.index.astype(int)
probabilities = probabilities.loc[range(columns.shape[0])]
probabilities.index = columns
probabilities = probabilities.loc[probabilities.max(1).sort_values(ascending = False).index]
argmax = probabilities.idxmax(1)
clusters = {x:[] for x in probabilities.columns}
for word,group in zip(argmax.index, argmax.values):
    clusters[group].append(word)
for i,x in enumerate(clusters.keys()):
    print('\nTopic {}\n------- [{}]'.format(i, clusters[x][0]))
    print(clusters[x][1:20])
from sklearn.datasets import make_classification, make_regression
data = read('techcrunch_posts.csv')
...
n = 100000
p = 200
X, y = make_regression(n_samples = int(n), n_features = p)
model = Inference(logistic = False)
model.fit(X, y)
coefficients = model.coefficients()
head(coefficients)
# Accroding to the P-values, 178 features are relatively significant (at significance level '@@@'), 
# 5 are slightly significant (at significance level '@@'), and the rest 17 are irrelavent (at significance level 'X')
print(coefficients.groupby('sig').count())
print(coefficients[coefficients['coef']>0].groupby('sig').count())
print(coefficients[coefficients['coef']<0].groupby('sig').count())