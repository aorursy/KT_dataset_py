...
def generate_samples(model, n, x_range, noise=0):
    ...
    ys = ''' how to get the ys for the xs? '''
...
def mean_squared_error(samples, model):
    xs = ''' get the xs... '''
    ys = ''' and the ys... '''
    predicted_ys = ''' and finally the predicted ys... '''
...
def standard_error(samples, model):
    return math.sqrt(''' what is the definition of standard error? ''')
