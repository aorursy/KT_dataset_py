...
#TODO
data['safe_loans']=1
data.loc[data['bad_loans']==1,'safe_loans']=-1
...
safe_loans = safe_ones.sample(frac=percentage, random_state=0)

# combine two kinds of loans
data_set = pd.concat([safe_loans,risky_loans]) #TODO
...
trainX, testX, trainY, testY = train_test_split(data_set.iloc[:,1:], data_set.iloc[:,0], test_size=0.2, random_state=33)
trainY=pd.DataFrame(trainY,columns=[data_set.columns[0]])
testY=pd.DataFrame(testY,columns=[data_set.columns[0]])
...
def best_split_entropy(data, features, target):
    ...
    for feature in features:
        ...
        entropy_split = left_entropy*len(left_split)/(len(left_split)+len(right_split))+right_entropy*len(right_split)/(len(left_split)+len(right_split))#TODO
...
class MyDecisionTree(BaseEstimator):
    ...
    def create_tree(self, data, features, target, current_depth = 0, max_depth = 10, min_error=0):
        ...
        # 剔除已经用过的特征
        remaining_features = features.drop(split_feature)# TODO
        ...
        # 递归上面的步骤
        left_tree = self.create_tree(left_split, remaining_features, target, current_depth +1,max_depth = self.max_depth, min_error=self.min_error)# TODO     
        right_tree =self.create_tree(right_split, remaining_features, target, current_depth +1,max_depth = self.max_depth, min_error=self.min_error) # TODO
    ...
    def create_leaf(self, target_values):
        ...
        positive_ones =target_values.apply(lambda x: x==1).sum()
        negative_ones =target_values.apply(lambda x: x==-1).sum()
    

        if positive_ones > negative_ones:
            leaf.prediction = 1
        else:
            leaf.prediction = -1
    ...
    def predict_single_data(self, tree, x, annotate = False):   
        ...
        if tree.is_leaf:
            ...
            if split_feature_value == 0:
                ...
                return self.predict_single_data(tree.left, x, annotate = False)
            else:
                #如果数据在该特征上的值为0，交给右子树来预测
                return self.predict_single_data(tree.right, x, annotate = False)    
    ...
    def count_leaves_helper(self, tree):
        ...
        return self.count_leaves_helper(tree.left)+self.count_leaves_helper(tree.right)+1
...
model_4 = MyDecisionTree(max_depth = 7, min_error = 0) # TODO
model_5 = MyDecisionTree(max_depth = 7, min_error = 20)# TODO
model_6 = MyDecisionTree(max_depth = 7, min_error = 50)# TODO
