...
def get_fmalec(passenger):
    emc,female=passenger
    if ( emc==1 and female==1):
        return 1
    else:
        return 0

titanic_df['Fc'] = \
    titanic_df[['C','Female']].apply( get_fmalec, axis=1 )
test_df['Fc'] = \
    test_df[['C','Female']].apply( get_fmalec, axis=1 )
...
titanic_df.drop(['Alone','SibSp','ParchBinary','Parch','MasterMiss','FamilySize','SmallFamily','S','C','Fc'],axis=1,inplace=True)
test_df.drop(['Alone','SibSp','ParchBinary','Parch','MasterMiss','FamilySize','SmallFamily','S','C','Fc'],axis=1,inplace=True)
...
rfe = RFE(model, 13)#(fitting model, number of features)
...
rf=random_search #from cross-validatio
knn = KNeighborsClassifier(n_neighbors = 3)


vcr=VotingClassifier(estimators=[('lg',logreg),('xgb',xgb),('rf',rf),('knn',knn)],voting='soft')
