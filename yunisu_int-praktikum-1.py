...
# Mögliche Folgezustände längere Code
"""def gib_folgezustaende(zustand):
    result = []
    m,k,b = zustand
    b_new = 0 if b else 1    
    ...
    # Beginn: Boot auf westlicher Seite ->  mit folgenden Möglichkeiten,
    #  welche Personen auf die andere Seite gebracht werden können, hier max. 4
    if b:
        if k > 0:
            if m > 0:
    ...
    # Endziel: Boot auf östlicher Seite/ anderer Seite ->  mit folgenden Möglichkeiten,
     # welche bzw. wie viele  Personen das Schiff wieder zur westlichen Seite bringen können
    if not b:
        if k < 4:
            if m < 4:
                result.append([m+1,k+1,b_new])
...
##suche(startzustand,[],debug=2)

max_value = 4
suche(newStartzustand,[],all_solutions=True,debug=0)
