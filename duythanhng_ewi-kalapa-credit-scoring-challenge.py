...
with open('../input/ewiaddeddatasetkalapachallenge/tinhhuyen.json', 'r') as openfile:
    diaChinhDict = json.load(openfile)
    
with open('../input/ewiaddeddatasetkalapachallenge/DATATinh.json', 'r') as openfile:  
    DATA = json.load(openfile) 
colname = DATA["ha noi"].keys()

with open('../input/ewiaddeddatasetkalapachallenge/QuanHuyenDict.json', 'r') as openfile:  
...
testDiaChi_Field48_Field49.head()
...
#     data = df.drop(columns=unless_features, inplace=True)
    ...
    s1 = u'ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚÝàáâãèéêìíòóôõùúýĂăĐđĨĩŨũƠơƯưẠạẢảẤấẦầẨẩẪẫẬậẮắẰằẲẳẴẵẶặẸẹẺẻẼẽẾếỀềỂểỄễỆệỈỉỊịỌọỎỏỐốỒồỔổỖỗỘộỚớỜờỞởỠỡỢợỤụỦủỨứỪừỬửỮữỰựỲỳỴỵỶỷỸỹ'
    s0 = u'AAAAEEEIIOOOOUUYaaaaeeeiioooouuyAaDdIiUuOoUuAaAaAaAaAaAaAaAaAaAaAaAaEeEeEeEeEeEeEeEeIiIiOoOoOoOoOoOoOoOoOoOoOoOoUuUuUuUuUuUuUuYyYyYyYy'
    def remove_accents_and_blank(x):
        if str(x) != 'nan':
            input_str = str(x).lower()
            s = ''
            for c in input_str:
            ...
            return x
    data['currentLocationCity'] = data['currentLocationCity'].apply(remove_accents_and_blank)
    data['currentLocationName'] = data['currentLocationName'].apply(remove_accents_and_blank)
    data['currentLocationState'] = data['currentLocationState'].apply(remove_accents_and_blank)
    data['homeTownCity'] = data['homeTownCity'].apply(remove_accents_and_blank)
    data['homeTownState'] = data['homeTownState'].apply(remove_accents_and_blank)
    data['homeTownName'] = data['homeTownName'].apply(remove_accents_and_blank)
...
merge_test_data = pd.concat([test_data, testLocation_EncodeOnly, testDiaChi_Field48_Field49], axis=1)
