...
# TODO 名字长度与幸存的关系
data['survived'].groupby(data['name_len']).mean()
...
# TODO 登船地点与幸存的关系
pd.crosstab(data.embarked, data.survived).plot(kind = "bar", stacked = True)
...
drop_columns = ['row.names', 'pclass', 'survived', 'name', 'age', 'embarked','sex', 'name_title', 'name_len']
...
#TODO 生成graph文件
graph =  tree.export_graphviz(model, out_file=None, feature_names=trainX.columns) 
