...
df= pd.read_csv("../input/washdatasets/washdash-download.csv")
df.head()
df['Service Type']
...
hygiene = pd.read_csv("../input/washdatasets/hygiene_cleaned.csv")

hygiene=hygiene.drop(['Unnamed: 0'], axis=1)

hygiene.columns=['Region', 'Year', 'Basic Hygiene','Limited Hygiene','No handwashing facility']

sanitation=pd.read_csv("../input/washdatasets/sanitation_data(new).csv")
