...
for column1, column2 in itertools.combinations(cat_features, 2): # math combinations, combines every pair of n things in an array

        new_column = '_'.join([column1, column2])

        new_values = clicks[column1].map(str) + "_" + clicks[column2].map(str)

        interactions[new_column] = encoder.fit_transform(new_values)
...
def count_past_events(series):
    ...
    count = series.rolling('6h').count() - 1
...
# No, you don't have future information if you are trying to use a model to make predictions.
