...
class my_gbdt(object):
    ...
    def __init__(self,num_trees,lr,min_samples_split=2,max_depth=2):
        ...
        self.tree = DecisionTreeRegressor(min_samples_split=self.min_samples_split, max_depth=self.max_depth)
    ...
    def fit(self, X, y):
        ...
        for i in range(self.num_trees):
            ...
            temp_tree = DecisionTreeRegressor(min_samples_split=self.min_samples_split, max_depth=self.max_depth)

            temp_tree.fit(X,res)

            pred = pred + self.lr * temp_tree.predict(X)
    ...
    def predict(self, X):
        ...
        for i in range(self.num_trees):
            ...
            pred_proba = pred_proba + self.lr * self.trees[i].predict(X)

        return np.argmax(pred_proba, axis=1)
my_model = my_gbdt(10,0.1,2,4)
