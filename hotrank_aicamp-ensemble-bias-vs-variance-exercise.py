...
# apply random sampling instead of just taking the first trainN data
all_idx = set(range(N))
train_idx = np.random.choice(N, size = trainN)
...
for k in range(Num_datasets):
    ...
    trainX, testX = allData[train_idx], allData[test_idx]
...
#     trainY, testY = Y[:trainN], Y[trainN:]
    ...
    for d in range(max_degree):
        ...
        train_prediction = all_predictions[train_idx]
        test_prediction = all_predictions[test_idx]
...
for d in range(max_degree):
    ...
    plt.plot(x_axis, prediction_curves[:,:,d].mean(axis = 1), color = 'red', linewidth = 3)
...
for d in range(max_degree):
    for i in range(trainN):
        #TODO
#         difference = train_predictions[i,:,d] - trueY_train[i]
        variances[i,d] = np.var(train_predictions[i,:,d] )
