...
def starts_and_ends_with_vowel(letters):
    ...
    for x in letters[0] and letters[-1]:

        if x in letters[0] and letters[-1] in vowels:
...
def first_to_last(letters):

    letters.insert(len(letters), letters.pop(0))
...
def median(number):
    ...
    if n % 2 == 1:
        ...
        return sorted(number)[n//2]

    else:

        return sum(sorted(number)[n//2-1:n//2+1])/2.0
...
# Write a function definition named get_nique_values that takes in a list and returns a set with only the unique values from that list.
...
def get_unique_values_from_two_lists(mylist, yourlist):

    list_set = set(mylist + yourlist)
...
def get_values_in_common(mylist, yourlist):

    return set(mylist) .intersection(yourlist)
...
def get_values_not_in_common(mylist, yourlist):

    return set(mylist).symmetric_difference(yourlist)
...
def get_paper_title(a_dictionary):
...
def get_year_published(a_dictionary):

    return a_dictionary["year_published"]
...
def get_price(a_dictionary):

        return a_dictionary["price"]
...
def get_book_author(a_dictionary):

    return a_dictionary["author"]
...
def get_number_of_books(a_dictionary):
...
def total_of_book_prices(a_dictionary):

    return sum(item["price"] for item in a_dictionary)
...
def get_average_book_price(a_dictionary):

    return sum(item["price"] for item in a_dictionary)/len(a_dictionary)
...
def highest_price_book(a_dictionary):

    return max(a_dictionary, key=lambda x: x['price'])
...
def lowest_price_book(a_dictionary):

    return min(a_dictionary, key=lambda x: x['price'])
...
def get_tax_rate(a_dictionary):

    return a_dictionary["tax"]
...
def number_of_item_types(a_dictionary):

    unique_items = a_dictionary["items"]
...
def total_number_of_items(a_dictionary):

    items = a_dictionary['items']

    count_of_items = items[0]['quantity'] + items[1]['quantity'] + items[2]['quantity'] + items[3]['quantity'] + items[4]['quantity']
...
def get_average_item_price(dik):
...
def get_average_spent_per_item(dik):

    items = dik['items']

    items_count = items[0]['quantity'] + items[1]['quantity'] + items[2]['quantity'] + items[3]['quantity'] + items[4]['quantity']
    ...
    items_average = items_total_price/items_count
