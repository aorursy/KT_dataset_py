...
plt.grid(True)
plt.axis([2,3,0,10])
print(plt.xlim(0,5))  # range of axis x
print(plt.ylim(-1,13))  # range of axis y
plt.plot([1,3,2,4])
# TODO
...
sin_y = np.sin(x)
cos_y = np.cos(x)
plt.plot(x, sin_y, color='red', linewidth=3.0, linestyle=':', label='sin')
plt.plot(x, cos_y, color='blue', linewidth=3.0, linestyle='-', label="cos")
plt.grid(True)
plt.title('Mumu')
plt.legend(loc=(1,0))
...
X,Y = np.meshgrid(x, y)
plt.imshow(f(X,Y))
Ct = plt.contourf(X, Y, f(X, Y), 8, alpha=.65, cmap=plt.cm.hot)
Ct = plt.contour(Ct, linewidths= 0.5)
plt.clabel(Ct, inline = True, fontsize = 10)
# use plt.contour to add contour lines
C_lines = plt.contour(X, Y, f(X, Y), 8, colors='black', linewidths=1.5)
#TODO 3D作图   
%matplotlib notebook
...
ax.plot_surface(X, Y, f(X, Y), rstride=1, cstride=1, cmap=plt.cm.RdBu)  
ax.contourf(X, Y, f(X, Y), zdir='z', offset=-3, cmap=plt.cm.RdBu)  
#ax.set_zlim(-3,3)  
#ax.plot_surface(X,Y,f(X, Y), cmap=plt.cm.coolwarm)
