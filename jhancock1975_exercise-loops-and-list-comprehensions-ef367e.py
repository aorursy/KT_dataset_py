...
def estimate_average_slot_payout(n_runs):
    ...
    return sum/n_runs
# use law of large numbers to estimate expected
# value of estimate_average_slot():
sum = 0
n_experiments = 10
for i in range(n_experiments):
    sum += estimate_average_slot_payout(1000000)
print('expected value of estimate_average_slot_payout is approximately: %f' % (sum/n_experiments))
q4.solution()
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    for i in range(n_simulations):
        bal = start_balance
        spins=0
        while (spins < n_spins) and (bal >= 1):
            bal -= 1
            bal += play_slot_machine()
            spins += 1
        if spins == n_spins:
            n_survivals += 1
    return n_survivals/n_simulations
q5.check()
#q5.solution()