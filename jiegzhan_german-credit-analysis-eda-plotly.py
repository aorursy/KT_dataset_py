...
numeric_matrix = numeric_columns.as_matrix()
...
# Fillnas of the two columns that have missing values.
original_df['Checking account'] = original_df['Checking account'].fillna(df['Checking account'].mode()[0])
original_df['Saving accounts'] = original_df['Saving accounts'].fillna(df['Saving accounts'].mode()[0])
original_df.isnull().sum().sort_values(ascending=False)
...
X_train_scaled_data = main_pipeline.fit_transform(X_train)
X_test_scaled_data = main_pipeline.fit_transform(X_test)
from sklearn.preprocessing import LabelEncoder
