...
#test set has nan values in both numeric and non-numeric values, dealing with missing values  

from sklearn.impute import SimpleImputer

# replace nan in objects with mode

z = X_test[low_cardinality_cols].isnull().any()

bad_test_obj_cols = list(z[z].index)

for col in bad_test_obj_cols:
...
OH_cols_test.index = X_test[low_cardinality_cols].index
...
my_emputer = SimpleImputer(strategy='median')

imputed_num_X_test = pd.DataFrame(my_emputer.fit_transform(num_X_test))
...
OH_X_test = pd.concat([imputed_num_X_test, OH_cols_test], axis=1)



#Create model an fit with all data 
