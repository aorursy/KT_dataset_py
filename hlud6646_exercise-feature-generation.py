...
for f1_name, f2_name in combinations(cat_features, 2):

    

    f3_name = f1_name + "_" + f2_name

    f3_vals = clicks[f1_name].map(str) + "_" + clicks[f2_name].map(str)

    

    enc = preprocessing.LabelEncoder()

    interactions[f3_name] = enc.fit_transform(f3_vals)
...
def count_past_events(series):
    ...
    return series.sort_index().rolling('6H').count() - 1
