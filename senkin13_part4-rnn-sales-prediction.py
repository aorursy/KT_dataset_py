...
df[ts_cols] = df[ts_cols].fillna(-9999)
...
def rnn(step, ts_cols):    
    ...
    model.add(LSTM(units=512, activation='relu', kernel_initializer='he_normal', 

                   return_sequences=True, input_shape=(step, len(ts_cols))))

    model.add(Dropout(0.1)) 

    model.add(LSTM(units=256, activation='relu', kernel_initializer='he_normal', return_sequences=True))

    model.add(Dropout(0.1)) 

    model.add(LSTM(units=128, activation='relu', kernel_initializer='he_normal', return_sequences=False))
...
hist = model.fit(train_x_ts, train_y, batch_size=32, epochs=30, validation_data=(test_x_ts, test_y),
