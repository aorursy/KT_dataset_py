...
data_X = data_X.drop(["ps_reg_03", "ps_car_03_cat", "ps_car_05_cat", "ps_car_14", "ps_ind_05_cat", "ps_car_07_cat"], axis = 1)
data_X[data_X == -1].count()
all_hists = X_train.hist(bins=20, figsize=(50,25))
X_train, X_test, y_train, y_test = train_test_split(data_X, data_Y, test_size = 0.3, shuffle = True)
...
# fit the object to our training data
x_train_smote, y_train_smote = resample.fit_sample(X_train_2, y_train_2)
...
y_test_2 = y_test[0:3500]
print('y_test_2 shape: ', y_test_2.shape)
resample = SMOTEENN(enn=EditedNearestNeighbours(sampling_strategy='minority'))
x_train_smote, y_train_smote = smote.fit_sample(X_train_2, y_train_2)
...
print('# of our predicted values: ' , sum(y_predict_smote))
data_test.shape
data_test = data_test.drop(["ps_reg_03", "ps_car_03_cat", "ps_car_05_cat", "ps_car_14", "ps_ind_05_cat", "ps_car_07_cat"], axis = 1)
data_test.shape
data_test.describe()
data_train.shape
data_train = data_train.drop(["ps_reg_03", "ps_car_03_cat", "ps_car_05_cat", "ps_car_14", "ps_ind_05_cat", "ps_car_07_cat"], axis = 1)
data_train.shape
data_test_2 = data_test.drop(columns=['id'])
data_train_X = data_train.drop(columns=['id'])
print('train shape:', data_train_X.shape)
...
X_train_smote_END, y_train_smote_END = resample.fit_sample(X_train_END, y_train_END)
...
data_out = pd.DataFrame(data_test['id'].copy())
data_out.insert(1, "target", y_predict_smote, True) 
data_out.to_csv("/home/ozge/ml_install/venv/project/data/final-project-dataset/submission.csv",index=False)
from io import StringIO
