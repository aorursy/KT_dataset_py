...
for i in range(0,49) :
    ...
    plt.axis('off')
    plt.imshow(images[i], cmap="gray_r")
    plt.title(labels[y[i]]) #on affiche le nom de l'objet et non un chiffre
from sklearn.neural_network import MLPClassifier
mlp = MLPClassifier(hidden_layer_sizes=(200,60))
mlp.fit(X_train,y_train)
y_mlp = mlp.predict(X_test)
...
from keras.layers import Dense
model = Sequential()
#on utilise des neuronnes denses pour le moment
#on utilise la fonction d'activation relu
model.add(Dense(200, activation='relu'))
model.add(Dense(60, activation='relu'))
#ici on ajoute une couche de 25 neuronnes (1 neuronne pour chaque lettre)
#pour la derniere couche on utilise une activation softmax. mettre cette fonction activation
# permet que la semble des valeurs des  neuronnes fasse 1
model.add(Dense(num_classes, activation='softmax'))
model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
train = model.fit(X_train , y_train , validation_data=(X_test,y_test), epochs=30, verbose=1)
model.evaluate(X_test,y_test)
