...
def load_data(batch_size=10):
        ...
        path_image = '../input/2d_images/'

        path_mask='../input/2d_masks/'
        ...
        for img_path in batch_images:
            ...
            img_A= imread(path_mask+img_path)

            img_B= imread(path_image+img_path)
...
len(images)
imgs_A,imgs_B=load_data(batch_size=10)
imgs_A.shape,imgs_B.shape
fig, ax = plt.subplots(1,2, figsize = (8,4))

ax[0].imshow(imgs_A[0], cmap='gray')

ax[1].imshow(imgs_B[0], cmap='gray')
...
def load_batch(batch_size=1):
       ...
       # path = glob('../input/%s/%s/%s/*' % (dataset_name,dataset_name, data_type))
        ...
        for i in range(n_batches-1):   #0-8

            batch = np.random.choice(images, size=batch_size)

            imgs_A, imgs_B = [], []

            for img in batch:

                img_A= imread(path_mask+img)

                img_B= imread(path_image+img)
...
img_A,img_B=next(load_batch(batch_size=20))
...
def train(epochs, batch_size=1, show_interval=10):
        ...
        for epoch in range(epochs):
            ...
            for batch_i, (imgs_A, imgs_B) in enumerate(load_batch(batch_size)):
                ...
                g_loss = combined.train_on_batch([imgs_A, imgs_B], [valid, imgs_A])   #[=[valid, fake_A]]
...
train(epochs=50, batch_size=20, show_interval=5)