...
for X_train, X_test in df_list:
    ...
    best_nb, false_preds, test_acc, train_acc = run_model(nb, params, cv, X_train, X_test, y_train, y_test )
...
with gzip.open('nb_best_model.pkl', 'wb') as output:
...
with gzip.open('nb_results.pkl', 'wb') as output:
