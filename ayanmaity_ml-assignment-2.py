...
clf_hp1.fit(train_x_hp,train_y_hp)
clf_hp1.score(test_x_hp,test_y_hp)
clf_hp2 = MLPRegressor(hidden_layer_sizes=(100,50),learning_rate_init=0.01,max_iter=1000,verbose=0)
train_x_hp = data_hp[:,:288]
train_y_hp = data_hp[:,288]
clf_hp2.fit(train_x_hp,train_y_hp)
clf_hp2.score(test_x_hp,test_y_hp)
loss_list1 = clf_hp1.loss_curve_
loss_list2 = clf_hp2.loss_curve_
import matplotlib.pyplot as plt
x1 = list(range(len(loss_list1)))
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.plot(x1,loss_list1)
x2 = list(range(len(loss_list2)))
plt.plot(x2,loss_list2)
