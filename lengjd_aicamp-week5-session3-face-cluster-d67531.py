...
def face_pca(faces, n_components):
    ...
    h = faces.shape[1] #高度
    w = faces.shape[2] #宽度
    faces_data = faces.reshape(faces.shape[0], h * w) #二维变一维，faces.shape[0]--多少张人脸，
     
    
    pca = PCA(n_components=n_components).fit(faces_data) #查下PCA中有没有减均值
    
    eigen_faces = pca.components_.reshape((n_components, h, w)) #特征脸
...
class KMeans():
    ...
    def fit(self, data):
        ...
        cluster_dist = np.full(n, np.inf) #这个点到cluster的最大距离
        centers = self._randCenter(data, self.k) #先random k个质心出来
        
        for _ in range(self.max_iter): #迭代这么多次去算 
            cluster_changed = False
            for i in range(n): #给每个点assign一个label,知道它是属于哪个质心的
                min_dist = np.inf
                min_index = -1
            ...
            if not cluster_changed: #如果质心没有变化，就不需要再进行迭代
                break
            
        
            
            for i in range(self.k): #如果质心变化，就需要更新质心
