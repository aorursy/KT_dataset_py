    ...
    while clusterChanged:
        ...
        for i in range(len(dataSet)):
            ...
            for j in range(k):
                ...
                distance = euclDistance(np.array(centroids)[j, :], np.array(dataSet)[i, :])
        ...
        for j in range(k):
            ...
            pointsInCluster = np.array(dataSet)[np.nonzero(np.array(clusterAssment)[:, 0] == j)[0],:]

            centroids[j][0] = np.mean(pointsInCluster, axis=0)[0]

            centroids[j][1] = np.mean(pointsInCluster, axis=0)[1]
