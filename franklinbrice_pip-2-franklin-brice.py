...
def euler(yin,omega,step):                #complete the function definition here
...
start_time =  0.0     # enter the start time of the propagation
N_loops = 8         # enter number of loops the particle should complete        increased to 8
steps_rev =  1000     # enter the number of time steps per revolution          increased by *2
...
plt.axis('equal')
plt.legend()
plt.title(f"Comparison for AvsN 8 loops 2000 steps per rev")
plt.xlabel(f"x position [m]") 
plt.ylabel(f"y position [m]") 
plt.show()
...
plt.axis([0,N_loops*2*np.pi/omega,-1*(position_y[0]+0.1),position_y[0]+0.1])
plt.plot(time,position_y,'o',label='Npos')
plt.plot(time,exact_position_y,linewidth=2,label='Apos')
plt.title(f"Comparison AvsN 8 loops 2000 steps per rev")
plt.xlabel(f"time [s]") 
plt.ylabel(f"y position [m]") 
plt.legend
plt.show()
