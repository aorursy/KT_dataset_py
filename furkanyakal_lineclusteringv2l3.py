...
column_names = L3_one_hot.columns[1:]
L3_one_hot = L3_one_hot.sample(len(L3_one_hot))
dbscan = DBSCAN()
...
max_cluster_for_l3 = int(np.max(cluster_number_list))

print("MAX NUMBER OF CLUSTERS: {}\n".format(max_cluster_for_l3))

print(cluster_number_list)
L3_one_hot.sort_values(by=['Id'], inplace=True)
n_clusters = max_cluster_for_l3
...
L3_one_hot.insert(1, "ClusterL3", pred)

L3_one_hot.sample(10)
ids_clusters = pd.DataFrame({"Id": one_hot_stations['Id'], "ClusterL3": 0})

ids_clusters.loc[L3_one_hot.index, ['ClusterL3']] = L3_one_hot['ClusterL3']

ids_clusters.to_csv("Cluster_L3_train.csv", index=False)
L3_one_hot[L3_one_hot["ClusterL3"] == randint(1, n_clusters)].head()
L3_one_hot[L3_one_hot["ClusterL3"] == randint(1, n_clusters)].head()
L3_one_hot[L3_one_hot["ClusterL3"] == randint(1, n_clusters)].head()
L3_one_hot[L3_one_hot["ClusterL3"] == randint(1, n_clusters)].head()
...
L3_one_hot_test.insert(0, "Id",one_hot_stations_test["Id"])
pred_test = kmeans.predict(L3_one_hot_test[column_names])
...
ids_clusters_test.to_csv("Cluster_L3_test.csv", index=False)
L3_one_hot_test[L3_one_hot_test["ClusterL3"] == 1].head(10)
L3_one_hot[L3_one_hot["ClusterL3"] == 1].head(10)
L3_one_hot_test[L3_one_hot_test["ClusterL3"] == n_clusters].head(10)
L3_one_hot[L3_one_hot["ClusterL3"] == n_clusters].head(10)