...
df_q1.head()
df_q1.shape
df_q1["date"] = pd.to_datetime(df["date"], format = "%d.%m.%Y")
...
plt.plot(df_q1, color = "red", label ="Daily sales")

plt.title("Daily sales")

plt.legend()
df_q1 = df_q1.resample("D")["item_cnt_day"].sum().to_frame()

df_q1["Variation"] = df_q1["item_cnt_day"].diff(7)/df_q1["item_cnt_day"].shift(7)

df_q1.head(20)
df_q1["MA7d"] = df_q1["item_cnt_day"].rolling(window=7).mean()
df_q1.head(20)
y_sales = df_q1["item_cnt_day"]
...
plot1 = ax.plot(y_sales, label = "Total daily sales", color = "blue", alpha = 0.5)



ax_bis = ax.twinx()

plot2 = ax_bis.plot(y_variation, label="Intra - 7day variation of sales", color = "green", alpha = 0.5)

plot3 = ax.plot(y_ma7d, label="Moving average 7days total sales", color="red", alpha = 0.5)
...
ax.set_title("Total daily sales and 7-day-before variation");
...
df_q2 = df_for_question_2[df_for_question_2["city"] == "Москва"]
df_q2.head()
...
axes[0].set_title("Decomposition of a series in Москва")
...
df_q3 = create_df()
df_q3["item_category_name"].head()
# groupby by item_category_name and sum all the sales

short_df = df_q3.groupby("item_category_name")["item_cnt_day"].sum().to_frame()
...
my_pct = short_df["item_cnt_day"]/short_df["item_cnt_day"].sum()
# create custom labels for each category with their total sales and overall contribution

labels = ['{} - Sales :{}k \n {}% of total'.format(item_category, sales/1000, round(pct, 2)*100) if (pct >= 0.01) else '' for item_category, sales, pct in zip(short_df.index, my_values, my_pct)]
...
# we want to normalize our values, otherwise a category will have the darkest collor and all the others will pale
...
plt.title("Sales by item_category_name and their % over total sales", fontsize = 15, fontweight = "bold")
