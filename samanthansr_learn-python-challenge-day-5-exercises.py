...
def elementwise_greater_than(L, thresh):
    ...
    boolean_list = []
    for num in L:
        boolean_list.append(num > thresh)
    return boolean_list
...
def estimate_average_slot_payout(n_runs):
    ...
    for i in range(n_runs):
        payouts.append(play_slot_machine()-1)
    return round((sum(payouts)/n_runs),4)
...
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    for i in range(n_simulations):
        ...
        spins_left = n_spins
        while balance >=1 and spins_left > 0:
            balance -= 1
...
#play_slot_machine() // returns winnings for 1 round
#estimate_average_slot_payout(n_runs) // returns average payout for n runs
q5.solution()