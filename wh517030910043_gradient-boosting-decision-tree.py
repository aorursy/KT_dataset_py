...
class my_gbdt(object):
    ...
    def __init__(self):
        ...
        for i in range(length):
            ...
            self.treegrow.append(DecisionTreeRegressor(min_samples_split=2, max_depth=2))
    ...
    def fit(self, X, y):
        ...
        for i in range(length):
            ...
            self.treegrow[i].fit(X,y)

            b=[]

            for j in range(a):

                b=grad(self.treegrow[i].predict(X)[j])   
    ...
    def predict(self, X):
        ...
        for i in range(length):
            ...
            preb_probalist.append(self.treegrow[i].predict(X))

        pred_proba=preb_probalist[0]

        for i in range(length-1):

            pred_proba=pred_proba+preb_probalist[i+1]
