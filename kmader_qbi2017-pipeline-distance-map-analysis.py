...
dist_knn_pipeline.fit(numb_image[train_idx],numb_id[train_idx])
rand_digit = np.random.choice(range(len(numb_image))) # just picks a random digit

guess_digit=dist_knn_pipeline.predict(np.expand_dims(numb_image[rand_digit],0))
...
guess_test_data = dist_knn_pipeline.predict(test_image)
