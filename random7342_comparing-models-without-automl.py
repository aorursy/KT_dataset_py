...
rulefit_model = H2ORuleFit(algorithm = "GBM", seed = 1234)
...
f1_sc = f1_score(test['DEFAULT'].as_data_frame(), predictions['predict'].as_data_frame())

roc_score = roc_auc_score(test['DEFAULT'].as_data_frame(), predictions['p1'].as_data_frame())

score = accuracy_score(test['DEFAULT'].as_data_frame(), predictions['predict'].as_data_frame())
...
predictions = binom_model.predict(test)
...
score = accuracy_score(test['DEFAULT'].as_data_frame(), predict_gb['predict'].as_data_frame())
...
print("acc", score)
(pd.Series(model.feature_importances_, index=X_data1.columns)

   .nlargest(20)

   .plot(kind='barh', title='GBM (Sklearn)', figsize=(10,8))).invert_yaxis()