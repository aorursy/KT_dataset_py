...
# TODO 打印name title信息
data['name_title'].head(20)
# TODO 名字title 与幸存的关系
# pd.crosstab(data['name_title'], data['survived']).sort_values(by = 0, ascending = False)[0:4].plot(kind = 'bar')

data['survived'].groupby(data['name_title']).mean()
# TODO 新增名字长度的变量
data['name_len'] = data['name'].apply(lambda x: len(x))
# TODO 名字长度与幸存的关系
data['survived'].groupby(pd.qcut(data['name_len'], 10)).mean()
# TODO 打印性别比例
...
grouped = data.groupby(['name_title', 'pclass'])
grouped.age
def name(data):
    data['name_len'] = data['name'].apply(lambda x: len(x))
    data['name_title'] = data['name'].apply(lambda x: x.split(',')[1] if len(x.split(',')) > 1 else x.split(',')[0]).apply(lambda x: x.split()[0])
...
drop_columns = ['row.names', 'home.dest', 'room', 'boat', 'ticket']
...
# TODO 创建深度为3，叶子节点至少含有5个sample的决策树
model = tree.DecisionTreeClassifier(max_depth = 3, min_samples_leaf = 5, criterion = 'entropy')
...
pd.concat((pd.DataFrame({'feature':trainX.columns}), pd.DataFrame({'importance': model.feature_importances_})), axis = 1).sort_values(by = 'importance', ascending = False)[:20]
