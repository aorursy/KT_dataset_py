...
X_train_new=np.asarray(binarised_train.drop(['Rating','PhoneId'],axis=1))

Y=np.asarray(binarised_train['Rating'])

X_test_new=np.asarray(binarised_test)
class MPNeuron:
...
mp_neuron.fit(X_train_new,Y)
Y_test_pred=mp_neuron.predict(X_test_new)
