...
df.dropna(axis=0, how='any', inplace=True, subset=['release_date','runtime'])
df['year'] = df.release_date.str[:4]

df['year'] = df['year'].astype(int)

df['runtime'] = df['runtime'].astype(int)
stats = df[['year','runtime']].groupby(by='year').agg(['count','mean', 'std', 'median'])

stats
# Code from joypy: https://github.com/sbebo/joypy
...
dfGreaterOne = df[df['year'].isin(stats[stats['runtime']['count']>5].index.values)]

dfFrom1920 = dfGreaterOne[dfGreaterOne.year >= 1920]

fig, axes = joyplot(dfFrom1920[['year','runtime']], by='year', column='runtime', x_range=range(0, 200), figsize=(8,20), color="red") 