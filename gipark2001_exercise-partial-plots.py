...
data.head()
data.describe()
abs(data['dropoff_longitude']).max()
...
feature_1 = ['pickup_longitude', 'dropoff_longitude']

inter1  =  pdp.pdp_interact(model=first_model, dataset=val_X, model_features=base_features, features=feature_1)



pdp.pdp_interact_plot(pdp_interact_out = inter1, feature_names = feature_1, plot_type='contour')
...
pdp_dist = pdp.pdp_isolate(model=my_model, dataset=my_df, model_features=['X1', 'X2'], feature='X2')

pdp.pdp_plot(pdp_dist, 'X2')
