...
df_train = pd.read_csv("../input/pre-processed-base(train)", sep="\t")
...
df_valid = pd.read_csv("../input/pre-processed-base(valid)", sep="\t")
...
mlp_pred_class = mlp_clf.predict(X_valid)

mlp_pred_scores = mlp_clf.predict_proba(X_valid)

accuracy, recall, precision, f1, auroc, aupr = compute_performance_metrics(y_valid, mlp_pred_class, mlp_pred_scores)
...
print(confusion_matrix(y_valid, mlp_pred_class))

#print('Curvas de erro de Treinamento e Validação:')
