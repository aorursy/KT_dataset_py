    ...
    for iters in range(iterations):
        ...
        for node in range(len(dataSet)):
            ...
            for cur_k in range(k):
                ...
                dist.append(euclDistance(centroids[cur_k], dataSet[node]))

            clusterAssment[node] = [np.argmin(dist)]
        ...
        for c in range(k):
            ...
            total.append(np.zeros([1, 2]))



        for node in range(len(dataSet)):

            c = clusterAssment[node][0]

            count[c] += 1

            total[c] += dataSet[node]
        ...
        for c in range(k):
            ...
            if count[c] != 0:
                ...
                centroids[c] = total[c] / count[c]

            else:

                centroids[c] = [centroids[c]]

    

        # Plot graphs

        max_c = np.mat(np.array(centroids))

        max_clu = np.mat(clusterAssment)

        showCluster(dataSet, k, max_c, max_clu)
