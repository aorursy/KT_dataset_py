...
binder.bind(globals())
from learntools.ml_insights.ex3 import *
...
q_1.solution()
# Add your code here
inter1  =  pdp.pdp_interact(model=first_model, dataset=val_X, model_features=base_features, features=['pickup_longitude', 
                                                                                                      'dropoff_longitude'])

pdp.pdp_interact_plot(pdp_interact_out=inter1, feature_names=['pickup_longitude', 'dropoff_longitude'], plot_type='contour')
plt.show()
q_2.solution()
savings_from_shorter_trip = 12.0
...
# create new features
data['abs_lon_change'] = (data.pickup_longitude - data.dropoff_longitude).map(abs)
data['abs_lat_change'] = (data.pickup_latitude - data.dropoff_latitude).map(abs)
...
X2 = 4 * rand(n_samples) - 2
# Create y. you should have X1 and X2 in the expression for y
y = [1 if (abs(x1) <= 0.05) and (abs(x2) <= 0.05) else 0 for x1,x2 in zip(X1,X2)]
