...
from keras.layers import Dense, Dropout, Embedding, Conv1D, Activation, GlobalMaxPool1D
...
model = Sequential()
model.add(Embedding(embedding_input, embedding_dim, weights=[embedding_matrix], input_length=maxlen, trainable=False))
...
model.summary()
history = model.fit(X_train_padded, y_train,epochs=30, verbose=False, validation_data=(X_test_padded, y_test))
