...
hitters_data = pd.read_csv('../input/hitters/Hitters.csv')
...
# Let`s erase the missing values.
df.dropna(inplace = True)
df.shape
#In the data set, we only select to use the CHits variable.
X = df[['CHits']]
y = df[['Salary']]
...
sonuc_test_veri_slr=np.sqrt(mean_squared_error(y_pred,y_test))
sonuc_test_veri_slr
...
#We will use this X, y, X_train and y_train in all remaining models.
y = df["Salary"]
X = df.drop("Salary",axis=1)
X = scaler.fit_transform(X)
for i in range(len(X)):
