...
clf = RandomForestRegressor(max_depth=2, random_state=0, n_jobs=-1, verbose=1)

clf.fit(train_val_x, train_val_y)



pred_val = clf.predict(test_val_x)



err = np.mean(abs(pred_val-test_val_y))

print("L1 err : {}".format(err))
from sklearn.ensemble import RandomForestRegressor
...
clf.fit(train_all_x, train_all_y)



pred_all = clf.predict(test_x)

pred_all = np.round(pred_all)
submit_csv['id'] = np.arange(0, submit_csv.shape[0])

submit_csv['count'] = pred_all
