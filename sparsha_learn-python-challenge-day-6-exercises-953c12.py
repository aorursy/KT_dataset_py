...
def multi_word_search(doc_list, keywords):
    ...
    for each_keyword in keywords: 
        dic[each_keyword]=word_search(doc_list,each_keyword)
...
def diamond(height):
    ...
    """
    s = ''
    l, r = '/', '\\' # To refer backlash as a literal backlash character
    line = height // 2
    for row in range(height):
        ...
        if row < line:   
            ...
            nchars = height - row
        left = (l * nchars).rjust(line)
        right = (r * nchars).ljust(line)
        s += left + right + '\n'
