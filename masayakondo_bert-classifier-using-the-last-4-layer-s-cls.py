...
datasets['text_cleaned'] = datasets['text'].apply(lambda s : clean(s))
...
train_df, test_df = train_test_split(datasets, train_size=0.8)



train_df.head()
train_df = train_df[['text_cleaned', 'target']]

test_df = test_df[['text_cleaned', 'target']]
...
bert_classifier = BertClassifier(tokenizer, model)
TEXT = torchtext.data.Field(sequential=True, tokenize=bert_classifier.bert_tokenizer, use_vocab=False, lower=False,
...
for param in bert_classifier.bert.encoder.layer[-3].parameters():

    param.requires_grad = True



for param in bert_classifier.bert.encoder.layer[-4].parameters():
...
optimizer = optim.Adam([
    ...
    {'params': bert_classifier.bert.encoder.layer[-2].parameters(), 'lr': 5e-5},

    {'params': bert_classifier.bert.encoder.layer[-3].parameters(), 'lr': 5e-5},

    {'params': bert_classifier.bert.encoder.layer[-4].parameters(), 'lr': 5e-5},
...
datasets = datasets[['text_cleaned', 'target']]

datasets.to_csv('./datasets.tsv', sep='\t', index=False, header=None)
...
train_data = torchtext.data.TabularDataset(path='./datasets.tsv', format='tsv', fields=[('Text', TEXT), ('Label', LABEL)])

BATCH_SIZE = 32

train_iter = torchtext.data.Iterator(train_data, train=True, batch_size=BATCH_SIZE)
# fine tuning
...
test_data = torchtext.data.TabularDataset(path='./s_test.tsv', format='tsv', fields=[('Text', TEXT), ('Label', LABEL)])



BATCH_SIZE = 32

test_iter = torchtext.data.Iterator(test_data, batch_size=BATCH_SIZE)
prediction = []
