...
# take a look at them
matches
# use the function above to replace close matches to "kuram agency" 
replace_matches_in_column(df=suicide_attacks, column='City', string_to_match="kuram agency")# use the function we just wrote to replace close matches to "d.i khan" with "d.i khan"
#Now let's check the unique values in our City column again and make sure we've tidied up correctly.
cities = suicide_attacks['City'].unique()
cities.sort()
cities
#Get the top 10 closest matches to "kuram agency" again
matches = fuzzywuzzy.process.extract("kuram agency", cities, limit=10, scorer=fuzzywuzzy.fuzz.token_sort_ratio)
