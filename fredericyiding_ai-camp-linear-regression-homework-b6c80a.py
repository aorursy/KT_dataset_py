...
import math #数学库
data_dir = '../input/'
data = pd.read_csv(data_dir + 'kc_house_data.csv')
data.dtypes
...
mse1 = ((predtrain - ytrain) ** 2).sum() / len(ytrain)
mse2 = ((predtrain - ytrain) * (predtrain - ytrain)).sum() / len(ytrain) # L2 loss
print(mse1, mse2)
mre = (abs(predtrain - ytrain)/ytrain).sum() / len(ytrain)
print(mre)
mae = (abs(predtrain - ytrain)).sum() / len(ytrain)  #L1 loss
print(mae)
