...
total_val = np.product(sf_permits.shape)
total_missing_val = missing_val.sum()

(total_missing_val/total_val)*100 
# look at the # of missing points in the first ten columns
missing_values_count[0:10]
...
print("Rows with na\'s dropped : %d" %rows_dropped.shape[0])
# Now try removing all the columns with empty values. Now how much of your data is left?
col_dropped = sf_permits.dropna(axis=1)
print('Columns in original dataset : ', sf_permits.shape[1])
print('Columns with na\'s dropped : ', col_dropped.shape[1])
