...
with open('../input/nlp5740/training.json') as f:
...
for i in data:
	...
	reviews_split.append(i['text'])

	labels_split.append(i['stars'])
print(reviews_split[:2000])
...
reviews_split = [l.lower() for l in reviews_split]



reviews_split = [''.join([c for c in review if c not in punctuation]) for review in reviews_split]
...
encoded_labels = [ int(i)-1 for i in labels]
...
#         fin_out = torch.argmax(sig_out)

        # return last sigmoid output and hidden state

#         return fin_out, hidden
