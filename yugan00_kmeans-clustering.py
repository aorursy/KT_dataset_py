...
def findclose(vector1,vector2): #实例分配给最近质心相关联的簇

    distance=list()

    for i in range(len(vector2)):

        distance.append(euclDistance(vector1, vector2[i]))
...
def my_k_means(dataSet, k):
    ...
    for i in range(k):
        ...
        centroids.append([dataSet[i,0],dataSet[i,1]])
    ...
    while(j<10):
        ...
        for i in range(len(dataSet)):
            ...
            closest.append(findclose(dataSet[i], centroids))

            clustertemp[closest[i]].append(dataSet[i])

        for i in range(k):

            centroids[i]=np.mean(clustertemp[i],axis=0)

            centroids[i]=centroids[i].tolist()[0]

        if(j<6): showCluster(dataSet, 4, np.mat(centroids), np.mat(clusterAssment))

        clusterAssment.clear()

        for i in range(len(dataSet)):

            clusterAssment.append([closest[i]])
