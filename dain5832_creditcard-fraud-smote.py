...
from sklearn.metrics import classification_report, roc_curve, roc_auc_score, precision_recall_curve, precision_score, recall_score, make_scorer
...
# oversamping with SMOTE
sm = SMOTE(random_state=42)
X_train_res, y_train_res = sm.fit_sample(X_train, y_train.ravel())

# check
print('# label 1', sum(y_train_res == 1))
print('# label 0', sum(y_train_res == 0))
...
axes1.set_title('PR Curve')

axes2 = fig.add_subplot(1, 2, 2)
axes2.set_xlabel('False Positive Rate')
axes2.set_ylabel('Ture Positive Rate')
axes2.set_title('ROC Curve')
...
rf_random = RandomizedSearchCV(estimator = rf, param_distributions = random_grid, scoring="recall", n_iter = 15, cv = 5, verbose=3, random_state=42, n_jobs = -1)
