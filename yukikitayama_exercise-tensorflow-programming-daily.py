...
def calc_accuracy(model, paths_to_hotdog_images, paths_to_other_images):
    ...
    num_hd = len(paths_to_hotdog_images)

    num_ot = len(paths_to_other_images)

    

    hd_data = read_and_prep_images(paths_to_hotdog_images)

    hd_preds = model.predict(hd_data)

    num_correct_hd = sum(is_hot_dog(hd_preds))

    

    ot_data = read_and_prep_images(paths_to_other_images)

    ot_preds = model.predict(ot_data)

    num_correct_ot = num_ot - sum(is_hot_dog(ot_preds))

    

    total_correct = num_correct_hd + num_correct_ot

    total_num = num_hd + num_ot

    

    out = total_correct / total_num
