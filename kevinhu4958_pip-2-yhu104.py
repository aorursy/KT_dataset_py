...
plt.show()
def customAnalyticalResult(N_loops = 2, steps_rev=1000, v=10):
    ...
    return position_x,position_y,velocity_x,velocity_y, time
# part a:

# 8 loops, default steps/rev

position_x8,position_y8,velocity_x8,velocity_y8,time = customAnalyticalResult(N_loops = 8)



plt.figure(5)

plt.plot(exact_position_x,exact_position_y,linewidth=1,label='exact position')

plt.plot(position_x8,position_y8,linewidth=0.2,label='numerical position')

plt.axis('equal')

plt.legend()

plt.title(f"Comparison for analytical (8 loops) and numerical solution")
...
# 8 loops, doubled steps/rev

position_x8new,position_y8new,velocity_x8new,velocity_y8new,time = customAnalyticalResult(N_loops = 8, steps_rev=steps_rev*2)



plt.figure(6)

plt.plot(position_x8new,position_y8new,linewidth=1,label='doubled steps/rev')

plt.plot(position_x8,position_y8,linewidth=0.2,label='default steps/rev')

plt.axis('equal')

plt.legend()

plt.title(f"Comparison for default and doubled steps/rev in analytical solution")
...
# velocity: original, halved, doubled

result_half = customAnalyticalResult(v=v/2)

result_double = customAnalyticalResult(v=v*2)



plt.figure(7)

plt.plot(position_x,position_y,linewidth=1,label='default velocity')

plt.plot(result_half[0],result_half[1],linewidth=0.2,label='half velocity')

plt.plot(result_double[0],result_double[1],linewidth=0.2,label='double velocity')

plt.axis('equal')

plt.legend()

plt.title(f"Comparison for different velocity in analytical solution")
...
plt.show()
#  (exact_position_x[i] - position_x[i]) ** 2 + (exact_position_y[i] - position_y[i]) ** 2)
def calc_error(exact_position_x, exact_position_y, position_x, position_y):

    '''

    quatify the error by calculate the square deviation of corresponding points

    all matrixs need to have the same length

    '''

    return [(exact_position_x[i] - position_x[i]) ** 2 + (exact_position_y[i] - position_y[i]) ** 2 for i in range(len(exact_position_x))]
plt.plot(calc_error(exact_position_x,exact_position_y,position_x,position_y))
# part 1

# the global error is the average of error (N=2i)

plt.figure(8)

for i in [200,400,800,1600]:

    result = customAnalyticalResult(steps_rev=i)

    exact_position_x = v/omega*np.sin(omega*result[-1] - phi) + rxc   # multiplication with numpy array time

    exact_position_y = v/omega*np.cos(omega*result[-1] - phi) + ryc   # yields a numpy array

    plt.plot(calc_error(exact_position_x,exact_position_y,result[0],result[1]),linewidth=0.2,label=str(i)+ " time steps")



plt.legend()

plt.title(f"Comparison error for different time step")
