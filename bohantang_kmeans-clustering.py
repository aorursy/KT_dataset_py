...
def my_k_means(dataSet, k):
    ...
    while(True):
        ...
        # Update the position of the cluster center is constant.
        ...
        for i in range(k):
            ...
            for j in range(len(dataSet)):
                ...
                if (clusterAssment[j][0] == i):
                    ...
                    x_sum = x_sum + dataSet.item(j,0)

                    y_sum = y_sum + dataSet.item(j,1)

                    num = num + 1

            if (centroids[i][0] == x_sum/num) & (centroids[i][1] == y_sum/num):

                flag += 1

            else:

                centroids[i][0] = x_sum/num

                centroids[i][1] = y_sum/num

                

        # Quit, when the position of the cluster center is constant

        if ((flag == k) and (iteration > 5)):
