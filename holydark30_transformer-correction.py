...
out = tokenizer.encode_batch(['ผมหิว','ร้อนมาก','ชิลหายแล้ววววว'])

print(f'idx 0: {out[0].ids}')

print(f'idx 1: {out[1].ids}')

print(f'idx 2: {out[2].ids}')
...
src_padded = torch.tensor([txt.ids for txt in tokenizer.encode_batch(src_txt)]).to(device)

trg_padded = torch.tensor([txt.ids for txt in tokenizer.encode_batch(trg_txt)]).to(device)

val_src_padded = torch.tensor([txt.ids for txt in tokenizer.encode_batch(val_src_txt)]).to(device)

val_trg_padded = torch.tensor([txt.ids for txt in tokenizer.encode_batch(val_trg_txt)]).to(device)
...
model = Seq2Seq(enc, dec, pad_tok, pad_tok, device).to(device)
import numpy as np
...
def train(model, optimizer, criterion, clip):
    ...
    for i in range(n_batch):
        ...
        src = src_padded[i*batch_size:(i+1)*batch_size]

        trg = trg_padded[i*batch_size:(i+1)*batch_size]
...
def evaluate(model, criterion):
    ...
    with torch.no_grad():
        ...
        for i in range(val_batch):
            ...
            src = val_src_padded[i*batch_size:(i+1)*batch_size]

            trg = val_trg_padded[i*batch_size:(i+1)*batch_size]
...
for epoch in range(N_EPOCHS):
    ...
    writer.add_scalar('Accuracy/train', math.exp(train_loss), epoch)

    writer.add_scalar('Accuracy/test', math.exp(valid_loss), epoch)    
...
#         src_mask = train_src == pad_tok

#         trg_mask = train_trg == pad_tok

        

#         train_src = train_src.transpose(0,1).unsqueeze(-1)

#         train_trg = train_trg.transpose(0,1).unsqueeze(-1)
...
#         preds = model(train_src, train_trg, src_mask, trg_mask)
