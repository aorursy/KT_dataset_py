...
#Возраст больше 50
df_train_all['age_50'] = (df_train_all.age > 50).astype('int')
df_test_all['age_50'] = (df_test_all.age > 50).astype('int')
express_mapper_train = df_pursh.groupby('client_id')['express_points_received'].sum()
express_mapper_test = df_pursh_test.groupby('client_id')['express_points_received'].sum()
df_train_all['is_express_points'] = (express_mapper_train>0).astype('int')
df_test_all['is_express_points'] = (express_mapper_test>0).astype('int')
df_train_all
pursh_ind = df_pursh.set_index('client_id')
pursh_ind_test = df_pursh_test.set_index('client_id')
regular_points_all_median = pursh_ind.regular_points_received.median()
regular_points_test_median = pursh_ind_test.regular_points_received.median()
df_train_all['regular_points_median'] = (pursh_ind.regular_points_received.groupby(['client_id']).sum() > regular_points_all_median).astype('int')
df_test_all['regular_points_median'] = (pursh_ind_test.regular_points_received.groupby(['client_id']).sum() > regular_points_test_median).astype('int')
pursh_for_datetime = pursh_ind.drop_duplicates('transaction_id').transaction_datetime.map(lambda ts: pd.to_datetime(ts))
hour = pursh_for_datetime.map(lambda ts: ts.hour)
pursh_for_datetime_test = pursh_ind_test.drop_duplicates('transaction_id').transaction_datetime.map(lambda ts: pd.to_datetime(ts))
hour_test = pursh_for_datetime_test.map(lambda ts: ts.hour)
df_train_all['hour_median'] = (hour.groupby(['client_id']).median() > hour.median()).astype('int')
df_test_all['hour_median'] = (hour_test.groupby(['client_id']).median() > hour_test.median()).astype('int')
pursh_for_datetime
df_pursh_id = df_pursh.reset_index()
df_pursh_id_test = df_pursh_test.reset_index()
purchases_by_id_all = df_pursh_id.drop_duplicates('transaction_id').client_id.value_counts()
purchases_by_id_test = df_pursh_id_test.drop_duplicates('transaction_id').client_id.value_counts()
df_train_all['transactions_mean'] = (purchases_by_id_all > purchases_by_id_all.median()).astype('int')
df_test_all['transactions_mean'] = (purchases_by_id_test > purchases_by_id_test.median()).astype('int')
df_new_features = df_train_all[['age_50', 'is_express_points', 'regular_points_median', 'hour_median', 'transactions_mean']]
df_new_features_test = df_test_all[['age_50', 'is_express_points', 'regular_points_median', 'hour_median', 'transactions_mean']]
