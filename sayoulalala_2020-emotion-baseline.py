...
# train = shuffle(train_100k, random_state=1234)

# split_index = int(len(train) * (1-val_percent))
...
# train['label']=train['label'].apply(lambda x:noise_label(x))
train=train_100k
import torch
...
# ds_test = EmotionDataSet(val, train_mode=False, labeled=True)

# test_loader = torch.utils.data.DataLoader(ds_test, batch_size=16, shuffle=False, num_workers=0, collate_fn=ds_test.collate_fn, drop_last=False)

# test_loader.num = len(val)





# logss=train_model(netG,train_loader,val_loader,test_loader,epochs=5)
...
for i,(train_index , test_index) in enumerate(kf.split(train,train)):
    ...
    val_loader=get_loader(val,BATXH_SIZE,True)

    

    model = EmotionModel().cuda(DEVICE)

    model_save_path=model_save_path='model_all_QA_{}.pkl'.format(i+1)

    logss=train_model(model,train_loader,val_loader,epochs=EPOCH,model_save_path=model_save_path)
...
for i in range(FOLD):
    ...
    sub_i.to_csv('sub{}'.format(i+1),index=False)

test_preds['y']=np.argmax(sum_preds,1)-1

test_preds[['id','y']].to_csv('sub_mean',index=False)
test_preds