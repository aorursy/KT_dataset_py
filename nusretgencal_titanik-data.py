...
test_df.columns #sütünlar
train_df.head() #ilk 5 hane
train_df.describe() # sayısal bilgiler
train_df.info()
...
train_df[["Pclass","Survived"]].groupby(["Pclass"], as_index = False).mean() # groupla ve ortalamasını göster
#Pclass - Survived
...
train_df = train_df.drop(detect_outliers(train_df, ["Age","SibSp","Parch","Fare"]), axis = 0).reset_index(drop = True)#satırları sil axis ile
train_df.loc[detect_outliers(train_df, ["Age","SibSp","Parch","Fare"])]
train_df_len = len(train_df)

train_df = pd.concat([train_df, test_df], axis = 0).reset_index(drop = True)
train_df.head()
train_df.columns[train_df.isnull().any()] # missing value var mı diye bakıyoruz null?
train_df.isnull().sum()
train_df[train_df["Embarked"].isnull()]
train_df.boxplot(column = "Fare",by = "Embarked")

plt.show()
train_df["Embarked"] = train_df["Embarked"].fillna("C") # NAN olanları C ile doldurduk çünkü 80 fare olan Cden binmiş
...
#virgüle göre noktaya göre ayıralım
...
Cross_Validation = pd.DataFrame({"Cross Validation Means" : cv_results,
...
sonuclar.to_csv("titanic.csv", index = False)
test_survived