...
train_new['Rating']=df
def convert_data(df,column):

    Max=df[column].max()

    Min=df[column].min()

    x=df[column].apply(lambda x:(x-Min)/(Max-Min))

    return x
a=train_new.columns.values
print(a)
b=a[11:]
...
for i in b:
    ...
    print(i,train_new[train_new['Rating']==0][i].mean()*103,train_new[train_new['Rating']==1][i].mean()*238)
c=['os_name_Blackberry','os_name_KAI','os_name_Nokia',

 'os_name_Tizen','SIM Slot(s)_Dual SIM, GSM+CDMA',

 'SIM Slot(s)_Dual SIM, GSM+GSM','Num_cores_312','Num_cores_Deca','Num_cores_Tru-Octa',

 'Brand_10.or','Brand_Billion','Brand_Comio','Brand_Coolpad','Brand_Do','Brand_Jivi',

 'Brand_Karbonn','Brand_LeEco','Brand_Lephone','Brand_Lyf',

 'Brand_Nubia','Brand_Razer','Brand_Reliance','Brand_Spice','Brand_Ulefone','Brand_VOTO','Brand_iVooMi']

f=['PhoneId','Height', 'Resolution', 'Screen Size' ,'Processor_frequency', 'RAM',
...
for j in g:
    ...
    e[j]=convert_data(e,j)

    X_test[j]=convert_data(X_test,j)
...
wt_matrix= perceptron.fit(X_train, Y_train, 1000, 2.2600000000000002)
