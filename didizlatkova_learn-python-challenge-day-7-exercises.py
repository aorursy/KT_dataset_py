...
q2.hint()
q2.solution()
def card_score(card, balance, cards_left):
    if card in ['J', 'Q', 'K']:
        return 10
    if card is 'A':
        if balance + 11 + cards_left > 21:
            return 1
        else:
            return 11
    return int(card)
...
def total_score(hand):
    hand = sorted(hand, key=lambda x: x is 'A')
    balance = 0
    for i in range(len(hand)):
        cards_left = len(hand) - i - 1
        balance += card_score(hand[i], balance, cards_left)
...
def blackjack_hand_greater_than(hand_1, hand_2):
    ...
    False
    """
    hand_1_score = total_score(hand_1)
    hand_2_score = total_score(hand_2)
...
def conditional_roulette_probs(history):
    ...
    for i in range(1, len(history)):
        ...
        if n1 not in counts:
            counts[n1] = {}
        n2 = history[i]
        if n2 not in counts[n1]:
            counts[n1][n2] = 0
        counts[n1][n2]+=1
        
    for key, value in counts.items():
        prob_sum = sum([v for v in value.values()])
        for n in value.keys():
            value[n] /= prob_sum
    return counts
def find_best_bet(stats):
    probs = {}
    for prev, nexts in stats.items():
        for next, count in nexts.items():
            probs[(prev, next)] = count
...
def my_agent(wheel):
    ...
    while wheel.num_remaining_spins() > 30:
        history.append(wheel.spin())
    
    prev = history[-1]
    best_prev, best_next = find_best_bet(conditional_roulette_probs(history))
    
    #print('Best', best_prev, best_next)
    
    while wheel.num_remaining_spins() > 0:
        guess = None
        if prev == best_prev:
            guess = best_next
        prev = wheel.spin(number_to_bet_on=guess)
