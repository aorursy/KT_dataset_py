...
def euler(yin,omega,step):
    ...
    yout = yin + step*(np.array([yin[2],yin[3],omega*yin[3],-omega*yin[2]]))
    return yout 
...
plt.figure('Comparison - Cyclotron Motion')
plt.plot(time,position_y,'x',label='numerical position')
plt.plot(time,exact_position_y,linewidth=1,label='Analytical position')
plt.axis([0,N_loops*2*np.pi/omega,-1*(position_y[0]+0.1),position_y[0]+0.1])
plt.title(f"Comparison analytical vs numerical result of y(t)")
...
plt.figure('Error')
plt.plot(time, error_distance,linewidth=3,label='error')
plt.legend()
plt.title(f"Error propogation between analytical and numerical solution")
plt.xlabel(f"time [s]") 
