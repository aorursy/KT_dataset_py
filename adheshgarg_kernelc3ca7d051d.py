...
def conv_net(x_dict, n_classes, dropout, reuse, is_training):
    ...
    with tf.variable_scope('ConvNet', reuse=reuse):
        ...
        x = tf.reshape(x_dict, shape=[-1, 256, 256, 1])
        ...
        conv4 = tf.layers.conv2d_transpose(conv3, 32, 5, padding='same', activation=tf.nn.relu)

        conv4 = tf.concat([conv4, conv3], axis=-1)

        conv5 = tf.layers.conv2d(conv4, 32, 5, padding='same', activation=tf.nn.relu)

        conv6 = tf.layers.conv2d_transpose(conv5, 32, 5, padding='same', activation=tf.nn.relu)

        conv6 = tf.concat([conv6, conv2], axis=-1)

        conv7 = tf.layers.conv2d(conv6, 32, 5, padding='same', activation=tf.nn.relu)

        conv8 = tf.layers.conv2d_transpose(conv7, 32, 5, padding='same', activation=tf.nn.relu)

        conv8 = tf.concat([conv8, conv1], axis=-1)

        conv9 = tf.layers.conv2d(conv8, 32, 5, padding='same', activation=tf.nn.relu)

        # Flatten the data to a 1-D vector for the fully connected layer

        fc1 = tf.contrib.layers.flatten(conv9)
