...
def map_smoking(column):
    ...
    return mapped
insurance["sex_norm"]=map_smoking(insurance["sex"])
...
cols_not_reg3=['age', 'obese', 'smoker_norm', 'sex_norm']
seed=2
results = []
names = []
...
for vals in coefs:
    ...
    sex_norm_coefs.append(vals[3])
age_coef=np.mean(age_coefs)
obesity_coef=np.mean(obesity_coefs)
smoking_coef=np.mean(smoking_coefs)
sex_norm_coef=np.mean(sex_norm_coefs)
print("a: ",age_coef," b: ",obesity_coef," c: ",smoking_coef,"d:",sex_norm_coef," intercept: ",avg_intercept)
def calculate_insurance(age,obesity,smoking,sex):
    y=(age_coef*age)+(obesity_coef*obesity)+(smoking_coef*smoking)+avg_intercept
    return y
print(calculate_insurance(34,1,1,1))
