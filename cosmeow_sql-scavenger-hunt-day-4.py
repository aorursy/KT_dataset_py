...
WITH time AS
(
SELECT TIMESTAMP_MILLIS(timestamp) AS ttime, transaction_id
FROM `bigquery-public-data.bitcoin_blockchain.transactions`
)
SELECT COUNT(transaction_id) AS transactions, EXTRACT(DAY from ttime) AS day, 
EXTRACT(MONTH from ttime) as month
FROM time
WHERE EXTRACT(YEAR from ttime) = 2017
GROUP BY month,day
ORDER BY month,day
"""
...
#plot transactions per merkler_root
plt.plot(transactions_per_merkle_root.transactions)
plt.title("Per Merkle Root Bitcoin Transactions")