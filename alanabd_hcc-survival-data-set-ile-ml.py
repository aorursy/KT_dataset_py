...
df=pd.read_csv("../input/hcc-survival-data-set/hcc.csv")
df.head()
df.info()
df1 = df.apply(lambda x: x.replace('?',np.nan)) # replacing ? for Nan.
...
print(df1['Class'].value_counts()) #instances per class

print('NaN per attribute')

print(df1.isnull().sum())

df1.drop_duplicates() #removing duplicate instances

df2=df1.dropna(thresh=40) #maintaining only the instances with at least 80 non-NA values, that is, 80%.  

df3=df2.dropna(axis=1, thresh=125) # maintaining only attributes with at least 125 non-NA values, ie 80%.
...
df3.isnull().sum() 
df3.info()
df4= df3.apply(pd.to_numeric, errors="ignore") # converting the data type  b = a.apply(pd.to_numeric, errors="ignore")

df4.describe(include='all')
df4.head()
df4.info()
data1=df4.iloc[:,0:19]  #categorical data 1/0

cat=df4.iloc[:,39]

data1['Class'] = cat

data2=df4.iloc[:,19:40]   #non categorical data
df4.iloc[:,0:19].mode()
x = range(19)

for n in x:

    df4=df4.replace({df4.columns[n]: np.nan}, df4.loc[:,df4.columns[n]].mode().loc[0]) 
df4.iloc[:,19:39].mean()
...
for n in x:
    ...
    df4=df4.replace({df4.columns[n]: np.nan}, df4.loc[:,df4.columns[n]].mean()) 
df4.isnull().sum() 
...
print(df4['Class'].value_counts()) #verificando quantidade de instância em cada classe

df4.head()
df3.head()
y=df4["Class"].values

x_data=df4.iloc[:,0:40]
...
for each in range(50,250,10):

    rfc2 = RandomForestClassifier(n_estimators=each,random_state=1)

    rfc2.fit(x_train,y_train)

    score_list2.append(rfc2.score(x_test,y_test))

    

plt.plot(range(50,250,10),score_list2)
