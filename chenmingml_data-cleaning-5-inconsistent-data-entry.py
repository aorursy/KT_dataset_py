...
suicide_attacks['Province'].head(10)
# get all the unique values in the 'City' column
cities = suicide_attacks['City'].unique()
...
#test with Ver 6
# look at the first ten thousand bytes to guess the character encoding
with open('../input/PakistanSuicideAttacks Ver 6 (10-October-2017).csv','rb') as rawdate2:
    result2=chardet.detect(rawdate2.read(100000))
print(result2)

suicide_attacks_v6=pd.read_csv("../input/PakistanSuicideAttacks Ver 6 (10-October-2017).csv", encoding="Windows-1252")
suicide_attacks_v6.head()
...
Cities_v6.sort()
len(Cities_v6)
df_match_rows = suicide_attacks_v6['City'].isin(['karachi'])
suicide_attacks_v6.loc[df_match_rows]
def replace_with_fuzzywuzzy(df, column, string_to_match, min_ratio = 90):
    strings=df[column].unique()
    matches=fuzzywuzzy.process.extract(string_to_match, strings, limit=10, scorer=fuzzywuzzy.fuzz.token_sort_ratio)
    close_matches= [match[0] for match in matches if match[1]>=min_ratio]
    df_match_rows = df[column].isin(close_matches)
    df.loc[df_match_rows, column]=string_to_match
    print ('done')
Cities_v6=suicide_attacks_v6['City'].unique()
Cities_v6.sort()
Cities_v6
replace_with_fuzzywuzzy(suicide_attacks_v6, 'City', 'd.i khan')
Cities_v6=suicide_attacks_v6['City'].unique()
Cities_v6.sort()
len (Cities_v6)