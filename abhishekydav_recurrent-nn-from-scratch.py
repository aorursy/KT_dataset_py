...
!mkdir /kaggle/tmp/data/nietzsche/
!cp -r ../input/neitzshe/* /kaggle/tmp/data/nietzsche/
!ls /kaggle/tmp/data/nietzsche/
...
PATH='/kaggle/tmp/data/nietzsche/'
...
xs[:cs,:cs]
y[:cs]
val_idx = get_cv_idxs(len(idx)-cs-1); val_idx.shape
...
it = iter(md.trn_dl)
*xst,yt = next(it)
torch.stack(xst).size(), yt.size()
torch.stack(xst).view(-1,n_hidden).size()
yt.transpose(0,1).contiguous().view(-1)
def nll_loss_seq(inp, targ):
    sl,bs,nh = inp.size()
    targ = targ.transpose(0,1).contiguous().view(-1)
    return F.nll_loss(inp.view(-1,nh), targ)
fit(m, md, 20, opt, nll_loss_seq)
set_lrs(opt, 1e-5)
