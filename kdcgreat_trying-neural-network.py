...
full_X = pd.concat( [ imputed , pclass , sex , family ] , axis=1 )
...
for a_seed in seeds:
    ...
    model, hist = train_model(model, 3e-4, 200)

    # faster convergence

    model, hist = train_model(model, 9e-4, 500)
...
for a_seed in promising_seeds:
    ...
    #models[a_seed], hists[a_seed] = train_model(models[a_seed], 9e-4, 3000)

    acc = np.mean(hist.history['val_acc'][-100:-1])
...
# try to predict by mean of all models

# predictions = []

# for a_seed in good_seeds:

#    predictions.append(models[a_seed].predict(test_X)[:,0])

# mean_prediction = np.zeros(len(test_X))

# for prediction in predictions:

#    mean_prediction += prediction/len(good_seeds)



prediction_model = models[best_seed]
test_y = (prediction_model + 0.5).astype("int")
