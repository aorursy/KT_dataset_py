...
base_model =keras.applications.ResNet50(include_top=False, weights='imagenet',input_shape=(64,64,3), classes=200)
from keras import regularizers

x = base_model.output
x = GlobalAveragePooling2D()(x)
x = Dropout(0.8)(x)
predictions = Dense(num_classes, activation= 'softmax', kernel_regularizer=regularizers.l2(0.0001))(x)
model = Model(inputs = base_model.input, outputs = predictions)
model.summary()
opt =keras.optimizers.Adam(lr=0.001)
model.compile(optimizer=opt,loss='categorical_crossentropy',metrics=['accuracy'])
model.save_weights('resnet_best.h5')
checkpoint = ModelCheckpoint('resnet_best.h5',monitor='val_accuracy',verbose=1,
                              save_best_only=True,mode='max')
#earlstop=EarlyStopping(monitor='val_loss', patience=10, verbose=1, mode='auto')
redlr=ReduceLROnPlateau( monitor="val_loss", factor=0.1, patience=10,verbose=1, mode="auto", min_delta=0.0001)

callback=[checkpoint,redlr]
