...
dataset_ex = pd.read_csv('../input/dataskripsi/data indonesia.csv')

dataset_in = pd.read_csv('../input/dataskripsi/data internasional.csv')
...
df_ex['pandas_SMA_3'] = df.iloc[:,1].rolling(window=3).mean()
print(df_in.head())

df_in['pandas_SMA_3'] = df.iloc[:,1].rolling(window=3).mean()