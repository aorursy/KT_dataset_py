...
#разделяем ярлыки на train и validation, форматируем
...
#берем готовую модель из библиотеки torchvision
...
def create_resnet9_model(output_dim: int = 1) -> nn.Module:
    ...
    return model
#я взяла улучшенную версию этой модели, уже натренированную на данных ImageNet

def create_resnet18_model(output_dim: int = 1) -> nn.Module:
...
#создаю оптимайзер для обучения моей модели

optimizer50 = Adam(resnet18.parameters(), lr)
#binary cross-enthropy

loss = nn.BCEWithLogitsLoss()
#функция, которая записывает в логгер roc_auc_score
...
def predict(model, dataloader):
    ...
    return y_true, y_hat
#технические функции для работы с тензорами pytorch
...
#триггеры для удобного запуска записи в логгер
...
do_step_trigger, train_loss_trigger, valid_loss_trigger = init_triggers(1, 10, 10)
...
def train(model, 
                    ...
                    valid_loss_trigger, epochs):

    

    #добавляем one-cycle learning rate
...
#                     valid_loss_trigger)
#предобученная resnet18, one-cycle policy и несколько эпох существенно улучшают результат

resnet18 = train(resnet18, 
...
y_hat_test = predict_test(resnet18, test_dataloader)
