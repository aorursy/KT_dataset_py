...
np.isnan(df_tr['Age'][df_tr['Age'].isnull()==True])
#checking for NaN in Embarked

df_tr['Embarked'][df_tr['Embarked'].isnull()==True]='C'

df_tr['Embarked'].count()
...
df_tr['Embarked'][df_tr['Embarked']=='S'] = 0

df_tr['Embarked'][df_tr['Embarked']=='C'] = 1

df_tr['Embarked'][df_tr['Embarked']=='Q'] =2

df_tr['Embarked'].astype(int)

df_tr.tail()
#checking for NaN in sex

np.isnan(df_tr['Age'][df_tr['Age'].isnull()==True])
features = df_tr.drop(df_tr[['Survived','Name','Ticket','Cabin','SibSp','Parch']], axis=1)
...
df_te['Embarked'][df_te['Embarked'].isnull()==True]='C'

df_te['Embarked'].count()
...
df_te['Embarked'][df_te['Embarked']=='S'] = 0

df_te['Embarked'][df_te['Embarked']=='C'] = 1

df_te['Embarked'][df_te['Embarked']=='Q'] =2

df_te['Embarked'].astype(int)

df_te.head()
te_features = df_te.drop(df_te[['Cabin','Name','Ticket','SibSp','Parch']],axis=1)
...
df_chk = pd.merge(op_df,te_re,how='inner',left_index=True,right_index=True,suffixes=('_Predicted','_Actual'))

df_chk.head()
df_wrong=DataFrame(df_chk.apply(np.sum,axis=1),index=df_chk.index.values)

df_wrong.columns=['mistakes']

accuracy = ( 1 - (np.sum(df_wrong['mistakes'][df_wrong['mistakes']==1])/418)) * 100

# df_wrong.count() 418 entries

# te_features.count() 418 entries

print('This code gives results accurate upto %f percent' % accuracy)