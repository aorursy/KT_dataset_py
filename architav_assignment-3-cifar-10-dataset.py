...
project_name = '03-cifar10-feedforward-archita'
...
for _, l in dataset:
    if classes[l] in class_count:
        class_count[classes[l]] += 1
    else: 
        class_count[classes[l]] = 1
...
class CIFAR10Model(ImageClassificationBase):
    def __init__(self, in_size, hidden_size, out_size):
        super().__init__()
        # hidden layer 1
        self.linear1 = nn.Linear(in_size, hidden_size[0])
        # hidden layer 2
        ...
        # output layer
        self.linear5 = nn.Linear(hidden_size[3], out_size)
    ...
    def forward(self, xb):
        ...
        return out        
hidden_size = [128,512,256,16]
model = to_device(CIFAR10Model(input_size, hidden_size, output_size), device)
...
test_set_loss = evaluate(model, test_loader)
test_set_loss
arch = " 4 layers (128,512,256,16)"
lrs = [0.1, 0.05, 0.1]
epochs = [25,15,15]
test_acc = test_set_loss['val_acc']
test_loss = test_set_loss['val_loss']
