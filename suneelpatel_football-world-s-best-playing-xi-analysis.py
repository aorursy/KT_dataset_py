...
#%matplotlib inline
df = pd.read_csv("../input/F_Player_dataset.csv")
...
sns.countplot(y=df.Nationality, palette = 'Set2') #Plot all the nations as Y Axis
...
df['gk_Skeeper'] = (b*df.Reactions + b*df.Composure + b*df.Speed + a*df.Short_Pass + a*df.Long_Pass + b*df.Jumping + b*df.GK_Positioning + b*df.GK_Diving + d*df.GK_Reflexes + b*df.GK_Handling + d*df.GK_Kicking + c*df.GK_Kicking + c*df.Vision)/(2*a + 4*b + 2*c + 1*d)
plt.figure(figsize = (15,6))
...
plt.ylabel("Shot Stopping Score")
#Choosing Defenders(4 best defenders) : 2 Centre backs and 2 wing backs
...
# Next, let us plot the right wing attacker
