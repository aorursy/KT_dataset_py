...
microsoft2 = microsoft.copy()
microsoft2.drop(columns=['Name'],inplace=True)
microsoft2['Volume'] = ss.fit_transform(microsoft2[['Volume']])
model = ARMA(microsoft2["Volume"].diff().iloc[1:], order=(3,3))
result = model.fit()
print(result.summary())
print("μ={}, ϕ={}, θ={}".format(result.params[0],result.params[1],result.params[2]))
result.plot_predict(start=1000, end=1100)
plt.show()
rmse = math.sqrt(mean_squared_error(microsoft2["Volume"].diff().iloc[1000:1101].values, result.predict(start=1000,end=1100)))
print("The root mean squared error is {}.".format(rmse))
