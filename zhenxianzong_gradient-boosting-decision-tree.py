...
class my_gbdt(object):
    ...
    def __init__(self, max_depth, min_samples_split, lr, itera):
    ...
    def fit(self, X, y):
        ...
        for k in range(1,self.itera):

            self.trees[k].fit(X, (y-y_pred)*self.lr)

            y_pred += self.trees[k].predict(X)
    ...
    def predict(self, X):
        ...
        for k in range(0, self.itera):

            pred_proba += self.lr * self.trees[k].predict(X)

        return np.argmax(pred_proba, axis=1)
my_model = my_gbdt(max_depth=6, min_samples_split=10, lr=0.5, itera=20)
