...
sns.heatmap(corr, mask=np.zeros_like(corr, dtype=np.bool), square=True, ax=ax, annot = False)
df_drop_corr = df_onehot.drop(columns = ['Number of Insignificant Quantities',"Number of Quantities", 'Total Number of Characters', 'Number of Sentences', 'ID'])
...
history = model.fit(x_train, y_train,validation_data=(x_test,y_test), epochs=2000,callbacks=[ckpt]);
model.load_weights(ckpt.filepath)
...
tf_drop_corr = tf_onehot.drop(columns = ['Number of Insignificant Quantities',"Number of Quantities", 'Total Number of Characters', 'Number of Sentences', 'ID'])
