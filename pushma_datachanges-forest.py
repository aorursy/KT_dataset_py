...
test = pd.read_csv('../input/norm-data/df_test.csv', sep=',')

xx = pd.read_csv('../input/norm-data/df_train (1).csv', sep=',')

test = test.drop(['ProductCategory_ticket'], axis=1)

from sklearn.ensemble import RandomForestClassifier

forest = RandomForestClassifier(n_estimators=500, max_depth=3, random_state=19)

X = xx.drop(['IsDefaulted'], axis=1)
...
y_TESTING = best_forest.predict(X_TESTING)
test.info()
dfans2 = dftest[['TransactionId', 'TransactionStatus']]

#dfans2['IsDefaulted'] = 0
...
for i in range(dfans2.shape[0]):
    ...
    if dfans2.at[i,'TransactionStatus'] == 0:
        ...
        y_TESTING = pd.Series(y_TESTING).drop([i])

dfans2['IsDefaulted'] = y_TESTING

#print(dfans2['IsDefaulted'].shape[0])

#print(y_TESTING.shape[0])

dfans2 = dfans2.drop(['TransactionStatus'], axis=1)

print(dfans2['IsDefaulted'])

dfans2.to_csv('Answer17.csv',index=False)
...
df_cv = pd.DataFrame(forest_grid.cv_results_)
