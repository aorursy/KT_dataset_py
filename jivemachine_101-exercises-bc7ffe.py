...
def has_vowels(x):
    ...
    word = [letter for letter in x if letter in vowels]
...
def only_odd_numbers(x):
    ...
    odds = [num for num in x if num % 2 == 1]
...
def only_positive_numbers(x):
    ...
    list = [num for num in x if num > 0]
...
def has_evens(x):
    ...
    list = [num for num in x if num % 2 == 0]
...
def has_odds(x):
    ...
    list = [num for num in x if num % 2 ==1]
...
def only_positive_evens(x):
    ...
    list = [num for num in x if num % 2 == 0 and num > 0]
...
def only_positive_odds(x):
    ...
    list = [num for num in x if num % 2 == 1 and num > 0]
...
def get_values_in_common(x,y):
    ...
    list = [var for var in x if var in y]
...
def get_values_not_in_common(x,y):
    ...
    list = [i for i in x + y if i not in x or i not in y]
...
def highest_price_book(x):
    ...
    maxPricedItem = max(x, key=lambda y:y['price'])
...
def lowest_price_book(x):
    ...
    lowpriceditem = min(x, key=lambda y:y["price"])

    return lowpriceditem
...
def get_average_spent_per_item(x):
    ...
    val1 = x["items"][0]["quantity"] * x["items"][0]["price"]

    val2 = x["items"][1]["quantity"] * x["items"][1]["price"]

    val3 = x["items"][2]["quantity"] * x["items"][2]["price"]

    val4 = x["items"][3]["quantity"] * x["items"][3]["price"]

    val5 = x["items"][4]["quantity"] * x["items"][4]["price"]

    quantity = x["items"][0]["quantity"]+x["items"][1]["quantity"]+x["items"][2]["quantity"]+x["items"][3]["quantity"]+x["items"][4]["quantity"]

    return (val1+val2+val3+val4+val5) / quantity
...
def most_spent_on_item(x):
    ...
    a = x["items"][0]["quantity"] * x["items"][0]["price"]

    b = x["items"][1]["quantity"] * x["items"][1]["price"]

    c = x["items"][2]["quantity"] * x["items"][2]["price"]

    d = x["items"][3]["quantity"] * x["items"][3]["price"]

    e = x["items"][4]["quantity"] * x["items"][4]["price"]
