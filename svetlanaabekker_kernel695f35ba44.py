...
test_purch = pd.read_csv('../../kaggle/input/x5-uplift-valid/test_purch/test_purch.csv', sep=',', encoding='cp1251')
...
test = pd.read_csv('../../kaggle/input/x5-uplift-valid/data/test.csv', sep=',', encoding='cp1251')
...
test_purch_merged = pd.merge(test_purch, products, on='product_id', how='inner')
#feature 1
...
test_purch_merged['day_of_week'] = test_purch_merged['transaction_datetime'].dt.dayofweek
tmp_df = test_purch_merged.groupby('client_id')['day_of_week'].agg(lambda x: x.value_counts().idxmax()).to_frame()
test_prep = pd.merge(test_prep, tmp_df, on='client_id', how='inner')
test_prep.head()
...
test_purch_merged['is_monday'] = test_purch_merged['day_of_week'].apply(lambda x: int(x == 0))
purchases_monday = test_purch_merged.drop_duplicates('transaction_id').groupby('client_id')['is_monday'].mean()
test_prep['monday_share'] = test_prep['client_id'].map(purchases_monday)
train_prep['monday_share_25'] = train_prep['monday_share'].apply(lambda x: int(x >= 0.25))

test_prep['monday_share_25'] = test_prep['monday_share'].apply(lambda x: int(x >= 0.25))
train_prep['monday_share_25'].value_counts()

test_prep['monday_share_25'].value_counts()
#feature 2
...
trn_sum = test_purch_merged.groupby('client_id')['trn_sum_from_iss'].sum()
test_prep['trn_sum'] = test_prep['client_id'].map(trn_sum)
median = test_prep['trn_sum'].median()
test_prep['trn_sum_median'] = test_prep['trn_sum'].apply(lambda x: int(x > median))

test_prep['trn_sum_median'].value_counts()
#feature 3
most_popular_level1 = test_purch_merged['level_1'].value_counts().idxmax()
test_purch_merged['most_pop_level1'] = test_purch_merged['level_1'].apply(lambda x: int(x == most_popular_level1))

most_pop_level1 = test_purch_merged.groupby('client_id')['most_pop_level1'].sum()
test_prep['level1_count'] = test_prep['client_id'].map(most_pop_level1)
median = test_prep['level1_count'].median()
test_prep['level1_count_m'] = test_prep['level1_count'].apply(lambda x: int(x > median))
...
test_prep['level1_count_m'].value_counts()
#feature 4
...
most_popular_level2 = test_purch_merged['level_2'].value_counts().idxmax()
test_purch_merged['most_pop_level2'] = test_purch_merged['level_2'].apply(lambda x: int(x == most_popular_level2))
most_popular_level2 = train_purch_merged['level_2'].value_counts().idxmax()
train_purch_merged['most_pop_level2'] = train_purch_merged['level_2'].apply(lambda x: int(x == most_popular_level2))
...
most_pop_level2 = test_purch_merged.groupby('client_id')['most_pop_level2'].sum()
test_prep['level2_count'] = test_prep['client_id'].map(most_pop_level2)
median = test_prep['level2_count'].mean()/2
test_prep['level2_count_m'] = test_prep['level2_count'].apply(lambda x: int(x > median))
train_prep['level2_count_m'].value_counts()

test_prep['level2_count_m'].value_counts()
# feature 5
...
exp_points_received = test_purch_merged.groupby('client_id')['regular_points_received'].sum()
test_prep['reg_points_received'] = test_prep['client_id'].map(exp_points_received)
train_prep['reg_points_received_50'] = train_prep['reg_points_received'].apply(lambda x: int(x > 50))

test_prep['reg_points_received_50'] = test_prep['reg_points_received'].apply(lambda x: int(x > 50))
train_prep['reg_points_received_50'].value_counts()

test_prep['reg_points_received_50'].value_counts()
df_new_features = train_prep[['monday_share_25','trn_sum_median','level1_count_m', 'level2_count_m', 'reg_points_received_50', 'client_id']]
df_new_features_test = test_prep[['monday_share_25','trn_sum_median','level1_count_m', 'level2_count_m', 'reg_points_received_50', 'client_id']]
X_learn_new = pd.merge(X_learn, df_new_features, on='client_id', how='inner')
X_valid_new = pd.merge(X_valid, df_new_features, on='client_id', how='inner')
X_test_new = pd.merge(X_test, df_new_features_test, on='client_id', how='inner')

X_learn_new = X_learn_new.drop(columns=['client_id'], axis=1)
X_valid_new = X_valid_new.drop(columns=['client_id'], axis=1)
X_test_new = X_test_new.drop(columns=['client_id'], axis=1)
clf_ = LGBMClassifier(
        boosting_type='rf',
        n_estimators=15000,
        num_leaves=40,
...
print('Build model for learn data set...')
clf = uplift_fit(clf_, X_learn_new, treatment_learn, target_learn)
print('Model is ready')
learn_pred = uplift_predict(clf, X_learn_new)
learn_scores = uplift_metrics(learn_pred, treatment_learn, target_learn)
print(f'Learn scores: {learn_scores}')
valid_pred = uplift_predict(clf, X_valid_new)
valid_scores = uplift_metrics(valid_pred, treatment_valid, target_valid)
print(f'Valid scores: {valid_scores}')
test_pred_new = uplift_predict(clf, X_test_new, z = True)
save_submission(indices_test,test_pred,'submission_svetlana_bekker.csv')
# 0.04391 score
