...
period_osc = (1/omega)*2*np.pi  # calculate the period of the oscillation (from the input variables)
...
def Leothelegend ( v, N_loops,steps_rev,Title1):
    ...
    plt.axis('equal')
    plt.legend()
    plt.title(f"Comparison for analytical and numerical solution" +Title1)
    plt.xlabel(f"x position [m]") 
    plt.ylabel(f"y position [m]") 
    plt.show()
    ...
    plt.plot(time,exact_position_y,linewidth=1,label='Analytical position')
    plt.axis([0,N_loops*2*np.pi/omega,-1*(position_y[0]+0.1),position_y[0]+0.1])
    plt.title(f"Comparison analytical vs numerical result of y(t) " +Title1)
    plt.xlabel(f"time [s]") 
    plt.ylabel(f"y position [m]") 
    plt.legend
    plt.show()
...
#Leothelegend (v, N_loops,steps_rev,title1, title2)
Leothelegend(10, 2,1000," Original")
Leothelegend(10, 8,1000," - 8 loops")
Leothelegend(10, 8,10000, " - 10000 steps")
Leothelegend(10*2, 8,1000," velocity of 20")
Leothelegend(10*0.5, 8,1000," velocity of 5")
...
def leo2(steps_rev):
    ...
    start_time = 0      # enter the start time of the propagation
    N_loops = 2          # enter number of loops the particle should complete 
        # enter the number of time steps per revolution 
    ...
    velocity_x = np.zeros(time_steps)
    velocity_y = np.zeros(time_steps)
    yep_itsjsutnumbers = np.zeros(time_steps)
    ...
    #calculate the 2 two radii at each point and take the difference of the exact and numerical values
    
    
    yep_itsjsutnumbers =((((position_y**2)+(position_x**2))**0.5)-(((exact_position_y**2)+(exact_position_x**2))**0.5)) 
    
    
    
    return yep_itsjsutnumbers
    

plt.figure()
plt.plot(leo2(2000),label='2000 Steps')
plt.plot(leo2(1000),label='1000 Steps')
plt.plot(leo2(500),label='500 Steps')

plt.legend()
plt.title(f"Comparison for analytical and numerical solution" )
plt.xlabel(f"Time steps") 
plt.ylabel(f"Deviation from actual postiton") 
plt.show()

print("the actual value being at x=0 ")
