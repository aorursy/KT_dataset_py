...
def get_validation_dataset(ordered=False):   ######9
...
def get_test_dataset(ordered=False):      ########10
...
with strategy.scope():
    ...
    pretrained_model = ResNet152V2(weights='imagenet', include_top=False ,input_shape=[*IMAGE_SIZE, 3])
...
if not SKIP_VALIDATION:
    ...
    cm_probabilities = (model_1.predict(images_ds) + model_2.predict(images_ds)+ model_3.predict(images_ds)) / 3
...
probabilities = (model_1.predict(test_images_ds) + model_2.predict(test_images_ds) + model_3.predict(test_images_ds)) / 3
...
m3 = model_3.predict(images)

probabilities = (model_1.predict(images) + model_2.predict(images) + model_3.predict(images)) / 3
