...
df["text_without_punctuation"] = df["text"].apply(lambda x: remove_punctuations(x))
...
df["text_without_stop"] = df["text_without_punctuation"].apply(lambda x: remove_stopwords(x))
...
for text in df["text_without_stop"]:
...
df["text_without_stopfreq"] = df["text_without_stop"].apply(lambda x: remove_freqwords(x))



df.head()
df.drop(["text_without_punctuation", "text_without_stop"], axis=1, inplace=True)
...
rare_words = set([word for (word, counter) in cnt.most_common()[:-n_rare_words-1:-1]])
...
df["text_wo_stopfreqrare"] = df["text_without_stopfreq"].apply(lambda x: remove_rarewords(x))
...
df.drop(["text_without_stopfreq", "text_wo_stopfreqrare"], axis=1, inplace=True) 
