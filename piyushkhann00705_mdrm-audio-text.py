...
traindf= pd.read_csv("../input/audiolstm/train_split_clf.csv")

testdf=pd.read_csv("../input/audiolstm/test_split_clf.csv")

valdf=pd.read_csv("../input/audiolstm/val_split_clf.csv")
...
def ModifyData(df,text_dict):
    ...
    for index,row in df.iterrows():
        ...
        y_3days.append(int(row['lab_3']))

        y_7days.append(int(row['lab_7']))

        y_15days.append(int(row['lab_15']))

        y_30days.append(int(row['lab_30']))  
...
#     print("MSE Train loss  : {train_loss}".format(train_loss = train_loss))

#     print("MSE Test loss : {test_loss}".format(test_loss = test_loss))
    ...
    print("Matthew Coeff="+str(matthews_corrcoef(y_test, test_pred)))
...
#     pred_save_path = os.path.join(plt_save_dir,'bi_lstm_pred_'+str(duration)+'.csv')

    pred_save_path="./MDRM_multimodal_pred"+str(duration)+'.csv'
...
#     spearman_list.append(Spearmancoef)
    ...
    save_path = "MDRM_Multi_model_epochs="+str(epochs)+"_learning-rate"+str(learning_rate)
...
#     plt.savefig(save_path+".png")
    ...
    return
train(duration=3,t_bilstm1=100,fc1=100,a_bilstm1=100,c_bilstm=100,c_fc=50 ,y_train=y_train3days, y_val=y_val3days, y_test=y_test3days, batch_size=64, epochs=64, learning_rate=0.001)
...
train(duration=30,t_bilstm1=100,fc1=50,a_bilstm1=100,c_bilstm=100,c_fc=100 ,y_train=y_train30days, y_val=y_val30days, y_test=y_test30days, batch_size=32, epochs=80, learning_rate=0.001)
