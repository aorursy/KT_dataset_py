...
def recommend(model, user_id, n=5):
    ...
    other columns).
    """
    predictions = model.predict([[user_id] * len(movies), movies.index])
    movies["predicted_rating"] = predictions + offset
    recommendations = movies.sort_values("predicted_rating", 
...
def recommend_nonobscure(model, user_id, n=5, min_ratings=1000):
    ...
    """
    recs = recommend(model, user_id, n=len(movies))
    recs_popular = recs[recs.n_ratings > min_ratings].head(n)
    return recs_popular
part4.hint()
part4.solution()
