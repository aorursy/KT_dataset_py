...
class zy_Model(nn.Module):
    ...
    def __init__(self, n_classes=3):
        ...
        self.hidden_fc_crf2= nn.Sequential(nn.Linear(dim+2, 3),nn.ReLU(True))
    ...
    def forward(self,ids,seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger,qa_start,qa_end,is_test=False):
        ...
        emissions2=self.hidden_fc_crf2(torch.cat((last_seq,outs.detach()),2))

        emissions=self.hidden_fc(torch.cat((last_seq,emissions2.detach(),outs.detach()),2))
...
#         emissions2=self.hidden_fc_crf2(torch.cat([last_seq,emissions.detach()],2))
        ...
        if not is_test:
            ...
            return loss,cls_loss/cls_label_65.size()[1]+cls_6_loss/cls_label_6.size()[1]+c_number_loss+c_type_loss+loss_crf+c_qaloss
...
#         print('crf2:',metric_fn_word(results_crf2,label_results_crf2)) #crf2的结果
        ...
        return metric_fn_word(results_crf2,label_results_crf2)

    else:

        return results,results_crf2
...
with open('right_recall1_val_large_trigger.pkl','wb') as f:
...
with open('right_recall1_test_large_trigger.pkl','wb') as f:
