...
tot_missing = sum(missing_val_count_by_column[missing_val_count_by_column > 0].values)
...
diff_cols = [c for c in X_train.columns if c not in reduced_X_train.columns]

print(diff_cols)

final_X_test = X_test.drop(columns=diff_cols)

print(len(final_X_test.columns))

final_X_test = pd.DataFrame(imp_mean.fit_transform(final_X_test))
