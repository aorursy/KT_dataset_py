...
classes = 5 #5
link_training ='https://raw.githubusercontent.com/cacoderquan/Sentiment-Analysis-on-the-Rotten-Tomatoes-movie-review-dataset/master/training.tsv'

link_testing ='https://raw.githubusercontent.com/cacoderquan/Sentiment-Analysis-on-the-Rotten-Tomatoes-movie-review-dataset/master/testing.tsv'
train = pd.read_csv(link_training,sep='\t')

test = pd.read_csv(link_testing,sep='\t')
...
print(fea_vec_test_dim, classes)
...
def baseline_cnn_model(fea_matrix, classes, mode, compiler):
...
adm = optimizers.Adam(learningrate = learningrate, decay = decay)

sgd = optimizers.SGD(learningrate = learningrate, nesterov = True, momentum = 0.7, decay = decay)

Nadam = optimizers.Nadam(learningrate = learningrate, beta_1=0.9, beta_2=0.999, epsilon = 1e-08)

model = baseline_cnn_model(X_train, classes, mode, Nadam)
