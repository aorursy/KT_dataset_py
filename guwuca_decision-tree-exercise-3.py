...
trainX, trainY = train_data.iloc[:,1:], pd.DataFrame(train_data.iloc[:,0])
...
def best_split(data, features, target):
    ...
    for feature in features:
        ...
        # 计算当前划分之后的分类犯错率
        error = (left_misses + right_misses)/num_data_points 
...
class MyDecisionTree(BaseEstimator):
    ...
    def fit(self, X, Y, data_weights = None):
        ...
        self.root_node = self.create_tree(data_set, features,target,
                                          0, self.max_depth, self.min_error)
    ...
    def create_tree(self, data, features, target, current_depth = 0, max_depth = 10, min_error=0):
        ...
        # 拷贝以下可用特征
        remaining_features = features.copy(deep=True) 
     #   print("remaining Features=", remaining_features)
        ...
        split_feature = best_split_entropy(data, remaining_features, target)  # 根据信息增益来划分
    ...
    def create_leaf(self, target_values):
        ...
        num_positive_ones = (target_values==1).sum()
        num_negative_ones = (target_values==-1).sum()
