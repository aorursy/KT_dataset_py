...
def starts_and_ends_with_vowel(word):
    ...
    starts_and_ends_with_vowel = True
    ...
    if word[-1] in vowels and word[0] in vowels:
        ...
        starts_and_ends_with_vowel = False

    return starts_and_ends_with_vowel
...
def get_number_of_books(values):
...
def total_of_book_prices(values):

    total_price = sum(books["price"] for books in values)
...
def get_average_book_price(values):

    return statistics.mean(books["price"] for books in values)
...
#i don't understand how this works. I found a similar example and tried it here.

def lowest_price_book(value):

    min_price = min(value, key=lambda x:x['price'])
...
def total_number_of_items(values):
...
def get_average_item_price(values):
...
def get_average_spent_per_item(values):
