...
from tensorflow.keras.layers import Dense, Input, Dropout, Add, Activation
...
def build_MLP(layer_sizes, input_shape, loss, learning_rate=0.001, dropout=0, l2_param=0):

    regul = regularizers.l2(l2_param)
    ...
    for layer_size in layer_sizes:
        ...
        x = Dense(layer_size, activation='relu', kernel_regularizer=regul)(x)
    ...
    return model
# model1 = build_MLP(layer_sizes=[50, 50], input_shape=(X_train.shape[1],),

#                    loss='mean_squared_error', learning_rate=1e-4)

# model1.fit(X_train, y_train, epochs=10, validation_data=(X_val, y_val), verbose=0)

# get_score(model1, X_val, y_val)
# model2 = build_MLP(layer_sizes=[100, 100], input_shape=(X_train.shape[1],),

#                    loss='mean_squared_error', learning_rate=1e-4)

# model2.fit(X_train, y_train, epochs=10, validation_data=(X_val, y_val), verbose=0)

# get_score(model2, X_val, y_val)
# model3 = build_MLP(layer_sizes=[150, 150], input_shape=(X_train.shape[1],),

#                    loss='mean_squared_error', learning_rate=1e-4)

# model3.fit(X_train, y_train, epochs=10, validation_data=(X_val, y_val), verbose=0)

# get_score(model3, X_val, y_val)
# model4 = build_MLP(layer_sizes=[200, 200], input_shape=(X_train.shape[1],),

#                    loss='mean_squared_error', learning_rate=1e-4)

# model4.fit(X_train, y_train, epochs=10, validation_data=(X_val, y_val), verbose=0)

# get_score(model4, X_val, y_val)
model5 = build_MLP(layer_sizes=[150, 150], input_shape=(X_train.shape[1],),

                   loss='mean_squared_error', learning_rate=1e-4, l2_param=1e-4)

history = model5.fit(X_train, y_train, epochs=1000, validation_data=(X_val, y_val), verbose=0)
...
model6 = build_MLP(layer_sizes=[150, 150], input_shape=(X_train.shape[1],),

                   loss='mean_squared_error', learning_rate=1e-4, l2_param=1e-4)

model6.fit(X_train, y_train, epochs=num_epochs, validation_data=(X_val, y_val), verbose=0)

get_score(model6, X_val, y_val)
preds_test = model6.predict(X_test).flatten()
