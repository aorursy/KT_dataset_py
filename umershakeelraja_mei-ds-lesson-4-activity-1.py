...
country_data.boxplot(column = ['life expectancy at birth'],by='Sub region', vert=True,figsize=(25, 16))
plt.show()
# draw a boxplot for GDP split up by region
country_data['GDP']=country_data['GDP per capita (US$)']*country_data['population']
country_data.boxplot(column = ['GDP'],by='Sub region', vert=False,figsize=(25, 16))
plt.show()
...
country_data['life expectancy at birth'].plot.hist(bins=[40,50,60,70,80,90,100])
plt.show()
# draw a histogram for GDP

country_data['GDP2']=country_data['GDP']/1000000
country_data['GDP2'].describe()
country_data['GDP'].plot.hist(bins=100,density=1)
plt.show()
...
country_data.plot.scatter(x='GDP per capita (US$)', y='birth rate per 1000')
plt.show()
# draw a hexagonal bin plot for life expectancy v GDP
country_data.plot.hexbin(x='GDP per capita (US$)', y='life expectancy at birth',gridsize=50)
plt.show()
# draw a hexagonal bin plot for birth rate per 1000 v GDP
country_data.plot.hexbin(x='GDP per capita (US$)', y='birth rate per 1000',gridsize=10)
plt.show()