...
for category in fl_count.index:
    ...
    for i in range(0,int(len(perm)*validation_split)):
        name = perm[i].split('/')[-1]
        shutil.copyfile(perm[i],'./data/train/' + str(category) + '/' + name)
    for i in range(int((len(perm)*validation_split)+1),len(perm)):
        name = perm[i].split('/')[-1]
...
y_pred = [label_map[k] for k in y_pred]
...
for i,sample in enumerate(X_test):
    ...
    y_pred_batch = [label_map[k] for k in y_pred_batch]
    ax[i//4, i%4].imshow(X_test[i])
    ax[i//4, i%4].set_title("{0}, predicted {1}".format(label_map[np.argmax(Y_test[i])], y_pred_batch[i]))
    ax[i//4, i%4].axis('off')
plt.show()   
