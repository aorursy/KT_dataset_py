...
class my_gbdt(object):
    ...
    def __init__(self):
        ...
        for i in range(self.times):
            ...
            self.tree.append(DecisionTreeRegressor(min_samples_split=2, max_depth=2))
    ...
    def fit(self, X, y):
        ...
        for i in range(1,self.times):
            ...
            former_tree = self.tree[i-1]

            new_tree = self.tree[i]

            P_former = former_tree.predict(X)

            y_label = y_label-P_former
    ...
    def predict(self, X):
        ...
        pred_proba = [self.tree[i].predict(X) for i in range(self.times)]
