...
cv_tot = int(.9*X_temp.shape[0])





X_train = X_temp[inds[:cv_tot]]

y_train = y_temp[inds[:cv_tot]]

X_cv = X_temp[inds[cv_tot:]]

y_cv = y_temp[inds[cv_tot:]]
...
def backprop(W,b,X,y,alpha=1e-4):
    ...
    regularization = 0.5*alpha*(np.sum(W[1]**2) + np.sum(W[0]**2)+np.sum(W[2]**2))
...
num_iter = 4000 # number of iterations of gradient descent
...
for i in range(num_iter):
    ...
    curr_batch =np.random.choice(inds,size=batch_size)

    X_temp = X_train[curr_batch]

    y_temp = y_train[curr_batch]

    

    dW, db = backprop(W,b,X_temp,y_temp,alpha)
    ...
    if i % 500 == 0:
        ...
        y_cv_pred = h(X_cv,W,b)

        print("Cross-Val accuracy after", i+1, "iterations is {:.4%}".format( 

              np.mean(np.argmax(y_cv_pred, axis=1)== y_cv)))
...
y_cv_pred_final = h(X_cv,W,b)

print("Final cross-entropy loss is {:.8}".format(loss(y_pred_final,y_train)))

print("Final training accuracy is {:.4%}".format(np.mean(np.argmax(y_pred_final, axis=1)== y_train)))

print("Final cross-val accuracy is {:.4%}".format(np.mean(np.argmax(y_cv_pred_final, axis=1)== y_cv)))
# predictions
