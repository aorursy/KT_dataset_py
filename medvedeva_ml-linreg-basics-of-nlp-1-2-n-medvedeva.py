...
print(zero_words[:50])
# Создаём экземпляр модели со списком украинских и русских минусслов, и не учитываем слова, которых меньше 5 в общей численности
tfidf = TfidfVectorizer(stop_words=stop_words_ukr_rus, min_df=5)

# Обучаем TF-IDF на столбцах name и description и преобразовываем текст в фичи
name_tfidf = tfidf.fit_transform(df_1['name'])

# Посмотрим, какие фичи выделились по названию вакансий
...
for i in range (1, 51):
    ...
    tfidf = TfidfVectorizer(stop_words=stop_words_ukr_rus, min_df=i)
    ...
    print('MSE:', mean_squared_error(y_valid, y_pred))
    print('R2:', r2_score(y_valid, y_pred))
L = [x*0.1 for x in range (1, 11)]
for i in L:

    tfidf = TfidfVectorizer(stop_words=stop_words_ukr_rus, max_df=i)
...
plt.show()
tfidf = TfidfVectorizer(stop_words=stop_words_ukr_rus, max_df=0.4, min_df=146)
...
for i in L:
    ...
    tfidf = TfidfVectorizer(stop_words=stop_words_ukr_rus, min_df=146, max_df=i)
    ...
    print('MSE:', mean_squared_error(y_valid, y_pred))
    print('R2:', r2_score(y_valid, y_pred))
tfidf = TfidfVectorizer(stop_words=stop_words_ukr_rus, min_df=146)
...
print('MSE:', mean_squared_error(y_valid, y_pred))
print('R2:', r2_score(y_valid, y_pred))
df_1['name_description'] = df_1['name'] + ' ' + df_1['description']
print(df_1['name'][0])
print(df_1['description'][0])
print(df_1['name_description'][0])
tfidf = TfidfVectorizer(stop_words=stop_words_ukr_rus, min_df=5)

name_description_tfidf = tfidf.fit_transform(df_1['name_description'])

# Посмотрим, какие фичи выделились по описаниям вакансий
print(tfidf.get_feature_names()[:20])
print(tfidf.get_feature_names()[-20:])

print(name_description_tfidf.shape)
X = name_description_tfidf
y = df_1['log_salary']
...
name_description_bow = bow.fit_transform(df_1['name_description'])


print(bow.get_feature_names()[:20])
print(bow.get_feature_names()[-20:])

print(name_description_bow.shape)
X = name_description_bow
y = df_1['log_salary']
...
for i in range (1, 101):
    ...
    bow = CountVectorizer(stop_words=stop_words_ukr_rus, min_df=i)
