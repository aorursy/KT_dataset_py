...
ax[0].plot(r.history['val_loss'], color='r', label="validation loss",axes =ax[0])
...
ax[1].plot(r.history['val_accuracy'], color='r',label="Validation accuracy")
...
Y_pred_classes = model.predict(X_val).argmax(axis=1) 
...
plt.title("True label: %s Predicted: %s" % (Y_val.values[i], Y_pred_classes[i]));
