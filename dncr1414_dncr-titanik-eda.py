...
train_df.loc[outlier_detect(train_df,["Age","SibSp","Parch","Fare"])]
# Drop Outliers 



train_df = train_df.drop(outlier_detect(train_df,["Age","SibSp","Parch","Fare"]),axis=0).reset_index(drop = True)
...
print("Training Acc: %{}".format(acc_LR_train))

print("Test Acc: %{}".format(acc_LR_test))
random_state = 17
...
classifier_param = [dt_param_grid, svc_param_grid, rf_param_grid, knn_param_grid, lr_param_grid]
cv_result = []
...
"SVM","RandomForestClassifier","KNeighborsClassifier","LogisticRegression"]})
