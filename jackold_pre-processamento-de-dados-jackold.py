...
vendas = pd.DataFrame({'Produto':['Chocolate','Banana'], 'Quantidade':[200, 80], 'Preço':['3,00', '0,5']})
...
a2018 = licit[licit['dataPublicacao'].str.contains('2018')]
...
licit['dataFormatada'] = [datetime.datetime.strptime(x, '%d/%m/%Y').strftime('%Y') for x in licit['dataPublicacao']]
...
licit.groupby(['dataFormatada']).numLicitacao.count()
