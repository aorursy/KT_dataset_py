...
def node_weighted_mistakes(targets_in_node, data_weights):
    ...
    # 计算lable 为-1的所有数据的权重和
    #TODO
    weight_negative = np.dot(data_weights, (targets_in_node == -1))# TODO
...
def best_split_weighted(data, features, target, data_weights):
    ...
    for feature in features:
        ...
        # 计算当前划分之后的分类犯错率
        error = (left_misses + right_misses) / np.sum(data_weights)# TODO
...
class MyAdaboost(BaseEstimator):
    ...
    def fit(self, X, Y):
        ...
        for m in range(self.M):
            ...
            # 计算加权错误
            weighted_error = np.dot(alpha, (prediction != Y[self.target]))# TODO
