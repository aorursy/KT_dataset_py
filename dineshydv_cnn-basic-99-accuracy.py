...
m.add(Conv2D(64,  # number of feature map

             3,3,  # dimension of feature detector matrix

             input_shape=(28,28,1) # shape of input X
...
# add second layer. Input is output of previous layer
...
y_pred
# remove the categories from y_pred and select the index with the maximum probability
...
X_test1 = pd.DataFrame(X_test.flatten().reshape(12600,784))
...
X_test1['pred'] = y_pred1

X_test_err = X_test1[X_test1['actual'] != X_test1['pred']]
...
pred
df_test1 = pd.DataFrame(df_test.flatten().reshape(28000,784))
...
for i in range(11):  
    ...
    plt.imshow(df_test1.values[i].reshape(28,28) )
...
submit.to_csv("cnn_basic_conv2D.csv",index=False)