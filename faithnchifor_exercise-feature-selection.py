...
def select_features_l1(X, y):
    ...
    lr=LogisticRegression(C=0.1,random_state=7,penalty='l1').fit(X,y)
    sfm=SelectFromModel(estimator=lr,prefit=True)
    X_new=sfm.transform(X)
    # Get back the kept features as a DataFrame with dropped columns as all 0s
    selected_features = pd.DataFrame(sfm.inverse_transform(X_new), 
                                        index=X.index,
                                        columns=X.columns)
