...
m.contourf(xx, yy,Biomes, cmap=plt.cm.get_cmap('rainbow', 15), latlon=True, vmin=1, vmax=15, levels=16);
#plt.colorbar(np.arange(1,5), Biomes_names);
cbar = plt.colorbar();
cbar.set_ticks(list())
...
for i in range(15):
    a = areas_delta[i]
    print(Biomes_names[i]+' area km^2:',np.int(
        a))