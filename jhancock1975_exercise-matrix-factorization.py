...
def recommend(model, user_id, n=5):
    ...
    other columns).
    """
    user_id_arr = [user_id]*len(movies)
    predictions = model.predict([
        user_id_arr, # User ids 
        movies.index, # Movie ids
    ])
    result = pd.DataFrame(user_id_arr + predictions)
...
def recommend_nonobscure(model, user_id, n=5, min_ratings=1000):
    ...
    """
    df = recommend(model, user_id, n)
    return df.loc(df['n_ratings']>=min_ratings)
...
l2_reccs = recommend_nonobscure(model, 26556)
l2_reccs
#part6.solution()
uid = 26556
