...
class my_gbdt(object):
    ...
    def __init__(self):
        ...
        for tmp in range(self.n_trees):

            self.trees.append(DecisionTreeRegressor(min_samples_split=6, max_depth=6))
    ...
    def fit(self, X, y):
        ...
        for tmp in range(1, self.n_trees):

            residual = y - pred_proba

            self.trees[tmp].fit(X, residual)

            pred_proba += self.lr * self.trees[tmp].predict(X)        
