...
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    def simulate():
        rem_balance = start_balance
        rem_spins = n_spins
        while rem_balance >= 1 and rem_spins:
            rem_balance = rem_balance + play_slot_machine() - 1
            rem_spins = rem_spins - 1
        
        return True if rem_spins == 0 else False
    
    return sum([simulate() for i in range(n_simulations)]) / n_simulations
