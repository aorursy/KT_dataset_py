...
dataset.subtask_b = dataset.subtask_b.apply(label_encoder)
...
def get_dataset(dataset, max_length):
    ...
    for idx, row in dataset.iterrows():
        ...
        label_list.append(row.subtask_b)
...
y_true = np.array(dataset.iloc[train_cutoff:].subtask_b)
print("classification report")

print(classification_report(y_true, y_pred, labels=[0,1,2], target_names=['UNT', 'TIN', "NaN"]))
...
print(classification_report(1-(y_true==2)*1, 1-(y_pred==2)*1, labels=[0,1], target_names=['NOT', 'OFF']))

print("confusion matrix")

print(confusion_matrix(1-(y_true==2)*1, 1-(y_pred==2)*1))

print("f1_score")

print(f1_score(1-(y_true==2)*1, 1-(y_pred==2)*1))
learning_rate = 2e-5
...
bert_history = model.fit(full_dataset, 
                         ...
                         epochs=number_of_epochs)
test_set = pd.read_csv("../input/nlp-unifor/testset-levelb.tsv", sep="\t")

test_set["subtask_b"] = np.nan

test_ds = get_dataset(test_set, max_len).batch(batch_size)

y_pred = np.argmax(model.predict(test_ds)[0], axis=1)

test_set["subtask_b"] = y_pred

test_set["subtask_a"] = test_set.subtask_b.apply(lambda x: 0 if x==2 else 1)
test_set[["tweet","subtask_a","subtask_b"]].to_csv("subtask_b.csv", index=False)