...
# Обучаем мешок слов на корпусе и преобразовываем текст в фичи
bow_matrix = bow.fit_transform(df2)
...
pd.DataFrame(bow_matrix.toarray(), columns=bow.get_feature_names())
# Словарь из отдельных слов + биграмм
bow = CountVectorizer(stop_words=stop_words_ukr_rus, ngram_range=(1, 2))

bow_matrix = bow.fit_transform(df2)
pd.DataFrame(bow_matrix.toarray(), columns=bow.get_feature_names())
# Убираем популярные слова
bow = CountVectorizer(stop_words=stop_words_ukr_rus, max_df=0.5)

bow_matrix = bow.fit_transform(df2)
pd.DataFrame(bow_matrix.toarray(), columns=bow.get_feature_names())
# Убираем редкие слова
bow = CountVectorizer(stop_words=stop_words_ukr_rus, min_df=2)
...
name_bow = bow.fit_transform(df_1['name'])
print(name_bow.shape)
X=name_bow
# Попробуем обучить линейную регрессию с параметрами по умолчанию на полученной матрице
y = df_1['log_salary']

from sklearn.model_selection import train_test_split
X_train, X_valid, y_train, y_valid = train_test_split(name_bow, y, test_size=0.25, random_state=1)
...
from sklearn.model_selection import train_test_split
X_train, X_valid, y_train, y_valid = train_test_split(name_tfidf, y, test_size=0.25, random_state=1)
...
from sklearn.model_selection import train_test_split
X_train, X_valid, y_train, y_valid = train_test_split(tfidf_matrix, y, test_size=0.25, random_state=1)

from sklearn.linear_model import LinearRegression
lin_reg = LinearRegression()
lin_reg.fit(X_train, y_train)
y_pred = lin_reg.predict(X_valid)
df3=df_1['description']
# Подключаем класс TfidfVectorizer
from sklearn.feature_extraction.text import TfidfVectorizer
...
# Обучаем мешок слов на корпусе и преобразовываем текст в фичи
bow_matrix = bow.fit_transform(df3)
...
from sklearn.model_selection import train_test_split
X_train, X_valid, y_train, y_valid = train_test_split(desc_bow, y, test_size=0.25, random_state=1)
