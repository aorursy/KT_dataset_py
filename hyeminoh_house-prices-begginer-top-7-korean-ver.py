...
# Exterior 1/2는 한 개의 결측치만 있으므로 빈도수가 가장 많은 값으로 채움
...
# 'MsZoning'에서 빈도수가 가장 높은 값은 RL이므로 그것으로 채움
...
# 지하실에 관한 feature들 중 결측치가 있는 것은 집에 지하실이 없다는 것을 의미.
# 결측치를 0으로 채움.
...
# 이웃한 집끼리 그룹화하여 Lotfrontage를 모든 이웃의 중간값으로 채움
...
# Functional에서 결측치는 일반적(Typical)임을 의미하여 Typ로 채움.
