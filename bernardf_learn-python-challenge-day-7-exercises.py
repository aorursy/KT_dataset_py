...
def prettify_graph(graph):
    ...
    graph.set_yticklabels('$ %.0f' % y for y in graph.get_yticks())
...
def blackjack_hand_greater_than(hand_1, hand_2):
    ...
    for card in hand_2:
        ...
        h2_total+=int(card)
    if h2_total>21:
        h2_total-=10*ace_counter
            
    return True if ((h1_total > h2_total) and (h1_total <= 21)) or (h2_total>21 and (h1_total <= 21)) else False
...
def my_agent(wheel):
    ...
    top_prob=0
    top_roll=0
    result=(0,0,0) #prev, next, prob
    for prev, nexts in probs.items():
        for roll, prob in nexts.items():
            if prob > top_prob:
                top_prob=prob
                top_roll=roll
        if top_prob > result[2]:
            result=(prev, top_roll, top_prob)
    while wheel.num_remaining_spins() > 0:
        if last_number == result[0]:
            # Lucky number
            guess = result[1]
        else:
            guess = None
        wheel.spin(number_to_bet_on=guess)

roulette.evaluate_roulette_strategy(my_agent, num_spins_per_simulation=1000)