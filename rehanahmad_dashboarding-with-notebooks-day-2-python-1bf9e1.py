...
mydata = pd.read_csv('../input/openpowerlifting.csv')
agecount = mydata['Age'].groupby(mydata.Age).agg('count')
agecount = agecount.to_frame()
agecount['Age col'] = agecount.index
agecount = agecount.rename(columns={agecount.columns[0]:"count", agecount.columns[1]:"age"})
agecount = agecount.reset_index(drop=True)
...
data = [go.Bar(x=agecount['age'], y=agecount['count'])]
layout = dict(title = "Number of Powerlifters of same age",
              xaxis= dict(title= 'Age'), yaxis=dict(title='count'))

fig = dict( data=data, layout=layout )
iplot(fig, filename='barplot-age-count')