...
def best_items(racers):
    ...
    for i in range(len(racers)):
        # The i'th racer dictionary
        racer = racers[i]
        #print(racer['name'],i,sep='*')

        # We're only interested in racers who finished in first
        if racer['finish'] == 1:
            for items in racer['items']:
                # Add one to the count for this item (adding it to the dict if necessary)
                if items not in winner_item_counts:
                    winner_item_counts[i] = 0
                winner_item_counts[i] += 1
...
def blackjack_hand_greater_than(hand_1, hand_2):
    ...
    False
    """
    trans = {**{str(k+1):k+1 for k in range(10)},**{'A':1,'J':10,'Q':10,'K':10}}
    def total(hand):
        ret = 0
        for card in hand:
            ret += trans[card]
        for i in range(0,hand.count('A')): 
            if (ret+10<21): 
                ret+=10
        return ret
    
    if (total(hand_1)>21): return False
    if (total(hand_2)>21): return True
    print(hand_1,total(hand_1),hand_2,total(hand_2))
    return (total(hand_1)>total(hand_2))
...
def my_agent(wheel):
    last_number = 0
    while wheel.num_remaining_spins() > 0:
        if (last_number in wons): 
            bet = wons[last_number]
        else:
            bet =  random.randint(0, 10)
        result = wheel.spin(number_to_bet_on=bet)
        wons[last_number] = result
        last_number = result
