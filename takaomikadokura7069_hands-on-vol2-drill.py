...
df = train.copy()
df['SalePrice_std']=scaler.fit_transform(train[['SalePrice']])

# ヒストグラム作成
sns.distplot(df['SalePrice_std'])
fig = plt.figure()

# Q-Qプロット作成
res = stats.probplot(df['SalePrice_std'], plot=plt)
plt.show()

# 変換後の要約統計量表示
df['SalePrice_std'].describe()
...
# 学習データをコピーし、新たなdataframeで検証
df = train.copy()
df['SalePrice_min-max']=scaler.fit_transform(train[['SalePrice']])

# ヒストグラム作成
sns.distplot(df['SalePrice_min-max'])
fig = plt.figure()

# Q-Qプロット作成
res = stats.probplot(df['SalePrice_min-max'], plot=plt)
plt.show()

# 変換後の要約統計量表示
df['SalePrice_min-max'].describe()
...
# sns.pairplot(train, y_vars=["SalePrice"],x_vars=['TotalSF','OverallQual','GrLivArea','GarageCars','FullBath','YearBuilt','YearRemodAdd','GarageYrBlt','Fireplaces','MasVnrArea','BsmtFinSF1','LotFrontage','OpenPorchSF','WoodDeckSF','HalfBath','LotArea','BsmtFullBath'])
# plt.show
