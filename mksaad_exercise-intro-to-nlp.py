...
arnlp = spacy.blank('ar')

ardoc = arnlp('السلام عليكم، كيف الحال؟')
...
#     print(idx)
    ...
    for match in matches:
        ...
        found_item = str(doc[match[1]:match[2]]).lower()

        stars = review.stars

        if found_item in item_ratings:

            item_ratings[found_item].append(stars)

        else:

            item_ratings[found_item] = [stars]
