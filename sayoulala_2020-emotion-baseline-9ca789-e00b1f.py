...
#         print(mask_x.size())
    ...
    def forward(self,ids,seg_ids,task_id=1):
        ...
        feat_all=torch.cat((task1_feat.detach(),task1_out.detach(),task2_feat),1)
...
class Myloss(nn.Module):
    ...
    def forward(self,preds,true,task_id=1):

        if task_id==0:

            return F.binary_cross_entropy(preds,(true==1)*1.0)

        else:

            return F.nll_loss(preds[1],true)
...
#             {'params':model.query,'lr':1e-3}
    ...
    for epoch in range(1,epochs+1):
        ...
        for i,(ids, seg_ids,labels) in enumerate(bar):
            ...
            outputs = model(ids.cuda(DEVICE),seg_ids.cuda(DEVICE),task_id)

            if task_id==0:

                loss = loss_fn(outputs, labels.cuda(DEVICE),task_id)
...
for i,(train_index , test_index) in enumerate(kf.split(train,train['label'].values)):
    ...
    np.save('multi_fold{}.npy'.format(i+1),preds)

    

    #val preds

    val_preds=predict_model_test(model,val_loader)

    train_preds[test_index,:]=val_preds
...
test_preds[['id','y']].to_csv('sub_multi_fold.csv',index=False)
test_preds