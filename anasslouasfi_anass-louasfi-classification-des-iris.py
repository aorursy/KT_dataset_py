...
sns.jointplot("sepal_length", "petal_length", df, kind='kde');
plt.figure(figsize=(12,12))

sns.kdeplot(df.sepal_length, df.petal_length,  shade=True)
plt.figure(figsize=(12,12))

sns.kdeplot(df[setosa].sepal_length, df[setosa].petal_length, cmap="Reds",  shade=True, alpha=0.3, shade_lowest=False)

sns.kdeplot(df[virginica].sepal_length, df[virginica].petal_length, cmap="Greens", shade=True, alpha=0.3, shade_lowest=False)

sns.kdeplot(df[versicolor].sepal_length, df[versicolor].petal_length, cmap="Blues", shade=True, alpha=0.3, shade_lowest=False)
...
fig.add_legend()
sns.lmplot(x="sepal_length", y="petal_length", data=df, fit_reg=False, hue='species')
sns.pairplot(df, hue="species")
data_train = df.sample(frac=0.8, random_state=1)          # 80% des données avec frac=0.8
...
fig.add_legend()
fig = sns.FacetGrid(df[df.petal_width>1.7], hue="species", aspect=3) # aspect=3 permet d'allonger le graphique
...
tree.plot_tree(dtc, feature_names=X_train.columns, class_names=['virginica','setosa','versicolor'], fontsize=14, filled=True)  
dtc1 = tree.DecisionTreeClassifier(max_depth = 4, min_samples_leaf = 20)

dtc1.fit(X_train,y_train)
plt.figure(figsize=(30,30))

tree.plot_tree(dtc1, feature_names=X_train.columns, class_names=['virginica','setosa','versicolor'], fontsize=14, filled=True)  
y_dtc1 = dtc1.predict(X_test)
