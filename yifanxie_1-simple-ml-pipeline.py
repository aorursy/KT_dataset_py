...
gbm=GradientBoostingClassifier(random_state=2017)

gbm.fit(x_train, y_train)

gbm_preds=gbm.predict_proba(x_val)

print(log_loss(y_val, gbm_preds))
...
print("it takes %.3f seconds to train and predict" % (end_time - start_time))
# create submission with Logistic Regression model



test_lr_preds=lr.predict_proba(test_input)

lr_submission=sample.copy()

lr_submission.predict_0=test_lr_preds[:,0]

lr_submission.predict_1=test_lr_preds[:,1]

lr_submission.predict_2=test_lr_preds[:,2]

lr_submission.to_csv('lr_submission_v1.csv.gz', compression='gzip', index=False)
...
rf_submission.to_csv('rf_submission_v1.csv.gz', compression='gzip', index=False)
preds=gbm.predict_proba(test_input)

submission=sample.copy()

submission.predict_0=preds[:,0]

submission.predict_1=preds[:,1]

submission.predict_2=preds[:,2]

submission.to_csv('gbm_submission_v1.csv.gz', compression='gzip', index=False)