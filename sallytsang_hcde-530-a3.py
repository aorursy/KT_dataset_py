...
#sum of count in boys' dictionaries 

total_boys=sum(boys.values())



#sum of count in girls' dictionaries 

total_girls=sum(girls.values())



total_babies=total_boys+total_girls

print("There are "+ str(total_babies)+" babies in the dataset.")

# Exercise 3) What is the longest name in dataset?



# build a new dictionary that combines both boys and girls

babies = {} # create an empty dictionary of key:value pairs for all babies
...
for line in open(NAMES_LIST, 'r').readlines():  # iterate through the lines and separate each line on its commas
    ...
    babies[name.lower()] = count 

    

# find out the longest length from all the keys in babies dictionary

maxkey=(len(max(babies, key=len)))



# find out the name that has 15 characters - takes over 2 mins to run 
...
print("The above are the longest names in this dataset with "+ str(maxkey)+" characters.")
...
# Find out how many boy's name used in girls
...
print(str(count)+" boy names are also girl names.")

        

# Find out how many girl's name used in boys
...
print(str(gNameCount)+" girl names are also boy names.")
