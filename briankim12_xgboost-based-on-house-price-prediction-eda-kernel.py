...
train_dataset = pd.get_dummies(df_train)
...
model.fit(df_train, y_reg)
train_preds = model.predict(df_train)

cv = np.sqrt(mean_squared_error(train_preds, y_reg))
...
test_xgb_preds = np.expm1(model.predict(df_test))

test_ensemble_preds = (test_lgb_preds + 2*test_xgb_preds)/3
submission1 = pd.DataFrame({'id': test_id, 'price': test_ensemble_preds})

submission1.to_csv('ensemble.csv', index=False)
submission_xgboot = pd.DataFrame({'id': test_id, 'price': test_xgb_preds})

submission_xgboot.to_csv('xgb_preds.csv', index=False)