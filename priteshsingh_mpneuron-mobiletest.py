...
Y_train.head()
X_train_binarised=X_train.apply(pd.cut, bins=2, labels=[0,1])

X_train_binarised.head()

Y_train_binarised=train_new['Rating'].map(lambda x: 1 if x>=4 else 0)

Y_train_binarised.tail()

X_test=test_new.drop(['PhoneId'],axis=1)

X_test.head()
X_test_binarised=X_test.apply(pd.cut, bins=2, labels=[0,1])

X_test_binarised.head()
...
mp_neuron.fit(X_train_binarised,Y_train_binarised)
