...
def estimate_average_slot_payout(n_runs):
    ...
    for i in range(n_runs):
        total_out += play_slot_machine() 
    return (total_out-n_runs)/n_runs
...
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    for i in range(n_simulations):
        ...
        current_spin = 0
        while current_balance >= 1 and current_spin < n_spins:
            current_spin += 1
            current_balance += play_slot_machine() - 1
        if current_spin == n_spins:
            favor+=1
    return favor/n_simulations
