...
data.head()
print (data['safe_loans'].value_counts())
...
percentage = float(len(bad_ones))/len(safe_ones)#TODO

risky_loans = bad_ones
safe_loans = safe_ones.sample(frac = percentage, random_state=33)#TODO

# combine two kinds of loans
data_set = pd.concat([risky_loans,safe_loans],ignore_index=True)#TODO
data_set.head()
...
data_set.head()
train_data, test_data = train_test_split(data_set)#TODO
