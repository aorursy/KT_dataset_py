...
cities.sort()
cities
# get all the unique values in the 'Targeted Sect if any' column
targeted = suicide_attacks['Targeted Sect if any'].unique()
targeted
# Fixing 'shiite' to 'Shiite'
replace_matches_in_column(df=suicide_attacks, column='Targeted Sect if any', string_to_match="Shiite")
targeted = suicide_attacks['Targeted Sect if any'].unique()
targeted
# Open the Pakistan Suicide Attacks v6
suicide_attacks_v6 = pd.read_csv("../input/PakistanSuicideAttacks Ver 6 (10-October-2017).csv", 
                              encoding='Windows-1252')
suicide_attacks_v6.head()
# get all the unique values in the 'City' column
citiesv6 = suicide_attacks_v6['City'].unique()

# sort them alphabetically and then take a closer look
citiesv6.sort()
citiesv6
# Replacing 'ATTOCK' per 'Attock'
replace_matches_in_column(df=suicide_attacks_v6, column='City', string_to_match="Attock")
# get all the unique values in the 'City' column
citiesv6 = suicide_attacks_v6['City'].unique()
