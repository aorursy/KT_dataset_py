...
Lockdown_4=covid_india[(covid_india["ObservationDate"]>=pd.to_datetime("2020-05-19"))]
...
C1=[]
while(1):
    rise_6k.append(int(india_datewise[india_datewise["Confirmed"]<=case_6k].iloc[[-1]]["Days Since"]))
    C1.append(case_6k)
    case_6k=case_6k+6000
    if(case_6k<india_datewise["Confirmed"].max()):
        continue
    else:
        break
rate_6k=pd.DataFrame(list(zip(C1,rise_6k)),columns=["No. of Cases","Days Since first Case"])
rate_6k["Days required for increase by 6K"]=rate_5k["Days Since first Case"].diff()
fig=go.Figure()
fig.add_trace(go.Scatter(x=rate_6k["No. of Cases"], y=rate_6k["Days required for increase by 6K"],
                    mode='lines+markers',
                    name='Weekly Growth of Confirmed Cases'))
fig.update_layout(title="Number of Days required for increase in number of cases by 6000",
...
ax1.set_ylabel("Country Name",fontsize=10)
ax1.set_xlabel("Mortality",fontsize=10)
ax1.set_title("Mortality in Neighbouring Countries",fontsize=12)
sns.barplot(y=comp_data.index,x=comp_data["Recovery"],ax=ax2)
ax2.set_xlabel("Recovery",fontsize=10)
ax2.set_title("Recovery in Neighbouring Countries",fontsize=12)
sns.barplot(y=comp_data.index,x=comp_data["Survival Probability"],ax=ax3)
ax3.set_xlabel("Survival Probability",fontsize=10)
ax3.set_title("Survival Probability in Neighbouring Countries",fontsize=12)
...
fig.update_layout(height=800,title="COVID-19 Journey of India's Neighbouring countries",
                 xaxis_title="Confirmed Cases (Logrithmic Scale)",yaxis_title="Active Cases (Logarithmic Scale)",
                 legend=dict(x=0,y=1,traceorder="normal"))
fig.show()
pd.set_option('float_format', '{:f}'.format)
country_names=["Spain","US","Italy","Iran","France","UK","Brazil","India"]
country_data=pd.concat([spain_datewise.iloc[[-1]],us_datewise.iloc[[-1]],italy_datewise.iloc[[-1]],iran_datewise.iloc[[-1]],
          france_datewise.iloc[[-1]],uk_datewise.iloc[[-1]],brazil_datewise.iloc[[-1]],india_datewise.iloc[[-1]]])
...
fig.update_layout(title="Weekly Growth of Death Cases",
                 ...
                 legend=dict(x=0,y=1,traceorder="normal"))
fig.show()
ac_median_age=[45.5,38.3,47.9,32.4,42,40.8,33.5,28.2]
ac_tourists=[75315000,76407000,52372000,4942000,82700000,35814000,6547000,14570000]
ac_weight=[70.556,81.928,69.205,67.608,66.782,75.795,66.093,52.943]
ac_gdp=[1.62,24.08,2.40,0.56,3.19,3.26,2.54,3.28]
ac_area=[505992,9833517,301339,1648195,640679,242495,8515767,3287263]
ac_pd=[93,34,200,51,123,280,25.43,414]
...
for country in country_names:
    fig.add_trace(go.Scatter(x=grouped_country.ix[country]["log_confirmed"], y=grouped_country.ix[country]["log_active"],
                    mode='lines',name=country))
fig.update_layout(height=800,title="COVID-19 Journey of some worst affected countries and India",
                 xaxis_title="Confirmed Cases (Logrithmic Scale)",yaxis_title="Active Cases (Logarithmic Scale)",
                 legend=dict(x=0,y=1,traceorder="normal"))
fig.show()
...
y_pred=valid.copy()
holt=Holt(np.asarray(model_train["Confirmed"])).fit(smoothing_level=0.3, smoothing_slope=2.0)
...
valid=india_datewise.iloc[int(india_datewise.shape[0]*0.95):]
y_pred=valid.copy()
es=ExponentialSmoothing(np.asarray(model_train['Confirmed']),seasonal_periods=9, trend='mul', seasonal='mul').fit()
...
models=["Polynomial Regression","Support Vector Machine Regresssor","Holt's Linear Model",
       "Holt's Winter Model","Auto Regressive Model (AR)","Facebook's Prophet Model"]
