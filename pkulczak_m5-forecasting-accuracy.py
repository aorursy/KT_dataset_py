...
try:
    ...
    stationarity_differences = pd.read_csv('/kaggle/input/m5precomputed/stationarity_differences.csv').iloc[:,0]
...
model.load_weights('/kaggle/input/m5checkpoints/croc_model5.ckpt')
...
batch_sequence = [2, 2, 2, 4, 4, 4, 4, 8, 8]
...
for epoch in range(len(batch_sequence)):
    ...
    for on, (X_val, y_true) in eval_series_data_gen(in_points=IN_POINTS, out_points=OUT_POINTS, end_of_data=1913, max_row=VAL_SIZE):
        ...
        epoch_val_loss.append(sqrt(val_loss))
...
for  on, (X, y) in eval_series_data_gen(in_points=IN_POINTS, out_points=OUT_POINTS, end_of_data=1913):
...
for  on, (X, y) in eval_series_data_gen(in_points=IN_POINTS, out_points=OUT_POINTS, end_of_data=1941):
