...
inception_model = Model(base_model.input, d2)
inception_model.compile(optimizer = 'adam', loss = categorical_crossentropy, metrics = ["accuracy"])
inception_model.summary()
inception_history=inception_model.fit_generator(train_generator, epochs=10, validation_data=val_generator)
scores= inception_model.evaluate_generator(test_generator, steps = 1, verbose = 1)
print("Accuracy = ", scores[1])
inception_model.save("inception_model.h5")
plot_history(inception_history)
import json
json_str = inception_model.to_json()
