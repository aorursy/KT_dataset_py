...
layout_object = go.Layout(title='US daily cases IDNUMBER',xaxis=dict(title='Date'),yaxis=dict(title='Number of people'))
fig = go.Figure(data=[daily_confirmed_object,daily_deaths_object],layout=layout_object)
iplot(fig)
fig.write_html('US_daily_cases_IDNUMBER.html')
