...
fertility=lit_birth_rate['fertility']

female_literacy=lit_birth_rate['female literacy']
# Create the figure: p
...
appl_df=pd.read_csv('/kaggle/input/visualizeusingbokeh/aapl.csv',parse_dates=True)
date=appl_df['date']

open_price=appl_df['open']
...
p.circle(date,open_price, fill_color='white', size=4)
...
show(p)
p = figure(x_axis_label='value of x between 0 to 5',y_axis_label='Cos in degrees')
...
show(p)
auto_df = pd.read_csv('/kaggle/input/visualizeusingbokeh/auto-mpg.csv')
...
p.circle(auto_df['hp'],auto_df['mpg'],size=10,color=auto_df['color'])
...
sprint_df=pd.read_csv('/kaggle/input/visualizeusingbokeh/sprint.csv')

source = ColumnDataSource(sprint_df)
...
p.circle('Year','Time',selection_color='red',nonselection_alpha=0.1,nonselection_fill_color='blue',source=source)
