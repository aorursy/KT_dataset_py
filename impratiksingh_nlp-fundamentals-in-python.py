...
# with open('../input/articles.txt','r') as file:
...
# \ is used before [] since [] have special meaning in re contect and but we want to treat it as a normal character. 
...
print(re.search(pattern1,scene_one)) # doubt
...
# retain sentence punctuation as separate tokens, but have '#1' remain a single token.
...
tweets=['This is the best #nlp exercise ive found online! #python', '#NLP is super fun! <3 #learning', 'Thanks @kaggle :) #nlp #python']
...
my_documents=['The movie was about a spaceship and aliens.','I really liked the movie!','Awesome action scenes, but boring characters.',
...
# Select the id for "Space": space_id

space_id = dictionary.token2id.get("space")



# Use space_id with the dictionary to print the word

print(dictionary.get(space_id))
...
for sent in chunked_sentences:
    ...
    for chunk in sent:
        ...
        if hasattr(chunk, "label") and chunk.label() == "NE":
            ...
            print(chunk)
# Create the defaultdict: ner_categories (Revisit )
...
nlp = spacy.load('en',tagger=False,parser=False,matcher=False) # nlp is spacy.lang.en.English



# Create a new document: doc # spacy.tokens.doc.Doc

doc = nlp(article) # It is the complete document
