...
q_4.hint()
q_4.solution()
inter1  =  pdp.pdp_interact(model=second_model, dataset=new_val_X, model_features=features_2, features=['pickup_longitude', 'dropoff_longitude'])

pdp.pdp_interact_plot(pdp_interact_out=inter1, feature_names=['pickup_longitude', 'dropoff_longitude'], plot_type='contour')
plt.show()
# no pdp only measures extent of output change in relation to change in input, while permutation importance measures size of error change. pdp does not take into account true value while perm does.
q_5.solution()
