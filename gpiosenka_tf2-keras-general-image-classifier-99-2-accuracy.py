...
def get_paths(source_dir,output_dir,mode,subject):
    ...
    if mode =='ALL':
        ...
        valid_path=os.path.join(source_dir,'val')
    ...
    if mode != 'ALL':
        ...
        train_gen=ImageDataGenerator(preprocessing_function=keras.applications.mobilenet.preprocess_input,
                ...
                samplewise_std_normalization=True).flow_from_directory(paths[0], target_size=(height, width),
                 ...
                 batch_size=batch_size, seed=rand_seed, subset='training')
        ...
        val_gen=ImageDataGenerator(preprocessing_function=keras.applications.mobilenet.preprocess_input,
                ...
                # database does not have enough validation data so creating a seperate validation set

                # thats why below paths[0] the path to the training data is used

                samplewise_std_normalization=True).flow_from_directory(paths[0], 

                target_size=(height, width), batch_size=batch_size,seed=rand_seed,subset='validation')
...
def TF2_classify(source_dir, output_dir, mode, subject, v_split=5, epochs=20, batch_size=80,
            ...
            if tr.best_acc>.9:
                ...
                if tr.best_acc<acc:
                    ...
                    if tr.p_count >= tr.patience:
                        ...
                        print(' \nadjusted learning rate after batch {0} to {1:9.7f}'.format(batch + 1, new_lr))

    

    class tr(tf.keras.callbacks.Callback):

        # this callback monitor training accuracy until it gets above .9.

        # if there are 10 consecutive training batches with no improvement in accuracy it will

        #reduce the learning rate by a factor of .95

        # after accuracy gets above ,9 this callback does nothing and the learning rate gets

        # adjusted by the val callback which monitors validation loss
        ...
        def on_batch_end(self, batch, logs=None): 
            ...
            if tr.best_acc>.9:
                ...
                if tr.focus=='acc':
                    ...
                    print(msg.format(Cblu, acc, Cend))
                ...
                if tr.best_acc<acc:
                   ...
                   # print(msg.format(batch + 1, tr.best_acc, acc ))
                    ...
                    if tr.p_count >= tr.patience:
                        ...
                        print(' adjusted learning rate after batch {0} to {1}'.format(batch + 1, new_lr))

                        

    class val(tf.keras.callbacks.Callback):

        # this callback does nothing until the training accuracy reaches .9.

        # at that point it monitor validation loss and lowers the learning rate by a factor of .5

        # if the validation loss fails to improve after 1 epoch
        ...
        def on_epoch_end(self, epoch, logs=None):            
            ...
            if v_loss<val.best_loss:
                ...
                if tr.focus=='val':
                        ...
                        msg='\n for epoch {0} current loss {1:7.4f} exceeds best boss of {2:7.4f} reducing lr to {3:9.7f}'

                        print(msg.format(epoch + 1, v_loss, val.best_loss, new_lr))
    ...
    while run:
        ...
        #tr_plot(tacc,vacc,tloss,vloss) # plot the data on loss and accuracy not needed for only 1 epoch
...
d1=os.listdir('/kaggle/input')

d2=os.path.join('/kaggle/input','kermany2018')

d3=os.listdir(d2)

d4=os.path.join(d2,'OCT2017 ' )

source_dir=d4

output_dir=r'c:\Temp\retina'
