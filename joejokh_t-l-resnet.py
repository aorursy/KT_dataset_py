...
def resnet50_model(nombre_class=20,show_models = True):
    ...
    #afficher les differents paramétres de la dérniere couche Dense
...
def frezz_blocks(model, Block_to_disable, show= True ):
    ...
    #l'indice de la dernier couche du stage 1 pour resnet

    Resnet_last_layer_stage1 = 38

    

    ##l'indice de la dernier couche du stage 2 pour resnet

    Resnet_last_layer_stage2 = 80
    ...
    if Block_to_disable == 1:
        ...
        for layer in model.layers[:Resnet_last_layer_stage1]:
        ...
        for layer in model.layers[:Resnet_last_layer_stage2]:
...
def get_model_resnet_freezed():

    """fonction qui retourne un modele avec que la couche FC entrainable"""

    modele_resnet = resnet50_model(show_models = False)

    return define_model(modele_resnet,0)



def get_model_resnet_freezed_1block():

    """fonction qui retourne un modele avec un block gelé"""

    modele_adapter = load_model("/kaggle/input/moddde/ResNet_freezed.h5")

    return define_model(modele_adapter,1)



def get_model_resnet_freezed_2block():
...
trainPath_res = "/kaggle/input/db-inf907/AFF20_resized/AFF20_resized/AFF20_resized_train"

testPath_res = "/kaggle/input/db-inf907/AFF20_resized/AFF20_resized/AFF20_resized_test"



#--->Question 1 et 2

trainGen_res, testGen_res, train_x_res, train_y_res, test_x_res, test_y_res = get_data(trainPath_res, testPath_res)
...
ResNet_freezed = get_model_resnet_freezed()

ResNet_freezed.summary()
...
#GridSearchCV_on_epoches(get_model_resnet_freezed,epoch_nbrs=[20,30,50])



#Lancer l'évaluation sur les tailles des batches

#GridSearchCV_on_batches(get_model_resnet_freezed, batch_sizes=[8,16,32])



#Lancer l'évaluation sur les fonction de dégradation du taux d'apprentissage

#GridSearchCV_on_decay(get_model_resnet_freezed)
...
ResNet_freezed, H = fit_and_save(ResNet_freezed,"ResNet_freezed",epochs,learning_rate,decay_learning_rate,trainGen_res)



#sauvgarder l'historique d'apprentissage, les diférant métrics et le graphe de ACC/LOSS

save_history_metrics(ResNet_freezed, H.history, "ResNet_freezed",test_x_res,test_y_res)
...
resnet_freezed_1block = get_model_resnet_freezed_1block()

resnet_freezed_1block.summary()
...
#GridSearchCV_on_epoches(get_model_resnet_freezed_1block,epoch_nbrs=[20,30,50])



#Lancer l'évaluation sur les tailles des batches

#GridSearchCV_on_batches(get_model_resnet_freezed_1block, batch_sizes=[8,16,32])



#Lancer l'évaluation sur les fonction de dégradation du taux d'apprentissage

#GridSearchCV_on_decay(get_model_resnet_freezed_1block)
...
resnet_freezed_1block, H = fit_and_save(resnet_freezed_1block,"resnet_freezed_1block",epochs,learning_rate,decay_learning_rate,trainGen_res)



#sauvgarder l'historique d'apprentissage, les diférant métrics et le graphe de ACC/LOSS

save_history_metrics(resnet_freezed_1block, H.history, "resnet_freezed_1block",test_x_res,test_y_res)
...
resnet_freezed_2block = get_model_resnet_freezed_2block()

resnet_freezed_2block.summary()
...
#GridSearchCV_on_epoches(get_model_resnet_freezed_2block,epoch_nbrs=[20,30,50])



#Lancer l'évaluation sur les tailles des batches

#GridSearchCV_on_batches(get_model_resnet_freezed_2block, batch_sizes=[8,16,32])



#Lancer l'évaluation sur les fonction de dégradation du taux d'apprentissage

#GridSearchCV_on_decay(get_model_resnet_freezed_2block)
...
resnet_freezed_2block, H = fit_and_save(resnet_freezed_2block,"resnet_freezed_2block",epochs,learning_rate,decay_learning_rate,trainGen_res)



#sauvgarder l'historique d'apprentissage, les diférant métrics et le graphe de ACC/LOSS

save_history_metrics(resnet_freezed_2block, H.history, "resnet_freezed_2block",test_x_res,test_y_res)
...
#extract_carts(model_path,"MiniDensNet", train_x_crops[:500],"train_crops")"""