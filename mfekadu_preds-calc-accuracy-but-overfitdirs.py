...
for i in range(0, len(train_gens)):
    ...
    histories[i] = model.fit_generator(generator=train_gens[i],
                        steps_per_epoch=STEP_SIZE_TRAIN,
                        validation_data=valid_gens[i],
                        validation_steps=STEP_SIZE_VALID,
                        epochs=EPOCHS)
# for i in range(0, len(valid_gens)):
# model.evaluate_generator(generator=valid_gens[8])
accuracies = []
val_accuracies = []
losses = []
val_losses = []
...
print(match,"/",count,"=","{:.4f}".format(match/count))