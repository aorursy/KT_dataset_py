...
for i in tqdm(range(1001, 1241)):

    with open('%s%d-c.gnt'%(dir_path, i), 'rb') as fp:

        while fp.read(1)!=b"":

            fp.seek(-1, 1)

            sample_size = struct.unpack("<I", fp.read(4))[0]

            char = str(fp.read(2), encoding='gbk')

            if not (char in idx_to_char[:NUM_CHAR]):
            ...
            assert sample_size == header_size + width*height

            img = 255 - np.array(list(fp.read(width*height)), dtype=np.uint8).reshape(height, width)

            images_trn.append(Image.fromarray(img))

            labels_trn.append(char_to_idx[char])





dir_path = '../input/cchwdb/HWDB1.1tst_gnt/HWDB1.1tst_gnt/'
...
for i in tqdm(range(1241, 1301)):
    ...
    with open('%s%d-c.gnt'%(dir_path, i), 'rb') as fp:
        ...
        while fp.read(1)!=b"":
            ...
            images_tst.append(Image.fromarray(img))

            labels_tst.append(char_to_idx[char])
class MyDataset(torch.utils.data.Dataset):

    def __init__(self, images, labels, std_size=64):
        ...
        self.transforms = tv.transforms.RandomChoice([    
                ...
                tv.transforms.RandomVerticalFlip(0.25),

                tv.transforms.RandomHorizontalFlip(0.25)

        ])

#         self.transforms = tv.transforms.RandomRotation(30)
...
class STN(nn.Module):
    ...
    def __init__(self, channels, feat_shape):
        ...
        for i in range(len(self.channels)-1):
            ...
            feat_shape = feat_shape[0]//2, feat_shape[1]//2

        self.conv = nn.Sequential(*layers)

        self.fc = nn.Sequential(

            nn.Linear(self.channels[-1]*np.prod(feat_shape), 128),
        ...
        self.fc[-1].bias.data = torch.FloatTensor([1,0,0,0,1,0])
...
class VGG(nn.Module):
    ...
    def __init__(self, img_shape, out_dim=3755):
        ...
        self.stn = STN([16, 32, 64], (img_shape[0]//2, img_shape[0]//2))
...
#         feat = F.grid_sample(feat, grid, align_corners=False)
        ...
        feat = affine(theta, feat)

        features = self.conv(feat).view(X.shape[0], -1)
...
train_dataset = MyDataset(images_trn, labels_trn, IMG_SIZE)

train_iter = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

test_dataset = MyDataset(images_tst, labels_tst, IMG_SIZE)
...
net = VGG((IMG_SIZE,IMG_SIZE), 3755).to(device)
checkpoint = torch.load('../input/cchwdb/results (2)/stn.pt')

net.load_state_dict(checkpoint['net'])
loss_trn, acc_trn, loss_tst, acc_tst = [], [], [], []

optimizer = torch.optim.SGD(net.parameters(), lr=1e-3, momentum=0.9)

# stn_optimizer = torch.optim.Adam(stn.parameters())
num_epoches = 20

log_fmt = "Epoch %d Train Loss %f Train Accuracy %f Test Loss %f Test Accuracy %f"
...
for epoch in tqdm(range(1, num_epoches+1)):    
    ...
    if epoch%1==0:
        ...
        msg = log_fmt%(epoch, loss_trn[-1], acc_trn[-1], loss_tst[-1], acc_tst[-1])

#         msg = log_fmt%(epoch, loss_trn[-1], acc_trn[-1], 0, 0)
...
fig, axis = plt.subplots(2, num_show, figsize=(4*num_show, 4*2))

_, theta = net(batch_images)

affined_images = affine(theta, batch_images)

for i in range(10):

    axis[0][i].imshow(batch_images[i,0].data.cpu())

    axis[1][i].imshow(affined_images[i,0].data.cpu())
