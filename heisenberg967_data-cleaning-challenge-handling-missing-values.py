...
c=sf_permits.isnull().sum()
total_absent=c.sum()

(total_absent/total_count) * 100
# look at the # of missing points in the first ten columns
missing_values_count[0:10]
...
x=sf_permits.dropna(axis=1)
print("Columns in original dataset: %d \n" % sf_permits.shape[1])
print("Columns with na's dropped: %d" % x.shape[1])
...
subset_sf_data = sf_permits.loc[:, 'Block':'Lot'].head()
