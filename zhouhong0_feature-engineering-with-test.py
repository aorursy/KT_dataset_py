...
test=pd.read_csv('/kaggle/input/save-my-data/test-flattened.csv')
...
tr = pd.concat([tr, test], axis=0).reset_index()
test.columns
...
train_0 = get_groupby_target(tr,434)

%time

train_1 = get_groupby_target(tr,496)

%time

train_2 = get_groupby_target(tr,558)

%time
use_train = pd.concat([train_0,train_1,train_2])
use_train.to_csv("my_train_third_part.csv", index=False)
