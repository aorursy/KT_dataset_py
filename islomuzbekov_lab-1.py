...
def expand(X):
    ...
    #X_expanded = np.array(X[:, 0], X[:,1], X[:, 0]*X[:, 0], X[:, 1]*X[:, 1], X[:, 0]*X[:, 1], 1)

    X_expanded = np.array([np.array([X[ind, 0], X[ind, 1], X[ind, 0]*X[ind, 0], X[ind, 1]*X[ind,1], X[ind, 0]*X[ind, 1], 1]) for ind in range(0, X.shape[0])])
...
def probability(X, w):
    ...
    prob = np.array([1/(1+np.exp(-np.dot(X[ind,:],w))) 
...
def compute_loss(X, y, w):
    ...
    for ind in range(0, l):
        ...
        p = probability(X[ind:ind+1, :], w)[0]

        losses[ind] = -y[ind]*np.log(p)-(1-y[ind])*np.log(1-p)
...
def compute_grad(X, y, w):
    ...
    grads = [(probability(X[ind:ind+1,:],w) - y[ind])*X[ind:ind+1,:] for ind in range(0,l)]

    grad = 1/l*np.sum(grads, axis=0)
...
for i in range(n_iter):
    ...
    w_new = np.squeeze(np.asarray(w - eta*compute_grad(X_expanded[ind, :], y[ind], w)))
...
for i in range(n_iter):
    ...
    v = np.squeeze(np.asarray(alpha*v + eta*compute_grad(X_expanded[ind, :], y[ind], w)))
...
for i in range(n_iter):
    ...
    G = alpha*G + (1-alpha)*np.square(g2)

    w = np.squeeze(np.asarray(w - eta*g2/np.sqrt(G+eps)))\
