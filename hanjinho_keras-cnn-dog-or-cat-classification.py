...
Image("../input/picture/picture.png")

# Add data를 해서 다운받기 전의 디렉토리
...
earlystop
val_acc # 바로 아래에 code를 보면 monitor에 "val_acc"가 나온다. 그래서 이전에 정의가 되었는지 살펴보았다.

        # 그런데 정의가 되지 않았다. 아래에 있는 'val_acc' code는 어디서 온것인지..?
...
# 이때, ReuceLROnplateau는 수량(??)을 모니터링하며 patience수의 epoch수가 '개선되지 않으면'(오차가 증가하면) 학습률이 감소한다.

# monitor : 모니터링 할 수량 (val_acc : validation accuracy)

# patience : 학습률이 감소한 후 개선되지 않은 epoch 수 ( 학습률이 감소한다 = 오차가 증가한다 = val_loss 증가 는 모두 같은말??)
...
train_generator = train_datagen.flow_from_dataframe( #generator가 뭔지 잘 모르겠네요..
...
print(Y_batch.shape)
epochs=3 if FAST_RUN else 3 # FAST_RUN은 mode 같은데 구글링해도 잘 안나오네요. else 뒤에 숫자를 바꾸는대로 Epoch가 변하는 건 확인했어요
