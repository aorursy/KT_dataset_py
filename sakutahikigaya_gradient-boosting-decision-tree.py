...
class my_gbdt(object):
    ...
    def __init__(self,learning_rate,n_iter, max_depth, min_samples_split):
        ...
        for i in range(self.n_iter):
            ...
            self.tree.append (DecisionTreeRegressor(min_samples_split=self.min_samples_split, max_depth=self.max_depth))
    ...
    def fit(self, X, y):
        ...
        y_pred = self.tree[0].predict(X)



        for i in range(1,n_iter):

            residuals = (y - y_pred) * self.learning_rate

            self.tree[i].fit(X,residuals )

            y_pred += self.tree[i].predict(X)
    ...
    def predict(self, X):
        ...
        for i in range(self.n_iter):
            ...
            pred_proba += self.learning_rate * self.tree[i].predict(X)
...
my_model = my_gbdt(learning_rate,n_iter, max_depth, min_samples_split)
