...
#print(my_df['trending_date'].head())

#my_df.columns

#my_df[['trending_date', 'title', 'channel_title', 'category_id', 'category','publish_date', 'publish_time', 'tags', 'views', 'likes', 'dislikes', 'comment_count', 'thumbnail_link', 'comments_disabled', 'ratings_disabled', 'video_error_or_removed', 'description']]

#my_df[['title', 'channel_title', 'category', 'tags', 'views', 'likes', 'dislikes', 'comment_count', 'thumbnail_link', 'comments_disabled', 'ratings_disabled', 'video_error_or_removed', 'description']].describe()
my_df.info()
my_df['publish_time'] = pd.to_datetime(my_df['publish_time'], format='%Y-%m-%dT%H:%M:%S.%fZ')

print(my_df['publish_time'].head())
...
print(my_df[['publish_date', 'publish_time']].head())
...
print(my_df[['category_id', 'category']].head())
...
plt.show()
category_count = my_df['publish_time'].value_counts() # frequency for each category

#category_count

type(my_df['publish_time'])
ax = category_count[:20].plot.bar()

ax.set_xticklabels(labels=category_count.index, rotation=45, fontsize=10)

plt.show()
str(my_df['publish_time'][1])[:2]
my_df['newpublish_time'] = list(map(lambda x : str(my_df['publish_time'][x])[:2], range(len(my_df['publish_time'])))) 
cat =my_df['newpublish_time'].value_counts()

cat
ax = cat.sort_index().plot.bar()

ax.set_xticklabels(labels=cat.sort_index().index, rotation=45, fontsize=10)
