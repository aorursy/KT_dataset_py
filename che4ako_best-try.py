...
def create_dt(is_train = True, nrows = None, first_day = 1200):
    ...
    #загружаю prices и задаю нужный формат столбцов
    ...
    #загружаю calendar и задаю нужный формат столбцов
...
def create_fea(dt):
    #lags = [7, 14, 21, 28]
    lags = [i*7 for i in range(1, 8+1)]
    #lags = [i for i in range(1, 28)]
    lag_cols = [f"lag_{lag}" for lag in lags ]
    for lag, lag_col in zip(lags, lag_cols):
        #groupby нужен, чтобы не попалось смещённое значение от другого продукта
