...
X, y = make_regression(n_samples = n, n_features = p, n_informative = int(n/10), noise = 0.2)
X = pd.DataFrame(X)
...
#This is varaible selection. the next part is LARS: most important column and do LR with only that one column
coef = np.zeros(p)
bias = y.mean()
bias
part = int(C[:,0][0])
