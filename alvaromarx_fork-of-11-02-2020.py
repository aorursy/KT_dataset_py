...
sound, samplerate = sf.read("/kaggle/input/stairwaytoheaven/Led Zeppelin -  Stairway To Heaven  (LegendadoTraduo PTBR).wav")
...
sound_rs = window(sound, samplerate, 10, 5)
...
for label in range(MODEL_SEL+1):
    ...
    for enum, s in enumerate(sound_rs_ch1[predicted[0] == label]):
        ...
        if not os.path.exists(f"/kaggle/working/label_{label}/sound_{enum}.wav"):
            ...
            sf.write(f"/kaggle/working/label_{label}/sound_{enum}.wav", s, 48000)
...
pylab.imshow(predicted[:,],cmap='jet')