...
data= pd.read_csv('../input/personal/comments.csv')
data.head(10)
data.shape
data
data.info()    ## No Null values in entire dataset
names= list(data['Category'].value_counts().index)
...
explode= [0.1,0.1,0.2,0.2,0.2,0.2,0.3]



plt.figure(figsize= (9,9))

plt.pie(vals, labels= names, shadow= True, startangle= 90, autopct= '%1.1f%%', explode= explode)
...
def sterilization(data):
    ...
    temp= data.encode('ascii', 'ignore')  #deleting non ascii chars
    ...
    return data
data.tail(4)
data['Comments']= data['Comments'].apply(lambda x : sterilization(x))
data.tail(4)
...
val= data[100001:130001]

ref= data[150001:200000]
...
class SentimentDataset(Dataset):
    ...
    def __init__(self, tokenizer, data_dir, type_path, max_len=200):
        ...
        self.question = "Comments"             # Comments column as input

        self.target_column = "Category"        # Category column as output
