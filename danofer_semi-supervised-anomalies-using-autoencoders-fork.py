...
vc
# non_fraud = data[data['Class'] == 0].sample(1000) # ORIG

non_fraud = data[data['Class'] == 0].sample(10000) # New - still  downsampled
...
# tsne_plot(X[0:80,:], Y, "original.png") # error? 

### running on full X is slow with many points
...
encoded = Dense(128, activation='elu', activity_regularizer=regularizers.l2(1e-5))(input_layer)
...
# norm_hid_rep = hidden_representation.predict(x_norm[:3000]) # ORIG
