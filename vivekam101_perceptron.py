...
train_new.drop(['Rating', 'PhoneId', 'Class'], axis=1, inplace=True)
...
def scaled(df):
    ...
    for col in ['RAM', 'Resolution', 'Pixel Density', 'Capacity', 'Weight',
        ...
        return scaled_df
train_new = scaled(train_new)

test_new = scaled(test_new)
for col in train_new.columns:

    if len(train_new[col].unique()) == 1:

        train_new.drop(col,inplace=True,axis=1)

        test_new.drop(col,inplace=True,axis=1)
print(train_new.shape)

print(test_new.shape)
X_train, X_val, Y_train, Y_val = train_test_split(train_new, train_y, 
...
# percept.fit(X_train, Y_train,10000, .01)

percept.fit(train_new, train_y, 10000, .05)
Y_pred_test = percept.predict(X_val)

print(accuracy_score(Y_pred_test, Y_val))
test_pred = percept.predict(test_new)
type(test_pred)
