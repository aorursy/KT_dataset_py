...
def estimate_average_slot_payout(n_runs):
    ...
    0.5
    """
    return sum([play_slot_machine()-1 for num in range(n_runs)]) / n_runs

estimate_average_slot_payout(9999999)
q4.solution()
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    .25
    """
#     print([start_balance + estimate_average_slot_payout(n_spins)*n_spins >= 0 for num in range(n_simulations)])
#     return sum([start_balance + estimate_average_slot_payout(n_spins)*n_spins >= 0 for num in range(n_simulations)]) / n_simulations
    successes = 0
    for _ in range(n_simulations):
        balance = start_balance
        spins_left = n_spins
        while spins_left>0 and balance>=1:
