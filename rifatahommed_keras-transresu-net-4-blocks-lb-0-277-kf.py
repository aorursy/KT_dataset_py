...
for k, (train_index, test_index) in enumerate(kf.split(X_train, Y_train)):
    ...
    model.compile(optimizer=Adam(lr = 0.7e-4), loss=dice_coef_loss, metrics=[iou, dice_coef, 'binary_accuracy'])
...
model = load_model('2_unet_nuclei_seg.hdf5', custom_objects={'dice_coef_loss': dice_coef_loss, 'iou': iou, 'dice_coef': dice_coef})
for i in range(20):
