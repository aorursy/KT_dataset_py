...
import matplotlib.pyplot as plt
plt.plot(accidents_by_hour.f1_,accidents_by_hour.f0_,'bs')
plt.title("Number of Accidents by Hour")

#-----------------------------------

query2 = """SELECT COUNT(registration_state_name), hit_and_run 
            FROM `bigquery-public-data.nhtsa_traffic_fatalities.vehicle_2015`
            GROUP BY hit_and_run
            ORDER BY COUNT(registration_state_name) DESC
        """
accidentsHitAndRun = accidents.query_to_pandas_safe(query2)
print(accidentsHitAndRun)