...
pima.plot(kind='box',sharex =False,sharey=False,layout=(3,3),figsize=(10,8),subplots=True)
...
features[0:5]
pima.head()
X_features = pd.DataFrame(features,columns=["Glucose","Insulin","BMI","Age"])
X_features.head()
Y = pima.iloc[:,8]
Y.head()
...
X=pd.DataFrame(rescaledX,columns=["Glucose","Insulin","BMI","Age"])
X.head()
