...
### Transactions by merkle root 

query_root = """SELECT COUNT(transaction_id) AS transactions,merkle_root
            FROM `bigquery-public-data.bitcoin_blockchain.transactions`
            GROUP BY merkle_root
        """

# note that max_gb_scanned is set to 40, rather than 1
transactions_per_root = bitcoin_blockchain.query_to_pandas_safe(query_root, max_gb_scanned=40)
