...
print(data[['bad_loans', 'safe_loans']].head())
...
cols = ['grade', 'term','home_ownership', 'emp_length']
data_set = dummies(data, columns=cols) #TODO
data_set.head()
...
def best_split(data, features, target):
    ...
    for feature in features:
        ...
        # 计算当前划分之后的分类犯错率
        error = float(left_misses + right_misses) / num_data_points #TODO
...
def best_split_entropy(data, features, target):
    ...
    for feature in features:
        ...
        entropy_split = (len(left_split) * left_entropy + len(right_split) * right_entropy) / num_data_points #TODO
...
class MyDecisionTree(BaseEstimator):
    ...
    def fit(self, X, Y, data_weights = None):
        ...
        target = Y.columns[0]
        self.root_node = self.create_tree(data_set, features, target, 0, max_depth=self.max_depth, min_error=self.min_error)# TODO
    ...
    def create_tree(self, data, features, target, current_depth=0, max_depth=10, min_error=0):
        ...
        # 选出最佳当前划分特征
        #split_feature = best_split(data, remaining_features, target) # TODO   #根据正确率划分
        split_feature = best_split_entropy(data, remaining_features, target) # TODO  # 根据信息增益来划分
