...
class NeuralNetwork():
    ...
    def __init__(self,dimIn,dimH,dimOut):    

        self.dimIn=torch.randn(dimIn,dimH)

        self.dimH=torch.randn(dimH,dimH)

        self.dimOut=torch.randn(dimH,dimOut)

        

        self.bH=torch.randn(dimH)

        self.bOut=torch.randn(dimOut)
    ...
    def fit(self,epoch,train_loader):
        ...
        self.dimIn.requires_grad_(True)

        self.dimH.requires_grad_(True)

        self.dimOut.requires_grad_(True)

        self.bH.requires_grad_(True)

        self.bOut.requires_grad_(True)
        ...
        for i in range(epoch):
            ...
            for x,y in train_loader:
                ...
                if self.dimIn.grad is not None:

                    self.dimIn.grad.zero_()

                if self.dimH.grad is not None:

                    self.dimH.grad.zero_()

                if self.dimOut.grad is not None:

                    self.dimOut.grad.zero_()

                if self.bH.grad is not None:

                    self.bH.grad.zero_()

                if self.bOut.grad is not None:

                    self.bOut.grad.zero_()

                    

                

                hid_out = x.matmul(self.dimIn) + self.bH

                hid_out = hid_out.relu()

                hid_out = hid_out.matmul(self.dimH) + self.bH

                hid_out = hid_out.relu()

                out = hid_out.matmul(self.dimOut) + self.bOut

                out = out.log_softmax(dim = 1)

                

                loss = -(1/len(x))*out[torch.arange(len(x)),y].sum()
                ...
                with torch.no_grad():
                    ...
                    self.dimIn   -= rate * self.dimIn.grad

                    self.dimH    -= rate * self.dimH.grad

                    self.dimOut  -= rate * self.dimOut.grad

                    self.bH      -= rate * self.bH.grad

                    self.bOut    -= rate * self.bOut.grad
        ...
        print('End')            
net=NeuralNetwork(54,500,7)
net.fit(15,train_loader)
