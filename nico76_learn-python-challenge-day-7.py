...
def blackjack_hand_greater_than(hand_1, hand_2):
    ...
    for card in hand_1:
        if card in figures:
            ...
            sum11 = sum11 + int(card)
            sum12 = sum12 + int(card)
    point_hand1 = max(sum11 * (sum11 <= 21), sum12 * (sum12 <= 21))
    sum21 = 0
    sum22 = 0
    for card in hand_2:
        if card in figures:
            ...
            sum21 = sum21 + int(card)
            sum22 = sum22 + int(card)
    point_hand2 = max(sum21 * (sum21 <= 21), sum22 * (sum22 <= 21))
    if (min(sum11, sum12) <= 21) and ((point_hand1 > point_hand2) or (min(sum21, sum22)>21)):
        return True
    else:
        return False
...
def my_agent(wheel):
    ...
    while wheel.num_remaining_spins() > 0:
        proba = conditional_roulette_probs(history)
        valeura = 0
        valeurb = 0
        for el in proba:
            if len(proba[el]) == 1:
                valeura = el
                valeurb = int(list(proba[el].keys())[0])
        if last_number == valeura:
            # Unlucky! Don't bet anything.
            guess = valeurb
        else:
            guess = None
