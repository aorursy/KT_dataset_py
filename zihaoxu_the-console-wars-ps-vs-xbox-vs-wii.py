...
video['User_Score'] = pd.to_numeric(video['User_Score'],errors = "coerce")
...
video.User_Score.isnull().sum()
sns.jointplot(x='Critic_Score',y='User_Score',data=video,kind='hex', size=6,dropna = True)
...
cols = video.select_dtypes(include = numerics).columns
...
video7th.shape
g = sns.FacetGrid(video7th, col="Year_of_Release",col_wrap=3,size =2,hue="Platform")

g.map(sns.kdeplot, "Global_Sales")
