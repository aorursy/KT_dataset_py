...
# test_main.csv has 19795 rows in reality, but we are only loading/previewing the first 100 rows
df1 = pd.read_csv('../input/test_main.csv', delimiter=',', nrows = nRowsRead)
df1.dataframeName = 'test_main.csv'
nRow, nCol = df1.shape
...
nRowsRead = 100 # specify 'None' if want to read whole file
# train_main.csv has 40648 rows in reality, but we are only loading/previewing the first 100 rows
df2 = pd.read_csv('../input/train_main.csv', delimiter=',', nrows = nRowsRead)
df2.dataframeName = 'train_main.csv'
nRow, nCol = df2.shape
print(f'There are {nRow} rows and {nCol} columns')
df2.head(5)
