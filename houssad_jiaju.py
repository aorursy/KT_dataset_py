...
train_infos=[]
test_infos=[]
train_path=['/kaggle/input/cls-jiaju1/cls_data/sofa','/kaggle/input/cls-jiaju1/cls_data/bed']
...
for i in range(2):
    ...
    for name in names[:int(0.7*length)]:
        info={}
        name=str(name)
        image=I.open(name)
...
class myDataset(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.label_map_image:
            negative_label=1-label
            negative_image=self.aug_image(I.open(random.choice(self.label_map_image[negative_label])))
...
def create_dataloader(infos,transform,batch_size,shuffle=False,label_map_image=None,collate_fn=None):
    
    dataset=myDataset(infos,transform,label_map_image)
    if collate_fn:
        dataloader=Data.DataLoader(dataset,batch_size=batch_size,collate_fn=collate_fn,shuffle=shuffle)
    else:
...
class Criterion:
    def __init__(self):
        ...
        self.global_criterion=MultiSimilarityLoss()
        self.local_criterion=TripletLoss()

    def __call__(self,logits,embedding,local_feat,label):
        global_loss = self.global_criterion(embedding, label)
    ...
    for epoch in range(epochs):
        with tqdm(total=len(train_dl)) as pbar:
            ...
            for image, labels in train_dl:
                ...
                logits,global_feat,local_feat = model(image)
                logits=logits.to('cpu')
                global_feat=global_feat.to('cpu')
                local_feat=local_feat.to('cpu')
                loss = criterion( logits,global_feat,local_feat,labels)
                train_loss += loss
                loss.backward()
                optimizer.step()
                pbar.update(1)
...
train_loader=create_dataloader(train_infos,transform_train,batch_size=5,shuffle=True,label_map_image=label_map_images,collate_fn=train_collate)
valid_loader=create_dataloader(test_infos,transform_valid,batch_size=8)
# main(train_loader,valid_loader)
model=myNet()
device=torch.device('cuda:0')
model=model.to(device)
model.load_state_dict(torch.load('/kaggle/input/houssad-checkpoint/base_step.pt')['state'])
model.eval()
