...
#several different optimizers to choose from
...
#A few loss function to choose from
...
cnn_model.fit(
    ...
    batch_size=2000, epochs=1, verbose=1)
cnn_model.summary()
test[1:5]
#use the model to predict the degits of the test data for mlp (784,1)

data = mlp_model.predict_classes(test, verbose=1)
#use the model to predict the degits of the test data using cnn test data (28,28)

cnn_test = test.iloc[:, :].values

cnn_test = cnn_test.reshape(-1,28,28,1)

data = cnn_model.predict_classes(cnn_test, verbose=1)
data[1:10]
