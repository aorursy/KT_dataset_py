...
class BinaryLinearModel(nn.Module):
    def __init__(self, dim_input, n_class):
        super(BinaryLinearModel, self).__init__()
        self.use_cuda = torch.cuda.is_available()
        torch.manual_seed(233)
        self.dim_input = dim_input
        self.linear_model = BinarizeLinear(dim_input, n_class, bias=False)
        self.linear_model.init_params(**dict(is_first_layer=True))
...
patience = 100
optim_params = dict(lr=0.003, weight_decay=1e-8, momentum=0.2)
n_class = 10
n_feature = X_train.shape[1]
verbose_eval = 100
save_path = 'model.pth'
use_cuda = torch.cuda.is_available()
model = BinaryLinearModel(n_feature, n_class)
if use_cuda:
    model = model.cuda()
