...
def menu_is_boring(meals):
    ...
    """
    x = [meals[i]==meals[i+1] for i in range(len(meals)-1)]
    return True if True in x else False
    pass
    ...
    if start_balance>=n_spins:
        ...
        for i in range(n_simulations):
            for j in range(n_spins):
                n = 1
                start_balance= start_balance+play_slot_machine()-1
                if start_balance<=0:
                    n = 0
                    break
                else:
