    ...
    def handValue(hand):
        ...
        for card in hand:
            if card in ['J','Q','K']:
                ...
                card_val = int(card)
            hand_val += card_val
        while ace_count >= 1 and ((21 - hand_val) >= 10):
            hand_val += 10
            ace_count -= 1
        return(hand_val)
    
    hand_1_val = handValue(hand_1)
    hand_2_val = handValue(hand_2)
    # print('hand_1 = {}\thand_2 = {}\n'.format(hand_1_val,hand_2_val))
    if hand_1_val > 21:
        return(False)
    if (hand_1_val < hand_2_val) and (hand_2_val > 21):
        return(True)
    if (hand_1_val > hand_2_val):
        return(True)
    return(False)
