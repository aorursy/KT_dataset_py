...
def count_past_events(series):
    ...
    timestamps=pd.Series(series.index, index=series.values, name="count_7_days").sort_index()
    result=timestamps.rolling('6h').count() - 1
    return result
...
def time_diff(series):
    ...
    time_in_sec=series.diff().dt.total_seconds()
    return time_in_sec
...
def previous_attributions(series):
    ...
    result=series.expanding(2).sum() - series
    return result
