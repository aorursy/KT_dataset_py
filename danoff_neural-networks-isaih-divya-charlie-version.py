...
train.head()
train.shape
#train = train.sample(frac=0.001,replace=False)
train.columns
train['A'].value_counts()
train['A'].isnull().sum()
#train.rename(columns={A:Acceleration})
...
# looking at offensive personnel options



off_personnel = train['OffensePersonnel'].unique()
...
train['OffensePersonnel'].value_counts('OL')
#add code here

def formation_numeric(txt):
    ...
    if "QB" in txt:
        ...
        QB = 1 - int(txt[txt.find("QB")-2:txt.find("QB")-1])

    if "RB" in txt:

        RB = 1 - int(txt[txt.find("RB")-2:txt.find("RB")-1])

    if "WR" in txt:

        WR = 3 - int(txt[txt.find("WR")-2:txt.find("WR")-1])

    if "TE" in txt:

        TE = int(txt[txt.find("TE")-2:txt.find("TE")-1]) - 1

    if "OL" in txt:

        OL = int(txt[txt.find("OL")-2:txt.find("OL")-1]) - 5

    if "DL" in txt:

        DL = int(txt[txt.find("DL")-2:txt.find("DL")-1]) 

    if "LB" in txt:

        LB = int(txt[txt.find("LB")-2:txt.find("LB")-1]) 

    if "DB" in txt:

        DB = -int(txt[txt.find("DB")-2:txt.find("DB")-1]) 

    return QB + RB + WR + TE + OL + DL + LB + DB
train['Formation_Heavy_Score'] = train["OffensePersonnel"].apply(formation_numeric)
train['Formation_Heavy_Score'].value_counts()
formation_numeric("1 TE, 9 OL")
...
def make_pred(df, sample, env, models):
    ...
    #df['Acceleration'] = df['A']

    #df.drop(['A'], axis=1, inplace=True)

    df['Formation_Heavy_Score'] = df["OffensePersonnel"].apply(formation_numeric)
