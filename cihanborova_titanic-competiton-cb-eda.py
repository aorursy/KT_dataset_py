...
def bar_plot(variable):
    ...
    # count umber of caegorical variable(value/sample)
...
def plot_hist(variable):
    ...
    plt.title("{} disturibition with hist ".format(variable))
...
train_df["Title"]=train_df["Title"].replace(["Lady","Don","Rev","Dr","Ms","Major","Sir","Col","Mlle","Jonkheer","Dona"],"other")
...
g.set_xticklabels(["Mr","Mrs","Master","other"])
...
lg_param_grid={"C":np.logspace(-3,3,7),
...
voitingC=VotingClassifier(estimators=[("dt",best_estimators[0]),
...
voitingC = voitingC.fit(x_train,y_train)

print(accuracy_score(voitingC.predict(x_test),y_test))
test_survived = pd.Series(voitingC.predict(test) , name = "Survived").astype(int)
