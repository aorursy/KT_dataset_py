...
# ela = ElasticNet(alpha=0.001, l1_ratio=0.25)
...
# stack_no_cv = StackingRegressor(regressors=[ela, knn, gbr], meta_regressor=reg)
...
# print('Time taken to train stacking model: {:.2f} min'.

#       format((time.time() - time_start) / 60))

# get_score(pipeline_stack, X_val, y_val)
# With CV
...
stack_cv = StackingCVRegressor(regressors=[ela, knn, gbr], meta_regressor=reg, random_state=seed)
...
get_score(pipeline_stack_cv, X_val, y_val)
preds_test = pipeline_stack_cv.predict(X_test)
