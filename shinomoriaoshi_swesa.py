...
def gradient_theta(X, W_new, label, C, theta, lamb = 1):
    ...
    sigmoid = 1/(1 + torch.exp(label * (torch.sparse.mm(X, W_new) @ theta).view(K,1)))

    

    first_term = W_new.T @ (((C * label * sigmoid) * X.to_dense()).mean(dim = 0).view(V,1))
    ...
    return(-first_term.flatten() + lamb * regu_term.flatten())   # '''
# The gradient of the loss function w.r.t. Gamma

def gradient_W(X, W, W_new, label, C, theta, lamb = 0.01):
    ...
    grad_W = - (((C * label * sigmoid) * X.to_dense()).mean(dim = 0).view(V,1)) @ theta.T

    

    return(grad_W)   #'''
def loss(X, W, W_new, label, C, theta, lamb = 1):
    ...
    cond_prob_label = torch.log(sigmoid)

    

    first_term = torch.mean(C * cond_prob_label)

    

    regu_term = torch.sum(theta**2)

    

    return(-first_term + lamb * regu_term)  #  + gam * regu_term'''
class word2vec_sentiment:

    def __init__(self, alpha = 0.05, tol = 1e-4, lamb = 10, tau = 50,
    ...
    def estimate(self, X, W, label):
        ...
        C = np.zeros((len(np.unique(label)),K))

        for i in range(C.shape[0]):

            C[i] = (1 + np.sign(float(i) - 0.5) * label)/2

        C = (C.T @ C.sum(axis = 1).reshape(len(np.unique(label)),1)).sum(axis = 1)

        C = torch.from_numpy(C/K).cuda().float()
        ...
        while it < self.num_iter:
            ...
            grad_theta = gradient_theta(X, W_tilde_old, label, C, theta_old, lamb = self.lamb)

            

            # Compute the gradient w.r.t. Gamma

            grad_W = gradient_W(X, W, W_tilde_old, label, C, theta_old)
            ...
            L[it] = loss(X, W, W_tilde_new, label, C, theta_old, lamb = self.lamb)
    ...
    def fit_cross_validation(self, X, W, label, tuning = 10, nfold = 5, type_classifier = 'logistic', type_weight = 'raw'):
        ...
        for i, lamb in enumerate(list(self.grid)):
            ...
            for doc_train_index, doc_test_index in kf.split(np.arange(0, K)):
                ...
                _, _, _, cm_test = predict_document_level(X.tocsr()[doc_train_index], np.sign(label[doc_train_index] - 2.5), 

                                                         X.tocsr()[doc_test_index], np.sign(label[doc_test_index] - 2.5), 
...
pred_label_train, cm_train, pred_label_test, cm_test = predict_document_level(doc_term_train, np.sign(score_train - 2.5), 

                                                                              doc_term_test, np.sign(score_test - 2.5), 
...
hyper_lamb = np.array([1e-3, 1e-2, 1e-1, 1, 10])
...
cv = validation(num_iter = 5000, tol = 1e-5, grid = hyper_lamb)
...
obj = word2vec_sentiment(num_iter = 5000, tol = 1e-5, type_learn = 'diminish', lamb = lamb)

t, W_new, l = obj.estimate(doc_term_train, word_vector_train.values, np.sign(score_train - 2.5))
...
sorted_words_sent
sorted_words_sent[::-1]
np.save('W_new_swesa_yelp.npy', new_word2vec)

np.save('Validation_acc.npy', acc)