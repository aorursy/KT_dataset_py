...
def run_models(X_train, y_train, X_test, y_test, model_type = 'Non-balanced'):
    ...
    clfs = {'GradientBoosting': GradientBoostingClassifier(verbose=1,max_depth= 8, n_estimators=150, max_features = 0.3),
            ...
            #'RandomForestClassifier': RandomForestClassifier(verbose=1,n_estimators=250,criterion='entropy') #10
    ...
    conf_matrix = dict(); feat_imp = dict()
    ...
    return models_report, conf_matrix, feat_imp
...
sm = SMOTENC(categorical_features=[i for i in range(14,37)], random_state=123, sampling_strategy=.6)

%time X_trainres, y_trainres = sm.fit_resample(X_train_sc, y_train)
pd.DataFrame(y_trainres,columns=['target']).target.value_counts(normalize=True)
%time models_report_bal, conf_matrix_bal, feat_imp_bal = run_models(X_trainres, y_trainres, X_test_sc, y_test, model_type = 'Balanced')
models_report_bal
conf_matrix_bal['LogisticRegression']
conf_matrix_bal['GradientBoosting']
conf_matrix_bal['RandomForestClassifier']
importance = feat_imp_bal['LogisticRegression']

for i,v in zip(l,importance):
...
plt.show()
importance = feat_imp_bal['GradientBoosting']
...
plt.show()
importance = feat_imp_bal['RandomForestClassifier']
...
print("Target",result[0],"\nProbability",prob)
loaded_model = pickle.load(open('GradientBoosting_Balanced.sav', 'rb'))
