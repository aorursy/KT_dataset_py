...
rfc.fit(train_new, y_train)

y_pred_rfc = pd.DataFrame(data = rfc.predict(test_new), columns = ['Survived'])



#%% fitting model

new_data = pd.concat([test_copy['PassengerId'],y_pred_rfc], axis = 1)

new_2 = pd.DataFrame(data = new_data, columns = ['PassengerId', 'Survived'])
new_2.to_csv('my_submission.csv', index=False)
