...
def load_data():
  ...
  return np.load('../input/prostate/X_train(Clahe).npy'), np.load('../input/prostate/y_train(Clahe).npy')
...
def keras_fit_generator(img_rows=256, img_cols=256, n_imgs=15 * 10 ** 3, batch_size=32, epochs = 50, regenerate=True):
    ...
    #model.load_weights('best_resnet18_model.h5')
...
swa = SWA('best_xception_model_weights.h5',epochs - 3)
