...
%%time
df = pd.read_csv('../input/disease/heart.csv')
df.head()
df.isna().sum()
...
grid = GridSearchCV(estimator=model, param_grid=parameters, cv=5)
grid.fit(X_train, y_train)
scores1 = np.around(np.mean(cross_val_score(grid, X_test, y_test, cv=5, scoring='roc_auc')), decimals=4)
print('Score: {}'.format(scores1))
predictions = cross_val_predict(model, X_test, y_test, cv=5)
print(classification_report(y_test, predictions))