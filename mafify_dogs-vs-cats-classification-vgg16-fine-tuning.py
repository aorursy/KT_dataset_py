...
!ls ./
file = './train/dog.5994.jpg'

'./train_preprocessed'+file.split('n')[1]
"""Preprocessing
"""
...
for file in glob.glob('./train/*.jpg'):
    ...
    if image.std() > 45.0:
        image = np.array(image * 0.85, dtype=np.uint8)
    cv2.imwrite('./train_preprocessed'+file.split('n')[1], image)
len(os.listdir('./train_preprocessed'))
