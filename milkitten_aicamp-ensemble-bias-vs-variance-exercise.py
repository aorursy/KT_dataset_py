...
def f(X):
    ...
    return np.sin(X)
x_axis = np.linspace(0,2*np.pi, 100)#TODO
...
for d in range(max_degree):
    ...
    for k in range(Num_datasets):
        ...
        plt.plot(x_axis,prediction_curves[:, k, d], color='pink', alpha=0.5)
    ...
    plt.plot(x_axis,prediction_curves[:, :, d].mean(axis=1))
...
for d in range(max_degree):
    ...
    mean_X = train_predictions[:, :, d].mean(axis=1)

    diff = mean_X-trueY_train

    squared_bias[d] = np.dot(diff,diff)/trainN                   
...
for d in range(max_degree):
    ...
    for i in range(trainN):
        ...
        difference = train_predictions[i,:,d] - train_predictions[i,:,d].mean()
