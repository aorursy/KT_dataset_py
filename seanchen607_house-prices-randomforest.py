...
print("打印RandomForest模型的得分模型的得分: {:.4f} ({:.4f})\n".format(score.mean(), score.std()), datetime.now(), )
...
print(datetime.now(), '对RandomForest模型进行参数训练')

RandomForest_model_full_data = rf.fit(X, y)
...
def blend_models_predict(X):
    ...
    return ((1* RandomForest_model_full_data.predict(X)))
