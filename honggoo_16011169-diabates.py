...
train_x = train_data.loc[:,train_data.keys()[1:-1]]

train_y = train_data.loc[:,train_data.keys()[-1]]
knn = KNeighborsClassifier(n_neighbors=20,p=2)
knn.fit(train_x,train_y)
test_data
test_x = test_data.loc[:,test_data.keys()[1:-1]]
...
submission_form = pd.DataFrame(result,columns = ["ID","Label"])
