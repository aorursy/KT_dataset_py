...
#Source code for fastai.structured module from v01
...
# To delete after feature engineering

train.describe().transpose()
train.isna().sum().sort_values(ascending = False).head(3)
#Check the row with the null value

train[train.winPlacePerc.isna()]
#Delete the row with null value
...
train.killsAndBoostsHeals.fillna(0, inplace = True)
outliers = [3673965, 2601666, 672993, 156599]

train.iloc[outliers]
#Players who acquired atleast one weapon and killed atleast one enemy without any move

train[features][(train.kills > 0) & (train.totalDistance == 0) & (train.weaponsAcquired > 0)]
train = train.drop(outliers)

train.drop(train[features][(train.kills > 0) & (train.totalDistance == 0) & (train.weaponsAcquired > 0)].index, inplace = True)
train.shape
#Turn matchType into categorical types
...
df = train.copy().drop(['winPlacePerc', 'Id'], axis = 1)
df.info()
#train.corr()
...
#plt.show()
#numeric variables having high correlation with winPlacePerc

corr_df['winPlacePerc'].sort_values(ascending = False)#.head(20)
...
test.drop(columns=['groupId', 'matchId', 'matchType'], inplace=True)
#Create new features
...
test.killsAndBoostsHeals.fillna(0, inplace = True)
test.fillna(0, inplace=True)

test_final = test.copy().drop('Id', axis = 1)


test_final.head()

predictions = np.clip(a = m1.predict(test_final), a_min = 0.0, a_max = 1.0)
