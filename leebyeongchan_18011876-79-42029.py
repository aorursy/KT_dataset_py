...
batch_size = 100
drop_prob = 0.3
xy_train = pd.read_csv('kbo_train.csv', header = None, skiprows=1, usecols=range(0, 9))
x_data = xy_train.loc[: , 0:7]
y_data = xy_train.loc[: , [8]]
...
xy_test = pd.read_csv('kbo_test.csv', header = None, skiprows=1, usecols = range(0, 8))
x_data = xy_test.loc[:, :]
x_data = np.array(x_data)
...
!kaggle competitions submit -c 18011854kbopredicton -f submit.csv -m "18011876 이병찬"