...
def word_search(doc_list, keyword):
    ...
    for i, document in enumerate(doc_list):
        document_split_list = document.split()
        cleaned_keyword = [document_split_str.strip('.,').lower() for document_split_str in document_split_list]        
        if keyword.lower() in cleaned_keyword:
            result.append(i)
    return result
...
def multi_word_search(doc_list, keywords):
    ...
    for keyword in keywords:
        result_dic[keyword] = word_search(doc_list, keyword)        
    return result_dic
...
def diamond(height):
    ...
    for i in range(height):
        ...
        if i < rad:
            ...
            nchars = height - i
        left = (fwd * nchars).rjust(rad)
        right = (bwd * nchars).ljust(rad)
        diamond_str += left + right + '\n'
    return diamond_str 
q4.check()
d4 = """ /\\ 
//\\\\
