...
PROBA_SELL = 0.8 # deprecated(weets, 1911228)
    ...
    if len(clean_matches):
        ...
        def _gen_market_pricer():
            ...
            if len(clean_matches) >= 10: # if sufficient transactions occur, use linear model
                ...
                return lambda am: max(0,lm.predict(np.array(am).reshape(1,-1)).item())
        ...
        houses['market_price'] = houses['amenities'].apply(lambda am: market_pricer(am)) # update(weets,191128)
...
# match_df[~match_df['highest_bid_value'].isna()].plot(kind='scatter',x='amenities',y='highest_bid_value')
# houses[~houses['market_price'].isna()].plot(kind='scatter',x='amenities',y='market_price')
...
fig, ax = plt.subplots(nrows=3, ncols=5,figsize=(12,7),
                      ...
                      gridspec_kw={"height_ratios":[1, 0.05, 2.0]})

plt.subplots_adjust(wspace=0.4)

im0 = ax[0,0].imshow(np.random.randn(10,10), vmin=0, vmax=2)

im1 = ax[0,1].imshow(np.random.randn(10,10), 

                     norm=colors.LogNorm(vmin=1, vmax=1000))

im2 = ax[0,2].imshow(np.random.randn(10,10), 
...
# todo - color people differently

scat_income_age = ax[2,2].scatter([], [], s=20, label="indvidual people")
...
ax[2,3].add_patch(hist_patch)

ax[2,3].set_ylim((0, 20))

ax[2,3].set_xlim((0, 600))

ax[2,3].set_title("histogram of wealth")



line_house_0, = ax[2,4].plot([], lw=3, label="empty")

line_house_1, = ax[2,4].plot([], lw=3, label="occupied")

line_house_2, = ax[2,4].plot([], lw=3, label="selling")
...
def update_plot():
    ...
    for index in list(houses.index):
        ...
        a1[index[0], index[1]] = utility_general(houses.loc[index])
