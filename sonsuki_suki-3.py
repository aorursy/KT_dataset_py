...
print("총 생존자 수: " + str((total_survived_females + total_survived_males))) 
...
print("총 생존자 수: " + str(total_survived_class)) 
...
for name in training["Name"]: training["Title"] = training["Name"].str.extract("([A-Za-z]+)\.",expand=True) 

for name in testing["Name"]: testing["Title"] = testing["Name"].str.extract("([A-Za-z]+)\.",expand=True)
training.head() 

#Title 열이 추가됨.
titles = set(training["Title"]) 

# SET화해서 중복 데이터를 제거해줍니다.
...
for i in titles: frequency_titles.append(title_list.count(i))

print(frequency_titles)

# 각 이름 제목 별로 갯수를 나타냅니다.
titles = list(titles) 

title_dataframe = pd.DataFrame({ "Titles" : titles, "Frequency" : frequency_titles }) 

print(title_dataframe)
title_replacements = {"Mlle": "Other", "Major": "Other", "Col": "Other", "Sir": "Other", "Don": "Other", "Mme": "Other", "Jonkheer": "Other", "Lady": "Other", "Capt": "Other", "Countess": "Other", "Ms": "Other", "Dona": "Other"} 
...
# Scaler가 배열을 사용하기 때문에 데이터를 다시 변환해야 합니다.
...
# MinMaxScaler를 사용하여 비교 성능을 확인할 수 있으며 StandardScaler를 MinMaxScale로 교체하십시오.
training.head()
testing.head()
...
# 유효성 검사 데이터세트를 생성하려면 다음과 같이 하세요.



X_training, X_valid, y_training, y_valid = train_test_split(X_train, y_train, test_size=0.2, random_state=0) 

# X_valid 및 y_valid는 검증 세트
svc_clf = SVC() 
...
model_performance = pd.DataFrame({ "Model": ["SVC", "Linear SVC", "Random Forest", "Logistic Regression", "K Nearest Neighbors", "Gaussian Naive Bayes", "Decision Tree", "XGBClassifier"], "Accuracy": [acc_svc, acc_linsvc, acc_rf, acc_logreg, acc_knn, acc_gnb, acc_dt, acc_xg] }) 
