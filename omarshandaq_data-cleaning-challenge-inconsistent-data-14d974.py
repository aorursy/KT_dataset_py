...
#i've put the unique values in "province_unique the i sorted it 
province_unique = suicide_attacks['Province'].unique()
province_unique.sort()

#i've converted the Province to lowercase then i removed trailing, there was "fata" and "Fata" now there is only fata when i turned it to lawercase
...
def replace_matches_in_column(df, column, string_to_match, min_ratio = 95):
    ...
    print("All done! we converted 'kuram agency' and 'kurram agency' to make one as 'kuram agency'")
replace_matches_in_column(df=suicide_attacks, column='City', string_to_match="kuram agency")
#now we can test it, and lets see if it worked
cities = suicide_attacks['City'].unique()

cities.sort()
cities
# you will see that "kurram agency" is gone and there are only "kuram agency" as we wanted