...
query_vector = model.embed_sentence(query)
test_query(cl_df)
query="Real-time tracking of whole genomes and a mechanism for coordinating the rapid dissemination of that information"
from pyserini.search import pysearch
searcher = pysearch.SimpleSearcher(luceneDir)
query_result = show_query_results(query, searcher, top_k=10)
files_list=query_id(query_result)
...
query="Access to geographic and temporal diverse sample sets to understand geographic distribution and genomic differences"
from pyserini.search import pysearch
searcher = pysearch.SimpleSearcher(luceneDir)
query_result = show_query_results(query, searcher, top_k=10)
files_list=query_id(query_result)
