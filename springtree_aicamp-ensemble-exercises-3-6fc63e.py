...
for feature in features:
    ...
    # 将特征的直方图画出来
    # TODO
    plt.hist(data[feature][data[target] == -1], bins=bins, color='lightgreen', label='B-healthy', alpha=1)
    plt.hist(data[feature][data[target] == 1], bins=bins, color='grey', label='M-bad', alpha=.5)
...
print("AdaBoost Performance: %f" % (cross_val_score(tree_model, trainX, trainY, cv=8).mean()))
logistic_model = LogisticRegression(solver='liblinear')
# TODO
