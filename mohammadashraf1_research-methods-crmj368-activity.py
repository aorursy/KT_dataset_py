...
# Step 2: Let's take a look at what the data in our dataset looks like
print(dfGSS.head(10))
# Step 3: List column names
dfGSS.columns.values
# Step 4: Create a list with the names of the variables we are interested in getting descriptive statistics for. 
...
lVar=['race', 'age', 'owngun', 'gunlaw', 'polviews', 'income', 'fear']
# Step 5: Print frequency distribution. 
...
for var in lVar:
    ...
    print(var, '\n', dfFreq, '\n')
# Step 6: Plot a histogram for numeric (interval and ratio level) variables.
...
# Step 7: Plot bar charts for categorical (nominal, binary, ordinal) variables.
my_variable = 'owngun' # Replace VAR with the name of a categorical variable.
...
plt.xticks(locs, my_xticks, rotation=90)
plt.show()
my_variable = 'polviews' # Replace VAR with the name of a categorical variable.
count=dfGSS[my_variable].value_counts()
sns.barplot(count.index, count.values, alpha=0.9, color='steelblue')
plt.title('Bar chart of '+my_variable)
plt.xlabel(my_variable)
...
plt.xticks(locs, my_xticks, rotation=90)
plt.show()
# Step 8: Print descriptive statistics
# Replace the label VARIABLE_LIST with the name of the list from step 2.
...
# Step 11: Run a correlation with p-value between two variables 
# Replace INDEPENDENT_VARIABLE and DEPENDENT_VARIABLE with the names of 
# your independent and dependent variables from the worksheet.
varA=dfGSS.loc[:,'income']
varB=dfGSS.loc[:,'fear']
