...
df=pd.read_csv("../input/ghana-covid19-dataset/Ghana_Covid19_DailyActive.csv")
...
fig.show()
px.histogram(df, x='death', color='date')
...
display(Markdown("Distribution of number of Deaths"))
...
fig.show()
fig = px.bar(df[['date', 'confirmed']].sort_values('confirmed', ascending=False), 
...
py.iplot(fig)
_ = pd.pivot_table(df, values='confirmed', index='date').plot(style='-o', title="Covid-19 Ghana confirmed cases")
#codes from Rodrigo Lima  @rodrigolima82

from IPython.display import Image

Image(url = 'https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcRZkNn2Fth3-GXaY2uOKj-6XP9bidHkTauCrLCEKWssCzkCnDdR&usqp=CAU',width=400,height=400)