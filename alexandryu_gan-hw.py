...
from keras.layers import Input, Dense, Conv2D, Reshape, Flatten, concatenate, UpSampling2D, MaxPooling2D, Dropout
...
seed_everything(43)
ROOT = '../input/ilovekantor-dataset/' # директория, в которую подгружаются файлы
...
if BadMemory:
    ...
    x = Dense(2048, activation='relu')(seed)
...
for k in range(it):  
    ...
    h = gan.fit(train, zeros, epochs = ep, batch_size=128, callbacks=[annealer], verbose=0)
...
images_path = [user_images_unzipped_path,'/kaggle/input/ilovekantor-dataset/all-dogs/all-dogs/']

public_path = '/kaggle/input/ilovekantor-dataset/classify_image_graph_def.pb'
...
! rm -r ../tmp #очистим временную диреткорию