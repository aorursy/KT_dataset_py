...
def one_hot_encode(input):
    ...
    OH_cols = pd.DataFrame(OH_encoder.fit_transform(input[low_cardinality_cols]))

    OH_cols.index = input.index



    num = input.drop(object_cols, axis=1)

    return pd.concat([num, OH_cols], axis=1)
