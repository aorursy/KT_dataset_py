...
def build_als(product_user_matrix, **kwargs):
    ...
    print(f'Building ALS model with alpha: {kwargs["alpha"]}... ', end='')
    ...
    model.fit(confidence_matrix(product_user_matrix, kwargs["alpha"]))
...
als_params = {'alpha': 15, 'random_state': 0}

als_params['path'] = f'imf_benchmark_als_{als_params["alpha"]}.pkl'



REBUILD_MODEL = True

if REBUILD_MODEL or not Path(als_params['path']).exists():

    build_als(product_user_matrix_full, **als_params)

with open(als_params['path'], 'rb') as f:
...
def assign_recommendations(row):
    ...
    return [(pid, score, rank) for rank, (pid, score) in enumerate(als_model.recommend(u_dict[row.user_id], sparse_user_product_matrix, N=N_REC, recalculate_user=True), start=1)]
...
results_df['products'] = results_df.progress_apply(assign_recommendations, axis=1)
...
hasil = pd.merge(results_df_new, relation_df, how='outer', left_on=['user_id','product_id'], right_on=['user_id','product_id'])
