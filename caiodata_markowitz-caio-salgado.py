...
df_brasil.tail()
# y = df_brasil['2010-01-01':'2010-04-01'] ## período
def mark_mont(y):
    #import ipdb; ipdb.set_trace() # debugging starts here
    base = y
    ret_10_1 = np.log(base/base.shift(1))
    indice_sharp = -10
    #ind_pesos = np.zeros(len(returns.columns))
    ...
    # formula nova
    for i in range(3000):
        # pesos = np.random.uniform(0,100,len(returns.columns))
...
# Aqui vc pode alterar a data como preferir, lembrando da limitação de dados da base
retornos, riscos, ind_sharp, peso, retorno, risco = mark_mont(df_brasil['2010-10-01':'2011-01-01'])
