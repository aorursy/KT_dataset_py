...
# First 5 rows of the original training data

df.head()
# First 5 rows of the original training data + synthetic records
...
sns.countplot(x='sex', data=df, palette="mako_r").set_title("Training distribution by Gender (Original)")
...
print("Percentage of Male Patients in original training dataset: {:.2f}%".format((countMale / (len(df.sex))*100)))
sns.countplot(x='sex', data=dfs.append(df, sort=False).append(dft, sort=False), palette="mako_r").set_title("Training distribution by Gender (Original + Synthetic)")
...
countMale = len(dfs[dfs.sex == 1])
total_set = dfs.append(df, sort=False).append(dft, sort=False)
...
print("Percentage of Male Patient in synthetic + training dataset: {:.2f}%".format((countMale / (len(total_set.sex))*100)))
...
dfs = dfs.drop(columns = ['cp', 'thal', 'slope'])
y = df.target.values
...
x_data_syn = dfs.drop(['target'], axis = 1)
# Normalize
...
x_syn = (x_data_syn - np.min(x_data_syn)) / (np.max(x_data_syn) - np.min(x_data_syn)).values
