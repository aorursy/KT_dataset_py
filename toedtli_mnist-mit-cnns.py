...
reduce_lr = ReduceLROnPlateau(monitor='val_accuracy', patience=2, verbose=2, factor=0.5, min_lr=0.0000001)
...
for i in range(10):
    ...
    plt.xlabel("true is {}, predicted as {}".format(np.argmax(y_test[err_index]), y_pred[err_index]))
test = test / 255
