...
EfficientNet_model.add(EfficientNetB4(weights = 'imagenet', include_top=False, pooling='avg' ))
...
# Evaluate the model
scores = EfficientNet_model.evaluate(test_generator)
print("\n%s: %.2f%%" % (EfficientNet_model.metrics_names[1], scores[1]*100))

end_time = datetime.datetime.now()
elapsed_time = end_time - start_time

print('Start Time  = {} ; End Time  = {} ; Total Time = {}'.format(start_time,end_time,elapsed_time))
EfficientNet_model.save('EfficientNetB4_model.h5')

print("\n%s: %.2f%%" % (EfficientNet_model.metrics_names[0], scores[0]*100))
print("\n%s: %.2f%%" % (EfficientNet_model.metrics_names[1], scores[1]*100))
print("\n%s: %.2f%%" % (EfficientNet_model.metrics_names[2], scores[2]*100))
print("\n%s: %.2f%%" % (EfficientNet_model.metrics_names[3], scores[3]*100))
print("\n%s: %.2f%%" % (EfficientNet_model.metrics_names[4], scores[4]*100))
...
preds = EfficientNet_model.predict_generator(generator=test_generator) # get proba predictions
