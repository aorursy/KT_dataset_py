...
class my_gbdt(object):
    ...
    def __init__(self):
        ...
        for i in range(self.n_estimators):
            ...
            self.trees.append(DecisionTreeRegressor(min_samples_split=2, max_depth=2))#建立n_estimators个树（将类直接append）
    ...
    def fit(self, X, y):
        ...
        self.trees[0].fit(X, y)#第一个树用来对原数据进行拟合

        y_pred = self.trees[0].predict(X)

        for i in range(1, self.n_estimators):

            p = 1 / (1 + np.exp(-y_pred))#预测值/下一轮新预测值

            loss = y - p #残差=实际值-预测值

            self.trees[i].fit(X, loss)#对残差拟合一棵树

            y_pred += np.dot(self.learning_rate,self.trees[i].predict(X))#上一轮预测值与这一轮残差的预测值求和
    ...
    def predict(self, X):
        ...
        y_pred = self.trees[0].predict(X)#第一棵树拟合出的预测值（Q为什么这里的预测值全部都是单一的数？A因为在init里append了self.tree导致所有的树都是同一棵树）

        for i in range(1, self.n_estimators):

            y_pred += np.dot(self.learning_rate, self.trees[i].predict(X))#最初的预测值累加上之后残差的预测值

        y_pred = 1 / (1 + np.exp(-y_pred))#转换为概率

        y_pred = np.argmax(y_pred, axis=1)#转化为0,1
