...
babyGirls = girls.values()  #defines values and babyGirls

babyBoys = boys.values()   #defines values and babyBoys



totalBabies = sum(babyGirls)+ sum(babyBoys)    #makes the variable 'totalBabies' equal to the sum of boy and girl values



print("There are %d babies in the dataset." %totalBabies)  #prints result

babyBoys = set(boys.keys()) #This was from the internet, the set function turns the dictionary keys to a set I can compare (https://stackoverflow.com/questions/6821156/how-to-find-range-overlap-in-python)



babyGirls = set(girls.keys()) #This was from the internet, the set function turns the dictionary keys to a set I can compare



sameNames = babyBoys.intersection(babyGirls) #This checks for an intersection between the two sets



print("There are %d boy names that are also girl names." %len(sameNames)) #prints where the intersection occurs
typedIn = input("Type Name: ")   #Allows user to input name



typedName = typedIn.lower()   #makes sure all inputed names read as lowercase





for listName in boys.keys():        #iterates through key in "boys" dictionary and finds the inputted name

    if typedName == listName:

       boyNum = boys[listName]

        

for listName in girls.keys():

    if typedName == listName:

        girlNum = girls[listName]    #iterates through key in "girls" dictionary and finds the inputted name



totalBabies2 = boyNum + girlNum    #Makes new variable thats equal to girls + boys found in the dictionary



print("There are a total of", totalBabies2 )
