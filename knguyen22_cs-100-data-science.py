...
train_data[['Survived', 'Name']].groupby(['Name']).mean()
...
people_survived_count = 0

for idx, row in train_data.iterrows():

    if row['Sex']=="female" and row['Age']>=3 :

        people_count += 1

        if row['Survived'] == 1:

            people_survived_count += 1

people_survived_count / people_count
predictions = []
