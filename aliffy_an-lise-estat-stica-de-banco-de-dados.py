...
df_dataset = pd.read_csv('../input/iris.csv', sep=',', index_col=None) 
...
# cria o histograma
n, bins, patches = plt.hist(df_dataset['comprimento_petala'].values,bins=10, color='yellow', edgecolor='darkred', linewidth=2.5)
...
plt.show()
# importa o arquivo e guarda em um dataframe do Pandas
df_dataset2 = pd.read_csv( '../input/data2.csv', sep=',', index_col=None) 
display(df_dataset2.head(7))
df_dataset2.describe()
n, bins, patches = plt.hist(df_dataset2['atributo_d'].values,bins=10, color='pink', edgecolor='darkred', linewidth=2.5)
...
def covariancia(atributo1, atributo2):
    ...
    mean1 = atributo1.mean()
    mean2 = atributo2.mean()
    
    for i in range(0, (n-1)):
        cov = cov + (atributo1[i]-mean1)*(atributo2[i]-mean2)
...
def correlacao(atributo1, atributo2):
    ...
    std1 = atributo1.std(ddof=1)
    std2 = atributo2.std(ddof=1)
    
    corr = covariancia(atributo1, atributo2) / (std1*std2)
