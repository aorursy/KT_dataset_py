...
def plot_decision_boundary(X, model):
    ...
    xx, yy = np.meshgrid(np.arange(x_min, x_max, h),
                       np.arange(y_min, y_max, h)) #创建2D的网格
...
class BaggedTreeClassifier:
    ...
    def fit(self, X, Y):
        ...
        # 创建深度为5的决策树
            ...
            #replace=True---有放回采样 #size可以随意取 #N, size=N 第一个N是总样本数
            #这里是一个一个取，每取一次就把取的那个放回去，所以有可能是重复的
    ...
    def predict(self,X):
        # 二分类预测 #要么是0, 要么是1
        #bagging方法：每个模型（树）给出判断，最后就是把每棵树的结果加起来再取平均，self.M是定义了树的个数
        predictions = np.zeros(len(X))
        for model in self.models:
            predictions += model.predict(X)
        return np.round(predictions / self.M) #取平均，拿到二分类的label
    
    def score(self,X,Y):
        result = self.predict(X)
        return np.mean(result == Y) #和真实的label比较，求均值
...
plot_decision_boundary(X, baggedTree)
plt.show()
#甜甜圈数据是linearly unseparable的，所以random forest肯定结果是更好的，一般的决策树也会比LR好一些
