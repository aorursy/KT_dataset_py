...
data=pd.read_csv("../input/data-set2/output.csv")

#data=pd.read_csv("../input/data-set2/output.csv")

# uncomment above line for see prediction on name + phone number as sensitive information which gives very high accuracy.
data.head(10)
data.describe()
import re
...
model.fit(x_train, y_train, validation_split=0.1,
          ...
          nb_epoch=26, batch_size=128)
output_test = model.predict(x_test)

print("test auc:", roc_auc_score(y_test,output_test[:,1]))
precision1, recall1, thresholds1=precision_recall_curve(y_test,output_test[:,1])
pyplot.plot(precision1,recall1 , marker='.')
...
pyplot.show()
fpr1, tpr1, thresholds1 = roc_curve(y_test,output_test[:,1])
