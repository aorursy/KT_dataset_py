...
# chosoe the last 5000 (out of 17000) examples for validation
...
def train_nn_regression_model(my_optimizer, steps, batch_size, hidden_units, training_examples, training_targets, validation_examples, validation_targets):
    ...
    In addion to training, this function also prints training progress information as well as a plot of the training and validation loss over time
    
    Args:
        my_optimizer: An instance of `tf.train.Optimizer`, the optimizer to use
        steps: A non-zero `int`, the total number of training steps. A training step consists of a forward and a backward pass using a single batch
    ...
    Returns:
        A tuple `(estimator, training_losses, validation_losses)`:
            estimator: the trained `DNNRegressor` objecct
