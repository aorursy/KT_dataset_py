...
train_df['infold_pred'] = predictions['infold_pred']

train_df['outfold_pred'] = predictions['outfold_pred']
remove_rows_based_on_cv = True
...
score_to_take_best_order = False
if remove_rows_based_on_cv:

    train_df = train_df[((train_df['target']==0)&(train_df['outfold_pred']<0.3))|((train_df['target']==1)&(train_df['outfold_pred']>0.7))]
...
if model_name == 'roberta':
    ...
    tokenizer = AlbertTokenizer.from_pretrained(model_shortcut_name, do_lower_case=do_lower_case)
...
from transformers import BertForSequenceClassification, AdamW, BertConfig,RobertaForSequenceClassification, AlbertForSequenceClassification



def start_model(model_name,model_shortcut_name):
    ...
    if model_name == 'bert':
        ...
        model = BertForSequenceClassification.from_pretrained(
            ...
            model_shortcut_name, # Use the 12-layer BERT model, with an uncased vocab.
...
def get_optimizer_and_scheduler(learning_rate,train_dataloader,epochs):
...
if run_test_to_get_best_epoch:
    ...
    for epoch_i in range(0, epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            labels_list.append(b_labels.to('cpu').numpy())
        ...
        training_accuracy = flat_accuracy(np.concatenate(logits_list),np.concatenate(labels_list),)

        training_f1_score = flat_f1_score(np.concatenate(logits_list),np.concatenate(labels_list),)
        ...
        testing_accuracy = flat_accuracy(np.concatenate(logits_list),np.concatenate(labels_list),)

        testing_f1_score = flat_f1_score(np.concatenate(logits_list),np.concatenate(labels_list),)
        ...
        training_stats.append(

            {
                ...
                'val_accuracy':testing_accuracy,

                'val_f1_score':testing_f1_score,
...
optimizer,scheduler = get_optimizer_and_scheduler(learning_rate,full_train_dataloader,best_epoch)
import random
...
for epoch_i in range(0, best_epoch):
    ...
    for step, batch in enumerate(full_train_dataloader):
...
submission.to_csv('torch_prediction.csv', index=False)
