...
import pickle
import json
metaDataPath = "/kaggle/input/CORD-19-research-challenge/metadata.csv"
...
metaDataPath = metaDataPath = "metadata_clean.csv"
metaData = pd.read_csv(metaDataPath, header = 0)
metaData.head()
...
print('Time spent: ' + str(round(time.time() - startTime, 4)) + 's.')
vectPath = "tf-idf_vectorizer.pkl"
with open(vectPath, "wb") as vectFile:
    pickle.dump(vectorizer, vectFile)
metaDataPath = "metadata_clean.csv"
metaData = pd.read_csv(metaDataPath, header = 0)
metaData.head()
metaData = metaData[metaData["clean_abstract"].notnull()]
metaData = metaData[metaData["clean_abstract"].str.contains("|".join(["coron", "covid"]))]
    ...
    for i in range(len(queries)):
        ...
        for idx in queryDocDist[i].argsort()[-nDoc:][::-1]:
            ...
            if showResults:
                ...
                print("cord_uid: ", docMeta["cord_uid"])
            ...
            outputFile.write("cord_uid: " + str(docMeta["cord_uid"]) + "\n")
...
def retrieveTopSent(queries, queryDocDist, dataPath, outputPath, nDoc = 5, nSent = 10, showResults = True):
    ...
    for i in range(len(queries)):
        ...
        for idx in queryDocDist[i].argsort()[-nDoc:][::-1]:
            ...
            sentUid = sentUid + [uid] * len(docRepSents)
        ...
        for iidx in querySentDist[0].argsort()[-nSent:][::-1]: # Show the top n related sentences
            ...
            outputFile.write("cord_uid: " + str(sentUid[iidx]) + "\n")
...
nDoc = 5
nSent = 10
dataPath = "kaggle/input/CORD-19-research-challenge"
outputPath = "topSentence.txt" 
