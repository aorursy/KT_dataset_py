...
print("la base d'apprentissage est au format : {} et la base test a pour format : {}\n".format(df_train.shape, df_test.shape))
# Conservation de la position des éléments de la base d'apprentissage
...
def missing(df):
    ...
    return df_null
missing(df)
# Imputation des valeurs manquantes générées par la fusion par - 1000

df["SalePrice"]=df["SalePrice"].fillna(-1000)
# Les variables ayant plus de 30% de valeurs manquantes

list(df.columns[df.isnull().sum() / len(df) > 0.30])
# Imputation des variables quantititatives
...
# Variables avec valeurs manquantes inexpliquées par les meta-données
...
# Imputation par la médiane selon le voisinage pour LotFrontage pour la variable quantitative
...
plt.show()
# Suppression des points atypiques
...
def whisker_based_outliers(data, whisker = 1.5):
    ...
    # restreint la base aux données de types numériques
...
# variables discrètes et catégorielles
...
# Boites à moustaches du prix du logement selon les explicatives
...
## Indicatrice si le logement à été rénové ou pas: 1 si YearBuild <> YearRemodAdd et O sinon
...
# Base 1: Modélisation du montant en unité de compte
...
def mape_cv(model):
    ...
    MAPE = np.abs(cross_val_score(model, df_train.values, y_train,
                                    ...
                                    ## mape n'est pas implémenter dans pour scoring, nous le construissions
...
# Score: Optimisé : 21857.153 (1404.104)
...
performance
## Dataframe contenant la soumission
