...
transaction_by_day = bitcoin_blockchain.query_to_pandas(query)

plt.plot(transaction_by_day.transaction)
...
query = """ SELECT merkle_root AS merkle, COUNT(transaction_id) AS transaction
            FROM `bigquery-public-data.bitcoin_blockchain.transactions`
            GROUP BY merkle
            ORDER BY transaction DESC
        """
transaction_by_merkle = bitcoin_blockchain.query_to_pandas(query)

transaction_by_merkle.head()