...
train_df = pd.read_csv(data_paths['train_feateng.csv'])

test_df = pd.read_csv(data_paths['test_feateng.csv'])

sub_df = pd.read_csv(data_paths['submission.csv'])
...
convert_chat_words("Will be right back ASAP")
def text_preprocessor(text):
    ...
    # text = convert_emojis(text) # or remove

    text = convert_emoticons(text) # or remove
    ...
    # text = stem_words(text)  # or lemmatize_words

   

    return text

# stem and lemmatizing - will take more time - depends on length of comment





text_preprocessor('hi this is a Test for you, return result ASAP :)')
...
for text in tqdm_notebook(train_df['comment_text']):

    preprocessed_comments.append(text_preprocessor(text))



train_df['comment_text'] = preprocessed_comments

train_end = datetime.now()

print('Train comments pre-processed in', train_end - start)
...
for text in tqdm_notebook(test_df['comment_text']):

    preprocessed_comments.append(text_preprocessor(text))

    

test_df['comment_text'] = preprocessed_comments

test_end = datetime.now()

print('Test comments pre-processed in', test_end - train_end)
