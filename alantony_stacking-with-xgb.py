...
xgb_stacker=XGBClassifier(n_estimators=500,subsample=0.9,colsample_bytree=0.8,max_depth=3,gamma=0,random_state=2)
xgb_stacker.fit(matrix_train,Y_train)
...
pred= xgb_stacker.predict_proba(matrix_test)
print(pred[0:100, :])
...
pred_train=xgb_stacker.predict_proba(matrix_train)
print(roc_auc_score(Y_train,pred_train[:,1]))
 
out.to_csv('results_out5.csv', index=False)