...
(train_values, train_labels), (test_values, test_labels) = datasets.imdb.load_data(num_words=10000)

print(f'{train_values.shape} Number of training records in imdb dataset')
print(f'{test_values.shape} Number of testing records in imdb dataset')
print('Values inside 1st record')
print(train_values[0])
print(f'Label of 1st record {train_labels[0]}')
# We have restricted number of words to 10000. Let check it
...
def onehot_vectorization(sequences, num_words=10000):
...
x_valid = x_train[random_shuf[:10000]]
y_valid = y_train[random_shuf[:10000]]

x_train = x_train[random_shuf[10000:]]
y_train = y_train[random_shuf[10000:]]
...
plt.show()
# By observing both graphs we can say that 3-4 epochs is more than enough for this architecture to perform best on both training and validation records
# Now train same network with 3 epochs
...
model.compile(loss=losses.binary_crossentropy, optimizer=optimizers.RMSprop(), metrics=metrics.binary_accuracy)
history = model.fit(x_train, y_train, epochs=3, batch_size=512, validation_data=(x_valid, y_valid))
results = model.evaluate(x_test, y_test)
print(f'The loss on test records is {results[0]:.3f} and binary accuracy is {results[1]:.3f}')
