...
#reuters_raw = nltk.corpus.reuters.raw('test/14826')

reuters_raw = nltk.corpus.reuters.raw()

print(reuters_raw[:10])
from nltk.tokenize import sent_tokenize
...
for i in sentences:
    ...
    listaWord.extend(word_tokenize(i))
...
for x in listaWord:
    ...
    for w in pos_tags:
        ...
        lemmas.extend(lemmatizer.lemmatize(w[0], pos_tag))
...
non_punctuation.extend([z for z in [w for w in lemmas if not w.lower() in stopwords_list] if not z in punctuation])

from collections import Counter



c = Counter(non_punctuation)
