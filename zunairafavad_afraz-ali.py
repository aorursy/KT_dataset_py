...
# ['DME', 'Alzeihmer Disease', 'NORMAL', '.DS_Store', 'DRUSEN']
from tqdm import tqdm
def get_data(folder):
    """
    ...
    for folderName in os.listdir(folder):
        if not folderName.startswith('.'):
            if folderName in ['NORMAL']:
                label = 0
            elif folderName in ['Alzeihmer Disease']:
                label = 1
...
for l in multipleImages[:25]:
    ...
    plt.imshow(cv2.cvtColor(im, cv2.COLOR_BGR2RGB)); plt.axis('off')
    i_ += 1
map_characters = {0: 'Normal', 1: 'Alzeihmers Disease' ,  2: 'Diabetic Macular Edema', 3: 'DRUSEN'}
...
map_characters1 = {0: 'Normal', 1: 'Alzeihmers Disease', 2: 'DME', 3: 'DRUSEN'}
