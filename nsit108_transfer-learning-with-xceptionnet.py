...
XceptionNet_weights_path = '/kaggle/input/xception/xception_weights_tf_dim_ordering_tf_kernels_notop.h5'
 
Xception_base_model = Xception(weights = XceptionNet_weights_path ,
                      include_top = False,
...
# Evaluate the model
scores = Xception_model.evaluate_generator(test_generator)
print("\n%s: %.2f%%" % (Xception_model.metrics_names[1], scores[1]*100))
end_time = datetime.datetime.now()
elapsed_time = end_time - start_time

print('Start Time  = {} ; End Time  = {} ; Total Time = {}'.format(start_time,end_time,elapsed_time))
print('Start Time  = {} ; End Time  = {} ; Total Time = {}'.format(start_time,end_time,elapsed_time))
print("\n%s: %.2f%%" % (Xception_model.metrics_names[0], scores[0]*100))
print("\n%s: %.2f%%" % (Xception_model.metrics_names[1], scores[1]*100))
print("\n%s: %.2f%%" % (Xception_model.metrics_names[2], scores[2]*100))
print("\n%s: %.2f%%" % (Xception_model.metrics_names[3], scores[3]*100))
print("\n%s: %.2f%%" % (Xception_model.metrics_names[4], scores[4]*100))
...
preds = Xception_model.predict_generator(generator=test_generator) # get proba predictions
