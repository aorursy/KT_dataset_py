...
masks_sum = np.sum(Y_train, axis=(1,2)).reshape((-1, 1))
...
for k, (train_index, test_index) in enumerate(kf.split(X_train, Y_train)):
    ...
    x_train = X_train_pre[train_index]

    y_train = Y_train[train_index]

    label_train = class_lab[train_index]

    x_test = X_train_pre[test_index]
    ...
    model = TransCGUNet(input_size=(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS))

    

    model.compile(optimizer=Adam(lr = 1e-5), loss={'seg':dice_coef_loss, 'class':'binary_crossentropy'}, \
...
for i in range(20):
    ...
    pred=model.predict(X_train_pre[index][np.newaxis, :, :, :])
