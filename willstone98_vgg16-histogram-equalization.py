...
input_size = 331
def img_transf(img):
    img =  np.resize(np.interp(img.flatten(), range(0,256), cdf), (image_size, image_size, 1))
    img -= img.mean()
...
for i, layer in enumerate(model.layers):
    ...
    elif len(model.layers) - i > 5: #freeze all but last 5 layers
        layer.trainable = False
...
history2 = model.fit_generator(crop_gen(train_gen),
                              ...
                              validation_steps=len(test_gen), 
                              verbose=1)
model.save_weights('VGG16_stretched_weights.h5')
full_history = dict()
