...
df=pd.read_csv('../input/audit-data/audit_data.csv')
...
df.info()
#eksik değerlerin sayısını öğrenmek için

df.isna().sum()
#Yukarıda da görüldüğü gibi Money_value'de bir eksik değer mevcut bunun yerine money_value alanının ortalama değerini atayacağız
