...
# Step 1: Initiate libraries and input directory
# You do not have to change anything in this code, but you need to run this cell before running any later cells.
...
# Step 2: Define the dataset as 'dfGSS'
# You do not have to change anything in this code, but you need to run this cell before running any later cells.
dfGSS=pd.read_csv('/kaggle/input/2020sp-gss-data/2020SP_GSS_data.csv', na_values=['.', '96', '97', '98', '99'], dtype='float64')
# Step 3: Let's take a look at what the data in our dataset looks like
# You do not have to change anything in this code.
print(dfGSS.head(10))
# Step 4: List column names
# You do not have to change anything in this code.
dfGSS.columns.values
# Step 5: Create a list (called lVar) with the names of the variables we are interested in getting descriptive statistics for. 
# Fill in VAR1, VAR2, VAR3, VAR4, & VAR5 with the variables you are interested in exploring for the worksheet. 
# Make sure the names you type below exactly match how the name is written in step 4's output.
lVar=['race', 'age', 'VAR1', 'VAR2', 'VAR3', 'VAR4', 'VAR5']
# Step 6: Print frequency distribution. 
# Replace the label VARIABLE_LIST with the name of the list from step 5. 
for var in VARIABLE_LIST:
    ...
    print(var, '\n', dfFreq, '\n')
# Step 7: Plot a histogram for numeric (interval and ratio level) variables.
# You do not need to run this line of code if none of your variables are interval or ratio level variables.
my_variable = 'VAR' # Replace VAR with the name of a numeric variable.
plt.hist(dfGSS[my_variable])
plt.xlabel(my_variable)
plt.ylabel('Frequency')
plt.title('Histogram of '+my_variable)
# Print a histogram for each of the numeric (interval and ratio level) variables from the list lVar in step 5.
# Hint: 
...
# Step 8: Identify code for bar chart labels
# You do not have to change anything in this code, but you must run this cell before running step 9.
test={}
...
# Step 9: Plot bar charts for categorical (nominal, binary, ordinal) variables.
...
plt.show()
# Print a bar chart for each of the categorical variables (nominal, ordinal, or interval) from the list lVar in step 5.
# Hint: 
# 1) Hover your cursor over this cell.
# 2) Click the "+ Code" button below the cell.
# 3) Copy and paste this code into the new cell:
...
# Step 10: Print descriptive statistics
# Replace the label VARIABLE_LIST with the name of the list from step 5.
...
print(dfUni)
# Step 11: Run a crosstab
# Replace INDEPENDENT_VARIABLE and DEPENDENT_VARIABLE with the names of your independent and dependent variables from the worksheet.
...
contingency_table
# Step 12: Calculate chi-square. 
# You do not have to change anything in this code.
chi2, p, dof, ex = stats.chi2_contingency(contingency_table, correction=False)
stats.chi2_contingency(contingency_table)

# Print the chi-square and p-value
print('Chi-squared: %.3f ' %chi2, 'p-value: %.4f ' %p)
# Step 13: Run a correlation with p-value between two variables 
# Replace INDEPENDENT_VARIABLE and DEPENDENT_VARIABLE with the names of your independent and dependent variables from the worksheet.
