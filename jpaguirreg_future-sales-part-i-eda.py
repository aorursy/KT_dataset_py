...
df.head(10)
df.city_id.unique()
df[df['city_id']==8].head(5)
# select a random city

df_city = df[df["city_id"]== 8]

df_timeindex = df_city.set_index("date").resample("W")["item_cnt_day"].sum().to_frame()
...
axes[0].set_title("Decomposition of Казань by Weekly Sales")
...
df.head(5)
short_df = df[df["YEAR"] == 2015][["item_cnt_day", "item_category_name"]]
...
my_values = pd.DataFrame(my_values)

my_pct = pd.DataFrame(short_df["item_cnt_day"]/short_df["item_cnt_day"].sum())

my_values.reset_index(inplace=True)

my_values.head(4)
my_pct.reset_index(inplace=True)
...
labels = ['{} - Sales :{}k \n {}% of total'.format(item_category_name, sales/1000, round(pct, 2)*100) for item_category_name, sales, pct in zip(short_df.index, my_values.item_cnt_day, my_pct.item_cnt_day) if round(pct, 2)*100>1]
...
squarify.plot(sizes = my_values.item_cnt_day, label = labels,  alpha = 0.6)

plt.title("Sales by Item Category and their % over total sales in 2015",fontsize = 23, fontweight = "bold")
