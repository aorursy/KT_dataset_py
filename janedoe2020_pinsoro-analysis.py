...
df_robot = pd.concat((pd.read_csv(filename, index_col=None, na_values= 'NaN') for filename in glob.glob('../input/data/Annotated/Child-Robot/**/*.csv', recursive=True)))
...
te_aimless_under = te_aimless.sample(8163)

te_goaloriented_under = te_goaloriented.sample(8163)

te_noplay_under = te_noplay.sample(8163)
...
se_onlooker_under = se_onlooker.sample(1301)

se_parallel_under = se_parallel.sample(1301)

se_solitary_under = se_solitary.sample(1301)
...
sa_prosocial_under = sa_prosocial.sample(16862)

sa_passive_under = sa_passive.sample(16862)

sa_sample_under = pd.concat([sa_prosocial_under, sa_assertive, sa_passive_under], axis=0)
