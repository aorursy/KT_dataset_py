...
def get_logged_series(s):
    s1 = np.array([log2(n) for n in s])
    return s1
    
def get_exp_series(s):
    s1 = np.array([np.expm1(n) for n in s])
    return s1
...
def get_dummies(dummy_na = False):
    def wrapper_get_dummies(s):
        return pd.get_dummies(s, prefix = s.name, dummy_na = dummy_na)
    return wrapper_get_dummies

def convert_features(df, raw_fields, scaled_fields, cat_fields, scaled_fields_proc = None, cat_fields_proc = None, label_name = None, label_proc = None, train_validate_ratio = None):
    if scaled_fields_proc is None:
        scaled_fields_proc = lambda x: x
    if cat_fields_proc is None:
        cat_fields_proc = lambda x: x
    if label_proc is None:
        label_proc = lambda x: x
    features = pd.DataFrame()
    labels = pd.DataFrame()
    for k in raw_fields:
        features[k] = df[k].copy()
    for k in scaled_fields:
        features[k] = scaled_fields_proc(df[k].copy())
    for k in cat_fields:
        features = features.join(cat_fields_proc(df[k].copy()))
    if label_name is not None:
        labels[label_name] = label_proc(df[label_name].copy())
    else:
        labels = None
    ...
    if labels is not None:
        train_labels = labels[label_name].head(train_num)
        validate_labels = labels[label_name].tail(validate_num)
    else:
        train_labels = None
        validate_labels = None
    return (train_features, train_labels, validate_features, validate_labels)
def train_xgb_regressor_model(
    train_param,
    train_rounds,
    training_examples,
    training_targets,
    validation_examples,
    validation_targets,
    targets_transform_fn = lambda x: x):
    """
    Trains a XGBoost model
    
    Args:
        train_param: XGBoost training parameters
        train_rounds: Rounds to train
        training_examples: A `DataFrame` containing one or more columns as 
            features for training.
        training_targets: A `DataFrame` containing exactly one column as label
            for training.
        validation_examples: A `DataFrame` containing one or more columns as 
            features for validation.
        validation_targets: A `DataFrame` containing exactly one column as label
            for validation.
        
    """
    if validation_examples is None or validation_targets is None:
        do_validation = False
    else:
        do_validation = True
    training_rmse = []
    ...
    validation_rmse = []
    validation_rmsle = []
    train_mat = xgb.DMatrix(training_examples, training_targets)
    validate_mat = xgb.DMatrix(validation_examples, validation_targets)
    xgb_regressor = None
    num_periods = 10
    num_boost_round = int(train_rounds / num_periods)
    if num_boost_round == 0:
        num_boost_round = 1
        num_periods = train_rounds
    train_param['silent'] = 1
    for period in range(0, num_periods):
        xgb_regressor = xgb.train(train_param,
                                  train_mat,
        ...
        training_predictions = xgb_regressor.predict(train_mat)
        
        # Compute training loss RMSE.
        training_root_mean_squared_error = math.sqrt(
            metrics.mean_squared_error(targets_transform_fn(training_targets), 
                                       targets_transform_fn(training_predictions)))

        # Compute training loss RMSLE.
        training_root_mean_squared_log_error = math.sqrt(
            metrics.mean_squared_log_error(targets_transform_fn(training_targets), 
                                           targets_transform_fn(training_predictions)))
               
        if do_validation == True:
            validation_predictions = xgb_regressor.predict(validate_mat)
          
            # Compute validation loss RMSE.
            validation_root_mean_squared_error = math.sqrt(
                metrics.mean_squared_error(targets_transform_fn(validation_targets), 
                                           targets_transform_fn(validation_predictions)))

            # Compute validation loss RMSLE.
            validation_root_mean_squared_log_error = math.sqrt(
                metrics.mean_squared_log_error(targets_transform_fn(validation_targets), 
                                               targets_transform_fn(validation_predictions)))

        # Occasionally print the current loss.
        msg = "  period %02d (%d rounds/trees):" % (period, num_boost_round * (period + 1))
        msg += "TRAIN :%0.2f, %0.4f" % (training_root_mean_squared_error, training_root_mean_squared_log_error)
        
        if do_validation == True:
            msg += ", VALIDATE: %0.2f, %0.4f" % (validation_root_mean_squared_error, validation_root_mean_squared_log_error)
    ...
    lim = max(targets_transform_fn(training_targets))
    if do_validation == True:
        lim = max(lim, max(targets_transform_fn(validation_targets)))
    lim *= 1.05
    plt.xlim(0, lim)
    plt.ylim(0, lim)
    plt.plot([0, lim], [0, lim], alpha = 0.5, color = 'red')
    plt.scatter(targets_transform_fn(training_predictions), 
                targets_transform_fn(training_targets), alpha = 0.5, label="training")
    if do_validation == True:
        plt.scatter(targets_transform_fn(validation_predictions), 
                    targets_transform_fn(validation_targets), alpha = 0.5, label="validation")
    plt.legend()
    plt.tight_layout()
    
    return xgb_regressor
# prepare training sample and validation sample
raw_fields = [
    'OverallQual','FullBath', 'GarageCars',
    'BsmtFinSF1', 'BsmtUnfSF', 'TotalBsmtSF','1stFlrSF',
    '2ndFlrSF', 'GrLivArea', 'GarageArea', 'MSSubClass',
]
...
train_param = {'objective':'reg:linear'}
xgb_regressor = train_xgb_regressor_model(train_param = train_param,
                                          train_rounds = 100,
...
ret = convert_features(df, 
                       ...
                       scaled_fields, 
                       categorical_fields, 
                       scaled_fields_proc = get_logged_series, 
                       cat_fields_proc = get_dummies(dummy_na = True),
                       label_name = 'SalePrice',
                       train_validate_ratio = 0.7,
                       label_proc = get_logged_series)
...
validation_features = ret[2]
validation_targets = ret[3]
train_param = {'objective':'reg:linear', 'learning_rate':0.1}
xgb_regressor = train_xgb_regressor_model(train_param = train_param,
                                          train_rounds = 100,
                                          training_examples = training_features,
                                          training_targets = training_targets,
                                          validation_examples = validation_features,
                                          validation_targets = validation_targets,
                                          targets_transform_fn = get_exp_series)
# prepare training sample and validation sample
raw_fields = [
    'OverallQual','FullBath', 'GarageCars',
...
categorical_fields = [
    'Neighborhood', 'CentralAir', 'KitchenQual'
]
...
validation_features = ret[2]
validation_targets = ret[3]
train_param = {'objective':'reg:linear', 'learning_rate':0.1, 'max_depth':2}
xgb_regressor = train_xgb_regressor_model(train_param = train_param,
                                          train_rounds = 300,
                                          training_examples = training_features,
                                          training_targets = training_targets,
...
test_predictions = xgb_regressor.predict(test_mat)
test_predictions = get_exp_series(test_predictions)
        
df_submit = pd.DataFrame()
df_submit['Id'] = df_test['Id']
df_submit['SalePrice'] = test_predictions
