...
print(data.info())
corr = data[['satisfaction_level', 'last_evaluation', 'average_montly_hours', 'time_spend_company', 'Work_accident', 'promotion_last_5years', ]].corr()
sns.heatmap(corr, square=True, annot=True, fmt='.2f')
salary_left = pd.crosstab(data['salary'], data['left'])
print(salary_left)
salary_left.plot(kind = 'bar')
plt.show()
salary_left.div(salary_left.sum(axis = 1), axis = 0).plot(kind = 'bar', stacked = True)
