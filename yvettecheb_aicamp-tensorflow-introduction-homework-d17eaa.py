...
trainY = f(trainX) + np.random.randn(N) * noise_level# TODO,搅动
...
class LinearRegressionTF(BaseEstimator):
    ...
    def fit(self, trainX,trainY):
        ...
        if self.annotate:
            ...
            plt.plot(trainX, self.sess.run(self.W) * trainX + self.sess.run(self.b), label='Fitted line') #TODO
            plt.legend()
...
class LogisticRegressionTF(BaseEstimator):
    ...
    def fit(self, trainX,trainY):
        ...
        # 参数的定义
        self.W = tf.Variable(np.random.randn(D,c), name="weight")# TODO, give a random d*c array
        self.b = tf.Variable(np.random.rand(c), name="bias")# TODO
        ...
        for epoch in range(self.training_epochs):
            for (x, y) in zip(trainX, trainY):
                self.sess.run(optimizer, feed_dict={self.X: np.asmatrix(x), self.Y: np.asmatrix(y)})#feed matrix
                # TODO
...
le.fit(trainY.reshape(N,-1)) #-1 is unspecified value
...
class NaiveBayesTF(BaseEstimator):
    ...
    def fit(self, trainX, trainY):
        ...
        moments =tf.nn.moments(input_x, axes=[1])# TODO#calculate means and variance of inputx
    ...
    def predict(self, testX):
        ...
        # 条件概率 log P(x|c)
        # (nb_samples, nb_classes),a function that gives the probability that a discrete random variable is exactly equal to some value.
