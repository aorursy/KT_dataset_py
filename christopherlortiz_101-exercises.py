...
def times_two_plus_three(number1):
    return number1 * 2 + 3
...
def count_vowels(word):
    ...
    for letter in list(word.lower()):
        if letter in vowels:
            total_vowels_in_word += 1
    return total_vowels_in_word
...
def median(sequence):
    elements_in_sequence = len(sequence)
    
    if elements_in_sequence % 2 == 0:
        return mean(sequence[elements_in_sequence // 2 - 1: elements_in_sequence // 2 + 1])
...
def mode(sequence):
    return max(sequence, key=sequence.count)  # if methods are used as keys, they do not need a specific argument
...
def only_positive_evens(sequence):
    positive_evens = [x for x in sequence if x % 2 == 0 if x > 0]
...
def only_positive_odds(sequence):
    positive_odds = [x for x in sequence if x % 2 != 0 if x > 0]
...
def only_negative_evens(sequence):
    negative_evens = [x for x in sequence if x % 2 == 0 if x < 0]
...
def only_negative_odds(sequence):
    negative_odds = [x for x in sequence if x % 2 != 0 if x < 0]
...
def get_values_not_in_common(list1, list2):
    not_in_common = [x for x in list1 if x not in list2] + [y for y in list2 if y not in list1]
...
def total_of_book_prices(dictionary):
    prices = [dict['price'] for dict in dictionary]
    return sum(prices)
...
def get_average_book_price(dicts):
    books = [dict['price'] for dict in dicts]
    return mean(books)
...
def highest_price_book(dicts):
    highest_book_price = max([x['price'] for x in dicts])
    
    for i in dicts:
        if i['price'] == highest_book_price:
            return dict(i.items())
...
#         if i['price'] == highest_book_price:
#             return i.items()
#     return highest_book_price
# highest_price_book(books)

def lowest_price_book(books):
    cheapest_book = min([x['price'] for x in books])
    book = [i for i in books if i['price'] == cheapest_book]
...
def total_number_of_items(dicts):
    return sum(x['quantity'] for x in dicts['items'])
...
def get_average_item_price(shopping_cart):
    return mean([x['price'] for x in shopping_cart['items']])
...
def get_average_spent_per_item(shopping_cart):
    total_price = 0
    total_quantity = sum([x['quantity'] for x in shopping_cart['items']])
...
def most_spent_on_item(shopping_cart):
    ...
    for i in shopping_cart['items']:
        ...
        if most_spent_on_cost < cost:
            most_spent_on = i.items()
            
            
    return dict(most_spent_on)
