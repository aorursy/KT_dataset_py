...
def clean_text(x, method = 'basic'):
    ...
    return x
files()
data = read('weatherAUS.csv')
head(data,7)
describe(data)
Y = data.raintomorrow

tallies = tally(Y)
plot(data = tallies, x = 'raintomorrow', y = 'tally', style = 'barplot')

X = data.drop(['raintomorrow'], axis=1)
head(X)
X = replace(X, [['No',0],['Yes', 1]])
X = X.fillna(0)
Y = replace(Y, [['No',0],['Yes', 1]])
Y = Y.fillna(0)
head(X)
X = clean(X)
X = exclude(X,['date','sunshine','evaporation'])
head(X)
model = LinearModel(scale = True)
model.fit(X,Y)
model.coefficients(plot = True)
model.analyse(plot = True)
X = exclude(X,['raintoday','risk_mm'])
head(X)
model = LinearModel(scale = True)
model.fit(X,Y)
model.analyse(plot = True)