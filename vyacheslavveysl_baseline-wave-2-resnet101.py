...
class ChartsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        except Exception as e:                #return self.__getitem__( self.prev_good )
...
data_transforms = albumentations.Compose([
   ...
   # albumentations.RandomBrightness(),
    ...
    albumentations.ShiftScaleRotate(rotate_limit=7, scale_limit=0.10),
...
#  __!!! dataloaders with   my_collate
...
optimizer = torch.optim.SGD(model.parameters(), lr=0.001, momentum=0.9) # 0.1-> 0.06

scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.8, patience=2,)
# запускаем обучение

model_resnet, train_loss, val_loss = train_model(model, trainloader, validloader, criterion, optimizer, scheduler, n_epochs=50,) # 25 -50
# тестируем модель
