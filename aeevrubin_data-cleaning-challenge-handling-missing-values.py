...
# look at the # of missing points in the first ten columns
print(missing_values_count_sf_permits)
...
# percent of data that is missing
print("percent of sf_permits data that is missing: ",(total_missing_sf_permits/total_cells_sf_permits) * 100)
# look at the # of missing points in the first ten columns
missing_values_count[0:10]
sf_permits[sf_permits['Street Number Suffix'].isnull() == True].head()
sf_permits[sf_permits['Street Number Suffix'].isnull() == False].head()
sf_permits[sf_permits['Zipcode'].isnull() == True].head()
sf_permits[sf_permits['Zipcode'].isnull() == False].head()
...
# remove all the rows that contain a missing value and find none left
sf_permits.dropna()
...
CA_addy_data = pd.read_csv("../input/openaddresses-us-west/ca.csv")
#so sf isn't even in this dataset so I can't use it to match addresses to zipcodes

pd.set_option('display.max_rows', 1000)

print(CA_addy_data.CITY.value_counts())
