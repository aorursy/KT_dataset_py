...
# AppleStore.csv has 7198 rows in reality, but we are only loading/previewing the first 100 rows
df1 = pd.read_csv('../input/AppleStore.csv', delimiter=',', nrows = nRowsRead)
df1.dataframeName = 'AppleStore.csv'
nRow, nCol = df1.shape
...
nRowsRead = 100 # specify 'None' if want to read whole file
# appleStore_description.csv has 7197 rows in reality, but we are only loading/previewing the first 100 rows
df2 = pd.read_csv('../input/appleStore_description.csv', delimiter=',', nrows = nRowsRead)
df2.dataframeName = 'appleStore_description.csv'
nRow, nCol = df2.shape
print(f'There are {nRow} rows and {nCol} columns')
df2.head(5)
