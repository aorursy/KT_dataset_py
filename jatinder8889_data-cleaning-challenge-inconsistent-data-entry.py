...
matches = fuzzywuzzy.process.extract("d.i khan", cities, limit=20, scorer=fuzzywuzzy.fuzz.token_sort_ratio)
...
cities
# look at the first ten thousand bytes to guess the character encoding
with open("../input/PakistanSuicideAttacks Ver 6 (10-October-2017).csv", 'rb') as rawdata1:
    result1 = chardet.detect(rawdata1.read(100000))

# check what the character encoding might be
print(result1)
# read in our dat
suicide_attacks_v6 = pd.read_csv("../input/PakistanSuicideAttacks Ver 11 (30-November-2017).csv", 
                              encoding='Windows-1252')
# get all the unique values in the 'City' column
cities = suicide_attacks_v6['City'].unique()

# sort them alphabetically and then take a closer look
cities.sort()
cities
suicide_attacks_v6.columns
# convert to lower case
...
# Your turn! It looks like 'kuram agency' and 'kurram agency' should
# be the same city. Correct the dataframe so that they are.
replace_matches_in_column(df=suicide_attacks_v6, column='City', string_to_match="kuram agency")
cities = (suicide_attacks_v6['City'].unique())
cities.sort()
cities