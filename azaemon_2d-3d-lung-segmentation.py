...
OUT_DIR= os.path.join('..','output/kaggle/working')
...
plot_anat(epi_img)
view_img(epi_img)
plot_anat(epi_img, draw_cross=False, display_mode='z')
plt.imshow(epi_img_data[0])
f, plots = plt.subplots(10, 1, figsize=(5, 40))
for i in range(10):
    
    plots[i].axis('off')
    plots[i].imshow(epi_img_data[i+50], cmap=plt.cm.gray ) 
from sklearn.metrics import f1_score
pred_output=[]
f1_scores=[]
for i in range(117):
