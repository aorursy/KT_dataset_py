...
sns.countplot(x=train["target"],hue=train["target"])
rcParams["figure.figsize"] = 10,10

train["target"].value_counts().plot(kind="pie")
real_text = train[train["target"]==1]

false_text = train[train["target"]==0]
real_text.dropna()
...
train['text'] = train['text'].apply((lambda y:re.sub("http://\S+"," ", y)))



test['text'] = test['text'].apply((lambda x:re.sub("\#", " ",x.lower())))

test['text'] = test['text'].apply((lambda y:re.sub("http://\S+"," ", y)))
...
sns.countplot(train['target'],hue=train['target'],palette="Set1")
# plt.figure(figsize=(20,8))

fig,(ax1,ax2)=plt.subplots(1,2,figsize=(20,8))

train_df_len1 = train[train['target']==1]['text'].str.len()

ax1.hist(train_df_len1,color='yellow')

ax1.set_title('disaster tweets')

train_Df_len2 = train[train['target']==0]['text'].str.len()

ax2.hist(train_Df_len2,color = 'blue')
...
submission.to_csv('bert_Model_submission2.csv', index=False)
