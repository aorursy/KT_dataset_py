...
#X_test = data[1] For submission, not needed now
...
def create_model(input_shape=X_train.shape[1:],
                ...
                neurons_per_hidden=50, # make it higher

                hidden_drop_rate= 0.1,      # too high, make it 0.1
    ...
    model.compile(loss=loss, 
                  ...
                  optimizer = SGD(lr=0.01, decay=1e-6, momentum=0.9, nesterov=True), #Adagrad(learning_rate = 0.01), #RMSprop(learning_rate=0.001, rho=0.9), #SGD(lr=0.01, decay=1e-6, momentum=0.9, nesterov=True), #Nadam(lr=0.0005),
...
y_test_pred_dnn = dnn_clf.predict_classes(x_test)
print("Train accuracy:",accuracy_score(y_test,y_test_pred_dnn))
...
# applying (10) k-fold cross validation

from sklearn.model_selection import cross_val_score

accuracies = cross_val_score(clf, x_train, y_train, cv=10)

print("Different fold's accuracy:",accuracies)
...
cm_svm = confusion_matrix(y_test,test_Predicted)

sns.heatmap(cm_svm,cbar=False,annot=True,cmap="Greens",fmt="d")
#This is for submision
