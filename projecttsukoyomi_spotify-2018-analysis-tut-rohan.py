...
Dance = pd.DataFrame(data, columns=['percent'], index=['Very', 'Regular', 'Instrumental'])
...
Energy=pd.DataFrame(data, columns=['percent'], index=['Very Energy', 'Regular Energy', 'Low Energy'])
Energy
...
Drake[['name', 'danceability', 'energy', 'energy', 'loudness', 'valence', 'tempo', 'Rhythm']]
...
sns.heatmap(MayoresD.corr(), cmap="YlGnBu")
# Heat map for less scales

sns.heatmap(MenoresD.corr(), cmap='YlGnBu')
#We create the variables and assign the colums that we want to correlate
...
sns.heatmap(MaycorD.corr(), annot=True, cmap='YlGnBu')
sns.heatmap(MencorD.corr(), annot=True, cmap="YlGnBu")
