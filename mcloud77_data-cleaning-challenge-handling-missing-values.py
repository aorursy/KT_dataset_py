...
np.random.seed(0) 
# look at a few rows of the nfl_data file.Note the missimg data.
nfl_data.sample(5)
...
# how many total missing values do we have?
sf_missing_values_counts = sf_permits.isnull().sum()
sf_total_cells = np.product(sf_permits.shape)
sf_total_missing = missing_values_counts.sum()

# percent of data that is missing
(sf_total_missing/sf_total_cells) * 100
# look at the # of missing points in the first ten columns
missing_values_count[0:10]
sf_permits['Street Number Suffix'].sample(5)
sf_permits['Zipcode'].sample(5)
...
# Remove all the rows from the sf_permits dataset that contain missing values. How many are left?
sf_permits.dropna()
...
sf_permits.fillna(method='ffill', axis=0,inplace=True).fillna("0")