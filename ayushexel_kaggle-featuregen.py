...
def train_model(train, valid, name ,test=None, feature_cols=None):
    wandb.init(project='Kaggle-FeatureGen',name=name)
...
_ = train_model(train, valid,'baseline')
import itertools
...
train, valid, test = get_data_splits(clicks)
_ = train_model(train, valid,'interactions')
...
train, valid, test = get_data_splits(clicks)
_ = train_model(train, valid,'numeric_features')
...
_ = train_model(train, valid,'past_events')
...
train, valid, test = get_data_splits(clicks)
_ = train_model(train, valid,'num_previous_downloads')
# Check your answer (Run this code cell to receive credit!)
q_6.solution()