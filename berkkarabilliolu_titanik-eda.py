...
plt.style.use("seaborn-whitegrid") # kareli defter gibi ayırt edilebilir tarz
...
warnings.filterwarnings("ignore") # hata yaptığındaki mesajları kapatıyor 
...
plt.style.available #istediğin tarzı kullanmak için
...
train_df[["Pclass","Survived"]].groupby(["Pclass"], as_index = False).mean().sort_values(by="Survived",ascending=False) # Sınıflara göre hayatta kalma durumu
# Pclass - Survived

train_df[["Sex","Survived"]].groupby(["Sex"], as_index = False).mean().sort_values(by="Survived",ascending=False) # Cinsiyete göre hayatta kalma durumu
# Pclass - Survived

train_df[["SibSp","Survived"]].groupby(["SibSp"], as_index = False).mean().sort_values(by="Survived",ascending=False) # Aileye göre hayatta kalma oranı
# Pclass - Survived

train_df[["Parch","Survived"]].groupby(["Parch"], as_index = False).mean().sort_values(by="Survived",ascending=False) # Sınıflara göre hayatta kalma durumu
...
g = sns.FacetGrid(train_df,row="Embarked",col="Survived",size=2.1)
...
train_df["Title"] = train_df["Title"].replace(["Lady","the Countess","Capt","Don","Col","Dr","Major","Rev","Sir","Jonkheer","Dona"],"other")
...
X_train , X_test , y_train , y_test = train_test_split(X_train,y_train , test_size= 0.33 , random_state = 42) # %33 test %67 train 
