...
def evaluate_cross_validation(model, X_train = X_train, y = y, n_splits=10):

    cv = ShuffleSplit(n_splits, test_size=0.1, train_size=None, random_state=None)

    rmse= np.sqrt(-cross_val_score(model, X_train, y, scoring="neg_mean_squared_error",  cv=cv))

    return(rmse)
n_estimators = [150, 170, 400, 500]

cv_rmse_gb = [(evaluate_cross_validation(GradientBoostingRegressor(n_estimators = n_estimator, max_depth=5))).mean()
...
plt.ylabel("rmse")
cv_gb.min()
model_gb = GradientBoostingRegressor(n_estimators = 150, max_depth=5).fit(X_train, y)
...
cv_rmse_ridge = [evaluate_cross_validation(Ridge(alpha = alpha, normalize = True)).mean() 
...
cv_rmse_lasso = evaluate_cross_validation(model_lasso).mean()
print (cv_rmse_lasso)
...
preds = 0.5*lasso_preds + 0.2*gb_preds + 0.1*xgb_preds + 0.2*ridge_preds
solution = pd.DataFrame({"id":test.Id, "SalePrice":preds})

solution.to_csv("lasso_gb_xgb_ridge.csv", index = False)