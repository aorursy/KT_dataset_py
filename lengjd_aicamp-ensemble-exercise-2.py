...
# 经过label encoding，23列变成161列
for col in cols:
    encoder = LabelEncoder()
    values = data[col].tolist()
    values.append('missing')  #加入missing这种值
    encoder.fit(values)
    labelEncoders[col] = encoder #encoder记录的是index
...
forest = RandomForestClassifier(n_estimators = 200)#TODO
...
class FakeRandomForest(BaseEstimator):
    ...
    def fit(self, X, Y, n_features=None):
        ...
        if n_features is None:
            # 特征的个数
            n_features = int(np.sqrt(D))#多少列也就是多少个特征
        ...
        for m in range(self.M):
            ...
            #随机抽取n_features个特征
            features = np.random.choice(D, size=n_features, replace=False)#不是有放回的采样
...
class BaggedTreeClassifier(BaseEstimator):
    ...
    def score(self, X, Y):
        P = self.predict(X)
        return np.mean(Y == P)
baggedtc = BaggedTreeClassifier(30)
cross_val_score(baggedtc, X, Y, cv=8).mean()
fakerf = FakeRandomForest(30)
...
hdata = transform_numerical(house_data)# TODO
train_data, test_data = train_test_split(hdata, test_size=0.2)# TODO
trainX, trainY = train_data[:,1:],train_data[:,0]# 第一列是标签，剩下所有的都是数据
...
rfregressor.fit(trainX, trainY)
print("random forest test score: %f" % (rfregressor.score(testX, testY)))
