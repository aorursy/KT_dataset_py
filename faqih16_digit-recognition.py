...
array = [12,5,26,6,42,28,1,14,13,10]
for i in array:
    img=train_images.iloc[i].values
    img=img.reshape(28,28)
    plt.imshow(img,cmap='gray')
    plt.title('Gambar ' + str(train_labels.iloc[i]))
    plt.figure()
...
# Todo
for i in array:
    train_labels.iloc[[i],0]
    plt.title("Histogram " + str(train_labels.iloc[i,0]))
    #plt.title(train_labels.iloc[[i],0])
    plt.hist(train_images.iloc[i])
    plt.figure()
...
#array = [12,5,26,6,42,28,1,14,13,10]
for i in array:
    plt.title("Jumlahnya seluruhnya")
    datai=train_images.iloc[i]
...
# berikut adalah hasil histogram yang baru. terdapat perubahan di angka yang minim sekarang tidak tercatat
