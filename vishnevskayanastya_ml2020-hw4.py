...
for i in range(18,60, 1):
    if max_uplf < (uplift_score(df_train_all[df_train_all.age>i]) - uplift_score(df_train_all[df_train_all.age<=i])):
        ...
        print(i)
#Новый признак - медианный чек
median_chech_sum_train = df_pursh.drop_duplicates('transaction_id').groupby('client_id')['purchase_sum'].median()
median_chech_sum_test = df_pursh_test.drop_duplicates('transaction_id').groupby('client_id')['purchase_sum'].median()

df_train_all['median_sum'] = df_train_all['client_id'].map(median_chech_sum_train)
df_test_all['median_sum'] = df_test_all['client_id'].map(median_chech_sum_test)
#Новый признак - пенсионер или нет
df_train['age'] = df_train['client_id'].map(clients.set_index('client_id')['age'])
df_test['age'] = df_test['client_id'].map(clients.set_index('client_id')['age'])
df_train['gender'] = df_train['client_id'].map(clients.set_index('client_id')['gender'])
df_test['gender'] = df_test['client_id'].map(clients.set_index('client_id')['gender'])
df_train_all['is_retired'] = (np.array([x == 'F' for x in df_train['gender']]) * np.array([x >= 60 for x in df_train['age']]) + np.array([x == 'M' for x in df_train['gender']]) * np.array([x >= 65 for x in df_train['age']])).astype('int')
df_test_all['is_retired'] = (np.array([x == 'F' for x in df_test['gender']]) * np.array([x >= 60 for x in df_test['age']]) + np.array([x == 'M' for x in df_test['gender']]) * np.array([x >= 65 for x in df_test['age']])).astype('int')
# Новый признак - level_1
lvl1Dict = {}
for lvl in df_products['level_1']:
    if lvl not in lvl1Dict:
        lvl1Dict.update({lvl : 2 ** len(lvl1Dict)})
        
mapper = df_products.set_index('product_id')['level_1']
df_pursh['level_1'] = [lvl1Dict[x] for x in df_pursh['product_id'].map(mapper)]
df_pursh_test['level_1'] = [lvl1Dict[x] for x in df_pursh_test['product_id'].map(mapper)]

lvl1_mapper_train = df_pursh.groupby('client_id')['level_1'].agg(lambda x: np.bitwise_or.reduce(x.values))
df_train_all['level_1_bitmap'] = df_train_all['client_id'].map(lvl1_mapper_train)

lvl1_mapper_test = df_pursh_test.groupby('client_id')['level_1'].agg(lambda x: np.bitwise_or.reduce(x.values))
df_test_all['level_1_bitmap'] = df_test_all['client_id'].map(lvl1_mapper_test)
# Новый признак - level_2
lvl2Dict = {}
for lvl in df_products['level_2']:
    if lvl not in lvl2Dict:
        lvl2Dict.update({lvl : 2 ** len(lvl2Dict)})
        
mapper = df_products.set_index('product_id')['level_2']
df_pursh['level_2'] = [lvl2Dict[x] for x in df_pursh['product_id'].map(mapper)]
df_pursh_test['level_2'] = [lvl2Dict[x] for x in df_pursh_test['product_id'].map(mapper)]

lvl2_mapper_train = df_pursh.groupby('client_id')['level_2'].agg(lambda x: np.bitwise_or.reduce(x.values))
df_train_all['level_2_bitmap'] = df_train_all['client_id'].map(lvl2_mapper_train)

lvl2_mapper_test = df_pursh_test.groupby('client_id')['level_2'].agg(lambda x: np.bitwise_or.reduce(x.values))
df_test_all['level_2_bitmap'] = df_test_all['client_id'].map(lvl2_mapper_test)
# алкоголь собственной марки
alco_mapper = df_products.set_index('product_id')['is_alcohol']
trademark_mapper = df_products.set_index('product_id')['is_own_trademark']
df_pursh['is_alcohol'] = df_pursh['product_id'].map(alco_mapper)
df_pursh_test['is_alcohol'] = df_pursh_test['product_id'].map(alco_mapper)
df_pursh['is_own_trademark'] = df_pursh['product_id'].map(trademark_mapper)
df_pursh_test['is_own_trademark'] = df_pursh_test['product_id'].map(trademark_mapper)
df_pursh['is_own_alcohol'] = np.array(df_pursh['is_own_trademark']) * np.array(df_pursh['is_alcohol'])
df_pursh_test['is_own_alcohol'] = np.array(df_pursh_test['is_own_trademark']) * np.array(df_pursh_test['is_alcohol'])

alco_mapper_train = df_pursh.groupby('client_id')['is_own_alcohol'].sum()
df_train_all['own_alco_produts_count'] = df_train_all['client_id'].map(alco_mapper_train)

alco_mapper_test = df_pursh_test.groupby('client_id')['is_own_alcohol'].sum()
df_test_all['own_alco_produts_count'] = df_test_all['client_id'].map(alco_mapper_test)

df_train_all['is_own_alcohol'] = (df_train_all['own_alco_produts_count'] > 0).astype('int')
df_test_all['is_own_alcohol'] = (df_test_all['own_alco_produts_count'] > 0).astype('int')
df_new_features = df_train_all[['is_own_alcohol', 'level_2_bitmap', 'level_1_bitmap', 'is_retired', 'median_sum']]
df_new_features_test = df_test_all[['is_own_alcohol', 'level_2_bitmap', 'level_1_bitmap', 'is_retired', 'median_sum']]
