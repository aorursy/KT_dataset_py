...
def has_evens(numbers):
    ...
    return any(t % 2 == 0 for t in numbers)
...
#     return (L[0]%2) + count_odds(L[1:]) if L else 0
...
def only_negative_evens(numbers):
    ...
    return [i for i in numbers if i % 2 != 1 and i < 0]
...
def shortest_string(myString):

    return min(myString, key=len)
...
def get_year_published(published):

    return published.get("year_published")
...
# Write a function called highest_priced_book that takes in the above defined list of dictionaries "books" and 

# returns the dictionary containing the title, price, and author of the book with the highest priced book.
...
def highest_price_book(hpb):

    return max(hpb, key=lambda x: x['price'])
...
def lowest_price_book(lpb):

    return min(lpb, key=lambda x: x['price'])
...
def number_of_item_types(diftype):
...
def total_number_of_items(count):
    ...
    while i < len(count["items"]):

        total = total + (count["items"] [i] ["quantity"])
...
def get_average_item_price(avg):
...
def get_average_spent_per_item(avg):
...
def most_spent_on_item(most):
    ...
    while i < len(most["items"]):

        if (most["items"][i]["quantity"])*(most["items"][i]["price"])> (d[0]["quantity"] * d[0]["price"]):

            d[0]["title"]= most["items"][i]["title"]

            d[0]["price"]= most["items"][i]["price"]

            d[0]["quantity"]= most["items"][i]["quantity"]
...
# help from forum on this problem to make it shorter

# def most_spent_on_item(most):

#     items = most['items']

#     return max(items, key=lambda x: x['price'] * x['quantity'])
