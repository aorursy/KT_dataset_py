...
train_df['ImageId_ClassId'] = train_df.apply(lambda row: row['ImageId']+"_"+str(row['ClassId']), axis = 1)

del train_df['ImageId']

del train_df['ClassId']

train_df = train_df[['ImageId_ClassId','EncodedPixels']]
...
for r in grouped_EncodedPixels.iteritems():
    ...
    if (ridx < 10000):
        ...
        if (r[1][i][0]=='1'):
           ...
           df = df.append({'ImageId_ClassId': r[0]+'_1','EncodedPixels': r[1][0][1]}, ignore_index=True)

           i = i+1

        else:

           df = df.append({'ImageId_ClassId': r[0]+'_1'}, ignore_index=True)

        if i == ll:

           df = df.append({'ImageId_ClassId': r[0]+'_2'}, ignore_index=True)

        elif (r[1][i][0]=='2'):

           df = df.append({'ImageId_ClassId': r[0]+'_2','EncodedPixels': r[1][0][1]}, ignore_index=True)
        ...
        if i == ll:
           ...
           df = df.append({'ImageId_ClassId': r[0]+'_3'}, ignore_index=True)

        elif (r[1][i][0]=='3'):

           df = df.append({'ImageId_ClassId': r[0]+'_3','EncodedPixels': r[1][0][1]}, ignore_index=True)
        ...
        if i == ll:
           ...
           df = df.append({'ImageId_ClassId': r[0]+'_4'}, ignore_index=True)

        elif (r[1][i][0]=='4'):

           df = df.append({'ImageId_ClassId': r[0]+'_4','EncodedPixels': r[1][0][1]}, ignore_index=True)
...
ten_rand_samples = defects_count_df[defects_count_df == 1].sample(10).index
