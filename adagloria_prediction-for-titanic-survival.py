...
for column in data.columns.values.tolist():
    print(column)
    print(checknull[column].value_counts())
    print(" ")
...
t.fit(data['Sex'])
print(list(t.classes_))
data['Sex_encode'] = t.transform(data['Sex']) 


t.fit(data['Embarked'])
print(list(t.classes_))
data['Embarked_encode'] = t.transform(data['Embarked']) 


t.fit(data['title'])
print(list(t.classes_))
data['title_encode'] = t.transform(data['title']) 
data.drop(['Sex','Embarked','title'], axis=1, inplace = True)
data.head()

#one_hot_encoded_training_predictors = pd.get_dummies(data)
Y = data.iloc[:,1].values
X = data.iloc[:,2:].values.astype('float64') # convertion of all type to float in order to not have DataConvertionWarning during data procession
...
# plot
feature_names = [f.replace("_encode","") for f in data.columns[2:]]
...
for column in test.columns.values.tolist():
    print(column)
    print(testnull[column].value_counts())
    print(" ")
...
test['Sex_encode'] = t.fit_transform(test['Sex']) 

test['Embarked_encode'] = t.fit_transform(test['Embarked'])
test['title_encode'] = t.fit_transform(test['title'])
