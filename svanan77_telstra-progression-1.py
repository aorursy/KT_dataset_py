...
dfloc = pd.DataFrame(data['location'].value_counts()).rename(columns={'location':'loc_count'})

# Viewing top few lines of the dataframe
dfloc.head()
# Getting dataframe details e.g columns, data types, total entries etc
dfloc.info()
#Adding loc_count as a new column in data df. 
#dfloc have lesser row than data, hence using the below way of merging and not the function
data = pd.merge(data, dfloc, how='left', left_on='location',right_index=True).fillna(0)
...
dfeve = pd.DataFrame(event_type['id'].value_counts()).rename(columns={'id':'eve_count'})
...
# Getting dataframe details e.g columns, data types, total entries etc
dfeve.info()
#event_type after one hot encoding is 'sum'med by id, this is same as 'count' too
event_type = event_type.groupby('id', sort=False).agg(sum).add_prefix('sum_').reset_index()

event_type.head()
#Adding eve_count as a new column in event_type df. 
event_type = pd.merge(event_type, dfeve, how='left', left_on='id',right_index=True).fillna(0)

# Viewing top few lines of the dataframe
event_type.head()    
len(event_type.index)
log_feature.head()
#Feature - log(1+volume) as volume too high compared with average
log_feature['log(1+volume)'] = np.log(1+log_feature.volume)
log_feature.head()
#log_feature's 'log_feature' is counted, 'volume' is 'sum'med,'average'd, minned, maxed, and std'ed
log_feature = log_feature.groupby('id', sort=False).agg(count_log_feature=('log_feature','count'), 
                                                        ...
                                                        sum_logvolume=('log(1+volume)', 'sum'),
                                                        mean_logvolume=('log(1+volume)', 'mean'),
                                                        min_logvolume=('log(1+volume)', 'min'),
                                                        max_logvolume=('log(1+volume)', 'max'),
                                                        std_logvolume=('log(1+volume)', 'std')
                                                      ).reset_index()
log_feature.head()
# Checking if there are missing values in the data frame
log_feature.isnull().sum()
#Replace null with 0
log_feature.std_volume = log_feature.std_volume.fillna(0)
log_feature.std_logvolume = log_feature.std_logvolume.fillna(0)
...
fname = "telstra_progression1_GaussinNB_predict.csv"
...
#Estimator: GaussianNB
#Best params                       : {'clf__priors': None}
#Best training data MAE score      : -0.56199486215359
#Best validation data MAE score (*): -0.5260663507109005
#Modeling time                     : 00:00:00
#y_pred                            :
#       id  predict_0     predict_1     predict_2
#0  11066   1.000000  1.733494e-13  3.056890e-20
#1  18000   0.000083  7.109276e-06  9.999101e-01
#2  16964   0.999909  9.113992e-05  4.807455e-20
#3   4795   0.000002  9.999985e-01  1.677581e-13
#4   3392   0.016024  6.779510e-01  3.060251e-01