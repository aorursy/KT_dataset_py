...
query = """SELECT CONCAT('The type "',type,'" has ',CAST(count(id) as string),' stories.') AS Prompt_1
...
"""
query = """SELECT CONCAT(CAST(COUNT(deleted) AS string),' comments have been deleted.') AS Prompt_2
            FROM `bigquery-public-data.hacker_news.full`
            WHERE deleted = True
            """
hacker_news.estimate_query_size(query)
...
query = """SELECT CONCAT(CAST(COUNTIF(deleted = True) AS string),' comments have been deleted.') AS Bonus
            FROM `bigquery-public-data.hacker_news.full`
        """
