...
filePath='/kaggle/input/energy.xlsx'

df=pd.read_excel(filePath,sheet_name='2016-2017')
df.describe()
df.info()
corrmat = df.corr()
...
drawScatter('xt5')
drawScatter('u358')
drawScatter('avg_temp')
drawScatter('duration')
from sklearn.model_selection import train_test_split
np.where(np.isnan(df))
df=df.dropna(how='any')

X=df[['ct6','xt5','u358','avg_temp','avg_humidity','duration']]
...
plt.scatter(range(1,y_test.shape[0]+1),gdbt_y_predict,label='predict' )

plt.scatter(range(1,y_test.shape[0]+1),y_test ,label='actual')
...
history=model.fit(train_data_processed, y_train,epochs=600, batch_size=16, verbose=0)
...
plt.scatter(range(1,y_test.shape[0]+1),predict_result,label='predict' )
