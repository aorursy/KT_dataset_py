...
# TODO 打印部分名字信息
data.name.head()
data['name_title'] = data.name.apply(lambda x: x.split(',')[1] if len(x.split(',')) >= 2 else x.split(',')[0]).apply(lambda x: x.split(' ')[1])
# 如果是.split()， 取[0]
# 如果是.split(' '), 取[1]
a = data.name.apply(lambda x: x.split(',')[1] if len(x.split(',')) >= 2 else x.split(',')[0])
a
a.apply(lambda x: x.split(' ')) #当string以' '开头时，比如' Miss Ma', s.split(' ')中第一个元素为' ', why?
...
def age(data):
    ...
    """
    if: pd.isnull(x.mean()) = True 说明该x group中全为nan
    else: pd.isnull(x.mean()) = False 说明该x group中不全为nan
    在计算DataFrame.mean()中skipna: default True
    """
    return data
...
model = tree.DecisionTreeClassifier(criterion='entropy', max_depth=3, max_leaf_nodes=5)
...
sub_model = tree.DecisionTreeClassifier(criterion='entropy', max_depth=3, max_leaf_nodes=5)
...
"""
In-class有NaN是因为trainX本身已经不包含label，全部的columns都是feature，所以不应做切片
如做切片应在train_test_split前的数据集data上进行
"""
