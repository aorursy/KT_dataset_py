...
nfl_ds_missing_values_count = nfl_data.isnull().sum()

sf_ds_missing_values_count = sf_permits.isnull().sum()
# look at the # of missing points in the first ten columns
nfl_ds_missing_values_count[0:10]
sf_ds_missing_values_count[0:10]
# how many total missing values do we have?
total_cells = np.product(sf_permits.shape)
total_missing = sf_ds_missing_values_count.sum()

# percent of data that is missing
(total_missing/total_cells) * 100
# your turn! Find out what percent of the sf_permit dataset is missing
total_cells = np.product(nfl_data.shape)
total_missing = nfl_ds_missing_values_count.sum()
(total_missing/total_cells) * 100
# look at the # of missing points in the first ten columns
nfl_ds_missing_values_count[0:1000]
...
subset_nfl_data = sf_permits.loc[:, 'Permit Number':].head()
subset_nfl_data
