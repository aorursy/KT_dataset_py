...
class my_gbdt(object):
    ...
    def __init__(self):
        ...
        self.tree = [DecisionTreeRegressor(min_samples_split=2, max_depth=4)

                    for index in range(self.tree_num)]
    ...
    def fit(self, X, y):
        ...
        for index in range(1,self.tree_num):

            old_tree = self.tree[index-1]

            new_tree = self.tree[index]

            pre_old = old_tree.predict(X)

            y_label = y_label-pre_old

            new_tree.fit(X,y_label)
    ...
    def predict(self, X):
        ...
        pred_proba = [self.tree[index].predict(X) for index in range(self.tree_num)]
