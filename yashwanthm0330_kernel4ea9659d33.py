...
for train, test in kfold.split(ids):
    ...
    conv_layer1 = Conv3D(filters=16, kernel_size=(32, 1, 1), strides=(32,1,1))(input_layer)
    conv_layer1 = BatchNormalization()(conv_layer1)
    conv_layer1 = Activation('relu')(conv_layer1)
    conv_layer2 = Conv3D(filters=8, kernel_size=(1, 1, 1), strides=(32,1,1))(conv_layer1)
    conv_layer2 = BatchNormalization()(conv_layer2)
    conv_layer2 = Activation('relu')(conv_layer2)
    conv_layer3 = Conv3D(filters=3, kernel_size=(1, 1, 1), strides=(32,1,1))(conv_layer2)
    ...
    flatten_layer2 = Reshape((32,32,3), input_shape=(3072,))(flatten_layer)
    x = Lambda(lambda x: K.tf.image.resize_images(x, (128, 128)), input_shape=(32, 32, 3))(flatten_layer2)
    base_model=MobileNetV2(weights='imagenet',include_top=False,input_shape=(128,128,3))(x) #imports the mobilenet model and discards the last 1000 neuron layer.
    ...
    model.fit(X, Y, batch_size=32, epochs=20, callbacks=[model_checkpoint, reduce_lr], validation_data=(x_val, y_val), shuffle=True)
    
    print()
    print("#######################################################")
    print()

    model_1 = Model(inputs=input_layer, outputs=preds)
    model_1.compile(loss='categorical_crossentropy', optimizer=opt, metrics=['accuracy', f1])
    model.load_weights('model_32.h5')
    ...
    # recall: tp / (tp + fn)
    recall1 = recall_score(testy, yhats)
...
for train, test in kfold.split(ids):
    ...
    ## convolutional layers
    conv_layer1 = Conv3D(filters=12, kernel_size=(32, 1, 1), strides=(32,1,1))(input_layer)
    conv_layer1 = BatchNormalization()(conv_layer1)
    conv_layer1 = Activation('relu')(conv_layer1)
    ...
    flatten_layer2 = Reshape((32,32,12), input_shape=(1228,))(flatten_layer)
    x = Conv2D(3, 3, padding='same')(flatten_layer2)
...
for train, test in kfold.split(ids):
    ...
    model_name = "model.h5"
    model_checkpoint = ModelCheckpoint(model_name, monitor='val_f1_rec', verbose=1, save_best_only=True, mode='max', save_weights_only=True)
    reduce_lr = ReduceLROnPlateau(factor=0.1, patience=3, min_lr=0.000001, verbose=1)
    
    
    model.compile(loss='categorical_crossentropy', optimizer=opt, metrics=['accuracy', f1_rec])
    model.fit(X, Y, batch_size=32, epochs=20, callbacks=[model_checkpoint, reduce_lr], validation_data=(x_val, y_val), shuffle=True)
    ...
    model_1 = Model(inputs=input_layer, outputs=preds)
    model_1.compile(loss='categorical_crossentropy', optimizer=opt, metrics=['accuracy', f1_rec])
    model.load_weights('model.h5')
