...
def get_paths(source_dir,output_dir,mode,subject):
    ...
    if mode =='ALL':
        ...
        # note for this data set there is a training set and a test set no validation set

        test_path=os.path.join(source_dir,'testdata')
        ...
        train_path=os.path.join(source_dir, 'traindata')

        train_path=os.path.join(train_path, 'traindata')

        valid_path=os.path.join(source_dir,'traindata')# dummy path not use
...
def make_model(classes,lr_rate, height,width,model_size, rand_seed):
    ...
    if model_size=='L':
        ...
        x=Dropout(rate=.3, seed=rand_seed)(x)
...
def make_generators( paths, mode, batch_size, v_split, classes, height, width):
    ...
    if mode != 'ALL':
        ...
        #Note for this data set split the training set into a training set and a validation set
...
source_dir=r'/kaggle/input/menwomen-classification'
