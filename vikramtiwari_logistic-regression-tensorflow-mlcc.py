...
# choose the last 500 (out of 17000) examples for validation
...
def train_linear_regressor_model(learning_rate, steps, batch_size, training_examples, training_targets, validation_examples, validation_targets):
    ...
    Args:
        ...
        steps: A non-zero `int`, the total number of training steps. A training steps consists of a forward and backward pass using a single batch
        ...
        validation_examples: A `DataFrame` containing one or more columns from `california_housing_dataframe` to use as input feautres for validation
        validation_targets: A `DataFrame` containing exactly one columns from `california_housing_datframe` to use as target for validation
    ...
    # create a linear rergessor object
...
def train_linear_classifier_model(learning_rate, steps, batch_size, training_examples, training_targets, validation_examples, validation_targets):
    ...
    Args:
        ...
        validation_targetsL A `DataFrame` containing exactly one column from `california_housing_dataframe` to use as target for validation
    ...
    for period in range(0, periods):
        ...
        # occasionally preint the current loss
