...
def calcul(start_w,end_w,num_pts):

    k=np.array(np.linspace(start = start_w, stop = end_w, num = num_pts))

    y_pred = regressor.predict(np.array(k).reshape(num_pts,1))

    return k,y_pred

#exemple:  w varie entre 10 et 100 et  on veut prendre 10 point dans cette intervale equi-espacés

input_w,output_force=calcul(10,100,10)
