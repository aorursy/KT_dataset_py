...
def starts_and_ends_with_vowel(string):
    ...
    for x in string[0] and string[-1]:

        if x in string[0] and string[-1] in vowels:
...
def mode(lst):
    ...
    mode_answer = statistics.mode(lst)
...
def get_paper_title(diktionary):

    return diktionary["title"]
...
def get_latest_paper(dik1, dik2):

    if dik1["year_published"] > dik2["year_published"]:
...
def get_average_book_price(dik):
...
def highest_price_book(lst):
    ...
    return max(lst, key=lambda x: x['price'])
