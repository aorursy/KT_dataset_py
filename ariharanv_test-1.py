...
class SigmoidNeuron:
    ...
    def grad_w_ce(self, x, y, y_pred):

        return (y_pred-y) * x

        

    def grad_b_ce(self, x, y, y_pred):
    ...
    def fit(self, X, Y, epochs=1, learning_rate=1, initialise=True, loss_fn="mse", display_loss=False):
        ...
        for i in tqdm_notebook(range(epochs), total=epochs, unit="epoch"):
            ...
            if loss_fn == 'mse':
                ...
                self.w -= (sum([self.grad_w_mse(x, y, self.sigmoid(x)) for x, y in zip(X, Y)])*learning_rate)

                self.b -= (sum([self.grad_b_mse(x, y, self.sigmoid(x)) for x, y in zip(X, Y)])*learning_rate)

                #for x, y in zip(X, Y):

                    #dw += self.grad_w_mse(x, y, self.sigmoid(x))

                    #db += self.grad_b_mse(x, y, self.sigmoid(x)) 

            elif loss_fn == "ce":

                self.w -= (sum([self.grad_w_ce(x, y, self.sigmoid(x)) for x, y in zip(X, Y)])*learning_rate)

                self.b -= (sum([self.grad_b_ce(x, y, self.sigmoid(x)) for x, y in zip(X, Y)])*learning_rate)

                #for x, y in zip(X, Y):

                    #dw += self.grad_w_ce(x, y, self.sigmoid(x))

                    #db += self.grad_b_ce(x, y, self.sigmoid(x))
...
sn_ce.fit(x_s_tr, y_tr, epochs=700, learning_rate=0.000009, loss_fn="ce", display_loss=True,  initialise = True)
...
def print_accuracy(sn, X_scaled_train, Y_train):
    ...
    Y_pred_binarised_train = (Y_pred_train >= 0.35).astype("int").ravel()
...
Y_pred_binarised_test = (Y_pred_test >= 0.35).astype("int").ravel()
