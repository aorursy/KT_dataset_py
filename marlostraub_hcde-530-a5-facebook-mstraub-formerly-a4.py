...
secret_value_0 = user_secrets.get_secret("FBAPI530")
secret_value_1 = user_secrets.get_secret("OpenCage Geocoder")
secret_value_2 = user_secrets.get_secret("OpenWeather")

token=secret_value_0 #rename the API code to a shorter variable name
graph=facebook2.GraphAPI
...
!pip install pytrends #install library for unofficial Google Trends API
!pip install pandas #required per pytrends documentation
!pip install requests #required per pytrends documentation
!pip install lxml #required per pytrends documentation

from pytrends.request import TrendReq #import this module

pytrends = TrendReq(hl='en-US', tz=360, timeout=(10,25), proxies=['https://34.203.233.13:80',], retries=2, backoff_factor=0.1) #connect to Google
#STEP 2A
...
with open('citytrackerwforecast.json', 'w') as outfile: #create the output .json encoded file named 'citytrackerwforecast' and open in write mode
    json.dump(data, outfile) #serialize the object 'data' in a json encoded stream to the outfile filepath (I'm not sure if I am using all the corret terminology)
