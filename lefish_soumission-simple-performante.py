...
def ImportData():
    ...
    path = '/kaggle/input/open-hackathon-data-science-avisia/train_booking.csv'

    train = pd.read_csv(path)

    # donnees val

    path = '/kaggle/input/open-hackathon-data-science-avisia/test_booking.csv'
...
def OneHot(train, val):
    ...
    for var in categorielles:
        ...
        for modalite in train[var].unique():

            

            # Si modalité dans l'ech de val

            if modalite in val[var].unique():
