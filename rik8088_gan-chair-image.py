...
def load_images(directory='', size=(64,64)):
    ...
    return images
def loadgray_images(directory='', size=(64,64)):
    images = []
    labels = []  # Integers corresponding to the categories in alphabetical order
    label = 0
    ...
    return images
images=load_images('../input/chairstool')
inputimg=loadgray_images('../input/inputimg')
...
class GAN():
    ...
    def train(self, epochs, batch_size=128, save_images=100, save_model=2000):
        ...
        #genimgstest = np.array(inputimg)
        #genimgstest = (genimgstest.astype(np.float32) - 127.5) / 127.5
        ...
        for epoch in range(epochs):
            ...
            imgs = X_train[idx]
            #idx1 = np.random.randint(0, genimgstest.shape[0], half_batch)
            #gen_imgs = genimgstest[idx1]

            #noise = np.random.normal(0, 1, (half_batch, self.noise_size))
            gen_imgs = self.generator.predict(inputimg[0])
    ...
    def save_images(self, epoch):
        ...
        fig.savefig("tools/chair_%d.png" % epoch)
        plt.close()
#This folder will contain the images generated during the training
!mkdir tools
gan=GAN()
gan.train(epochs=15001, batch_size=128, save_images=1000, save_model=15000)
