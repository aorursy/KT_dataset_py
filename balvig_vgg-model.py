...
print(check_output(["ls", "../input/flickr8k/flickr8k"]).decode("utf8"))
...
train_features = load_photo_features('features.pkl', train_ids)
...
test_features = load_photo_features('features.pkl', test_ids)
...
checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='min')
generator = data_generator(tokenizer, max_length, train_descriptions, train_features)
model=load_model('../input/epochs-9/model_19.h5')
model.summary()
...
# load the model
filename = '../input/epochs-9/model_19.h5'
...
max_length = 34
filename = '../input/epochs-9/model_9.h5'
model = load_model(filename)
photo = extract_features('../input/images/skateboards.png')
# generate description
