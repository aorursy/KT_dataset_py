...
missing_values_sf_permits
# percentage
total_cell = np.product(sf_permits.shape)
total_cell
total_missing = missing_values_sf_permits.sum()
total_missing
...
sf_permits_dropped_columns

# just how much data did we lose?
print("Columns in original dataset: %d \n" % sf_permits.shape[1])
print("Columns with na's dropped: %d" % sf_permits_dropped_columns.shape[1])
...
sf_permits_subset = sf_permits.loc[:, 'Street Number':'Record ID']
sf_permits_subset.fillna(0)
# comes directly after it and then 
sf_permits_subset.fillna(method='bfill', axis=0)