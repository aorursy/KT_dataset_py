...
mar
sum(deaths["InjuryAtWork"] == 'Y') / len(deaths)
pla= pd.read_csv('../input/PlaceOfInjury.csv')
pla
print([x for x in icd10["Description"] if x.lower().find("gun") != -1])
deaths["Age"]
...
ggplot(aes(x='Age', fill='Sex'), data=ms) + \
    geom_density(alpha=0.6)
