...
trainImages, trainLabels = (Images, Labels)
...
model.summary()
amountToTrain = int(0.6*len(trainImages))

resultTrain = model.fit(trainImages,trainLabels,epochs=40, validation_split=0.20)
...
# Decoding predictions for writing data

import os

imageNames = []

for dirname, _, filenames in os.walk('/kaggle/input/exam-minor-2019/img_kagl_test/test/'):
...
maximumValues = prediction.argmax(axis=1)

imageNamesId = []

imageTypesId = []

for (name, photoType) in zip(imageNames,maximumValues):

    imageNamesId.append(name)

    imageTypesId.append(photoType + 1)

resultFrame = pd.DataFrame(data = {'Image':imageNamesId, 'Type':imageTypesId})

resultFrame.to_csv(path_or_buf = '/kaggle/working/result.csv', index=False)
