...
class Criterion:
    ...
    def __call__(self, epoch,global_feat, local_feat, cls_score, label,Att_M,Aff_theta,M):
        ...
        if epoch<=20:
            total_loss=global_loss + local_loss + cls_loss+0.0005*center_loss+16*Att_loss+16*Aff_loss
        else:
            total_loss=global_loss + local_loss + cls_loss+0.0005*center_loss+16*Att_loss+16*Aff_loss+10*L_bcn
    ...
    for epoch in range(epochs):
        with tqdm(total=len(train_dl)) as pbar:
            ...
            for image, labels in train_dl:
                ...
                loss = criterion(epoch,global_feat, local_feat, cls_score,labels,Att_M,Aff_theta,M)
                train_loss += loss
                loss.backward()
                optimizer.step()
                pbar.update(1)
...
# valid_ds=TestDataset(valid_names,image_labels,transform_valid)
# train_dl=Data.DataLoader(train_ds,batch_size=6,collate_fn=train_collate,shuffle=True,drop_last=True)
# valid_dl=Data.DataLoader(valid_ds,batch_size=8,drop_last=True)
...
m=m.view(-1,6)
m[0]
images=torch.rand((16,3,448,448))
model = torchvision.models.resnet50(pretrained=True)
print(model)
act1 = Dynamic_relu_b(64)
