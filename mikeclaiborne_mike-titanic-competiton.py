...
for dataset in combine:
    ...
    dataset['Title'] = dataset['Title'].replace(['Lady', 'Countess','Capt', 'Col',\
 	...
 	'Don', 'Dr', 'Major', 'Sir', 'Jonkheer', 'Dona'], 'Rare')
...
title_mapping = {"Mr": 1, "Miss": 2, "Mrs": 3, "Master": 4, "Rev": 5, "Rare": 0, "None": 6}

for dataset in combine:

    dataset['Title'] = dataset['Title'].map(title_mapping)

    dataset['Title'] = dataset['Title'].fillna(6)
...
for dataset in combine:
    ...
    for i in range(0, 6):
        ...
        for j in range(0, 3):
            ...
            guess_df = dataset[(dataset['Title'] == i) & \

                                   (dataset['Pclass'] == j+1)]['Age'].dropna() 

                                    #MWC: changed Sex to Title

            #print("i/Title:", i, ", j/Pclass:", j)

            #print("guess_df:", guess_df)
            ...
            if np.isnan(age_guess):
                ...
                age_guess = dataset[(dataset['Title'] == i)]['Age'].median()

                #print("age_guess new:", age_guess)
    ...
    for i in range(0, 6):
        ...
        for j in range(0, 3):
            ...
            dataset.loc[ (dataset.Age.isnull()) & (dataset.Title == i) & (dataset.Pclass == j+1),\
...
train_df[['Age*Class', 'Survived']].groupby(['Age*Class'], as_index=False).mean().sort_values(by='Age*Class', ascending=False)
...
submission.to_csv('mike_submission.csv', index=False)