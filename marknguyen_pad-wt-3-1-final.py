...
SIZE = 1_000_000 # For numbers, Python ignores the underscore to make reading large numbers easier to read

time_list = [i for i in range(SIZE)]

time_array = np.array(test_list)
...
test_list = [i+1 for i in range(10)]
...
print("adding scalar:", test_array + 5)

print('subtracting scalar:', test_array - 3)

print('dividing scalar:', test_array / 2) # multiplication works too

print('raise to power of scalar:',test_array**2)



## Add and operate on other vectors 

print('\nadding vector:', test_array + test_array)

print('dividing vector:', test_array / test_array)

print('multiplying vector:', test_array * test_array)
...
heights = pd.Series(height_values, index=students)
...
print(heights.iloc[4])

print(heights.loc[name])
...
print("\n{}'s height: \n{}".format("'s and ".join(name), heights.loc[name]))



## Subset based on conditions, use the loc function  

tall = heights.loc[heights >= 69]

print("\nPeople taller than 69 inches:\n", tall)
# Demo: Simply analyze the code and run the code cell
...
test_Series = pd.Series([i for i in range(10)])
