...
def col_choose(data, col):
    '''Only returns a dataframe where the columns in col are not included'''
    if type(col) is str: col = [col]
    columns = list(data.columns)
    leave = list(set(columns) - set(col))
    return data[list(set(col))]

def col_merge(data, col):
    '''Only returns a dataframe where the columns in col are not included'''
    #if type(col) is str: col = [col]
    columns = list(data.columns)
    new_col = list(col.columns)
    leave = list(set(columns) + set(new_col))
    return leave
files()
tally(data['name_income_type'])
### write by chengzhu xie
data = read('data-cleaning-for-projectx/application_for_clustering.csv')
data_child = col_choose(data,['target','cnt_children','code_gender'])
#data_income = col_choose(data,['target','cnt_children','name_income_type_businessman','name_income_type_commercial_associate'
#                        ,'name_income_type_maternity_leave','name_income_type_pensioner','name_income_type_state_servant','name_income_type_student',
#                        'name_income_type_unemployed','name_income_type_working'])
#data_gender = col_choose(data,['target','code_gender_f','code_gender_m'])

### cleaning data 将data里所有的内容都转换成数字的
income_type = data['name_income_type']
education_type = data['name_education_type']
gender = data['code_gender']

gender = replace(gender,[['M',1],['F',0]])
income_type = replace(income_type,[['Student',0],
                                   ['Unemployed',1],
                                   ['State servant',2],
                                   ['Commercial associate',3],
                                  ['Pensioner',4],
                                   ['Working',5]])
education_type = replace(education_type,[['Academic degree',0],
                                         ['Incomplete higher',1],
                                         ['Lower secondary',2],
                                         ['Higher education',3],
...
#data_numeric = col_choose(data,['target','cnt_children','region_population_relative'])
#data_numeric = col_merge(data_numeric,gender) #怎么把俩个data放在一起
...
head(data,5)
a = tally(data_child['cnt_children'])
a
plot(data=data, x='cnt_children', y = 'code_gender',column='target',style='scatterplot')
plot(data=data, x='name_income_type', y = 'cnt_children',column='target',style='scatterplot')
data_children_income_region = exclude(data,['target','code_gender','name_education_type'])
data_children_education_region = exclude(data,['target','name_income_type','code_gender'])
head(data_children_education_region,2)
X = data_children_income_region.values
# Data Normalization
from sklearn.preprocessing import StandardScaler
X_std = StandardScaler().fit_transform(X)

model = PCA(n_components = 3)
model.fit(data_children_income_region)
%matplotlib inline

X = model.transform(X_std)
...
### INPORTANT
# As we can see, from k = 5, it changes slow. 
# So I selected the value of K = 5 for the elbow point as shown in the figure
# 3 KMEANs CLUSTERING
clf = KMeans(n_clusters=4)
# Compute cluster centers and predict cluster indices
X_clustered = clf.fit_predict(X)
numSamples =len(X)
...
for i in range(numSamples):
    plt.plot(X[i][0],X[i][1],mark[clf.labels_[i]])
...
### 下面这个图是data_children_income_region，3个columns
# 3 KMEANs CLUSTERING
...
plt.show()
### 接下来是
X = data_children_education_region.values
# Data Normalization
from sklearn.preprocessing import StandardScaler
X_std = StandardScaler().fit_transform(X)

model = PCA(n_components = 3)
model.fit(data_children_education_region)
%matplotlib inline
...
### 下面这个图是 data_children_education_region，3个columns
