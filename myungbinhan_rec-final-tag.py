...
def return_tag_len(row):
    return len(row['tag_morphs_tfidf_label2'])
...
tag = df_ques[(df_ques['tag_len'] > 0)]
tag = tag[[ 'id', 'tag_morphs_tfidf_label2', 'songs',  'updt_date']]
tag
...
df_total_2 = df_total_2[df_total_2['id'].isin(tag.id.to_list())].reset_index(drop=True)
df_total_2
...
for idx in tqdm(df.index[10 : 10+total_length], desc='rec songs'):
    cur_row = df.iloc[idx]
