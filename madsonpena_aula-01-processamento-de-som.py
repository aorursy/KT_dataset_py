...
print("Max:",peak_max,"Min:",peak_min)
peak_max = sound_ch1.max()
peak_min = sound_ch1.min()
...
df_letras.head()
segundo = np.arange(windows.shape[0])
maximo = peaks_max
minimo = peaks_min
media = means
df = pd.DataFrame(data=np.vstack([segundo, maximo, minimo, media]).T, columns = ['Segundo', 'Máximo', 'Mínimo', 'Média'])
df.head()
