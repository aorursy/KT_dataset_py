...
bird.shape
bird.describe()
size_of_each_group = bird.groupby("type").size().sort_values(ascending=True)
...
smallest_group = bird_extended.groupby("type").size().argmin()

smallest_size = bird_extended.groupby("type").size().min()



bird_balanced = pd.concat(

    [bird_extended[bird_extended.type==g].sample(smallest_size) for g in bird_extended.type.unique()]
...
train_features, test_features, train_labels, test_labels = train_test_split(
    ...
    bird_balanced[feature_columns_extended], bird_balanced.type,
...
predict_labels = lr.predict(test_features)
...
predict_labels_encode = model.predict_classes(test_features.values, verbose=0)
print(classification_report(test_labels_encode, predict_labels_encode))