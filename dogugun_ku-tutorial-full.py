...
accuracy_score(y_test, y_pred_test)
model_lr = LogisticRegression(C=100, class_weight=None, dual=False, fit_intercept=True,
...
indices = np.random.choice(np.arange(y_base.size), replace=False,

                           size=int(y_base.size * 0.8))
...
plt.plot(fpr, tpr, label = 'RF score: ' + str(round(auc, 5)))
