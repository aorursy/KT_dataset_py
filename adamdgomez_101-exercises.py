...
def mode(lst):
    ...
    while i < len(lst):
        ...
        i += 1
    d1 = dict(zip(lst, L1))
    d2 = [key for key, value in d1.items() if value == max(L1)]
    #This won't work if there is more than one mode. Could use a dictionary instead of a list for d2, but then I don't know how to capture the keys from that dictionary and turn them into returnable values
    return d2[0]
...
def only_negative_numbers(nums):
    negative_nums = []
    for number in nums:
        if number < 0:
...
def only_positive_evens(nums):
    pos_evens = []
    for number in nums:
        if (number % 2 == 0) and (number > 0):
            pos_evens.append(number)
    return pos_evens
...
def only_positive_odds(nums):
    pos_odds = []
    for number in nums:
        if (number % 2 == 1) and (number > 0):
            pos_odds.append(number)
    return pos_odds
...
def only_negative_evens(nums):
    neg_evens = []
    for number in nums:
        if (number % 2 == 0) and (number < 0):
            neg_evens.append(number)
    return neg_evens
...
def only_negative_odds(nums):
    neg_odds = []
    for number in nums:
        if (number % 2 == 1) and (number < 0):
            neg_odds.append(number)
    return neg_odds
...
def shortest_string(lst):
    shortest_index = 0
    shortest_string = len(lst[0])
    for i in range(len(lst)):
        if len(lst[i]) < shortest_string:
            shortest_string = len(lst[i])
            shortest_index = i
    return lst[shortest_index]
...
def longest_string(lst):
    longest_index = 0
    longest_string = len(lst[0])
    for i in range(len(lst)):
        if len(lst[i]) > longest_string:
            longest_string = len(lst[i])
            longest_index = i
    return lst[longest_index]
...
def get_values_not_in_common(lst1, lst2):
    ...
    for number in lst1:
        if number not in lst2:
            uncommon_values.append(number)
    for number in lst2:
        if number not in lst1:
            uncommon_values.append(number)
    return set(uncommon_values)
...
def total_of_book_prices(dictlst):
    total_prices = 0
    prices = [dictionary['price'] for dictionary in dictlst]
    for price in prices:
        total_prices += price
    return total_prices
...
def get_average_book_price(dictlst):
    total_prices = 0
    prices = [dictionary['price'] for dictionary in dictlst]
    for price in prices:
        total_prices += price
...
def highest_price_book(dictlst):
    prices = [dictionary['price'] for dictionary in dictlst]
    highest_price_value = prices[0]
    highest_price_index = 0
    for i in range(len(prices)):
        if prices[i] > highest_price_value:
            highest_price_value = prices[i]
            highest_price_index = i
    return dictlst[highest_price_index]
...
def lowest_price_book(dictlst):
    prices = [dictionary['price'] for dictionary in dictlst]
    lowest_price_value = prices[0]
    lowest_price_index = 0
    for i in range(len(prices)):
        if prices[i] < lowest_price_value:
            lowest_price_value = prices[i]
            lowest_price_index = i
    return dictlst[lowest_price_index]
...
def total_number_of_items(cart):
    quantities = [dictionary['quantity'] for dictionary in cart['items']]
    quant_count = 0
...
def get_average_item_price(cart):
    total_price = 0
    prices = [dictionary['price'] for dictionary in cart['items']]
    for price in prices:
        total_price += price
...
def most_spent_on_item(cart):
    ...
    price_and_quantity = []
    for i in range(len(prices)):
        price_and_quantity.append((quantities[i] * prices[i]))
    max_price = price_and_quantity[0]
    max_index = 0
    for index in range(len(price_and_quantity)):
        if price_and_quantity[index] > max_price:
            max_price = price_and_quantity[index]
            max_index = index
    return cart['items'][max_index]
