...
label_count.plot(kind='bar', title='Label Distribution');
def get_class_weights(train_targets):
    ...
    return dict(zip(targets, class_weights))
...
class_weigths = get_class_weights(y_train)
...
for i, prediction in enumerate(predictions, start=0): 
    ...
    results = results.append({'ImageId': i, 'Label': prediction}, ignore_index=True)
