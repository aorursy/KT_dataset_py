...
# df_todos.head(3)
# novas_colunas_ohe_sex = pd.get_dummies(df_todos['Sex'])
# df_todos = pd.concat([df_todos,novas_colunas_ohe_sex], axis=1) # axis = 1 concatena colunas. axis = 0 concatena linhas
# df_todos.head(3)
...
df_todos['Embarked_int'] = pd.factorize(df_todos['Embarked'])[0]
df_todos.drop('Sex', axis=1, inplace=True)
df_todos.drop('Embarked', axis=1, inplace=True)
df_todos.drop('Pclass', axis=1, inplace=True)
df_todos.head(3)

passenger_Id = df_todos['PassengerId'] # precisamos guardar para fazer a submissão para o kaggle

df_todos_final = df_todos.drop(['PassengerId', 'Name', 'Ticket', 'Cabin'], 
...
nfolds=5
kf = KFold(n_splits=nfolds, shuffle=True, random_state=111)
...
def avalia_classificador(clf, kf, X, y, f_metrica, X_test):
    y_full_test =[] ##Aqui guardamos as previsões de cada modelo (classificador) em todo o dado de teste
    metrica = []
    for train, valid in kf.split(X,y):
        ...
        ##nesses dados
        y_full_test.append(clf.predict(X_test))
    return y_full_test, np.array(metrica).mean()
# X_test.shape
# x_valid.shape
from sklearn.preprocessing import MinMaxScaler, StandardScaler
from sklearn.ensemble import RandomForestClassifier
X_n = StandardScaler().fit_transform(X_train)
#X_n = PolynomialFeatures(2, interaction_only=True).fit_transform(X_n)

rf = RandomForestClassifier(n_estimators=700)
preds, media_acuracia = avalia_classificador(rf, kf, X_n, y_train, accuracy_score, X_test) 
