# kaggle titanic python solution and tutorial
import numpy as np 
import pandas as pd
...
g.add_legend()
plt.show()
# Does the title has more information
...
# Filling missing age with respect Title mean is better idea than just with mean of whole age column
...
test['Age'] = test['Age'].fillna(test['Title'].map(t_age))
new = train['Age']
print(new.describe())
...
for ind, val in enumerate(['Age', 'Fare']):
    ...
    ax[ind].legend(['Not Survived', 'Survived'])
