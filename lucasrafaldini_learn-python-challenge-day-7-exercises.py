...
def prettify_graph(graph):
    ...
    formatter_x = ticker.FormatStrFormatter('%1.0f tries')
    graph.yaxis.set_major_formatter(formatter_y)
    graph.xaxis.set_major_formatter(formatter_x)
...
full_dataset[5]['name'] = str(None) #Estou meio incrédulo que era só isso, mas vou deixar essa solução aqui por ora.
...
def blackjack_hand_greater_than(hand_1, hand_2):
    ...
    def convert_cards(lista):
        cards = []
        i=0
        letters = ['A','K','J','Q']
        for i in range(len(lista)):
            if lista[i] in letters:
                if lista[i] == 'A':
                    cards.append(11)
                if lista[i] == 'K':
                    cards.append(10)
    ...
    hand_1 = convert_cards(hand_1) 
    hand_2 = convert_cards(hand_2)
    
    if hand_1 > 21:
        hand_1 = False
    if hand_2 > 21:
        return bool(hand_1)
    if hand_1 <= 21 and hand_1 > hand_2:
        return True
    else:
        return False
...
def my_agent(wheel):
    pass
####Ainda não entendo essas problemas das roletas. É somente uma dificuldade conceitual, mas é melhor não ficar muito tempo empacado com isso###
