...
epochs=50
batch_size=500
train_data=pd.read_csv('train_data.csv',header=None,skiprows=1,usecols=range(0,10))
test_data=pd.read_csv('test_data.csv',header=None,skiprows=1,usecols=range(0,9))
...
data_loader=torch.utils.data.DataLoader(dataset=train_dataset,
                                        ...
                                        shuffle=True,
                                        drop_last=True)
linear1=torch.nn.Linear(9,128,bias=True)
...
model=torch.nn.Sequential(linear1,tanh,
                          linear2,tanh,
                          linear3).to(device)
...
!kaggle competitions submit -c stay-or-leave -f submission.csv -m "Message"
