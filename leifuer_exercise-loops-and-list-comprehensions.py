...
def has_lucky_number(nums):
    ...
    lucky_list = [num for num in nums if num % 7 ==0]

    return sum(lucky_list)>0
...
def elementwise_greater_than(L, thresh):
    ...
    return [result > thresh for result in L]
...
def estimate_average_slot_payout(n_runs):
    ...
    net_profit = [play_slot_machine() for _ in range(n_runs)]

    return (sum(net_profit)-len(net_profit))/len(net_profit)



estimate_average_slot_payout(200000)
q4.check()
