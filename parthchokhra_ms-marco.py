...
!ls
!ls ../input/
train_path = "../input/ms-marco-dataset/train.csv"
...
#                 true_false = "no"
#             input_ = ": "+ input_ + ' </s>'
            input_ = "question: %s context: %s </s>" % (context,question)
            target = "%s </s>" %(target)

            # tokenize inputs
            tokenized_inputs = self.tokenizer.batch_encode_plus(
                [input_], max_length=400, pad_to_max_length=True, return_tensors="pt"
            )
            # tokenize targets
            tokenized_targets = self.tokenizer.batch_encode_plus(
                [target], max_length=120, pad_to_max_length=True, return_tensors="pt"
            )

            self.inputs.append(tokenized_inputs)
            self.targets.append(tokenized_targets)
dataset = GrammerDataset(tokenizer, '../input/ms-marco-dataset', 'valid', 128)
...
!ls
!pwd
args_dict.update({'data_dir': '../input/ms-marco-dataset', 'output_dir': '/kaggle/working/result', 'num_train_epochs':1,'max_seq_length':900})
...
!pwd
!ls
!mv pytorch_model.bin /kaggle/working
