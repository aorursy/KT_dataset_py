...
df1 = pd.read_csv('/kaggle/input/hindi-text-short-summarization-corpus/test.csv', delimiter=',', nrows = nRowsRead)
...
graph_df['text'].hist(bins = 25, range=(0, 400))
...
#model.load_weights('/kaggle/input/hindi-text-summarisation-using-lstm-in-keras/LSTM_1_weights_2.h5')

#model.load_weights(checkpoint_path)

model.compile(optimizer='rmsprop', loss='sparse_categorical_crossentropy')

#loss, acc = model.evaluate([x_tr,y_tr[:,:-1]],  y_tr.reshape(y_tr.shape[0],y_tr.shape[1], 1)[:,1:], verbose=2)
...
model.save_weights('LSTM_1_weights_2.h5')