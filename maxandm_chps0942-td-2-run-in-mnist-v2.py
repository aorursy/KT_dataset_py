...
#calcul d'accuracy sur l'ensemble de test
test_acc = pd.read_csv('../input/accuracy/mnist_test.csv')
...
#et pour l'ensemble complet de test
test_acc_label = test_acc["label"]
test_acc.drop("label", axis = 1 , inplace=True)
test_acc_label.value_counts().sort_index()
# Normalisation des valeurs des images [0-255] -> [0.0-1.0]
train      = train/255.0
test100    = test100/255.0
evaluation = evaluation/255.0
test_acc = test_acc/255.0
...
test_acc_image = np.array(test_acc).reshape(-1,28,28,1)
...
test_acc_label_cat = to_categorical(test_acc_label) #pour le test sur l'ensemble de test
# import de Keras 
...
#validation_data permet de calculer l'accuracy sur l'ensemble des donnees de test (les 10 000 images)
classifier.fit(train_image, train_label_cat,
               batch_size=batch_size,
               validation_data=(test_acc_image, test_acc_label_cat),
               epochs=epochs, 
