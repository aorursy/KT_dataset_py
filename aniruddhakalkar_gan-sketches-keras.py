...
class GAN():
    ...
    def get_batch(self, image_files, width, height, mode):
        #print(image_files)
        data_batch = np.array([self.get_image(sample_file[0], width, height, mode) for sample_file in image_files])
        noise_batch= np.array([self.get_image(sample_file[1], 64, 64, 'L') for sample_file in image_files]).reshape((len(image_files),4096))
...
if __name__ == '__main__':
    gan = GAN()
    gan.train(epochs=24000, batch_size=188, save_interval=200)
