print ("please note that if some graphs appear blank, selectivly running the respective code box for the graph will display it")
import numpy as np
%matplotlib notebook
...
period_osc = (2*3.1415926535897932384626)/(omega)                     # calculate the period of the oscillation (from the input variables)
...
plt.show()
plt.figure(11)
plt.plot(time,exact_velocity_x,linewidth=3,label='x velocity')
plt.plot(time,exact_velocity_y,linewidth=3,label='y velocity')
plt.legend()
plt.title(f"x and y component of particle's velocity vs. time")
plt.xlabel(f"time [s]") 
...
def euler(yin,omega,step):
    ...
    return(np.add(yin,step*der(yin,omega)))
...
steps_rev =  100000    # enter the number of time steps per revolution 
...
print("increasing the steps per revolution makes the solution much more acurate the the margin or error in eulers method shrinks when smaller steps are used, as an example i used 100x more steps than before and the exact position is much closer")
...
plt.legend
plt.show()
print("the figures show that the doubling and halving of the initial velocity caused a respective doubling and halving in the max positional values for our particle and hence also the radius of the paths. due to r =mv/(Bq) This is what would be expected as we can see that radius is directly proportional to the velocity of the motion.")
q_m_ratio =  1      # charge-to-mass ratio
b0 = 1              # magnetic field (homogeneous, in z direction) 
...
num_error_x=((position_x)/exact_position_x)
num_error_y=((position_y)/exact_position_y)

print(num_error_y)
print(num_error_x)
print("my best idea was to use the code that was already given to us to show the margin of error by using our exact and calculated positions and deviding them against each other. the way this has been done shows that with the varibles used in the set directly above, on the end time step there is a 17.108% diffrence in y values and hence a 17% error. this has been done in a way that allows for altering of the variables and re examination.")
