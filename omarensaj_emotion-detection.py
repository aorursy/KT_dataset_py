...
image = img_to_array(image)
image = np.expand_dims(image, axis=0)
first = data_test['Innocent_15.jpg']
predict = model.predict(first)
...
data_predict = pd.DataFrame.from_dict(data_predict, orient='index')
data_predict
final_df = pd.DataFrame(data_predict.items(),columns=['Image','Emotion'])
final_df.head()
mapping_emotion = {0:'anger', 1:'disgust', 2:'fear', 3:'happiness', 6:'neutral', 4:'sadness', 5:'surprise'}
final_df['Emotion'] = final_df['Emotion'].map(mapping_emotion)
final_df.to_csv('submision.csv')
