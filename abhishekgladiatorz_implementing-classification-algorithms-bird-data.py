...
data = pd.read_csv('../input/birddata1/bird.csv')
data.head(3)
data['type'].unique()
data.describe()
data.isnull().any()
...
svm_svc3 = SVC(kernel = 'rbf',random_state = 0 , gamma = 0.35 ,C=10.0)
...
random_clf1 = RandomForestClassifier(n_estimators=50,criterion='gini',n_jobs=1)
...
#making other copy of data and label encoding y and splitting data again to implement this test 
y1 = y
x1 = x
...
model.fit(x1_train, y1_train, epochs = 80, batch_size = 2)
