...
myMLP, train_cost, train_accuracy, test_cost, test_accuracy=train_mlp(myMLP,X_train[:,:2000], y_train[:,:2000], X_test[:,:2000], y_test[:,:2000],  alphagrad=0.1)
def attaque_montee_gradient(iterations, epsilons, X_test):





    perturbations_L2_moyennes = []
    ...
    for epsilon in epsilons:
        ...
        for i in range(len(X_test)):
            ...
            image_perturbee = X_test[:,i:i+1].copy()

            pred_initiale = np.argmax(myMLP.M_forwardPropagation(image_perturbee))
            ...
            for j in range(iterations):
                ...
                myMLP.M_forwardPropagation(image_perturbee)

                perturbations = np.sign(myMLP.M_fgsm(image_perturbee,y_test[:,i:i+1].reshape(1,1)))

                image_perturbee = image_perturbee + epsilon*perturbations

                image_perturbee = np.where(image_perturbee > 1, np.ones(image_perturbee.shape) ,image_perturbee)

                image_perturbee =  np.where(image_perturbee < 0, np.zeros(image_perturbee.shape) ,image_perturbee)

            

            nouvelle_prediction = np.argmax(myMLP.M_forwardPropagation(image_perturbee))     

            attaque_reussie.append(pred_initiale != nouvelle_prediction)

            perturbations_L2.append(np.linalg.norm(image_perturbee - X_test[:,i:i+1]))

    

        perturbations_L2 = np.array(perturbations_L2)

        attaque_reussie = np.array(attaque_reussie)



        taux_de_reussite = len(attaque_reussie[np.argwhere(attaque_reussie == True)])/len(attaque_reussie)

        perturbation_L2_moyenne = np.mean(perturbations_L2)

    

        taux_de_reussites.append(taux_de_reussite)

        perturbations_L2_moyennes.append(perturbation_L2_moyenne)

        

    return(perturbations_L2_moyennes,taux_de_reussites)

perturbations_L2_1_iterations,taux_de_reussites_1_iterations = attaque_montee_gradient(1, np.linspace(0.01,0.8, num = 10), X_test)



plt.figure()

plt.plot(perturbations_L2_1_iterations,taux_de_reussites_1_iterations)
perturbations_L2_2_iterations,taux_de_reussites_2_iterations = attaque_montee_gradient(2, np.linspace(0.01,0.5, num = 10), X_test)



plt.figure()

plt.plot(perturbations_L2_2_iterations,taux_de_reussites_2_iterations)
perturbations_L2_5_iterations,taux_de_reussites_5_iterations = attaque_montee_gradient(5, np.linspace(0.01,0.2, num = 10), X_test)



plt.figure()

plt.plot(perturbations_L2_5_iterations,taux_de_reussites_5_iterations)
perturbations_L2_10_iterations,taux_de_reussites_10_iterations = attaque_montee_gradient(10, np.linspace(0.01,0.2, num = 10), X_test)



plt.figure()

plt.plot(perturbations_L2_10_iterations,taux_de_reussites_10_iterations)
perturbations_L2_50_iterations,taux_de_reussites_50_iterations = attaque_montee_gradient(50, np.linspace(0.001,0.05, num = 10), X_test)



plt.figure()

plt.plot(perturbations_L2_50_iterations,taux_de_reussites_50_iterations)
import pylab

pylab.plot(perturbations_L2_50_iterations,taux_de_reussites_50_iterations, label ="50")

pylab.plot(perturbations_L2_10_iterations,taux_de_reussites_10_iterations, label ="10")

pylab.plot(perturbations_L2_5_iterations,taux_de_reussites_5_iterations, label ="5")

pylab.plot(perturbations_L2_2_iterations,taux_de_reussites_2_iterations, label ="2")

pylab.plot(perturbations_L2_1_iterations,taux_de_reussites_1_iterations, label ="1")
