...
QUERY = (
        ...
        f"""
            ...
            SELECT
                ...
                u.reputation, u.up_votes, u.down_votes, u.views,

                b.n_badges, c.n_comments, q.n_questions, a.n_answers,

                c.avg_score_comments,

                q.avg_score_questions, q.avg_answers_questions, avg_comments_questions, q.avg_views_questions, q.avg_tags_questions,

                a.avg_comments_answers, a.avg_score_answers,

                b.n_badges / (DATE_DIFF('2020-03-01', DATE(u.creation_date), DAY)+1) as avg_badges_day,
                ...
                DATE_DIFF('2020-03-01', DATE(u.creation_date), DAY) as n_days,
...
std_array
km = DBSCAN(eps=2, min_samples=3)



res = km.fit_predict(std_array)
...
for n in set(res):
    ...
    cluster[n] = df.loc[lambda a: a.cluster == n].mean()
...
for n in set(res):
    ...
    std_cluster[n] = pd.DataFrame(std_array).loc[res == n].mean()

    

std_cluster.index = cluster.index[:21]
