...
TRAIN_FILE_PATH = '../input/emotion-dataset-neutral-joy-sadness-anger/emotion_dataset/train.csv'

VALID_FILE_PATH = '../input/emotion-dataset-neutral-joy-sadness-anger/emotion_dataset/test.csv'
...
class BertForSentimentClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.cls_layer = nn.Linear(config.hidden_size, 4)
...
train_set = EmotionDataset(filename=TRAIN_FILE_PATH, maxlen=MAX_LEN_TRAIN, tokenizer=tokenizer)

valid_set = EmotionDataset(filename=VALID_FILE_PATH, maxlen=MAX_LEN_VALID, tokenizer=tokenizer)
## Data Loaders
...
def classify_emotion(sentence):
    ...
    with torch.no_grad():
        ...
        if output == 0:
            ...
            print('Sadness {}'.format(int(prob*100)))

        elif output == 2:

            print('Joy {}'.format(int(prob*100)))

        else:

            print('Anger {}'.format(int(prob*100)))
sentence = "If this notebook helped you. Just give me an upvote"

classify_emotion(sentence)