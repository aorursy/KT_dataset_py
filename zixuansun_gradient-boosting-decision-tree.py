...
class my_gbdt(object):
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_estimators):
            ...
            y_pred -= np.multiply(self.learning_rate, self.tree[i].predict(X))
    ...
    def predict(self, X):
        ...
        pred_proba = self.tree[0].predict(X)

        

        for i in range(self.n_estimators):

            pred_proba -= np.multiply(self.learning_rate, self.tree[i].predict(X))      
