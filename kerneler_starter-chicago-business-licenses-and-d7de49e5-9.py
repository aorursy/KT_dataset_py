...
# business-licenses.csv has 943504 rows in reality, but we are only loading/previewing the first 100 rows
df1 = pd.read_csv('../input/business-licenses.csv', delimiter=',', nrows = nRowsRead)
df1.dataframeName = 'business-licenses.csv'
nRow, nCol = df1.shape
...
nRowsRead = 100 # specify 'None' if want to read whole file
# business-owners.csv has 272441 rows in reality, but we are only loading/previewing the first 100 rows
df2 = pd.read_csv('../input/business-owners.csv', delimiter=',', nrows = nRowsRead)
df2.dataframeName = 'business-owners.csv'
nRow, nCol = df2.shape
print(f'There are {nRow} rows and {nCol} columns')
df2.head(5)
