...
class ResnetBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn, repetitions):
        ...
        f2= Dense(units=64, kernel_initializer="he_normal",
                      activation="relu")(f2)
        f2=Dropout(0.5)(f2)
...
ind=int(len(indices)*0.30)
# train data
X_val2=X_train_all[indices[:ind]] 
y_val2=y_train_all[indices[:ind]]
# validation data
X_train2=X_train_all[indices[-(len(indices)-ind):]] 
y_train2=y_train_all[indices[-(len(indices)-ind):]]
...
#from keras.preprocessing.image import ImageDataGenerator
path_model2='model_simple_keras_starter2.h5' # save model at this location after each epoch
#K.tensorflow_backend.clear_session() # destroys the current graph and builds a new one
# create the model
checkpointer2 = ModelCheckpoint('model_simple_keras_starter2.h5', verbose=1, save_best_only=True)
#K.set_value(model.optimizer.lr,1e-3) # set the learning rate
# fit the model

model2.fit(X_train2, y_train2, batch_size=128,
            epochs=50, 
            verbose=1, 
...
model2.load_weights('model_simple_keras_starter2.h5')
score = model2.evaluate(X_val2,y_val2, verbose=0)
print(len(X_val2))
print('Test accuracy:', score[1])
predictions_prob2=model2.predict(X_test_all)
n_sample=200
np.random.seed(42)
ind=np.random.randint(0,len(X_test_all), size=n_sample)

imshow_group(X=X_test_all[ind],y=None,y_pred=predictions_prob2[ind], phase='prediction')
pf=(predictions_prob2+predictions_prob)/2.0
imshow_group(X=X_test_all[ind],y=None,y_pred=pf[ind], phase='prediction')
indices=list(range(len(X_train_all)))
np.random.seed(100)
np.random.shuffle(indices)
...
ind=int(len(indices)*0.30)
# train data
X_val3=X_train_all[indices[:ind]] 
y_val3=y_train_all[indices[:ind]]
# validation data
X_train3=X_train_all[indices[-(len(indices)-ind):]] 
y_train3=y_train_all[indices[-(len(indices)-ind):]]
from keras.layers import  GlobalMaxPooling2D
from keras.optimizers import Adam
def build_model2( ):
    
    image_input = Input( shape = (28, 28, 1), name = 'images' )
...
model3= build_model2( )
#from keras.preprocessing.image import ImageDataGenerator
path_model3='model_simple_keras_starter3.h5' # save model at this location after each epoch
#K.tensorflow_backend.clear_session() # destroys the current graph and builds a new one
# create the model
checkpointer3 = ModelCheckpoint('model_simple_keras_starter3.h5', verbose=1, save_best_only=True)
#K.set_value(model.optimizer.lr,1e-3) # set the learning rate
# fit the model

model3.fit(X_train3, y_train3, batch_size=128,
            epochs=25, 
            verbose=1, 
...
model3.load_weights('model_simple_keras_starter3.h5')
score = model3.evaluate(X_val2,y_val2, verbose=0)
print(len(X_val3))
print('Test accuracy:', score[1])
predictions_prob3=model3.predict(X_test_all)
n_sample=200
np.random.seed(42)
ind=np.random.randint(0,len(X_test_all), size=n_sample)

imshow_group(X=X_test_all[ind],y=None,y_pred=predictions_prob2[ind], phase='prediction')
pf=(predictions_prob3+predictions_prob2+predictions_prob)/3.0
n_sample=200
np.random.seed(42)
ind=np.random.randint(0,len(X_test_all), size=n_sample)

imshow_group(X=X_test_all[ind],y=None,y_pred=pf[ind], phase='prediction')
labels=[np.argmax(pf) for pred in predictions_prob2]
keys=[get_key(path) for path in paths_test_all ]
create_submission(predictions=labels,keys=keys,path='submission_simple_keras_starter_kindofgood2.csv')