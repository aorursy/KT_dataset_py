...
def PCA_LogRegr_Model(X_std, y, N, penalty):
    ...
    LogRegr_model = linear_model.LogisticRegression(solver = 'lbfgs',max_iter = 1000, C = penalty)
    LogRegr_model.fit(X_pca, y)

    return(pca_model, LogRegr_model, variance_N,)
     
def Apply_PCA_LogRegr(pca_model, LogRegr_model, X_std):
    X_pca = pd.DataFrame(pca_model.transform(X_std))
    # predict y-values training
    y_pred = pd.DataFrame(LogRegr_model.predict(X_pca))
...
for i in tqdm(N_range): #run for all selected components number
    ...
    pca_model, LogRegr_model, variance_N = PCA_LogRegr_Model(X_train_std, y_train, i, penalty)

    #print('Applying PCA and SVM model based on training set...')
    y_train_pred = Apply_PCA_LogRegr(pca_model, LogRegr_model, X_train_std)
    y_val_pred = Apply_PCA_LogRegr(pca_model, LogRegr_model, X_val_std)
...
print('Determining PCA and Logistic regression models based on training set...')
pca_model, LogRegr_model, variance_N = PCA_LogRegr_Model(X_train_std, y_train, N_components, penalty)

print('Applying PCA and Logistical Regression model based on training set...')
y_train_pred = Apply_PCA_LogRegr(pca_model, LogRegr_model, X_train_std)
y_val_pred = Apply_PCA_LogRegr(pca_model, LogRegr_model, X_val_std)
...
for i in tqdm(m_range):
    ...
    #print('Determining PCA and logitic regression models based on training set...')
    pca_model, LogRegr_model, variance_N = PCA_LogRegr_Model(Xm_train_std, ym_train, N_components, penalty)

    #print('Applying PCA and logitic regression model based on training set...')
    ym_train_pred = Apply_PCA_LogRegr(pca_model, LogRegr_model, Xm_train_std)
    ym_val_pred = Apply_PCA_LogRegr(pca_model, LogRegr_model, Xm_val_std)
...
plt.xlabel('Number of Training Exampls (m)')
plt.ylabel('Error')
plt.grid(True)
...
print('Determining PCA and Logistical regression models based on training set...')
pca_model_FINAL, LogRegr_model_FINAL, variance_N = PCA_LogRegr_Model(X_all_std, y_all, N_components, penalty)

print('Applying PCA and Logistical regressionmodel based on training set...')
y_all_pred = Apply_PCA_LogRegr(pca_model_FINAL, LogRegr_model_FINAL, X_all_std)
...
y_test_pred = pd.DataFrame(LogRegr_model_FINAL.predict(X_test_pca))
...
submission.to_csv("Digits_PCA_LogRegr.csv",index = False)
