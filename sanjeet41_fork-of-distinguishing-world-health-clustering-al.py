...
def univariateAnaly(dataset):
    sns.distplot(dataset['Happiness_Score'])
def heatmapAnaly(dataset):
    plt.figure(figsize=(10,8))
    datasetCorr=dataset.drop(['Happiness_Rank'],axis = 1)
...
def clustering(type,dataset,params):
    ds=dataset.drop(['Happiness_Rank','Happiness_Score'],axis=1)
    if(type=='kmeans'):
...
dataset.head(10)
data = dict(type = 'choropleth',locations = dataset['Country'],locationmode = 'country names',z = dataset['Happiness_Score'],
           text = dataset['Country'],colorbar = {'title':'Happiness_Score'})
layout = dict(title = 'Global Happiness_Score', 
