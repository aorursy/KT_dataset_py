...
train = datafunc('../input/pneumonia/data/train')

test = datafunc('../input/pneumonia/data/test')

val = datafunc('../input/pneumonia/data/val')
trainlabel = []
...
history = model.fit(datagen.flow(x_train,y_train, batch_size = 32) ,epochs = 9 , validation_data = datagen.flow(x_val, y_val),callbacks=[checkpoint,early])
