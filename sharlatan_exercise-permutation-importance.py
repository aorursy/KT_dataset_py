...
def normalize_pd(data):
    panda = data.copy()
    for colname in list(panda):
        panda[colname] = panda[colname] / max(panda[colname])
    return panda
...
X_norm = normalize_pd(X)

new_train_X, new_val_X, new_train_y, new_val_y = train_test_split(X_norm, y, random_state=1)
