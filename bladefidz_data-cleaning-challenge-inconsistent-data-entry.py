...
def replace_matches_in_column(df, column, string_to_match, min_ratio = 90):
    ...
    # Return format [(str, distance), (str, distance), ...]
    matches = fuzzywuzzy.process.extract(string_to_match, strings, 
                                         limit=10, scorer=fuzzywuzzy.fuzz.token_sort_ratio)

    # only get matches with a ratio > 90
    close_matches = [matches[0] for matches in matches if matches[1] >= min_ratio]  # Select only str in each (str distance)
...
# take a look at them
print("Matched and levenshtein distances to kurram agency")
print(matches)
print()
