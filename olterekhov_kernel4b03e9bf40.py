...
from keras.layers import Input, Dense, Conv2D, Reshape, Flatten, concatenate, UpSampling2D, LeakyReLU, Dropout
...
# x = LeakyReLU(0.2)(x)
# x = Dropout(0.3)(x)
# # x = Conv2D(256, (2,1), padding='same', use_bias=False, name='conv_1')(x)
# x = Dense(512)(x)
# x = LeakyReLU(0.2)(x)
# x = Dropout(0.3)(x)
# x = Dense(256)(x)
...
# x = Conv2D(1, (2,1), padding='same', use_bias=False, name='conv')(x) # если добавлял слои
...
for k in range(iters):
    ...
    print('Epoch',(k+1)*10,f'/{iters * 10} - loss =',h.history['loss'][-1] )
...
gan.get_layer('model_5').get_layer('conv').set_weights([np.array([[[[-1 ]]],[[[255.]]]])])
...
class DogGenerator:
    ...
    def getDog(self,seed):
        xx = np.zeros((10000))
        xx[self.index] = 0.7
        xx[np.random.randint(10000)] = 0.3
...
def _get_model_layer(sess, model_name):
    ...
    print(sess, '---', model_name, layername)
...
! rm -r ../tmp #очистим временную директорию
