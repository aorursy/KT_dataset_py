...
print(threshold)
df1.loc[df1_scores == threshold]
df1.loc[df1_scores < threshold].index
df1.iloc[[4, 22, 32, 82, 100, 113, 217, 236, 249, 272, 274, 295, 302, 314,
            315],:]=[[557,142,21,58,81,23,18,8759,2583,271,1138,1299,478,1160,53,7,1450.0,1,1,1]]
df1.iloc[4,:]
## Applying Standart Scaler on only X variables
dff=df1.drop(["Salary","League_N","Division_W","NewLeague_N"], axis=1)
...
scaled_cols
ayrikdf=df1.loc[:, "League_N":"NewLeague_N"]
...
component_df=pd.DataFrame(data= pca_fit, columns=["1st Component","2nd Component","3rd Component","4th Component","5th Component"])
component_df.head()
DF2=pd.concat([component_df,ayrikdf],axis=1)
DF2.head()
#Regression
y=df1["Salary"]
...
np.sqrt(mean_squared_error(y_test, y_pred))
# Overfitting?  NO. RMSE values for test and train data sets are close to each other.
ridge_model=Ridge().fit(X_train,y_train)
