...
files()
data=read('train_Beauty.csv')
head(data)
describe(data)
data['category_name'] = remove(data['category_name'], ' ')
splits = split(data['category_name'], '/')
head(splits, 30)
category_name = tally(splits, multiple = True)
head(category_name, 10)
data['brand_name'] = remove(data['brand_name'], ' ')
brand_name = tally(data['brand_name'], multiple = True, min_count=10)
head(brand_name, 10)
X = hcat (data,brand_name,category_name)
X
final_data = clean(X)
head(final_data)
head(X)
