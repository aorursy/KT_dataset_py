...
data.head()
data['class'] = data['class'].apply(lambda x: 1 if x == 'p' else -1) # TODO
data['class'].head()
def dummies(data, columns=['pclass','name_title','embarked', 'sex']):
    ...
    return data
target ='class' # TODO
cols = data.drop('class', axis = 1).columns # TODO
...
testX, testY = test_data[test_data.columns[1:]], pd.DataFrame(test_data[target])# TODO
...
def node_weighted_mistakes(targets_in_node, data_weights):
    ...
    weight_positive = data_weights[targets_in_node == +1].sum() # TODO
    ...
    # 计算lable 为-1的所有数据的权重和
    #TODO
    weight_negative = data_weights[targets_in_node == -1].sum() # TODO
...
def best_split_weighted(data, features, target, data_weights):
    ...
    for feature in features:
        ...
        # 左分支对应当前特征为0的数据点
        left_split = data[target][data[feature] == 0] # TODO
        ...
        # 右分支对应当前特征为1的数据点
        right_split = data[target][data[feature] == 1] # TODO 
        ...
        # 重点！！
        # 计算左边分支里犯了多少错 (加权结果！！)
        left_misses, left_class = node_weighted_mistakes(left_split, left_data_weights)  # TODO            

        # 计算右边分支里犯了多少错 (加权结果！！)
        right_misses, right_class = node_weighted_mistakes(right_split, right_data_weights) # TODO
            
        # 计算当前划分之后的分类犯错率
        error = (left_misses + right_misses)/data_weights.sum() # TODO
...
def create_weighted_tree(data, data_weights, features, target, current_depth = 0, max_depth = 10, min_error=1e-15):
    ...
    # TODO:
    error, _= node_weighted_mistakes(target_values, data_weights)
    if error < min_error:
        print("Termination 1 reached.")     
        return create_leaf(target_values, data_weights) # TODO
    ...
    # 选出最佳当前划分特征
    split_feature = best_split_weighted(data, remaining_features, target, data_weights ) # TODO  # 根据加权错误来划分
...
class MyAdaboost(BaseEstimator):
    ...
    def fit(self, X, Y):
        ...
        for m in range(self.M):
            tree = WeightedDecisionTree(max_depth = 4, min_error = 1e-15) # TODO
            #TODO fit your current model
            tree.fit(X, Y, data_weights = alpha)
            prediction = tree.predict(X) # TODO
            
            # 计算加权错误
            weighted_error = alpha.dot(Y[self.target] != prediction) # TODO
            
            # 计算当前模型的权重
            model_weight = 0.5 * (np.log(1 - weighted_error) - np.log(weighted_error))  # TODO
            
            # 更新数据的权重
            alpha = alpha * np.exp(- weighted_error * Y[self.target] * prediction) # TODO
            # 数据权重normalize
            alpha = alpha/alpha.sum() # TODO
...
rf.score(testX, testY.values.ravel())