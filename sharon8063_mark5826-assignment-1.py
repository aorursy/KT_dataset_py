...
head(data, 5)
tally(data['style'])
plot(x = data['stars'], style = 'histogram', data = data['stars'])
plot(x = 'brand', y = 'stars', data = data, style = 'boxplot')
tally(data['country'], top = 10, graph = True, percent = True)
query(data, column = 'brand', condition = '=="Ve Wong"')
stars_sort = sort(data['stars'], how = 'descending')
print(stars_sort[-12:])
HighStars = query(data, column = 'stars', condition = '> 3')
country_styles = tabulate(HighStars['country'],HighStars['style'])
plot(data = country_styles,x = 'country', y = 'count', colour = 'style')
