...
col_miss=[col for col in X_train.columns if X_train[col].isnull().any() ] # Your code here
...
final_X_train = pd.DataFrame(fimp.fit_transform(X_train))

final_X_valid =  pd.DataFrame(fimp.transform(X_valid))
...
final_X_test = pd.DataFrame(fimp.transform(X_test))
