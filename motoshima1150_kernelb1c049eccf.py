...
# train_df.describe(include=['O']) -> オブジェクトの指定

train_df[['Pclass','Survived']].groupby(['Pclass'], as_index=False).mean().sort_values(by='Survived',ascending=False)
train_df[['Sex','Survived']].groupby(['Sex'], as_index=False).mean().sort_values(by='Survived',ascending=False)
# train_df[['Sex','Survived']]  ->全要素取り出し
# .groupby(['Sex'], as_index=False)　->Sexでまとめる。 as_indexはインデックス番号を付ける（つけないとヘッダーが変な感じなる）
# .mean() -> リスト表示出来るようになる。
# .sort_values(by='Survived',ascending=False)　->Survivedでソート
