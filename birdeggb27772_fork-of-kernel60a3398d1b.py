...
'''
for i in df.columns:
    if 'sensor' in i and ('28'  in i or '31'  in i or '36'  in i or '26'  in i or '32'  in i):
        df = df.drop(i,axis=1)        
...
'''
for i in df.columns:
    if 'sensor' in i and '28' not in i and '31' not in i and '36' not in i and '26' not in i and '32' not in i:
        df = df.drop(i,axis=1)        
for i in df2.columns:
    if 'sensor' in i and '28' not in i and '31' not in i and '36' not in i and '26' not in i and '32' not in i:
        df2 = df2.drop(i,axis=1)   
...
df["time_left"][df["time_left"]<1440]=0
df["time_left"][df["time_left"]>=1440]=1
df2["time_left"][df2["time_left"]<1440]=0
df2["time_left"][df2["time_left"]>=1440]=1
...
def buildOneToOneModel_old(shape):
  ...
  model.add(LSTM(128,input_length=1, input_dim=5*1440,return_sequences=True))
...
def buildOneToOneModel(shape):
  model = Sequential()
  #model.add(Conv2D(32, 5, 5, input_shape=(4,38, 38, 50),activation = 'relu'))
  model.add(LSTM(32,input_length=1, input_dim=5*1440,return_sequences=True))
...
#開始訓練
batch_size=128 #128 (Steve modified!)
...
#驗證訓練集，移除print的註解符號會印出預測和實際的label
count=0
trueCount=0
predict_array=[]
y_array=[]

for i in range(0,len(X_train),6):
    x_train=np.array(X_train[i])
    y_train=np.array(Y_train[i])
    x_train=np.reshape(x_train,(1,x_train.shape[0]))
    y_train=np.reshape(y_train,(1))
    prediction=model.predict(x_train[:,np.newaxis])
    prediction[prediction>0.5]=1
    prediction[prediction<0.5]=0
    
    predict_array.append(unnormalize(prediction[0][0])[0])
    y_array.append(unnormalize(y_train)[0])
    # Steve modified!
    #if unnormalize(prediction[0][0])[0]<=unnormalize(y_val)[0]+0.5 and unnormalize(prediction[0][0])[0]>=unnormalize(y_val)[0]-0.5:
    if unnormalize(prediction[0][0])[0]<=unnormalize(y_train)[0]+0.01 and unnormalize(prediction[0][0])[0]>=unnormalize(y_train)[0]-0.01:
        trueCount+=1
    count+=1
    #if count<200 or count>len(X_train)-200:
        #print('predict: ', unnormalize(prediction[0][0])[0], 'day')
        #print('label:   ', unnormalize(y_val)[0], 'day') 
        #print('\n')
y_array=pd.Series(y_array)
predict_array=pd.Series(predict_array)
pd.crosstab(y_array, predict_array)
print('accuracy of train: ',trueCount/count)
pd.crosstab(y_array, predict_array)
#驗證測試集，移除print的註解符號會印出預測和實際的label
count=0
trueCount=0
predict_array=[]
y_array=[]
for i in range(0,len(X_val),1):
    ...
    predict_array.append(unnormalize(prediction[0][0])[0])
    y_array.append(unnormalize(y_val)[0])
    # Steve modified!
    #if unnormalize(prediction[0][0])[0]<=unnormalize(y_val)[0]+0.5 and unnormalize(prediction[0][0])[0]>=unnormalize(y_val)[0]-0.5:
    if unnormalize(prediction[0][0])[0]<=unnormalize(y_val)[0]+0.01 and unnormalize(prediction[0][0])[0]>=unnormalize(y_val)[0]-0.01:
        trueCount+=1
    count+=1
        #if count<200 or count>len(X_val)-200:
        #print('predict: ', unnormalize(prediction[0][0])[0], 'day')
        #print('label:   ', unnormalize(y_val)[0], 'day') 
        #print('\n')
y_array=pd.Series(y_array)
