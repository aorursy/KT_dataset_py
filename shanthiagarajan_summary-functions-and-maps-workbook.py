...
# Your code here
def remedian_price(df):
    df.price = df.price = price_median
    return df

reviews.apply(remedian_price, axis="columns")
# Your code here
...
# Your code here
cv = reviews[['country','variety']]
cv = cv.dropna()
cv[['country','variety']].apply(lambda x: ' - '.join(x), axis = 1).value_counts()
