...
dummies_X=pd.get_dummies(X_test,drop_first=True)
preds=logmodel.predict(dummies_X)
...
log_sub=pd.DataFrame({'PassengerId':test.PassengerId,'SurVived':preds})

log_sub.to_csv('submission.csv',index=False)