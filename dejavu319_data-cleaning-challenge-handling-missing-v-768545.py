...
missing_values_count_sel = sf_permits.isnull().sum()
total_missing_sel=missing_values_count_sel.sum()
total_cells_sel=np.product(sf_permits.shape)
# percent of data that is missing
(total_missing_sel/total_cells_sel) * 100
# look at the # of missing points in the first ten columns
missing_values_count[0:10]
missing_values_count_sel
...
# replace all NA's with 0
sf_permits_1=sf_permits.fillna(0)
sf_permits_2=sf_permits.fillna(method = 'bfill', axis=0).fillna("0")
sf_permits_2