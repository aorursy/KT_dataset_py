...
missing_sfvalues_count.sort_values(ascending=False)[:20]



total_cells1 = np.product(sf_permits.shape)
total_missing = missing_sfvalues_count.sum()
(total_missing/total_cells1) * 100
# look at the # of missing points in the first ten columns
missing_values_count[0:10]
#view missing values count
missing_sfvalues_count.sort_values(ascending=False)[:30]
sf_permits[sf_permits['Zipcode'].isnull()].sample(42)
