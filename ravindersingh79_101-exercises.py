...
def ends_with_vowel(c):

    return c[-1] in "aeiouAEIOU"
...
def starts_and_ends_with_vowel(x):
    ...
    return x[0] in 'AEIOUaeiou' and x[-1] in 'AEIOUaeiou'
...
def highest_price_book(books):
    ...
    x=[{"title":"t", "price":0.00, "author":"a"}]

    while i<len(books):

        if (books[i]['price']) >= (x[0]["price"]):

            x[0]["price"]= books[i]["price"]

            x[0]["title"]= books[i]["title"]

            x[0]["author"] = books[i]["author"]
...
def lowest_price_book(books):
    ...
    x=[{"title":"t", "price":10000, "author":"a"}]
...
print(shopping_cart["items"][1]["price"])
# Exercise 96
...
#print(shopping_cart[1]["quantity"])
...
def total_number_of_items(s):
    ...
    while i< len(s["items"]):
        ...
        total = total + (s["items"][i]["quantity"])
...
def get_average_item_price(s):
    ...
    while i<len(s["items"]):
        ...
        total=total+(s["items"][i]["price"])

        i=i+1

    return total/(len(s["items"]))
...
def get_average_spent_per_item(s):
    ...
    while i< len(s["items"]):
        ...
        totalq=totalq+s["items"][i]["quantity"]

        totalp=totalp + (s["items"][i]["price"]*s["items"][i]["quantity"])
...
def most_spent_on_item(s):
    ...
    while i < len(s["items"]):
        ...
        if (s["items"][i]["quantity"])*(s["items"][i]["price"])> (d[0]["quantity"] * d[0]["price"]):

            d[0]["title"]= s["items"][i]["title"]

            d[0]["price"]= s["items"][i]["price"]

            d[0]["quantity"]= s["items"][i]["quantity"]
