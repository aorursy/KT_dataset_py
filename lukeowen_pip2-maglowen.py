...
end_time =    start_time + (period_osc*N_loops)                 # calculate the end time of the propagation (from the input variables)
...
start_time =   0    # enter the start time of the propagation
N_loops =     8   # increasing the loops from 2 to 8


steps_rev =    10000 # enter the number of time steps per revolution- increased to 10000 from 1000 previously, 
#do note that increasing steps_rev increasing the computational work needed - to avoid problems and long loads only run this program by itself, not the whole notebook

#by increasing the number of time steps per revolution(steps_rev), it decreases the error of the numerical solution, this is because it approximates the circular motion 
#with a polygon of step_rev number of side, so as step_rev approaches infinity,the smaller the error between the approximate polygon to the real circle.
