...
len(dailydata["betriebsdatum"])
dailydata.tail()
sd_m_dis=dailydata["departureDateTime"].dt.day.value_counts() #ilość odjazdów z przystanków w ciągu dnia

sd_m_dis=sd_m_dis.sort_index() 

sd_m_Hr=dailydata["departureDateTime"].dt.hour.value_counts() #ilość odjazdów z przystanków w ujęciu godzinowym
...
#funkcja do automatycznego oznaczania 
...
def autolabel(rects):
    ...
    for rect in rects:
        ...
        plt.text(rect.get_x()+rect.get_width()/6., 1.03*height, '%s' % int(height))
...
dailydata[dailydata["departure_plan_diff"] > 1000]
...
plt.scatter(departure_plan_diff.index, departure_plan_diff.values, alpha=.3, s=400) 

plt.show()
len(dailydata.columns)
figure(4)

dailydata["departure_plan_diff"].plot() 
