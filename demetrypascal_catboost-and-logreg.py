...
train = train.drop(['sig_id', 'cp_type'], 1)



test = test.drop(['sig_id', 'cp_type'], 1)
...
params = {'learning_rate' : [0.02, 0.04, 0.05, 0.06, 0.07, 0.08, 0.1],
...
for i, categ in enumerate(categs):
    ...
    cb_model.fit(train, targets[categ],  cat_features = cat_features_index, verbose = False)
...
for rate, categ in zip(best_rates, categs):
    ...
    cb_model = cb.CatBoostClassifier(loss_function = 'Logloss', iterations = iterations, learning_rate = rate)
