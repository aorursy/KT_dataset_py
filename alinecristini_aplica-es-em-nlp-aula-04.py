...
#ListaPos = sentence_polarity.sents(categories=['pos'])

#ListaNeg = sentence_polarity.sents(categories=['neg'])
import nltk
...
for fileid in nltk.corpus.sentence_polarity.fileids():
    ...
    x_data_pos.extend([' '.join(sent) for sent in nltk.corpus.sentence_polarity.sents(categories=['pos'])])
...
for fileid in nltk.corpus.sentence_polarity.fileids():
    ...
    x_data_neg.extend([' '.join(sent) for sent in nltk.corpus.sentence_polarity.sents(categories=['neg'])])
...
def my_tokenizer(doc):
    ...
    non_stopwords = [w for w in pos_tags if not w[0].lower() in stopwords_list] #.lower()
...
res = obj_classifier.predict(["the jokes are flat , and the action looks fake"])
...
print(res)
positivo = [x_data_test[i] for i in range(len(res)) if res[i] == 1]
...
negativo = [x_data_test[i] for i in range(len(res)) if res[i] == 0]
...
for txt in negativo:
    ...
    print("%s\n" % txt)
res2 = obj_classifier.predict(["the film provides some great insight into the neurotic mindset of all comics"])
