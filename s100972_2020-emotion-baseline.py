...
def test_test_loader(test):
    ...
    for token_ids, seg_ids,_ in loader:
...
class EmotionModel(nn.Module):
    ...
    def __init__(self, n_classes=3):
        ...
        self.query = torch.nn.Parameter(torch.empty(3,768))

        self.fcc= nn.Sequential(nn.Linear(self.query.size()[0]*self.query.size()[1], n_classes),nn.LogSoftmax())
...
#         print(len(hidden_states))
        ...
        pad_mask = pad_mask.unsqueeze(1).expand(-1, self.query.size()[1], -1)

        attout,_=self.Emotion_attention(self.query,last_seq,last_seq,scale=1/math.sqrt(768))

        

#         print(attout.squeeze().size())

        out=attout.view(attout.size()[0],-1)
