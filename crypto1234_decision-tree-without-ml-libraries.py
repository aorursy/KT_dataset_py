...
def gini_calc(data,labels):
    ...
    """Function that finds Gini values of each variable in a dataset given a set of data and labels. Also returns information if the variable is numeric including the splitting

    criteria of the numeric variable and the index in the list of Ginis that contain numeric variables"""
...
def recursive_tree(data,labels,max_depth = 3):
    ...
    for i in range(max_depth):
        ...
        best_gini, df_1, df_2, labs_one, labs_two, best_var, is_numeric, numeric_splitter,majority_class = decision_tree(data_frame_splits[i+counter],labs_list[i+counter])
...
def recursive_through_yarf(data,is_numeric_final,best_variables, inclusions,majority_class_list_final,numer_splitter_lst_final, max_depth = 3):
...
testing = recursive_through_yarf(test_data_tree_dummy,numeric_final,best_vars, inclusions,majority_class_lst,numer_splitter_lst_final,max_depth = 3)
