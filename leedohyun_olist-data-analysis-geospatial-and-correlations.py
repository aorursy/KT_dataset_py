...
plt.show()
df['time gap btw purchase and deliver'] = df.order_delivered_customer_date - df.order_purchase_timestamp
deliver_gap = pd.DatetimeIndex(df['time gap btw purchase and deliver'])
df['days taken for deliver'] = deliver_gap.day
df['times taken for deliver'] = deliver_gap.day*24 + deliver_gap.hour
df = df.dropna(subset=['days taken for deliver','customer_latitude','customer_longitude'])
y = df['times taken for deliver']
X = df[['customer_latitude','customer_longitude']]
X_train, X_test, y_train, y_test = train_test_split(X,y,test_size=0.33)
model = sklearn.svm.SVC()
model.fit(y=y_train,X=X_train)
model.predict(X=X_train)
model.score(y=y_train,X=X_train)
...
sns.scatterplot(list(range(1,len(estimator.predict(X_test))+1)),y_test)
plt.figure(figsize=(10,5))
sns.distplot(estimator.predict(X_test),color='b')
sns.distplot(y_test,color='c')