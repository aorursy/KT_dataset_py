...
for sim in range(n_simulations):
    ...
    stocks = simulate_basket(n_stocks, vol_matrix, n_timesteps = m, n_paths = n_evaluations)
    ...
    print("\n\nOPTION PRICES:" + str(np.mean(realized_prices)) + "," + str(q_hat[0](stocks[0,0,:].reshape((1,5)))))

    option_prices[sim] = np.mean(realized_prices)
...
stocks = simulate_basket(5, vol_matrix, n_timesteps = m, n_paths = 5)
