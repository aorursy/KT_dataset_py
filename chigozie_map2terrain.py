# Display images inline in pandas dataframe https://www.kaggle.com/stassl/displaying-inline-images-in-pandas-dataframe
import glob
import random
import base64
import pandas as pd
...
DATA_DIR = '../input/TerrainMapPairs'
...
# get images from file path, load as numpy arrays
df['img'] = df['path'].transform(lambda path: np.array(Image.open(path)))
# downsample images to 256x256 px
df['img_256'] = df['img'].transform(lambda img: img[::2,::2,:])
# pull out map from left of image and terrain from right of image
df['map'] = df['img_256'].transform(lambda img: img[:,:256,:])
df['terrain'] = df['img_256'].transform(lambda img: img[:,256:,:])

# make tensors
df['tensor_256'] = df['img_256'].transform(lambda img: tf.convert_to_tensor(img))
# randomly choose data to be test, train and validation, give dataframe column
df['set'] = [np.random.choice(['test','train','val'], 
                              p=[2/3, 1/6, 1/6])
            for i in range(len(df))]
# look at map, terrain and set columns from df
HTML(df[['map','terrain','set']].head().to_html(formatters={'img': image_formatter,
                                   'img_256': image_formatter,
                                   'map': image_formatter,
                                   'terrain': image_formatter},
                       escape=False))
PATH = '../input/TerrainMapPairs/'
BUFFER_SIZE = 400
BATCH_SIZE = 1
IMG_WIDTH = 256
IMG_HEIGHT = 256
...
train_files = df.loc[df['set']=='train','path']
test_files = df.loc[df['set']=='test','path']
val_files = df.loc[df['set']=='val','path']
...
checkpoint = tf.train.Checkpoint(generator_optimizer=generator_optimizer,
                                 ...
                                 generator=generator,
                                 discriminator=discriminator)
# Original version by Thomas Pappas used 450 epochs,
...
def generate_images(model, test_input, tar):
    ...
    for i in range(3):
        ...
        plt.axis('off')
    plt.show()
def train(dataset, epochs, save_every, ckpt=None):
    if ckpt:
    ...
    for epoch in range(epochs):
        ...
        print ('Time taken for epoch {} is {} sec\n'.format(epoch + 1,
                                                            time.time()-start))
train(train_dataset, EPOCHS, SAVE_EVERY, ckpt=tf.train.latest_checkpoint(checkpoint_dir))
# restoring the latest checkpoint in checkpoint_dir
checkpoint.restore(tf.train.latest_checkpoint(checkpoint_dir))
# Run the trained model on some of the test dataset
