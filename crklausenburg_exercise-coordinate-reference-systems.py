...
path_gdf.plot(cmap='tab20b', linestyle='-', linewidth=1, zorder=1)

start_gdf.plot(markersize=12, ax=ax, color='red')

end_gdf.plot(ax=ax, markersize=12, color='black')
...
ax = south_america.plot(figsize=(10,10), color='whitesmoke', linestyle=':', edgecolor='gray')
...
totalArea2 = sum(south_america.geometry.to_crs(epsg=3035).area) / 10**6
...
#print(birds[birds['location-lat'] < 0].head())



birds[birds['location-lat'] < 0].plot(markersize=8, ax=ax, color='red', alpha=0.6, zorder=2)
