...
pdf = os.path.join(outputFolder,'eabc0927.full.pdf[2]')

with Img(filename=pdf, resolution=300) as img:

    img.compression_quality = 99

    img.convert("RGBA").save(filename='/kaggle/working/eabc0927.full.jpg') # intro page to preview later
# Parse a PDF file and convert it to CSV using PyTesseract

import pytesseract

pdfimage = Image.open('/kaggle/working/eabc0927.full.jpg')

text = pytesseract.image_to_string(pdfimage)  

df = pd.DataFrame([text.split('\n')])
# Plot WordCloud of page 2
...
wordBarGraphFunction(df.T,0,"Most Common Words on Page 2 of eabc0927.full")
...
parsed = parser.from_file('/kaggle/working/eabc0927.full.jpg') 
...
with Img(filename=pdf, resolution=300) as img:
    ...
    img.convert("RGBA").save(filename='/kaggle/working/eabc0927.full.jpg')

pdfimage2 = Image.open('/kaggle/working/eabc0927.full.jpg')
# PDF to JPG for p5

pdf = os.path.join(outputFolder,'eabc0927.full.pdf[5]')
...
pdfimage5 = Image.open('/kaggle/working/eabc0927.full.jpg')



# PDF to JPG for p3

pdf = os.path.join(outputFolder,'eabc0927.full.pdf[3]')
...
pdfimage3 = Image.open('/kaggle/working/eabc0927.full.jpg')
# Parse a PDF file and convert it to CSV using PyTesseract (p3)
...
# Parse a PDF file and convert it to CSV using Tika-Python (p1-9)

tika.initVM()

parsed = parser.from_file('/kaggle/working/eabc0927.full.jpg')

parsed2 = parser.from_file('/kaggle/working/eabc0927.full.jpg')
...
dfcombined = pd.concat([df, df2, df3]) # combine pages 1-9
#Explore page 3 - Mueller Report. Here I don't know how many pages each Cheat Sheet. There are 9 pages 
...
# I will do this for Pages 1 to 9 but
...
pdfimage5 = Image.open('/kaggle/working/eabc0927.full.jpg')
#Explore page  - Mueller Report. Here I don't know how many pages each Cheat Sheet. There are 30 pages 

w, h = pdfimage5.size # crop image

pdfimage5.crop((0, 1240, w, h-1300)) # display exerpt of PDF
# Pages 2, 3 and 5

dfcombined.head() # preview csv of 1-9
