...
plt.style.available  #Available Graphic Sections
...
#According to Pclass, group the survived and died numbers, take the means and sort values
# Sex vs Survived
...
train_df.columns[train_df.isnull().any()] #Detect missing values' titles
train_df.isnull().sum()
train_df[train_df["Embarked"].isnull()]
#We can predict approximate result of Embarked by using fare

train_df.boxplot(column = "Fare", by = "Embarked")

plt.show()
#As we can see, C class is more closer to the 80 which is fare for unknown passengers
...
#His class was 3, we can predict his fare according to Pclass (Mean value of passengers have their Pclass is 3)
...
#To predict ages, we can see the genders
...
plt.show()
#Let's have a look at the Pclass

sns.factorplot(x="Sex",y="Age",hue = "Pclass",data=train_df, kind = "box")

plt.show()
    #Let's have a look at the Parch
...
s.split(".")[0].split(",")[1].strip() #strip method deletes the spaces where begin and end.
name = train_df["Name"]
...
plt.show()
train_df
#Dividing title feature as 4 feature

train_df = pd.get_dummies(train_df, columns = ["Title"])

train_df.head()
train_df.head()
train_df["Fsize"] =train_df["SibSp"] + train_df["Parch"] +1  # +1 for the passenger
train_df
...
train_df["Ticket"].head(20)
train_df.head()
train_df = pd.get_dummies(train_df, columns = ["Ticket"], prefix = "T") #When the columns are created, use "T" as title instead of "Ticket"

train_df.head(10)
sns.countplot(x = "Pclass", data = train_df)

plt.show()
train_df["Pclass"] = train_df["Pclass"].astype("category") #Converting categorical
...
print("Testing Accuracy: % {}".format(accuracy_log_test))
random_state = 42
...
for i in range(len(classifier)):
    ...
    print(cv_result[i])
cv_results = pd.DataFrame({"Cross Validation Means": cv_result, "ML Models": ["DecisionTreeClassifier", "SVC", 
...
#We selected 3 best accurate ML models and we will merge them via Voting Classifier
