...
#counting the total number of boy name births
for name in boys.keys():
    #print(boys[name])
    total_boy_names += boys[name]

#counting the total number of girl name births
for name in girls.keys():
    #print(girls[name])
    total_girl_names += girls[name]

#Printing each value
print('total boy births =', total_boy_names)
print('total girl births =', total_girl_names)

#printing the sum of each value
print('Assuming no double counts, total births =',total_boy_names + total_girl_names)
#make an empty list to hold just the names
all_names = []

# read the text file, break down the data into three categories based on position in line, 
# create a new variable to hold the names in lower cases
# and then add the names to the new list as you go through the data
for line in open(NAMES_LIST, 'r').readlines():
    name, gender, count = line.strip().split(",")
    new_name = name.lower()
    all_names.append(new_name)

#find the largest value in the list based on length
res = max(all_names, key = len)

#print the longest value (even if we add longer names to the list)
for x in all_names:
    if len(x) == len(res):
        print(x)  
