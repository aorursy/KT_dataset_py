...
# Create spectrograms
sampling_rate = 48000
spectrograms = np.zeros(shape = (len(df), 129, 1))

for i, whistle in enumerate(signature_whistles):
    spectro = signal.spectrogram(whistle, sampling_rate)[-1].sum(axis=1, keepdims=True)
    spectrograms[i, :, :] = spectro

plt.plot(spectrograms[0])
...
class DolphinsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        spectrograms = np.repeat(spectrograms, axis = 1, repeats=129)
...
val_specs = np.repeat(val_spectrograms, axis = 2, repeats=129)

for i, (specto, label) in enumerate(zip(np.repeat(val_specs[:, None, :, :], axis = 1, repeats=3), val_y)):
    specto = torch.Tensor(specto[None, :, :, :]).to(device)
    # print(specto.shape)
    # print(model_ft(specto).detach().cpu().numpy())
