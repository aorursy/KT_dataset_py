...
class my_gbdt(object):
    ...
    def __init__(self,m_estimators,lr):

        self.tree = DecisionTreeRegressor(min_samples_split=2, max_depth=3)

        self.tree0= DecisionTreeRegressor(min_samples_split=2, max_depth=3)

        self.m_estimators = m_estimators
    ...
    def fit(self, X, y):
        ...
        for i in range(self.m_estimators):

            self.tree.fit(X, residual)

            self.trees.append(self.tree)

            tmp_proba = self.tree.predict(X)

            residual = residual - self.lr*tmp_proba #use learning_rate in case of overfitting
    ...
    def predict(self, X):
        ...
        pred_proba = self.tree0.predict(X)[:,1]
        ...
        for tree in self.trees:
            ...
            pred_proba = pred_proba+self.lr* tmp_proba[:,1]

        return np.round(pred_proba)
my_model = my_gbdt(10,0.1)
...
accuracy = np.sum(np.array(gbdt_valid_label) == np.array(pred_label)) / len(pred_label)
