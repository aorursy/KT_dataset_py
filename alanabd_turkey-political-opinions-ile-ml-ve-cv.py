...
def yenimetot(y_test,y_score):
    ...
    y_score = label_binarize(y_score, classes=[1,2,3,4,5,6])
    ...
    return roc_auc[5],fpr[5],tpr[5],thr[5]
df=pd.read_csv("../input/turkey-political-opinions/yonelimfinal.csv")
...
df['Cinsiyet'] = df['Cinsiyet'].map(sex_mapping)
...
df['soru1'] = df['soru1'].map(question_mapping)

df['soru2'] = df['soru2'].map(question_mapping)

df['soru3'] = df['soru3'].map(question_mapping)

df['soru4'] = df['soru4'].map(question_mapping)

df['soru5'] = df['soru5'].map(question_mapping)

df['soru6'] = df['soru6'].map(question_mapping)

df['soru7'] = df['soru7'].map(question_mapping)

df['soru8'] = df['soru8'].map(question_mapping)

df['soru9'] = df['soru9'].map(question_mapping)

df['soru10'] = df['soru10'].map(question_mapping)
...
plt.title('Turkey Political Opinions Dataset ile Sınıflandırma',fontsize = 20,color='blue')
