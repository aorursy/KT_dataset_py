...
def elementwise_greater_than(L, thresh):
    ...
    """
    # my initial solution - no comprehension, but correct
    # LR = []
    # for check in L:
        # LR.append(check > thresh)
    # return LR

    # my solution with list comprehension
    return[check > thresh for check in L]
...
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    # setting counter of "wins"
    wins = 0
    
    # simulations loop
    for i in range(n_simulations):
        
        # setting balance and "fault" controller
        ...
        for j in range(n_spins):
            ...
            # check if another play possible with remaining balance
            if balance >= 1:
                balance -= 1
                balance += play_slot_machine()
            
            # count as failed if another play not possible
            else:
                failed = True
                            
        # count as win if particular simulation wasn't failed 
        if failed == False:
            wins += 1
        
    # calculate probability
    prob = wins / n_simulations
    print("For balance of", start_balance, "and for", n_spins, "spins, probability equals", prob)
    return prob
