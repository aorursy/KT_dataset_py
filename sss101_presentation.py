...
sns.heatmap(dataq10.astype(float).corr().round(2), linewidths=0.1, vmax=1.0, vmin=-1., square=True, cmap=colormap, linecolor='white', annot=True,  fmt='.1g' )
X_test_pres = pd.concat([datatest.drop( ['Unnamed: 0','Unnamed: 0.1' ,'default', 'occupation', 'ZIP', 'sex', 'minority'], axis=1), 

                     pd.get_dummies (datatest['ZIP'])], axis=1)

X_train_pres = pd.concat([datatrain.drop( ['Unnamed: 0', 'default', 'occupation', 'ZIP', 'sex', 'minority'], axis=1), 

                  pd.get_dummies (datatrain['ZIP'])], axis=1)
...
clf.fit(X_train_pres,Y_train)



clf.score(X_test_pres, Y_test)



#77% now vs 73% without occupation 
Y_oos_pred_pres = clf.predict(X_test_pres)

out_pres = pd.Series(data=Y_oos_pred_pres)

out_pres=out_pres.rename('defaults_pres')



dataq12_pres=pd.concat([out_pres, datatest.default, datatest.sex, datatest.minority],axis=1)

paid_pres = dataq12_pres[(dataq12_pres.default==0)]



paid_pres.groupby(by='sex').mean().defaults_pres

paid_pres.groupby(by='minority').mean().defaults_pres
