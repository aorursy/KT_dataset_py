...
def recommend(model, user_id, n=5):
    ...
    preds = model.predict([np.repeat(uid, len(movie_ids)), movie_ids,])
    movies.loc[movie_ids, 'predicted_rating'] = preds + offset
    return movies.sort_values(by='predicted_rating', ascending=False).head(n)
