...
loan.term.replace(['n/a'],0,inplace=True)

#loan.term.replace(['n/a'],'Self-Employed',inplace=True)
...
elif 'member_id'in loan.columns:

    del loan['member_id']

elif 'url' in loan.columns:

    del loan['url']

elif 'zip_code' in loan.columns:

    del loan['zip_code']
print("Now we are left with",loan.shape ,"rows & columns.")
numeric_columns = ['loan_amnt','funded_amnt','funded_amnt_inv','installment','int_rate','annual_inc','dti','open_acc','mort_acc','total_acc','delinq_2yrs','inq_last_6mths','pub_rec']
