...
model1=XGBRegressor(n_estimators=600)
...
tmp["importance"] = model1.feature_importances_
...
print(model1.score(x_valid,y1_valid))
...
sns.lineplot(x=data['dayofyear'],y=data['ConfirmedCases'],label='true')

pred=model1.predict(train[col_var][(train['place_id']==place)&(train['dayofyear']>92)])

sns.lineplot(x=data['dayofyear'][data['dayofyear']>92],y=pred,label='pred')



# pred2=model1.predict(test[col_var][(test['place_id']==place)])
...
#     sns.lineplot(x=data['dayofyear'][data['dayofyear']>92],y=pred,label='pred_case')
...
#     plt.show()
model_predcase=model1.predict(X_pred)

plt.plot(model_predcase)
test['Pred_case']=model_predcase
...
tmp["importance"] = model2.feature_importances_
...
print(model2.score(x_valid,y2_valid))
...
pred=model2.predict(train[col_var2][(train['place_id']==place)&(train['dayofyear']>92)])
...
plt.show()
model_predfatal=model2.predict(X_pred2)

plt.plot(model_predfatal)
test['Pred_death']=model_predfatal
...
submission['ConfirmedCases'] = model_predcase

submission['ConfirmedCases'] = submission['ConfirmedCases'].astype("int")

submission['Fatalities'] = model_predfatal
