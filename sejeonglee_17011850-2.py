...
! unzip 2020-ai-exam-fashionmnist-2
import torch
...
Scaler = preprocessing.StandardScaler() # 정규화 standardscaler이용
...
data_loader = torch.utils.data.DataLoader(dataset=train_dataset,
                                          ...
                                          drop_last=True)
linear = torch.nn.Linear(784,10,bias=True) # 단층layer

torch.nn.init.xavier_uniform_(linear.weight) #초기화 -> xavier
model = torch.nn.Sequential(linear).to(device)
# 손실함수와 최적화 함수

loss = torch.nn.CrossEntropyLoss().to(device) # softmax 내부적으로 계산

optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate, momentum=0.9) #SGD, momentum=0.9
...
! kaggle competitions submit -c 2020-ai-exam-fashionmnist-2 -f submit.csv -m "."