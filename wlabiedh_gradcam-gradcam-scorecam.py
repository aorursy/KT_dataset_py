...
def get_images(dir_name='../input/yalefaces/yalefaces/', label_to_class=label_to_class):  
    ...
    for label_name in os.listdir(dir_name):
        ...
        for img_name in os.listdir('/'.join([dir_name, label_name])):
            img = load_img('/'.join([dir_name, label_name, img_name]), target_size=(243, 340))
...
def build_model():
    ...
    # Resnet
    input_tensor = Input(shape=(243, 340, 3)) # To change input shape
    resnet50 = ResNet50(
...
history = model.fit_generator(
    datagen_train.flow(x_train, y_train, batch_size=1),
    epochs=2,
    validation_data=datagen_test.flow(x_test, y_test, batch_size=1),
)
...
def score_cam(model, x, layer_name, max_N=-1):
    ...
    act_map_resized_list = [cv2.resize(act_map_array[0,:,:,k], (340,243), interpolation=cv2.INTER_LINEAR) for k in range(act_map_array.shape[3])]
...
!ls ../input/yalefaces/yalefaces/sub03/

img = load_img("../input/yalefaces/yalefaces/sub03/subject03.glasses.jpg", target_size=(243, 340))
img = img_to_array(img)
imgg = np.array(img, dtype=np.float32)
ximgg=imgg.reshape(1,243,340,3)
prd=model.predict(ximgg)>0.5
print(prd)