...
data_set.head()
train_data, test_data = train_test_split(data_set, test_size=0.2, random_state=0)

# trainX, trainY = train_data[1:], pd.DataFrame(train_data[0])
...
def count_errors(labels_in_node):
    ...
    positive_ones = (labels_in_node == 1).sum()

    negative_ones = (labels_in_node == -1).sum()
