...
consumer_complaints_train.head(10)
consumer_complaints_train.shape
fig = plt.figure(figsize=(20,20))

consumer_complaints_train.groupby(['State'])['Complaint ID'].count().sort_values().plot.barh(

    color='blue', title='NUMBER OF COMPLAINTS REGISTERED IN EACH STATE\n')

plt.xlabel('Statewise count', fontsize=15, color = 'Orange')

plt.ylabel('State', fontsize=15, color = 'Orange')
...
consumer_complaints_test = pd.read_csv('../input/consumercomplaints/Edureka_Consumer_Complaints_test.csv')
...
test_df = pd.read_csv('../input/consumercomplaints/Edureka_Consumer_Complaints_test.csv')
test_df.head()
test_df_1 = test_df
