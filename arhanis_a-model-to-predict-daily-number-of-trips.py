...
#The weather data frame is 5 times as long as out data frame, therefore there are 5 entries per date.
...
#max_wind and max_gust are well correlated, so we can use max_wind to help fill the nulls of max_gust
...
#The max date is before the end of the train data frame, therefore the service has no been adding new stations recently.

print (stations.installation_date.min())

print (stations.installation_date.max())
#For each day in train.date, find the number of docks (individual bike stations) that were installed 
...
#Train and predict with the 3 best models.
...
#Weight the top 3 models to find the best prediction
