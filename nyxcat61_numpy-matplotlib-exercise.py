...
plt.plot(x, sin_y, linewidth=3, color='red', linestyle=':', label="Sin")

plt.plot(x, cos_y, linewidth=3, color='blue', linestyle='-', label="cos")
...
plt.text(0.5, 0.5, 'subplot(2,2,2)', ha='center', va='center', size=20, alpha=0.7)
...
e = .1 * np.abs(np.random.randn(len(y)))
...
plt.contourf(X, Y, f(X,Y), 12, alpha=0.9, cmap=plt.cm.summer)
plt.imshow(f(X,Y))
...
ax.plot_surface(X, Y, Z, cstride=2, rstride=1, cmap='hot')
import numpy as np
def update(frame):
