...
output_path = '../dataset/'
split_folders.ratio(orig_path, output=output_path, seed=1, ratio=(.75, .25))
from tensorflow.keras.preprocessing.image import ImageDataGenerator

IMAGE_TARGE_SIZE = 64
train_gen = ImageDataGenerator(rescale = 1. / 255, rotation_range = 0.05, shear_range = 0.2, 
                               horizontal_flip = True, vertical_flip = True, fill_mode = 'nearest')

val_gen = ImageDataGenerator(rescale = 1. / 255)

train_set = train_gen.flow_from_directory(output_path + 'train/', 
                                          target_size = (IMAGE_TARGE_SIZE, IMAGE_TARGE_SIZE), 
                                          batch_size = 100, class_mode = 'binary')
val_set = val_gen.flow_from_directory(output_path + 'val/', 
                                      target_size=(IMAGE_TARGE_SIZE, IMAGE_TARGE_SIZE),
                                      batch_size=100, class_mode='binary', shuffle = False)
...
def draw_image(ax, imgPath, xlabel, fontsize = 25):
...
model = tf.keras.Sequential([
    Conv2D(filters=19, kernel_size=(3, 3), activation='relu', 
           input_shape=(IMAGE_TARGE_SIZE, IMAGE_TARGE_SIZE, 3), padding='same'),
...
model.summary()
history = model.fit(train_set, epochs = 16, batch_size = 250, validation_data = val_set)
