...
import utils_madair as utils
# set input dirname
input_dir = os.path.join('/kaggle/input', 'seleccion-datos-aire-primavera-2019-1-estacion')
# 1. Cargar datos
...
for i in range(len(test)):
    # make prediction
    predictions.append(history[-step_forecasting])
    # observation
...
plt.show()
from sklearn.preprocessing import MinMaxScaler
def normalize_data_min_max(data, training_size):
    '''
...
X_norm, scaler = normalize_data_min_max(X, training_size)
...
(X_train, y_train), (X_test, y_test) = utils.train_test_split(X_norm, test_prop,
                                                                      seq_len = 23, steps = step_forecasting)
...
def run_test_nonlinear_reg(x, y):
    ...
    rmse = np.sqrt(mse)
    mape = mean_absolute_percentage_error(y_orig, y_pred)
