...
data = pd.read_csv("../input/para2-input/train.tsv",sep="\t")
text = data['sentence1']
summary = data['sentence2']
...
for index, row in summary.iteritems(): 
    target_texts.append(row+" "+'<eos>')
    if index == 5000:
        break
print(target_texts[3:4])
#for i, row in summary.iterrows():
 #   ifor_val = row
    
  #  ifor_val = row+'<eos>'
   # summary.at[i,'row'] = ifor_val
#summary.head()
for index, row in summary.iteritems(): 
    target_texts_inputs.append('<sos>'+" "+row)
    if index == 5000:
        break
print(target_texts_inputs[3:4])

for index, row in text.iteritems(): 
    input_texts.append(row)
    if index == 5000:
        break
print(input_texts[3:4])
...
max_len_input = 25# max(len(s) for s in input_sequences)
print(max_len_input)
...
max_len_target =25 #max(len(s) for s in target_sequences)
