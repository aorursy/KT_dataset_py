...
seq_array1=seq_array[0:(int)(seq_array.shape[0]/2)]
label_array1=label_array[0:(int)(label_array.shape[0]/2)]
seq_array2=seq_array[(int)(seq_array.shape[0]/2):]
label_array2=label_array[(int)(label_array.shape[0]/2):]

nb_features = seq_array1.shape[2]
...
t0 = time.time()
# fit the network
model.fit(seq_array1, # Training features
          label_array1, # Training labels
...
# training metrics
scores = model.evaluate(seq_array1, label_array1, verbose=1, batch_size=200)
print('Training Accurracy: {}'.format(scores[1]))

scores = model.evaluate(seq_array2, label_array2, verbose=1, batch_size=200)
print('Training Accurracy: {}'.format(scores[1]))
...
cm
# make predictions and compute confusion matrix
y_pred2 = model.predict_classes(seq_array2,verbose=1, batch_size=200)
y_true2 = label_array2
print('Training Confusion matrix\n- x-axis is true labels.\n- y-axis is predicted labels')
...
print( 'Training Precision: ', precision2, '\n', 'Training Recall: ', recall2, '\n', 'Training F1 Score:', f1)
print(fbeta_score(y_true2, y_pred2,beta=0.5))