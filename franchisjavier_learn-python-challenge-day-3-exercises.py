...
def sign(number):
    return abs(number)/number if number!=0 else 0
...
def exactly_one_topping(ketchup, mustard, onion):
    ...
    on their hot dog.
    """
    suma=int(ketchup)+int(mustard)+int(onion)
    return 1 if suma==1 else 0
...
def should_hit(player_total, dealer_total, player_aces):
    ...
    state, or False if the player should stay. player_aces is the number of aces the player has.
    """
    return player_total<dealer_total or player_total<18
