...
class DisasterDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        ids, masks, tweet = self.get_input_data(row)
...
class DisasterModel(nn.Module):
  ...
  def __init__(self):
    ...
    self.config = RobertaConfig.from_pretrained('/kaggle/input/roberta-base/config.json', output_hidden_states=True)

    self.roberta = RobertaModel.from_pretrained('/kaggle/input/roberta-base/pytorch_model.bin', config=self.config)



    self.clsHead = RobertaClassificationHead()

  

  def forward(self, input_ids, attention_masks):

    a, _, hs = self.roberta(input_ids, attention_masks)

    hid_out = torch.stack([hs[-1][:,0,:], hs[-2][:,0,:], hs[-3][:,0,:], hs[-4][:,0,:]])

    hid_out = torch.mean(hid_out, 0)

    x = self.clsHead(hid_out)
...
batch_size = 16
train_df = pd.read_csv('/kaggle/input/extendeddatafornlpgettingstarted/cleaned_augmented_data.csv')
...
def run():
    ...
    train_index = np.random.randn(len(train_df)) <= 0.8

    df_train = train_df.iloc[train_index].reset_index(drop=True)

    df_val = train_df.iloc[~train_index].reset_index(drop=True)
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        dataset=DisasterDataset(df_train),
    ...
    val_loader = torch.utils.data.DataLoader(
        ...
        dataset=DisasterDataset(df_val),
...
def make_predictions(test_loader):
    ...
    for _, data in enumerate(tk):
        ...
        for i in range(len(ids)):
            ...
            predictions.append(np.argmax(logits[i]))
...
test_loader = torch.utils.data.DataLoader(
    ...
    dataset=DisasterDataset(test_df),
