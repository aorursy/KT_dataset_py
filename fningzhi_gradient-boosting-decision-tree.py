...
class my_gbdt(object):
    ...
    def fit(self, X, y):
        ...
        for i in range(self.items):
            ...
            tree_t = DecisionTreeRegressor(min_samples_split=2, max_depth=3)

            tree_t.fit(X,y_er)

            y_pre += tree_t.predict(X)*self.lr
    ...
    def predict(self, X):
        ...
        for i in range(self.items):
            ...
            pred_proba += self.trees[i].predict(X)*self.lr
