...
percentage_not_disaster = 4342 / (4342 + 3271)
print("percentage_not_disaster: " + str(percentage_not_disaster))

percentage_disaster = 3271 / (4342 + 3271)
print("percentage_disaster: " + str(percentage_disaster))
...
plt.show()
percentage_missing_keyword_train = (df_train['keyword'].isnull().sum() / df_train.shape[0])*100
print("percentage_missing_keyword_train: " + str(percentage_missing_keyword_train))

percentage_missing_location_train = (df_train['location'].isnull().sum() / df_train.shape[0])*100
print("percentage_missing_location_train: " + str(percentage_missing_location_train))

percentage_missing_keyword_test = (df_test['keyword'].isnull().sum() / df_test.shape[0])*100
print("percentage_missing_keyword_test: " + str(percentage_missing_keyword_test))

percentage_missing_location_test = (df_test['location'].isnull().sum() / df_test.shape[0])*100
print("percentage_missing_location_test: " + str(percentage_missing_location_test))
...
for i, feature in enumerate(METAFEATURES):
    sns.distplot(df_train.loc[~DISASTER_TWEETS][feature], label='Not Disaster', ax=axes[i][0], color='green', kde_kws={'bw':0.1})
    sns.distplot(df_train.loc[DISASTER_TWEETS][feature], label='Disaster', ax=axes[i][0], color='red', kde_kws={'bw':0.1})

    sns.distplot(df_train[feature], label='Training', ax=axes[i][1], kde_kws={'bw':0.1})
    sns.distplot(df_test[feature], label='Test', ax=axes[i][1], kde_kws={'bw':0.1})
...
for tweet in df_train[DISASTER_TWEETS]['text']:
    for word in generate_ngrams(tweet):
        disaster_unigrams[word] += 1 #add count for the particular word
        
#counting number of particular words for non-disaster tweets
for tweet in df_train[~DISASTER_TWEETS]['text']:
    for word in generate_ngrams(tweet):
        nondisaster_unigrams[word] += 1
type(disaster_unigrams)
print(disaster_unigrams)
#converting to dataframe and sorted descending numerically
...
#defining number of of unigrams to list
N = 100 

#y=column 0 of dataframe, x=column 1 of dataframe
...
def build_vocab(X):
    ...
    for tweet in tweets:
        for word in tweet:
            try:
                vocab[word] += 1
            except KeyError: #if there is no word into the array
                vocab[word] = 1                
    return vocab
...
def check_embeddings_coverage(X, embeddings):
    ...
    vocab = build_vocab(X) #adding words into the vocab array    
...
gc.collect()
df_train.to_csv('df_train_lookat.csv', index=False)
df_mislabeled = df_train.groupby(['text']).nunique().sort_values(by='target', ascending=False) # finding same texts that appear more than once,meaning that there are different interpretations of whether it's an emergency or not
df_mislabeled.to_csv('df_mislabeled.csv', index=False) 

df_mislabeled = df_mislabeled[df_mislabeled['target'] > 1]['target'] #labels that have duplicates since there are more than 1 tweet per unique tweet
df_mislabeled.index.tolist()
df_train.head(20)
df_train['target_relabeled'] = df_train['target'].copy() 

#relabel by yourself based on what you think whether it is emergency or not 
