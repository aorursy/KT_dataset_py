...
data = pd.concat([train["TotRmsAbvGrd"],train["SalePrice"]],axis=1)
...
train = train.drop(train[(train['TotRmsAbvGrd']>9) & (train['SalePrice']>710000)].index)
...
train = train.drop(train[(train['GarageArea']>600) & (train['SalePrice']>500000)].index)
...
train = train.drop(train[(train['OverallCond']<3) & (train['SalePrice']>380000)].index)

#train = train.drop(train[(train['OverallCond']>1200) & (train['SalePrice']<300000)].index)

#train = train.drop(train[(train['OverallCond']>600) & (train['SalePrice']>500000)].index)
...
plt.scatter(test_ID,np.exp(bst.predict(dtest)))
