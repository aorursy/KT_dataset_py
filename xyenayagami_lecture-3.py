...
print('Succesfully.')
#done with import matplotlib , svm , and train_test_split
# load the data
...
for i in range(10):
    ...
    fir=plt.figure()
    plt.imshow(img,cmap='gray')
    title=''+str(train_labels.iloc[j,0])
    plt.title(title)
    plt.show()
    plt.close()
...
for i in range(10):
    ...
    plt.figure()
    plt.hist(data)
    plt.title(''+str(train_labels.iloc[j,0]))
    plt.show()
    plt.close()
datas=np.append(train_images.iloc[data[0]],(train_images.iloc[data[1:]]))
print(len(datas))
plt.figure()
plt.hist(datas)
plt.title('histo class')
plt.show()
plt.close()
clf = svm.SVC()
