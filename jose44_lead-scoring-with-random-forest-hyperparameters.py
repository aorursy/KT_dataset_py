...
def rf_model(n,l):
    ...
    model = RandomForestClassifier(n_estimators=n,criterion='entropy',min_samples_leaf=l,random_state=100)
    ...
    print('minimum sample on the leaf: ', l)
    ...
    return [n,l,trainaccuracy,testaccuracy]
...
for n in range(20):
    ...
    for l in range(10):
        ...
        aux.append(rf_model(n*10+10,l*10+10))

    

df=pd.DataFrame(aux)

df.columns=['n_estimators','min_samples_leaf','train_accuracy','test_accuracy']
rf_optimin=df[df.test_accuracy==max(df.test_accuracy)]
