...
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    .25
    """
    def shot (balance, spins):
        if (spins == 0):
            return True
        elif (balance < 1):
            return False
        else:
            return shot(balance + play_slot_machine() - 1, spins - 1)
    return sum([ shot(start_balance, n_spins) for i in range(n_simulations) ]) / n_simulations
print(slots_survival_probability(0, 0, 10))
print(slots_survival_probability(0, 1, 10))
print(slots_survival_probability(1, 0, 10))
print(slots_survival_probability(1, 2, 10))
q5.check()
q5.solution()