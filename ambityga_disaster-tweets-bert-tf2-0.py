...
class ClassificationReport(Callback):
    ...
    def on_epoch_end(self,epoch, logs = {}):
        ...
        print('\nEpoch: {} - Validation Precision: {:.6} - Validation Recall: {:.6} - Validation F1: {:.6}'.format(epoch+1,val_precision,val_recall,val_f1))
%%time

bert_layer = hub.KerasLayer('https://tfhub.dev/tensorflow/bert_en_uncased_L-12_H-768_A-12/1', trainable=True)
class Classifier:
    def __init__(self,bert_layer,max_seq_length = 128,lr=0.00005,epochs = 20, batch_size = 36):
    ...
    def encode(self,texts):
        ...
        for text in texts:
            ...
            input_sentence = ['[CLS]'] + text + ['[SEP]']
            pad_len = self.max_seq_length - len(input_sentence)
            
            tokens = self.tokenizer.convert_tokens_to_ids(input_sentence)
            tokens += [0]*pad_len
            pad_masks = [1]*len(input_sentence) + [0]*pad_len
            segment_ids = [0]*self.max_seq_length
            
            input_ids.append(tokens)
            mask_ids.append(pad_masks)
            segment_mask.append(segment_ids)
        
        return np.array(input_ids), np.array(mask_ids), np.array(segment_mask)
    
    def build_model(self):
        
        input_ids = Input(shape=(self.max_seq_length,),dtype = tf.int32, name = 'input_ids')
        mask_ids = Input(shape=(self.max_seq_length,),dtype = tf.int32, name = 'mask_ids')
        segment_ids = Input(shape=(self.max_seq_length,),dtype = tf.int32, name = 'segment_ids')
        
        _, sequence_output = self.bert_layer([input_ids,mask_ids,segment_ids])
        clf_output = sequence_output[:,0,:]
        
        out = Dense(1,activation = 'sigmoid')(clf_output)
        
        model = Model(inputs = [input_ids,mask_ids,segment_ids], outputs = out)
    ...
    def plot_learning_curve(self):
        ...
        fig, axes = plt.subplots(nrows=4, ncols=2, figsize=(20, K * 6), dpi=100)
    ...
    def predict(self,X):
        X_test = self.encode(X['text_cleaned'].str.lower())
        ...
        return y_pred
%%time
clf = Classifier(bert_layer, max_seq_length=128, lr=0.00005, epochs=20, batch_size=36)

clf.train(train)
X_test = clf.encode(['She wrote down a poem which made the heart of the readers go fire up'])
answer = np.zeros((X_test[0].shape[0],1))

for model in clf.models:
    answer += model.predict(X_test) / len(clf.models)

if np.round((answer)) == 0:
    print("Fake")
#     print(answer)
else:
    print("Not Fake")
X_test = clf.encode(['The earthquake shook the capital last night'])
answer = np.zeros((X_test[0].shape[0],1))

for model in clf.models:
    answer += model.predict(X_test) / len(clf.models)

if np.round((answer)).astype('int') == 0:
    print("Fake")
    print(answer)
else:
    print("Not Fake")
