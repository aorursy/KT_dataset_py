...
EMBEDDING_SIZE = 32
df=pd.read_csv('https://files.catbox.moe/h1kg2l.csv')

sw=pd.read_fwf('https://files.catbox.moe/c5vlra.txt',header=None)



sw.columns=['stopwords']



sw=sw['stopwords'].tolist()
...
for index, row in df.iterrows():
    ...
    documents.append(read_questions(row,"headline"))

for index, row in ukara.iterrows():

    documents.append(read_questions(row,"teks"))
...
for i in range(len(documents)):
    ...
    for j in range(len(documents[i])):
        ...
        if documents[i][j] not in sw:

            words.append(documents[i][j])

    doc_nosw.append(words)



model=gensim.models.Word2Vec(size=EMBEDDING_SIZE,window=5,min_count=3)

model.build_vocab(doc_nosw)



filename='comb_word2vec.txt'

model.wv.save_word2vec_format(filename,binary=False)



file=open('comb_word2vec.txt',encoding='utf-8')
...
tokenizer.fit_on_texts(doc_nosw)
