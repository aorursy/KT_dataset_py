...
query1 = """SELECT COUNT(consecutive_number), 
           ...
           ORDER BY COUNT(consecutive_number) DESC 
        """
accidents_by_hours = accidents.query_to_pandas_safe(query1)
accidents_by_hours.head()
# Which state has the most hit and run?
query2 = """SELECT COUNT(hit_and_run) AS Num, registration_state_name AS State
