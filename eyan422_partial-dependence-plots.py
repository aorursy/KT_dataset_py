...
cols_to_use = ['BuildingArea','Rooms','Distance', 'Landsize']
def get_some_data():
...
gb = GradientBoostingClassifier(random_state=0).fit(imputed_titanic_X, titanic_y)
kwargs = dict(X=imputed_titanic_X, percentiles=(0, 1), grid_resolution=2)
...
#cols_to_use = ['Distance', 'Landsize', 'BuildingArea']
cols_to_use = ['BuildingArea','Rooms']
def get_some_data():
    data = pd.read_csv('../input/melbourne-housing-snapshot/melb_data.csv')
    y = data.Price
    X = data[cols_to_use]
