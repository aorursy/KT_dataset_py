...
model_file.close()
res = obj_classifier.predict(["if you sometimes like to go to the movies to have fun , wasabi is a good place to start ."])
...
good = [x_data_test[i] for i in range(len(res)) if res[i] == 1]

for txt in good:

    print("%s\n" % txt)

bad = [x_data_test[i] for i in range(len(res)) if res[i] == 0]
...
res2 = obj_classifier.predict(["it is bad"])

#res2 = obj_classifier.predict(x_data_test)

#print(accuracy_score(y_data_test, res2))

res3 = obj_classifier.predict_proba(["it is bad"])

res = obj_classifier.predict_proba(["like to go to the movies to have fun , wasabi is a good place"])
