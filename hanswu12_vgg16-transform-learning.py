...
dataset_path = '/kaggle/input/covid19/covid19/dataset/'
def ceildiv(a, b):
    return -(-a // b)
...
def loadimg(imagfile):
    ...
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    image = cv2.resize(image, (224, 224)) 
    return (np.array(image)/255).reshape(1,224,224,3)

test_d= loadimg("/kaggle/input/covid19/covid19/test_image/ok.jpg")
