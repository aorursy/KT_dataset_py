...
file_dir='../input/2020mcm/pacifier.tsv'
...
X=[data[data['verified_purchase']=='Y'].count()[0]+data[data['verified_purchase']=='y'].count()[0],data[data['verified_purchase']=='n'].count()[0]+data[data['verified_purchase']=='N'].count()[0]]
...
verified_data=data[data['verified_purchase'].isin(['Y','y'])]
...
plt.title("the sales number pie chart"+" pacifier")

plt.savefig("all pacifier share.jpg")

plt.show()
len(data['customer_id'].value_counts())
top_XX=sales_number.nlargest(50,columns='cnt').tail(50)
...
plt.title("slim hair pacifier share")

plt.savefig("slim hair pacifier share.jpg")
...
top_XX_data.info()
labels
top_XX_data.to_csv("./top_50_pacifier.csv",index=False)
tmp=pd.read_csv('./top_50_pacifier.csv',sep=',')
...
def get_p_or_n(comments): # 获取情绪极化评分，并划定阈值确定是积极、消极或中立
    ...
    init_df=pd.read_csv('./top_50_pacifier.csv',sep=',')

    init_df['score']=result_df['score']

    init_df.to_csv('./pacifier_score.csv',index=False)
...
def main():
    ...
    filename = "./top_50_pacifier.csv"

    comments, words = getComments(filename)

    print(len(comments))

    getWordCloud(words, "WordCloud_pacifier.png")
...
data=pd.read_csv('./pacifier_score.csv',sep=',')
...
data.to_csv("ini_review_pacifier.csv",index=False)
...
df_final_score.to_csv("final_pacifier_score.csv")