...
train.head() # Printing top 5 rows of train dataset.
train[train.SalePrice>300000]
train.tail() # Printing last 5 rows of train dataset.
train.info() # For finding the number of NaNs & data types in train dataset.
train.describe() # This describes the train dataset.
correlation_train=train.corr()
sb.set(font_scale=2)
plt.figure(figsize = (50,35))
ax = sb.heatmap(correlation_train, annot=True,annot_kws={"size": 25},fmt='.1f',cmap='YlGnBu', linewidths=.5)
...
test.head() # Printing top 5 rows of test dataset.
test.tail() # Printing last 5 rows of test dataset.
test.info()
test.describe()
train_test=pd.concat([train,test],axis=0,sort=False) # Combining both train & test datasets.
train_test.head()
pd.set_option('display.max_rows', 5000)
...
corr_dict2
# Printing columns with more than 40% positive or negative correlations with SalePrice.
best_columns=[]
...
sb.set(font_scale=2)
train.isnull().sum() # Number of NaNs column-wise in train dataset.
test.isnull().sum() # Number of NaNs column-wise in test dataset.
del test['SalePrice']
train.head()
