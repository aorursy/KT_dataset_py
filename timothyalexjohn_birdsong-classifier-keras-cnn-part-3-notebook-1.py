...
tk.__version__    # Important: set the Environment Preferences to "Latest Environment"

                  # tk._version should be >= 2.4.0 
test_dir = '../input/birdsong-recognition/test_audio/'

test_csv_dir = '../input/birdsong-recognition/test.csv'

test_filesave_dir = '/kaggle/working/'
...
def create_spectrogram(df, start_time, duration, mode):
    ...
    try:
        ...
        if mode== 'test':
            ...
            plt.savefig((test_filesave_dir +df[1] +'.jpg'), bbox_inches='tight',pad_inches=0, facecolor='black')
...
try:
    ...
    for img in row_id:
        ...
        target.append(predict(test_filesave_dir +img +'.jpg'))
...
try:   
    ...
    predict_df = pd.read_csv('../input/birdsong-recognition/sample_submission.csv')
# Very Important to remove all other output files other than "submission.csv"
...
try:
    ...
    for img in row_id:
        ...
        os.remove(test_filesave_dir +img +'.jpg')
