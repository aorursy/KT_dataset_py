...
def euler(yin,omega,step):
    ...
    vx = yin[0] + step * y1[0]
    vy = yin[1] + step * y1[1]
    omega_vx = yin[2] + step * y1[2]
    omega_vy = yin[3] + step * y1[3]
    yout = [vx, vy, omega_vx, omega_vy]
    return yout
...
#accuracy decreases the more loops there are. This is most apparent in the x-y plot.
#increasing the number of steps increases accuracy. It corresponds with Euler's method observations.
#radius is appears proportional to initial velocity. These are consistent with equations above where dr/dt = v.
