...
FinalListbyHC=df_hc[df_hc['Cluster_labels']==0].sort_values(by = ['child_mort','income','gdpp',], ascending = [False, True, True])
...
df_list_cm = pd.DataFrame(FinalListbyHC.groupby(['country'])['child_mort'].mean().sort_values(ascending = False)).head()
...
df_list_in = pd.DataFrame(FinalListbyHC.groupby(['country'])['income'].mean().sort_values(ascending = True)).head()
...
df_list_gdp =pd.DataFrame(FinalListbyHC.groupby(['country'])['gdpp'].mean().sort_values(ascending = True)).head()
