...
nfl_missing_percentage = (total_missing/total_cells) * 100
print("total missing percentage of nfl data: {0:.2f}%".format(nfl_missing_percentage))
# your turn! Find out what percent of the sf_permits dataset is missing
sf_total_cells = np.product(sf_permits.shape)
sf_missing_values_count = sf_permits.isnull().sum().sum()
sf_missing_percentage = (sf_missing_values_count / sf_total_cells) * 100
print("total missing percentage of sf permits: {0:.2f}%".format(sf_missing_percentage))
# look at the # of missing points in the first ten columns
missing_values_count[0:10]
