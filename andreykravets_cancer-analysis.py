...
# определим объекты из данных и ответов для загрузки в data loader
...
# Making model and stating what to do with GPU
def to_gpu(tensor):
...
# Making optimizer to dealing with backprop
lr = 1e-3
optimizer = RMSprop(resnet9.parameters(), lr)
# Crossentropy loss
loss = nn.BCEWithLogitsLoss()
# Func for printing score
...
def auc_writer(y_true, y_predicted, iteration):
    ...
    logger.info(f'iteration: {iteration}, score: {score}')
...
def predict(model, dataloader):
    ...
    return y_true, y_hat
# Stuff to deal with tensor and convert it to numpy
...
# Some triggers for doing backprop or printing score
...
# Big func mixing all functions above together
...
def train_one_epoch(model, 
    ...
    for iteration, (x, y) in enumerate(train_data_loader):
        ...
        if valid_loss_trigger(iteration):
            ...
            score.append(auc_writer(y_true_valid, y_hat_valid, iteration)) 
    
    return model, score

def train_several_epoch(num_epoch,
                        model, 
                        train_dataloader, 
                        valid_dataloader, 
                        loss, 
    ...
    for n in range(num_epoch):
        print(f'{n} epoch started')
        model, score = train_one_epoch(model, 
                    train_dataloader, 
                    valid_dataloader, 
                    loss, 
                    optimizer,
    ...
    print(f'Average ROC-AUC score: {np.array(score_list).mean()}')
    
    return model
do_step_trigger, valid_loss_trigger, train_loss_trigger = init_triggers(1, 20, 10)
resnet9 = train_several_epoch(3,
                        resnet9, 
                        train_dataloader, 
                        valid_dataloader, 
                        loss, 
