...
df = pd.read_csv('../input/AH-Data - Data.csv')
df.head(2)
...
ax[0,0].set_title('AVERAGE STRESS LEVELS BY AGE GROUP')

ax[0,1].set_title('STRESS BY GENDER')

ax[0,2].set_title('STRESS BY AGE & GENDER')

ax[1,0].set_title('STRESS & DAILY HOURS OF FLOW')

ax[1,1].set_title('STRESS & DAILY HOURS OF MEDITATION')

ax[1,2].set_title('STRESS & SUFFICIENT INCOME:2=sufficient,1=not')
...
sns.pointplot(x = 'DAILY_MEDITATION', y = 'DAILY_STRESS', data=df, ax = ax[1,1])

sns.pointplot(x = 'SUFFICIENT_INCOME', y = 'DAILY_STRESS',  data=df, ax = ax[1,2])



f.suptitle('HEALTHY MIND\nWHAT DRIVES OUR DAILY STRESS LEVELS?', fontsize=20)
