...
train_cats(df)
df.BsmtFinType2.cat.categories
df.BsmtFinType2.cat.set_categories(['GLQ', 'ALQ', 'BLQ', 'Rec', 'LwQ', 'Unf', 'NA'], ordered=True, inplace=True)
...
m.fit(df1, y)
m.score(df1,y)
test_df = pd.read_csv('/kaggle/input/home-data-for-ml-course/test.csv', low_memory=False)
...
def print_score(m):
    ...
    print(res)
train_cats(test_df)
test_df.BsmtFinType2.cat.set_categories(['GLQ', 'ALQ', 'BLQ', 'Rec', 'LwQ', 'Unf', 'NA'], ordered=True, inplace=True)
test_df.BsmtFinType2 = test_df.BsmtFinType2.cat.codes
...
preds = m.predict(df2)
df2['SalePrice'] = preds
m.score(df2.drop('SalePrice', axis=1),df2['SalePrice'])
df3 = df2[['Id', 'SalePrice']]
