...
def rf_evaluator(ad,depth):
    ...
    return model
## cleaning and evaluating data to produce a baseline for further development
...
model_evaluator(all_data)
##CV mean encoder for df[column to be encoded, target]
...
## extracting the first item from shop names, label_encoding them and mapping them to all_data and test_data
...
## splitting the category names, first term/second term contains type of product eg. PC,PS2,etc
...
## Mean encoding all the label encoded columns
...
## mapping label encoded type_1 and type_2 data to test data
...
## mapping the mean_encoded data into the label_encoded data
...
## mean_encoding the data to test city_id
...
##selecting the most important k=10 features and assinging them to selected_columns_full 

##and selected_columns_full_test
...
rf=model_evaluator2(all_data[selected_columns_full])
...
for tree in rf.estimators_:
    ...
    predictions.append(tree.predict(valid[selected_columns_full_test]))
...
a=np.cumsum(predictions,axis=0)



for i in np.arange(1, predictions.shape[0] + 1):
...
for i in np.arange(0,a.shape[0]):
    ...
    mse.append(np.sqrt(mean_squared_error(valid['item_cnt_month'],a[i])))
...
## it can be seen that a RandomForest of 50 estimators is good
## this bubble takes a lot of time and it was run and the results and time taken are saved in the next bubble
...
for i in depth:
    ...
    rf_evaluator(all_data,i)

    print('time taken= ', time.time()-ts)
error=[2.1700737359987605,2.078964839071924, 2.0607923027275024, 2.0607906628081216, 2.0538587360590643,

              2.0567278128214124,2.0312918309481023,2.0252462465953505, 2.0363327297155247,2.0100612866687046]
...
time_taken=[76,90,103,118,131,141,151,163,173,184]
...
axes[0].plot(depth,error)

axes[0].set_title('depth vs error')

axes[1].plot(depth,time_taken)

axes[1].set_title('depth vs time_taken')
rf=rf_evaluator(all_data,7)
...
sample_submission_copy.to_csv('rf_with_text_and_index_lag_depth7.csv',index=False)

pd.read_csv('rf_with_text_and_index_lag_depth7.csv')

## best model with estimators=50,depth=7
all_data.to_pickle('all_data_19_features.pkl')

test.to_pickle('test_data_19_features.pkl')
...
print('training done for curr_date ',curr_date)
##stacking the meta data for test section
...
## training a linear model on the meta data
