...
slope=0.2
print('Number of images: ', len(os.listdir('../input/stanford-dogs-dataset/images/Images/')))
    ...
    def _load_subfolders_images(self, root):
        ...
        for path in pbar:
            ...
            if is_valid_file(path) and file not in black_list:
                ...
                annotation_dirname = next(dirname for dirname in os.listdir('../input/stanford-dogs-dataset/annotations/Annotation/') if dirname.startswith(annotation_basename.split('_')[0]))

                annotation_filename = os.path.join('../input/stanford-dogs-dataset/annotations/Annotation/', annotation_dirname, annotation_basename)
...
if load_cropped:
    ...
    dataset = FullCroppedDogsFolderDataset(root='../input/stanford-dogs-dataset/images/', transform=transform)

else:

    dataset = dset.ImageFolder(root='../input/stanford-dogs-dataset/images/', transform=transform)
...
for epoch in tqdm(range(n_epochs)): # We iterate over the epochs.
    ...
    if epoch % 5 == 0: # code to print the current learning rate
...
for i, j in enumerate(images[:32]):
    ...
    plt.imshow(j)
def getImages(n_images = 256):

    images = []

    for i in range(n_images):

        img = numpy.asarray(PIL.Image.open(f'{OUT_DIR}/image_{i:05d}.png'))
