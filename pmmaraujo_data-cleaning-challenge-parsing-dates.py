...
processing_dates['Year'] = unkown_out_volcanos['Last Known Eruption'].str.split(' ', expand=True)[0] # just year
processing_dates['Period'] = unkown_out_volcanos['Last Known Eruption'].str.split(' ', expand=True)[1] # just the period
processing_dates.head() # looks good
# How many 'options' there is for period?
processing_dates['Period'].unique() # 2, makes sence

BCE = list(processing_dates.Year[processing_dates['Period'].str.match('BCE')]) # just those BCE
CE = list(processing_dates.Year[processing_dates['Period'].str.match('CE')]) # just those BC
...
sns.distplot(new_period, kde=False, bins=31) # and done