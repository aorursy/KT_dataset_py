...
data.head()
data['class'] = data['class'].apply(lambda x:1 if x=='p' else -1).astype('int64')
data.head()
...
trainY=trainY.to_frame(name=target)
testY=testY.to_frame(name=target)
...
def node_weighted_mistakes(targets_in_node, data_weights):
    ...
    weight_positive = data_weights.dot(targets_in_node==1)# TODO
    ...
    # 计算lable 为+1的所有数据的权重和
    #TODO
    weight_negative = data_weights.dot(targets_in_node==-1)# TODO
    ...
    #将加权错误和对应的预测标签一起输出
    #TODO
    return (weighted_mistakes_negative,-1) if weight_positive<weight_negative else (weighted_mistakes_positive,1)
#test
example_targets = np.array([-1, 1, -1, 1, 1])
example_data_weights = np.array([1., 2., .5, 1., 1.])
print(node_weighted_mistakes(example_targets, example_data_weights))
def best_split_weighted(data, features, target, data_weights):
    ...
    for feature in features:
       # print("feature",feature)
        ...
        # 进入左分支数据点的权重
        left_data_weights = data_weights[data[feature]==0]# TODO
        ...
        # 进入右分支数据点的权重
        right_data_weights = data_weights[data[feature]==1] # TODO
        #Debug notes: 这里一定要将target转化成int64,切记。 
        # 重点！！
        # 计算左边分支里犯了多少错 (加权结果！！)
        left_misses, left_class = node_weighted_mistakes(left_split[target].astype('int64'), left_data_weights)          
        # 计算右边分支里犯了多少错 (加权结果！！)
        right_misses, right_class = node_weighted_mistakes(right_split[target].astype('int64'), right_data_weights) 
            
        # 计算当前划分之后的分类犯错率
        error = left_misses+right_misses # TODO
       # print(error)
...
print(best_split_weighted(train_data, features, target, example_data_weights))
def create_leaf(target_values, data_weights):
    ...
    weighted_error, prediction_class = node_weighted_mistakes(target_values.astype('int64'), data_weights)
...
def create_weighted_tree(data, data_weights, features, target, current_depth = 0, max_depth = 10, min_error=1e-15):
    ...
    # TODO:
    error,_= node_weighted_mistakes(data[target].astype('int64'), data_weights)
    if error<=min_error:
    ...
    # 递归上面的步骤
    left_tree = create_weighted_tree(left_split,left_data_weights,remaining_features,target, current_depth +1, max_depth = max_depth, min_error=1e-15)       
    right_tree = create_weighted_tree(right_split,right_data_weights,remaining_features,target, current_depth +1, max_depth = max_depth, min_error=1e-15)
...
class WeightedDecisionTree(BaseEstimator):
    ...
    def __init__(self, max_depth=10, min_error=1e-15):
    ...
    def fit(self, X, Y, data_weights = None):
        ...
        features = X.columns
        target = Y.columns[0]
        self.root_node =create_weighted_tree(data_set, data_weights, features, target, current_depth = 0, max_depth = 3, min_error=1e-15)
...
class MyAdaboost(BaseEstimator):
    ...
    def fit(self, X, Y):
        ...
        N, _ = X.shape
        alpha =np.array([1/N]*N) # TODO    # data weights
