...
x.head()
df[["date", "item_cnt_day"]].set_index("date").resample("M").sum().plot()
...
df_resample_question_1 = df_for_question_1.resample('D')['item_cnt_day'].sum().to_frame()
df_resample_question_1.head(2)
df_resample_question_1['MA_sales'] = df_resample_question_1['item_cnt_day'].rolling(window=7).mean()
df_resample_question_1.head(2)
df_resample_question_1['MA_7_var'] = df_resample_question_1['MA_sales'].diff()/df_resample_question_1['MA_sales'].shift()
df_resample_question_1.head(2)
df_resample_question_1.dropna(inplace=True,axis=0)
df_resample_question_1.head(2)
fig = plt.figure(figsize = (20,10))

ax = fig.add_subplot(111)

ax.plot(df_resample_question_1['MA_sales'], label= 'Moving Average')
...
#puedes generar 2 modelos diferentes para la parte de trend y para la parte seasonal y después añadirle el ruido
...
df_for_question_2 = df[df['city']=='Адыгея']
df_timeindex1 = df_for_question_2.set_index("date").resample("W")["item_cnt_day"].sum().to_frame()
...
result1 = seasonal_decompose(df_timeindex1["item_cnt_day"])
...
result1.observed.plot(ax = axes[0], legend = False)
...
result1.trend.plot(ax = axes[1], legend = False)
...
result1.seasonal.plot(ax = axes[2], legend = False)
...
result1.resid.plot(ax = axes[3], legend = False)

axes[3].set_ylabel('Residual')



xtick_location = df_timeindex1.index.tolist()
...
porcen = df_for_question_3[my_pct>=0.010]
...
labels = ['{} - Sales :{}k \n {}% of total'.format(city, sales/1000, round(pct, 2)*100) for city, sales, pct in zip(df_for_question_3.index, my_values, my_pct)]
...
# remove axes, set title and display plot

plt.title("Sales by item and their % over total sales", fontsize = 23, fontweight = "bold")

plt.axis('off')

plt.tight_layout()
df.pivot_table(index = ["date"], columns = ['item_id'], values = "item_cnt_day", aggfunc = sum)
df.pivot_table(index = ["date"], columns = ['item_id'], values = "item_cnt_day", aggfunc = sum).isnull()
...
# that we don't have any missing values. But its not true, we only have the reported salesa
...
plot.set_title("Null sales by item_id and day");
#se podría poner las ventas esporadicas como 0 para mejorar
gc.collect()
...
#se podrían eliminar las tiendas que han cerrado
gc.collect()
...
#los ouliers los podemos eliminar para despues hacer el modelo
