...
data=pd.read_csv('../input/2020mcm-c-2/2b_data_pacifier.csv',sep=',')
...
plt.show()
def get_season_data_down(year,index):

    season_data=small_data[index][str(year)+'-7'].append(small_data[index][str(year)+'-8']).append(small_data[index][str(year)+'-9']).append(small_data[index][str(year)+'-10']).append(small_data[index][str(year)+'-11']).append(small_data[index][str(year)+'-12'])

    return season_data

def get_season_data_up(year,index):

    season_data=small_data[index][str(year)+'-1'].append(small_data[index][str(year)+'-2']).append(small_data[index][str(year)+'-3']).append(small_data[index][str(year)+'-4']).append(small_data[index][str(year)+'-5']).append(small_data[index][str(year)+'-6'])

    return season_data
get_season_data_down(2013,1).to_csv('./top_2_2013.csv')

get_season_data_up(2014,1).to_csv('./top_2_2014.csv')