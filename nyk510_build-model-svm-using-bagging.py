...
def timer(logger=None, format_str='{:.3f}[s]', prefix=None, suffix=None):
    ...
    log_func('start at {} ... '.format(start))
...
bagging_svm = BaggingRegressor(
    ...
    n_jobs=1, # run single (parallel run in `MultiOutputRegressor` phase.)
...
for (ff, (id0, id1)) in enumerate(kf.split(xtrain)):
    ...
    with timer(logger, prefix='fold {} fit '.format(ff)):

        mo_base.fit(x0,y0)

    

    with timer(logger, prefix='fold {} predict '.format(ff)):
