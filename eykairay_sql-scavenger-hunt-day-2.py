...
query = """SELECT type, COUNT(id) as count
           ...
           GROUP BY type
        """
type_countstories = hacker_news.query_to_pandas_safe(query)
type_countstories
hacker_news.head('comments')
...
# Summary stats for top 10 publishing authors
query3 ="""SELECT `by`, 
                COUNT(`by`) as count,
           ...
           GROUP BY `by`
           HAVING SUM(score) > 0
           ORDER BY COUNT(`by`) DESC, AVG(score)
           LIMIT 10
        """
type_stats = hacker_news.query_to_pandas_safe(query3)
type_stats