...
for feat_name in base_features:
    ...
    pdp.pdp_plot(pdp_dist, feat_name)
    plt.show()
# People from the center of New York will take a shorter ride any other place.
q_1.solution()
# Add your code here
feature_names = ['pickup_longitude', 'dropoff_longitude']

inter1 = pdp.pdp_interact(model = first_model, dataset = val_X, model_features = base_features, features = feature_names)

pdp.pdp_interact_plot(pdp_interact_out = inter1, feature_names = feature_names, plot_type = 'contour')
plt.show()
# The value on the diagonal means near pickup and dropoff places.
q_2.solution()
savings_from_shorter_trip = 25.50 - 10.50
...
X2 = 4 * rand(n_samples) - 2
# Create y. you should have X1 and X2 in the expression for y
# Product of two uniform distribution
y = X1 * X2
