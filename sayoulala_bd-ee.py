...
class Feature(object):
    ...
    def __init__(self,item_id,or_text,mapping,mapping_off,token_ids,labels,entype_trigger_indexs,cls_label_65,cls_label_6,cls_label_number,cls_type,slip_off,crf_trigger):
...
class zy_DataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, data_list,train_mode=False, val_mode=False,test_mode=False,labeled=True):
        ...
        if self.train_mode:
            ...
            self.features =self.get_val_features(self.data_list)
    ...
    def get_val_features(self,data_list):
...
#                 print(data)
            ...
            for event_key,arguments in enent_key_argument:
                ...
                if start:
                    ...
                    type_set.append(enent_type) #有多少个trigger

                    cls_label_number+=1  #有多少number
...
#                 print(data)
            ...
            for event_key,arguments in enent_key_argument:
                ...
                if start:
                    ...
                    entype_trigger_indexs.append(event_key) #有的事件
...
#             cls_type-=1
    ...
    def __getitem__(self,index):
        ...
        cls_label_type=torch.tensor(np.array(feature.cls_type).astype(np.float32))

        crf_trigger=torch.tensor(np.array(feature.crf_trigger).astype(np.float32)).long()

        return token_ids,seg_ids,labels,cls_label_65,cls_label_6,cls_label_number,cls_label_type,crf_trigger
...
def debug_loader(train_data):
    ...
    for token_ids, seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger in train_loader:
...
#     if t.id=='06aa1d16634f3bbff6c5c87ac29fe372':

#         print(t)

# #     entype_trigger_indexs=t.entype_trigger_indexs

# #     for e in entype_trigger_indexs:
...
class zy_Model(nn.Module):
    ...
    def __init__(self, n_classes=3):
        ...
        self.hidden_fc= nn.Sequential(nn.Linear(dim, num_labels),nn.ReLU(True))
        ...
        self.zy_cls_type_hidden= nn.Sequential(nn.Linear(dim+num_labels, 300),nn.ReLU(True),nn.Linear(300, 1))
    ...
    def forward(self,ids,seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger,is_test=False):
        ...
        emissions=self.hidden_fc(last_seq)
        ...
        out_type=self.zy_cls_type_hidden(type_feat_hid_number).sigmoid()

        

        #crf2

#         emissions2=self.hidden_fc_crf2(torch.cat([last_seq,emissions.detach()],2))
        ...
        if not is_test:
            ...
            c_type_loss=criten(out_type**2,cls_type)
...
def debug_label():
    ...
    for token_ids, seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger in loader:

        print(token_ids.size())

        print(cls_number.size())

        loss,cls_loss = model(token_ids, seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger,is_test=False)

        print(loss)

        print(cls_loss)

        y= model(token_ids, seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger,is_test=True)
...
#     torch.cuda.manual_seed(2020)
    ...
    for i,(ids,seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger) in enumerate(bar):

        decode,cls_pred,decode2= model(ids.cuda(DEVICE),seg_ids.cuda(DEVICE),labels.cuda(DEVICE),cls_label_65.cuda(DEVICE),cls_label_6.cuda(DEVICE),cls_number.cuda(DEVICE),cls_type.cuda(DEVICE),crf_trigger.cuda(DEVICE),is_test=True)
...
#     crf_p=[n for n, p in param_optimizer if str(n).find('crf')!=-1]
    ...
    optimizer_grouped_parameters = [
            ...
            {'params': [p for n, p in param_optimizer if not any(nd in n for nd in no_decay) and n not in crf_p+crf2_bias+crf_p2], 'weight_decay': 0.8},

            {'params': [p for n, p in param_optimizer if any(nd in n for nd in no_decay) and n not in crf_p+crf2_bias+crf_p2], 'weight_decay': 0.0},

            {'params': [p for n, p in param_optimizer if n in crf_p], 'lr': 15e-4,'weight_decay': 0.8},

            {'params': [p for n, p in param_optimizer if n in crf_p2], 'lr': 15e-4,'weight_decay': 0.8},
    ...
    for epoch in range(1,epochs+1):
        ...
        for i,(ids,seg_ids,labels,cls_label_65,cls_label_6,cls_number,cls_type,crf_trigger) in enumerate(bar):
            ...
            loss,cls_loss_65 = model(ids.cuda(DEVICE),seg_ids.cuda(DEVICE),labels.cuda(DEVICE),cls_label_65.cuda(DEVICE),cls_label_6.cuda(DEVICE),cls_number.cuda(DEVICE),cls_type.cuda(DEVICE),crf_trigger.cuda(DEVICE),is_test=False)
...
losses=train_model(model,train_loader,val_loader,val_features,accumulation_steps=2,early_stop_epochs=2,epochs=20,model_save_path='right_recall1_2decode.pkl')
for l in losses:

    print(l)
model=zy_Model(num_labels).cuda(DEVICE)

model_save_path='right_recall1_2decode.pkl'
...
test_loader,test_features=get_loader(test_data,batch_size=8,test_mode=True,train_val=False)
# test_features
test_features[0]
...
def predict_to_file_qa(results,in_file,out_file):
    ...
    with open(in_file) as fr:
        ...
        for index,l in enumerate(fr):
            ...
            result=new_test_preds[l['id']]

            arguments={}

            for key,ans in result:
    ...
    fw.close()
def predict_to_file(results,in_file,out_file):
    ...
    fw.close()
# predict_to_file_qa(new_test_preds,data_path+'test1.json','sub_baseline_qa_crf2.json')
# new_test_preds['4fb231d4f3ae2d20154bbf15cca0b2ee']