...
def highest_prob_pair(crb):
    a = 0
    b = 0
    max_prob = 0
    for k1, second_rolls in crb.items():
        for k2, roll_prob in second_rolls.items():
            if roll_prob > max_prob:
                max_prob = roll_prob
                a = k1
                b = k2
    return a, b
...
crb = {1: {3: 0.5, 5: 0.6}, 3: {1: 1.0}, 5: {1: 1.0}}
hpp = highest_prob_pair(crb)
print(hpp)
a = hpp[0]
b = hpp[1]
print(a)
...
def highest_prob_pair(probs):
    a = 0
    b = 0
    max_prob = 0
    for k1, second_rolls in probs.items():
        for k2, roll_prob in second_rolls.items():
            if roll_prob > max_prob:
                max_prob = roll_prob
                a = k1
...
# collect a history of dice rolls
# find out what a and b is. after landing on a, significantly more likely to land on b

# agent to bet only on b
