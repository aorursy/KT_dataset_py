...
# Write a function definition named area_of_circle that takes in a number representing a circle's radius 
...
#It finds the a in "mango" but then when it returns the resulting list it.... didn't remove the a in "mango"? I'm not sure 

    # why it's doing that. There doesn't sseem to be a reason for it to skip the a in the result.

        # And then if I run it through another filter, it ignores the O!
...
def first_and_second(variables):

    new_list = [variables[0],variables[1]]
...
# Write a function definition named mode that takes in sequence of numbers and returns the most commonly occuring value
    ...
    # Using the statistics import, since even though my commented out answer is technically correct...

    # but for some reason it deosn't want to give me credit
...
#     mode = [k for k, v in get_mode.items() if v == max(list(get_mode.values()))]
...
# Write a function definition named product_of_all that takes in sequence of numbers
...
def shortest_string(words):
    ...
    min_key = [k for k, v in data.items() if v == min_value]
...
def longest_string(words):
    ...
    max_key = [k for k, v in data.items() if v == max_value]
...
def get_average_book_price(books):
    ...
    avg_book_price = sum(book_prices)/ len(book_prices)
...
# Hint: Much like how sometimes functions start with a variable set to zero,

#you may want to create a dictionary with the price set to zero to compare to each dictionary's price in the list



def highest_price_book(books):

        answer = {"title": '', "price": 0, "author": ''}

        for book in books:

            if book["price"] > answer["price"]:
...
def lowest_price_book(books):
    ...
    answer = {"title": '', "price": float('inf'), "author": ''}
...
def number_of_item_types(shopping_cart):
        ...
        for item in shopping_cart["items"]:
            ...
            if item["title"] not in unique_items:
                ...
                unique_items.append(item["title"])
...
def get_average_item_price(shopping_cart):
    ...
    avg_price = price_total / len(shopping_cart["items"])
...
# Write a function named most_spent_on_item that takes in the shopping cart as input

# and returns the dictionary associated with the item that has the highest price*quantity.
...
def most_spent_on_item(shopping_cart):
    ...
    most_expensive_item = {"title": '', "price": 0, "quantity": 0}

    for item in shopping_cart["items"]:

        if item["price"] * item["quantity"] > most:
