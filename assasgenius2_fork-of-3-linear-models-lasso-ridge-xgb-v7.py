...
model = xgb.cv(params, dtrain,  num_boost_round=500, nfold=2, early_stopping_rounds=100)
model.loc[30:,["test-rmse-mean", "train-rmse-mean"]].plot()
model_xgb = xgb.XGBRegressor(n_estimators=200, max_depth=10, learning_rate=0.2) #the params were tuned using xgb.cv
...
predictions1 = pd.DataFrame({"xgb":xgb_preds, "lasso":lasso_preds})

predictions1.plot(x = "xgb", y = "lasso", kind = "scatter")
predictions2 = pd.DataFrame({"ridge":ridge_preds, "lasso":lasso_preds})

predictions2.plot(x = "ridge", y = "lasso", kind = "scatter")
predictions3 = pd.DataFrame({"ridge":ridge_preds, "xgb":xgb_preds})

predictions3.plot(x = "ridge", y = "xgb", kind = "scatter")
preds = 0.4*lasso_preds + 0.4*ridge_preds + 0.2*xgb_preds
