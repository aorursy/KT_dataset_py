...
class my_gbdt(object):
    ...
    def __init__(self):
        ...
        self.tree = DecisionTreeRegressor(min_samples_split=2, max_depth=6)
    ...
    def fit(self, X, y):
        ...
        for i in range(1, self.iter):
            ...
            self.tree.fit(X, (y - y_pred) * self.eta)

            y_pred = y_pred + self.tree.predict(X)
    ...
    def predict(self, X):
        ...
        for i in range(self.iter):
            ...
            pred_proba = pred_proba + self.eta * self.tree_list[i].predict(X)
