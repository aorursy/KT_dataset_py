...
a = 0
while (a < 1.0002): 
    rdge = Ridge(alpha=a).fit(trainx,trainy) 
    p = rdge.predict(xtestvals)
...
test_vals.head()
r = Ridge(alpha=0.05).fit(trainx,trainy)
predictions = r.predict(test_vals)
output=pd.DataFrame({'Id':np.arange(1,2127),'OverallScore':predictions})
output.to_csv('out_ridge.csv', index = False)
