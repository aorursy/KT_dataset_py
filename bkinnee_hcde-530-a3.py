...
#I experimented with the lengths of the names for both the boys' and girls' names to return only the longest names by simply reading the length of the keys
for name in boys.keys():
    if len(name)>=15:
        print(name)
for name in girls.keys():
    if len(name)>=14:
        print(name)
# the longest boys' names are 'christianjoseph', 'christopherjame', and 'jayden alexander'
# the longest girls' names are 'mariaguadalupe', 'mariadelcarmen', and 'sarahelizabeth'
# How many babies are in the dataset?
len(boys.keys()) + len(girls.keys()) #find the length of both lists, then add them together


    
# How many boy names are also girl names? How many girls' names are also boys' names?
for name in boys.keys():
    if name==name in girls.keys():
        print (count)
        
        # I don't know what is happening here, and why I always get '5' when I try to return 'count' as a variable?
