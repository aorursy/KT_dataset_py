...
df['Symbol'] = 'D'
df.head()
mercado = ['pan', 'leche', 'arroz', 'pastas']
mercado[1:3]
precios = {'pan': 1000, 'leche': 2000, 'arroz': 5000}
precios['pan']
df[['Close', 'Open', 'Symbol']].head()
...
df.head()
df = pd.read_csv('../sample_data/D.csv')
...
for i in simbolos:
    print(i)
    df_simbolo = df_uni[df_uni['Symbol'] == i]
    avg_volstat_simbolo = df_simbolo['VolStat'].mean()
    print(avg_volstat_simbolo)
...
df2[['retornos', 'utilidades']][df2['valor_final'] >= 3000000].describe()
# Una posible solución

# Se añade una columna para el año
lista_año = []
for i in df_uni['Date']:
    lista_año.append(i[:4])
    
df_uni['YYYY'] = lista_año

# Se agrupa por símbolo y luego se ejecuta un ciclo sobre cada grupo para volver a agrupar por año 
# y calcular el promedio de la volatilidad (VolStat)
grp = df_uni.groupby('Symbol')
for item in grp:
    print('------Símbolo: ', item[0])
    grp_df = item[1]
    df_vol_año = grp_df[['YYYY','VolStat']]
    print(df_vol_año.groupby('YYYY').mean())
df_uni.head()
df_uni['Volume_Millions'][(df_uni['Return'] >= 0) & (df_uni['Symbol'] == 'D')].mean()
df_uni[['YYYY', 'Volume_Millions']][(df_uni['Return'] >= 0)].groupby('YYYY').mean()
