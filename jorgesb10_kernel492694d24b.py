...
TEST_CSV = '/kaggle/input/jovian-pytorch-z2g/submission.csv'   # Contains dummy labels for test image
train_df = pd.read_csv(TRAIN_CSV)
test_df = pd.read_csv(TEST_CSV)

# Consulta el tamaño del conjunto de datos disponibles 
print('Train_shape: {}  |  Test_Shape: {}'.format(train_df.shape,test_df.shape))
print('Total_Train_Images: {}  |  Total_Test_Images: {}'.format(len(os.listdir(TRAIN_DIR)), len(os.listdir(TEST_DIR))))
labels = {
    0: 'Mitochondria',
    1: 'Nuclear bodies',
    2: 'Nucleoli',
...
lbls_count = {}
for lbls in train_df['Label']:
    for label in lbls.split(" "):
        if label not in lbls_count:
            lbls_count[label] = 1
        else:
            lbls_count[label]+= 1

print(collections.OrderedDict(sorted(lbls_count.items())))
    ...
    for i, x in enumerate(target):
        if (x >= threshold):
            if text_labels:
                ...
                result.append(str(i))
    return ' '.join(result)
encode_label('2 4 5'),decode_target(torch.tensor([0, 0, 1, 0, 1, 1, 0, 0, 0, 0.]))
...
def create_datasets(dataset, seed):
    ...
    val_size = int(len(dataset)*5/100)
    train_size = int(len(dataset)-val_size)
    train_ds, val_ds = random_split(dataset, [train_size, val_size])
    print("Total_Dataset : {}   |   len_train_ds : {}   |   len_val_ds : {}".format(len(dataset),len(train_ds),len(val_ds)))
    return train_ds,val_ds

train_ds, val_ds = create_datasets(dataset,2020)
batch_size = 64
...
class ProteinCnnModel(MultilabelImageClassificationBase):
    def __init__(self):
        ...
        self.network = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=4, padding=2, stride=1),
            nn.PReLU(),
            nn.MaxPool2d(2, 2),
            nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=2),#pading 1
            nn.PReLU(),
            nn.MaxPool2d(2, 2),
...
predict_single(dataset[23][0])
show_sample(*dataset[23])
predict_single(dataset[2345][0])
show_sample(*dataset[2345])
...
import jovian
name_project = "Kaggle_exercise"
jovian.commit(project=name_project)
