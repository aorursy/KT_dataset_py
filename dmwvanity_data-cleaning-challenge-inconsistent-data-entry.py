...
cities.sort()
cities
# get the top 10 closest matches to "baluchistan" to know how to set min_ratio
matches = fuzzywuzzy.process.extract("baluchistan", province, limit=10, scorer=fuzzywuzzy.fuzz.token_sort_ratio)

# take a look at them
matches
# use the function we just wrote to replace close matches to "baluchistan" with "baluchistan" in the Province column we looked at above. min_ration = 90 will work fine.
replace_matches_in_column(df=suicide_attacks, column='Province', string_to_match="baluchistan")
# now let's check the list of unique values in Province column
# get all the unique values in the 'City' column
province = suicide_attacks['Province'].unique()
