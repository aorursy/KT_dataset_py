...
target_enc = ce.TargetEncoder(cols=no_ip)



# Learn encoding from the training set. Use the 'is_attributed' column as the target.

target_enc.fit(train[no_ip], train['is_attributed'])
...
train_encoded = train.join(target_enc.transform(train[no_ip]).add_suffix('_target'))

valid_encoded = valid.join(target_enc.transform(valid[no_ip]).add_suffix('_target'))



print(train_model(train_encoded, valid_encoded))
