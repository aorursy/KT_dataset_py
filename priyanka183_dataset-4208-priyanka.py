...
data = pd.read_csv('../input/file-abcdefghi/filename-a-b-c-d-e-f-g-h-i.csv')
data.keys()
#text = data['article']
#print(text[1])
#print(data.keys())
...
output_tokenizer  = Tokenizer(num_words=max_features) 
output_tokenizer.fit_on_texts(outputs+outputs1)
#output_tokenizer.fit_on_texts(label)
...
y_voc  =   len(output_word_index) + 1
print('Found %s unique tokens.' %y_voc)
max_output_len = max(len(sen) for sen in output_sequence1)
#max_output_len1=max_output_len/2
print("Length of longest sentence in input: %g" % max_output_len)
...
history=model.fit([encoder_input_sequences_train,decoder_input_sequences_train],decoder_targets_one_hot[0:2819]
    ,validation_split=0.33,epochs=20,batch_size=128)
model.save('model.h5')
train_acc = model.evaluate([encoder_input_sequences_train, decoder_input_sequences_train],decoder_targets_one_hot[0:2819],
                           verbose=0)
print (train_acc)
test_acc = model.evaluate([encoder_input_sequences_test, decoder_input_sequences_test],decoder_targets_one_hot[2819:],
                          verbose=0)
print (test_acc)
...
print('-')
print('Input:', text[i])
print('Response1:', seq2key(decoder_input_sequences_test[i]))
print('Original:', label[i])
print('Response2:', translation)
nb_words=min(max_features,len(input_word_index))
print(nb_words)
embedding_matrix=np.zeros((nb_words+1,embed_size))      #50000,embed size rakhlam
...
encoder_input_sequences_train,encoder_input_sequences_test,decoder_input_sequences_train,decoder_input_sequences_test=train_test_split(encoder_input_sequences,decoder_input_sequences,test_size=0.33,random_state=42)        
print(len(encoder_input_sequences_train))
embedding_layer=Embedding(max_features+1,embed_size,input_length=max_input_len)
decoder_targets_one_hot = np.zeros((
        len(text),
        max_output_len,
        y_voc
...
from tensorflow.keras.callbacks import EarlyStopping
es = EarlyStopping(monitor='val_loss', mode='min', verbose=1,patience=5)
history1=model.fit([encoder_input_sequences_train,decoder_input_sequences_train],decoder_targets_one_hot[0:2819]
