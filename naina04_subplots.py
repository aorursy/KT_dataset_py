...
reviews['points'].value_counts().sort_index().plot.bar(
    ax=axarr[0][1],color='red'
)

reviews['province'].value_counts().head(20).plot.bar(
    ax=axarr[1][1],color='red'
)
...
reviews['country'].value_counts().plot.hist(
    ax=axarr[0][2], fontsize=12, color='mediumvioletred'
)
axarr[0][2].set_title("Country", fontsize=18)

reviews['designation'].value_counts().plot.hist(
    ax=axarr[1][2], fontsize=12, color='mediumvioletred'
)
axarr[1][2].set_title("Designation", fontsize=18)
...
fig, axarr = plt.subplots(2, 2, figsize=(20, 12))
pokemon['Attack'].plot.hist(ax=axarr[0][0], title='Pokemon Attack Ratings')
pokemon['Defense'].plot.hist(ax=axarr[1][1], title='Pokemon Defense Ratings')