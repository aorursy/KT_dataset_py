...
resnet50
class ProteinResnet50(ProteinResnet34):
    def __init__(self):
        super().__init__()
        # Use a pretrained model
...
resnet101
class ProteinResnet101(MultilabelImageClassificationBase):
    def __init__(self):
        super().__init__()
        # Use a pretrained model
...
densenet121
class ProteinDensenet121(ProteinResnet50):
    def __init__(self):
        super().__init__()
        # Use a pretrained model
...
history += fit_one_cycle(epochs, max_lr, model, train_dl, val_dl, 
                         ...
                         opt_func=opt_func)
model.unfreeze()
#model = to_device(ProteinResnet101(), device)
...
img.shape
predict_single(test_dataset[7333][0])
predict_single(test_dataset[12400][0])
...
sub_fname = sub_fname.format(readable)
