...
train
train.columns
cols = ['pe_present_on_image', 'negative_exam_for_pe', 'qa_motion', 'qa_contrast', 'flow_artifact', 'rv_lv_ratio_gte_1', 'rv_lv_ratio_lt_1',

       'leftsided_pe', 'chronic_pe', 'true_filling_defect_not_pe', 'rightsided_pe', 'acute_and_chronic_pe', 'central_pe', 'indeterminate']
...
fig.show()
data = train[['pe_present_on_image', 'negative_exam_for_pe', 'qa_motion', 'qa_contrast', 'flow_artifact', 'rv_lv_ratio_gte_1', 'rv_lv_ratio_lt_1',
...
ax.imshow(dcm.dcmread("../input/rsna-str-pulmonary-embolism-detection/train/4833c9b6a5d0/57e3e3c5f910/f4fdc88f2ace.dcm").pixel_array)
...
plt.title("HU unit distributions for 50 examples")
...
for i in im_path:
    ...
    example_dcm = os.listdir(train_path  + i + "/")[0]

    dataset = dcm.dcmread(train_path + i + "/" + example_dcm)
