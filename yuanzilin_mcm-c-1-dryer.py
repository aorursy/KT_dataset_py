...
slim_data = verified_data.loc[:,['review_id','product_id','vine','review_body','star_rating','helpful_votes','total_votes','review_date']]
...
small_sales=sales_number[(sales_number['cnt']>=1)&(sales_number['cnt']<=10)]
...
top_XX_data.info()
labels
top_XX_data.to_csv("./top_50_dryer.csv",index=False)
tmp=pd.read_csv('./top_50_dryer.csv',sep=',')
...
data=pd.read_csv('./dryer_score.csv',sep=',')
...
data.to_csv("ini_review_dryer.csv",index=False)
...
df_final_score.to_csv("final_dryer_score.csv")
data_2b.info()