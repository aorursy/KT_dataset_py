...
data['flag'].value_counts()/data.shape[0]
...
for i, col in enumerate(X.columns): 
      data.groupby('flag')[col].plot.kde()
      plt.ylabel('Probability')
      plt.xlabel(col)
      plt.title('KDE Plot for column '+ col)
      plt.legend(labels=data['flag'].unique().tolist())
      plt.show()
...
for i, col in enumerate(X.columns):
    ...
    dat2 = data[col][data['flag']==1]
    print(kruskal(dat1,dat2))
X = X[['currentBack','motorTempBack','positionBack','trackingDeviationBack','currentFront','motorTempFront']]
#correlation matrix
cor = round(X.corr(),2)
cor
#heatmap depicting the correlation between independent variables
sns.heatmap(cor, cmap="Blues", linewidths=0.3)
X.drop(["currentFront","motorTempFront"],axis=1, inplace=True)
X.columns
