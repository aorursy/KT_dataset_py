...
chess_games.head()
# Your code here
# def game_count_normalize(df):
#     chess_games_counts = len(chess_games)
#     df = df / chess_games_counts
#     return df
# check_q1(chess_games.winner.value_counts().pipe(game_count_normalize))
...
print(ramen_reviews.columns)
# Your code here
check_q6(((wine_reviews['points'].dropna() - 80) / 4).value_counts().sort_index().rename_axis("Wine Ratings"))
# Your code here
check_q7(ramen_reviews.Stars.replace('Unrated', None).dropna().astype('float64').value_counts().sort_index())
# Your code here
check_q8(ramen_reviews.Stars.replace('Unrated', None).dropna().astype('float64').map(lambda s: round(s * 2) / 2).value_counts().sort_index())