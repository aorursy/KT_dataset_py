...
[dictionary[i] for i in range(0,10)]
train_data.head()
...
def DT_RF_classifier(data, kind='DT', test_size=0.3, max_depth=None):
    ...
    return clf
DT_RF_classifier(train_data)
DT_RF_classifier(train_data, kind='RF')
...
plt.show()
test_features = prepare_test_data(test_data.drop('label', axis=1))
