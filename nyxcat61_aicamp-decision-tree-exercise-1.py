...
plt.show()
model_depth_3 = DecisionTreeClassifier(max_depth=3)    # 调用决策树模型并限制树深度为3
...
print('score for max_depth=%s tree: %s' % (3, model_depth_3.score(X, Y)))
...
print('score for max_depth=%s tree: %s' % (5, model_depth_5.score(X, Y)))
...
print('score for basic tree: %s' % model.score(X, Y))
...
print('score for max_depth=3 tree %s' % model_depth_3.score(X, Y))
...
print('score for max_depth=5 tree %s' % model_depth_5.score(X, Y))
...
theta = np.random.rand(N//2) * np.pi * 2  # 随机生成夹角
...
print('score for basic logistic regression: %s' % lr.score(X, Y))

plot_decision_boundary(X, lr)
