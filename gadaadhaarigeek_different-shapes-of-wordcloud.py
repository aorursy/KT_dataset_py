...
comments.shape
articles.shape
def plot_wordcloud(text, mask=None, max_words=400, max_font_size=120, figure_size=(20.0,10.0), 
...
plot_wordcloud(comments_text, comments_mask, max_words=100000, max_font_size=120,

               title = 'Most common words in all of the comments', title_size=50)
n = round(comments.shape[0])
...
plot_wordcloud(editorsPick_comments_text, star_mask, max_words=8000, max_font_size=120,

               title = 'Most common words in the comments selected as Editor`s pick', image_color=True)
userDisplayNames = str(comments.userDisplayName)*800000

user_mask = np.array(Image.open(d + 'user.png'))

plot_wordcloud(userDisplayNames, user_mask, max_words=1200, max_font_size=120, title = 'Usernames with most comments')
userLocations = str(comments.userLocation)*100000

loc_mask = np.array(Image.open(d + 'loc.png'))

plot_wordcloud(userLocations, loc_mask, max_words=800, max_font_size=40, figure_size=(11,16),

               title = 'Most common locations of the commenters')
headlines = str(articles.headline)*100

plot_wordcloud(headlines, max_words=800, max_font_size=30, figure_size=(12, 10),
