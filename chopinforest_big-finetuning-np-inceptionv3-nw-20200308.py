...
train_data1 = '../input/drd-ddr-aptos/ddr_drd_aptos/'



train_df1 = pd.read_csv('../input/regular-deepdrid/DR_label/DR_label/drd_ddr_aptos_shuffle.csv')

train_df1.rename(columns={'image':'image_id','level':'DR_level'},inplace=True) 

train_df1['path'] = train_df1['image_id'].map(lambda x: os.path.join(train_data1,

                                                         '{}'.format(x)))

train_df1['exists'] = train_df1['path'].map(os.path.exists)
...
train_df1=train_df1.loc[~train_df1.where(train_df1.exists==False).any(axis=1)]



print('Number of train samples: ', train_df1.shape[0])

print(train_df1['exists'].sum(), 'images found of', train_df1.shape[0], 'total')

display(train_df1.head())



train_df1.to_csv('drd32795.csv')
#数据EDA
...
ax1 = sns.countplot('DR_level', data=train_df1)
...
STEP_SIZE = len(train_df1) // BATCH_SIZE
...
model.save_weights('../working/Big-Finetuning-np-InceptionV3-nW-20200308.h5')
