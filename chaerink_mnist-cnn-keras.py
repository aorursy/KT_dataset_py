...
X_sample, X_val, Y_sample, Y_val = train_test_split(X_train, Y_train, test_size=0.25, random_state = 19)
def build_cnn_model():
...
cnn_model.fit(X_sample, Y_sample, batch_size = BATCH_SIZE, epochs=5)
...
sample.head(2)
sample.to_csv('CNN_2layers_Adagrad.csv', index=False)