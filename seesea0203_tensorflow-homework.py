...
x = tf.Variable(3, name='x_input')# TODO
y = tf.Variable(4, name='y_input')# TODO
f = x * x * y + y + 2# TODO
...
with tf.Session() as sess:
    ...
    print(x.eval())
    print(y.eval())
    result = f.eval(feed_dict={x:2, y:1}) # We can use either sess.run(f, feed_dict={}) or f.eval(feed_dict={}) 
print(result)
...
class LinearRegressionTF(BaseEstimator):
    ...
    def fit(self, trainX,trainY):
        N = len(trainX) # TODO  总共多少个数据
        # 图的输入
        self.X = tf.placeholder(tf.float32) # TODO
        self.Y = tf.placeholder(tf.float32)# TODO
        ...
        # mean squre error
        cost = tf.square(tf.reduce_sum(tf.subtract(self.pred, self.Y))) / N # TODO
        ...
        if self.annotate:
            ...
            plt.plot(trainX, self.sess.run(self.pred, feed_dict={self.X:trainX, self.Y:trainY}), label='Fitted line')
            plt.legend()
        ...
        for epoch in range(self.training_epochs):
            ...
            if (epoch+1) % display_step == 0:
                c = self.sess.run(cost,feed_dict={self.X:trainX, self.Y:trainY}) # We can not use cost.eval(feed_dict={}) because No default session is registered.
...
lr = LinearRegressionTF(learning_rate, training_epochs, display_step, annotate=True)# TODO
...
class LogisticRegressionTF(BaseEstimator):
    ...
    def fit(self, trainX,trainY):
        ...
        self.pred = tf.nn.sigmoid(output_logits)#TODO   # turn logits to probability
        ...
        # 优化器
        optimizer = tf.train.GradientDescentOptimizer(learning_rate).minimize(cost)# TODO
...
logisticTF = LogisticRegressionTF(learning_rate,training_epochs,display_step,annotate=True)#TODO
logisticTF.fit(trainX, trainY)
from sklearn.model_selection import cross_val_score
cross_val_score(logisticTF, trainX, trainY, cv=5).mean()
# The visulization with cross validation can not work.
# I can not figure out which parameter I need adjust and solve the problem.
logisticTF = LogisticRegressionTF(learning_rate,training_epochs,display_step)#TODO
logisticTF.fit(trainX, trainY)
from sklearn.model_selection import cross_val_score
cross_val_score(logisticTF, trainX, trainY, cv=5).mean()
tf.reset_default_graph()
...
#data_folder = "data/ninechapter-digitsub"
...
xtr = tf.placeholder(tf.float32, [None, 784])# TODO
xte = tf.placeholder(tf.float32, [784])# TODO
distance = tf.sqrt(tf.reduce_sum(tf.pow(tf.subtract(xtr, xte), 2), axis=1))# TODO
...
'''
I have revised KVALUE into 2, but got the same result.
I'm wondering if I have neglected some rivision in next cell to make the change of KVALUE work.
'''
...
with tf.Session() as sess:
    ...
    for i in range(len(testX)):
        ...
        result = c.most_common()[0][0] # TODO
        '''
        We can use c.most_common() with either KVALUE or no KVALUE
        '''
        # 计算最近邻的标签和真实标签值
...
class NaiveBayesTF(BaseEstimator):
    ...
    def predict(self, testX):
        ...
        cond_probs = tf.reduce_sum(self.dist.log_prob(tf.reshape(tf.tile(testX, [1,num_features]),[-1, num_classes, num_features])),axis=2)# TODO
        ...
        posterior = tf.add(cond_probs, priors)# TODO
...
class RandomForestTF(BaseEstimator):
    ...
    def fit(self, X, Y, num_steps, batch_size,max_nodes):
        ...
        correct_prediction = tf.equal(tf.argmax(self.infer_op, axis=1), tf.cast(self.Y, tf.int64))# TODO
        self.accuracy_op = tf.reduce_mean(tf.cast(correct_prediction, tf.float64)) # 'float', tf.float32, tf.float64
