...
g = sns.FacetGrid(df, col="Position", col_wrap=15)
g.map(sns.kdeplot, "Overall")
df = footballers[footballers['Position'].isin(['ST', 'GK'])]
...
pokemon
g = sns.FacetGrid(pokemon, row="Legendary",row_order=[True,False])
g.map(sns.kdeplot, "Attack")
g = sns.FacetGrid(pokemon, col="Legendary", row="Generation",row_order=[6,5,4,3,2,1],col_order=[False,True])
g.map(sns.kdeplot, "Attack")
sns.pairplot(pokemon[['HP', 'Attack', 'Defense']])
import matplotlib.pyplot as plt
s = sns.pairplot(pokemon[['HP', 'Attack', 'Defense']])
s.map_diag(sns.distplot)
