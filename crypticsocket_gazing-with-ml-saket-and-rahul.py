...
def Architecture():
    model = Sequential()
    model.add(Lambda(lambda x : x, input_shape=(110,110,3))) # Depending on size of input image modify number
...
def process_images(path):
    arr = np.zeros(shape=(1,110,110,3))
    img = plt.imread(path[0])
    arr[0] = img[160:270,160:270]
    return arr
...
imgp = imgp.reshape((110,110,3)) # input size here
