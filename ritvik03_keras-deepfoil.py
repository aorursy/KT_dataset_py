...
model.summary()
def load_data(filename = "../input/final_translated_data_1_6.csv"):
...
#   plot_time_data(time)
model.load_weights("../input/keras-deepfoil/18X32_120epoch.h5")
...
checkpoints=keras.callbacks.callbacks.ModelCheckpoint("model_3.h5", monitor='mse', verbose=0, save_best_only=True, save_weights_only=False, mode='min', period=1)

uvpEta_train = np.concatenate([u_train,v_train,p_train,eta_train],1)

h = model.fit([t_train,x_train,y_train],[uvpEta_train], initial_epoch = init_epoch,epochs=n_epoch, batch_size=batch_size, shuffle=True, callbacks = [checkpoints])
model.save("18X32_140epoch.h5")
import keras
