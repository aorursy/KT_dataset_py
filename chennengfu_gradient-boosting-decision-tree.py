...
#change lamda to 0.1 and alpha to 0.1
...
# change metric to 'auc' ,max_depth to 6, learning_rate to 0.1
...
gbdt_train_label_1 = gbdt_train_label
...
print('gbdt train label: ', gbdt_train_label[0:10])
...
class my_gbdt(object):
    ...
    def __init__(self):
        ...
        for i in range(self.num_trees):
            ...
            self.trees.append(DecisionTreeRegressor(min_samples_split=2, max_depth=6))
    ...
    def fit(self, X, y):
        ...
        y1 = self.trees[0].predict(X)

        for j in range(1, self.num_trees):

            delta = y1 - y

            self.trees[j].fit(X, delta)

            y1 -= self.trees[j].predict(X) * self.learning_rate
    ...
    def predict(self, X):
        ...
        for j in range(1, self.num_trees):
            ...
            pred_proba -= self.trees[j].predict(X) * self.learning_rate
