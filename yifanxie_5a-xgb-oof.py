...
train_input.head(5)
def cross_validate_xgb(params, x_train, y_train, x_test, kf, verbose=True, verbose_eval=50):
    ...
    d_test = xgboost.DMatrix(x_test, silent=True)
    ...
    for i, (train_index, val_index) in enumerate(kf.split(x_train, y_train)):
        ...
        pred = bst.predict(d_test, ntree_limit=bst.best_ntree_limit) # generate test prediction from each in-fold model

        test_pred += pred #add in-fold test prediction into the final prediction
    ...
    test_pred /= kf.n_splits #perform weigh averaging on each test prediction
    ...
    return bst, np.array(nround).mean(), test_pred
xgb_params = {
...
bst, n_round, test_xgb_preds=cross_validate_xgb(xgb_params, train_input, y, test_input, kf)
...
xgb_submission.to_csv('xgb_oof_submission_kf5.gz', compression='gzip', index=False)
...
xgb_submission.to_csv('xgb_oof_submission_kf10.gz', compression='gzip', index=False)
xgb_params = {
    ...
    "eta":0.0519,  # default 0.3

    "max_depth" : 6, # default 6

    "subsample" : 0.7829, # default 1

    "colsample_bytree" : 0.6123, # default 1
...
xgb_submission.to_csv('xgb_oof_submission_kf10_bayesopt.gz', compression='gzip', index=False)