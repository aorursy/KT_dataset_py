...
def estimate_average_slot_payout(n_runs):
    ...
    """
    return((sum([play_slot_machine()-1 for r in range(n_runs)])/n_runs))
estimate_average_slot_payout(10000)
q4.solution()
def indamoney(start_balance, n_spins):
    #print("sb:{} spin:{}".format(start_balance,n_spins))
    rbalance = start_balance
    for i in reversed(range(n_spins)):
        rbalance += play_slot_machine()-1
        #print("{}: rb:{}".format(i,rbalance))
        if rbalance < 1 and i > 0:
            return False
    return True
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    .25
    """
    return(sum([indamoney(start_balance, n_spins) for _ in range(n_simulations)])/n_simulations)
