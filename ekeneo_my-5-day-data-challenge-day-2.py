...
# Or This version will show all the columns, including non-numeric
...
print(nutrition.columns)
# get the Trans Fat (g) column

TransFat = nutrition["Trans Fat (g) "]
# Plot a histogram of Trans Fat content

plt.hist(TransFat, bins=9, edgecolor = "black")

plt.title("Trans Fat content in Starbucks Menu Items")# add a title

plt.xlabel("Trans Fat in grams") # label the x axes 

plt.ylabel("Count") # label the y axes. In histogram it is usually called "Count".
## ploting a histogram (from the pandas plotting API)

# using figsize is an argument to make it bigger

nutrition.hist(column= "Trans Fat (g) ", figsize = (12,12), edgecolor = "black")
