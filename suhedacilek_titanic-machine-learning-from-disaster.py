...
name = train_df["Name"]  #strip gereksiz şeyleri siliyor.

train_df["Title"] = [i.split(".")[0].split(",")[-1].strip() for i in name] #virgülden sonra sonuncu eleman [-1], noktadan önce string [0].
...
train_df.head()
train_df.head()
train_df["Fsize"] = train_df["SibSp"] + train_df["Parch"] + 1 #+1 mean is a person who doesn't have any parents and children.
train_df.head()
...
train_df["Ticket"].head(20)
train_df.head()
train_df = pd.get_dummies(train_df, columns= ["Ticket"], prefix = "T") #ticket _ oluşturma t_ şeklinde oluştur.
