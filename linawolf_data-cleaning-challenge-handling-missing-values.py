...
rowCount = sf_permits['Permit Number'].count()
print (rowCount)
# your turn! Find out what percent of the sf_permit dataset is missing
missing_values_count_permits = sf_permits.isnull().sum()
missing_values_percent_permits =  round (sf_permits.isnull().sum() / rowCount * 100)

# look at the # of missing points in the first ten columns
print (missing_values_count_permits)
...
# percent of data that is missing
print ((missing_values_percent_permits/total_cells_permits) * 100)
# look at the # of missing points in the first ten columns
missing_values_count[0:10]
...
# remove all columns with at least one missing value
columns_with_na_dropped_permits = sf_permits.dropna(axis=1)
columns_with_na_dropped_permits.head()
