...
def plot_precision_recall_f1_vs_threshold(precisions, recalls, thresholds):
    ...
    plt.figure(figsize=(16, 12))
    plt.title('Precision and Recall and F1 Scores (decision threshold)')
    plt.plot(thresholds, precisions[:-1], 'b-',linewidth=2, label='Precision')
    plt.plot(thresholds, recalls[:-1], 'g', linewidth=2, label='Recall')
    plt.plot(thresholds, f1scores[:-1], 'r', linewidth=2, label='F1')
    plt.axvline(t, color='k', linestyle='--', label='Threshold')
    plt.axhline(pr, color='blue', linewidth=2, linestyle='--')
    plt.axhline(rec, color='green', linewidth=2, linestyle='--')
    plt.ylabel('Score')
    plt.xlabel('Decision Threshold')
    plt.legend(loc='best')
    plt.savefig('5.prec_recc_f1_threshold.png')
    plt.show();
precision_voting, recall_voting, thresholds_voting
plot_precision_recall_f1_vs_threshold(precision_voting, recall_voting, thresholds_voting)
...
level0 = list()
#level0.append(('xgb', xgb.XGBClassifier(n_jobs = -1, n_estimators = 200)))
level0.append(('dt', DecisionTreeClassifier()))
level0.append(('knn', KNeighborsClassifier()))
level0.append(('svm', SVC()))
level0.append(('lt', LogisticRegression(C = 0.1,penalty = 'l2', class_weight = 'balanced')))
level0.append(('rf', RandomForestClassifier(n_estimators = 200, max_features = 3, min_samples_leaf = 1, min_samples_split = 2, n_jobs = -1, random_state = 42)))
...
plot_confusion_matrix(cm, 
                      classes = class_names, 
                      title = 'stack_model Confusion matrix')
plt.savefig('1.stack_model_confusion_matrix.png')
plt.show()
...
fpr_stack_model, tpr_stack_model, t_stack_model = fpr, tpr, t
precision_stack_model, recall_stack_model, thresholds_stack_model = precision, recall, thresholds
precision_stack_model, recall_stack_model, thresholds_stack_model
precision_voting, recall_voting, thresholds_voting
plot_precision_recall_vs_threshold(precision_voting, recall_voting, thresholds_voting)
plot_precision_recall_f1_vs_threshold(precision_stack_model, recall_stack_model, thresholds_stack_model)