...
clf.fit(X_training,y_training.values[:,1])
print(clf.score(X_testing,y_testing.values[:,1]))
...
SurviverOutput = clf.predict(xTestData)
SurviverOutput
newDF = pd.DataFrame()
newDF['PassengerId'] = test_data['PassengerId']
newDF['Survived'] = SurviverOutput
newDF
print(newDF.to_csv('outputWithLogisticRegression.csv',index=False))
