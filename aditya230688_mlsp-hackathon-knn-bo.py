...
def bayesian_optimization(dataset, function, parameters):
   ...
   return BO.max
def knn_optimization(cv_splits):
    def function(n_neighbors,leaf_size):
        return cross_val_score(
               KNeighborsClassifier(
                   n_neighbors=int(max(n_neighbors,0)),                                                               
                   leaf_size=int(max(leaf_size,1)),
                   n_jobs=-1, 
                   weights="uniform",
                   algorithm="auto"),  
                   X=X_train_r, 
    ...
    parameters = {'n_neighbors':(5,50),
                  'leaf_size':(1,10),
                 }
    return function, parameters
def model_KNN(dataset, function, parameters):
    best_solution = bayesian_optimization(dataset, function, parameters)      
    params = best_solution["params"]
    classifier_KNN = KNeighborsClassifier(
                   n_neighbors=int(max(params["n_neighbors"],0)),                                                               
                   leaf_size=int(max(params["leaf_size"],1)),
                   n_jobs=-1,
                   weights="uniform",
                   algorithm="auto")
    return classifier_KNN
dataset = (X_train_r, y_train, X_test_r, y_test)
cv_splits = 10

function, parameters = knn_optimization(cv_splits)
knn_model = model_KNN(dataset, function, parameters)
knn_model.fit(X_train_r,y_train)
y_pred = knn_model.predict(X_test_r)
score = roc_auc_score(y_test,y_pred)
print(score)
from sklearn.externals import joblib
joblib.dump(rf_model, 'MLSP_Hackathon_KNeighborsClassifier_BO.pkl')
