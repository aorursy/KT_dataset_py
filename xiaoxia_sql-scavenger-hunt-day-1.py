...
# Which country have a value of pollutant exactly 0?
pollutant_zero_query = """ SELECT DISTINCT country
                         FROM `bigquery-public-data.openaq.global_air_quality`
                         WHERE value = 0
                        """
distinct_pollutant_zero = open_aq.query_to_pandas_safe(pollutant_zero_query)
print("Number of distinct countries with pollutant 0 is :" + str(distinct_pollutant_zero.size))
distinct_pollutant_zero.head()