...
human_data.shape
human_data.isna().sum()
human_data['DaysLateLast30'].fillna(0, inplace=True)

human_data['DateofTermination'].fillna('till now', inplace=True)

human_data.drop(columns=['LastPerformanceReview_Date', 'Employee_Name', 'EmpID', 'ManagerID'], inplace=True)
...
sns.swarmplot(x="ManagerName", hue="EmpSatisfaction", data=human_data)
...
sns.swarmplot(x=human_data['RecruitmentSource'],hue=human_data['PerformanceScore'])
