...
print("Reading train_V2.csv done")
train.info()
train.head()
...
plt.show()
print("{} players ({:.4f}%) walked 0 meters. This means that they die before even taking a step (more possible).".format(len(data[data['walkDistance'] == 0]), 100*len(data1[data1['walkDistance']==0])/len(train)))
...
print("{} players ({:.4f}%) drived for 0 meters.".format(len(data[data['rideDistance'] == 0]), 100*len(data1[data1['rideDistance']==0])/len(train)))
...
plt.grid()
plt.show()
print("The average person swims for {:.1f}m, 99% of people have swimemd {}m or less".format(train['swimDistance'].mean(), train['swimDistance'].quantile(0.99), train['swimDistance'].max()))
swim = train.copy()
