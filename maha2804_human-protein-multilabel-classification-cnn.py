...
class MultilabelImageClassificationBase(nn.Module):
    ...
    def epoch_end(self, epoch, result):
        print(f"Epoch [{epoch}], train_loss: {result['train_loss']:.4f}, val_loss: {result['val_loss']:.4f}, val_score: {result['val_score']:.4f}")
...
#Manually enter the training time
train_time = '17:56'
...
def predict_dl(dl, model):
    ...
    return [decode_target(x) for x in batch_probs], batch_probs
test_preds, test_probs = predict_dl(test_dl, model)
test_probs[:5]
submission_df = pd.read_csv(TEST_CSV)
submission_df.Label = test_preds
submission_df.tail()
submission_file_name = 'submission_cnn_v1.csv'
...
jovian.log_metrics(val_loss=history[-1]['val_loss'], 
                   ...
                   time=train_time)
jovian.commit(project='zerogans-protein-competition-cnn')
