...
def eval_metrics(outputs, targets, num_classes, cls_score,ignore_index=255,log=False):

    ious=0

    class_ious=defaultdict(int)
...
#             iou=smp.utils.metrics.IoU()(output,target)
...
#                 cls_iou=smp.utils.metrics.IoU()(cls_output,cls_target)

#                 if cls_iou!=0:

#                     count[i]+=1

#                 class_ious[i]+=int(cls_iou)
...
#     class_ious[0]=class_ious[0]/count[0]

#     class_ious[1]=class_ious[1]/count[1]
...
class Mask_Criterion:
    ...
    def __call__(self,y_pr,y_gt):
        ...
        bce_loss=CE_loss(y_pr,y_gt)

        return torch.log(torch.exp(dice_loss)+torch.exp(-1*dice_loss)/2.0)+bce_loss
...
class CCT(BaseModel):
    ...
    def forward(self,x_l=None,target_l=None,x_ul=None,target_ul=None,label_l=None,label_ul=None,curr_iter=None,epoch=None):
        ...
        if p>0.5:
            ...
            rand_index = torch.randperm(x_l.size()[0]).cuda()



            bbx1,bby1,bbx2,bby2=rand_bbox(x_l.shape,lam)

            x_l[:,:,bbx1:bbx2,bby1:bby2]=x_l[rand_index,:,bbx1:bbx2,bby1:bby2]

            target_l[:,:,bbx1:bbx2,bby1:bby2]=target_l[rand_index,:,bbx1:bbx2,bby1:bby2]

            labels_a=label_l

            labels_b=label_l[rand_index]

            lam = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (x_l.size()[-1] * x_l.size()[-2]))

            encoder_out=self.encoder(x_l)



            cls_loss_sup=lam*self.cls_head(encoder_out,labels_a)+(1-lam)*self.cls_head(encoder_out,labels_b)
    ...
    def get_other_params(self):
            ...
            if self.mode == 'semi':
                ...
                return chain(self.encoder.get_module_params(),self.cls_head.parameters(), self.main_decoder.parameters(),
...
#         self.optimizer =RangerLars(model.parameters(),lr=0.001)
        ...
        self.lr_scheduler=Poly(self.optimizer,epochs,iters_per_epoch)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.do_validation and epoch % 3==0:
...
class Trainer(BaseTrainer):
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, target,labels) in enumerate(tbar):
                ...
                iou,class_iou = eval_metrics(output, target, self.num_classes,cls_score,log=True)
                ...
                tbar.set_description('EVAL ({}) | Loss: {:.3f}, Mean IoU: {:.2f} |Class IOU[0]{:.2f}|Class IOU[1|{:.2f}|class acc:{}'.format(epoch,

                                                                                                             total_loss_val.average,

    

                                                                                                             iou,class_iou[0],class_iou[1],cls_metric))



                self._save_checkpoint(epoch, save_best=self.improved)

        print('eval: iou:{}'.format(total_iou/len(self.val_loader)))

        seg_metrics = {'Iou':total_iou/len(self.val_loader)}
    ...
    def _compute_metrics(self, outputs, target_l, target_ul, epoch):
        ...
        self._update_seg_metrics(ious, True)

        seg_metrics_l = self._get_seg_metrics(True)

        self.mIoU_l= seg_metrics_l['Mean_IoU']
...
#             self.mIoU_ul, self.class_iou_ul = seg_metrics_ul.values()
    ...
    def _update_seg_metrics(self, ious, supervised=True):
...
def get_iou(outputs,masks,cls_scores,cls_threshold,mask_threshold,min_size):
    ...
    for i in range(outputs.shape[0]):
        ...
        for cls in range(2):
            ...
            if score>cls_threshold:
                ...
                prob= cv2.threshold(prob, mask_threshold, 1, cv2.THRESH_BINARY)[1]

                num_component, component = cv2.connectedComponents(prob.astype(np.uint8))

                predictions = np.zeros(prob.shape, np.float32)
                ...
                iou=smp.utils.metrics.IoU()(torch.from_numpy(predictions),torch.from_numpy(true_mask))
...
def get_param(cls_threshold,mask_threshold,min_size):
    ...
    model=CCT(num_classes=2)

    checkpoint=torch.load('/kaggle/input/hulianwang-checkpoint/best_model.pth')['state_dict']
    ...
    with tqdm(total=len(test_loader)) as pbar:
        ...
        for images,masks,labels in test_loader:
            ...
            iou=get_iou(output,masks,cls_score,cls_threshold,mask_threshold,min_size)
    ...
    return total_iou/len(test_loader),total_acc/len(test_loader)





cls_thresholds=[0.6,0.65,0.7,0.75,0.8,0.85,0.9]

mask_thresholds=[0.5,0.55,0.65,0.6,0.7,0.75,0.8,0.85,0.9]

min_size=[100,500,1000,2500,5000,7500,10000,15000,20000]
...
for i in cls_thresholds:

    for j in mask_thresholds:

        for k in min_size:

            iou,cls_acc=get_param(i,j,k)

            print("cls_threshold:{},mask_threshold:{}.min_size:{}.iou:{},acc:{}".format(i,j,k,iou,cls_acc))
