...
q_5.solution()
# No, we can't. The higher importance only means that the predictions lose more accuracy when permuting this feature,
# but not that the actual dependent variable has a higher value.

q_6.solution()
## Full euclidean distance experiment
# create new features
import numpy as np
data['abs_lon_change'] = abs(data.dropoff_longitude - data.pickup_longitude)
data['abs_lat_change'] = abs(data.dropoff_latitude - data.pickup_latitude)
data['euclidean_dist'] = np.sqrt(data['abs_lon_change']**2+data['abs_lat_change']**2)
