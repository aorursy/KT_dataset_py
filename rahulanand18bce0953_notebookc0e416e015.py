...
y_test = test_df.toxic.values.reshape(-1,1)
...
scores = predict(val_dist_dataset)

score=[int(i>=0.5) for i in scores]

scores_model.append({'Model': 'Transforer','AUC_Score': roc_auc(scores[:m],y_val),'Accuracy':accuracy_score(y_val,score[:m])})

print("Auc: %.2f%%" % (roc_auc(scores[:m],y_val)))
scores_model
from sklearn.metrics import accuracy_score
...
scores = predict(test_dist_dataset)

score=[int(i>=0.5) for i in scores]

scores_model.append({'Model': 'Transforer','AUC_Score': roc_auc(scores[:m],y_test),'Accuracy':accuracy_score(y_test,score[:m])})

print("Auc: %.2f%%" % (roc_auc(scores[:m],y_test)))
scores_model