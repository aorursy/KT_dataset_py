...
#The len of the balanced data frame
...
data = data.sample(full, weights=sample_weights)
...
data.info()
data['classes'] = data.apply(lambda row : row['Finding Labels'].split('|'), axis = 1)
data.sample(4, random_state=5)
data.to_pickle('data-chest-x-ray-multilabel-14-all-balanced.plk')