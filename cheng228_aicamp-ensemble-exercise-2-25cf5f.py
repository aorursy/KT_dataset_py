...
data.head()
data[0] = data[0].apply(lambda x: 0 if x == 'e' else 1) # TODO
...
forest = RandomForestClassifier() #TODO
...
rfregressor = RandomForestRegressor() # TODO
...
print("linear regression performance: %f" % (cross_val_score(lr, trainX, trainY, cv=8)).mean()) #TODO
print("random forest regressor performance: %f" % (cross_val_score(rfregressor, trainX, trainY, cv=8)).mean()) #TODO)
