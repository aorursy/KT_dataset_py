...
temp = pd.read_csv(f'../input/lanl-vettejeep-split-2/raw_data_4.csv', dtype={'acoustic_data': np.int16, 'time_to_failure': np.float32})
...
start_indices = np.loadtxt(fname=os.path.join(INDEX_DIR, 'start_indices_4k.csv'), dtype=np.int32, delimiter=',')
...
for i in range(6):
    ...
    print(start_indices[:,i].max(), start_indices[:,i].min())
...
# print(start_indices.max(), start_indices.min())
...
# for seg_id, start_idx_seed in zip(range(seg_st, seg_st + NUM_SEG_PER_PROC), start_indices):

#     start_idx = (len_df * proc_id) + start_idx_seed
...
def run_mp_build():
    ...
    results = [pool.apply_async(build_fields, args=(pid, )) for pid in range(NUM_THREADS_FOR_SPLIT,6)]
