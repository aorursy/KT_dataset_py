...
len(df_train)
X_train = df_train[pd.to_datetime(df_train.issue_d) >= dt.datetime(2015,1,1)].copy()
...
X_test = df_test
X_train.to_csv('/kaggle/working/X_train.2015.csv',index=False)

#X_test.to_csv('/kaggle/working/X_test.2016.csv',index=False)
X_test.info()
## 総債務支払い額
...
lgb_clf = LGBMClassifier(boosting_type='gbdt', class_weight=None,
               ...
               subsample_freq=0)
pred = nn_model.predict(dnn_test)
history = nn_model.fit(dnn_train, y_train,
...
pred[2]
x=np.zeros((2,8,len(X_test)))

x[0][0] = pred_test_cb.reshape(1,-1)

x[0][1] = pred_test_cb.reshape(1,-1)

x[0][2] = pred_test_cb.reshape(1,-1)
len(X_test)
pred_test_cb
# RGF
...
clf = LGBMClassifier(boosting_type='gbdt', class_weight=None,
               ...
               n_estimators=999, n_jobs=-1, num_leaves=73, objective=None,
...
for i,(idx_train, idx_validate) in tqdm(enumerate(kf.split(X_train,y_train))):
    ...
    clf.fit(cv_X_train, cv_y_train, early_stopping_rounds=20, eval_metric='auc', eval_set=[(cv_X_validate, cv_y_validate)])
...
#clf.fit(X_train,y_train)
#pred_test = (0.4*pred_test_gbdt + 0.2*pred_test_rgf.reshape(-1,) + 0.4*(pred_test_dnn.reshape(-1,)))
