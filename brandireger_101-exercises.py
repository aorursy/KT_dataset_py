...
def has_vowels(arg):
    ...
    if any(char in vowel for char in arg):
...
def remove_vowels(arg):
    ...
    newarg = [l for l in arg if l not in vowel]
...
def starts_and_ends_with_vowel(arg):
    ...
    if arg[0] in vowel and arg[-1] in vowel:
...
def only_odd_numbers(arg):
    ...
    temp = [num for num in arg if num % 2 == 1]
...
def only_even_numbers(arg):
    ...
    temp = [num for num in arg if num % 2 == 0]
...
def only_positive_numbers(arg):
    ...
    temp = [num for num in arg if num > 0]
...
def only_positive_evens(arg):
    ...
    temp = [num for num in arg if num > 0 and num % 2 == 0]
...
def get_unique_values_from_two_lists(arg1, arg2):
...
def get_values_in_common(arg1, arg2):

    temp1 = set(arg1)

    temp2 = set(arg2)

    return(temp1.intersection(temp2))
...
def get_values_not_in_common(arg1, arg2):

    return set(arg1).symmetric_difference(set(arg2))
...
def get_book_author(arg):

    return arg.get('author')
...
def highest_price_book(books):
    ...
    srted = sorted(books, key=lambda k: (k['price']))
...
def most_spent_on_item(input):
    ...
    srted = sorted(input['items'], key=lambda k: (k['quantity']))
