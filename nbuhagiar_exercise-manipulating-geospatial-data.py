...
def geocoder(x):
    ...
    point = geocode(x, provider="nominatim").geometry.iloc[0]        
...
for _, location in berkeley_locations.iterrows():

    Marker([location["Latitude"], location["Longitude"]]).add_to(m_2)
...
for _, location in gpd.sjoin(starbucks_gdf, sel_counties).iterrows():

    Marker([location["Latitude"], location["Longitude"]]).add_to(m_6)
