...
meta_features = ['hashtag_count', 'mention_count']

fig, axes = plt.subplots(ncols=2, nrows=len(meta_features), figsize=(20,10), dpi=100)



for i, feature in enumerate(meta_features):

    sns.distplot(df_train.loc[~disaster_tweets][feature], label='Not Disaster', ax=axes[i][0], color='green', kde=False)

    sns.distplot(df_train.loc[disaster_tweets][feature], label='Disaster', ax=axes[i][0], color='red', kde=False)

    sns.distplot(df_train[feature], label='Training', ax=axes[i][1], kde=False)

    sns.distplot(df_test[feature], label='Test', ax=axes[i][1], kde=False)
...
# df_train['text_cleaned'] = df_train['text'].apply(lambda s : correct_spellings(s))

# df_test['text_cleaned'] = df_test['text'].apply(lambda s : correct_spellings(s))
