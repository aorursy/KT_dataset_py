...
data=pd.read_csv('../input/2020mcm-c-2/2b_data_dryer.csv',sep=',')
...
for i in range(len_small_data):
    ...
    # 摘取完成    
small_data[0].resample('QS').mean().to_period('M')
...
def get_season_data(year,index):
    ...
    season_data=small_data[index][str(year)+'-10'].append(small_data[index][str(year)+'-11']).append(small_data[index][str(year)+'-12']).append(small_data[index][str(year)+'-7']).append(small_data[index][str(year)+'-8']).append(small_data[index][str(year)+'-9'])

    return season_data
get_season_data(2013,0).to_csv('./top_1_2013.csv')

get_season_data(2013,1).to_csv('./top_2_2013.csv')

get_season_data(2013,2).to_csv('./top_3_2013.csv')