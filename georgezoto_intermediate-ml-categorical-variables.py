...
#step_1.solution()
print('X_train.columns # {} \ndrop_X_train.columns # {}'.format(len(X_train.columns), len(drop_X_train.columns)))
...
good_label_cols = [col for col in object_cols if 
                   ...
                   (set(X_train[col]) == set(X_valid[col])) and 
...
sum([i[1]>10 for i in d.items()])
# Fill in the line below: How many categorical variables in the training data

# have cardinality greater than 10?

high_cardinality_numcols = sum([i[1]>10 for i in d.items()])
...
imputed_X_test = pd.DataFrame(my_imputer.fit_transform(num_X_test))



# Fill in the lines below: imputation removed column names; put them back

imputed_X_test.columns = X_test.drop(object_cols, axis=1).columns
...
X_test
X_test.shape
extra_columns = list(set(X_test.columns) - set(label_X_train.columns))

extra_columns
set(label_X_train.columns) - set(X_test.columns)
