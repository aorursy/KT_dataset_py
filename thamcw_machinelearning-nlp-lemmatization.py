...
#It makes use of WordNetLemmatizer module from the Natural Language Toolkit (nltk) in Python to

#extract root-form of constituent words in the speech and to check for correctness of each word.
...
#ntlk.download()
#import the WordNetLemmatizer module from nltk that is used to extract root-words from a text and also check for its correctness.
...
lemmatizer.lemmatize('worked')
#The root-word is extracted correctly.

lemmatizer.lemmatize('intelligently')
...
#Then lemmatize each word in the sentence and re-assign these lemmatized words back into the sentence
...
#The results show the tokenized sentences with lemmatized words.

#Notice how, compared to original passage, some of the words have become root-words e.g. "years" becomes "year".

#Notice also how some words could not be stemmed properly e.g. "as" is wrongly determined as "a".
