...
data = d_real.drop(['id', 'news_url', 'tweet_ids'], axis=1).rename(columns={'title': 'headline'})
import os
...
def read_data(d):
    ...
    for fname in files:
        ...
        if len(text) == 2:
            ...
            # Only one or zero lines in file
...
data = pd.concat([data, real_headlines])
...
def format_data(data, max_features, maxlen, n=7, shuffle=False):
...
X, Y, tokenizer = format_data(data.copy(), max_features, max_len, shuffle=True)
