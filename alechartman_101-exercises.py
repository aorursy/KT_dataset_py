...
def has_vowels(x):
    ...
    return ("a" in x) or ("e" in x) or ("i" in x) or ("o" in x) or ("u" in x) or ("A" in x) or ("E" in x) or ("I" in x) or ("O" in x) or ("U" in x)
...
def first_to_last(newList):

    newList.append(newList.pop(newList.index(newList[0])))    
...
def only_odd_numbers(x):
    ...
    new_list = [num for num in x if num % 2 == 1]
...
def only_positive_numbers(x):
    ...
    new_list = [num for num in x if num > 0]
...
def only_positive_evens(x):
    ...
    return [num for num in x if (num > 0) and (num % 2 == 0)]
...
def only_positive_odds(x):
    ...
    return [num for num in x if (num > 0) and (num % 2 == 1)]
...
def get_average_book_price(x):
    ...
    return total_price/total_books
...
def get_average_item_price(shopping_cart):
    ...
    unique_items = len(shopping_cart["items"])

    for item in shopping_cart["items"]:

        total_price += item["price"]

    return total_price/unique_items
...
def get_average_spent_per_item(shopping_cart):
    ...
    return total_item_cost/total_items
...
def most_spent_on_item(shopping_cart):
    ...
    for item in shopping_cart["items"]:
        ...
        if (item["price"] * item ["quantity"]) > (dic["price"] * dic["quantity"]):
