...
dataframe_raw.head()
your_name = 'abcde' # at least 5 characters
...
print(num_rows)
num_cols = len(dataframe.columns)   # or dataframe.shape[1]
print(num_cols)
input_cols = dataframe.columns[:-1]
input_cols
categorical_cols = list(dataframe.select_dtypes(exclude=[int, float]).columns)
categorical_cols
...
average = dataframe['charges'].sum() / len(dataframe['charges'])
print('Min:',minimum)
print('Max:',maximum)
print('Avg:',average)
...
plt.plot([value['val_loss'] for value in history])
val_loss = 4844.4170
