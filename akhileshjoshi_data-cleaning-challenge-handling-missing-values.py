...
# percent of data that is missing
(total_missing_sf_permits/total_cell_sf_permits) * 100
# look at the # of missing points in the first ten columns
...
columns_with_na_dropped_sf_permits = sf_permits.dropna()
print("Columns in original dataset: %d \n" % sf_permits.shape[1])
print("Columns with na's dropped: %d" % columns_with_na_dropped_sf_permits.shape[1])
...
first = sf_permits.columns[0]
last = sf_permits.columns[-1]

updated_sf_permits= sf_permits.fillna(method = 'bfill', axis=0).fillna("0")
