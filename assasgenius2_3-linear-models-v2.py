...
def rmse_cv(model):
    ...
    return(rmse)
n_estimators = [150, 170 , 200, 400, 500, 600]

cv_rmse_gb = [rmse_cv(GradientBoostingRegressor(n_estimators = n_estimator, max_depth=2)).mean() 
...
plt.ylabel("rmse")
cv_gb.min()
model_gb = GradientBoostingRegressor(n_estimators = 500, max_depth=2).fit(X_train, y)
...
cv_rmse_ridge = [rmse_cv(Ridge(alpha = alpha, normalize = True)).mean() 
...
plt.ylabel("rmse")
cv_ridge.min()
model_ridge = Ridge(alpha = 0.3, normalize = True).fit(X_train, y)
#let's look at the residuals as well:
...
preds = 0.4*lasso_preds + 0.3*gb_preds + 0.3*ridge_preds
solution = pd.DataFrame({"id":test.Id, "SalePrice":preds})

solution.to_csv("gb_lasso_ridge.csv", index = False)