...
for i in range(5):
    ...
    for j in range(2):
        ...
        count += 1
train = train.drop('date', axis=1) # date는 month 와 year로 대체 하였으므로 삭제

train = train.drop('item_category_id', axis=1) # 카테고리 정보를 지워버림........

train = train.groupby(['shop_id', 'item_id', 'date_block_num', 'month', 'year']).sum()

train = train.sort_index()
train # 그룹핑되어 있는 데이터 프레임
train.iloc[0] # value는 item_price, item_cnt_day 두개이고, 다른 정보(날짜, 아이템id, 상점id)는 Name으로 저장됨
...
train.head() # 정규회된 item_price, item_cnt_day
train.reset_index().groupby(['item_id', 'date_block_num', 'shop_id']).mean() # 평균으로 체우면 이럴것이다..
...
assert closest_date_block(18, 30, 5) == 18 # 잘 동작하는지 체크
...
BLOCKS = [6, 18, 30] # 7,8,9,10월만 사용하기 위해 (6: 2013년 7월) (18: 2014년 7월)
