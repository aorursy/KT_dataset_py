...
TP= np.sum(np.logical_and(y_pred_DTree==1, y_test==1))

TN= np.sum(np.logical_and(y_pred_DTree==0, y_test==0))

FP= np.sum(np.logical_and(y_pred_DTree==1, y_test==0))

FN= np.sum(np.logical_and(y_pred_DTree==0, y_test==1))
...
print('Accuracy score is ', accuracy_score(y_test,y_pred_DTree).round(2))



#precision score

print('Precision score is ', precision_score(y_test, y_pred_DTree).round(2))



# Recall Score

print('Recall_score is ', recall_score(y_test, y_pred_DTree).round(2))



# F1 Score

print('F1 score is ', f1_score(y_test, y_pred_DTree).round(2))



# ROC_AUC

print('ROC AUC is ', roc_auc_score(y_test, y_pred_DTree).round(2))
...
area_under_curve = roc_auc_score(y_test, y_pred_DTree)
...
area_under_curve = roc_auc_score(y_test, y_pred_RF)
...
GB= GradientBoostingClassifier(learning_rate=0.05, max_depth=6,n_estimators=200, max_features = 0.3,min_samples_leaf = 5)

GB_fit= GB.fit(X_train, y_train)

y_pred_GB= GB_fit.predict(X_test)

TP = np.sum(np.logical_and(y_pred_GB == 1, y_test == 1))

TN = np.sum(np.logical_and(y_pred_GB == 0, y_test == 0))

FP = np.sum(np.logical_and(y_pred_GB == 1, y_test == 0))

FN = np.sum(np.logical_and(y_pred_GB == 0, y_test == 1))
...
print('Accuracy score is ', accuracy_score(y_test, y_pred_GB).round(2))



# Precision Score

print('Precision score is ', precision_score(y_test, y_pred_GB).round(2))



# Recall Score

print('Recall_score is ', recall_score(y_test, y_pred_GB).round(2))



# F1 Score

print('F1 score is ', f1_score(y_test, y_pred_GB).round(2))



# ROC_AUC

print('ROC AUC is ', roc_auc_score(y_test, y_pred_GB).round(2))
...
area_under_curve = roc_auc_score(y_test, y_pred_GB)

fpr, tpr, thresholds = roc_curve(y_test, GB_fit.predict_proba(X_test)[:,1])
