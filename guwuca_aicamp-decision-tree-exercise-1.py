...
# 打印深度为3的决策树模型的表现
print("score for depth=3:", model_depth_3.score(X, Y))
plt.scatter(X[:,0], X[:,1], s=100, c=Y, alpha=0.5)
#TODO 
#调用plot_decision_boundary 函数画出决策边界
plot_decision_boundary(X,model_depth_3)
...
# 打印深度为5的决策树模型的表现
#TODO
print("score for depth=5", model_depth_5.score(X,Y))
plt.scatter(X[:,0], X[:,1], s=100, c=Y, alpha=0.5)
#TODO 
#调用plot_decision_boundary 函数画出决策边界
plot_decision_boundary(X, model_depth_5)
...
print('depth=3 tree score=', model_depth_3.score(X,Y))
plt.scatter(X[:,0], X[:,1],s=100,c=Y, alpha=0.5)
plot_decision_boundary(X, model_depth_3)
plt.show()


print('depth=5 tree score=', model_depth_5.score(X,Y))
plt.scatter(X[:,0], X[:,1],s=100,c=Y, alpha=0.5)
plot_decision_boundary(X, model_depth_5)
plt.show()
...
theta =  np.random.randn(N//2)*2*np.pi   # 随机生成夹角
...
print('basic logistic regression score=', model.score(X,Y))
plot_decision_boundary(X, model)
plt.show()
