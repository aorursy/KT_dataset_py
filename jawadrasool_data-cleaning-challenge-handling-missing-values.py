...
sf_permits_imputation = sf_permits.fillna(method = 'bfill', axis=0).fillna(0)
# your turn! Find out what percent of the sf_permits dataset is missing
# get the number of missing data points per column
missing_values_count_sf_imputation = sf_permits_imputation.isnull().sum()

# how many total missing values do we have?
total_cells_sf = np.product(sf_permits.shape)
total_missing_sf_imputation = missing_values_count_sf_imputation.sum()

# percent of data that is missing
(total_missing_sf_imputation/total_cells_sf) * 100