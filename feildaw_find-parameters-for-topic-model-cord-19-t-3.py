...
query = "What do we know about the virus origin and management measures at the human-animal interface?"
...
corpus_similarities_print3(query_topicvec, index, documents_dict)
query = "Evidence that livestock could be infected (e.g., field surveillance, genetic sequencing, receptor binding) and serve as a reservoir after the epidemic appears to be over. Evidence of whether farmers are infected, and whether farmers could have played a role in the origin."

query_topicvec = topic_distribution(query, id2word_dict, lda_model)

corpus_similarities_print3(query_topicvec, index, documents_dict)
query = "Evidence that livestock could be infected (e.g., field surveillance, genetic sequencing, receptor binding) and serve as a reservoir after the epidemic appears to be over. Surveillance of mixed wildlife- livestock farms for SARS-CoV-2 and other coronaviruses in Southeast Asia."

query_topicvec = topic_distribution(query, id2word_dict, lda_model)

corpus_similarities_print3(query_topicvec, index, documents_dict)
query = "Evidence that livestock could be infected (e.g., field surveillance, genetic sequencing, receptor binding) and serve as a reservoir after the epidemic appears to be over. Experimental infections to test host range for this pathogen."
