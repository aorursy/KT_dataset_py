...
class C_MultiLayerPerceptron:
    ...
    def M_fgsm(self, X, y):
        ...
        dZ1 = np.multiply(dA1, F_dRelu(self.Z1))
...
plt.grid(True)
def plot_decision_function_2d(function_pred, X, Y):
   ...
   # create a mesh to plot in
    ...
    Z = function_pred( np.transpose(np.c_[xx.ravel(), yy.ravel()]))
...
x_min, x_max = XX[:, 0].min() - offset, XX[:, 0].max() + offset

y_min, y_max = XX[:, 1].min() - offset, XX[:, 1].max() + offset
...
plt.scatter(XX[0,0], XX[0,1], marker=11,c="r", s=100)
...
def adversarials(point,myMLP):
    ...
    for eps in np.arange(0,0.5,0.05):
...
def adversarials_analyse2(points,myMLP):

    

    advs=np.zeros((points.shape[0],10,2))

    accuracies=np.zeros((10,1))

    diffs=np.zeros((points.shape[0],10))

    #diffmeans=np.zeros((10,1))
    ...
    for i,point in enumerate(points) : 
        ...
        perturbations=myMLP.M_fgsm(point.reshape(2,1),np.ones((1,1)))

        perturbations=perturbations.reshape(1,2)

        

        for j,eps in enumerate(np.arange(0,0.3,0.03)):



            #advs.append(points-perturbations*eps)

            advs[i,j,:]=point-perturbations*eps

            diffs[i,j]= np.linalg.norm(point-advs[i,j,:])

    

    for i in range(10):

        y_hat=myMLP.M_forwardPropagation(advs[:,i,:].T)

        accuracies[i]=F_computeAccuracy(y_hat.reshape(1,100),np.zeros((1,100)))

    diffmeans=np.mean(diffs,axis=0)

    print(diffmeans.shape) 

        

    return advs,accuracies,diffmeans
advs1,accuracies1, l21=adversarials_analyse2(X0[:100],myMLP)
...
plt.plot(np.arange(0,0.2,0.02),accuracies1)

plt.title("Accuracy vs epsilon")

plt.subplot(1,2,2)

plt.plot(l21,accuracies1)
...
#for i in range(5):
    ...
    #plot_advs(X0[i],myMLP)

#plt.scatter(point[0], point[1], marker=11,c="w", s=100, zorder=10)

#plt.scatter(advs[:,0], advs[:,1], marker=11,c="r", s=100, zorder=5)
class C_MultiLayerPerceptron2:
    ...
    def __init__(self, n_in, n_h, n_out,n):
...
myMLP2 = C_MultiLayerPerceptron2(n_in, n_hidden, n_out,2)
...
for i in range(0, num_epoch):
    ...
    hat_y_train = myMLP2.M_forwardPropagation(X_train)
    ...
    myMLP2.M_backwardPropagation(X_train, y_train)

    

    # --- Update

    myMLP2.M_gradientDescent(alpha=0.1)
    ...
    hat_y_test = myMLP2.M_forwardPropagation(X_test)
...
Z = myMLP2.M_forwardPropagation( np.transpose(np.c_[xx.ravel(), yy.ravel()]))
...
def adversarials_analyse(points,myMLP):

    

    yhat=myMLP.M_forwardPropagation(points.reshape(2,points.shape[0]))

    perturbations=myMLP.M_fgsm(points.reshape(2,points.shape[0]),np.ones((points.shape[0])).reshape(1,points.shape[0]))
    ...
    perturbations=perturbations.reshape(points.shape[0],2)
    ...
    for i,eps in enumerate(np.arange(0,0.8,0.08)):

        

        #advs.append(points-perturbations*eps)

        advs[i,:,:]=points-perturbations*eps

        

    return advs
advs2,accuracies, l2=adversarials_analyse2(X0[:100],myMLP2)
...
plt.plot(np.arange(0,0.2,0.02),accuracies)

plt.title("Accuracy vs epsilon")

plt.subplot(1,2,2)

plt.plot(l2,accuracies)
...
plt.plot(l21,accuracies1, label="model relu")

plt.plot(l2,accuracies, label="model relu^2")
