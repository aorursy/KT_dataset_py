...
class BaggedTreeClassifier:
    ...
    def fit(self, X, Y):
        # TODO
        N = len(X)#how many features
        self.models = list()#bag for all the decision trees
        for m in range(self.M):#loop for each decision tree
            idx = np.random.choice(N, size=N, replace=True)
            Xc=X[idx]
            Yc=Y[idx]
            model = DecisionTreeClassifier(max_depth=5)#create a new decision tree
            model.fit(Xc,Yc)
    ...
    def predict(self,X):
        ...
        for model in self.models:
            predictions +=model.predict(X)#add predicted value of each model
