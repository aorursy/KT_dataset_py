...
def classify_image_ssim(example_dict, in_image):

    score_dict = {c_digit: ssim(in_image, c_img) for c_digit, c_img in example_dict.items()}

    best_digit, best_score = sorted(score_dict.items(), reverse=True,
...
guess_digit, guess_dict = classify_image_ssim(digit_examples, numb_image[rand_digit])
...
ax_score.set_ylabel('SSIM')

ax_score.set_title('SSIM for each digit')
test_vec = np.loadtxt(test_data_path, delimiter = ',', skiprows = 1)
...
guess_test_data = [classify_image(digit_examples, c_img)[0] for c_img in test_image]
with open('submission2.csv', 'w') as out_file:
