...
train_feat[:, [1, 6]] = None  # name和ticket置为None，与是否生还不相关，降低拟合复杂度
test_feat[:, [1, 6]] = None
...
class my_gbdt(object):
    def __init__(self):
        # TO DO
        self.len = 10
        self.trees = [DecisionTreeRegressor(min_samples_split=2, max_depth=6) for i in range(self.len)]
    ...
    def fit(self, X, y):
        ...
        for i in range(1, self.len):
            tree_before = self.trees[i-1]
            tree_after = self.trees[i]
            pred_prob = tree_before.predict(X)
            y_copy = y_copy - pred_prob
            tree_after.fit(X, y_copy)
    
    def predict(self, X):
        # TO DO
        pred_proba = [self.trees[i].predict(X) for i in range(self.len)]
        pred_proba = np.sum(np.array(pred_proba), 0)
