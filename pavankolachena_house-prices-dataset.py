...
#Head of the dataset for train
train.head()
#Head of the dataset for test
test.head()
#shape of the dataset for train
train.shape
#shape of the dataset for test
test.shape
...
encoding = pd.get_dummies(all_features).reset_index(drop=True)
encoding.shape
encoding.head()
encoding.shape
encoding = encoding.loc[:,~encoding.columns.duplicated()]
X = encoding.iloc[:len(train_labels), :]
X_test = encoding.iloc[len(train_labels):, :]
