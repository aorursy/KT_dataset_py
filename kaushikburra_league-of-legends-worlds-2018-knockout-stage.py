...
%matplotlib inline
#importing data#
df= pd.read_csv('../input/LOL Worlds 2018 Knockout stage - Player Ratings.csv')
df.head()
df.columns
...
df['Deaths(Total)'] = df['Deaths(Total)'].astype(float)
df['Assists(Total)'] = df['Assists(Total)'].astype(float)
...
print('EDG iBoy has a KDA of 7 with 4 games played is considered exemplary than the highest KDA of 9.2 of IG Rookie')
...
plt.show()
#Correlation matrix
corr = df[['KDA Ratio','Kills(Total)','Deaths(Total)','Assists(Total)','Games Played','CS(Total)']]
