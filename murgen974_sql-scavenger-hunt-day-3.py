...
#Hunt 1 hours of the day with the highest accidents
...
#HUNT 2 state with the most hit and run
accidents.head("accident_2016")
accidents.head("vehicle_2016")

query_state = """ SELECT registration_state_name, 
                    COUNT(consecutive_number)
                FROM `bigquery-public-data.nhtsa_traffic_fatalities.vehicle_2015`
                GROUP BY registration_state_name
                ORDER BY COUNT(consecutive_number) DESC
"""
accidents_by_state = accidents.query_to_pandas_safe(query_state)
