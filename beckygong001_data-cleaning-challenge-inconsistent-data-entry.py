...
matches2 = fuzzywuzzy.process.extract("kurram agency", cities, limit=10, scorer=fuzzywuzzy.fuzz.token_sort_ratio)
...
def replace_matches_in_column(df, column, string_to_match, min_ratio = 70):
    ...
    # get the top 10 closest matches to our input string
    matches2 = fuzzywuzzy.process.extract(string_to_match, strings, 
                                         limit=10, scorer=fuzzywuzzy.fuzz.token_sort_ratio)

    # only get matches with a ratio > 90
    close_matches2 = [matches2[0] for matches2 in matches2 if matches2[1] >= min_ratio]

    # get the rows of all the close matches in our dataframe
    rows_with_matches2 = df[column].isin(close_matches2)

    # replace all rows with close matches with the input matches 
    df.loc[rows_with_matches2, column] = string_to_match
