...
df.pivot_table(values=['alco'],index=['gender']) #2
df.pivot_table(values=['smoke'],index=['gender']) #3
df.pivot_table(values=['age'],index=['smoke']) #4
df['BMI'] = df['weight'] / (df['height_cm'] * df['height_cm'])

df['BMI'].mean() #5a
df.pivot_table(values=['BMI'], index=[ 'gender']) #5b
df.pivot_table(values=['BMI'], index=[ 'cardio']) #5c
df.pivot_table(values=['BMI'], index=['cardio', 'gender', 'alco'], aggfunc='mean') #5d
ap_df = df.drop(df[(df['ap_hi'] > df['ap_lo'])].index) #6

ap_df.shape[0] / df.shape[0]
df.groupby('cholesterol')['age'].mean().plot(kind='bar') #8

plt.ylabel('Age') 

plt.show()
df['BMI'].hist(bins=100) #9.1
sns.boxplot(df['BMI']) #9.2
df.groupby('cardio')['BMI'].mean().plot(kind='bar') 

plt.ylabel('BMI') # добавляем подпись 
