...
def evaluate_PE(user_product_graph, product_user_graph, target_products, elite_threshold, elite_accounts, print_=False):
    """
    This function will be used to calculate the Practical Effect of spamming after the black-box detectors filter some of the spams.
    ...
    :param target_products: those products whose revenues will be targetd.

    :param elite_threshold: the least number of reviews that an account must have to attain the elite status. Used for find elite users in ANY time.

    :param spammer_accounts: a dictionary of user_ids and their elite status (1:elite, 0:non-elite) at the spammer's disposal.
            The elite status is pre-defined and fixed.
...
def evaluation(user_product_graph,
    ...
    # step 3: run the detector on the new data
    detected_spams = detector(user_product_graph_copy, prod_user_graph_copy, product_range, spammer_account_range, top_k)
    # remember to filter out the false positives.
    attacking_spams_lookup = set([(s[0], s[1]) for s in attacking_spams])
    detected_spams_lookup = set([ds[0] for ds in detected_spams])
    remaining_spams = list(attacking_spams_lookup.intersection(detected_spams_lookup))
    
    print(f'Precision: {float(len(remaining_spams))/len(detected_spams_lookup)}')
    print(f'Recall: {float(len(remaining_spams))/len(attacking_spams)}')

    # step 4: remove the detected spams from the graph
    detected_review_ids = [s[0] for s in detected_spams]
    ...
    print(f'Practical Effect = {sum([newRevenue[k] for k in newRevenue.keys()]) - old_revenue}')
import pandas as pd

if __name__ == '__main__':
    data_path = '../input/'
    user_product_graph, prod_user_graph  = read_graph_data(data_path + 'metadata')

    df = pd.read_csv(data_path + 'attack_parameters.csv')
    
    for case in range(df.shape[0]):
    
        all_products = [k for k in prod_user_graph.keys()]
        elite_users = [k for k in user_product_graph.keys() if len(user_product_graph[k]) >= df['elite_threshold'][case]]
        normal_users = [k for k in user_product_graph.keys() if len(user_product_graph[k]) < df['elite_threshold'][case]]

        random.seed(1)
        product_range = random.sample(all_products, k=df['num_potential_targets'][case])
        target_products = random.sample(product_range, k=df['num_targets'][case])

        spammer_accounts = {}
        elite_spammer_account_range = random.sample(elite_users, k=df['num_potential_elite_spammers'][case])
        for s in random.sample(elite_spammer_account_range, k=df['num_elite_spammers'][case]):
            spammer_accounts[s] = 1

        normal_spammer_account_range = random.sample(normal_users, k=df['num_potential_normal_spammers'][case])
        for s in random.sample(normal_spammer_account_range, k=df['num_normal_spammers'][case]):
            spammer_accounts[s] = 0

        spammer_range = elite_spammer_account_range + normal_spammer_account_range
        ...
        attack_period = [datetime.strptime(df['start_attack_date'][case], '%Y-%m-%d'), datetime.strptime(df['end_attack_date'][case], '%Y-%m-%d')]

        # generate spams
        size = df['num_spams_per_target'][case]
        num_spams = {}
        for target in target_products:
            num_spams[target] = size

        attacking_spams = attack(user_product_graph, num_spams, spammer_accounts, attack_period)
