import numpy as np

import pandas as pd
train_data = pd.read_csv('/kaggle/input/titanic/train.csv')

test_data = pd.read_csv('/kaggle/input/titanic/test.csv')
train_data[:10]
train_data.describe(include='all')
women = train_data[train_data['Sex'] == 'female']['Survived']

rate_women = sum(women)/len(women)

print('% of women who survived:', rate_women)
men = train_data[train_data.Sex == 'male']['Survived']

rate_men = sum(men)/len(men)

print('% of men who survived:', rate_men)
# alternative way of computing the above

train_data[['Sex', 'Survived']].groupby(['Sex']).mean()
train_data[['Pclass', 'Survived']].groupby(['Pclass']).mean()
# generate correlation data (larger values signify a clear positive/negative correlation between row/column labels)

train_data.corr()
women_count = 0

women_survived_count = 0



for idx, row in train_data.iterrows():

    if row['Sex'] == 'female':

        women_count += 1

        if row['Survived'] == 1:

            women_survived_count += 1

            

print(women_survived_count)

print(women_count)

women_survived_count / women_count
men_count = 0

men_survived_count = 0



for idx, row in train_data.iterrows():

    if row['Sex'] == 'male' and row['Age'] <= 1 and row['Pclass'] == 3:

        men_count += 1

        if row['Survived'] == 1:

            men_survived_count += 1

            

print(men_survived_count)

print(men_count)

men_survived_count / men_count      
count = 0

survived_count = 0



for idx, row in train_data.iterrows():

    if row['Parch'] >= 1 and row['Sex'] == 'male':

        count += 1

        if row['Survived'] == 1:

            survived_count += 1

            

print(survived_count)

print(count)

survived_count / count
# this prediction model yielded 78.229% success

predictions = []

count = 0



for idx, row in test_data.iterrows():

    # make your changes in this cell!

    if row['Sex'] == 'female':

        if row['Pclass'] == 1 or row['Pclass'] == 2:

            predictions.append(1)

        elif row['Pclass'] == 3 and row['Age'] <= 1:

            predictions.append(1)

        else:

            predictions.append(0)

    elif row['Sex'] == 'male':

        if row['Age'] <= 18 and row['Pclass'] == 1:

            predictions.append(1)

        elif row['Age'] <= 16 and row['Pclass'] == 2:

            predictions.append(1)

        else:

            predictions.append(0)



# check data length

print(len(predictions))

print(len(test_data))
assert len(predictions) == len(test_data), 'Number of predictions must match number of test data rows!'
test_data['Survived'] = predictions
test_data[['PassengerId', 'Survived']].to_csv('submission.csv', index=False)