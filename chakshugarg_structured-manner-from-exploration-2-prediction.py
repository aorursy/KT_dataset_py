...
def PlotDataCorrelation(data):
    print(data.corr().abs().unstack().sort_values()['price'])
...
print("========Loading Data========")
FileLocation="../input/kc_house_data.csv"
FileType="csv"
data=readData(FileLocation,FileType)
print("========Cleaning Data========")
columnsToBeDropped=['id', 'date']
data=DataCleaning(data, columnsToBeDropped)
print("========Scatter Plot========")
ColumnsToPlot=['sqft_lot','sqft_above','price','sqft_living','bedrooms']
hueColumn='bedrooms'
scatterPlot(data,ColumnsToPlot,hueColumn)
print("========Data Correlation========")
PlotDataCorrelation(data)
...
columnsToBeDropped=['sqft_basement','sqft_lot15','sqft_lot','yr_renovated','sqft_above']
data = data.drop(columnsToBeDropped,axis=1)
TrainLinearRegressionModel(data,y)