...
test_new.head()
X=train_new.drop(["PhoneId","Rating"], axis=1).values
...
class MP_Neuron:
  ...
  def fit(self,X,Y):
    ...
    print("Higest accuracy is ", accuracy[best_b])
...
test_rating=mp_neuron.predict(X_test)

submission = pd.DataFrame({'PhoneId':test_new['PhoneId'], 'Class':test_rating.astype(int)})
