...
class FakeRandomForest(BaseEstimator):
    ...
    def fit(self, X, Y, n_features=None):
        ...
        for m in range(self.M):
            ...
            #TODO 有放回的随机抽取N个数据
            id = np.random.choice(N, size=N, replace=True)
            X_current = X[id]
            Y_current = Y[id]
...
# TODO
plt.scatter(testY, predictions, color='purple')
plt.xlabel("target")
...
print("linear regression performance: %f" % (cross_val_score(lr, trainX, trainY, cv=3).mean()))
print("random forest regressor performance: %f" % (cross_val_score(rfregressor, trainX, trainY, cv=3).mean()))
