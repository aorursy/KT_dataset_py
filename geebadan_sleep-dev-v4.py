...
def cross_validate_random_forest(X, y, Xt, yt, criterion, max_height, n_folds):
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        ytrain, ytest = y.iloc[train_index].values.ravel(), y.iloc[test_index].values.ravel()
        ...
        Xtrain_min, Xtrain_max = Xtrain.min(), Xtrain.max()

        Xtrain = (Xtrain - Xtrain_min)/(Xtrain_max-Xtrain_min)

        Xtest  = (Xtest  - Xtrain_min)/(Xtrain_max-Xtrain_min)



        Xtrain = Xtrain[Xtrain.columns[~(Xtrain_min==Xtrain_max)]]

        Xtest  = Xtest [Xtest.columns[~(Xtrain_min==Xtrain_max)]]

        

        Xt_min, Xt_max = Xt.min(), Xt.max()

        Xt=(Xt - Xt_min)/(Xt_max-Xt_min)

        Xt = Xt[Xt.columns[~(Xt_min==Xt_max)]]

        #Train

        #To increase generality, we set the number of trees to be large

        dt=RFC(n_estimators=100,criterion=criterion,max_depth=max_height)
        ...
        general_acc.append(accuracy_score(ytest,pred))

        f1s.append(f1_score(ytest,pred))

        f1su+=f1_score(ytest,pred)
        ...
        if Xt is not None and yt is not None:
            ...
            test_acc.append(accuracy_score(yt,pred))

        all_feature_importance.append(dt.feature_importances_)

        

    return [[np.mean(general_acc),np.std(general_acc)],[np.mean(train_acc),np.std(train_acc)],[np.mean(test_acc),np.std(test_acc)],all_feature_importance,f1s,f1su]
n_folds=10
...
results=cross_validate_random_forest(pd.DataFrame(X_train),pd.DataFrame(y_train),pd.DataFrame(X_test),pd.DataFrame(y_test),criterion, max_height, n_folds)
print('General Accuracy: ',results[0][0], 'STD: ',results[0][1])

print('Train Accuracy: ',results[1][0])

print('Test Accuracy: ',results[2][0])

print('F1 Avg: ',results[-1]/10)
