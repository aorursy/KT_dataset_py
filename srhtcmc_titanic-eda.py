...
def bar_plot(variable):
    ...
    plt.bar(x=varValue.index,height =varValue)

    #labeling the bars

    for x,y in zip(varValue.index,varValue):
    ...
    plt.title("{} --- Total : {}".format(variable,total))
...
categories = ["Pclass","Sex","SibSp","Parch","Age","Cabin","Fare","Embarked","Ticket"]

corr = []

for category in categories:

    temp = train_df[[category,"Survived"]].groupby(category,as_index = False).mean().sort_values(by = "Survived",ascending = False) 
...
for i in range(len(corr)):
    ...
    plt.bar(corr[i].iloc[:,0],corr[i].Survived)  

    plt.xlabel(categories[i])

    plt.ylabel("Survived")

    plt.title("{} - Survived".format(categories[i]))
...
train_df[train_df["Fare"].isnull()]
list1 = ["Pclass", "Age", "Sex", "SibSp","Parch", "Fare", "Survived"]
...
for i in index_nan_age:
    ...
    age_prediction = train_df["Age"][((train_df["SibSp"] == train_df.iloc[i]["SibSp"])&
...
train_df["Title"] = train_df["Title"].replace(["Lady","the Countess", "Capt", "Col","Don","Dr","Major","Rev","Jonkheer","Dona","Sir"],"Others")

train_df["Title"] = [0 if i == "Master" else 1 if i == "Miss" or i == "Mlle" or i == "Mrs" else 2 if i == "Mr" else 3 for i in train_df["Title"]]
sns.countplot(x="Title",data = train_df)
...
cv_results = pd.DataFrame({"Cross Validation Means":cv_result,"ML Models":["DecisionTreeClassifier","SVC","RandomForestClassifier",
