...
def generate_cord19_schema():
    ...
    [1] https://ai2-semanticscholar-cord-19.s3-us-west-2.amazonaws.com/2020-04-03/json_schema.txt
    ...
    section_schema = ArrayType(
        ...
        StructType(
            ...
            [
                ...
                StructField("cite_spans", spans_schema),  # Citations 

                StructField("ref_spans", spans_schema),   # References to figures / tables
...
def extract_dataframe_kaggle(spark):
    ...
    for source in sources:
        ...
        for sub_source in sub_sources:
            ...
            path = f"{base}/{source}/{source}/{sub_source}"
...
def join_abstract(rows) -> str:
    ...
    return " ".join([row.asDict().get('text', '') for row in rows])
...
(
    ...
    .select("paper_id", join_abstract("abstract").alias("abstract"))
...
spark.sql(query).show(n=5)
df.columns
# df_final = df.select(['paper_id', join_abstract('body_text').alias('body_text'),

#                                  F.when(F.isnull('abstract'), '').otherwise(join_abstract('abstract')).alias('abstract'),

#                      'bib_entries', 'ref_entries', 'metadata']).cache()
...
SELECT paper_id,
       ...
       join_abstract(body_text) as body_text,

       join_abstract(abstract) as abstract,

       bib_entries, ref_entries, metadata
...
df_final.show(3)
df_final.count()
df_final.write.json('/kaggle/working/df_final_output_non_joined.json')
metadata_csv = spark.read.csv('/kaggle/input/CORD-19-research-challenge/metadata.csv', header=True, inferSchema=True).cache()
metadata_csv.count()
metadata_csv.columns
df_final_joined = metadata_csv.drop('abstract').join(df_final, df_final.paper_id == metadata_csv.sha, how='inner').cache()
# Heap space error

df_final_joined.count()
col_mapping = {c: c.replace(" ", "_") for c in df_final_joined.columns}



col_mapping
df_final_joined = df_final_joined.toDF(*[col_mapping[c] for c in df_final_joined.columns])
df_final_joined.show(5)
df_final_joined.write.json('/kaggle/working/final_cleaned_dataset.json')