    ...
    for i in range(5):
        ...
        for m in range(len(dataSet)):
            ...
            for u in range(k):
                ...
                distances.append(np.linalg.norm(centroids[u]-dataSet[m], ord=2))

            clusterAssment[m] = [distances.index(min(distances))]
...
#         print(clusterAssment)
        ...
        for u in range(k):
            ...
            update_centroid = np.zeros((2), dtype=float)

            count = 0

            for m in range(len(dataSet)):

                if clusterAssment[m] == [u]:

                    update_centroid += np.squeeze(np.array(dataSet[m]))

                    count += 1

            if count > 0:

                update_centroid /= count

                centroids[u] = update_centroid
