...
modalidade = pd.read_csv('../input/modalidade.csv',encoding='utf-8')

modalidade.head(3)
# 4.1 Renomeie a coluna codmodalidade do dataset modalidade para codigomodalidade

modalidade.rename(columns={ 'codmodalidade':'codigomodalidade'})
...
a2017 = licit.loc[licit['dataPublicacao'].str.contains('2017'),['dataPublicacao','situacao']]
# Importa datetime
...
licit.groupby(["ano"]).ano.count()



# 2. Crie um DataFrame com o resultado da quantidade de licitacoes por ano

licit_ano = licit.groupby('ano').numLicitacao.count()  
