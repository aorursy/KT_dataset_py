...
ramen_reviews.head()
(wine_reviews['points'].dropna()-80/4).value_counts().sort_index(ascending=True).rename_axis("Wine_Ratings")
#print(answer_q6())
(ramen_reviews
    .Stars
    ...
    .sort_index())
ramen_reviews.Stars.replace('Unrated', None).dropna().astype('float64').map(lambda v: int(v) if v - int(v) < 0.5 else int(v) + 0.5).value_counts().rename_axis("Ramen Reviews").sort_index()
