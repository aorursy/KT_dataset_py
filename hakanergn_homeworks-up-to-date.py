...
list_label = ["team","champ_num"]

list_col = [team,champ_num]
...
df
df['last_champ_year'] = ["2017","2014"]

df
datac = data.copy()
datac = data.loc[:,["NA_Sales","EU_Sales","JP_Sales"]]

datac.plot();
datac.plot(subplots = True);
datac.plot(kind="scatter",x="NA_Sales",y="JP_Sales");
datac.plot(kind="hist",y = "NA_Sales",bins = 100,range = (0,1),normed = True)
datax = data.copy()
datax['Year'] = pd.to_datetime(datax['Year'])
datax.head()
datax = datax.set_index("Year")

datax
print(datax.loc["1975-06-30"])

print(datax.loc["1975-06-30":"1975-06-20"])
datax.resample("A").mean()
datax.resample("M").mean()