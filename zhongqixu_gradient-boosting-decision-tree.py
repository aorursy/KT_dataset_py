...
class my_gbdt(object):
    ...
    def __init__(self, n_feat, rate):
        ...
        for i in range(n_feat):
            ...
            self.trees.append(DecisionTreeRegressor(min_samples_split=2, max_depth=2))
    ...
    def fit(self, X, y):
        ...
        for i in range(1, self.n_feat):
            ...
            g = np.exp(y_pred) / np.expand_dims(np.sum(np.exp(y_pred), axis=1), axis=1) - y

            self.trees[i].fit(X,g)

            y_pred -= self.rate*self.trees[i].predict(X)
    ...
    def predict(self, X):
        ...
        for i in range(1,self.n_feat):
            ...
            pred_proba -= self.rate*self.trees[i].predict(X)

        return np.argmax(pred_proba, axis=1)
my_model = my_gbdt(20,0.8)
