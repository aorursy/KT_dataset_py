...
class SquareLoss():
        ...
        def loss(self, y, pre):
            ...
            return 0.4 * np.power((y - pre), 3)

        def gradient(self, y, pre):
...
class my_gbdt(object):
    ...
    def fit(self, x, y):
        ...
        pre = self.tree[0].predict(x)

        for i in range(1, self.n_estimators):

            gradient = self.loss.gradient(y, pre)

            self.tree[i].fit(x, gradient)

            pre -= np.multiply(self.learning_rate, self.tree[i].predict(x))
    ...
    def predict(self, x):
        ...
        if not self.regression:
            ...
            pre = np.exp(pre) / np.expand_dims(np.sum(np.exp(pre), axis=1), axis=1)
