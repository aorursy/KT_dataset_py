...
# test['label']=0
sub=pd.read_csv('../input/0726test/sub_mean.csv')

test['label']=sub['y'].values+1
test['label'].value_counts()
train_100k.shape
train_100k=train_100k[~train_100k['content'].isnull()]
train=train_100k[list(test.columns)]
...
train['content']=list(map(lambda x,y:x+','+y,train['post_time'],train['content']))

test['content']=list(map(lambda x,y:x+','+y,test['post_time'],test['content']))
# data_detact
...
train_preds=np.zeros((len(train),3))
...
#     tra=pd.concat([train.iloc[train_index,:],test],axis=0,sort=True)
    ...
    train_preds[test_index,:]=np.exp(predict_model_test(model,val_loader))
...
np.save('test_preds.npy',sum_preds)
f1_score(train['label'].values,np.argmax(train_preds)+1,average='macro')
train['pred1']=train_preds[:,0]

train['pred2']=train_preds[:,1]

train['pred3']=train_preds[:,2]

train[['user_id','label']+['pred1','pred2','pred3']].to_csv('train_preds_all.csv',index=False)
for l in all_loss:
...
sub_mean = pd.read_csv("../input/sub_mean.csv")