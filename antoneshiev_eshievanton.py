...
df_train['new_target'] = 0

df_train.loc[(df_train['treatment_flg'] == 1) & (df_train['target'] == 1), 'new_target'] = 1

df_train.loc[(df_train['treatment_flg'] == 0) & (df_train['target'] == 0), 'new_target'] = 1
...
merged_train = merged_train[merged_train['target'].notnull()]

merged_train['first_issue_date'] = (merged_train['first_issue_date'].dt.weekday > 4).astype(int)

merged_train['first_redeem_date'] = (merged_train['first_redeem_date'].dt.weekday > 4).astype(int)

merged_train['last_month_trans_count'] = (merged_train['last_month_trans_count'] > 4).astype(int)

merged_train['regular_points_received_sum_last_month'] = (merged_train['regular_points_received_sum_last_month'] > 12).astype(int)

merged_train['purchase_sum_last_month'] = (merged_train['purchase_sum_last_month'] > 1731).astype(int)
...
merged_train.pop('target')
merged_train
treatment = merged_train.drop('treatment_flg',axis = 1)

treatment_x = treatment.drop('new_target',axis = 1)

treatment_y = treatment['new_target']
...
df_test['new_target'] = 1

merged_test = pd.concat([df_test,df_clients,features_test],axis = 1,sort = True)

merged_test = merged_test[~merged_test['new_target'].isnull()].copy()
merged_test['first_issue_date'] = (merged_test['first_issue_date'].dt.dayofweek > 4).astype(int)

merged_test['first_redeem_date'] = (merged_test['first_redeem_date'].dt.dayofweek > 4).astype(int)

merged_test['last_month_trans_count'] = (merged_test['last_month_trans_count'] > 4).astype(int)

merged_test['regular_points_received_sum_last_month'] = (merged_test['regular_points_received_sum_last_month'] > 12).astype(int)

merged_test['purchase_sum_last_month'] = (merged_test['purchase_sum_last_month'] > 1731).astype(int)
...
merged_test
test_x = merged_test.drop('new_target',axis = 1)
pred = treatment_model.predict_proba(test_x)[:,1]
