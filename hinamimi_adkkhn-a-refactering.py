...
top30=sorted(d.items(), key=itemgetter(1), reverse=True)[:30]
...
_ = plt.title('Common words in disaster tweets')
d=defaultdict(int)
...
top30=sorted(n_d.items(),key=lambda x:x[1],reverse=True)[:30]
fig=plt.figure(figsize=(8.0, 8.0))
...
_ = plt.title('Common words in non-disaster tweets')
d=defaultdict(int)
...
for w,a,b,p in LI[:31]:
    ...
    print()
#target==is_targetでWを含むツイートを全て表示
...
view('happy',1)
view('apocalypse',0)
def create_wordcloud(text,stop,is_target):
