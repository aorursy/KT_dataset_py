...
train_data['Fare']=train_data['Fare'].replace({np.NaN:np.mean(train_data['Fare'])})
...
df = pd.DataFrame(z).rename(columns={0:'Imp_value'})
X_final.columns
df['Feature']=X_final.columns
df
df.sort_values('Imp_value',ascending=False)