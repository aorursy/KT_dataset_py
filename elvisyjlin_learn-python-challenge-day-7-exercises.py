...
def prettify_graph(graph):
    ...
    graph.set_ylim(bottom=0)
    graph.set_ylabel('Balance')
    graph.set_yticklabels(['${:d}'.format(int(x)) for x in graph.get_yticks().tolist()])
    ...
    def count(hand):
        ...
        for card in hand:
            if card.isdigit():
                ...
                value += 10
        if num_of_As > 0 and value + 10 <= 21:
            value += 10
        if value > 21:
            value = -1
        return value
    return count(hand_1) > count(hand_2)
...
r = roulette.RouletteSession(11, 10000, 0.5)
history = []
while r.num_remaining_spins() > 0:
    num = r.spin(None)
    history.append(num)
# From my day 6 exercises
def conditional_roulette_probs(history):
    ...
    for idx in range(len(history)-1):
        dice_a = history[idx]
        dice_b = history[idx+1]
        if dice_a not in land_on:
            land_on[dice_a] = {}
        if dice_b not in land_on[dice_a]:
            land_on[dice_a][dice_b] = 1
        else:
            land_on[dice_a][dice_b] += 1
        if dice_a not in total:
            total[dice_a] = 1
        else:
            total[dice_a] += 1
    prob = {}
    for dice_a in land_on.keys():
        prob[dice_a] = {}
        for dice_b in land_on[dice_a].keys():
            prob[dice_a][dice_b] = land_on[dice_a][dice_b] / total[dice_a]
    return prob

conditional_roulette_probs(history)
# From my day 6 exercises
def conditional_roulette_strategy(history):
    """
    ...
    for dice_a in land_on.keys():
        ...
        for dice_b in land_on[dice_a].keys():
            prob[dice_a][dice_b] = land_on[dice_a][dice_b] / total[dice_a]
            if prob[dice_a][dice_b] > 0.2:
                strategy.append((dice_a, dice_b))
    return strategy
def my_agent(wheel):
    last_number = 0
    history = []
    ...
    while wheel.num_remaining_spins() > 0:
        ...
        strategy = conditional_roulette_strategy(history)
