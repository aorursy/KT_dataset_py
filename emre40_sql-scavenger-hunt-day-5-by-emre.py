...
commits = bq_helper.BigQueryHelper(active_project="bigquery-public-data",
                                              dataset_name="github_repos")
query = ("""      
        SELECT sf.repo_name, COUNT(sf.path) AS number_of_commits
        FROM `bigquery-public-data.github_repos.sample_files` as sf
        -- Table to merge into commit_files
        ...
        """)
commit_counts_by_repo = commits.query_to_pandas_safe(query, max_gb_scanned=6)
commit_counts_by_repo