...
grid_lasso_sc = GridSearchCV(lasso, parameters, cv=nr_cv, verbose=1, scoring = score_calc)

grid_lasso_sc.fit(X_sc, y)



score_lasso_sc = get_best_score(grid_lasso_sc)



pred_lasso_all_sc = grid_lasso_sc.predict(X_test_sc)
pred_lasso_all_sc
# 予測結果の対数を外す

pred_lasso_all_sc = np.exp(pred_lasso_all_sc)



# 負の予測結果を平均値で埋める

pred_lasso_all_sc[pred_lasso_all_sc < 0] = pred_lasso_all_sc.mean()
pred_lasso# submit用のファイルを作成

sub_lasso_sc = pd.DataFrame()

sub_lasso_sc['Id'] = df_test['Id']

sub_lasso_sc['SalePrice'] = pred_lasso_all_sc

sub_lasso_sc.to_csv('lasso_sc.csv',index=False)
sub_lasso_sc.head()
...
pred_enet_all_sc = grid_enet.predict(X_test_sc)
# 予測結果の対数を外す

pred_enet_all_sc = np.exp(pred_enet_all_sc)



# 負の予測結果を平均値で埋める

pred_enet_all_sc[pred_enet_all_sc < 0] = pred_enet_all_sc.mean()
# submit用のファイルを作成

sub_enet_sc = pd.DataFrame()

sub_enet_sc['Id'] = df_test['Id']

sub_enet_sc['SalePrice'] = pred_enet_all_sc

sub_enet_sc.to_csv('enet_sc.csv',index=False)
sub_enet_sc.head()
# アンサンブル

sub_ensemble_sc = pd.DataFrame()

sub_ensemble_sc['Id'] = df_test['Id']

sub_ensemble_sc['SalePrice'] = np.mean(

    [pred_linreg_all_sc, pred_ridge_all_sc, pred_lasso_all_sc, pred_enet_all_sc], axis=0)

sub_ensemble_sc.to_csv('emsemble_sc.csv',index=False)
sub_ensemble_sc.head()