...
# MY_TRAIN_FILE = 'train.csv'

# MY_TEST_FILE  = 'test.csv'



# cleaned files are kept here for speedup

MY_TRAIN_FILE = "/kaggle/input/cleaned-data/train.csv"

MY_TEST_FILE  = "/kaggle/input/cleaned-data/test.csv"
...
def cleanDFs ():
    ...
    for file in files:
        ...
        if file == TRAIN_FILE:
            ...
            df.to_csv (MY_TRAIN_FILE, index=False)

        else:

            df.to_csv (MY_TEST_FILE, index=False)

    return
# cleanDFs ()
tokenizer = BertTokenizer.from_pretrained ('bert-base-uncased', do_lower_case=True)
trainDF = pd.read_csv (MY_TRAIN_FILE)

trainDF['text_cleaned'] = trainDF.text_cleaned.astype (str)

trainDF['labels'] = trainDF.target_relabeled.astype (np.int64)

trainDF      = trainDF[['text_cleaned', 'labels']]

train_texts  = list (trainDF.text_cleaned.values)

train_labels = list (trainDF.labels.values)
testDF = pd.read_csv (MY_TEST_FILE)
...
# assign dummy lables as this helps in using the same custom dataset class below

# testDF['labels'] = [0] * (testDF.shape[0]//2) + [1] * (testDF.shape[0] - testDF.shape[0]//2)

# testDF['labels'] = testDF.labels.astype (np.int64)

testDF['labels'] = testDF.target.astype (np.int64)



testDF      = testDF[['id', 'text_cleaned', 'labels']]

test_texts  = list (testDF.text_cleaned.values)

test_labels = list (testDF.labels.values)
sentences = trainDF.text_cleaned.values
...
# Run this to see how the eval metrics change with #epochs.

# Then train on the whole training dataset for these many epochs(=1)
...
perfect_labels = pd.read_csv ('/kaggle/input/perfect-submission-benchmark/perfect_submission.csv').target.values

precision, recall, f1, _ = precision_recall_fscore_support (perfect_labels, pred_labels, average='binary')

acc = accuracy_score (perfect_labels, pred_labels)
