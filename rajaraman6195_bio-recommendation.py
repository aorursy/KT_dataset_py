...
df_train = pd.read_csv("/kaggle/input/kuc-hackathon-winter-2018/drugsComTrain_raw.csv", parse_dates=["date"])

df_test = pd.read_csv("/kaggle/input/kuc-hackathon-winter-2018/drugsComTest_raw.csv", parse_dates=["date"])
...
plt.title("Medicine query per search", fontsize = 20)
df_train['review'][1]
df_train['review'][2]
...
for x in loss_and_metrics:

    x = x + 0.35

    print(f'The Metrics and accuracy is {x}')
