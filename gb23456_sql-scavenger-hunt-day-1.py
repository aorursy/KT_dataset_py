...
# the query_to_pandas_safe will only return a result if it's less
# than one gigabyte (by default)
not_ppm_country = open_aq.query_to_pandas_safe(country_query)
# distinct COUNTRY with value not ppm
not_ppm_country
...
# the query_to_pandas_safe will only return a result if it's less
# than one gigabyte (by default)
zero_pollutant = open_aq.query_to_pandas_safe(pollutant_query)
# distinct pollutant with value = 0
zero_pollutant