...
# submission = find_probabilty(model1, ds_testAug, steps, cfg, filenames_test, csv_name) # for inference on test set
with strategy.scope():
    model1 = get_model_b1()
...
model1 = compileNewModel(cfg, model1)
history = learnModel(model1, ds_train, stepsTrain, cfg, ds_val, stepsVal)
pd.DataFrame.from_dict(history.history).to_csv('history-B3.csv',index=False)
with strategy.scope():
    model1 = get_model_b4()
...
model1 = compileNewModel(cfg, model1)
history = learnModel(model1, ds_train, stepsTrain, cfg, ds_val, stepsVal)
pd.DataFrame.from_dict(history.history).to_csv('history-B5.csv',index=False)
# with strategy.scope():
#     model1 = get_model_b6()
...
# submission = find_probabilty(model2, ds_testAug, steps, cfg, filenames_test, csv_name) # for inference on test set
with strategy.scope():
    model3 = get_model_b5()
...
# submission = find_probabilty(model3, ds_testAug, steps, cfg, filenames_test, csv_name) # for inference on test set
# cfg['batch_size'] = 32
# history = []
...
# GCS_PATH3 = KaggleDatasets().get_gcs_path('tfrecord-1920x1080')
# filenames_test2 = np.array(tf.io.gfile.glob(GCS_PATH3 + '/test*.tfrec'))
# cfg['batch_size'] = 39
