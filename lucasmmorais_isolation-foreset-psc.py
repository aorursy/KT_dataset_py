...
dataset = pd.read_csv('../input/testwithbonusanomaly/df_test.csv')

anomalies = dataset.loc[:,"bonusAnomalie"]
print(dataset)
...
low_cardinality_cols = dataset[['bonus', 'age', 'hours_per_week']]
...
for i in sample:
        ...
        if (np.abs(test_data[i]-anomalies[i])==1):
