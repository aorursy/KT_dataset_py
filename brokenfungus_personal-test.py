...
lgbm_eval = lgbm.Dataset(validation[numeric_columns + string_columns], validation[target_column])
...
#model = lgbm.train(params, lgbm_train, num_boost_round=20, valid_sets=lgbm_eval, early_stopping_rounds=5)

lgbm_train = lgbm.Dataset(train[numeric_columns + string_columns], train[target_column])

model = lgbm.train(params, lgbm_train, num_boost_round=20)
#Now lets prediction on the test data

grades = model.predict(test[numeric_columns + string_columns], num_iteration=model.best_iteration)
