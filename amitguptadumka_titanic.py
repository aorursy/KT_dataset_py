...
models = pd.DataFrame({'Model': ['Support Vector Machines', 'KNN', 'Logistic Regression', 'Random Forest', 'Naive Bayes', 'Perceptron',
       'Stochastic Gradient Decent', 'Linear SVC', 'Decision Tree','Linear Regression'],'Score': [acc_svc, acc_knn, acc_log,acc_random_forest, acc_gaussian, acc_perceptron, 
       acc_sgd, acc_linear_svc, acc_decision_tree,acc_lin]})
plt.bar(models['Model'],models['Score'],color="blue")
plt.ylabel('Score')
plt.xticks(rotation=90)
for a,b in zip(models['Model'],models['Score']):
    plt.text(a, b, str(b))
plt.show()
...
#import module
# import inspect
# src = inspect.getsource(LinearRegression)
# print(src)
