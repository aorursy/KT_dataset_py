...
df.isnull().sum()
df
import seaborn as akash

akash.distplot(df.charges)
akash.lineplot(x = df.age , y = df.charges , hue = df.smoker, style = df.sex)
akash.set(rc={'figure.figsize': [20,20]})

akash.scatterplot(x= df.charges , y = df.age)
df.info()
df.isnull().sum()
