...
print(earthquakes['Date'].sample(20))
...
# From comments on Rachael's original notebook, others had the same issue (which wasn't an issue for the landslides data).
# Tanveer Baba said "yeah you are right, we can also use another property errors to work like this:
# earthquakes['date_parsed'] = pd.to_datetime(earthquakes['Date'], format = "%m/%d/%Y", errors = 'coerce')
# "

# So, trying again, and adding `, errors = 'coerce'` (And #ing my answer above) :
earthquakes['date_parsed'] = pd.to_datetime(earthquakes['Date'], format = "%m/%d/%Y", errors = 'coerce')
