...
def get_valid_moves(obs, config):
    ...
    return [col for col in range(config.columns) if any(obs.board[i * config.columns + col] == 0 for i in range(config.rows))]
...
def agent_q2(obs, config):
    ...
    opponent_winning_moves = [move for move in valid_moves if check_winning_move(obs, config, move, (obs.mark) % 2 + 1)]

    if len(winning_moves) != 0:

        return random.choice(winning_moves)

    elif len(opponent_winning_moves) != 0:
