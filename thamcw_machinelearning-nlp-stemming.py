...
#It makes use of PorterStemmer module from the Natural Language Toolkit (nltk) in Python to

#break down the speech into its constituent words and then determine the root form of each word.



#first import the nltk package for NLP processing work
...
#ntlk.download()
#import the PorterStemmer module from nltk that is used to extract root-words from a text.
...
#test out the stemming function using some general text samples.

sentences_one="John is working now."

sentences_two="John is working intelligently."
#The root-word is extracted correctly.

stemmer.stem('working')
#The root-word for this word will not be formed correctly.

stemmer.stem('intelligently')
#assign the speech to variable paragraph

#use """" to allow paragraph to span multiple lines

paragraph="""I am happy to join with you today in what will go down in history as the greatest demonstration for 
...
#check that the paragraph is created properly. Sometimes, you are reading it from a csv or tsv file.
...
#Then stem each word in the sentence and re-assign these stemmed words back into the sentence
...
#You can see that stemming is not able to create perfect root-word extraction as it does not check words

#for correctness. For more accurate word extraction, lemmatization should be used.

#However,stemming is still used in situations where one is not concerned with accuracy of words but is just

#hunting for, say, spam.
