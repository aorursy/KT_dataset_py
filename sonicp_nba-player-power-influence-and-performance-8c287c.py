...
sns.heatmap(attendance_valuation_elo_df.corr())
plt.subplots(figsize=(15,15))

sns.heatmap(br_stats_df.corr())
df_social_sal = pd.read_csv("../input/nba_2017_players_with_salary_wiki_twitter.csv");df_social_sal.head()
#Salary vs winning

sns.lmplot(x="SALARY_MILLIONS", y="WINS_RPM", data= df_social_sal)
#Twitter favourite count vs winning

sns.lmplot(x="TWITTER_FAVORITE_COUNT", y="WINS_RPM", data= df_social_sal[df_social_sal.TWITTER_FAVORITE_COUNT<2000])
#pageviews vs winning

sns.lmplot(x="PAGEVIEWS", y="WINS_RPM", data= df_social_sal[df_social_sal.PAGEVIEWS<2000])
df1 = pd.read_csv("../input/nba_2016_2017_100.csv")
#merging 2 data sets to compare two different data points

df_final = pd.concat([df1, df_social_sal],axis = 1)
sns.lmplot(x="TWITTER_FOLLOWER_COUNT_MILLIONS", y="WINS_RPM", data= df_final[df_final.TWITTER_FOLLOWER_COUNT_MILLIONS<4])