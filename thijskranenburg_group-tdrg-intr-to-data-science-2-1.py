...
iplot(figure)
# Groep attacks by month



terror_month_sum = np.asarray(data_terrorism.groupby('imonth').nkill.sum())

terror_month_count = np.asarray(data_terrorism.groupby('imonth').nkill.count())

terror_months = np.arange(1,13)



# drop values from the rows where the month was unknown

terror_month_sum = np.delete(terror_month_sum,0,0)

terror_month_count = np.delete(terror_month_count,0,0)
# Create graph
...
trace1 = go.Bar(
    ...
    name='Number of people killed per month'
...
trace2 = go.Bar(
    ...
    y=terror_month_count,

    name='Number of attacks per month'
...
to_drop2 = ['iyear','country', 'region', 'vicinity', 'success', 'attacktype1', 'targtype1','individual', 'nperps', 'claimed', 'weaptype1',  'nkill', 'boolean_nkill', 'nwoundte', 'country_txt', 'latitude', 'longitude', 'property', 'propextent', 'avg_nkill']
...
# Create the confusion matrices using a subsample of 25 percent to becrease the running time significantly



# create dataframe including y, for random sampling

to_drop5 = ['iyear','country', 'region', 'vicinity', 'success', 'attacktype1', 'targtype1','individual', 'nperps', 'claimed', 'weaptype1', 'nwound',  'nkill', 'nwoundte', 'country_txt', 'latitude', 'longitude', 'property', 'propextent', 'avg_nkill']

data5_feat_space = data2.drop(to_drop5,axis=1)



# get subsample of total data

data_confusion = data5_feat_space.sample(frac=0.25)



# create dataframe excluding y

to_drop5 = ['boolean_nkill']

X5 = data_confusion.drop(to_drop5,axis=1)



# transform True False to boolean

y5 = data_confusion['boolean_nkill']

y5 = y5*1



X5 = X5.as_matrix().astype(np.float)
...
confusion_matrices5 = [
    ...
    ( "Support Vector Machines", confusion_matrix(y5,run_cv(X5,y5,SVC)) ),

    ( "Random Forest", confusion_matrix(y5,run_cv(X5,y5,RF)) ),

    ( "K-Nearest-Neighbors", confusion_matrix(y5,run_cv(X5,y5,KNN)) ),
...
for idx in range(3):
    ...
    plot_confusion_matrix(confusion_matrices5[idx][1],["False","True"],title=confusion_matrices5[idx][0])
# Pull out features for future use
...
to_drop6 = ['nwound','nkillter','iyear','country', 'region', 'vicinity', 'success', 'attacktype1', 'individual', 'claimed', 'weaptype1', 'nwound',  'nkill', 'nwoundte', 'country_txt', 'latitude', 'longitude', 'property', 'propextent', 'avg_nkill']

data6_feat_space = data4.drop(to_drop6,axis=1)



# get subsample of total data

data_confusion = data6_feat_space.sample(frac=0.25)



# create dataframe excluding y

to_drop6 = ['boolean_nkill']

X6 = data_confusion.drop(to_drop6,axis=1)



# transform True False to boolean

y6 = data_confusion['boolean_nkill']

y6 = y6*1



X6 = X6.as_matrix().astype(np.float)
...
confusion_matrices6 = [
    ...
    ( "Support Vector Machines", confusion_matrix(y6,run_cv(X6,y6,SVC)) ),

    ( "Random Forest", confusion_matrix(y6,run_cv(X6,y6,RF)) ),

    ( "K-Nearest-Neighbors", confusion_matrix(y6,run_cv(X6,y6,KNN)) ),
...
for idx in range(3):
    ...
    plot_confusion_matrix(confusion_matrices6[idx][1],["False","True"],title=confusion_matrices6[idx][0])