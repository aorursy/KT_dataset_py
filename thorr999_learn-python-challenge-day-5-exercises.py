...
#q1.hint()
#q1.solution()
#[1,2,3,4] > 2   # cannot loop thru list
...
sum([estimate_average_slot_payout(10000) for i in range(1000)])/1000    # central limit thoerem
q4.solution()
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    for _ in range(n_simulations): 
        ...
        for i in range(n_spins): 
            bal -= 1    # cost of each spin
            if bal < 0: 
                success.append(0)
                break
            bal +=  play_slot_machine()  # add winning if any
        else: success.append(1)
    return sum(success) / n_simulations
    #return success
slots_survival_probability(10, 20, 10000)
#q5.check()
q5.solution()