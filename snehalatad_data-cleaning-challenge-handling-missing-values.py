...
sf_missing_cells = sf_missing_values.sum()

(sf_missing_cells/sf_total_cells) * 100
# look at the # of missing points in the first ten columns
missing_values_count[0:10]
...
sf_dropped_na_columns = sf_permits.dropna(axis=1)
sf_dropped_na_columns.head()

print("Columns in original dataset : %d" %sf_permits.shape[1])
print("Columns in tranformed dataset : %d " %sf_dropped_na_columns.shape[1])
