...
for paper in np.random.choice(papers):
...
corpus['text_prep'] = corpus['text'].apply(text_preprocessing)
...
prev_time = measure_time_step(0, True)

vectorizer = TfidfVectorizer()

vectorizer.fit(corpus['text_prep'].apply(lambda x: ' '.join(x)))

prev_time = measure_time_step(prev_time)
...
def get_key_words(text):
    ...
    vector = np.array(vectorizer.transform([text]).todense())[0]

    for i in np.argsort(vector)[-5:]:

        print(vectorizer.get_feature_names()[i], vector[i])

        

get_key_words(' '.join(text_preprocessing(text)))
vectorizer.transform([corpus['text_prep'].apply(lambda x: ' '.join(x)).values[1]])
np.array(vectorizer.transform([corpus['text'].apply(lambda x: ' '.join(x)).values[1]]).todense())[0]
vectorizer.transform([corpus['text'].apply(lambda x: ' '.join(x)).values[1]])
corpus['text_prep'].apply(lambda x: ' '.join(x)).values[1]