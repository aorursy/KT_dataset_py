...
pd.read_csv('/kaggle/input/bosch-production-line-performance/train_categorical.csv.zip',nrows= 10)
...
for i in pd.read_csv('/kaggle/input/bosch-production-line-performance/train_categorical.csv.zip', dtype = str,iterator=True, chunksize=size,compression='zip'):
    ...
    i.insert(loc=2141,column= 'finito',value=0)
    ...
    while flag == 0 :
        ...
        newframe.to_csv(station_name+'_categorical.csv',index = False)
    ...
    while(flag == 1) and (len(i) != 0):
        ...
        #newframe.insert(loc=2141,column= 'finito',value=0)
        ...
        newframe.to_csv(station_name+'_categorical.csv',header = False,index = False,mode = 'a')
...
for i in stations:
    ...
    df = pd.read_csv( i+'_categorical.csv')
...
for i in stations:
    ...
    while(True):
        ...
        if counter > len(df.columns)-1:
            ...
            break
Features
pd.read_csv('S1_categorical.csv')
pd.read_csv('S10_categorical.csv')
