...
for colname, colvalue in video.iteritems():
    ...
    print(colname, 'SPACE', colvalue[2])
...
for group, frame in video8th.groupby(['Platform']):

    #print(frame.columns)

    str_list = [] # empty list to contain columns with strings (words)

    for colname, colvalue in frame.reset_index().iteritems():
    ...
    num_list = frame.columns.difference(str_list)

    # Create Dataframe containing only numerical features

    video_num = frame[num_list]

    f, ax = plt.subplots(figsize=(7, 7))

    title_str = ('Pearson Correlation of Video Game Numerical Features' 
    ...
    sns.heatmap(video_num.astype(float).corr(),
