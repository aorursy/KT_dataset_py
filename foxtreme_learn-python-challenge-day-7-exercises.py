...
def prettify_graph(graph):
    ...
    graph.set_yticklabels(['${}'.format(int(ytick)) for ytick in yticks])
...
def sum_hand(hand):
    ...
    while aces>0:
        asEleven = total +11
        asOne = total +1
        if (asEleven>asOne) and asEleven<=21:
            total+=11
        else:
            total+=1
        aces-=1
...
def blackjack_hand_greater_than(hand_1, hand_2):
    ...
    if ((sum_hand_1 > sum_hand_2) and (sum_hand_1 <=21)) or ((sum_hand_1 <=21) and (sum_hand_2>21)):
        return True
    else:
        return False
