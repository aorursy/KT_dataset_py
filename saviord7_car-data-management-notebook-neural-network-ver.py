...
df_Labels.dataframeName = 'first Labels'
df_Labels.head()
...
iv_test.head()
# Применим алгоритм многослойного перцептрона (MLP)
...
Train_results['predictedRating']=pd.DataFrame(mlp.predict(iv_test)).reset_index(drop=True)
# Выведем результат.
