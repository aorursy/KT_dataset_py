...
#write a loop to find the ideal tree size from candidates 
min_mae= get_mae(candidate_max_leaf_nodes[0], train_X, train_y, val_X,val_y)
best_index=0 
for i in range(1,len(candidate_max_leaf_nodes)):
    mae= get_mae(candidate_max_leaf_nodes[i], train_X,train_y,val_X,val_y)
    print (mae)
    if mae< min_mae:
        min_mae=mae
        best_index=i
