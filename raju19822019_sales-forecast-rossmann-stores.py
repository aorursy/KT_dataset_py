...
train.info()
train.head()
# adding new variable, no of sales per customer on the perticular day

train['SalePerCustomer'] = train['Sales']/train['Customers']
train['SalePerCustomer'].describe()
train['SalePerCustomer'].value_counts().head(10)
train.nlargest(10, ['SalePerCustomer'])
train.Year.unique()
train.isnull().sum()
train.fillna(0, inplace = True) # inplace true is making changes in the original file 
...
# closed stores and nill sales 
train[(train.Open == 0) & (train.Sales == 0)].shape
# opened stores with zero sales
zero_sales = train[(train.Open != 0) & (train.Sales == 0)]
print("In total: ", zero_sales.shape)
zero_sales.head(5)
#print("Closed stores and days which didn't have any sales won't be counted into the forecasts.") so lets consider open days and not zero sales to forecast
...
# store types
sales_a.resample('M').sum().plot(color = c, ax = ax1)
sales_b.resample('M').sum().plot(color = c, ax = ax2)
sales_c.resample('M').sum().plot(color = c, ax = ax3)
sales_d.resample('M').sum().plot(color = c, ax = ax4)
f, (ax1, ax2, ax3, ax4) = plt.subplots(4, figsize = (12, 13))
