...
#計算を高速で行うための拡張モジュール　とりあえず。

import numpy as np



#データの読み込み、欠損値の確認、グラフ化等に必要。　必須。

import pandas as pd



#グラフの描画に特化したライブラリ。　グラフ描くなら必須。

from matplotlib import pyplot as plt



#matplotlibよりも美しくグラフが描けるライブラリ。　必要に応じて。
...
#歪度：正の値だと、左に寄っている(右側の裾が広い)

#尖度：正の値だと、正規分布より尖っている(データが真ん中に寄っている)
#物件の広さを合計した変数を作成
...
train = train.drop(train[(train['TotalSF']>7500) & (train['SalePrice']<500000)].index)

train = train.drop(train[(train['TotalSF']<3000) & (train['SalePrice']>350000)].index)
...
train = train.drop(train[(train['GarageArea']<800) & (train['SalePrice']>500000)].index)
...
train_other = train.drop("SalePrice",axis=1)

train_sale = train["SalePrice"]



#学習データとテストデータを統合

all_data = pd.concat([train_other,test],axis=0,sort=True)
...
print("train_other: "+str(train_other.shape))

print("train_sale: "+str(train_sale.shape))
...
train_sale = np.log1p(train_sale)
...
train_other = all_data.iloc[:train_other.shape[0],:].reset_index(drop=True)

test = all_data.iloc[train_other.shape[0]:,:].reset_index(drop=True)
...
train_other, valid_other, train_sale, valid_sale = train_test_split(
...
dtrain = xgb.DMatrix(train_other,label=train_sale)

dvalid = xgb.DMatrix(valid_other,label=valid_sale)
