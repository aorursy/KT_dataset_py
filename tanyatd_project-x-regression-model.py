# ***Fariz*** - can you please load the clean dataset and write the script for the following:

# We want to run the following logistic regression models:

    # Model 1 - Target = Days Employed
    # Model 2 - Target = Car_Ownership + House_Ownership
    # Model 3 - Target = Amt_Credit / Amt_Income
    # Model 4 - Target = Days Employed + Car_Ownership + House_Ownership + Amt_Credit / Amt_Income
    
# for each model we need output (exported to a csv file ) for the following:
    # Model Fit - lab wk 6 
    # VIF / p value  - lab wk 7
    # Coefficients - lab wk 4
    # confusion matrix - lab wk 5
...
'''
app_train = pd.read_csv('../input/application_for_regression.csv')
app_vis = pd.read_csv('../input/application_for_visualisation.csv')
app_train.describe()
...
#app_train_model1= app_train.drop([x.upper() for x in ['code_gender','flag_own_car','flag_own_realty','name_income_type','name_education_type','occupation_type']], axis=1)
app_train
from sklearn.preprocessing import LabelEncoder
le = LabelEncoder()
le_count = 0
...
app_train
#running second model,  #  # Model 2 - Target = Car_Ownership + House_Ownership
Y2 = app_train['TARGET']
X2 = app_train.drop(['TARGET','AMT_INCOME_TOTAL','AMT_CREDIT'], axis = 1)


model2_inference = Inference(logistic = True)
model2_inference.fit(X2, Y2)
model2_inference.coefficients()


model2_logistic = LinearModel(logistic = True)
model2_logistic.fit(X2, Y2)
model2_logistic.plot(model2_logistic.predict(X2), Y2)
model2_logistic.coefficients(plot = True)

model2_logistic.analyse(plot = True)

model2_logistic.score(model2_logistic.predict(X2), Y2)

accuracy_model2 = (model2_logistic.predict(X2) == Y2).sum()/len(Y2)
accuracy_model2
app_train['CREDIT/INCOME'] = app_train['AMT_CREDIT']/app_train['AMT_INCOME_TOTAL']
#running third model,  #  #Model 3 - Target = Amt_Credit / Amt_Income
Y3 = app_train['TARGET']
X3 = app_train.drop(['TARGET','AMT_INCOME_TOTAL','AMT_CREDIT','FLAG_OWN_REALTY','FLAG_OWN_CAR'], axis = 1)

model3_inference = Inference(logistic = True)
model3_inference.fit(X3, Y3)
model3_inference.coefficients()

model3_logistic = LinearModel(logistic = True)
model3_logistic.fit(X3, Y3)



model3_logistic.score(model3_logistic.predict(X3), Y3)

model3_logistic.coefficients(plot = True)
model3_logistic.analyse(plot = True)
prediction_model3 = tally(model3_logistic.predict(X3))
prediction_model3

accuracy_model3 = (model3_logistic.predict(X3) == Y3).sum()/len(Y3)
accuracy_model3
app_train["ASSET"]= app_train['FLAG_OWN_REALTY'] + app_train["FLAG_OWN_CAR"]
#app_train["ASSET"]
#running FOURTH model,  #  #Model 4 - Target = ALL VARIABLES
Y4 = app_train['TARGET']
X4 = app_train.drop(['TARGET','AMT_INCOME_TOTAL','AMT_CREDIT','FLAG_OWN_REALTY','FLAG_OWN_CAR'], axis = 1)

#model4 = Inference(logistic = True)
#model4.fit(X4, Y4)
#model4.coefficients()

model4_logistic = LinearModel(logistic = True)
model4_logistic.fit(X4, Y4)
model4_logistic.plot(model4_logistic.predict(X4), Y4)
model4_logistic.coefficients(plot = True)

model4_logistic.score(model4_logistic.predict(X4), Y4)

model4_logistic.analyse(plot = True)

model4_logistic.coefficients(plot = True)

prediction_model4 = tally(model4_logistic.predict(X4))
prediction_model4

accuracy_model4 = (model4_logistic.predict(X4) == Y4).sum()/len(Y4)
accuracy_model4
...
model4_logistic.score(model4_logistic.predict(X4), Y4)
Y5 = app_train['TARGET']
X5 = app_train.drop(['TARGET','AMT_INCOME_TOTAL','AMT_CREDIT','FLAG_OWN_REALTY','FLAG_OWN_CAR'], axis = 1)
X5["DAYS_BIRTH"] = app_vis["DAYS_BIRTH"]
X5["NAME_INCOME_TYPE"] = app_vis["NAME_INCOME_TYPE"]

X5 = pd.get_dummies(X5)
model5_logistic = LinearModel(logistic = True)
model5_logistic.fit(X5, Y5)

model5_logistic.plot(model5_logistic.predict(X5), Y5)
model5_logistic.coefficients(plot = True)

model5_logistic.score(model5_logistic.predict(X5), Y5)

model5_logistic.analyse(plot = True)

accuracy_model5 = (model5_logistic.predict(X5) == Y5).sum()/len(Y5)
accuracy_model5



prediction_model5 = tally(model5_logistic.predict(X5))
prediction_model5