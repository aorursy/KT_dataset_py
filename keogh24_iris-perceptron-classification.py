...
plt.scatter(df['SepalWidthCm'], df['PetalWidthCm'], c=colors)
colors = df['Species'].replace(to_replace=['Iris-setosa', 'Iris-virginica', 'Iris-versicolor'], value=['red', 'green', 'green'])
plt.scatter(df['SepalWidthCm'], df['PetalWidthCm'], c=colors)
df.head()
...
y = df2[['Species']]
y.head()
X = df2[['SepalWidthCm', 'PetalWidthCm']]
X.head()
# X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25)
# inputs = X_train.as_matrix()
...
min = pd.Series.min(df['SepalWidthCm'])
max = pd.Series.max(df['SepalWidthCm'])
x1 = [min, max]
