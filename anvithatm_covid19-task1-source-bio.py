...
related_articles_index_file = pd.read_excel("/kaggle/input/bio-related-articles-info/Level1_task1_related articles.xlsx")
import pickle
...
vect_scores_text = tfidf_vect_text.fit_transform(preprocessed_text_bio)

vect_scores_abs = tfidf_vect_abs.fit_transform(preprocessed_abs_bio)
...
def extract_article(search_words):
    ...
    for search_word in search_words:
        ...
        for i in range(0,csv_data_bio.shape[0]):

            #i = 67

            #print(i)

            if search_word in csv_data_bio.iloc[i,5].lower():
                ...
                for word in word_tokens:
                    ...
                    match_word = difflib.get_close_matches(word, tfidf_vect_text.get_feature_names(),cutoff=0.75)

                    if match_word:

                        #new_search_word = new_search_word + " " + match_word[0]

                        new_search_word = match_word[0]

                        importance_score = get_word_imp_score_article(new_search_word,i)
            ...
            if importance_score > final_imp_score:

                #print("importance_score:",importance_score)

                #print("final_imp_score:",final_imp_score)

                final_imp_score = importance_score

                #print("article_number:",i)

                #print("****")

                final_article = csv_data_bio.iloc[i,5]

                article_idx = i

        articles = [articles_imp_score.index(score) for score in articles_imp_score if score > 0.7*max(articles_imp_score)]

        articles_score = [score for score in articles_imp_score if score > 0.7*max(articles_imp_score)]
        ...
        articles_scores.append(articles_score)

       

#     with open('C:/Users/anvitha/Desktop/Anvitha/Kaggle/COVID19/output/articles/final_articles.txt', 'w',encoding="utf-8") as f:
...
#             f.write("%s\n" % item)
    ...
    #print("Final articles is present in ../output/articles/final_articles.txt")

    return final_articles_idx,articles_scores
def extract_abstract(final_articles_idx):

    final_article_abstracts = []

    final_article_abstracts = [csv_data_bio.iloc[idx,4] for idx in final_articles_idx]

    with open('C:/Users/anvitha/Desktop/Anvitha/Kaggle/COVID19/output/abstract/final_abstract.txt', 'w',encoding="utf-8") as f:

        for item in final_article_abstracts:

            f.write("%s\n" % item)

    print("Abstracts related to articles is present in ../output/similar_abstract/similar_abstract.txt")

    return final_article_abstracts
...
def save_related_articles_extract_abstract(sub_cat_no):
    ...
    final_abstracts = [csv_data_bio.iloc[idx,4] for idx in art_idx]
    ...
    article_title= [csv_data_bio.iloc[idx,1] for idx in art_idx]
