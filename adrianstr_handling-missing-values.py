...
display(missing_values_count_permits[0:10])

total_cells_permits = np.product(sf_permits.shape)
total_missing_permits = missing_values_count_permits.sum()

print('Percentage of missing data: ~ %s%%' % ((total_missing_permits/total_cells_permits) * 100).astype(int))
# look at the # of missing points in the first ten columns
missing_values_count[0:10]
...
display(cols_with_na_dropped.head())
print("Columns in original dataset: %d \n" % sf_permits.shape[1])
