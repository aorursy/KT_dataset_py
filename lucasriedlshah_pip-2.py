...
plt.legend
plt.show()
def euler_method_error(position_x, position_y, exact_position_x, exact_position_y):
    return np.sqrt(position_x ** 2 + position_y ** 2) - np.sqrt(exact_position_x ** 2 + exact_position_y ** 2)

plt.figure(9)
plt.plot(
    time,
    euler_method_error(position_x, position_y, exact_position_x, exact_position_y)
)
plt.title("Difference between the analytical value\nand the value from the euler method")
plt.xlabel("time [s]") 
plt.ylabel("error [m]")
plt.show()
