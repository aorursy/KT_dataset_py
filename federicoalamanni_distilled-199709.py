...
#emiss*sigma*(Tp**4) = 2*emiss*sigma*(Ta**4) 
#Save= So/4 
#Tp = Save*(1-alpha)/((1 - 0.5*emiss)sigma)**1/4 
#T1 = (So/4/sigma*(1-alpha) / (1 - 0.5*emiss))**(1/4)

Tp=(Save*(1-alpha)/(1-0.5*emiss)/sigma)**(1/4)
print(Tp)

#emiss*sigma*(Tp**4) = 2*emiss*sigma*(Ta**4) 
#Ta= (emiss*sigma*(Save*(1-alpha)/(1-0.5*emiss)/sigma)/(2*emiss*sigma))**0.25
#Tp**4 = 2*(Ta**4)
Ta= Tp/2**(1/4)
print(Ta)
print('Temperatura atmosfera  : ' + str(round(Ta- 273.15, 3)) + '°C')
print('Temperatura del pianeta: ' + str(round(Tp- 273.15,3)) + '°C')
...
cbar = plt.colorbar();
cbar.set_ticks(list())
#plt.title(' Biomi primari nel 2020');
plt.title(' Biomi primari nel 2100');
...
cbar = plt.colorbar();
cbar.set_ticks(list())
#plt.title(' Biomi secondari nel 2020')
plt.title(' Biomi secondari nel 2100')
