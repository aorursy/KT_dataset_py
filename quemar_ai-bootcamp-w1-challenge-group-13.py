...
model = train(model,X,y,learning_rate=learning_rate,epochs=10000,print_loss=True)
def forward_prop_three(model,a0):
    '''
    Forward propagates through the model, stores results in cache.
    See: https://stats.stackexchange.com/questions/147954/neural-network-forward-propagation
    A0 is the activation at layer zero, it is the same as X
...
def backward_prop_three(model,cache,y):
    '''
    Backward propagates through the model to calculate gradients.
    Stores gradients in grads dictionary.
    See: https://en.wikipedia.org/wiki/Backpropagation
...
def initialize_parameters_three(nn_input_dim,nn_hdim1,nn_hdim2,nn_output_dim):
    '''
    Initializes weights with random number between -1 and 1
    Initializes bias with 0
    Assigns weights and parameters to model
...
def update_parameters_three(model,grads,learning_rate):
    '''
    Updates parameters accoarding to gradient descent algorithm
    See: https://en.wikipedia.org/wiki/Gradient_descent
    '''
...
def train_three(model,X_,y_,learning_rate, epochs=20000, print_loss=False):
    # Gradient descent. Loop over epochs
    for i in range(0, epochs):

        # Forward propagation
        cache = forward_prop_three(model,X_)
        #a1, probs = cache['a1'],cache['a2']
        # Backpropagation
        
        grads = backward_prop_three(model,cache,y_)
        # Gradient descent parameter update
        # Assign new parameters to the model
        model = update_parameters_three(model=model,grads=grads,learning_rate=learning_rate)
...
model_three = initialize_parameters_three(nn_input_dim=13, nn_hdim1= hlayer1_size,nn_hdim2 = hlayer2_size, nn_output_dim= 3)
model_three = train_three(model_three,X,y,learning_rate=0.01,epochs=10000,print_loss=True)