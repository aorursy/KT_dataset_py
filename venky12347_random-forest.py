...
cat_col=['campaign_type','brand_type', 'category', 

           'quantity','other_discount', 'coupon_discount','marital_status',

           'rented','income_bracket','age_range','brand']

num_col=x_data.select_dtypes(exclude=['object'])

one_hot=pd.get_dummies(x_data)

amex_df=pd.concat([one_hot,num_col],axis=1)
one_hot.info()
from sklearn.ensemble import RandomForestClassifier

cat_model =  RandomForestClassifier(n_estimators=5,criterion="gini",max_depth=6,

                                min_samples_split=2,min_samples_leaf=1)

cat_model.fit(amex_df,y_data)
clwght = {0:0.3,1:0.7}
...
print ("\nRandom Forest for Ensemble - Confusion Matrix\n\n",pd.crosstab(y_data,cat_model.predict(amex_df),rownames = ["Actual"],colnames = ["Predicted"]))      

print ("\nRandom Forest for Ensemble - accuracy",round(accuracy_score(y_data,cat_model.predict(amex_df)),3))

print ("\nRandom Forest for Ensemble - Classification Report\n",classification_report(y_data,cat_model.predict(amex_df)))
y_pred = cat_model.predict(amex_df)
...
amex_df = test_data_merge[columns]
...
num_col=amex_df.select_dtypes(exclude=['object'])
