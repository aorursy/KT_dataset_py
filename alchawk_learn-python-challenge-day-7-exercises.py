...
def prettify_graph(graph):
    ...
    new_labels = ['${}'.format(int(sign)) for sign in labels]
...
def blackjack_hand_greater_than(hand_1, hand_2):
    ...
    """
    def scores(hand):
        card_scores = {str(num):num for num in range(1, 11)}
        card_scores.update({"J":10, "Q":10, "K":10})

        total = 0
        hand_As = hand.count("A")
        for card in hand:
            if card not in ("A"):
                total += card_scores[card]
        for num in range(hand_As):
            if (total + 11) <= 21:
                total += 11
            else:
                total += 1
    ...
    return scores(hand_1) > scores(hand_2)
