...
print("歪度:%f"%train["SalePrice"].kurt())



#歪度が正の値→右裾が長い、左に偏っている

#尖度が高い→尖っている、平均付近に集中している
#物件の広さを合計した変数を作成
...
"2500以上の欠損血を持つデータについて、カラムを削除してみる"
...
#lotfrontageは　　　　　　すべてのデータのNeighborhoodごとのLotFrontageを    変形する　「「返す」」欠損血をｘのmedianで埋めて
...
#欠損血が存在、かつobjectのリストを作成

object_list=all_data[na_col_list].dtypes[all_data[na_col_list].dtypes=="object"].index.tolist()



all_data[float_list]=all_data[float_list].fillna(0)

all_data[object_list]=all_data[object_list].fillna("None")
...
flg,ax=plt.subplots(figsize=(12,9))
...
plt.show()
print("train",min(train_metric),"eval",min(eval_metric))
#特徴量ごとの重要度を可視化する
