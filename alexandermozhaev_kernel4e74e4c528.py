...
from keras.layers import Input, Dense, Conv2D, Reshape, Flatten, concatenate, UpSampling2D, BatchNormalization
...
seed_everything(43)
ROOT = '/kaggle/input/' # директория, в которую подгружаются файлы
...
if BadMemory:
    ...
    x = Reshape((8,8,32))(x)
    x = Conv2D(256, (3, 3), activation='softmax', padding='same')(x)
    x = BatchNormalization()(x)
    x = UpSampling2D((2, 2))(x)
    x = Conv2D(128, (3, 3), activation='elu', padding='same')(x)
    x = BatchNormalization()(x)
    x = UpSampling2D((2, 2))(x)
    x = Conv2D(64, (3, 3), activation='softmax', padding='same')(x)
    x = BatchNormalization()(x)
    x = UpSampling2D((2, 2))(x)
