...
def starts_and_ends_with_vowel(text):
    ...
    return text.startswith(vowels) and text.endswith(vowels)
...
def first_to_last(sequence):
    ...
    new_last = sequence.pop(0)

    sequence.append(new_last)
...
def has_evens(sequence):
    ...
    return any(num for num in sequence if num % 2 == 0)
...
def count_evens(sequence):
    ...
    return len([num for num in sequence if num % 2 == 0])
...
def has_odds(nums):
    ...
    return any(num for num in nums if num % 2 != 0)
...
assert get_values_in_common([1, 1], [2, 2, 3]) == set()
...
def get_paper_title(my_dict):

    return my_dict.get('title')
...
def get_latest_paper(my_dict1, my_dict2):

    a = my_dict1.get('year_published')

    b = my_dict2.get('year_published')
...
def get_price(my_dict):

    return my_dict.get("price")
...
def get_author(my_dict):

    return my_dict.get("author")
...
def total_of_book_prices(lst):
    ...
    return sum([book.get('price') for book in lst])
...
def get_average_book_price(lst):
    ...
    return statistics.mean([book.get('price') for book in lst])
...
def highest_price_book(lst):
    ...
    x = max([book.get("price") for book in lst])
...
def lowest_price_book(lst):
        ...
        x = min([book.get("price") for book in lst])
...
#     if max < item["price"]*item["quantity"]:
...
def number_of_item_types(my_dict):
...
# List Comprehension version (works in Spyder)



def total_number_of_items(my_dict):

    return sum(["quantity" for item in my_dict])
...
def get_average_item_price(my_dict):

    total = sum([item["price"] for item in my_dict["items"]])

    quant = len(my_dict["items"])
...
def get_average_spent_per_item(my_dict):

    total_price = sum([item["price"] * item["quantity"] for item in my_dict["items"]])

    total_quantity = sum([item["quantity"] for item in my_dict["items"]])
...
def most_spent_on_item(my_dict):

    max_price = max([item["price"] * item["quantity"] for item in my_dict["items"]])

    for item in my_dict["items"]:

        if item["price"] * item["quantity"] == max_price:

            return item

# attempt at list comprehension version

# def most_spent_on_item(my_dict):

#     max_price = max([item["price"] * item["quantity"] for item in my_dict["items"]])

#     return [item for item in my_dict["items"] if item["price"] * item["quantity"] == max_price]
