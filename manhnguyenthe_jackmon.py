...
for i in range(len(data_test.iloc[:,0])):
    try:
        image_data.append(image.img_to_array(image.load_img(dir1+data_test.iloc[:,0][i],target_size=(224,224))))
...
data_test.to_csv('result_Arceus.csv',index=False)
train_gen.class_indices
modelN.evaluate_generator(generator=val_gen, steps=STEP_SIZE_VALID)
