...
class my_gbdt(object):
    ...
    def __init__(self, max_depth, min_samples_split, lr, iterations):
    ...
    def fit(self, X, y):
        ...
        for i in range(1,self.iterations):
            ...
            self.trees[i].fit(X, (y-y_pred)*self.lr)
    ...
    def predict(self, X):
        ...
        for i in range(self.iterations):
            ...
            pred_proba += self.lr * self.trees[i].predict(X)

        return np.argmax(pred_proba, axis=1)
my_model = my_gbdt(max_depth = 6, min_samples_split = 10, lr = 0.5, iterations = 16)
