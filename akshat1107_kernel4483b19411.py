...
combine=pd.concat([train_data.iloc[:,:-1],test_data])
...
combine['Functional']=combine['Functional'].fillna("None")
...
combine['SaleType']=combine['SaleType'].fillna("None")
...
combine['KitchenQual']=combine['KitchenQual'].fillna("None")

combine['Electrical']=combine['Electrical'].fillna("None")
...
combine['Exterior2nd']=combine['Exterior2nd'].fillna("None")

combine['Exterior1st']=combine['Exterior1st'].fillna("None")
...
combine=pd.get_dummies(combine, columns=object_columns, drop_first=True)
...
#Getting dummies and dropping original columns
