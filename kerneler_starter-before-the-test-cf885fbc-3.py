...
# test.csv has 3589 rows in reality, but we are only loading/previewing the first 100 rows
...
nRowsRead = 100 # specify 'None' if want to read whole file
# train.csv has 28709 rows in reality, but we are only loading/previewing the first 100 rows
...
nRowsRead = 100 # specify 'None' if want to read whole file
# val.csv has 3589 rows in reality, but we are only loading/previewing the first 100 rows
df3 = pd.read_csv('../input/val.csv', delimiter=',', nrows = nRowsRead)
df3.dataframeName = 'val.csv'
nRow, nCol = df3.shape
print(f'There are {nRow} rows and {nCol} columns')
df3.head(5)
