...
class my_gbdt(object):
    ...
    def __init__(self):
        ...
        for i in range(self.iteration):
            ...
            self.tree.append(DecisionTreeRegressor(min_samples_split=4, max_depth=6))
    ...
    def fit(self, X, y):
        ...
        for i in range(1, self.iteration):
            ...
            grad = y - 1 / (1 + np.exp(pred_proba))

            self.tree[i].fit(X, grad)

            pred_proba += self.lr * self.tree[i].predict(X)
