...
pred = [1 if item =='P' else 0 for item in pred]



y = train['Polarity']

y = [0 if item=='N' else 1 for item in y]
...
for i, (train_i, test_i) in enumerate(cv): 
    ...
    probas_ = classifier.fit(test_features.toarray(), y[train_i]).predict_proba(test_features.toarray()[test_i])
...
print (pred_5)
cnf_matrix = cnf_matrix * 4

plot_confusion_matrix(cnf_matrix, classes=['P','N'],

                      title='Confusion matrix')
sns.countplot(pred_5)
print(len(pred_4))
print('prediction 1 accuracy: ', accuracy_score(test['Polarity'], pred_1))

print('prediction 2 accuracy: ', accuracy_score(test['Polarity'], pred_2))

print('prediction 3 accuracy: ', accuracy_score(test['Polarity'], pred_3))

print('prediction 4 accuracy: ', accuracy_score(test['Polarity'], pred_4))

print('prediction 5 accuracy: ', accuracy_score(test['Polarity'], pred_5))
bar = [0.827, 0.83,0.82,0.83]
...
x1 = np.arange(len(p_list))

x2 = [i+0.3 for i in x1]

plt.bar(x1,p_list,color='b', width=.3, label='P')

plt.bar(x2,n_list,color='r', width=.3, label='N')
...
test_features = vectorizer.transform(clean_test_reviews)
train['Polarity'].loc[train['Polarity']=="N"] = 0

train['Polarity'].loc[train['Polarity']=="P"] = 1

test['Polarity'].loc[test['Polarity']=="N"] = 0

test['Polarity'].loc[test['Polarity']=="P"] = 1
...
print('prediction 6 accuracy: ', accuracy_score(y_true, y_pred))    