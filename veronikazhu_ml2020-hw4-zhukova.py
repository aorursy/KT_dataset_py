...
#Общее Соединение df_clients, df_train, df_test
...
for i in range(18,60, 1):
    if max_uplf < (uplift_score(df_train_all[df_train_all.age>i]) - uplift_score(df_train_all[df_train_all.age<=i])):
        ...
        print(i)
#Признак 1 - 'Покупка на сумму свыше 700 рублей'
dop = df_pursh.groupby('client_id')['purchase_sum'].agg(lambda x: x.value_counts().idxmax()).to_frame()
df_train_all = pd.merge(df_train_all, dop, on='client_id', how='inner')

dop = df_pursh_test.groupby('client_id')['purchase_sum'].agg(lambda x: x.value_counts().idxmax()).to_frame()
df_test_all = pd.merge(df_test_all, dop, on='client_id', how='inner')

df_test_all['more_than_700'] = (df_test_all['purchase_sum'] >= 700).astype('int')
df_train_all['more_than_700'] = (df_train_all['purchase_sum'] >= 700).astype('int')

#Признак 2 - 'Мужчина старше 30 лет'
df_train_all['over_30'] = ((df_train_all['age'] >=30) & (df_train_all['gender']=='M')).astype('int')
df_test_all['over_30'] = ((df_test_all['age'] >=30) & (df_test_all['gender']=='M')).astype('int')
#Признак 3 - 'Женщина старше 60 лет'
df_train_all['over_60'] = ((df_train_all['age'] >=60) & (df_train_all['gender']=='F')).astype('int')
df_test_all['over_60'] = ((df_test_all['age'] >=60) & (df_test_all['gender']=='F')).astype('int')
#Признак 4 - 'Покупка была сделана в субботу' (УБЕРЕМ, НЕ ТАК ЗНАЧ)
#day_of_week = pd.to_datetime(df_pursh['transaction_datetime']).dt.dayofweek
#train_prep['saturday'] = (day_of_week == 5).astype('int')
#Признак 5 - 'Наличие популярного товара level 2'
most_popular_level2 = df_products['level_2'].value_counts().idxmax()
df_pursh['popular_level2'] = df_products['level_2'].apply(lambda x: int(x == most_popular_level2))
most_pop_level2 = df_pursh.groupby('client_id')['popular_level2'].sum()
df_train_all['level_2'] = df_train_all.index.map(most_pop_level2)

most_popular_level2 = df_products['level_2'].value_counts().idxmax()
df_pursh_test['popular_level2'] = df_products['level_2'].apply(lambda x: int(x == most_popular_level2))
most_pop_level2 = df_pursh_test.groupby('client_id')['popular_level2'].sum()
df_test_all['level_2'] = df_test_all.index.map(most_pop_level2)
df_new_features = df_train_all[['more_than_700', 'over_30', 'over_60', 'level_2']]
df_new_features_test = df_test_all[['more_than_700', 'over_30', 'over_60', 'level_2']]
