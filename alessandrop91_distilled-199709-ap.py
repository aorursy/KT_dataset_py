...
print(city + ', ' + country)
temperatures.shape
data = temperatures.loc[(temperatures.City == city)&(temperatures.Country==country)]
data.shape
data.head()
data = data.set_index('dt')
temp = data.AverageTemperature
#isola dalle colonne adiacenti solamente i valori presenti nella colonna "AvarageTemperature"
temp
#visualizza solo la colonna Avarage temperature in funzione del tempo dt; i gradi sono espressi Celsius.
plt.figure(figsize=(16,8));
plt.plot(temp);
plt.grid();
...
#con la seuguente immagine viene rappresentata la variazione di temperatura media nella città di León dal 1835 al 2013
