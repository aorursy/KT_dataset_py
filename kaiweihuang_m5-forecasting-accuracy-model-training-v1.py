...
def load_lag_features(df, version):



    # Set environment variables

    SALES_LAG_PATH = f"/kaggle/input/m5-forecasting-accuracy-sales-lag-features"



    # Get lag features

    df_lag_features = pd.read_pickle(f"{SALES_LAG_PATH}/m5-forecasting-accuracy-ver{version}/sales_lag_features.pkl")
    ...
    return df_features
df_fast_lag_training = load_lag_features(df_fast_basic_training, VERSION)
...
lag_model = make_fast_training(df_fast_lag_training, feature_list, NUMBER_OF_TRAIN, lgb_params)
...
lgb.plot_importance(lag_model, figsize = (18, 12))
...
for feature in feature_list:
    ...
    if df_temp[feature].dtypes.name != "category":
        ...
        df_temp["prediction"] = lag_model.predict(df_temp[feature_list])
...
for feature in feature_list:
    ...
    if df_fast_lag_training[feature].dtypes.name != "category":
...
df_fast_lag_training_pca = df_fast_lag_training[numeric_feature_list].fillna(0).replace([np.inf, -np.inf], 0)
# Run PCA
...
pca.fit(df_fast_lag_training_pca)

lag_after_pca = pca.transform(df_fast_lag_training_pca)
# Print PCA results
...
df_pca = pd.DataFrame(
    ...
    , columns = df_fast_lag_training_pca.columns
