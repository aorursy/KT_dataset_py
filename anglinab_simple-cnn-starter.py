...
# Get stats
dummy = ImageFolder(DATA_DIR, transform=tt())
mean = 0.0
std = 0.0
for img, _ in dummy:
    mean += img.mean([1,2])
...
df_stats = ([0.0792, 0.0529, 0.0544], [0.1288, 0.0884, 0.1374])
train_tf = tt.Compose([tt.RandomCrop(32, padding=4, padding_mode='reflect'), 
                         tt.RandomHorizontalFlip(), 
                         tt.ToTensor(), 
                         tt.Normalize(*df_stats,inplace=True)])
test_tf = tt.Compose([tt.ToTensor(), tt.Normalize(*df_stats)])
val_pct = 0.2
...
len(TRAIN_CSV)
train_ds = HumanProteinDataset(train_split_df, TRAIN_DIR, train_tf)
valid_ds = HumanProteinDataset(TRAIN_CSV, val_split_df, test_tf)
len(valid_ds)
