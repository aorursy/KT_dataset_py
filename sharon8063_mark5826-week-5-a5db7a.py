...
model.fit(X, falling)
model.coefficients(plot = True)
model.score(model.predict(X), falling)
# It can be seen that Egg monitoring rate has a strong connection with falling(100% Probability)
# And it seems falling also has a negative impact on sugar level(81% Probability)
# Falling also has a positive link with Blood circulation and monitoring time
# However, the accuracy rate of the model is only 57% and the score is only 0.13
#so we use the Neural Network for a better result
model = LinearModel(logistic = True,layers = 1)
model.fit(X, falling)
model.analyse(plot = True)
model.plot(model.predict(X), falling)
model.score(model.predict(X), falling)
model = LinearModel(logistic = True,layers = 2)
model.fit(X, falling)
model.analyse(plot = True)
model.plot(model.predict(X), falling)
model.score(model.predict(X), falling)
# While it turns out when layer is one the accuracy rate is highest, which is 60%,
# the score is 0.25<0.3, and it is still not good enough