...
with tf.Session() as sess:
    ...
    result = sess.run(f, feed_dict={x:1, y:4})
print(result)
...
trainY = f(trainX) + noise_level * np.random.randn(N)
...
class LinearRegressionTF(BaseEstimator):
    ...
    def fit(self, trainX,trainY):
        ...
        self.pred = tf.add(tf.multiply(self.W, self.X), self.b)

        

        # mean squre error

        cost = tf.reduce_sum(tf.pow(self.Y - self.pred, 2)) / (2 * N)
        ...
        if self.annotate:
            ...
            plt.plot(trainX, self.sess.run(self.pred, feed_dict={self.X:trainX}), label='Fitted line')
...
lr = LinearRegressionTF(learning_rate, training_epochs, display_step, annotate=False)
    ...
    def fit(self, trainX,trainY):
        ...
        for epoch in range(self.training_epochs):
            ...
            if (epoch+1) % display_step == 0:
                ...
                c = self.sess.run(cost, feed_dict={self.X:np.asmatrix(trainX), self.Y:np.asmatrix(trainY)})
...
logisticTF = LogisticRegressionTF(learning_rate, training_epochs, display_step, annotate=False)
...
pred= tf.nn.top_k(-distance, KVALUE)
...
class NaiveBayesTF(BaseEstimator):
    ...
    def fit(self, trainX, trainY):
        ...
        points_by_class = np.array([
            ...
            [x for x, y in zip(trainX, trainY) if y == c] for c in unique_classes

        ])

        

        input_x = tf.placeholder(tf.float64, points_by_class.shape)
    ...
    def predict(self, testX):
        ...
        cond_probs = tf.reduce_sum(
            ...
            self.dist.log_prob(np.reshape(np.tile(testX, [1, num_classes]), [-1, num_classes, num_features])), 
        ...
        priors = tf.log(np.array([1./num_classes] * num_classes))
