...
mean_ratings = {key: sum(item_ratings[key]) / len(item_ratings[key]) for key in item_ratings.keys() }



# Find the worst item, and write it as a string in worst_text. This can be multiple lines of code if you want.

worst_item = str([key for key in mean_ratings.keys() if mean_ratings[key] == min(mean_ratings.values())][0])
