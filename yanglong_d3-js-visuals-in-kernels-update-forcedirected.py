...
##############################
# fourth 
doc = {"name": "Audience Demographics", "color": "#ffae00", "percent": "", "value": "", "size": 25, "children": []}
...
def vcs(col):
    ...
    ddoc = {"name": col, "color": "#93ff59", "percent": "", "value": "", "size": 25, "children": []}
    for i,x in enumerate(keys):
        percent = round(100 * float(vals[i]) / sum(vals), 2)
        size = getsize(percent)
        doc = {"name": x+" ("+str(percent)+"%)", "color": "#70ccea", "percent": str(percent), "value": str(vals[i]), "size": size, "children": []}
...
doc['children'].append(vcs('Employment'))
...
js_d1="""require(["d3"], function(d3) {
var treeData = """ +json.dumps(doc) + """
var root,margin={top:20,right:90,bottom:50,left:90},width=960-margin.left-margin.right,height=900-margin.top-margin.bottom,svg=d3.select("#four").attr("width",width+margin.right+margin.left).attr("height",height+margin.top+margin.bottom).append("g").attr("transform","translate("+margin.left+","+margin.top+")"),i=0,duration=750,treemap=d3.tree().size([height,width]);function collapse(t){t.children&&(t._children=t.children,t._children.forEach(collapse),t.children=null)}function update(n){var t=treemap(root),r=t.descendants(),e=t.descendants().slice(1);r.forEach(function(t){t.y=180*t.depth});var a=svg.selectAll("g.node").data(r,function(t){return t.id||(t.id=++i)}),o=a.enter().append("g").attr("class","node").attr("transform",function(t){return"translate("+n.y0+","+n.x0+")"}).on("click",function(t){t.children?(t._children=t.children,t.children=null):(t.children=t._children,t._children=null);update(t)});o.append("circle").attr("class","node").attr("r",function(t){return t.data.size}).style("fill",function(t){return t.data.color}),o.append("text").attr("dy",".35em").attr("x",function(t){return t.children||t._children?-13:13}).attr("text-anchor",function(t){return t.children||t._children?"end":"start"}).text(function(t){return t.data.name});var c=o.merge(a);c.transition().duration(duration).attr("transform",function(t){return"translate("+t.y+","+t.x+")"}),c.select("circle.node").attr("r",function(t){return t.data.size}).style("fill",function(t){return t.data.color}).attr("cursor","pointer");var l=a.exit().transition().duration(duration).attr("transform",function(t){return"translate("+n.y+","+n.x+")"}).remove();l.select("circle").attr("r",function(t){return t.data.size}),l.select("text").style("fill-opacity",1e-6);var d=svg.selectAll("path.link").data(e,function(t){return t.id});console.log(),d.enter().insert("path","g").attr("class","link").attr("d",function(t){var r={x:n.x0,y:n.y0};return u(r,r)}).merge(d).transition().duration(duration).attr("d",function(t){return u(t,t.parent)});d.exit().transition().duration(duration).attr("d",function(t){var r={x:n.x,y:n.y};return u(r,r)}).remove();function u(t,r){var n="M"+t.y+","+t.x+"C"+(t.y+r.y)/2+","+t.x+" "+(t.y+r.y)/2+","+r.x+" "+r.y+","+r.x;return console.log(n),n}r.forEach(function(t){t.x0=t.x,t.y0=t.y})}(root=d3.hierarchy(treeData,function(t){return t.children})).x0=height/2,root.y0=0,root.children.forEach(collapse),update(root);});
"""
...
demographics = ['Gender', 'Age', 'SexualOrientation', 'RaceEthnicity', 'EducationParents', 'Dependents', 'MilitaryUS']
...
.link {
    ...
    <body>
    <br><br>
    <svg id="five" width="960" height="1300"></svg>
</body>
"""
...
############## assbenefits 
# nodes.append({'id' : 'AssessBenefits', 'group' : 0, 'size' : 15})
# links.append({"source": 'Questions', "target": "AssessBenefits", "value": 1})
# AssessBenefits = ["AssessBenefits"+str(k) for k in range(1,12)]
# for i, col in enumerate(AssessBenefits):
#     nodes.append({'id' : col, 'group' : i+1, "size": 12})
#     t = df[col].dropna().value_counts()
#     x = list(t.index) 
#     y = list(t.values)
#     links.append({"source": 'AssessBenefits', "target": col, "value": 1})
    
#     for j, each in enumerate(x):
#         nodes.append({'id' : col+"-"+str(each).replace(".0",""), 'group' : i+1, "size" : get_size(y[j])}) 
#         links.append({"source": col, "target": col+"-"+str(each).replace(".0",""), "value": 1})
