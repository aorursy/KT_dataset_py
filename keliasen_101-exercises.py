...
for veggie in vegetables:
    ...
    fruits_and_veggies.append(veggie)
...
def is_positive(nbr):
    ...
    return False if nbr < 0 else True
...
def is_odd(nbr):
    ...
    return True if nbr % 2 == 1 else False
...
def is_positive_odd(nbr):

    return is_positive(nbr) and is_odd(nbr)
...
def is_positive_even(nbr):

    return is_positive(nbr) and is_even(nbr)
...
def is_negative_odd(nbr):

    return is_negative(nbr) and is_odd(nbr)
...
def is_negative_even(nbr):

    return is_negative(nbr) and is_even(nbr)
...
def is_multiple_of_three(nbr):

    return True if nbr % 3 == 0 else False
...
def is_multiple_of_five(nbr):

    return True if nbr % 5 == 0 else False
...
def is_multiple_of_both_three_and_five(nbr):

    return is_multiple_of_three(nbr) and is_multiple_of_five(nbr)
...
def remove_vowels(ltrs):
    ...
    for ltr in ltrs:
        ...
        newltr = '' if is_vowel(ltr) else ltr
...
def starts_with_vowel(ltrs):

    return True if is_vowel(ltrs[0]) else False
...
def ends_with_vowel(ltrs):

    return True if is_vowel(ltrs[-1]) else False
...
def starts_and_ends_with_vowel(ltrs):

    return starts_with_vowel(ltrs) and ends_with_vowel(ltrs)
...
def first_and_last(lst):
    ...
    rlst.append(first(lst))

    rlst.append(last(lst))
...
def median(nbrs):
    ...
    n2 = (n1 - 1) if get2 else n1

    return (nbrs[n1] + nbrs[n2]) / 2
...
def only_odd_numbers(nbrs):

    return [nbr for nbr in nbrs if is_odd(nbr)]
...
def only_even_numbers(nbrs):

    return [nbr for nbr in nbrs if is_even(nbr)]
...
def only_positive_numbers(nbrs):

    return [nbr for nbr in nbrs if is_positive(nbr)]
...
def only_negative_numbers(nbrs):

    return [nbr for nbr in nbrs if is_negative(nbr)]
...
def count_evens(nbrs):

    return len(only_even_numbers(nbrs)) 
...
def count_odds(nbrs):
    ...
    return len(only_odd_numbers(nbrs)) 
...
def count_negatives(nbrs):

    return len(only_negative_numbers(nbrs)) 
...
def count_positives(nbrs):

    return len(only_positive_numbers(nbrs)) 
...
def only_positive_evens(nbrs):

    return only_positive_numbers(only_even_numbers(nbrs))
...
def only_positive_odds(nbrs):

    return only_positive_numbers(only_odd_numbers(nbrs))
...
def only_negative_evens(nbrs):

    return only_negative_numbers(only_even_numbers(nbrs))
...
def only_negative_odds(nbrs):

    return only_negative_numbers(only_odd_numbers(nbrs))
...
def get_total_spend(lst):
    ...
    for itm in lst['items']:
        ...
        #print(itm['title'], itm['price'], itm['quantity'], itm_spend)
