...
preds_B0 = model_B0.predict(ds_testAug, verbose=1, steps=steps)
#CFG['batch_size'] = 256
...
#preds_B1 = model_B1.predict(ds_testAug, verbose=1, steps=steps)
#CFG['batch_size'] = 256
...
#preds_B2 = model_B2.predict(ds_testAug, verbose=1, steps=steps)
CFG['batch_size'] = 256
...
preds_B3 = model_B3.predict(ds_testAug, verbose=1, steps=steps)
#CFG['batch_size'] = 256
...
#preds_B4 = model_B4.predict(ds_testAug, verbose=1, steps=steps)
CFG['batch_size'] = 256
...
preds_B0 = preds.reshape(-1)
preds = np.stack(preds_B1)
...
preds_B1 = preds.reshape(-1)
#preds = np.stack(preds_B2)
...
#preds_B2 = preds.reshape(-1)
preds = np.stack(preds_B3)
...
preds_B3 = preds.reshape(-1)
#preds = np.stack(preds_B4)
...
preds =preds_B0*0.2 + preds_B3*0.3 + preds_B5*0.2 + preds_B6*0.3
