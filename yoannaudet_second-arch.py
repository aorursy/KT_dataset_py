...
nn_model.add(ks.layers.Dense(50, activation="selu", name="Dense05"))
nn_model.add(ks.layers.Dense(50, activation="selu", name="Dense06"))
nn_model.add(ks.layers.Dense(50, activation="selu", name="Dense07"))
nn_model.add(ks.layers.Dense(50, activation="selu", name="Dense08"))
nn_model.add(ks.layers.Dense(1, activation=None, name="output"))
opt = ks.optimizers.Adam(lr=0.001, decay=5e-4)
nn_model.compile(optimizer=opt, loss=mse, metrics=[rmse]) #mean_squared_logarithmic_error
X_train = np.asarray(raw_data)
Y_train = np.asarray(red_shift)
results = nn_model.fit(X_train, Y_train, batch_size=32, epochs=1000)
nn_model.summary()

pd.DataFrame(results.history).plot(figsize=(8, 5))
plt.grid(True)
plt.yscale("log")
pd.DataFrame(results2.history).plot(figsize=(8, 5))
plt.grid(True)
plt.yscale("log")
