...
def RFR_model_evaluation(estimatorsL):
    ...
    plt.show()
estimatorsL = list(np.arange(70, 200, 20))

RFR_model_evaluation(estimatorsL)
model_forest = RandomForestRegressor(n_estimators=130, n_jobs=-1)
...
y_fit = model_forest.predict(X_test)



plt.plot(np.mean((y[0: 1459]-y_fit)**2));