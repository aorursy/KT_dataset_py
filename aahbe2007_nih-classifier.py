...
sgd= keras.optimizers.Adam(lr=1e-4)
...
output_weights_name='weightsanothermodel70.h5'
...
Res_model.load_weights('../input/nih-classifier/weightsanothermodel55.h5')
'''

for ID in range(32):

    qq = Res_model.predict(aa)
    ...
    p = np.where(oo<0.75, 0, oo)

    print('Pred = ', oo)

    print('Pred = ', np.around(p).astype(int))
