...
# def my_geocoder(row, lat, long):

#     if (not (pd.isna(lat) or pd.isna(long))):
...
# starbucks[['Latitude', 'Longitude']] = starbucks.apply(lambda x: my_geocoder(x['Address'],x['Latitude'],x['Longitude']), axis=1)
...
#CA_counties, CA_pop, CA_high_earners, CA_median_age
...
#sel_counties = CA_stats.loc[((CA_stats.high_earners > 100000)&(CA_stats.median_age<38.5)&(CA_stats.density>285)) & ((CA_stats.high_earners > 500000)|(CA_stats.median_age<35.5)|(CA_stats.density>1400))].copy()
...
stores = gpd.sjoin(starbucks_gdf, sel_counties)
