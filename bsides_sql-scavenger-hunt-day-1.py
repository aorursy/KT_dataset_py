...
countries.to_csv("countries_no_ppm.csv")

query2="""SELECT DISTINCT pollutant FROM `bigquery-public-data.openaq.global_air_quality` WHERE value= 0 """
zero_pollutants = open_aq.query_to_pandas_safe(query2)

zero_pollutants.to_csv("zero_pollutant.csv")
