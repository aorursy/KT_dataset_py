...
titanic_df['Bc'] = \
    titanic_df[['Child','BigFamily']].apply( get_Bc, axis=1 )
test_df['Bc'] = \
    test_df[['Child','BigFamily']].apply( get_Bc, axis=1 )
...
titanic_df.drop(['SibSp','ParchBinary','Parch','MasterMiss','FamilySize','SmallFamily','S','C','Fc'],axis=1,inplace=True)
test_df.drop(['SibSp','ParchBinary','Parch','MasterMiss','FamilySize','SmallFamily','S','C','Fc'],axis=1,inplace=True)
...
Y_pred_knn = pd.Series(knn.predict(X_train),name='k')
...
Y_test_knn = pd.Series(knn.predict(X_test),name='k')
Y_test_ada = pd.Series(ada.predict(X_test),name='g')


sec_deg_train=pd.concat([Y_pred_logreg,Y_pred_rf,Y_pred_svc,Y_pred_extree,Y_pred_knn,Y_pred_gb],axis=1)
sec_deg_test=pd.concat([Y_test_logreg,Y_test_rf,Y_test_svc,Y_test_extree,Y_test_knn,Y_test_gb],axis=1)
...
random_search = RandomizedSearchCV(clf, param_distributions=param_dist,
                                   n_iter=n_iter_search)
rf=random_search
vcr=VotingClassifier(estimators=[('lg',logreg),('rf',rf)],voting='soft')
