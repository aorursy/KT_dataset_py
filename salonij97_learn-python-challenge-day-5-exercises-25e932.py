...
def elementwise_greater_than(L, thresh):
    ...
    bool_list = []
    for element in L:
        bool_list.append(element > thresh)
    return bool_list
...
def estimate_average_slot_payout(n_runs):
    ...
    for i in range(n_runs):
        payout = payout + play_slot_machine()
    return (payout - n_runs)/n_runs
q4.solution()
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    for j in range(n_simulations):
        ...
        for i in range(n_spins):
            if(balance < 1):
                ran_out_of_money = ran_out_of_money + 1
                break
            balance = balance + play_slot_machine() - 1
    return 1 - ran_out_of_money/n_simulations
