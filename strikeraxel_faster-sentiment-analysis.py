...
def generate_bigram(inp):
    bi_grams = set(zip(*[inp[i:] for i in range(2)]))
    for bi_gram in bi_grams:
        inp.append(" ".join(bi_gram))
    return inp        
a = ["I", "am", "Batman"]
generate_bigram(a)
# Download Train and test datasets
TEXT = data.Field(tokenize='spacy', preprocessing=generate_bigram)
...
class FastText(nn.Module):
    def __init__(self, vocab_dim, embed_dim, pad_inx , out_dim):
        super().__init__()
        self.embedding = nn.Embedding(vocab_dim, embed_dim, padding_idx=pad_inx)
        self.fc = nn.Linear(embed_dim, out_dim)
        
    def forward(self, txt):
        embeds = self.embedding(txt)
        # size = (src_len, batch_size, embed_size)
        embeds = embeds.permute(1,0,2)
        # size = (batch_size, src_len, embed_size)
        avg = F.avg_pool2d(embeds, (embeds.shape[1], 1)).squeeze(1)
        probs = self.fc(avg)
        return probs
EMBED_SIZE = 100
...
model = FastText(VOCAB_SIZE, EMBED_SIZE, PAD_IDX, OUT_DIM)
...
model.embedding.weight.data[UNK_IDX] = torch.zeros(EMBED_SIZE)
