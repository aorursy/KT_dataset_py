...
model_lasso = LassoCV(alphas = [1, 0.0004360, 0.01, 0.005,0.0005] , max_iter=50000).fit(X_train, y)
...
svm_preds = np.expm1(clf.predict(X_test))
# intrude 4
...
rfr_preds = np.expm1(rfr.predict(X_test))
# intrude 5
...
dtr_preds = np.expm1(dtr.predict(X_test))
...
model_xgb = xgb.XGBRegressor(n_estimators=480, max_depth=2, learning_rate=0.1) #the params were tuned using xgb.cv

model_xgb.fit(X_train, y)

xgb_preds = np.expm1(model_xgb.predict(X_test))

outs = [lasso_preds, ridge_preds, svm_preds, rfr_preds, dtr_preds,xgb_preds]
...
#print( lasso_preds.shape, ridge_preds.shape, svm_preds.shape)

#print(lasso_preds[:10])

# lasso, ridge and svm giving better output than random_forest and decisionTree
final = 0.40*xgb_preds + 0.30*ridge_preds + 0.30*lasso_preds



solution = pd.DataFrame({"id":test.Id, "SalePrice":final})

solution.to_csv("lasso_ridge_svm_sol.csv", index = False)


model_lasso = Lasso(alpha=5e-4, max_iter=50000).fit(X_train, y)

#model_lasso = Lasso(alpha = 0.0004436, max_iter=50000).fit(X_train, y)
