...
plt.imshow(data[4501][:,:,0],cmap='gray')
...
for i in range(5):
    for j in range(6):
      ax[i,j].imshow(cv2.cvtColor(data[(i*1000)+j], cv2.COLOR_BGR2RGB))
      ax[i,j].axis('off')
      ax[i,j].set_title(le.inverse_transform(data_encod[(i*1000)+j]))
plt.show()
...
test_image=image.load_img('../input/test_set/test_set/dogs/dog.4005.jpg',target_size=(64,64))
