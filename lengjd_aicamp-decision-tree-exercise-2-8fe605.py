...
#data = pd.read_csv(os.path.join("your_data_folder", "titanic.csv"), sep=",")
#data = pd.read_csv(os.path.join("/Users/jiadileng/Desktop/machine learning/jiuzhang/week4/input", "titanic.csv"), sep=",")
...
# TODO 打印部分名字信息
data['name'].head()
#先按“，”分解，分解完了按“ ”分解，这样可以提取出mr,miss之类的，看看上面的每个人名字具体长什么样子
...
# TODO 名字长度与幸存的关系
data['survived'].groupby(pd.qcut(data['name_len'],5)).mean() #分成5个区间
# TODO 打印性别比例
data['sex'].value_counts(normalize=True)
# TODO 性别与幸存的关系
data['survived'].groupby(data['sex']).mean() #male 与 Mr 存活率一样
# TODO 年龄与幸存的关系
...
# TODO 可视化登船地点与舱位的关系
sns.countplot(data['embarked'], hue=data['pclass']) #Cherbourg（法国小镇，那会儿法国很强）登船的1等舱人多
...
#one-hot encoding, not dummies as what teacher Hou taught in class
...
# TODO 创建深度为3，叶子节点数不超过5的决策树 #criterion='entropy'--以熵增益为标准取特征
