...
def prettify_graph(graph):
    ...
    labels = [f"{amount} $" for amount in ticks]
    ...
    def calculate_hand_value(hand):
        ...
        for card in hand:
            if card.isdigit():
                ...
                total_value += 10
        aces = hand.count('A')
        while total_value <= (21 - 11) and aces > 0:
            total_value += 11
            aces -= 1
        total_value += aces
        return total_value
    
    hand_1_val = calculate_hand_value(hand_1)
    hand_2_val = calculate_hand_value(hand_2)
    return hand_1_val <= 21 and (hand_1_val > hand_2_val or hand_2_val > 21)
