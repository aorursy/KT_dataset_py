...
def get_boxes(boxes, labels, thresh):
   ...
   # boxes.append({"position": {"middle": basis, "width": size_1, "height": size_2},

   #                   "meta": {"label": cat_to_name[cat],
...
category_dict = {"person": 0, "bicycle": 1, "car": 2, "motorbike": 3, "aeroplane": 4,
                 ...
                 "bus" : 5, "train": 6, "truck" : 7, "boat":8, "tree": 9, "traffic light": 10}
...
def draw_boxes(filename, v_boxes, v_labels, v_scores):
    ...
    for i in range(len(v_boxes)):
        ...
        bboxes.append({"position": {"middle": (x1 + math.floor(width/2.0), y1 + math.floor(height/2.0)), "width": width, "height": height},

                       "meta": {"label": v_labels[i],

                                "category" : category_dict[v_labels[i]],
    ...
    return wandb.Image(
        ...
        metadata=wandb.Metadata({
            ...
            "type": "bounding_boxes/beta",

            "boxes": bboxes

        }))

    #pyplot.savefig("car.jpg")
...
os.mkdir("lyft_samples")



for i, img in enumerate(images):

    photo_filename = DATA_PATH + 'train_images/' + img

    loaded = load_img(photo_filename)

    save_img("lyft_samples/lyft_sample_"+str(i)+".jpg", loaded)    
import os
...
def load_image_pixels(filename, shape):
    ...
    return image, width, height
!pip install --upgrade 'git+git://github.com/wandb/client.git@feature/image-metadata#egg=wandb' -qq
import wandb

wandb.init(project="yolo-drive", entity="stacey")
wandb.init(project="yolo-drive", entity="stacey")
...
for file in images[:10]:
    ...
    labels = ["person", "bicycle", "car", "motorbike", "aeroplane", "bus", "train", "truck","boat", "tree", "traffic light"]
    ...
    wb_image = draw_boxes(photo_filename, v_boxes, v_labels, v_scores)

    logged_boxes.append(wb_image)

    

for i, b in enumerate(logged_boxes):

    wandb.log({"box_" + str(i) : logged_boxes[i]})