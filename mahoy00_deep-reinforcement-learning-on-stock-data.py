...
data.head()
data.info()
date_split = '2016-01-01' #以该日期为界限划分train and test
...
class Environment1:
    ...
    def step(self, act):
        ...
        # act = 0: stay, 1: buy, 2: sell act的值决定了每一个step的行为
        if act == 1:
...
def train_dqn(env):
    ...
    optimizer = chainer.optimizers.Adam()#用Adam定义优化器
    optimizer.setup(Q)
