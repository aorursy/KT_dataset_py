...
def cosine_annealing(epoch):
    ...
    if epoch <= 4 :
        ...
        return 0.5 * 0.001 * (np.cos(np.pi * epoch / 4.0) + 1)

    elif epoch <= 11:

        return 0.5 * 0.001 * (np.cos(np.pi * (epoch - 4.0) / 7.0) + 1)

    elif epoch <= 20:

        return 0.5 * 0.001 * (np.cos(np.pi * (epoch - 11.0) / 9.0) + 1)



y = [exponential_decay(x) for x in rng]
...
y = [cosine_annealing(x) for x in rng]
...
exp_schedule = tf.keras.callbacks.LearningRateScheduler(exponential_decay, verbose=1)

cos_schedule = tf.keras.callbacks.LearningRateScheduler(cosine_annealing, verbose = 1)
