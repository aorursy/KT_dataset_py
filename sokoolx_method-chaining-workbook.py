...
chess_games.head()
check_q1(chess_games.winner.value_counts()/len(chess_games.winner))
...
check_q8(ramen_reviews.Stars.replace('Unrated', None).dropna().astype('float64').map(lambda v: round(v * 2) / 2).value_counts().rename_axis("Ramen Reviews").sort_index())