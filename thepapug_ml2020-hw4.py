...
#DATA_PATH = '/kaggle/input/x5-uplift-valid/data/test.csv'
...
save_submission(indices_test,test_pred,'submission.csv')
print('Submission is ready')
df = pd.read_csv('../../kaggle/input/x5-uplift-valid/data/train.csv', sep=',', encoding='cp1251')
train_purchases = pd.read_csv('../../kaggle/input/x5-uplift-valid/train_purch/train_purch.csv', sep=',', encoding='cp1251')
...
test_purchases = pd.read_csv('../../kaggle/input/x5-uplift-valid/test_purch/test_purch.csv', sep=',', encoding='cp1251')
train_purchases_merged = pd.merge(train_purchases, products, on='product_id', how='inner')
test_purchases_merged = pd.merge(test_purchases, products, on='product_id', how='inner')
train_feature = df.copy()
netto = train_purchases_merged.groupby('client_id')['netto'].sum()
...
netto = test_purchases_merged.groupby('client_id')['netto'].sum()
test['netto'] = test['client_id'].map(netto)
thrsh = test['netto'].median()
test['netto_57'] = test['netto'].apply(lambda x: int(x > thrsh))
exp_points_spent = train_purchases_merged.groupby('client_id')['express_points_spent'].sum()
...
exp_points_spent = test_purchases_merged.groupby('client_id')['express_points_spent'].sum()
test['exp_points_spent'] = test['client_id'].map(exp_points_spent)
test['exp_points_spent_not_null'] = test['exp_points_spent'].apply(lambda x: int(x != 0))
train_purchases_merged['transaction_datetime'] = pd.to_datetime(train_purchases_merged['transaction_datetime'])
train_purchases_merged['hour'] = train_purchases_merged['transaction_datetime'].apply(lambda x: x.hour)
test_purchases_merged['transaction_datetime'] = pd.to_datetime(test_purchases_merged['transaction_datetime'])
test_purchases_merged['hour'] = test_purchases_merged['transaction_datetime'].apply(lambda x: x.hour)
hour = train_purchases_merged.groupby('client_id')['hour'].mean()
train_feature['hour'] = train_feature['client_id'].map(hour)
train_feature['day'] = train_feature['hour'].apply(lambda x: int(x > 12 and x <18))
hour = test_purchases_merged.groupby('client_id')['hour'].mean()
test['hour'] = test['client_id'].map(hour)
test['day'] = test['hour'].apply(lambda x: int(x > 12 and x <18))
train_feature['morning'] = train_feature['hour'].apply(lambda x: int(x <= 12))
test['morning'] = test['hour'].apply(lambda x: int(x <= 12))
most_popular_vendor_id = products['vendor_id'].value_counts().idxmax()
train_purchases_merged['most_pop_vend'] = train_purchases_merged['vendor_id'].apply(lambda x: int(x == most_popular_vendor_id))
most_pop_vend = train_purchases_merged.groupby('client_id')['most_pop_vend'].sum()
...
most_popular_vendor_id = products['vendor_id'].value_counts().idxmax()
test_purchases_merged['most_pop_vend'] = test_purchases_merged['vendor_id'].apply(lambda x: int(x == most_popular_vendor_id))
most_pop_vend = test_purchases_merged.groupby('client_id')['most_pop_vend'].sum()
test['vendor_count'] = test['client_id'].map(most_pop_vend)
test['no_vendor'] = test['vendor_count'].apply(lambda x: int(x == 0))
cols = ['netto_57','exp_points_spent_not_null','day', 'morning', 'no_vendor', 'client_id']
train_prep = train_feature[cols]
test_prep = test[cols]
X_learn.head()
X_learn_extended = pd.merge(X_learn, train_prep, on='client_id', how='inner')
X_valid_extended = pd.merge(X_valid, train_prep, on='client_id', how='inner')
X_test_extended = pd.merge(X_test, test_prep, on='client_id', how='inner')
X_learn_extended = X_learn_extended.drop(columns=['client_id'], axis=1)
X_valid_extended = X_valid_extended.drop(columns=['client_id'], axis=1)
X_test_extended = X_test_extended.drop(columns=['client_id'], axis=1)
print('Build model for learn data set...')
clf = uplift_fit(clf_, X_learn_extended, treatment_learn, target_learn)
print('Model is ready')
learn_pred = uplift_predict(clf, X_learn_extended)
learn_scores = uplift_metrics(learn_pred, treatment_learn, target_learn)
print(f'Learn scores: {learn_scores}')
valid_pred = uplift_predict(clf, X_valid_extended)
valid_scores = uplift_metrics(valid_pred, treatment_valid, target_valid)
print(f'Valid scores: {valid_scores}')
test_pred_ext = uplift_predict(clf, X_test_extended, z = True)
save_submission(indices_test,test_pred_ext,'submission_extended.csv')