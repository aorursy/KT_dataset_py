...
def build_feat_map(data):
    ...
    return feat_map
#print(np.vstack([train_feat, test_feat]))
...
print('gbdt train label: ', gbdt_train_label[:5])
print(np.shape(gbdt_train_label))
...
class my_gbdt(object):
    ...
    def fit(self, X, y, n_estimators, lr, max_depth, min_samples_split, subsample=None):
        ...
        for i in range(n_estimators):
            ...
            residuals = self._get_residuals(y, y_hat)
            print(i,"round residual:",np.sum(residuals))
    ...
    def predict(self,X):
        ...
        sum =[self.init_val] * 179
        for tree in self.trees:
            sum += self.lr * tree.predict(X) 
        return [(a >= 0.5) for a in sum]
        #pred_proba = self.tree.predict(X)
        #return np.argmax(pred_proba, axis=1)
my_model = my_gbdt()
gbdt_train_label_1 = []
for label in gbdt_train_label:
    gbdt_train_label_1.append(label[1])
#gbdt_train_label = [label[1] for label in gbdt_train_label]
my_model.fit(gbdt_train_data, gbdt_train_label_1,n_estimators=4,
            lr=0.7, max_depth=5, min_samples_split=2)
