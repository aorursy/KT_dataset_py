...
sns.barplot(x=gender_count.index, y= gender_count.values, color=color[3])
...
print("Number of countries where the surveys count was more than the average survey count: ", end=" ")
...
plt.show()
# For the sake of a good plot, I will be including all those countries from the graph

# where the number of surveys is more than mean i.e 315

country_count_ex = country_count[country_count.values > 315]

plt.figure(figsize=(20,20))

sns.barplot(y=country_count_ex.index, x= country_count_ex.values, color=color[2], orient='h')
...
def split_values(x, samples_dict):
    ...
    # Split values based on comma, just don't split the values with comma inside a parantheses
