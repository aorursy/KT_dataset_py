...
diff_title = df[df['original_title'] != df['title']][['title', 'original_title']]
...
plt.show()
df["production_countries"].shape[0]
...
plt.show()
plt.pie(lang_df.iloc[1:11]["number"],labels = lang_df.iloc[1:11]["language"],autopct='%1.1f%%', radius = 3, shadow = True, counterclock = True)
...
gross_top = df[['poster_path', 'title', 'budget', 'revenue', 'year']].sort_values('revenue', ascending=False).head(5)
