...
train = sales.join(items, on='item_id', rsuffix='_')\
    ...
    .join(cats, on='item_category_id', rsuffix='_').drop(['item_id_', 'shop_id_', 'item_category_id_'], axis=1)
train.head()
train.info()
...
print('Data set size is reduced by: ', (train.shape[0] - lk_train.shape[0])/train.shape[0] * 100, '%')
find_NA(lk_train)
lk_train = downcast_dtypes(lk_train)
print(lk_train.info())
lk_train.columns
...
#lk_train['date'] = pd.to_datetime(lk_train['date'], format="%d.%m.%Y")
del train
del shops
del items
del cats
...
X_train.info()
# col_list = ['shop_id', 'item_id','item_price','mean_item_price','item_cnt','mean_item_cnt','transactions',
#            'type_code','subtype_code','city_code','item_cnt_min','item_cnt_max','item_cnt_mean','item_cnt_std','item_price_unit',
#             'hist_min_item_price','hist_max_item_price','price_decrease','item_cnt_shifted1','item_cnt_shifted2','item_cnt_shifted3',
#             'item_trend','year','month','shop_mean','item_mean','shop_item_mean','year_mean','month_mean']
# X_train = X_train[col_list]
# X_validation = X_validation[col_list]
