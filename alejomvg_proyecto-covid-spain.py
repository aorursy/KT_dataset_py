...
sf = gpd.read_file('../input/shpspain/ESP_adm1.shp') #Regions Geometry Map 
...
def anotate_values(ax,df,var_name,f_type=lambda x: x,decimals = 2):
    ...
    """
    try:
        df.apply(lambda x: ax.annotate(s=f_type(round(x[var_name], decimals)), xy=x.geometry.centroid.coords[0], ha='left',color="Black"),axis=1)
        return ax
    except: 
        pass

def anotate_values2(ax,df,var_name,f_type=lambda x: x,decimals = 2):
    try:
        df.apply(lambda x: ax.annotate(s=x[var_name] + ' ', xy=x.geometry.centroid.coords[0], ha='right',color="Black"),axis=1)
        return ax
    except: 
        pass
...
def create_evolution(data,ax):
    ...
    return ax
def plot_col_map(df,col_name, index, f_type=lambda x: x,title="",color="Reds",decimals = 2):
    """
    ...
    ax = anotate_values2(ax,result,index,f_type,decimals)
    ax.set_title(title)
    ax.set_axis_off()
    ax.plot()
...
result['space'] = result['index'] + ' '
result['space'].unique()
plot_col_map(result,"Casos", "space", lambda x: int(x),"Número de casos por CCAA","Reds",0)
result['Casos_Población'] = result['Casos']/result['Población']
plot_col_map(result,"Recuperados","space", title="Número de recuperados",f_type=lambda x: int(x),color="YlGn",decimals=0)
result['Recuperados_Casos'] = (result['Recuperados']/result['Casos'])*100
plot_col_map(result,"Recuperados_Casos","space", title="Porcentaje de recuperados en cada CCAA",f_type=lambda x: "{} %".format(int(x)),color="YlGn",decimals=0)
plot_col_map(result,"Casos_Población", "space",title="Proporción de casos en relación con la población de cada región",f_type=lambda x: x,color="Reds",decimals=5)
result['Fallecidos_Casos'] = result['Fallecidos'] / result['Casos']
plot_col_map(result,"Fallecidos_Casos","space", title="Proporción de casos en relación con la población de cada región",f_type=lambda x: x,color="Reds",decimals=2)
df_groups = df.copy()
