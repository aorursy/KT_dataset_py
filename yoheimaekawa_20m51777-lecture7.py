...
fig.update_layout(title_text='Yohei Maekawa',title_x=0.5)
fig.show()
df = pd.read_csv('../input/nepal-earthquake/query.csv',header=0)

df.index = pd.to_datetime(df['time'])
df['time'] = df.index.strftime('%Y-%m-%H:00:00')
fig = px.scatter_geo(df,lat='latitude',lon='longitude',color='mag',animation_frame='time',color_continuous_scale='Rainbow',range_color=(4.,6.))
fig.show()