...
def my_k_means(dataSet, k):
    ...
    while True:
        ...
        for i in range(len(dataSet)):
            ...
            clusterAssmentN.append([distMin])

        if clusterAssmentN != clusterAssment:

            #改变centroids(迭代条件)

            for cent in range(k):

                ptsInClust = dataSet[np.nonzero(np.mat(clusterAssmentN).A == cent)[0]]  # get all the point in this cluster

                centroids[cent] = np.mean(ptsInClust, axis=0)  # assign centroid to mean

            centroids = np.array(centroids)

            clusterAssment = clusterAssmentN
