...
for i in [keras.optimizers.SGD(),keras.optimizers.adam(),keras.optimizers.Adamax()]:
    ...
    h = model.fit_generator(datagen.flow(X_train,Y_train, batch_size=batch_size),epochs = epochs, validation_data = (X_val,Y_val),verbose = 1, steps_per_epoch=X_train.shape[0] // batch_size, callbacks=[learning_rate_reduction],)
    ...
    loss_dict[str(str(i).split("keras.optimizers.")[1].split("()")[0])+"lr"]=h.history['lr']
