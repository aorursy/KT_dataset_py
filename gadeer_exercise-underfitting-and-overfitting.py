...
for leaf in candidate_max_leaf_nodes:
    ...
    if get_mae(leaf, train_X, val_X, train_y, val_y) < minimum:

        minimum = get_mae(leaf, train_X, val_X, train_y, val_y)
...
for i in  range(len(candidate_max_leaf_nodes)):
    ...
    my_dic[candidate_max_leaf_nodes[i]] =mae

# print(my_dic)

# print(sorted([ (v,k) for k,v in my_dic.items() ])[0][1])



# Store the best value of max_leaf_nodes (it will be either 5, 25, 50, 100, 250 or 500)

best_tree_size =sorted( [ (v,k) for k,v in my_dic.items() ] )[0][1]
