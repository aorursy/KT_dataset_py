...
df = pd.read_csv('../input/news-classification/fake_news_dataset.csv')
df.head()
len(df)
df = df.dropna(subset = ['type'])
df
cates = df.groupby('type')
...
# using 500 for padding length
maxlen = 500
X = list(sequence.pad_sequences(df.words, maxlen=maxlen))
# category to id
categories = df.groupby('type').size().index.tolist()
print(categories)
category_int = {}
int_category = {}
...
df['c2id'] = df['type'].apply(lambda x: category_int[x])
...
title = "intentional Christian community Archives â€“ Baptist News Global"
content = "Enderly Park is blistering under an unseasonable September heat, and Frank Byers saunters across Tuckaseegee Road to the rec center where he likes to play cards with his neighbors. He doesnâ€™t use the crosswalk, but in many ways heâ€™s earnedâ€¦"

text = content + " " + title

test = tokenizer.texts_to_sequences([text])
test = list(sequence.pad_sequences(test, maxlen=maxlen))
test = np.array(test)
pred1 = BiGRU.predict(test)
pred1 = np.round(np.argmax(pred1, axis=1)).astype(int)
int_category[pred1[0]]
title = "SWITZERLAND cracks down on Muslim â€œwelfare jihadistsâ€ â€“ economic migrants posing as refugees/asylum seekers"
content = "Many of the migrants entering Switzerland have also come without any form of identification, making it even more difficult to process their asylum claim or deport those not approved for asylum status. Simonetta Sommaruga revealed late last year that the government is unaware of the true identity of nine out of 10 asylum seekers in the country, mainly because they discard any documents they had, and try to claim to be refugees from Syria, which they believe will give them priority status. Swiss national councillor More about Islam and SWITZERLAND"
