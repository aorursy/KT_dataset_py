...
dfpew=pd.read_csv('/kaggle/input/pew-core-trends-2019/January 8-February 7 2019 - Core Trends Survey - CSV.csv') #load this csv into pandas dataframe
...
sk15=dfsp[dfsp['q15x']==' '].index #get the indexes (indicies?) for which column Q15 has value " ", name this output
dfS15=dfsp.drop(sk15) #Delete that output from dataFrame so we have no skipped responses for Q15
...
#repeat for Q17
sk17=dfhe[dfhe['q17a']==' ' or ['q17b']==' ' or ['q17c']==' 'or ['q17d']==' ' or ['q17e']==' ' or ['q17f']==' ' or ['q17g']==' 'or ['q17h']==' '].index
dfH17=dfhe.drop(sk17) #Delete that output from dataFrame so we have no skipped responses for Q17
print(dfHE17.shape)
...
print(fifS)
fifEp=fifE.drop(['Response_Ct'], axis=1)
fifSp=fifS.drop(['Response_Ct'], axis=1)
fifEp.Percent=pd.to_numeric(fifEp.Percent)
...
fifSp['Key'] = 's'# " "
DF = pd.concat([fifEp,fifSp],keys=['e','s']) #create a new pandas dataframe
DFGroup = DF.groupby(['Percent','Key'])
DFGroup.sum().unstack('Key').plot(kind='bar')


#df1['Key'] = 'df1'
#df2['Key'] = 'df2'
#DF = pd.concat([df1,df2],keys=['df1','df2'])
#DFGroup = DF.groupby(['Gender','Key'])
#DFGroup.sum().unstack('Key').plot(kind='bar')
#this attempt plots both columns, just to see if the data is plottable without getting the error in the code chunj
#I was able to plot this on my initial attempt, but after 30 minutes, I still get an error
...
#rects1=ax.bar(new15['En_Percent'])
#rects2=ax.bar(new15['Es_Percent'])
#def autolabel(rects):
    #for rect in rects:
...
#autolabel(rects1)
#autolabel(rect2)
#labels=['Very_Easy', 'Somewhat_Easy', 'Somewhat_Difficult', 'Very_Difficult', 'DK']
