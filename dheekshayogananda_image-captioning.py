...
os.listdir("../input/flickr8k/Flickr_Data/Flickr_Data/Images")
...
test_imgpath = '../input/testimage'
os.path.join(test_imgpath,test_pic)
test_pic = 'testimage.jpg'

def preprocess1(test_imgpath):
    # Convert all the images to size 299x299 as expected by the inception v3 model
    img = image.load_img('../input/testimage/testimage.jpg', target_size=(299, 299))
    # Convert PIL image to numpy array of 3-dimensions
    x = image.img_to_array(img)
    # Add one more dimension
    x = np.expand_dims(x, axis=0)
...
x = plt.imread(os.path.join('../input/testimage' ,test_pic))
plt.imshow(x)
plt.show()
print(greedy_search(img))
