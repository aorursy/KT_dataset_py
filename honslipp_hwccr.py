...
images_trn = np.load("../input/cchwdb/images48x48_trn.npy")

images_trn = 255-images_trn

labels_trn = np.load("../input/cchwdb/adjusted_dataset/adjusted_dataset/fill64x64/train_labels.npy")



images_tst = np.load("../input/cchwdb/images48x48_tst.npy")

images_tst = 255-images_tst

labels_tst = np.load("../input/cchwdb/labels_tst.npy")
# images_trn = images_trn[labels_trn<100]

# labels_trn = labels_trn[labels_trn<100]

# images_tst = images_tst[labels_tst<100]

# labels_tst = labels_tst[labels_tst<100]
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = self.transforms(self.images[index])
...
transforms = tv.transforms.Compose([tv.transforms.RandomHorizontalFlip(), 
...
#         self.conv1 = CrossConv2d(in_channels, out_channels, 3, 3, stride=stride)

#         self.conv2 = CrossConv2d(out_channels, out_channels, 3, 3)
...
class Attention(nn.Module):
    ...
    def forward(self, X):
        ...
        avg_channel = F.avg_pool2d(X, kernel_size=5, padding=2, stride=1)

        max_channel = F.max_pool2d(X, kernel_size=5, padding=2, stride=1)

        Y = self.conv(torch.cat([avg_channel, max_channel], dim=1))

        h, w = X.shape[2:]

        return F.softmax(Y.view(-1, 1, w*h), dim=2).view(Y.shape)*w*h
class VGG(nn.Module):

    def __init__(self, out_dim=3755):
...
#             nn.Dropout(0.5),
    ...
    def forward(self, X):
        ...
        features = self.conv(X*weight).view(X.shape[0], -1)
...
train_dataset = MyDataset(images_trn, labels_trn, transforms)

train_iter = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

test_dataset = MyDataset(images_tst, labels_tst, transforms)
...
# def train(net, optimizer, num_epoches, loss_trn, acc_trn, loss_tst, acc_tst, itv=1):
...
plt.legend()
plt.plot(acc_trn, label='train')

plt.plot(acc_tst, label='test')

plt.legend()
torch.save(net, 'VGG_att.pt')