...
files()
# read and store data
demo = read('demographic.csv')
hm = read('cleaned_hm.csv')
# merge two tables based on wid
data = hm.merge(demo, left_on='wid', right_on='wid', how='left')
head(data)
describe(data)
data = exclude(data, ['original_hm', 'modified','ground_truth_category','country','wid','num_sentence','hmid'] )
text = data['cleaned_hm']
head(text, 10)
cleaned_text_basic = clean_text(text, method = 'basic')
head(cleaned_text_basic, 4)
tallied = tally(cleaned_text_basic, multiple = True, min_count = 50, method = 'tfidf')
head(tallied, 5)
y = data['predicted_category']
X = exclude(data, 'predicted_category')
