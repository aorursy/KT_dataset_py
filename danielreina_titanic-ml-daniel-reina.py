...
df_scale[columns] = power_transform(df.iloc[:,2:9],method='yeo-johnson')
...
sns.distplot( df_scale["Pclass"], ax=axes[0,0])

sns.distplot( df_scale["Sex"], ax=axes[0,1])

sns.distplot( df_scale["Age"], ax=axes[0,2])

sns.distplot( df_scale["SibSp"], ax=axes[0,3])

sns.distplot( df_scale["Parch"], ax=axes[1,0])

sns.distplot( df_scale["Fare"], ax=axes[1,1])

sns.distplot( df_scale["option"], ax=axes[1,2])
...
tree_cv = RandomizedSearchCV(tree, param_dist, cv=3, scoring=scoring)
...
# New Instantiate the model

# adicionado - models.append(('KNN', KNeighborsClassifier()))
...
cv_auc = cross_val_score(kn, X, y, cv=5, scoring='roc_auc')
...
df_scale[columns] = power_transform(df.iloc[:,1:7],method='yeo-johnson')
...
X_pred = df_scale.iloc[:,1:9]

X_pred.info()

y_pred=kn.predict(X_pred)
...
Titanic_Submision = pd.DataFrame({"PassengerId":df.PassengerId, "Survived":y_pred})

Titanic_Submision.to_csv("Titanic_Submission.csv", index=False)
