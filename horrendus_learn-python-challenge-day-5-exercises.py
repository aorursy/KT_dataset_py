...
def estimate_average_slot_payout(n_runs):
    ...
    0.5
    """
    payouts = sum([(play_slot_machine() - 1) for i in range(n_runs)])
    return payouts / n_runs
...
def slots_survival_probability(start_balance, n_spins, n_simulations):
    ...
    for i in range(n_simulations):
        ...
        while balance >= 1.0 and spins < n_spins:
            spins += 1
            balance -= 1
            balance += play_slot_machine()
        survived.append(spins == n_spins)
        
    survived_simulations = sum([int(s) for s in survived])
    probability = survived_simulations / n_simulations
    return probability
