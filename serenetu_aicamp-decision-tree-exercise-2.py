...
def find_title(x):
    ...
    if len(xs) > 1:
        ...
        x1 = xs[0]
    return x1.split()[0]
data['name_title'] = data['name'].apply(find_title)
...
pd.DataFrame({'importance': model.feature_importances_, 'feature': trainX.columns}).sort_values(by='importance', ascending=False)[:20]
