...
for i in tqdm(range(100)):
  ...
  try:
    ...
    next
  if i < 10:
    train_idx_2m.append(idx)
    train_opt_2m.append(origin_seq)
    train_ipt_2m.append(non_acc_seq)
  elif i < 100:
    val_idx_100k.append(idx)
    val_opt_100k.append(origin_seq)
    val_ipt_100k.append(non_acc_seq)
  else:
    test_idx_100k.append(idx)
    test_opt_100k.append(origin_seq)
    test_ipt_100k.append(non_acc_seq)
import pickle
...
_save_pickle('train_tv_idx_2m.pkl', train_idx_2m)
_save_pickle('val_tv_idx_100k.pkl', val_idx_100k)
_save_pickle('test_tv_idx_100k.pkl', test_idx_100k)
...
train_examples = tf.data.Dataset.from_tensor_slices((train_ipt_2m, train_opt_2m))
val_examples = tf.data.Dataset.from_tensor_slices((val_ipt_100k, val_opt_100k))
max_sequence_length=500
def convert_lines(lines,vocab,bpe):
  ouputs=np.zeros((len(lines),max_sequence_length))
  cls_id=0
  eos_id=2
  pad_id=1
...
print('x1 tenor decode:',phoBERT.decode(torch.tensor(x1))[:103])
X = convert_lines(train_idx_2m, vocab, phoBERT.bpe)
print('X shape: ', X.shape)
print(X[0])
X1=torch.from_numpy(X).long()
all_layers = phoBERT.extract_features(X1, return_all_hiddens=False)
print(all_layers)
import pickle
...
def _load_pickle(path):
  with open(path, 'rb') as f:
    obj = pickle.load(f)
  return obj
_save_pickle('X1.pkl', X)
X = _load_pickle('./X1.pkl')
...
np_tensor = all_layers.detach().numpy()
ouput = tf.convert_to_tensor(np_tensor)
print( PhoBert_encode.shape)
...
class Transformer(tf.keras.Model):
  ...
  def call(self, inp, tar, training, enc_padding_mask, 
    ...
    enc_output = ouput
    #self.encoder(inp, training, enc_padding_mask)  # (batch_size, inp_seq_len, d_model)
