...
return_anamoly = stores_data.loc[ (stores_data['Sales Qty'] <= 0) & (stores_data['Discount'] != 0), ['SKU Code', 'Store Code','MRP', 'Sales Price', 'Discount', 'Sales Qty']]
...
def wavg(group, avg_name, weight_name):
    ...
    try:
        ...
        return 0
stores_data[stores_data['SKU Code'] == 'SKU0457']
stores_data[(stores_data['Sales Qty'] < 0) & (stores_data['Sale/Return'] == 'Sale')]
mrp_anamoly = stores_data[stores_data['MRP Anamoly']][['SKU Code', 'Store Code','MRP', 'Sales Price', 'Sales Qty']]

print(mrp_anamoly.shape)

print(mrp_anamoly.head())

print(mrp_anamoly.describe())
zero_data = stores_data.loc[(stores_data['MRP'] == 0) & (stores_data['Sales Price'] == 0), ['SKU Code', 'Store Code','MRP', 'Sales Price', 'Sales Qty']]

print(zero_data.shape)

print(zero_data.head())

print(zero_data.describe())
stores_data.describe()
negative_discount = stores_data.loc[ (stores_data['Discount Rate'] < 0) & (stores_data['Sales Anamoly']), ['SKU Code', 'Store Code','MRP', 'Sales Price', 'Sales Qty', 'Discount']]

print(negative_discount.shape)

print(negative_discount.head())

print(negative_discount.describe())
...
discounts_count_store_wise.sort_values(sort_by, ascending=False)
stores_data[stores_data['Discount'] == 0].shape
...
discounts_month_wise.to_csv('discounts_month_wise.csv')

discounts_month_wise
...
discounts_store_wise.to_csv('discounts_store_wise.csv')

discounts_store_wise
stores_data.groupby('Category')['Brand Code'].unique()
...
discounts_category_wise
stores_cat4_brand_wise_group = stores_data[(stores_data['Discount Rate'] > 0) & (stores_data['Category'] == 'Cat04')].groupby(['Store Code', 'Category', 'Brand Code'])

discounts_brand_cat4_store_wise = stores_cat4_brand_wise_group.agg(aggregator)

discounts_brand_cat4_store_wise['Weighted Average Discount'] = stores_cat4_brand_wise_group.apply(wavg, 'Discount', 'Sales Qty')

discounts_brand_cat4_store_wise
cat4_brand_groups = stores_data[(stores_data['Discount Rate'] > 0) & (stores_data['Category'] == 'Cat04')].groupby(['Category', 'Brand Code'])

discounts_brand_cat4_wise = cat4_brand_groups.agg(aggregator)

discounts_brand_cat4_wise['Weighted Average Discount'] = cat4_brand_groups.apply(wavg, 'Discount', 'Sales Qty')

discounts_brand_cat4_wise.sort_values('Weighted Average Discount', ascending=False)
...
discounts_brand_cat4_overall.sort_values('Weighted Average Discount', ascending=False)
...
sales_store_wise.sort_values(sort_by, ascending=False)
...
sales_store_category_wise.sort_values(sort_by, ascending=False)
category_grouped_sales = stores_data.groupby('Category')

category_wise_sales_price = category_grouped_sales.agg(aggregator2)

category_wise_sales_price['Weighted Sales Price Average'] = category_grouped_sales.apply(wavg, 'Sales Price', 'Sales Qty')

category_wise_sales_price.sort_values(sort_by, ascending=False)
...
sales_store_brand_wise.sort_values(sort_by_second, ascending=False)
brand_grouped_data = stores_data.groupby('Brand Code')

sales_brand_wise = brand_grouped_data.agg(aggregator2)

sales_brand_wise['Weighted Sales Price Average'] = brand_grouped_data.apply(wavg, 'Sales Price', 'Sales Qty')

sales_brand_wise.sort_values(sort_by_second, ascending=False)
margin = 0.5
...
brand_margin.sort_values(sort_by2, ascending=False)
stores_data.shape
for col in stores_data.columns:

    print('Column: ' + col)

    print(stores_data[col].unique().shape)