...
# Exercice : 
# 1. Can you print the number of patients using the functions len and print ?
# 2. Can you print the number of patients with cancer using the functions print and np.sum ? (don't look at next section)
# How many images ?
...
batch = 0 # Each batch plot 25 negative images on the left and 25 positive images on the right. You can change the value 0 to any value (10, 87 etc.)

for row in range(5): 
    plt.figure(figsize=(20, 10))
    for col in range(5):
        plt.subplot(1,11,col+1)
        plt.imshow(imgs0[25*batch + 5*row+col])
        plt.axis('off')
        
        plt.subplot(1,11,col+7)
        plt.imshow(imgs1[25*batch + 5*row+col])
        plt.axis('off')
...
model = PCA(n_components=2) # Exercice : try to replace PCA by SpectralEmbedding, TSNE !
...
print("before pca", resnet_features.shape)
# we use pca to reduce the dimensionality of the inputs
n_components_to_keep = 100
model = PCA()
resnet_features_pca = model.fit_transform(resnet_features)[:,:n_components_to_keep]
print("after pca", resnet_features_pca.shape)

# Nearest Neighbors
plot_accuracy(lambda k : KNeighborsClassifier(n_neighbors=k, n_jobs=8), resnet_features_pca, range(5,15))
...
# Exercice : can you run this code with resnet_features_pca ? Are results better ?  

from sklearn.linear_model import LogisticRegression
Y_pred_01 = (run_model(LogisticRegression(n_jobs=8, solver='lbfgs'), histogram_features) > 0.5).astype('int')
print('Accuracy : {:.2f}%'.format(100* np.mean(Y == Y_pred_01)))
# Random Forests
# Exercice : can you find the best depth of the tree ? Are Resnet features better than histogram features ? 

from sklearn.ensemble import RandomForestClassifier
Y_pred_01 = (run_model(RandomForestClassifier(n_jobs=8, n_estimators = 300, max_depth = 5), histogram_features) > 0.5).astype('int')
print('Accuracy : {:.2f}%'.format(100* np.mean(Y == Y_pred_01)))
import keras # We use keras library with tensorflow backend
