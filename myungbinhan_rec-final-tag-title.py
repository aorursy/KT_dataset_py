...
tag_and_title = df_ques[(df_ques['tag_len'] > 0) & (df_ques['title_len'] > 0)]
tag_and_title = tag_and_title[['tags', 'id', 'plylst_title', 'songs', 'like_cnt', 'updt_date']]
tag_and_title
...
df_total_2 = df_total_2[df_total_2['id'].isin(tag_and_title.id.to_list())].reset_index(drop=True)
df_total_2
...
for idx in tqdm(df.index[0 : total_length], desc='rec songs'):
    ...
    if len_cur_title != 0:
        # 태그, 타이틀로 각각 유사곡 찾아서 합!
        tag_rec_song_list = rec_songs_by_one_vector(sp_tag_scaled, cur_tag_list, cur_song_list, cur_updt_date)
        title_rec_song_list = rec_songs_by_one_vector(sp_title_scaled, cur_title_list, cur_song_list, cur_updt_date)
        rec_sum = rec_song_sum(tag_rec_song_list, title_rec_song_list, 1)
        rec_song_list = list(pd.unique(rec_song_list + rec_sum))[:100]
    else:
        # 태그로 찾고 땡
        rec_song_list = rec_songs_by_one_vector(sp_tag_scaled, cur_tag_list, cur_song_list, cur_updt_date, largest_n=100)
...
rate
df_ques[df_ques['id'] == 60710]['plylst_title']
# # b.iloc[9]['id']
