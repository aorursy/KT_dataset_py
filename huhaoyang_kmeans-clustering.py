...
def my_k_means(dataSet, k):
    ...
    for i in range(num):
        ...
        label = 0
        for j in range(k):
            temp = euclDistance(np.array(data2[i]),centroids[j])
            if (temp<dis):
                dis = temp
                label = j
        clusterAssment.append([label])
    ...
    for i in range(iteration):
        for j in range(k):
            ...
            for n in range(num):
                data3=np.array(data2[n])
                if (clusterAssment[n]==[j]):
                    x+=data3[0]
                    y+=data3[1]
                    num_label+=1
                    

            centroids_x=x/num_label
            centroids_y=y/num_label
            centroids[j]=np.array([centroids_x,centroids_y])
        ...
        for j in range(num):
            ...
            label = 0
            for n in range(k):
                temp = euclDistance(np.array(data2[j]),centroids[n])
                if (temp<dis):
                    dis = temp
                    label = n
            clusterAssment[j]=[label]
