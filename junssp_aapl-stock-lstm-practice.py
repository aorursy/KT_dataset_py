...
import os
for dirname, _, filenames in os.walk('kaggle/input/aaplstock.csv'):
    for filename in filenames:
        print(os.path.join(dirname, filename))
os.getcwd()
#Reading Tesla Stock Price Data Set
df_aapl=pd.read_csv('../input/aaplestock/AAPL (1).csv')
...
print(np.var(df_aapl.Close-df_aapl.Adj_Close))
#Drop Adj_Close column which is not required here.
df_aapl.drop('Adj_Close',axis=1,inplace=True)
# Verify columns
df_aapl.head(1)
...
plt.plot(df_aapl['Close'])                            # Plotting the close price
...
sns.regplot(x ='Open', y ='Close', data = df_aapl,marker='+',color="g") 
...
df_aapl_final=df_aapl[column_list]                                                                         
df_aapl_final.head(2)
df_aapl_final.info()
#Convert Date column data type from object to Date.
df_aapl_final['Date'] = pd.to_datetime(df_aapl_final['Date'])
df_aapl_final.info()
#Set index on Date Column
df_aapl_final = df_aapl_final.set_index("Date")
df_aapl_final.head(2)
# Given DataSet contains data from 29-Jun-2010 to 17-Mar-2017
df_aapl_final.tail(2)
...
prediction_days = 9800
ts_test= df_aapl_final[prediction_days:]       # load 30 days values to test by removing 1662 records.
ts_train= df_aapl_final[:prediction_days]      # load 1662 days values for train
ts_test
...
plt.plot(range(df_aapl_final.shape[0]),df_aapl_final,color='cyan', label='Test Data')
...
plt.plot(test_set[:, 0], color='red', label='Real AAPL Price')                      # Actual Price
plt.plot(predicted_price[:, 0], color = 'blue', label = 'Predicted Close Price')      # Predicted Price
plt.title('AAPL Price Prediction from 3-02-2017 to 13-03-2017', fontsize = 20)
plt.xlabel('Time', fontsize=40)
plt.ylabel('AAPL Price', fontsize = 40)
plt.legend(loc = 'best')
plt.show()
# Predictions on ts_test
# Preprocess the test data
...
test1 = np.array(341.5849)
test1 = test1.reshape(-1, 1)
inputs = min_max_scaler.transform(test1)                                # Scaling the data
inputs = np.reshape(inputs, (len(inputs), 1, 1))
