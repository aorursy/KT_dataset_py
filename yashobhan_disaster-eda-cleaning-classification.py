...
generate_ngrams('have you heard about the bird?')
N = 100
...
df_nondisaster_unigrams = pd.DataFrame(sorted(nodisaster_unigrams.items(), key=lambda x: x[1])[::-1])
# Bigrams
...
df_mislabeled = df_mislabeled[df_mislabeled['target'] > 1]['text']
df_mislabeled
...
print('Training set keyword count: ', train_df['keyword'].nunique())
for fold, (train_idx, val_idx) in enumerate(skf.split(train_df['text_cleaned'], train_df['target']), 1):

    print('Fold {} training shape: {}, Validation shape: {}'.format(fold, train_df.loc[train_idx, 'text_cleaned'].shape, train_df.loc[val_idx, 'text_cleaned'].shape))
...
class DisasterDetector:
    ...
    def predict(self, X):
        ...
        return np.argmax(self.models[0].predict(X_padded))
disaster = DisasterDetector()
%%time

disaster.train(train_df, glove_embeddings)
