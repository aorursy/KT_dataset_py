...
# funtools - используется для функций высшего порядка, которые возвращают другие функции
...
# проверим, что у нас внутри tv_X_train
...
train_dataset, valid_dataset, test_dataset = split_train_valid_test(dataset, valid_ratio=0.4, test_ratio=0.0)
...
# строим нейронную сеть (Вариант 1 - Простая линейная сеть с нормализованным TF-IDF)
...
def train_epoch(model, optimizer, train_loader):
    ...
    for tfidf, target in train_loader:
        ...
        # Выполняем gradient descent, backward проход
...
def validate_epoch(model, valid_loader):
    ...
    with torch.no_grad():
        ...
        for tfidf, target in valid_loader:
            ...
            # Считаем на сколько модель ошибается
...
for epoch_num in range(max_epochs):
    ...
    if len(valid_losses) > 10 and all(valid_loss >= loss for loss in valid_losses[-11:]):
...
for i, fields in enumerate(test_loader):
    ...
    if i == batch_idx:
        ...
        print('Случайный таргет: ', target[example_idx], '\n')
# Эффективность Pytorch модели на TFIDF
