...
def insert_labels(subplot, prefixo='', sufixo=''): 
    ...
    for i in subplot.patches:
        ...
        subplot.text(i.get_width(),
                     ...
                     fontsize=12)
df = pd.read_csv('../input/datasetcerto/datasetV0.4.csv')
...
pesos_df = pd.read_csv("../input/datasetcerto/datasetV0.4_pesos.csv")
...
texts = [plt.text(grade_per_price['price/user'][i], grade_per_price['percent_grade'][i], grade_per_price['platforms'][i], ha='center', va='center') for i in range(len(grade_per_price['platforms']))]
