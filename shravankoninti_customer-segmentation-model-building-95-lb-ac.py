...
test_df['is_train'] = 0
# Find common Ids in both train and test
...
print(test_df.shape, test_df_leak.shape)
# separate non-leak ids

#Remove leaked_ids

test_df_non_leak = test_df[~test_df['ID'].isin(common_ids)]

test_df_non_leak.head()
test_df_leak_predictions = train_df[train_df['ID'].isin(common_ids)]

test_df_leak_predictions[['ID', 'Segmentation']].head()
test_df_leak_predictions[['ID', 'Segmentation']].shape
...
create_download_link(test_df_leak_predictions)
...
cat_preds = model.predict(test_X)
np.unique(cat_preds, return_counts=True)
sub_df['Segmentation'] = cat_preds
