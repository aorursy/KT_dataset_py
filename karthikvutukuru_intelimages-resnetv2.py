...
def loadBatchImages(path, nSamples, nVal):
    ...
    imagesTrain, labelsTrain, imagesVal, labelsVal = [], [], [], []

    for category in categoriesList[0:256]:
        ...
        for image in imageList[0:nSamples + nVal]:
            ...
            # img.resize((224,224)) # - Use this if VGG is the model

            if idx < nSamples:

                labelsTrain.append(int(image[0:3])-1)

                imagesTrain.append(img)

            else:

                labelsVal.append(int(image[0:3])-1)
...
train = shuffleData(np.asarray(data),labels)
...
multipleImages = glob('../input/caltech256/256_objectcategories/256_ObjectCategories/009.bear/**')
...
if version == 2:
    ...
    model = resnet_v2(input_shape=(256, 256,3), depth=depth, num_classes=257)

else:

    model = resnet_v1(input_shape=(input_shape,3),  depth=depth)
