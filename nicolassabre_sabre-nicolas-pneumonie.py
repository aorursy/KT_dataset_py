...
for i in range(0,49) :
    ...
    plt.imshow(X[n_pneumonie+i])

    plt.title('Label: %i' % y[n_pneumonie+i])
X = np.array(X)
...
model.add(MaxPooling2D(pool_size=(2, 2)))#divise taille des images par 2

model.add(Dropout(0.2))#eviter le sur-apprentissage en mettant au repos 20% des neurones a chaque fois (même si pas sur-apprentissage ici)
