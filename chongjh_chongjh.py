...
#xtable is the diamond table length

#table is the ratio of "the table length" divide by "the diamond length, x " 



xtable = (diamonds["table"]) * (diamonds["x"]) / 100



diamonds["xtable"] = xtable
...
train_r2 = model.score(X_polytrain,y_train) #R^2 score

pred = model.predict(X_polytrain)
...
test_r2 = model.score(X_polytest,y_test) # R^2 score

pred_test = model.predict(X_polytest)
...
xtable = (diamonds_submission["table"]) * (diamonds_submission["x"]) / 100



diamonds_submission["xtable"] = xtable
...
diamonds_submission_ready_1 = poly.fit_transform(diamonds_submission_ready)

diamonds_submission_pred = model.predict(diamonds_submission_ready_1)
...
price_submission.to_csv("chong12.csv", index=False)