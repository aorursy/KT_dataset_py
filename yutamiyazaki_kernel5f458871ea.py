...
print("尖度: %f" % train['SalePrice'].skew())
#物件の広さを合計したTotalSFを定義
...
train = train.drop(train[(train['TotalSF']>7500) & (train['SalePrice']<250000)].index)
...
train = train.drop(train[(train['YearBuilt']<2000) & (train['SalePrice']>550000)].index)
...
train = train.drop(train[(train['GarageCars']<3) & (train['SalePrice']>200000)].index)

train = train.drop(train[(train['GarageCars']<5) & (train['SalePrice']>500000)].index)
