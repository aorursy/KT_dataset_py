...
def set_missing_ages(df):
    ...
    print(age_df.shape)
    # 乘客分成已知年龄和未知年龄两部分
    tmp = age_df[age_df.Age.isnull()]
    print(tmp.shape)
    unknown_age = tmp.as_matrix()
...
df['Fare_scaled'] = scaler.fit_transform(df_Fare, age_scale_param)
df.head()
# 训练模型
...
def plot_learning_curve(estimator, title, X, y, ylim=None, cv=None, n_jobs=1, 
    ...
    if plot:
        ...
        plt.fill_between(train_sizes, test_scores_mean - test_scores_std, test_scores_mean + test_scores_std, 
                         alpha=0.1, color="r")
        plt.plot(train_sizes, train_scores_mean, 'o-', color="b", label=u"scores on training set")
        plt.plot(train_sizes, test_scores_mean, 'o-', color="r", label=u"scores on cv set")
...
X = all_data.values[:,1:]
y = all_data.values[:,0]
scores = cross_validation.cross_val_score(bagging_clf, X, y, cv=5)
bagging_clf.fit(X,y)
print(bagging_clf.score)
print(scores)