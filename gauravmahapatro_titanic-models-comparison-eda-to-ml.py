...
age_null=df_titanic_mod['Age'].isnull()
average_age=df_titanic_mod[~age_null]['Age'].sum()/df_titanic_mod[~age_null].shape[0]
...
df_titanic_mod.head()
df_titanic_mod['Title'].value_counts()
...
features=['Pclass', 'Sex', 'Title', 'Age', 'Fare', 'Parch', 'SibSp', 'Cabin_NA', 'Tkt_lvl_Ordinary']
...
model_names = ['Decision Tree', 'Logit Reg', 'SVM Classifer', 'Random Forest', 'Gradient Boosting', 'AdaBoost', 'XGB']
models = [DTree, logit_clf, SVM_clf, Rforest_clf, GB_clf, AdaB_clf, XGBoost_clf]
AUC_inf = [DT_ROC, Log_ROC, SVM_ROC, RF_ROC, GB_ROC, AdaB_ROC, XGB_ROC ]
