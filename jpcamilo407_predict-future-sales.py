# Loading Libraries
# Pendiente comentariar cada import
import numpy as np
import pandas as pd
...
# Some basic information of train data
...
train.head()
## Some basic information of test data
print ('size of test: ', test.shape)
test.head()
# Some basic information of item data
print ('number of categories: ', items['item_category_id'].max()) 
print('missing Values:\n', items.isnull().sum())
print('null values:\n', items.isna().sum())
items.head()
# Some basic information of submission example
print ('size of submission of example: ', submission.shape)
submission.head()
# To better verify the data we change the characteristics of date to a date with year month and day
...
train.head()
# graph of counting months for the year
grouped = pd.DataFrame(train.groupby(['year','month'])
...
# item_price for the month and year
grouped_price = pd.DataFrame(train.groupby(['year','month'])
...
sns.jointplot(x='item_price',y='item_cnt_day',data=train,height=10)
plt.show()
# I make a count of the 'item_id' from highest to lowest
train['item_id'].value_counts(ascending=False)[:5]
# look for item 20949 to see what it is
items.loc[items['item_id']==20949]
# I look for the item in the category data
item_cats.loc[item_cats['item_category_id']==71]
# now filter on test data by item_id 20949
test.loc[test['item_id']==20949].head(5)
# now count values are organized for days
train['item_cnt_day'].sort_values(ascending=False)[:5]
# busco el conteo de la busqueda anterior
train[train['item_cnt_day'] == 2169]
# look for item_id 11373
items[items['item_id'] == 11373]
# I look for the number 11373 in the training date
train[train['item_id'] == 11373]
# Due to the previous analysis we will remove the outlier found
# and as we saw previously we put a cap of 2000
train = train[train['item_cnt_day'] < 2000]
# we verify that the column is now consistent
train['item_cnt_day'].sort_values(ascending=False)[:5]
# we order the column 'item price'
train['item_price'].sort_values(ascending=False)[:5]
# We look again for this value in the train data
train[train['item_price'] == 307980]
# We search the item data for the value
items[items['item_id'] == 6066]
# I look for this value in the training data in case it happens more times
train[train['item_id'] == 6066]
# as with the previous anomaly we remove that record
train = train[train['item_price'] < 300000]
train['item_price'].sort_values(ascending=False)[:5]
# We perform the same search on the 'item_price' feature
train['item_price'].sort_values()[:5]
# We look for the item to which it belongs
train[train['item_price'] == -1]
# knowing the item id of the product we look for other transactions
train[train['item_id'] == 2973]
# I take the average and change it in the training date
...
# I get the total number of stores found in the train and test data
...
# preview of the first elemts
shops.head(5)
# fixing up the features
shops.loc[shops.shop_name == 'Сергиев Посад ТЦ "7Я"', 'shop_name'] = 'СергиевПосад ТЦ "7Я"'
shops['city'] = shops['shop_name'].str.split(' ').map(lambda x: x[0])
shops.loc[shops.city == '!Якутск', 'city'] = 'Якутск'
shops['city_code'] = LabelEncoder().fit_transform(shops['city'])
shops = shops[['shop_id','city_code']]
# fixing up the categories data
...
time.time() - ts
# product of item_price multiply item_cnt_day 
...
print('-Save predict')
submission.to_csv('submissionJC_XGBOOST.csv', index=False)
print('output file :','submissionJC_XGBOOST','saved')