...
cols = ['instant','season', 'yr', 'mnth', 'holiday', 'weekday', 'temp', 'atemp', 'hum', 'windspeed', 'casual','registered',
       'cnt']
bike.drop(cols, axis = 1, inplace = True)
bike.head()
# Sort the Order Date 
bike = bike.sort_values('dteday')
...
bike.groupby('dteday')['weathersit'].sum().reset_index()

# min and max values of Order Date
print(bike['dteday'].min())
print(bike['dteday'].max())
#set 'Order Date' as index
bike = bike.set_index('dteday')
bike.index
# average the daily sales value for each month 
# use start of each month as the timestamp
bike.index = pd.to_datetime(bike.index)
y = bike['workingday'].resample('MS').mean()
y['2017':]
y.plot(figsize = (15, 6))
plt.show()
