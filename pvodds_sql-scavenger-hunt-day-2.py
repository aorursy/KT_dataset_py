...
query2 = """SELECT Type, COUNT(id)
            ...
            GROUP BY Type
        """
type_of_stories = hacker_news.query_to_pandas_safe(query2)
...
query4 = """SELECT author, SUM(ranking)
            FROM `bigquery-public-data.hacker_news.comments`
            GROUP BY author
        """
maximum_ranking_author = hacker_news.query_to_pandas_safe(query4)
maximum_ranking_author.head()