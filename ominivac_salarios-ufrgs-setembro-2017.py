...
teto = 25322.25
df = pd.read_csv("../input/servidores_v4_todos_UFRGS_2017.csv",  sep='|', thousands = '.' , decimal = ',',  names = colunas)
len(df)
df.shape
df.dtypes
df.loc[ df.sal_liq == 0 ]
...
result_maiores_per_sal_liq.to_html('maiores_sal_liq_por_cargo_UFRGS_SET.html')

result_maiores_per_sal_liq
result_maiores_per_sal_bruto = df.loc[df.groupby('cargo').sal_bruto.agg('idxmax')]

result_maiores_per_sal_bruto.to_html('maiores_sal_bruto_por_cargo_UFRGS_SET.html')
...
result_menores_per_sal_liq.to_html('menores_sal_bruto_por_cargo_UFRGS_SET.html')

result_menores_per_sal_liq
result_menores_per_sal_bruto = df.loc[df.groupby('cargo').sal_bruto.agg('idxmin')]

result_menores_per_sal_bruto.to_html('menores_sal_bruto_por_cargo_UFRGS_SET.html')
...
dat_resul.to_html("mediana_salarial_porCargo-ufrgs_set.html")
resul_med
type(resul_med)
...
df_adm.head(10)
maskara_liquido_menores = (df_adm['sal_liq'] < 5024.645)

menores_adm = df_adm.loc[maskara_liquido_menores]
menores_adm
len(menores_adm)
maskara_liquido = (df_adm['sal_liq'] > 5024.645)
...
df_contador
len(df_contador)
maskara_contador_liq_menos = (df_contador['sal_liq'] < 5542.545)

menores_contador = df_contador.loc[maskara_contador_liq_menos]

menores_contador
len(menores_contador)
maskara_contador_liq_mais = (df_contador['sal_liq'] > 5542.545)
...
df_prof_magisterio
maskara_prof_liq_mais = (df_prof_magisterio['sal_liq'] > 9295.26)

maiores_professor = df_prof_magisterio.loc[maskara_prof_liq_mais]
maskara_prof_liq_menos = (df_prof_magisterio['sal_liq'] < 9295.26)
