...
df.head()
data_train = df.sample(frac=0.8, random_state=2)          # 80% des données avec frac=0.8
data_test = df.drop(data_train.index)     # le reste des données pour le test
X_train = data_train.drop(['diagnosis'], axis=1)
y_train = data_train['diagnosis']
X_test = data_test.drop(['diagnosis'], axis=1)
...
sns.jointplot("petal_length", "petal_width", df, kind='kde');
...
sns.kdeplot(df[versicolor].sepal_length, df[versicolor].sepal_width, cmap = "Blues", shade=True, alpha=0.3, shade_lowest=False)
plt.figure(figsize = (6,6))
...
sns.kdeplot(df[versicolor].petal_length, df[versicolor].petal_width, cmap = "Blues", shade=True, alpha=0.3, shade_lowest=False)
