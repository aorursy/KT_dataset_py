...
sf_total_missing = sf_missing_values_count.sum() # first sum gives total by columns, second sum gives total null cells
(sf_total_missing / sf_total_cells) * 100.0
# look at the # of missing points in the first ten columns
missing_values_count[0:10]
sf_missing_values_count.loc[['Street Number Suffix', 'Zipcode']]
...
"(rows, cols): {0}".format(sf_permits.dropna().shape)
# Now try removing all the columns with empty values. Now how much of your data is left?
"(rows, cols): {0}".format(sf_permits.dropna(axis=1).shape)
