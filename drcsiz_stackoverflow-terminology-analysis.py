...
(
  ...
  ON q.id = c.post_id
  WHERE EXTRACT(YEAR FROM q.creation_date) BETWEEN 2014 AND 2020
  AND q.body LIKE '% home router%'
);
"""

matchingResponse = stackOverflow.query_to_pandas_safe(matchingQuery, max_gb_scanned=70)
print(len(matchingResponse))
#matchingResponse.head(10)
matchResponsesAsArray = matchingResponse.values
for item in matchResponsesAsArray:
    print(item)
wifiQuery = """SELECT question_id, creation_date, question_body, answer_body, comment
FROM
(
  SELECT
    q.body AS question_body,
  ...
  JOIN
    `bigquery-public-data.stackoverflow.comments` c
  ON q.id = c.post_id
  WHERE EXTRACT(YEAR FROM q.creation_date) BETWEEN 2019 AND 2020
  AND lower(q.body) LIKE '% wifi%'
);
"""

wifiResponse = stackOverflow.query_to_pandas_safe(wifiQuery, max_gb_scanned=70)
wifiResponse.to_csv('/kaggle/working/results.csv', index=False)