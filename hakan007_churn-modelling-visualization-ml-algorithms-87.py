...
sns.heatmap(df.drop(["RowNumber",'CustomerId'],axis = 1).corr(),robust=True,fmt='.1g',linewidths=1.3,linecolor = 'gold', annot=True,);
...
fig.show(); 
print(df.groupby("Geography")["CreditScore"].mean()) ### Cikarilabilir
...
sns.factorplot(x = "new_credit", y = "Exited", data = df,kind = "bar")
...
cat_score = accuracy_score(y_test,y_pred)*100
