...
def first_and_last(sequence):
    ...
    count = len(sequence)
    newlist.append(sequence[count-1])
...
def shortest_string(strlst):
    shortest = min(len(x) for x in strlst)
    for x in strlst:
        if len(x) == shortest:
            return x
        else:
...
def get_unique_values(collection):
...
def get_unique_values_from_two_lists(collection1, collection2):
    newlist = collection1 + collection2
    return set(newlist)
...
def lowest_price_book(dictionaries):
    lowest = {"title": "", "author": "", "price": float('inf')}
    for x in dictionaries:
        lowest = x
        pass
...
def number_of_item_types(shoplist):
    
    count = len(shoplist["items"])
    return count
...
def total_number_of_items(shoplist):
...
def get_average_item_price(shoplist):
    ...
    #find total price spent on items
    money_spent = 0
    for x in shopping_cart["items"]:
        items_cost = x["price"]
        money_spent = money_spent + items_cost

    avg_item_price = money_spent/total_items
...
def get_average_spent_per_item(shoplist):
    
    #counting total items or quantity in cart
    total_quantity = 0
    for x in shoplist["items"]:
        amount = x["quantity"] 
        total_quantity = total_quantity + amount
        
    #counting total price spent
    total_price = 0
    for x in shoplist["items"]:
        price = x["price"] * x["quantity"]
        total_price += price
        
    avg_spent = total_price/total_quantity
    return avg_spent
...
def most_spent_on_item(shoplist):
    #empty dictionary
    most_spent = {"title": "", "price": 0, "quantity": 0}
    
    #for loop to compare
    for x in shoplist["items"]:
        
        #check price spent on item currently in new dictionary most_spent
        most_spent_price = most_spent["price"] * most_spent["quantity"]
        
        #check price of current for loop iterary
        new_spent_price = x["price"] * x["quantity"]
        
        #if loop to compare price
        if new_spent_price > most_spent_price:
            most_spent["title"] = x["title"]
            most_spent["price"] = x["price"]
            most_spent["quantity"] = x["quantity"]
