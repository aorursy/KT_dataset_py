...
class my_gbdt(object):
    ...
    def __init__(self):
        ...
        for i in range(self.iterations):
            ...
            self.iter_trees.append(DecisionTreeRegressor(min_samples_split=self.min_samples_split, max_depth=self.max_depth))
    ...
    def fit(self, X, y):
        ...
        self.iter_trees[0].fit(X, y)

        prediction = self.iter_trees[0].predict(X)

        for i in range(1, self.iterations):

            self.iter_trees[i].fit(X, (y-prediction) * self.lr)

            prediction += self.iter_trees[i].predict(X)
    ...
    def predict(self, X):
        ...
        for i in range(1, self.iterations):
           ...
           prediction += self.lr * self.iter_trees[i].predict(X)
