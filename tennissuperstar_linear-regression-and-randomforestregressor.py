...
#skewed_feats = train[numeric_feats].apply(lambda x: skew(numeric_feats)) #compute skewness
...
#print(result.predict(local_testX))
from sklearn.ensemble import RandomForestRegressor

clf = RandomForestRegressor(n_estimators=500, n_jobs=-1)

#randomForestResult = clf.fit(X, Y)
from sklearn.cross_validation import train_test_split
...
randomForestResult = clf.fit(X_train1, y_train1)



predictions = np.log(randomForestResult.predict(local_testX) + 1)
...
print(randomForestResult.predict(local_testX))

#y_pred = clf.predict(X_test1)

#predictions = np.log(y_pred + 1)
