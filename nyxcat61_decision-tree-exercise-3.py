...
def count_errors(labels_in_node):
    ...
    positive_ones = (labels_in_node.values == +1).sum()

    negative_ones = (labels_in_node.values == -1).sum()
...
def best_split(data, features, target):
    ...
    for feature in features:
        ...
        error = (left_misses + right_misses) / num_data_points * 1.0
...
class MyDecisionTree(BaseEstimator):
    ...
    def __init__(self, max_depth, min_error, criterion='entropy'):
    ...
    def create_tree(self, data, features, target, current_depth = 0, max_depth = 10, min_error=0):
        ...
        if self.criterion == 'entropy':
            ...
            raise ValueError("criterion can only be entropy or error")
    ...
    def create_leaf(self, target_values):
        ...
        num_positive_ones = (target_values.values == +1).sum()

        num_negative_ones = (target_values.values == -1).sum()
...
m.fit(trainX, trainY)
m.score(testX, testY)
m_error = MyDecisionTree(max_depth = 10, min_error = 1e-15, criterion='error')

m_error.fit(trainX, trainY)
m_error.score(testX, testY)
m.count_leaves()
