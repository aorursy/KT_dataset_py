...
train.head()
train[['AgeBand']].groupby(train['AgeBand']).count()
# AgeBand別の生存率
g = sns.factorplot(x="AgeBand", y="Survived", hue="Sex", size=6, data=train, kind="bar")
train['FareBand'] = pd.qcut(train['Fare'], 4)
test['FareBand'] = pd.qcut(test['Fare'], 4)
train.head()

train[['FareBand']].groupby(train['FareBand']).count()
g = sns.factorplot(x="FareBand", y="Survived", hue="Sex", size=6, data=train, kind="bar")
...
le = LabelEncoder()
train['Age_cat'] = le.fit_transform(train['AgeBand'])
train['Fare_cat'] = le.fit_transform(train['FareBand'])
test['Age_cat'] = le.fit_transform(test['AgeBand'])
test['Fare_cat'] = le.fit_transform(test['FareBand'])
train.head()
...
#不要カラム削除
train = train.drop(["PassengerId", "Name", "Ticket", "Cabin","AgeBand","FareBand","Age","Fare"],axis=1)
test = test.drop(["PassengerId", "Name", "Ticket", "Cabin","AgeBand","FareBand","Age","Fare"],axis=1)
test.head()
...
# 係数を見やすく表示
print('係数： {}'.format(pd.DataFrame({"Name":X_train.columns,"Coefficients":lr.coef_[0]}).sort_values(by='Coefficients')))

# 各オッズ比を確認
fig = plt.figure(figsize=(7, 7))

odds_ratio = np.exp(lr.coef_[0])
plt.barh(y=X_train.columns, width=odds_ratio)
